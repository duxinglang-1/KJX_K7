...
#define  OSC_U32ID_ENHMUTEX               0x86868686
...
//Enhmutex only: start from 0x00010000
#define  OSC_U32FLAG_ENHMUTEX_MAX_WPRIO           0x00010000
#define  OSC_U32FLAG_ENHMUTEX_SUSPENSION_COUNT    0x00020000
#define  OSC_U32FLAG_ENHMUTEX_OWNER_THREAD        0x00040000
...
typedef enum
{
   ...
   OSC_ISR_SRC_PLUGIN,							//Requested by XiaoWu for MoDIS Plug In
   //mtk user add end
...
typedef enum
{
   ...
   OSC_STATE_SUSPEND_ENHMUTEX,    //thread only
...
#define OSC_TIMEOUT_NONE    0
#define OSC_TIMEOUT_LOCK    1
#define OSC_TIMEOUT_ABANDON 2
#define OSC_TIMEOUT_FOREVER 0xFFFFFFFF //this is also "INFINITE" in windows
typedef unsigned int osc_enum_timeout;
...
typedef struct _osc_struct_timer
{
   ...
   osc_enum_timer_type			timer_type;
...
typedef struct _osc_struct_thread
{
   ...
   osc_type_func_ptr          cleanup;      /* Clean-up routine       */
   osc_type_void_ptr          cleanup_info; /* Clean-up information   */
   osc_type_u32				  timer_active;
   struct _osc_struct_timer   timer_control;
   struct _osc_struct_dd_hdr  *timer;
   osc_type_void_ptr          waiting_list;
   osc_type_u32				  activation_count;
   osc_type_u32				  enhmutex_count;
   osc_type_void_ptr          extend;
}  osc_struct_thread;
...
/* ---------------------------------------------------
Enh_mutex control structure
----------------------------------------------------*/
typedef struct _osc_struct_enhmutex_suspend {
    /* circular linked-list pointer */
    /* emts_prev, emts_next must be the first 2 elements */
    /* and in the correct order with struct oscar_enhmutex */
    struct _osc_struct_enhmutex_suspend *emts_prev;
    struct _osc_struct_enhmutex_suspend *emts_next;
    
    struct _osc_struct_enhmutex *emts_enhmutex;
    struct _osc_struct_thread  *emts_thread;
}  osc_struct_enhmutex_suspend;

typedef struct _osc_struct_enhmutex
{
    osc_type_u32               guard;
    osc_type_u8				  name[OSC_CONST_NAME_LENGTH];
    osc_type_u32    emtx_max_wprio;           // maximum(highest) waiting task priority
    osc_type_u32    emtx_suspension_count;    // how many task suspended on it
    struct _osc_struct_thread  *emtx_owner_thread;
    struct _osc_struct_enhmutex_suspend *waiting_list;
    osc_type_void_ptr          extend;
}  osc_struct_enhmutex;
...
typedef struct _osc_struct_event_waiting
{
   ...
   struct _osc_struct_event   *event_group;
   osc_type_u32               event_need;
   osc_type_u32               event_actual;
   osc_type_u32               action;
   osc_type_s32			   event_return_status;	  /* Return status			*/
   osc_type_void_ptr          extend;
