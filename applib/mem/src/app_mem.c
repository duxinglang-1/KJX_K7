...
#if (!defined(__ASM_CACHE_PROMOTION__) && defined(__MMI_REDUCED_UI_BUFFER__) && defined(__MED_IN_ASM__) &&!defined(__MMI_USE_MMV2__))
...
#define ALLOCK_TYPE_MASK        0xF0000000
...
typedef struct
{
    ...
    **************************************************************************/
	...
	ADM_MB_HEAD * block_layout_array[120];
	kal_uint32 blocks_size_info[120];
	kal_uint32 blocks_count;
	kal_uint32 blocks_analyze;
	void* pre_alloc_list[60];
#endif //__ASM_CACHE_PROMOTION__
    /**************************************************************************
     * cache *
    **************************************************************************/
                              ...
                              asm_alloc_flag alloc_flag, kal_char *file, kal_uint32 line, kal_bool is_anonymous)
{
    ...
    {	
	#if defined(__ASM_CACHE_PROMOTION__)
		chunk = applib_smart_alloc(chunk_size, alloc_flag, ALLOCK_TYPE_ANONYMOUS);
	#else
        if (is_fb)
        {
//            kal_uint32 align_size = __MMI_GDI_LAYER_FRAMEBUF_ALIGNMENT__;
    ...
    {
		#if defined(__ASM_CACHE_PROMOTION__)
		chunk = applib_smart_alloc(chunk_size, alloc_flag, ALLOCK_TYPE_APP);
		#else
        if (is_fb)
        {
//            kal_uint32 align_size = __MMI_GDI_LAYER_FRAMEBUF_ALIGNMENT__;
...
#define BLOCK_FREE_CACHE(index) applib_free_cache_by_index(index)
...
static void applib_pre_alloc_to_index(kal_uint32 index, kal_bool is_topmost_dir, kal_uint32 size)
{
	...
	if(is_topmost_dir)
	{
		while(g_applib_mem_cntx.pre_alloc_list[pre_alloc_count] != NULL)
			pre_alloc_count++;
		temp_size = size;
		temp_index = applib_get_first_block(&temp_size, is_topmost_dir, ALLOCK_TYPE_FREE);
...
//			kal_prompt_trace(MOD_MMI,"[ASM] pre alloc = %d", pre_alloc_count - 1);
	...
	else
	{
		...
		for(; free_pre_count < 60; free_pre_count--)
		{
...
static void applib_free_cache_by_index(kal_uint32 index)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	if(BLOCK_TYPE(index) == ALLOCK_TYPE_CACHE)
	{
...
static kal_uint32 applib_get_max_free_size(kal_uint32 from, kal_uint32 to)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	if (from <= to && to < g_applib_mem_cntx.blocks_count)
	{
		for (i = from; i <= to; i++)
		{
			info = g_applib_mem_cntx.blocks_size_info[i];
			if ((info&ALLOCK_TYPE_MASK) == ALLOCK_TYPE_FREE)
			{
...
static kal_bool applib_pre_alloc_start(kal_uint32 mem_size, kal_uint32 analizeType)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	kal_uint32 i;
	void** pre_list = g_applib_mem_cntx.pre_alloc_list;
	kal_bool dir;
	kal_uint32 from, to, size;
    kal_uint32 count = g_applib_mem_cntx.blocks_count;
	
	kal_uint32* analyze = &(g_applib_mem_cntx.blocks_analyze);
	kal_uint32 last_app = ((*analyze) >> 24);
	kal_uint32 first_cache = (((*analyze) >> 16) & 0xFF);
	kal_uint32 last_cache = (((*analyze) >> 8) & 0xFF);
	kal_uint32 first_anonymous = ((*analyze) & 0xFF);
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	...
	switch(analizeType)
	{
		...
		{			
			size = mem_size;
			to = applib_get_first_block(&size, KAL_FALSE, ALLOCK_TYPE_CACHE | ALLOCK_TYPE_FREE);
			for (i = 0; i <= to; i++)
				BLOCK_FREE_CACHE(i);			
			...
			else if((to&0xFF) <= last_cache && last_cache != 0xFF)
			{				
				applib_pre_alloc_to_index(last_cache+1 , KAL_TRUE, mem_size);
			}
			else if((to&0xFF) <= first_anonymous && first_anonymous != 0xFF)
			{
				applib_pre_alloc_to_index(first_anonymous+1 , KAL_TRUE, mem_size);
			}
			else
			{
		...
		{
			size = mem_size;
			from = applib_get_first_block(&size, KAL_TRUE, ALLOCK_TYPE_CACHE | ALLOCK_TYPE_FREE);
...
//	kal_prompt_trace(MOD_MMI,"[ASM] enter applib_smart_alloc(size = %d, type = %x", mem_size, analizeType);
	...
	memset(g_applib_mem_cntx.block_layout_array, 0, 120*sizeof(kal_uint32));
	memset(g_applib_mem_cntx.blocks_size_info, 0, 120*sizeof(kal_uint32));
	memset(g_applib_mem_cntx.pre_alloc_list, 0, 60*sizeof(kal_uint32));
	
	g_applib_mem_cntx.blocks_analyze = 0x0FFFFFFFF;
	kal_adm_traverse((KAL_ADM_ID)g_applib_mem_cntx.app_pool_id, KAL_ADM_TRAVERSE_NO_CONCHECK, applib_adm_layout, NULL);
	is_topmost = applib_pre_alloc_start(mem_size, analizeType);
	if(is_topmost)
	{
...
static kal_bool applib_adm_layout(KAL_ADM_ID adm_id, kal_uint32 flags, ADM_MB_HEAD *adm_mb_head, void *user_data)
{
    ...
    if ((flags & KAL_ADM_MB_INUSE) == KAL_ADM_MB_INUSE) 
    {
        ...
        {
            applib_mem_cache_header_struct *q = (applib_mem_cache_header_struct *)p;
//            MMI_TRACE(MMI_FW_TRC_G1_FRM, TRC_MMI_FRM_ASM_BLOC_CA, *count, q + 1, q->size, ((kal_uint32*)(q+1)) + ((q->size)>>2));
            //ASM_TRACE(TRC_MMI_FRM_ASM_DUMP_ALLOC, "[ASM][pool][ptr = 0x%x, id = %d, size = %d]\n", q + 1, 0, q->size);
			info[*count] = q->size;
...
//				MMI_TRACE(MMI_FW_TRC_G1_FRM, TRC_MMI_FRM_ASM_BLOC_AP, *count, p + 1, p->chunk_size, ((kal_uint32*)(p+1)) + ((p->chunk_size)>>2));
			...
			{
//				MMI_TRACE(MMI_FW_TRC_G1_FRM, TRC_MMI_FRM_ASM_BLOC_AN, *count, p + 1, p->chunk_size, ((kal_uint32*)(p+1)) + ((p->chunk_size)>>2));
            	info[*count] = p->chunk_size;
    ...
    else if ((flags & KAL_ADM_MB_FREE) == KAL_ADM_MB_FREE)
    {
		...
		if(size > 60)
		{
//			MMI_TRACE(MMI_FW_TRC_G1_FRM, TRC_MMI_FRM_ASM_BLOC_FREE, *count, adm_mb_head, size, ((kal_uint32*)(adm_mb_head)) + (size>>2));
         	//ASM_TRACE(TRC_MMI_FRM_ASM_DUMP_FREE, "[ASM][pool][Free: ptr = 0x%x, size = %d]\n", adm_mb_head, size);
			block[*count] = adm_mb_head;
			info[*count] = size;
            ...
            applib_mem_cache_free_cb_type free_cb, kal_char *file, kal_uint32 line)
{
    ...
    APP_ASM_UNLOCK;
	MMI_TRACE(MMI_FW_TRC_G1_FRM, TRC_MMI_FRM_ASM_CACHE_ALLOC, chunk_size, alloc_flag, free_cb, line, mem_ptr);
    MMI_PRINT(MOD_MMI_FW, MMI_FW_TRC_G1_FRM, "%s", get_file_name(file));
