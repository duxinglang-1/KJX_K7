    ...
    else if ( ( (wc >= 0x0461) && (wc <= 0x04f9) ) || ( (wc >= 0x1E01) && (wc <= 0x1EF9)) )
    {
        if ((wc & 1) == 1) /* odd is small case */
            ...
            return wc;
             ...
             wc >= 0x0182 && wc <= 0x0185 ||wc >= 0x0198 && wc <= 0x01A5 || /* Latin extend-B */
             wc >= 0x01DE && wc <= 0x01EF ||wc >= 0x01F8 && wc <= 0x01FF || /* Latin extend-B */
             wc >= 0x0222 && wc <= 0x0233 ||wc >= 0x0200 && wc <= 0x021F) /* Latin extend-B */ 
    {
        if ((wc & 1) == 1) /* odd is small case */
            return wc - 1;
        else
...
kal_wchar *app_ucs2_wcsstr(const kal_wchar *string, const kal_wchar *str_char_set)
{
    ...
    /*----------------------------------------------------------------*/
    ASSERT(!((kal_uint32)string & 0x01));
    ASSERT(!((kal_uint32)str_char_set & 0x01));
    while (*str_tmp)
    {
        cursor1 = str_tmp;
...
kal_int32 app_ucs2_wcsicmp(const kal_wchar *str_src, const kal_wchar *str_dst)
{
    ...
    ASSERT(!((kal_uint32)str_dst & 0x01));
    do {
        last = app_ucs2_towupper(*str_dst);
        first = app_ucs2_towupper(*str_src);
        str_dst++;
        str_src++;
    } while (first && (first == last));
...
kal_uint16 app_asc_str_to_ucs2_wcs(kal_wchar *pOutBuffer, kal_int8 *pInBuffer)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ASSERT(!((kal_uint32)pOutBuffer & 0x01));
    while (*pInBuffer)
    {
        *(pOutBuffer++) = (kal_wchar)(*((kal_uint8 *)pInBuffer++));
    }
...
kal_uint16 app_asc_str_to_ucs2_wcs_ex(kal_wchar *dest, kal_int8 *src, kal_uint32 *src_end_pos)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ASSERT(!((kal_uint32)dest & 0x01));
    while (*src)
    {
        *(dest++) =  (kal_wchar)(*((kal_uint8 *)src++));
    }
...
kal_uint16 app_ucs2_wcs_to_asc_str(kal_int8 *pOutBuffer, kal_wchar *pInBuffer)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ASSERT(!((kal_uint32)pInBuffer & 0x01));
    while (*pInBuffer)
    {
...
kal_uint8 app_ucs2encoding_to_unicode(kal_wchar *pUnicode, kal_uint8 *arr)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ASSERT(!((kal_uint32)pUnicode & 0x01));
 ...
 *  app_strlcpy
 * DESCRIPTION
 *  Copy at most (buffer_size - 1) characters to dest. This function guarantees
 *  to NULL-terminate the dest buffer.
 * PARAMETERS
 *  dest            [OUT] Destination buffer
 *  src             [IN]  ASCII string to be copied
 *  buffer_size     [IN]  The size of dest. At most (buffer_size - 1) characters
 *                        will be copied.
 * RETURNS
 *  The length of the src string.
 *****************************************************************************/
kal_int32 app_strlcpy(kal_char *dest, const kal_char *src, const kal_int32 buffer_size)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  app_ucs2_atoi
 * DESCRIPTION
 *  Converts UCS2 string pointed to by nptr to binary. Overflow is not detected.
 * PARAMETERS
 *  nptr           [IN]    ptr to string to convert    
 * RETURNS
 *  return kal_int32 value of the string
 *****************************************************************************/
kal_int32 app_ucs2_atoi(const kal_int8 *nptr)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	kal_wchar wc;             /* current char */
	kal_int32 total;         /* current total */
	kal_wchar sign;           /* if '-', then negative, otherwise positive */
	...
	sign = wc;           /* save sign indication */
	if (wc == L'-' || wc == L'+')
	{
		nptr += 2;
		wc = *(nptr+1)<<8|(kal_uint8)(*nptr);    /* skip sign */
	}
	...
	while ((wc >= L'0') && (wc <= L'9')) 
	{
		total = 10 * total + (wc - L'0');     /* accumulate digit */
		nptr += 2;
		wc = *(nptr+1)<<8|(kal_uint8)(*nptr);    /* get next char */
	}
 ...
 *  return kal_int32 value of the string
 *****************************************************************************/
kal_int32 app_ucs2_wtoi(const kal_wchar *nptr)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	sign = *nptr;           /* save sign indication */
	if (*nptr == L'-' || *nptr == L'+')
	{
		nptr++;       /* skip sign */
	}
	...
	{
		total = 10 * total + (*nptr - L'0');     /* accumulate digit */
		nptr++;                                  /* get next char */
	}
 ...
 *  buf           [OUT]    String result. 
 *  radix         [IN]     Base of value; must be in the range 2 ¨C 36. 
 * RETURNS
 *  returns a pointer to string.
 *****************************************************************************/
kal_int8 *app_ucs2_itoa (kal_int32 val, kal_int8 *buf, kal_int32 radix)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	kal_int8 *p;                /* pointer to traverse string */
	kal_int8 *firstdig;         /* pointer to first digit */
	kal_wchar temp;             /* temp char */
	kal_uint32 digval;          /* value of digit */
	kal_bool is_neg;
	...
	if (is_neg) {
		...
		*p++ = '-';
		*p++ = 0;
		val = (kal_uint32)(-(kal_int32)val);
	}
	
	firstdig = p ;           /* save pointer to first digit */
	
	do {
		digval = (kal_uint32) (val % radix);
		val /= radix;       /* get next digit */
		
		/* convert to ascii and store */
		if (digval > 9)
		{
			*p++ = (kal_int8) (digval - 10 + 'a');  /* a letter */
			*p++ = 0;
		}
		else
		{
			*p++ = (kal_int8) (digval + '0');       /* a digit */
			*p++ = 0;
		}
	} while (val > 0);
	...
	p[0] = '\0';            
	p[1] = '\0';/* terminate string; p points to last digit */
	p -= 2;
	...
	do {
		...
		*firstdig = temp;   /* swap *p and *firstdig */
		p -= 2;
		firstdig += 2;         /* advance to next two digits */        
 ...
 *  returns a pointer to string.
 *****************************************************************************/
kal_wchar *app_ucs2_itow (kal_int32 val, kal_wchar *buf, kal_int32 radix)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	kal_wchar *p;                /* pointer to traverse string */
	kal_wchar *firstdig;         /* pointer to first digit */
	kal_wchar temp;             /* temp char */
	kal_uint32 digval;          /* value of digit */
	kal_bool is_neg;
	...
	do {
		...
		if (digval > 9)
		{
			*p++ = (kal_wchar) (digval - 10 + 'a');  /* a letter */
		}
		else
		{
			*p++ = (kal_wchar) (digval + '0');       /* a digit */
		}
	} while (val > 0);
 ...
 *****************************************************************************/
kal_uint8 app_ucs2_is_newline (kal_wchar *buf)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    else if (*buf == 0x0A || *buf == 0x2028 || *buf == 0x2029)
    {
        count = 1;
    }
...
static void mmi_frm_string_app_asc_str_to_ucs2_wcs(void);
static void mmi_frm_string_app_asc_str_to_ucs2_wcs_ex(void);
static void mmi_frm_string_app_asc_str_n_to_ucs2_wcs(void);
static void mmi_frm_string_app_asc_str_n_to_ucs2_wcs_ex(void);
static void mmi_frm_string_app_asc_str_n_to_ucs2_str(void);
static void mmi_frm_string_app_ucs2_wcs_to_asc_str(void);
static void mmi_frm_string_app_wcsicmp(void);

typedef void (*mmi_frm_string_ut_funptr)(void); 
typedef struct
{
    kal_int32 index;
    mmi_frm_string_ut_funptr test_case_func;
} mmi_frm_app_str_test_tbl_struct;

static mmi_frm_app_str_test_tbl_struct mmi_frm_string_ut_table[] = 
{
    {0, mmi_frm_string_app_asc_str_to_ucs2_wcs},
    {1, mmi_frm_string_app_asc_str_to_ucs2_wcs_ex},
    {2, mmi_frm_string_app_asc_str_n_to_ucs2_wcs},
    {3, mmi_frm_string_app_asc_str_n_to_ucs2_wcs_ex},
    {4, mmi_frm_string_app_asc_str_n_to_ucs2_str},
    {5, mmi_frm_string_app_ucs2_wcs_to_asc_str},
    {6, mmi_frm_string_app_wcsicmp},
    {-1, NULL}
};
#define UT_ARRTY_SIZE   (sizeof(mmi_frm_string_ut_table) / sizeof(mmi_frm_string_ut_table[0]))

/*****************************************************************************
 * FUNCTION
 *  mmi_frm_string_app_asc_str_to_ucs2_wcs
 * DESCRIPTION
 *  function app_asc_str_to_ucs2_wcs unit test
 * PARAMETERS
 *  void
 * RETURNS
 *  void
...
static void mmi_frm_string_app_asc_str_to_ucs2_wcs(void)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_int8 input[4] = {0xa1,0xf2,0x01,0x00};
    kal_wchar output[4];
    kal_int8 verifybyte;
    int i;
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    app_asc_str_to_ucs2_wcs(output, input);
    for(i = 0; i < 4; i++)
    {
       verifybyte = output[i] >> 8;
       ASSERT(verifybyte == 0x00);
       verifybyte = output[i] & 0x00ff;
       ASSERT(verifybyte == input[i]);
    }
}
...
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_string_app_asc_str_to_ucs2_wcs_ex
 * DESCRIPTION
 *  function app_asc_str_to_ucs2_wcs_ex unit test
 * PARAMETERS
 *  void
 * RETURNS
 *  void
...
{  
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    app_asc_str_to_ucs2_wcs_ex(output, input, &src_end_pos);
    for(i = 0; i < 4; i++)
    {
       verifybyte = output[i] >> 8;
       ASSERT(verifybyte == 0x00);
    ...
    ASSERT(src_end_pos == (kal_uint32)(&(input[3])));
}


/*****************************************************************************
 * FUNCTION
 *  mmi_frm_string_app_asc_str_n_to_ucs2_wcs
 * DESCRIPTION
 *  function app_asc_str_n_to_ucs2_wcs unit test
 * PARAMETERS
 *  void
 * RETURNS
 *  void
...
static void mmi_frm_string_app_asc_str_n_to_ucs2_wcs(void)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_int8 input[5] = {0xa1,0xf2,0x01,0xa9,0x00};
    kal_wchar output[5];
    kal_int8 verifybyte;
    int i;
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    app_asc_str_n_to_ucs2_wcs(output, input, 5);
    for(i = 0; i < 5; i++)
    {
       verifybyte = output[i] >> 8;
       ASSERT(verifybyte == 0x00);
...
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_string_app_asc_str_n_to_ucs2_wcs_ex
 * DESCRIPTION
 *  function app_asc_str_n_to_ucs2_wcs_ex unit test
 * PARAMETERS
 *  void
 * RETURNS
 *  void
...
static void mmi_frm_string_app_asc_str_n_to_ucs2_wcs_ex(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    app_asc_str_n_to_ucs2_wcs_ex(output, input, 5, &src_end_pos);
    for(i = 0; i < 5; i++)
    {
       verifybyte = output[i] >> 8;
       ASSERT(verifybyte == 0x00);
    ...
    ASSERT(src_end_pos == (kal_uint32)(&(input[4])));
}


/*****************************************************************************
 * FUNCTION
 *  mi_frm_string_app_asc_str_n_to_ucs2_str
 * DESCRIPTION
 *  function app_asc_str_n_to_ucs2_str unit test
 * PARAMETERS
 *  void
 * RETURNS
 *  void
...
static void mmi_frm_string_app_asc_str_n_to_ucs2_str(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    app_asc_str_n_to_ucs2_str(output, input, 5);
    for(i = 0; i < 5; i++)
    {
        ASSERT(output[2*i] == input[i]);
        ASSERT(output[2*i+1] == 0x00);
    }
}
...
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_string_app_ucs2_wcs_to_asc_str
 * DESCRIPTION
 *  function app_ucs2_wcs_to_asc_str unit test
 * PARAMETERS
 *  void
 * RETURNS
 *  void
...
static void mmi_frm_string_app_ucs2_wcs_to_asc_str(void)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_wchar input[4] = {0xdda1,0xfff2,0x0201,0x0000};
    kal_int8 output[8];
    kal_int8 verifybyte;
    int i;
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    app_ucs2_wcs_to_asc_str(output, input);
    for (i = 0; i < 4; i++)
    {
        verifybyte = input[i] & 0x00ff;
        ASSERT(verifybyte == output[i]);
    }
}
...
/*****************************************************************************
 * FUNCTION
 *  mmi_frm_string_app_wcsicmp
 * DESCRIPTION
 *  test for wcs compare-interface
 * PARAMETERS
 *  void
 * RETURNS
 *  void
...
static void mmi_frm_string_app_wcsicmp(void)
{
    ...
    /*----------------------------------------------------------------*/
    kal_int32 i, j;
    kal_wchar *small_tbl[] = {L"abc", L"ABC"};
    kal_wchar *big_tbl[] = {L"bcd", L"acd", L"ABD"};
    #define SMALL_TBL_SIZE  sizeof(small_tbl) / sizeof(small_tbl[0])
    #define BIG_TBL_SIZE  sizeof(big_tbl) / sizeof(big_tbl[0])
    
	/*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    for (i = 0; i < SMALL_TBL_SIZE; i++)
    {
        for (j = 0; j < BIG_TBL_SIZE; j++)
        {
            ASSERT(app_ucs2_wcsicmp(small_tbl[i], big_tbl[j]) < 0);
            ASSERT(app_ucs2_wcsicmp(big_tbl[j], small_tbl[i]) > 0);
            ASSERT(0 == app_ucs2_wcsicmp(small_tbl[i], small_tbl[i]));
            ASSERT(0 == app_ucs2_wcsicmp(big_tbl[j], big_tbl[j]));

			ASSERT(app_ucs2_wcsnicmp(small_tbl[i], big_tbl[j], app_ucs2_wcslen(small_tbl[i])) < 0);
			ASSERT(app_ucs2_wcsnicmp(big_tbl[j], small_tbl[i], app_ucs2_wcslen(small_tbl[i])) > 0);
            ASSERT(app_ucs2_wcsnicmp(small_tbl[i], small_tbl[i], app_ucs2_wcslen(small_tbl[i])) == 0);
            ASSERT(app_ucs2_wcsnicmp(big_tbl[j], big_tbl[j], app_ucs2_wcslen(small_tbl[i])) == 0);
        }
    }
}
 ...
 *  mmi_frm_string_ut
 * DESCRIPTION
 *  function string api unit test entry function
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_frm_string_ut(kal_uint8 index)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (0 == index)
    {
        /* Test all cases */
        kal_uint8 i = 0;
        while (mmi_frm_string_ut_table[i].index >= 0 && mmi_frm_string_ut_table[i].test_case_func)
    {
            mmi_frm_string_ut_table[i].test_case_func();
            i++;
        }
    }
    else
    {
        mmi_frm_string_ut_table[index].test_case_func();
    }
}
