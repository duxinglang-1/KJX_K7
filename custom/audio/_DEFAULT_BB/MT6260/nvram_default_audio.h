...
#define NVRAM_EF_AUDIO_EQUALIZER_SIZE       sizeof(nvram_ef_audio_equalizer_struct)  /*for MMI write back*/
#define NVRAM_EF_AUDIO_EQUALIZER_TOTAL      1
#define NVRAM_EF_AUDIO_EQUALIZER_COUNT      8
#if defined(__BES_EQ_FLEXIBLE_SUPPORT__) 
#define NVRAM_EF_AUDIO_EQUALIZER_BAND_NUM   30
#else
#define NVRAM_EF_AUDIO_EQUALIZER_BAND_NUM   8
#endif

typedef struct {
    kal_int16 count;
    kal_int16 index;
    kal_int8  magnitude[NVRAM_EF_AUDIO_EQUALIZER_COUNT][NVRAM_EF_AUDIO_EQUALIZER_BAND_NUM];
    kal_uint8 setting_name[NVRAM_EF_AUDIO_EQUALIZER_COUNT + 1][(23 + 1) * 2];
} nvram_ef_audio_equalizer_struct;

extern const nvram_ef_audio_equalizer_struct NVRAM_EF_AUDIO_EQUALIZER_DEFAULT[];

#define NVRAM_EF_AUDIO_BES_EQ_FLEXIBLE_SIZE         sizeof(nvram_ef_audio_bes_eq_flexible_struct)
#define NVRAM_EF_AUDIO_BES_EQ_FLEXIBLE_TOTAL        1
#define NVRAM_EF_AUDIO_BES_EQ_FLEXIBLE_INIT_SIZE    122

typedef struct {
   kal_uint8 audio_bes_eq_initial_parameter[NVRAM_EF_AUDIO_BES_EQ_FLEXIBLE_INIT_SIZE];
