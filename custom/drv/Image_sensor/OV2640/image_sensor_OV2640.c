...
kal_bool OV2640_gVGAmode = KAL_TRUE, OV2640_MPEG4_encode_mode = KAL_FALSE;
#ifdef __MEDIA_VT__
/* under construction !*/
#endif
static kal_uint8  g_iPV_PCLK_Divider = 1;
kal_uint16 OV2640_extra_exposure_lines = 0;
static kal_uint16 OV2640_g_iBackupExtraExp = 0;
static kal_uint16 OV2640_ExpLines = 0;
kal_uint16 OV2640_sensor_global_gain=BASEGAIN, OV2640_sensor_gain_base=0x0;
OV2640_OP_TYPE g_iOV2640_Mode = OV2640_MODE_NONE;
/* MAX/MIN Explosure Lines Used By AE Algorithm */
kal_uint16 OV2640_MAX_EXPOSURE_LINES = 1000;
kal_uint8  OV2640_MIN_EXPOSURE_LINES = 2;
static kal_uint16 g_iPreview_Column_Pixel = 0;

static kal_uint16 OV2640_dummy_pixels=0, OV2640_dummy_lines=0;

static kal_uint32 OV2640_fix_video_frame_rate=300;
kal_uint8 OV2640StartX=0, OV2640StartY=0;
kal_uint16 OV2640CapShutter=0;
//#define OV2640_DEBUG

IMAGE_SENSOR_INDEX_ENUM OV2640SensorIdx;
sensor_data_struct *pNvramSensorData;
extern camcorder_info_struct OV2640_dsc_support_info;

static void OV2640PageWriteCmosSensor(kal_uint32 iAddr, kal_uint32 iPara)
{
  ...
  CamWriteCmosSensor(OV2640_PAGE_SETTING_REG, iRegPage);
  CamWriteCmosSensor(iRegAddr, iPara);
}

static kal_uint32 OV2640PageReadCmosSensor(const kal_uint32 iAddr)
{
  kal_uint16 iRegPage, iRegAddr;
...
static void OV2640WriteShutter(kal_uint16 shutter)
{
  ...
  if (OV2640_gVGAmode)
  {
    Max_shutter = OV2640_PV_PERIOD_LINE_NUMS + OV2640_dummy_lines;
  }
  else
  {
    Max_shutter = OV2640_FULL_PERIOD_LINE_NUMS +OV2640_dummy_lines;
  }

  if (shutter > Max_shutter)
  {
    OV2640_extra_exposure_lines = shutter - Max_shutter;
    shutter = Max_shutter;
  }
  else
  {
    OV2640_extra_exposure_lines = 0;
  }

    CamWriteCmosSensor(OV2640_PAGE_SETTING_REG, 0x01);
  ...
  if ((OV2640_g_iBackupExtraExp == 0 && OV2640_extra_exposure_lines > 0)||
     g_iOV2640_Mode == OV2640_MODE_CAPTURE )
  {
    CamWriteCmosSensor(0x2D, OV2640_extra_exposure_lines & 0xFF);             // ADVFL(LSB of extra exposure lines)
      CamWriteCmosSensor(0x2E, (OV2640_extra_exposure_lines & 0xFF00) >> 8);    // ADVFH(MSB of extra exposure lines)
  }
  
  OV2640_g_iBackupExtraExp = OV2640_extra_exposure_lines;
}   /* write_OV2640_shutter */

static kal_uint16 OV2640Reg2Gain(const kal_uint8 iReg)
{
  kal_uint8 iI;
...
static kal_uint8 OV2640Gain2Reg(const kal_uint16 iGain)
{
    kal_uint8 iReg = 0x00;
...
*
*************************************************************************/
static void OV2640SetDummy(const kal_uint16 iPixels, const kal_uint16 iLines)
{
  ASSERT(iPixels < 4096);
  CamWriteCmosSensor(OV2640_PAGE_SETTING_REG, 0x01);
...
{
#ifdef OV2640_PROCESSING_RAW
    CamWriteCmosSensor(OV2640_PAGE_SETTING_REG, 0x00);
  ...
  CamWriteCmosSensor(0x12, 0x40); // SVGA mode
    ...
    CamWriteCmosSensor(0x05, 0x00);
    //
    CamWriteCmosSensor(0x87, OV2640_WPC_BPC_CTRL);// Pixel correction on
...
*
************************************************************************
static void OV2640_GetID(kal_uint8 *pWriteID, kal_uint8 *pReadID)
{
  *pWriteID = OV2640_SW_SCCB_WRITE_ID;
  *pReadID = OV2640_SW_SCCB_READ_ID;
}
*/
...
*
*************************************************************************/
static void OV2640_GetSize(kal_uint16 *pWidth, kal_uint16 *pHeight)
{
  *pWidth = OV2640_IMAGE_SENSOR_FULL_WIDTH;
  *pHeight = OV2640_IMAGE_SENSOR_FULL_HEIGHT;
}
...
*
*************************************************************************/
static void OV2640_GetPeriod(kal_uint16 *pPixels, kal_uint16 *pLines)
{
#if (defined(DRV_ISP_6238_SERIES))
  *pPixels = OV2640_PV_PERIOD_PIXEL_NUMS + OV2640_dummy_pixels;
  *pLines =  OV2640_PV_PERIOD_LINE_NUMS + OV2640_dummy_lines;
#else
  *pPixels = OV2640_PV_PERIOD_PIXEL_NUMS;
  *pLines = OV2640_PV_PERIOD_LINE_NUMS;
#endif
}

static void OV2640SetSXVGA (void)
{
kal_uint8 iTemp=0;
  CamWriteCmosSensor(OV2640_PAGE_SETTING_REG, 0x01);
...
static void OV2640SetMirrorFlip(kal_uint8 image_mirror)
{
  kal_uint8 iTemp;
...
static void OV2640SetUXGA(void)
{
  kal_uint8 iTemp=0;
  CamWriteCmosSensor(OV2640_PAGE_SETTING_REG, 0x01);
  ...
  CamWriteCmosSensor(0x87, OV2640_WPC_BPC_CTRL);
...
*
*************************************************************************/
static void OV2640SetShutter(kal_uint16 iShutter)
{
  OV2640_ExpLines = iShutter;
  
  OV2640WriteShutter(iShutter);
}
...
*
*************************************************************************/
static kal_uint16 OV2640SetGain(kal_uint16 iGain)
{
  const kal_uint16 iBaseGain = OV2640Reg2Gain(pNvramSensorData->cct[OV2640_GLOBAL_GAIN_INDEX].para);

  const kal_uint16 iGain2Set = iGain * iBaseGain / BASEGAIN;
  kal_uint8 iReg = OV2640Gain2Reg(iGain2Set);
  ...
  return OV2640Reg2Gain(iReg) * BASEGAIN / iBaseGain;
}

static kal_uint16 OV2640ReadGain(void)
{  
  kal_uint8 iReg = 0;
  ...
  return OV2640Reg2Gain(iReg);
}

static void OV2640SetFlashlight (kal_bool enable)
{
  //flashlight_power_on(enable);
  FlashlightPowerOn(OV2640SensorIdx, enable);
}
...
static void OV2640CameraParaToSensor(nvram_camera_para_struct *pCameraPara)
{
  kal_uint32  i;
  
  for(i=0; (0xFFFFFFFF!=pCameraPara->SENSOR.reg[i].addr); i++)
  {
    OV2640PageWriteCmosSensor(pCameraPara->SENSOR.reg[i].addr, pCameraPara->SENSOR.reg[i].para);
  }
...
static void OV2640SensorToCameraPara(nvram_camera_para_struct *pCameraPara)
{
  kal_uint32  i;

  for(i=0; (0xFFFFFFFF!=pCameraPara->SENSOR.reg[i].addr)&&(i<ENGINEER_START_ADDR); i++)
  {
    pCameraPara->SENSOR.reg[i].para = OV2640PageReadCmosSensor(pCameraPara->SENSOR.reg[i].addr);
  }
  for(i=ENGINEER_START_ADDR; (0xFFFFFFFF!=pCameraPara->SENSOR.reg[i].addr); i++)
  {
    pCameraPara->SENSOR.reg[i].para = OV2640PageReadCmosSensor(pCameraPara->SENSOR.reg[i].addr);
...
static void  OV2640GetSensorGroupCount(kal_int32* sensor_count_ptr)
{
  *sensor_count_ptr = OV2640_GROUP_TOTAL_NUMS;
}

static void OV2640GetSensorGroupInfo(kal_uint16 group_idx, kal_int8* group_name_ptr, kal_int32* item_count_ptr)
{
   switch (group_idx)
   {
  case PRE_GAIN:
...
static void OV2640GetSensorItemInfo(kal_uint16 group_idx,kal_uint16 item_idx, CAL_CCT_SENSOR_ITEM_INFO_STRUCT* info_ptr)
{
    ...
    case PRE_GAIN:
      switch (item_idx)
      {
        case SENSOR_BASEGAIN:
          ...
          temp_gain = OV2640Reg2Gain(pNvramSensorData->cct[SENSOR_BASEGAIN].para);
...
static kal_bool OV2640SetSensorItemInfo(kal_uint16 group_idx, kal_uint16 item_idx, kal_int32 ItemValue)
{
   kal_uint8 temp_reg;
    ...
    case PRE_GAIN:
      switch (item_idx)
      {
        case SENSOR_BASEGAIN:
          ...
          temp_reg = OV2640Gain2Reg(temp_gain);        

          pNvramSensorData->cct[SENSOR_BASEGAIN].para = temp_reg;
          OV2640PageWriteCmosSensor(pNvramSensorData->cct[SENSOR_BASEGAIN].addr,temp_reg);
          break;
        ...
        case PRE_GAIN_R_INDEX:
          ...
          OV2640PageWriteCmosSensor(pNvramSensorData->cct[PRE_GAIN_R_INDEX].addr,temp_reg);
          break;
        case PRE_GAIN_B_INDEX:
          temp_reg = pNvramSensorData->cct[PRE_GAIN_B_INDEX].para;
          temp_reg &= ~0x0C;
          ...
          OV2640PageWriteCmosSensor(pNvramSensorData->cct[PRE_GAIN_B_INDEX].addr,temp_reg);
        break;
        ...
        case PRE_GAIN_Gr_INDEX:
          ...
          OV2640PageWriteCmosSensor(pNvramSensorData->cct[PRE_GAIN_Gr_INDEX].addr,temp_reg);
          break;
        case PRE_GAIN_Gb_INDEX:
          temp_reg = pNvramSensorData->cct[PRE_GAIN_Gb_INDEX].para;          
          temp_reg &= ~0x03;
          ...
          OV2640PageWriteCmosSensor(pNvramSensorData->cct[PRE_GAIN_Gb_INDEX].addr,temp_reg);
        break;
...
*
*************************************************************************/
static void OV2640GetSensorInfo(P_IMAGE_SENSOR_GET_SENSOR_INFO_STRUCT pSensorInfo)
{
  pSensorInfo->SensorId = OV2640_SENSOR_ID;
  ...
  pSensorInfo->PreviewWidth = OV2640_IMAGE_SENSOR_PV_WIDTH;
  pSensorInfo->PreviewHeight = OV2640_IMAGE_SENSOR_PV_HEIGHT;
  pSensorInfo->FullWidth = OV2640_IMAGE_SENSOR_FULL_WIDTH;
  pSensorInfo->FullHeight = OV2640_IMAGE_SENSOR_FULL_HEIGHT;
...
*
*************************************************************************/
static kal_uint16 OV2640PowerOn(void)
{
  ...
  kal_uint8 i = 0;
	...
	#ifdef OV2640_DEBUG
		kal_wap_trace(MOD_ENG,TRACE_INFO,"OV2640PowerOn");
	#endif
  CisModulePowerOn(OV2640SensorIdx, KAL_TRUE);
  
  CameraSccbOpen(CAMERA_SCCB_SENSOR, OV2640_SW_SCCB_WRITE_ID, CAMERA_SCCB_8BIT, CAMERA_SCCB_8BIT,  300);
  kal_sleep_task(3); // To wait for Stable Power
   
  /* set sensors chip enable pin to low to activate SIV100B */
  CamPdnPinCtrl(OV2640SensorIdx, 0);
  
  /* Reset the sensor */
  CamRstPinCtrl(OV2640SensorIdx, 1);
  /* delay for stable sensor */  
  kal_sleep_task(4);  
  CamRstPinCtrl(OV2640SensorIdx, 0);
  kal_sleep_task(4);
  CamRstPinCtrl(OV2640SensorIdx, 1);
  kal_sleep_task(4);
  ...
  kal_sleep_task(2);// at least one frame delay time needed after SW reset
	...
	CamWriteCmosSensor(0x3c, 0x3f);	// Reset PLL
	kal_sleep_task(2);
	CamWriteCmosSensor(0x3c, 0x3f);	// Reset PLL
	kal_sleep_task(2);
  /* Check ID */
  for (i=0;i<3;i++)
  {
	...
	#ifdef OV2640_DEBUG
	  kal_wap_trace(MOD_ENG,TRACE_INFO,"SensorId =%x",SensorId);
	#endif
  return SensorId;
} /* OV2640PowerOn() */
...
*
*************************************************************************/
static kal_uint32 OV2640DetectSensorId(void)
{
  MM_ERROR_CODE_ENUM OV2640SensorClose(void);
  kal_uint16 sensorId, AnotherSensorIdx;
  //kal_int8 i, j;
  
  if (IMAGE_SENSOR_MAIN == OV2640SensorIdx)
  {
  ...
  //for (i = 1; i >= 0; i--)
  {
    ...
    //for (j = 1; j >= 0; j--)
    {
      ...
      sensorId = OV2640PowerOn();
      OV2640SensorClose();
#ifdef OV2640_DEBUG
      kal_wap_trace(MOD_ENG,TRACE_INFO,"OV2640DetectSensorId:%x",sensorId);
#endif
      if (OV2640_SENSOR_ID == sensorId)
      {
        return OV2640_SENSOR_ID;
      }
    }
  }
  return -1;
...
static MM_ERROR_CODE_ENUM OV2640SensorOpen(void)
{
	#ifdef OV2640_DEBUG
		kal_wap_trace(MOD_ENG, TRACE_INFO, "enter OV2640SensorOpen");
	#endif
	IspSetInterruptTriggerDelayLines(1);  // 0: VD INT, others: TG INT & start line
  
  if (OV2640_SENSOR_ID != OV2640PowerOn()) return MM_ERROR_SENSOR_READ_ID_FAIL;
  ...
  CamWriteCmosSensor(OV2640_PAGE_SETTING_REG, 0x01);
  OV2640_sensor_gain_base = CamReadCmosSensor(0x00);
	
	#ifdef OV2640_DEBUG
		kal_wap_trace(MOD_ENG, TRACE_INFO, "exit OV2640SensorOpen");
	#endif
...
*
*************************************************************************/
static void OV2640Preview(CAL_SCENARIO_ENUM ScenarioId,
                               P_IMAGE_SENSOR_SCENARIO_PARA_IN_STRUCT pParaIn,
                               P_IMAGE_SENSOR_SCENARIO_PARA_OUT_STRUCT pParaOut)
{
	#ifdef OV2640_DEBUG
		kal_wap_trace(MOD_ENG, TRACE_INFO, "enter OV2640Preview");
	#endif
    OV2640SetSXVGA();

  OV2640_gVGAmode = KAL_TRUE;
  
    OV2640SetMirrorFlip(pParaIn->ImageMirror);

    // compensate OV2640 ABLC for every frame
    CamWriteCmosSensor(OV2640_PAGE_SETTING_REG, 0x01);
    CamWriteCmosSensor(0x71, 0x94);  // reserved register

    OV2640_dummy_pixels = 0;
    OV2640_dummy_lines = 50; 

    g_iOV2640_Mode = OV2640_MODE_PREVIEW;

	OV2640SetDummy(OV2640_dummy_pixels, OV2640_dummy_lines);
	g_iPV_PCLK_Divider = 3;
	g_iPreview_Column_Pixel = OV2640_PV_PERIOD_PIXEL_NUMS + OV2640_dummy_pixels;

    pParaOut->GrabStartX = OV2640StartX;
    pParaOut->GrabStartY = OV2640StartY;
    pParaOut->ExposureWindowWidth = OV2640_IMAGE_SENSOR_PV_WIDTH;
    pParaOut->ExposureWindowHeight = OV2640_IMAGE_SENSOR_PV_HEIGHT;

    OV2640WriteShutter(OV2640_ExpLines);
	
	#ifdef OV2640_DEBUG
		kal_wap_trace(MOD_ENG, TRACE_INFO, "exit OV2640Preview");
	#endif
}

static void OV2640GetPreviewFlashlightInfo(P_FLASHLIGHT_PREVIEW_PARA_STURCT pPreviewInfo)
{
  pPreviewInfo->pixels = OV2640_PV_PERIOD_PIXEL_NUMS;
  pPreviewInfo->lines = OV2640_IMAGE_SENSOR_PV_HEIGHT;
  pPreviewInfo->frame_lines = OV2640_PV_PERIOD_LINE_NUMS;
}
...
*
*************************************************************************/
static void OV2640Capture(CAL_SCENARIO_ENUM ScenarioId,
                               P_IMAGE_SENSOR_SCENARIO_PARA_IN_STRUCT pParaIn,
                               P_IMAGE_SENSOR_SCENARIO_PARA_OUT_STRUCT pParaOut)
{
    kal_uint16 iCapture_Column_Pixel = 0, iShutter = OV2640_ExpLines;
    kal_uint16 iDummyPixels = 0, iDummyLines = 0;
    kal_uint8 iTemp, iCP_PCLK_Div = 1;
    kal_uint16 iGrabWidth = 0, iGrabHeight = 0;

	#ifdef OV2640_DEBUG
		kal_wap_trace(MOD_ENG, TRACE_INFO, "exit OV2640Capture");
	#endif
    ...
    if ((pParaIn->ImageTargetWidth<=OV2640_IMAGE_SENSOR_PV_WIDTH)&&
        (pParaIn->ImageTargetHeight<=OV2640_IMAGE_SENSOR_PV_HEIGHT))
    {
		OV2640_gVGAmode = KAL_TRUE;
            OV2640_dummy_pixels= 0;         
      OV2640_dummy_lines = 0;

      iGrabWidth = OV2640_IMAGE_SENSOR_PV_WIDTH;
      iGrabHeight = OV2640_IMAGE_SENSOR_PV_HEIGHT;
      g_iOV2640_Mode = OV2640_MODE_CAPTURE;
		...
		iCapture_Column_Pixel = OV2640_PV_PERIOD_PIXEL_NUMS + OV2640_dummy_pixels;
		iCP_PCLK_Div = 3;
   ...
   OV2640SetMirrorFlip(pParaIn->ImageMirror);
    ...
    else if(pParaIn->ZoomFactor<200)/* 1x~2x capture */
      {
			...
			{
				OV2640_dummy_pixels= 300;        
				OV2640_dummy_lines = 0;      
      }
    else /* 4x~8x capture */
      {
        OV2640_dummy_pixels= 1600;        
        OV2640_dummy_lines = 0;      
      }
   ...
   iGrabWidth = OV2640_IMAGE_SENSOR_FULL_WIDTH;
   iGrabHeight = OV2640_IMAGE_SENSOR_FULL_HEIGHT;
   
   iCapture_Column_Pixel = OV2640_FULL_PERIOD_PIXEL_NUMS + OV2640_dummy_pixels;
   ...
   iShutter = iShutter * 2 * g_iPV_PCLK_Divider / iCP_PCLK_Div;   // x2 for SVGA -> UXGA switch
	...
	//  To force OV2640 re-calibrate ABLC by chaning global gain
	CamWriteCmosSensor(OV2640_PAGE_SETTING_REG,0x01);
  ...
  CamWriteCmosSensor(0x71,0x96);        //compensate OV2640 ABLC for every frame   
...
static void OV2640GetCaptureFlashlightInfo(P_FLASHLIGHT_CAPTURE_SENSOR_PARA_IN_STRUCT pParaIn, P_FLASHLIGHT_CAPTURE_PARA_STRUCT pCaptureInfo)
{
    
    pCaptureInfo->default_pclk=26000000;

    pCaptureInfo->pixels = OV2640_FULL_PERIOD_PIXEL_NUMS + OV2640_dummy_pixels;
    pCaptureInfo->frame_lines = OV2640_FULL_PERIOD_LINE_NUMS+OV2640_dummy_lines;
    pCaptureInfo->lines = OV2640_IMAGE_SENSOR_FULL_HEIGHT;
    pCaptureInfo->shutter = OV2640CapShutter;
    //Shutter = flashlight_sensor_capture_config(&flashlight_capture_para, pParaIn);    

}

static void OV2640Video(CAL_SCENARIO_ENUM ScenarioId,
                               P_IMAGE_SENSOR_SCENARIO_PARA_IN_STRUCT pParaIn,
                               P_IMAGE_SENSOR_SCENARIO_PARA_OUT_STRUCT pParaOut)
{
	#ifdef OV2640_DEBUG
		kal_wap_trace(MOD_ENG, TRACE_INFO, "enter OV2640Video MaxVideoFrameRate=%d",pParaIn->MaxVideoFrameRate);
	#endif
  ...
  OV2640_gVGAmode = KAL_TRUE;
	...
	OV2640SetDummy(OV2640_dummy_pixels, OV2640_dummy_lines);
  OV2640_fix_video_frame_rate = pParaIn->MaxVideoFrameRate;
  OV2640_MAX_EXPOSURE_LINES = (kal_uint16)((260000000/OV2640_fix_video_frame_rate)/(OV2640_PV_PERIOD_PIXEL_NUMS+OV2640_dummy_pixels));      
  if((OV2640_MAX_EXPOSURE_LINES>OV2640_PV_PERIOD_LINE_NUMS)&&(OV2640_dummy_lines<(OV2640_MAX_EXPOSURE_LINES - OV2640_PV_PERIOD_LINE_NUMS)))    
    OV2640_dummy_lines = OV2640_MAX_EXPOSURE_LINES - OV2640_PV_PERIOD_LINE_NUMS;
      
  CamWriteCmosSensor(OV2640_PAGE_SETTING_REG, 0x01);
  CamWriteCmosSensor(0x46, OV2640_dummy_lines & 0x00FF);
  CamWriteCmosSensor(0x47, OV2640_dummy_lines >> 8);
 
    pParaOut->GrabStartX = OV2640StartX;
    pParaOut->GrabStartY = OV2640StartY;
    pParaOut->ExposureWindowWidth = OV2640_IMAGE_SENSOR_PV_WIDTH-OV2640StartX;
    pParaOut->ExposureWindowHeight = OV2640_IMAGE_SENSOR_PV_HEIGHT-OV2640StartY;

	#ifdef OV2640_DEBUG
		kal_wap_trace(MOD_ENG, TRACE_INFO, "enter OV2640Video");
	#endif

}

static MM_ERROR_CODE_ENUM OV2640BinningInfo(P_CAL_FEATURE_CTRL_STRUCT pFeatureParaIn,
                                        ...
                                        kal_uint32 FeatureParaOutLen, kal_uint32 *pRealFeatureParaOutLen)
{
  ...
  kal_uint8 i;
  	...
  	#ifdef OV2640_DEBUG
	kal_wap_trace(MOD_ENG, TRACE_INFO, "enter OV2640BinningInfo");
	#endif
...
*
*************************************************************************/
static MM_ERROR_CODE_ENUM OV2640StillCaptureSize(P_CAL_FEATURE_CTRL_STRUCT pFeatureParaIn,
                                               P_CAL_FEATURE_CTRL_STRUCT pFeatureParaOut,
                                               kal_uint32 FeatureParaOutLen, kal_uint32 *pRealFeatureParaOutLen)
{
...
*
*************************************************************************/
static void OV2640InitOperationPara(P_IMAGE_SENSOR_GET_INIT_OPERATION_PARA_STRUCT pInitOperPara)
{
  ...
  pInitOperPara->PreviewDisplayWaitFrame = 1; /* wait stable frame when sensor change mode (cap to pre) */
...
*
*************************************************************************/
static void OV2640DscSupportInfo(P_IMAGE_SENSOR_GET_DSC_SUPPORT_INFO_STRUCT pDscSupportInfo)
{
  kal_mem_cpy(pDscSupportInfo, &OV2640_dsc_support_info, sizeof(camcorder_info_struct));
}

static MM_ERROR_CODE_ENUM OV2640Sharpness(P_CAL_FEATURE_CTRL_STRUCT pFeatureParaIn,
                                        P_CAL_FEATURE_CTRL_STRUCT pFeatureParaOut,
                                        kal_uint32 FeatureParaOutLen, kal_uint32 *pRealFeatureParaOutLen)
{
...
*
*************************************************************************/
static MM_ERROR_CODE_ENUM OV2640SensorFeatureCtrl(kal_uint32 FeatureId,
                                                ...
                                                kal_uint32 FeatureParaOutLen, kal_uint32 *pRealFeatureParaOutLen)
{
  ...
  {
    return OV2640WebcamFeatureCtrl(FeatureId, pFeatureParaIn, pFeatureParaOut,
  ...
  case IMAGE_SENSOR_FEATURE_SET_ESHUTTER:
    OV2640SetShutter(((P_IMAGE_SENSOR_COMM_DATA_STRUCT)pFeatureParaIn)->FeatureValue);
    break;
  case IMAGE_SENSOR_FEATURE_SET_GAIN:
    OV2640SetGain(((P_IMAGE_SENSOR_COMM_DATA_STRUCT)pFeatureParaIn)->FeatureValue);
    break;
  case IMAGE_SENSOR_FEATURE_GET_SENSOR_INFO:
    OV2640GetSensorInfo((P_IMAGE_SENSOR_GET_SENSOR_INFO_STRUCT) pFeatureParaOut);
    break;
  ...
  case IMAGE_SENSOR_FEATURE_GET_SENSOR_ID:
    ((P_IMAGE_SENSOR_COMM_DATA_STRUCT) pFeatureParaOut)->FeatureValue=OV2640DetectSensorId();
    break;
  case IMAGE_SENSOR_FEATURE_SET_CAMERA_SOURCE:
    OV2640SensorIdx = ((P_IMAGE_SENSOR_SET_CAMERA_SOURCE_STRUCT) pFeatureParaIn)->ImageSensorIdx;
    break;
  case IMAGE_SENSOR_FEATURE_CTRL_NIGHTMODE:
    break;
  case IMAGE_SENSOR_FEATURE_GET_PREVIEW_FLASHLIGHT_INFO:
    OV2640GetPreviewFlashlightInfo((P_FLASHLIGHT_PREVIEW_PARA_STURCT)pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_GET_CAPTURE_FLASHLIGHT_INFO:
    OV2640GetCaptureFlashlightInfo((P_FLASHLIGHT_CAPTURE_SENSOR_PARA_IN_STRUCT)pFeatureParaIn, (P_FLASHLIGHT_CAPTURE_PARA_STRUCT)pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_CTRL_FLASHLIGHT:
    OV2640SetFlashlight(((P_IMAGE_SENSOR_COMM_CTRL_STRUCT)pFeatureParaIn)->FeatureEnable);
    break;
  case IMAGE_SENSOR_FEATURE_GET_AE_INFO:
    break;
  case IMAGE_SENSOR_FEATURE_GET_AE_LUT:
    OV2640GetAeLut((P_AE_LUT_INFO_STRUCT) pFeatureParaIn, (P_IMAGE_SENSOR_AE_LUT_STRUCT)pFeatureParaOut,
                        FeatureParaOutLen, pRealFeatureParaOutLen);
    break;
  ...
  case CAL_FEATURE_CAMERA_BINNING_INFO:
    ErrorCode=OV2640BinningInfo((P_CAL_FEATURE_CTRL_STRUCT)pFeatureParaIn,
  ...
  case CAL_FEATURE_CAMERA_STILL_CAPTURE_SIZE:
    ErrorCode=OV2640StillCaptureSize((P_CAL_FEATURE_CTRL_STRUCT)pFeatureParaIn,
                                 (P_CAL_FEATURE_CTRL_STRUCT)pFeatureParaOut, FeatureParaOutLen, pRealFeatureParaOutLen);
    break;
  // for YUV sensor sample code
  // the following are for YUV sensor only
  case CAL_FEATURE_CAMERA_SHARPNESS:
  // please refer the sample code of OV2640Sharpness();
    ErrorCode=OV2640Sharpness((P_CAL_FEATURE_CTRL_STRUCT)pFeatureParaIn,
                                   (P_CAL_FEATURE_CTRL_STRUCT)pFeatureParaOut, FeatureParaOutLen, pRealFeatureParaOutLen);
    break;
  ...
  case CAL_FEATURE_CCT_OP_CAMERA_PARA_TO_SENSOR:
    OV2640CameraParaToSensor(pFeatureParaIn);
    break;
  case CAL_FEATURE_CCT_OP_SENSOR_TO_CAMERA_PARA:
    OV2640SensorToCameraPara(pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_GET_INIT_OPERATION_PARA:
    OV2640InitOperationPara((P_IMAGE_SENSOR_GET_INIT_OPERATION_PARA_STRUCT)pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_GET_DSC_SUPPORT_INFO:
    OV2640DscSupportInfo((P_IMAGE_SENSOR_GET_DSC_SUPPORT_INFO_STRUCT)pFeatureParaOut);
    break;

  case IMAGE_SENSOR_FEATURE_SET_SENSOR_POWER_DOWN:
	CamPdnPinCtrl(OV2640SensorIdx,1);
	CamRstPinCtrl(OV2640SensorIdx,0);
    break;
  ...
  case IMAGE_SENSOR_FEATURE_GET_GAIN:
    ((P_IMAGE_SENSOR_COMM_DATA_STRUCT)pFeatureParaOut)->FeatureValue=OV2640ReadGain();
    break;
  ...
  case IMAGE_SENSOR_FEATURE_GET_CCT_GROUP_COUNT:
    OV2640GetSensorGroupCount((kal_int32 *)pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_GET_CCT_GROUP_INFO:
    OV2640GetSensorGroupInfo(((P_CAL_CCT_SENSOR_GROUP_INFO_STRUCT)pFeatureParaIn)->GroupIdx,((P_CAL_CCT_SENSOR_GROUP_INFO_STRUCT)pFeatureParaIn)->GroupNamePtr, &((P_CAL_CCT_SENSOR_GROUP_INFO_STRUCT)pFeatureParaOut)->ItemCount);
    break;  
  case IMAGE_SENSOR_FEATURE_GET_CCT_ITEM_INFO:
    OV2640GetSensorItemInfo(((P_CAL_CCT_SENSOR_ITEM_INFO_STRUCT)pFeatureParaIn)->GroupIdx,((P_CAL_CCT_SENSOR_ITEM_INFO_STRUCT)pFeatureParaIn)->ItemIdx, (P_CAL_CCT_SENSOR_ITEM_INFO_STRUCT)pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_SET_CCT_ITEM_INFO:
    OV2640SetSensorItemInfo(((P_CAL_CCT_SENSOR_ITEM_INFO_STRUCT)pFeatureParaIn)->GroupIdx,((P_CAL_CCT_SENSOR_ITEM_INFO_STRUCT)pFeatureParaIn)->ItemIdx, ((P_CAL_CCT_SENSOR_ITEM_INFO_STRUCT)pFeatureParaIn)->ItemValue);
    break;
  default:
    ErrorCode = MM_ERROR_SENSOR_FEATURE_NOT_SUPPORT;
    break;
...
*
*************************************************************************/
static MM_ERROR_CODE_ENUM OV2640SensorCtrl(CAL_SCENARIO_ENUM ScenarioId, void *pParaIn,
                                         void *pParaOut, kal_uint16 ParaOutLen)
{
  ...
  case CAL_SCENARIO_WEBCAM_CAPTURE:
    OV2640Preview(ScenarioId, pParaIn, pParaOut);
    break;
  case CAL_SCENARIO_VIDEO:
    OV2640Video(ScenarioId, pParaIn, pParaOut);
    break;
  case CAL_SCENARIO_CAMERA_STILL_CAPTURE:
    OV2640Capture(ScenarioId, pParaIn, pParaOut);
    break;
  default:
    break;
  }
...
*
*************************************************************************/
static MM_ERROR_CODE_ENUM OV2640SensorClose(void)
{
	#ifdef OV2640_DEBUG
		kal_wap_trace(MOD_ENG, TRACE_INFO, "enter OV2640SensorClose");
	#endif

	CisModulePowerOn(OV2640SensorIdx, KAL_FALSE);

	CameraSccbClose(CAMERA_SCCB_SENSOR);
  
	#ifdef OV2640_DEBUG
	  kal_wap_trace(MOD_ENG, TRACE_INFO, "exit OV2640SensorClose");
	#endif
  return MM_ERROR_NONE;
}
...
{
  static IMAGE_SENSOR_FUNCTION_STRUCT ImageSensorFuncOV2640 =
  {
    OV2640SensorOpen,
    OV2640SensorFeatureCtrl,
    OV2640SensorCtrl,
  ...
  *pfSensorFunc = &ImageSensorFuncOV2640;
}
