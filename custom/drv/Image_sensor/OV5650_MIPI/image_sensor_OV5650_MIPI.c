...
#define __OV5650Mipi_DEBUG_TRACE__

kal_bool OV5650Mipi_gVGAmode = KAL_TRUE, OV5650Mipi_MPEG4_encode_mode = KAL_FALSE;
#ifdef __MEDIA_VT__
/* under construction !*/
/* under construction !*/
#endif
static kal_uint8  g_iPV_PCLK_Divider = 1;
kal_uint16 OV5650Mipi_extra_exposure_lines = 0;
static kal_uint16 OV5650Mipi_g_iBackupExtraExp = 0;
static kal_uint16 OV5650Mipi_ExpLines = 0;
kal_uint16 OV5650Mipi_sensor_global_gain=BASEGAIN, OV5650Mipi_sensor_gain_base=0x0;
OV5650_MIPI_OP_TYPE g_iOV5650Mipi_Mode = OV5650_MIPI_MODE_NONE;
/* MAX/MIN Explosure Lines Used By AE Algorithm */
kal_uint16 OV5650Mipi_MAX_EXPOSURE_LINES = 1000;
kal_uint8  OV5650Mipi_MIN_EXPOSURE_LINES = 2;
static kal_uint16 g_iPreview_Column_Pixel = 0;

static kal_uint32 OV5650Mipi_g_fPV_PCLK = 52 * 1000000;//52
static kal_uint32 OV5650Mipi_g_fCP_PCLK = 52 * 1000000;

static kal_uint16 OV5650Mipi_dummy_pixels=0, OV5650Mipi_dummy_lines=0;

//extern void dump_pic_from_isp(void);

static kal_uint32 OV5650Mipi_fix_video_frame_rate=300;
kal_uint8 OV5650MipiStartX=0, OV5650MipiStartY=0;
kal_uint16 OV5650MipiCapShutter=0;

IMAGE_SENSOR_INDEX_ENUM OV5650MipiSensorIdx;
CAL_CAMERA_SOURCE_ENUM CameraSource;
sensor_data_struct *pNvramSensorData;
extern camcorder_info_struct OV5650_MIPI_dsc_support_info;

static void OV5650MipiPageWriteCmosSensor(kal_uint32 iAddr, kal_uint32 iPara)
{
  ...
  CamWriteCmosSensor(OV5650_MIPI_PAGE_SETTING_REG, iRegPage);
  CamWriteCmosSensor(iRegAddr, iPara);
}

static kal_uint32 OV5650MipiPageReadCmosSensor(const kal_uint32 iAddr)
{
  kal_uint16 iRegPage, iRegAddr;
...
static void OV5650MipiWriteShutter(kal_uint16 shutter)
{
 
	 kal_uint16 iExp = shutter;
	 kal_uint16 Total_line;

	 if (OV5650Mipi_gVGAmode) {
		 if (iExp <= OV5650_MIPI_PV_PERIOD_LINE_NUMS + OV5650Mipi_dummy_lines) {
			 Total_line = OV5650_MIPI_PV_PERIOD_LINE_NUMS + OV5650Mipi_dummy_lines;
		 }else {
			 Total_line = iExp+28;
		 }
 
	 }else {
		 if (iExp <= OV5650_MIPI_FULL_PERIOD_LINE_NUMS + OV5650Mipi_dummy_lines) {
			 Total_line = OV5650_MIPI_FULL_PERIOD_LINE_NUMS+ OV5650Mipi_dummy_lines;
		 }else {
			 Total_line = iExp+28;
		 }
	 }
...
}   /* write_OV5650Mipi_shutter */

static kal_uint16 OV5650MipiReg2Gain(const kal_uint8 iReg)
{
  kal_uint8 iI;
...
static kal_uint8 OV5650MipiGain2Reg(const kal_uint16 iGain)
{
    kal_uint8 iReg = 0x00;
...
*
*************************************************************************/
static void OV5650MipiSetDummy(const kal_uint16 iPixels, const kal_uint16 iLines)
{
     kal_uint16 iTotalLines,iTotapixels;
   
	if(OV5650Mipi_gVGAmode == KAL_TRUE)
		{
		iTotalLines=OV5650_MIPI_XGA_MODE_WITHOUT_DUMMY_LINES + iLines;
		iTotapixels=OV5650_MIPI_XGA_MODE_WITHOUT_DUMMY_PIXELS+ iPixels;
		}
	else
		{
		iTotalLines=OV5650_MIPI_QXGA_MODE_WITHOUT_DUMMY_LINES + iLines;
		iTotapixels=OV5650_MIPI_QXGA_MODE_WITHOUT_DUMMY_PIXELS+ iPixels;
		}
	if(iTotapixels > 8160)
		iTotapixels = 8160;
...
*
*************************************************************************/
static void OV5650MipiInitialSetting(void)
{
		CamWriteCmosSensor(0x3008,0x82);
		kal_sleep_task(2);
		CamWriteCmosSensor(0x3008,0x42);
		CamWriteCmosSensor(0x3103,0x93);
		CamWriteCmosSensor(0x3b07,0x0c);	 //mipi modify from ox0d
		CamWriteCmosSensor(0x3017,0xff);	 //mipi modify from ox7f
		...
		CamWriteCmosSensor(0x3713,0x22);   //mipi modify from 92
		CamWriteCmosSensor(0x3714,0x27);   //mipi modify from 17
...
/* under construction !*/
#endif
		CamWriteCmosSensor(0x380c,0x0c);   //modify from 0a
		CamWriteCmosSensor(0x380d,0xb4);	//modify from 3c
		CamWriteCmosSensor(0x380e,0x07);   //modify from 04
		CamWriteCmosSensor(0x380f,0xb0);   //modify from c4
		...
		CamWriteCmosSensor(0x5059,0x80);
			CamWriteCmosSensor(0x3615,0x52);	 //mipi add
		CamWriteCmosSensor(0x505a,0x0a);
		CamWriteCmosSensor(0x505b,0x2e);
			CamWriteCmosSensor(0x3826,0x00);	 //mipi add
		...
		CamWriteCmosSensor(0x3c09,0xc2);
			CamWriteCmosSensor(0x4000,0x05);	 //mipi add
			CamWriteCmosSensor(0x4001,0x02);	 //mipi add
		CamWriteCmosSensor(0x5046,0x01);	 //mipi modify from 09	awb en
		CamWriteCmosSensor(0x3810,0x40);
		CamWriteCmosSensor(0x3836,0x41);
			CamWriteCmosSensor(0x505f,0x04);	 //mipi add
		CamWriteCmosSensor(0x5000,0x00);	 //mipi modify from fe
		CamWriteCmosSensor(0x5001,0x00);	 //mipi modify from 01
...
/* under construction !*/
		...
		CamWriteCmosSensor(0x401f,0x03);
			CamWriteCmosSensor(0x3a0f,0x78);	 //mipi add
			CamWriteCmosSensor(0x3a10,0x68);	 //mipi add
			CamWriteCmosSensor(0x3a1b,0x78);	 //mipi add
			CamWriteCmosSensor(0x3a1e,0x68);	 //mipi add
			CamWriteCmosSensor(0x3a11,0xd0);	 //mipi add
			CamWriteCmosSensor(0x3a1f,0x40);	 //mipi add
	
			CamWriteCmosSensor(0x300f,0x8a);	 //mipi add     from 8b
			CamWriteCmosSensor(0x3010,0x00);	 //mipi add
			CamWriteCmosSensor(0x3011,0x14);	 //mipi add
			CamWriteCmosSensor(0x3012,0x04);	 //mipi add
	
	/* add for mipi */
			CamWriteCmosSensor(0x3007,0x3b);	 //mipi add 
			CamWriteCmosSensor(0x4801,0x0f);
		CamWriteCmosSensor(0x3003,0x03);
		CamWriteCmosSensor(0x300e,0x0c);   //modify from 04
			CamWriteCmosSensor(0x4803,0x50);
			CamWriteCmosSensor(0x4800,0x04);	 //mipi add
		CamWriteCmosSensor(0x3815,0x82);
		CamWriteCmosSensor(0x3003,0x01);
		
	//end
	
			CamWriteCmosSensor(0x3503,0x13);	 //mipi add
			CamWriteCmosSensor(0x3501,0x11);	 //mipi add
			CamWriteCmosSensor(0x3502,0x90);	 //mipi add
			CamWriteCmosSensor(0x350b,0x7f);	 //mipi add
		CamWriteCmosSensor(0x5001,0x01);	//manual AWB
		...
		CamWriteCmosSensor(0x5000,0x06);   //lens off,wbc on
...
*
*************************************************************************/
static void OV5650Mipi_GetID(kal_uint8 *pWriteID, kal_uint8 *pReadID)
{
  *pWriteID = OV5650_MIPI_SW_SCCB_WRITE_ID;
  *pReadID = OV5650_MIPI_SW_SCCB_READ_ID;
}
...
*
*************************************************************************/
static void OV5650Mipi_GetSize(kal_uint16 *pWidth, kal_uint16 *pHeight)
{
  *pWidth = OV5650_MIPI_IMAGE_SENSOR_FULL_WIDTH;
  *pHeight = OV5650_MIPI_IMAGE_SENSOR_FULL_HEIGHT;
}

/*************************************************************************
* FUNCTION
*  OV5650Mipi_get_period
*
* DESCRIPTION
*  This function return the image width and height of image sensor.
*
...
*
*************************************************************************/
static void OV5650Mipi_GetPeriod(kal_uint16 *pPixels, kal_uint16 *pLines)
{
#if (defined(DRV_ISP_6238_SERIES))
  *pPixels = OV5650_MIPI_PV_PERIOD_PIXEL_NUMS + OV5650Mipi_dummy_pixels;
  *pLines =  OV5650_MIPI_PV_PERIOD_LINE_NUMS + OV5650Mipi_dummy_lines;
#else
  *pPixels = OV5650_MIPI_PV_PERIOD_PIXEL_NUMS;
  *pLines = OV5650_MIPI_PV_PERIOD_LINE_NUMS;
#endif
}
...
CamWriteCmosSensor(0x3008,0x42);    //mipi add
CamWriteCmosSensor(0x3613,0xc4);    //mipi add
...
CamWriteCmosSensor(0x3801,0x34);    
CamWriteCmosSensor(0x3803,0x0a);     
CamWriteCmosSensor(0x3804,0x05);
CamWriteCmosSensor(0x3805,0x00);    //....................from 10
CamWriteCmosSensor(0x3806,0x03);
CamWriteCmosSensor(0x3807,0xc0);    //....................from c0
...
CamWriteCmosSensor(0x380d,0x3c);   //....................from 78
CamWriteCmosSensor(0x380e,0x04);    
CamWriteCmosSensor(0x380f,0x08);    

CamWriteCmosSensor(0x3815,0x81);
...
CamWriteCmosSensor(0x3827,0x0a);

CamWriteCmosSensor(0x3A08,0x1b);
CamWriteCmosSensor(0x3A09,0xc0);
CamWriteCmosSensor(0x3A0A,0x17);
CamWriteCmosSensor(0x3A0B,0x20);
CamWriteCmosSensor(0x3A0D,0x01);
CamWriteCmosSensor(0x3A0E,0x01);

CamWriteCmosSensor(0x3A00,0x38);
CamWriteCmosSensor(0x401d,0x08);
...
CamWriteCmosSensor(0x381a,0x4a);
#if 1
CamWriteCmosSensor(0x300f,0x8a);    //modify from 8b
CamWriteCmosSensor(0x3010,0x00);
CamWriteCmosSensor(0x3011,0x0c);    //modify from 15
...
static void OV5650MipiSetPvMirrorFlip(kal_uint8 image_mirror)
{
  switch (image_mirror)
  {
...
static void OV5650MipiSetCapMirrorFlip(kal_uint8 image_mirror)
{
  switch (image_mirror)
  {
	  case IMAGE_SENSOR_MIRROR_NORMAL://h_mirror 
...
static void OV5650MipiSetUXGA(void)
{
    CamWriteCmosSensor(0x3008,0x42);
     CamWriteCmosSensor(0x3613,0x44);
     CamWriteCmosSensor(0x370d,0x04);  //MIPI add       //...............from 04
     CamWriteCmosSensor(0x3703,0xe6);  //MIPI add
     CamWriteCmosSensor(0x3705,0xda);
     CamWriteCmosSensor(0x370a,0x80);
     CamWriteCmosSensor(0x370c,0x00);
     ...
     CamWriteCmosSensor(0x3801,0x54);  //...............from 50
     CamWriteCmosSensor(0x3803,0x0c);  //MIPI add
     CamWriteCmosSensor(0x3804,0x0a);
     CamWriteCmosSensor(0x3805,0x28);  
     CamWriteCmosSensor(0x3806,0x07);
     CamWriteCmosSensor(0x3807,0x9c);   
     CamWriteCmosSensor(0x3808,0x0a);
     CamWriteCmosSensor(0x3809,0x28);    
     CamWriteCmosSensor(0x380a,0x07);
     CamWriteCmosSensor(0x380b,0x9c);
     ...
     CamWriteCmosSensor(0x3824,0x01);
     CamWriteCmosSensor(0x3825,0xb4);
     CamWriteCmosSensor(0x3827,0x0a);    //modify from 0c
     CamWriteCmosSensor(0x3A08,0x12);
     CamWriteCmosSensor(0x3A09,0x70);
     CamWriteCmosSensor(0x3A0A,0x0f);
     CamWriteCmosSensor(0x3A0B,0x60);
     ...
     CamWriteCmosSensor(0x401d,0x28);
...
static void OV5650MipiSetPrePCLK(kal_uint32 Clk)
{
  static const kal_uint8 ClkSetting[][5] =
  {
    {0x8A,0x00,0x0F,0x04}, /* MCLK: 26M, SYS CLK: 65M, PCLK: 65M  */
    {0x8A,0x00,0x0C,0x04}, /* MCLK: 26M, SYS CLK: 52M, PCLK: 52M */
    {0x8A,0x10,0x14,0x04}, /* MCLK: 26M, SYS CLK: 43333333M, PCLK: 43333333M */
    {0x8A,0x10,0x0C,0x04}, /* MCLK: 26M, SYS CLK: 43333333M, PCLK: 43333333M */
  };
  kal_uint8 i = 0;

  OV5650Mipi_g_fPV_PCLK = Clk;
  switch (OV5650Mipi_g_fPV_PCLK)
  {
  case 65000000: i = 0; break;
  case 52000000: i = 1; break;
  case 43333333: i = 2; break;
...
*/
  CamWriteCmosSensor(0x300F, ClkSetting[i][0]);      //300F[1:0]  10: RAW8   11:RAW10
  CamWriteCmosSensor(0x3010, ClkSetting[i][1]);
  CamWriteCmosSensor(0x3011, ClkSetting[i][2]);    
  CamWriteCmosSensor(0x3012, ClkSetting[i][3]);

  CamWriteCmosSensor(0x3008,0x02);
}


static void OV5650MipiSetCAPPCLK(kal_uint32 Clk)
{
  static const kal_uint8 ClkSetting[][5] =
  {
    {0x8B,0x00,0x0F,0x04}, /* MCLK: 26M, SYS CLK: 52M, PCLK: 52M  */
    {0x8B,0x00,0x14,0x04}, /* MCLK: 26M, SYS CLK: 69333333M, PCLK: 69333333M */
    {0x8B,0x00,0x19,0x04}, /* MCLK: 26M, SYS CLK: 86666666M, PCLK: 86666666M */
    {0x8B,0x00,0x08,0x04}, /* MCLK: 26M, SYS CLK: 26M, PCLK: 26M*/
  };
  kal_uint8 i = 0;

  OV5650Mipi_g_fCP_PCLK = Clk;
  switch (OV5650Mipi_g_fCP_PCLK)
  {
  case 52000000: i = 0; break;
  case 69333333: i = 1; break;
  case 86666666: i = 2; break;
  case 26000000: i = 3; break;
  default: ASSERT(0);
  }
...
*
*************************************************************************/
static void OV5650MipiSetShutter(kal_uint16 iShutter)
{
  OV5650Mipi_ExpLines = iShutter;
  
  OV5650MipiWriteShutter(iShutter);
 
#ifdef __OV5650Mipi_DEBUG_TRACE__
   kal_prompt_trace(MOD_ENG,"OV5650MipiSetShutter ...........iShutter = %d",iShutter);
#endif 
}
...
*
*************************************************************************/
static kal_uint16 OV5650MipiSetGain(kal_uint16 iGain)
{
  const kal_uint16 iBaseGain = OV5650MipiReg2Gain(pNvramSensorData->cct[OV5650_MIPI_INDEX_BASE_GAIN].para);
  const kal_uint16 iGain2Set = iGain * iBaseGain / BASEGAIN;
  kal_uint8 iReg = OV5650MipiGain2Reg(iGain2Set);

  CamWriteCmosSensor(0x350b, iReg);//CamWriteCmosSensor(0x3001, iReg);
  
#ifdef __OV5650Mipi_DEBUG_TRACE__
  kal_prompt_trace(MOD_ENG,"OV5650Mipi_SetGain ...........iGain = %d,iReg = %x",iGain,iReg);
#endif 
  
  return OV5650MipiReg2Gain(iReg) * BASEGAIN / iBaseGain;  
}

static kal_uint16 OV5650MipiReadGain(void)
{  
  kal_uint8 iReg = 0;

 iReg = CamReadCmosSensor(0x350b);

  return OV5650MipiReg2Gain(iReg);
}

static void OV5650MipiSetFlashlight (kal_bool enable)
{
  //flashlight_power_on(enable);
  FlashlightPowerOn(OV5650MipiSensorIdx, enable);
}
...
static void OV5650MipiCameraParaToSensor(nvram_camera_para_struct *pCameraPara)
{
  kal_uint32  i;
  
  for(i=0; (0xFFFFFFFF!=pCameraPara->SENSOR.reg[i].addr); i++)
  {
    OV5650MipiPageWriteCmosSensor(pCameraPara->SENSOR.reg[i].addr, pCameraPara->SENSOR.reg[i].para);
  }
...
static void OV5650MipiSensorToCameraPara(nvram_camera_para_struct *pCameraPara)
{
  kal_uint32  i;

  for(i=0; (0xFFFFFFFF!=pCameraPara->SENSOR.reg[i].addr)&&(i<ENGINEER_START_ADDR); i++)
  {
    pCameraPara->SENSOR.reg[i].para = OV5650MipiPageReadCmosSensor(pCameraPara->SENSOR.reg[i].addr);
  }
  for(i=ENGINEER_START_ADDR; (0xFFFFFFFF!=pCameraPara->SENSOR.reg[i].addr); i++)
  {
    pCameraPara->SENSOR.reg[i].para = OV5650MipiPageReadCmosSensor(pCameraPara->SENSOR.reg[i].addr);
...
static void  OV5650MipiGetSensorGroupCount(kal_int32* sensor_count_ptr)
{
  *sensor_count_ptr = OV5650_MIPI_GROUP_TOTAL_NUMS;
}

static void OV5650MipiGetSensorGroupInfo(kal_uint16 group_idx, kal_int8* group_name_ptr, kal_int32* item_count_ptr)
{
   switch (group_idx)
   {
  case PRE_GAIN:
...
static void OV5650MipiGetSensorItemInfo(kal_uint16 group_idx,kal_uint16 item_idx, CAL_CCT_SENSOR_ITEM_INFO_STRUCT* info_ptr)
{
  kal_uint8 temp_reg=0x00;
  kal_uint16 temp_gain=0;

  CamWriteCmosSensor(OV5650_MIPI_PAGE_SETTING_REG,0x01);
  switch (group_idx)
  {
    case PRE_GAIN:
      switch (item_idx)
      {
        case SENSOR_BASEGAIN:
          ...
          temp_gain = OV5650MipiReg2Gain(pNvramSensorData->cct[SENSOR_BASEGAIN].para);
...
static kal_bool OV5650MipiSetSensorItemInfo(kal_uint16 group_idx, kal_uint16 item_idx, kal_int32 ItemValue)
{
   kal_uint8 temp_reg;
    ...
    case PRE_GAIN:
      switch (item_idx)
      {
        case SENSOR_BASEGAIN:
          ...
          temp_reg = OV5650MipiGain2Reg(temp_gain);        
...
*
*************************************************************************/
static void OV5650MipiGetSensorInfo(P_IMAGE_SENSOR_GET_SENSOR_INFO_STRUCT pSensorInfo)
{
  pSensorInfo->SensorId = OV5650_MIPI_SENSOR_ID;
  ...
  pSensorInfo->PreviewWidth = OV5650_MIPI_IMAGE_SENSOR_PV_WIDTH;
  pSensorInfo->PreviewHeight = OV5650_MIPI_IMAGE_SENSOR_PV_HEIGHT;
  pSensorInfo->FullWidth = OV5650_MIPI_IMAGE_SENSOR_FULL_WIDTH;
  pSensorInfo->FullHeight = OV5650_MIPI_IMAGE_SENSOR_FULL_HEIGHT;
  pSensorInfo->SensorAfSupport = KAL_FALSE;
  pSensorInfo->SensorFlashSupport = KAL_FALSE;
}

void OV5650MipiGetMipiInfo(P_IMAGE_SENSOR_MIPI_PARA_IN_STRUCT pParaIn, P_IMAGE_SENSOR_MIPI_PARA_OUT_STRUCT pParaOut)
{
        ...
        ||(CAL_SCENARIO_CCT_CAMERA_PREVIEW == pParaIn->ScenarioMode))
    {
        ...
        pParaOut->LowPwr2HighSpeedDelayCount = 12;
    }
    ...
    if ((pParaIn->TargetWidth<=OV5650_MIPI_IMAGE_SENSOR_PV_WIDTH)
        &&(pParaIn->TargetHeight<=OV5650_MIPI_IMAGE_SENSOR_PV_HEIGHT))
        {
    ...
    else
        {
            ...
            if (100==pParaIn->DigitalZoomFactor) /* 1x capture */
                ...
                {
                pParaOut->MipiLaneNum = 1;
                pParaOut->LowPwr2HighSpeedDelayCount = 30;
                }
...
*
*************************************************************************/
static kal_uint16 OV5650MipiPowerOn(void)
{
  kal_uint16 SensorId = NULL_SENSOR_ID;
  kal_uint8 i = 0;
  CamPdnPinCtrl(OV5650MipiSensorIdx, 1);
  CisModulePowerOn(OV5650MipiSensorIdx, KAL_TRUE);
  CameraSccbOpen(CAMERA_SCCB_SENSOR, OV5650_MIPI_HW_SCCB_SLAVE_ADDR, CAMERA_SCCB_16BIT, CAMERA_SCCB_8BIT, 320);
  
  kal_sleep_task(2); // To wait for Stable Power

  CamPdnPinCtrl(OV5650MipiSensorIdx, 0);

  /* Reset the sensor */
  CamRstPinCtrl(OV5650MipiSensorIdx, 1); //Sensor RESET pin high
  kal_sleep_task(4);

  CamRstPinCtrl(OV5650MipiSensorIdx, 0); //Sensor RESET pin low
 ...
 CamRstPinCtrl(OV5650MipiSensorIdx, 1);  //Sensor RESET pin high
  ...
  for (i=0;i<3;i++)
  {
    ...
    if (SensorId==OV5650_MIPI_SENSOR_ID)
    break;
  }
  
#ifdef __OV5650Mipi_DEBUG_TRACE__
	 kal_prompt_trace(MOD_ENG,"OV5650MipiPowerOn ...........SensorId = %x",SensorId);
#endif 
  return SensorId;
} /* OV5650MipiPowerOn() */
...
*
*************************************************************************/
static kal_uint32 OV5650MipiDetectSensorId(void)
{
  MM_ERROR_CODE_ENUM OV5650MipiSensorClose(void);
  kal_uint16 OVOV5650MipiPowerOn();
  kal_uint16 SensorId;
  IMAGE_SENSOR_INDEX_ENUM AnotherSensorIdx;
  kal_int8 i, j;
  
  if (IMAGE_SENSOR_MAIN == OV5650MipiSensorIdx)
  {
  ...
  for (i = 1; i >= 0; i--)
  {
    for (j = 1; j >= 0; j--)
    {
      ...
      SensorId = OV5650MipiPowerOn();
      OV5650MipiSensorClose();
#ifdef __OV5650Mipi_DEBUG_TRACE__
      kal_wap_trace(MOD_ENG,TRACE_INFO,"OV5650MipiDetectSensorId:%x",SensorId);
#endif
      if (OV5650_MIPI_SENSOR_ID == SensorId)
      {
        return OV5650_MIPI_SENSOR_ID;
      }
    }
  }
...
static MM_ERROR_CODE_ENUM OV5650MipiSensorOpen(void)
{
  IspSetInterruptTriggerDelayLines(1);  // 0: VD INT, others: TG INT & start line
  
  if (OV5650_MIPI_SENSOR_ID != OV5650MipiPowerOn()) 
  	return MM_ERROR_SENSOR_READ_ID_FAIL;
...
*
*************************************************************************/
static void OV5650MipiPreview(CAL_SCENARIO_ENUM ScenarioId,
                               P_IMAGE_SENSOR_SCENARIO_PARA_IN_STRUCT pParaIn,
                               P_IMAGE_SENSOR_SCENARIO_PARA_OUT_STRUCT pParaOut)
{
    OV5650MipiStartX = 2;
	OV5650MipiStartY = 2;
	OV5650Mipi_gVGAmode = KAL_TRUE;

    OV5650MipiSetSXVGA();  //set preview mode 
    
    //OV5650MipiSetPrePCLK(52000000);
	
    OV5650MipiSetPvMirrorFlip(pParaIn->ImageMirror);

    OV5650Mipi_dummy_pixels = 555;    //don't modify this dummy 
    OV5650Mipi_dummy_lines = 0; 

    g_iOV5650Mipi_Mode = OV5650_MIPI_MODE_PREVIEW;

    OV5650MipiSetDummy(OV5650Mipi_dummy_pixels, OV5650Mipi_dummy_lines);
	
	g_iPreview_Column_Pixel = OV5650_MIPI_XGA_MODE_WITHOUT_DUMMY_PIXELS + OV5650Mipi_dummy_pixels;

    pParaOut->GrabStartX = OV5650MipiStartX;
    pParaOut->GrabStartY = OV5650MipiStartY;
    pParaOut->ExposureWindowWidth = OV5650_MIPI_IMAGE_SENSOR_PV_WIDTH - OV5650MipiStartX  - 16;
    pParaOut->ExposureWindowHeight = OV5650_MIPI_IMAGE_SENSOR_PV_HEIGHT - OV5650MipiStartY - 12;

    //OV5650MipiWriteShutter(OV5650Mipi_ExpLines);
#ifdef __OV5650Mipi_DEBUG_TRACE__
		 kal_prompt_trace(MOD_ENG,"OV5650MipiPreview ...........OV5650Mipi_ExpLines = %d",OV5650Mipi_ExpLines);
#endif 


}

static void OV5650MipiGetPreviewFlashlightInfo(P_FLASHLIGHT_PREVIEW_PARA_STURCT pPreviewInfo)
{
  pPreviewInfo->pixels = OV5650_MIPI_PV_PERIOD_PIXEL_NUMS;
  pPreviewInfo->lines = OV5650_MIPI_IMAGE_SENSOR_PV_HEIGHT;
  pPreviewInfo->frame_lines = OV5650_MIPI_PV_PERIOD_LINE_NUMS;
}
...
*
*************************************************************************/
static void OV5650MipiCapture(CAL_SCENARIO_ENUM ScenarioId,
                               P_IMAGE_SENSOR_SCENARIO_PARA_IN_STRUCT pParaIn,
                               P_IMAGE_SENSOR_SCENARIO_PARA_OUT_STRUCT pParaOut)
{
    volatile kal_uint16 Shutter=OV5650Mipi_ExpLines;	
	float temp_iShutter = OV5650Mipi_ExpLines;
    kal_uint16 iCapture_Column_Pixel = 0, iShutter = OV5650Mipi_ExpLines;
    ...
    if ((pParaIn->ImageTargetWidth<=OV5650_MIPI_IMAGE_SENSOR_PV_WIDTH)&&
        (pParaIn->ImageTargetHeight<=OV5650_MIPI_IMAGE_SENSOR_PV_HEIGHT))
    {
      OV5650Mipi_gVGAmode = KAL_TRUE;
	  
      OV5650Mipi_dummy_pixels= 555;         
      OV5650Mipi_dummy_lines = 0;

      g_iOV5650Mipi_Mode = OV5650_MIPI_MODE_CAPTURE;
  	  
	  iCapture_Column_Pixel = OV5650_MIPI_XGA_MODE_WITHOUT_DUMMY_PIXELS + OV5650Mipi_dummy_pixels;
	  
	  pParaOut->GrabStartX = OV5650MipiStartX;
	  pParaOut->GrabStartY = OV5650MipiStartY;
	  pParaOut->ExposureWindowWidth = OV5650_MIPI_IMAGE_SENSOR_PV_WIDTH - OV5650MipiStartX  - 14;
	  pParaOut->ExposureWindowHeight = OV5650_MIPI_IMAGE_SENSOR_PV_HEIGHT - OV5650MipiStartY - 10;
   
    }   
    
    else 
    {
     OV5650Mipi_gVGAmode = KAL_FALSE;
	 OV5650MipiStartX = 33;
	 OV5650MipiStartY = 2;
	 
     OV5650MipiSetUXGA();
     OV5650MipiSetCapMirrorFlip(pParaIn->ImageMirror);
	 
     //OV5650MipiSetCAPPCLK(69333333);
      ...
      if (100==pParaIn->ZoomFactor) /* 1x capture */
        ...
        {
          OV5650MipiSetCAPPCLK(52000000);
          OV5650Mipi_dummy_pixels= 1100;        
        }
      else if(pParaIn->ZoomFactor<=400)/* 2x~4x capture */
        {
          OV5650MipiSetCAPPCLK(52000000);
          OV5650Mipi_dummy_pixels= 5000;           
        }
      else /* 4x~8x capture */
        {
          OV5650MipiSetCAPPCLK(26000000);
          OV5650Mipi_dummy_pixels= 5000;            
        }
     ...
     #endif
     OV5650Mipi_dummy_lines = 24;  
	 
    iGrabWidth = OV5650_MIPI_IMAGE_SENSOR_FULL_WIDTH - OV5650MipiStartX  - 16;
    iGrabHeight = OV5650_MIPI_IMAGE_SENSOR_FULL_HEIGHT- OV5650MipiStartY - 12;
   
    iCapture_Column_Pixel = OV5650_MIPI_QXGA_MODE_WITHOUT_DUMMY_PIXELS + OV5650Mipi_dummy_pixels;
  ...
  temp_iShutter = (temp_iShutter * OV5650Mipi_g_fCP_PCLK / OV5650Mipi_g_fPV_PCLK) * g_iPreview_Column_Pixel / iCapture_Column_Pixel;
  
  if(OV5650Mipi_gVGAmode == KAL_FALSE)
	  temp_iShutter *= 4;
  ...
  OV5650MipiCapShutter=iShutter;
}

static void OV5650MipiGetCaptureFlashlightInfo(P_FLASHLIGHT_CAPTURE_SENSOR_PARA_IN_STRUCT pParaIn, P_FLASHLIGHT_CAPTURE_PARA_STRUCT pCaptureInfo)
{
    
    pCaptureInfo->default_pclk=52000000;

    pCaptureInfo->pixels = OV5650_MIPI_QXGA_MODE_WITHOUT_DUMMY_PIXELS + OV5650Mipi_dummy_pixels;
    pCaptureInfo->frame_lines = OV5650_MIPI_FULL_PERIOD_LINE_NUMS+OV5650Mipi_dummy_lines;
    pCaptureInfo->lines = OV5650_MIPI_IMAGE_SENSOR_FULL_HEIGHT;
    pCaptureInfo->shutter = OV5650MipiCapShutter;
    //Shutter = flashlight_sensor_capture_config(&flashlight_capture_para, pParaIn);    

}

static void OV5650MipiVideo(CAL_SCENARIO_ENUM ScenarioId,
                               P_IMAGE_SENSOR_SCENARIO_PARA_IN_STRUCT pParaIn,
                               P_IMAGE_SENSOR_SCENARIO_PARA_OUT_STRUCT pParaOut)
{
    OV5650MipiStartX = 2;
    ...
    OV5650Mipi_fix_video_frame_rate = pParaIn->MaxVideoFrameRate;
	
    OV5650Mipi_MAX_EXPOSURE_LINES = (kal_uint16)(((52000000*10)/OV5650Mipi_fix_video_frame_rate)/(OV5650_MIPI_PV_PERIOD_PIXEL_NUMS+OV5650Mipi_dummy_pixels));      
  if((OV5650Mipi_MAX_EXPOSURE_LINES>OV5650_MIPI_PV_PERIOD_LINE_NUMS)&&(OV5650Mipi_dummy_lines<(OV5650Mipi_MAX_EXPOSURE_LINES - OV5650_MIPI_PV_PERIOD_LINE_NUMS)))    
    OV5650Mipi_dummy_lines = OV5650Mipi_MAX_EXPOSURE_LINES - OV5650_MIPI_PV_PERIOD_LINE_NUMS;
      
  CamWriteCmosSensor(0x380e, (OV5650Mipi_MAX_EXPOSURE_LINES >> 8) & 0xFF);
  CamWriteCmosSensor(0x380f, OV5650Mipi_MAX_EXPOSURE_LINES & 0xFF);
...
static MM_ERROR_CODE_ENUM OV5650MipiBinningInfo(P_CAL_FEATURE_CTRL_STRUCT pFeatureParaIn,
...
*
*************************************************************************/
static MM_ERROR_CODE_ENUM OV5650MipiStillCaptureSize(P_CAL_FEATURE_CTRL_STRUCT pFeatureParaIn,
                                               P_CAL_FEATURE_CTRL_STRUCT pFeatureParaOut,
                                               kal_uint32 FeatureParaOutLen, kal_uint32 *pRealFeatureParaOutLen)
{
...
*
*************************************************************************/
static void OV5650MipiInitOperationPara(P_IMAGE_SENSOR_GET_INIT_OPERATION_PARA_STRUCT pInitOperPara)
{
...
*
*************************************************************************/
static void OV5650MipiDscSupportInfo(P_IMAGE_SENSOR_GET_DSC_SUPPORT_INFO_STRUCT pDscSupportInfo)
{
  kal_mem_cpy(pDscSupportInfo, &OV5650_MIPI_dsc_support_info, sizeof(camcorder_info_struct));
}

static MM_ERROR_CODE_ENUM OV5650MipiSharpness(P_CAL_FEATURE_CTRL_STRUCT pFeatureParaIn,
                                        P_CAL_FEATURE_CTRL_STRUCT pFeatureParaOut,
                                        kal_uint32 FeatureParaOutLen, kal_uint32 *pRealFeatureParaOutLen)
{
...
*
*************************************************************************/
static MM_ERROR_CODE_ENUM OV5650MipiSensorFeatureCtrl(kal_uint32 FeatureId,
                                                ...
                                                kal_uint32 FeatureParaOutLen, kal_uint32 *pRealFeatureParaOutLen)
{
  ...
  {
    return OV5650MipiWebcamFeatureCtrl(FeatureId, pFeatureParaIn, pFeatureParaOut,
  ...
  case IMAGE_SENSOR_FEATURE_SET_ESHUTTER:
    OV5650MipiSetShutter(((P_IMAGE_SENSOR_COMM_DATA_STRUCT)pFeatureParaIn)->FeatureValue);
    break;
  case IMAGE_SENSOR_FEATURE_SET_GAIN:
    OV5650MipiSetGain(((P_IMAGE_SENSOR_COMM_DATA_STRUCT)pFeatureParaIn)->FeatureValue);
    break;
  case IMAGE_SENSOR_FEATURE_GET_SENSOR_INFO:
    OV5650MipiGetSensorInfo((P_IMAGE_SENSOR_GET_SENSOR_INFO_STRUCT) pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_GET_MIPI_INFO:
	OV5650MipiGetMipiInfo((P_IMAGE_SENSOR_MIPI_PARA_IN_STRUCT)pFeatureParaIn,(P_IMAGE_SENSOR_MIPI_PARA_OUT_STRUCT) pFeatureParaOut);
  ...
  case IMAGE_SENSOR_FEATURE_GET_SENSOR_ID:
    ((P_IMAGE_SENSOR_COMM_DATA_STRUCT) pFeatureParaOut)->FeatureValue=OV5650MipiDetectSensorId();
    break;
  case IMAGE_SENSOR_FEATURE_SET_CAMERA_SOURCE:
    OV5650MipiSensorIdx = ((P_IMAGE_SENSOR_SET_CAMERA_SOURCE_STRUCT) pFeatureParaIn)->ImageSensorIdx;
    break;
  case IMAGE_SENSOR_FEATURE_CTRL_NIGHTMODE:
    break;
  case IMAGE_SENSOR_FEATURE_GET_PREVIEW_FLASHLIGHT_INFO:
    OV5650MipiGetPreviewFlashlightInfo((P_FLASHLIGHT_PREVIEW_PARA_STURCT)pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_GET_CAPTURE_FLASHLIGHT_INFO:
    OV5650MipiGetCaptureFlashlightInfo((P_FLASHLIGHT_CAPTURE_SENSOR_PARA_IN_STRUCT)pFeatureParaIn, (P_FLASHLIGHT_CAPTURE_PARA_STRUCT)pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_CTRL_FLASHLIGHT:
    OV5650MipiSetFlashlight(((P_IMAGE_SENSOR_COMM_CTRL_STRUCT)pFeatureParaIn)->FeatureEnable);
    break;
  case IMAGE_SENSOR_FEATURE_GET_AE_INFO:
    break;
  case IMAGE_SENSOR_FEATURE_GET_AE_LUT:
    OV5650MipiGetAeLut((P_AE_LUT_INFO_STRUCT) pFeatureParaIn, (P_IMAGE_SENSOR_AE_LUT_STRUCT)pFeatureParaOut,
                        FeatureParaOutLen, pRealFeatureParaOutLen);
    break;
  ...
  case CAL_FEATURE_CAMERA_BINNING_INFO:
    ErrorCode=OV5650MipiBinningInfo((P_CAL_FEATURE_CTRL_STRUCT)pFeatureParaIn,
  ...
  case CAL_FEATURE_CAMERA_STILL_CAPTURE_SIZE:
    ErrorCode=OV5650MipiStillCaptureSize((P_CAL_FEATURE_CTRL_STRUCT)pFeatureParaIn,
                                 (P_CAL_FEATURE_CTRL_STRUCT)pFeatureParaOut, FeatureParaOutLen, pRealFeatureParaOutLen);
    break;
  // for YUV sensor sample code
  // the following are for YUV sensor only
  case CAL_FEATURE_CAMERA_SHARPNESS:
  // please refer the sample code of OV5650MipiSharpness();
    ErrorCode=OV5650MipiSharpness((P_CAL_FEATURE_CTRL_STRUCT)pFeatureParaIn,
                                   (P_CAL_FEATURE_CTRL_STRUCT)pFeatureParaOut, FeatureParaOutLen, pRealFeatureParaOutLen);
    break;
  ...
  case CAL_FEATURE_CCT_OP_CAMERA_PARA_TO_SENSOR:
    OV5650MipiCameraParaToSensor(pFeatureParaIn);
    break;
  case CAL_FEATURE_CCT_OP_SENSOR_TO_CAMERA_PARA:
    OV5650MipiSensorToCameraPara(pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_GET_INIT_OPERATION_PARA:
    OV5650MipiInitOperationPara((P_IMAGE_SENSOR_GET_INIT_OPERATION_PARA_STRUCT)pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_GET_DSC_SUPPORT_INFO:
    OV5650MipiDscSupportInfo((P_IMAGE_SENSOR_GET_DSC_SUPPORT_INFO_STRUCT)pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_SET_SENSOR_POWER_DOWN:
	CamPdnPinCtrl(OV5650MipiSensorIdx,0);
	CamRstPinCtrl(OV5650MipiSensorIdx,0);
	break;
  ...
  case IMAGE_SENSOR_FEATURE_GET_GAIN:
    ((P_IMAGE_SENSOR_COMM_DATA_STRUCT)pFeatureParaOut)->FeatureValue=OV5650MipiReadGain();
    break;
  case IMAGE_SENSOR_FEATURE_GET_HW_DEPENDENT_SETTING:
    CamGetHWInfo(OV5650MipiSensorIdx, pFeatureParaOut);
    break;
  ...
  case IMAGE_SENSOR_FEATURE_GET_CCT_GROUP_COUNT:
    OV5650MipiGetSensorGroupCount((kal_int32 *)pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_GET_CCT_GROUP_INFO:
    OV5650MipiGetSensorGroupInfo(((P_CAL_CCT_SENSOR_GROUP_INFO_STRUCT)pFeatureParaIn)->GroupIdx,((P_CAL_CCT_SENSOR_GROUP_INFO_STRUCT)pFeatureParaIn)->GroupNamePtr, &((P_CAL_CCT_SENSOR_GROUP_INFO_STRUCT)pFeatureParaOut)->ItemCount);
    break;  
  case IMAGE_SENSOR_FEATURE_GET_CCT_ITEM_INFO:
    OV5650MipiGetSensorItemInfo(((P_CAL_CCT_SENSOR_ITEM_INFO_STRUCT)pFeatureParaIn)->GroupIdx,((P_CAL_CCT_SENSOR_ITEM_INFO_STRUCT)pFeatureParaIn)->ItemIdx, (P_CAL_CCT_SENSOR_ITEM_INFO_STRUCT)pFeatureParaOut);
    break;
  case IMAGE_SENSOR_FEATURE_SET_CCT_ITEM_INFO:
    OV5650MipiSetSensorItemInfo(((P_CAL_CCT_SENSOR_ITEM_INFO_STRUCT)pFeatureParaIn)->GroupIdx,((P_CAL_CCT_SENSOR_ITEM_INFO_STRUCT)pFeatureParaIn)->ItemIdx, ((P_CAL_CCT_SENSOR_ITEM_INFO_STRUCT)pFeatureParaIn)->ItemValue);
    break;
  default:
    ErrorCode = MM_ERROR_SENSOR_FEATURE_NOT_SUPPORT;
    break;
...
*
*************************************************************************/
static MM_ERROR_CODE_ENUM OV5650MipiSensorCtrl(CAL_SCENARIO_ENUM ScenarioId, void *pParaIn,
                                         void *pParaOut, kal_uint16 ParaOutLen)
{
  ...
  case CAL_SCENARIO_WEBCAM_CAPTURE:
    OV5650MipiPreview(ScenarioId, pParaIn, pParaOut);
    break;
  case CAL_SCENARIO_VIDEO:
    OV5650MipiVideo(ScenarioId, pParaIn, pParaOut);
    break;
  case CAL_SCENARIO_CAMERA_STILL_CAPTURE:
    OV5650MipiCapture(ScenarioId, pParaIn, pParaOut);
    break;
  default:
    break;
  }
...
*
*************************************************************************/
static MM_ERROR_CODE_ENUM OV5650MipiSensorClose(void)
{
  CisModulePowerOn(OV5650MipiSensorIdx, KAL_FALSE);
...
*
*************************************************************************/
MM_ERROR_CODE_ENUM OV5650MipiSensorFunc(P_IMAGE_SENSOR_FUNCTION_STRUCT *pfSensorFunc)
{
  static IMAGE_SENSOR_FUNCTION_STRUCT ImageSensorFuncOV5650Mipi =
  {
    OV5650MipiSensorOpen,
    OV5650MipiSensorFeatureCtrl,
    OV5650MipiSensorCtrl,
    OV5650MipiSensorClose,
  };
  
  *pfSensorFunc = &ImageSensorFuncOV5650Mipi;
}


/* AT COMMAND USE */
static kal_bool OV5650MipiAtGetValue(char *Str, kal_uint32 *Data)
{
...
kal_bool AtFuncTest(kal_char *Str)
{
    ...
    if (!OV5650MipiAtGetValue(Str, Data))
    {
        return KAL_FALSE;
    }
    if (!Data[0])
