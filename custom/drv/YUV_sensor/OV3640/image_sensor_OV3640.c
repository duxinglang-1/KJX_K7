...
static void OV3640SetClock(kal_uint32 Clk)
{
  static const kal_uint8 ClkSetting[][3] =
  {
    ...
    {0x33,0x21,0x82}, /* MCLK: 26M, SYS CLK: 56.3M, PCLK: 56.3M for XGA preview */
    {0x38,0x21,0x82}, /* MCLK: 26M, SYS CLK: 34.7M, PCLK: 34.6M for XGA preview */
#else
    {0x33,0x21,0x85}, /* MCLK: 26M, SYS CLK: 56.3M, PCLK: 22.5M for VGA preview */
    {0x38,0x21,0x85}, /* MCLK: 26M, SYS CLK: 34.7M, PCLK: 13.9M for VGA preview */
#endif
    {0x35,0x21,0x81}, /* MCLK: 26M, SYS CLK: 47.7M, PCLK: 95.3M for capture */
    {0x32,0x22,0x81}, /* MCLK: 26M, SYS CLK: 45.5M, PCLK: 91.0M for capture */
    {0x35,0x23,0x81}, /* MCLK: 26M, SYS CLK: 23.8M, PCLK: 47.7M for capture */
  };
  kal_uint8 i = 0;
#if (defined(DRV_ISP_6276_SERIES) || defined(DRV_ISP_MT6236_HW_SUPPORT))
  kal_bool SensorPclkInv = KAL_TRUE;
#endif
  ...
  */
  ...
  CamWriteCmosSensor(0x300F, ClkSetting[i][1]);
...
#endif
#ifdef OV3640_QUARTER_PREVIEW
    CamWriteCmosSensor(0x30B2, 0x19);
#else
...
#endif
    CamWriteCmosSensor(0x30B2, 0x1B);
  }
#endif
}
...
static kal_uint16 OV3640ReadShutter(void)
{
  ...
  if (Shutter > ShutterLimit) /* may be wrong at some time, should be correct */
  {
    Shutter = ShutterLimit;
  }
...
*
*************************************************************************/
static void OV3640UvAdjEnable(kal_bool Enable)
{
#ifdef OV3640_GAIN2SHUTTER
  static kal_int8 TH2, Offset;
  
  if (OV3640Sensor.FirstPv)
  {
    TH2 = -1;
  }
  if (-1 == TH2 && !Enable)
  {
    ...
    TH2 = CamReadCmosSensor(0x30BB);
    Offset = CamReadCmosSensor(0x30B9);
    ...
    /* turn off auto uv */
    CamWriteCmosSensor(0x30BB, 0x00);
    CamWriteCmosSensor(0x30B9, UvValue);
  }
  else if (TH2 != -1 && Enable)
  {
    CamWriteCmosSensor(0x30BB, TH2);
    CamWriteCmosSensor(0x30B9, Offset);
    TH2 = -1;
  }
#endif
}
...
static void OV3640CalFps(void)
{
  ...
  else
  {
    ...
    case 1:
        ...
        /* fix frame rate for avoid AE flash in this case */
        OV3640Sensor.AutoCtrl2 &= 0xF7;
        CamWriteCmosSensor(0x3014, OV3640Sensor.AutoCtrl2);
        /* extra line can not auto be set to 0 when fix frame rate */
        CamWriteCmosSensor(0x302D, 0x00); /* AEC VTS output */
...
*
* DESCRIPTION
*    This function calculate capture Shutter and apply to sensor
*
* PARAMETERS
*    PvShutter, PvGain, PvClk, PvLineLength, MaxCapFps
*
* RETURNS
*    None
*
...
*
*************************************************************************/
static void OV3640CapShutter(kal_uint16 PvShutter, kal_uint16 PvGain, kal_uint32 PvClk, kal_uint16 PvLineLength, kal_uint16 MaxCapFps)
{
  kal_uint32 CapShutter, CapLineLength;
  kal_uint8 Mul = 1;
  
#ifdef OV3640_GAIN2SHUTTER
  while (PvGain >= 0x40 * 2) /* reduce gain and increase exposure */
  {
    PvGain >>= 1;
    Mul <<= 1;
    break; /* just reduce 2x */
  }
  if (Mul > 1)
  {
    const kal_uint16 PvFps = PvClk * OV3640_FPS(1) / (PvLineLength * PvShutter);
    
    /* if frame rate is too slow, do not do gain2shutter function */
    if (PvFps > OV3640_FPS(4) * Mul)
    {
      OV3640WriteGain(PvGain);
    }
    else
    {
      Mul = 1;
...
#endif
  CapLineLength = OV3640_CAPTURE_CLK * OV3640_FPS(1) / (OV3640_FULL_PERIOD_LINE_NUMS * MaxCapFps);
  if (CapLineLength < OV3640_FULL_PERIOD_PIXEL_NUMS)
  {
    CapLineLength = OV3640_FULL_PERIOD_PIXEL_NUMS;
  }
  if (CapLineLength > OV3640_MAX_LINELENGTH) /* register limitation */
  {
    ASSERT(0);
  }
  /* shutter translation */
  CapShutter = OV3640HalfAdjust(PvShutter * (OV3640_CAPTURE_CLK >> 14), PvClk >> 14);
  CapShutter = OV3640HalfAdjust(CapShutter * PvLineLength * Mul, CapLineLength);
  CapShutter <<= 1; /* for preview binning mode */
  OV3640SetDummy(CapLineLength - OV3640_FULL_PERIOD_PIXEL_NUMS, 0);
  OV3640WriteShutter(CapShutter);
}
...
*
*************************************************************************/
static MM_ERROR_CODE_ENUM OV3640Saturation(P_CAL_FEATURE_CTRL_STRUCT In, P_CAL_FEATURE_CTRL_STRUCT Out)
{
  if (CAL_FEATURE_QUERY_OPERATION == In->FeatureCtrlCode)
  {
  ...
  else if (CAL_FEATURE_SET_OPERATION == In->FeatureCtrlCode)
  {
    ...
    case CAM_SATURATION_HIGH:
        CamWriteCmosSensor(0x3358, 0x50); /* saturation U */
        CamWriteCmosSensor(0x3359, 0x50); /* saturation V */
        break;
    case CAM_SATURATION_MEDIUM:
        CamWriteCmosSensor(0x3358, 0x40); /* saturation U */
        CamWriteCmosSensor(0x3359, 0x40); /* saturation V */
        break;
    case CAM_SATURATION_LOW:
        CamWriteCmosSensor(0x3358, 0x30); /* saturation U */
        CamWriteCmosSensor(0x3359, 0x30); /* saturation V */
        break;
    default:
        return MM_ERROR_SENSOR_FEATURE_NOT_SUPPORT;
    }
...
*
*************************************************************************/
static MM_ERROR_CODE_ENUM OV3640Sharpness(P_CAL_FEATURE_CTRL_STRUCT In, P_CAL_FEATURE_CTRL_STRUCT Out)
{
  if (CAL_FEATURE_QUERY_OPERATION == In->FeatureCtrlCode)
  {
  ...
  else if (CAL_FEATURE_SET_OPERATION == In->FeatureCtrlCode)
  {
    ...
    case CAM_SHARPNESS_HIGH:
        CamWriteCmosSensor(0x332D, 0x46);
        break;
    case CAM_SHARPNESS_MEDIUM:
        CamWriteCmosSensor(0x332D, 0x44);
        break;
    case CAM_SHARPNESS_LOW:
        CamWriteCmosSensor(0x332D, 0x42);
        break;
    default:
        return MM_ERROR_SENSOR_FEATURE_NOT_SUPPORT;
    }
...
static void OV3640InitialSetting(void)
{
  ...
  CamWriteCmosSensor(0x30B2, 0x18);
  /* pixel shift: resolve internal clock shift at high comsumption */
  CamWriteCmosSensor(0x3085, 0x20);
#if ((defined(MT6276) && defined(DRV_ISP_6276_SERIES)) || defined(DRV_ISP_MT6236_HW_SUPPORT))
  CamWriteCmosSensor(0x3600, 0xD0);
#else
  CamWriteCmosSensor(0x3600, 0xC0);
#endif
  ...
  CamWriteCmosSensor(0x3343, 0x1B);
  CamWriteCmosSensor(0x3344, 0x3D);
  CamWriteCmosSensor(0x3345, 0x58); /* 0x3343 + 0x3344 = 0x3345 */
  CamWriteCmosSensor(0x3346, 0x4E); /* 0x3347 + 0x3348 = 0x3346 */
  CamWriteCmosSensor(0x3347, 0x40);
  CamWriteCmosSensor(0x3348, 0x0E);
  ...
  CamWriteCmosSensor(0x3355, 0x06); /* SDE */
...
static void OV3640GetSensorInfo(P_IMAGE_SENSOR_GET_SENSOR_INFO_STRUCT Info)
{
  ...
  Info->FullWidth = OV3640_IMAGE_SENSOR_FULL_WIDTH_DRV;
  Info->FullHeight = OV3640_IMAGE_SENSOR_FULL_HEIGHT_DRV;
...
static void OV3640InitOperationPara(P_IMAGE_SENSOR_GET_INIT_OPERATION_PARA_STRUCT Para)
{
  ...
  Para->PreviewDisplayWaitFrame = 16;
}
...
*
*************************************************************************/
static void OV3640GetEvAwbRef(P_SENSOR_AE_AWB_REF_STRUCT Ref)
{
...
*
*************************************************************************/
static void OV3640GetCurAeAwbInfo(P_SENSOR_AE_AWB_CUR_STRUCT Info)
{
    Info->SensorAECur.AeCurShutter = OV3640ReadShutter();
    Info->SensorAECur.AeCurGain = OV3640ReadGain() * 2; /* 128 base */
    CamWriteCmosSensor(0x33A0, 0x00); /* read R gain, set R gain index */
    Info->SensorAwbGainCur.AwbCurRgain = CamReadCmosSensor(0x33CA) << 1; /* 128 base */
    CamWriteCmosSensor(0x33A0, 0x02); /* read B gain, set B gain index */
    Info->SensorAwbGainCur.AwbCurBgain = CamReadCmosSensor(0x33CA) << 1; /* 128 base */
}
...
{
  kal_bool PreMode = OV3640Sensor.PvMode;
  ...
  OV3640UvAdjEnable(KAL_TRUE);
  
  Out->WaitStableFrameNum = 0;
  /* in colorinv, should delay more time to avoid flash as AE working  */
  if (!OV3640Sensor.FirstPv && !PreMode && CAM_EFFECT_ENC_COLORINV == In->ImageEffect)
  {
    Out->WaitStableFrameNum = 5;
  }
  OV3640Sensor.FirstPv = KAL_FALSE;
...
*
* DESCRIPTION
*    This function set to full size preview mode
*
* PARAMETERS
*    Id, In, Out
*
...
*
*************************************************************************/
static void OV3640FullPreview(CAL_SCENARIO_ENUM Id, P_IMAGE_SENSOR_SCENARIO_PARA_IN_STRUCT In, P_IMAGE_SENSOR_SCENARIO_PARA_OUT_STRUCT Out)
{
  kal_uint16 DummyPixel;

  OV3640Sensor.NightMode = 0xFF; /* force init night mode */
  if (OV3640Sensor.FirstPv)
  {
    OV3640Sensor.PvMode = KAL_TRUE;
  }
  OV3640Sensor.VideoMode = KAL_FALSE;
  OV3640Sensor.Fps = In->NightMode ? OV3640_FPS(7.5) : OV3640_FPS(15);
  DummyPixel = 0;
  OV3640SetMirror(In->ImageMirror);
  OV3640SetClock(OV3640_CAPTURE_CLK);
  OV3640OutputWindow(KAL_FALSE, OV3640_IMAGE_SENSOR_FULL_WIDTH_DRV, OV3640_IMAGE_SENSOR_FULL_HEIGHT_DRV);
  OV3640SetDummy(DummyPixel, 0);
  if (CAM_BANDING_50HZ == In->BandingFreq)
  {
    OV3640Sensor.AutoCtrl2 |= 0x80;
  ...
  Out->WaitStableFrameNum = 0;
  OV3640Sensor.FirstPv = KAL_FALSE;
  Out->GrabStartX = OV3640_FULL_X_START;
  Out->GrabStartY = OV3640_FULL_Y_START;
  Out->ExposureWindowWidth = OV3640_IMAGE_SENSOR_FULL_WIDTH;
  Out->ExposureWindowHeight = OV3640_IMAGE_SENSOR_FULL_HEIGHT;
}
#endif
...
static void OV3640Capture(CAL_SCENARIO_ENUM Id, P_IMAGE_SENSOR_SCENARIO_PARA_IN_STRUCT In, P_IMAGE_SENSOR_SCENARIO_PARA_OUT_STRUCT Out)
{
  ...
  if (In->ImageTargetWidth <= OV3640_IMAGE_SENSOR_PV_WIDTH_DRV && In->ImageTargetHeight <= OV3640_IMAGE_SENSOR_PV_HEIGHT_DRV)
  {
  ...
  {
    const kal_uint32 PvClk = OV3640Sensor.SysClk;
    const kal_uint16 PvLineLength = OV3640Sensor.LineLength;
    
    OV3640AeEnable(KAL_FALSE);
    OV3640AwbEnable(KAL_FALSE);
    OV3640UvAdjEnable(KAL_FALSE);
    ...
    OV3640CapShutter(OV3640Sensor.Shutter, OV3640Sensor.Gain, PvClk, PvLineLength, In->MaxVideoFrameRate);
...
static MM_ERROR_CODE_ENUM OV3640SensorFeatureCtrl(kal_uint32 Id, void *In, void *Out, kal_uint32 OutLen, kal_uint32 *RealOutLen)
{
  ...
  case CAL_FEATURE_CAMERA_SATURATION:
    ErrCode = OV3640Saturation(In, Out);
    break;
  case CAL_FEATURE_CAMERA_SHARPNESS:
    ErrCode = OV3640Sharpness(In, Out);
    break;
  ...
  case CAL_FEATURE_CAMERA_IMAGE_EFFECT:
    ...
    break;
#ifdef __ZERO_SHUTTER_DELAY_SUPPORT__
  case CAL_FEATURE_CAMERA_ZSD_CAPTURE_SIZE: /* the same with still capture size */
#endif
