...
#include "custom_em.h"
#ifdef __KJX_FUN__	//xb add 2017-04-18
#include "Kjx_prin.h"
#endif
...
kal_uint8 custom_cfg_vbat_level_regulator( kal_uint32 measure_voltage, kal_uint8 *last_level, kal_uint8 *disp_level, kal_uint8 *hit_count )
{
...
#ifdef __KJX_FUN__	//xb add 2017-04-19 增加电池电压偏移值的修正
	...
	//LOGD("VBAT_GROWUP_PENALTY:%d, measure_voltage:%d, level:%d, last_level:%d, disp_level:%d, hit_count:%d", VBAT_GROWUP_PENALTY, measure_voltage, level, *last_level, *disp_level, *hit_count);
#endif
	...
	kal_prompt_trace(MOD_UEM, "Begin: [level %d] [last %d] [disp %d] [hit %d]", level, *last_level, *disp_level, *hit_count);
	...
	kal_prompt_trace(MOD_UEM, "Penalty: [level %d] [last %d] [disp %d] [hit %d]", level, *last_level, *disp_level, *hit_count);
#endif
	...
	kal_prompt_trace(MOD_UEM, "End: [level %d] [last %d] [disp %d] [hit %d]", level, *last_level, *disp_level, *hit_count);

	*last_level = level;

#if defined(__KJX_LOG_OUTPUT__)	
	//LOGD("BATTERY_LOW_WARNING:%d, level:%d, last_level:%d, disp_level:%d", BATTERY_LOW_WARNING, level, *last_level, *disp_level);
#endif
