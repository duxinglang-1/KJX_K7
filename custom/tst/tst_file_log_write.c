...
void trc_WriteToFile(kal_uint8* buffer, kal_uint32 size, kal_uint8 isFrameHeaderPacket)
{
...
void tst_file_write_logacc_logs(void)
{
  #if defined(__LOGACC_ENABLE__) && !defined(UNIT_TEST) 
    ...
    //Activate lga hisr
    //tst_lga_activate_hisr(TST_HISR_LGA);
...
void trc_WriteToFile(kal_uint8* buffer, kal_uint32 size, kal_uint8 isFrameHeaderPacket){}
void tst_file_write_logacc_logs(void){}
kal_bool tst_file_check_lga_data_saved(void){return KAL_TRUE;}
