...
static bool bIsSleepHandleInit = 0;
...
void vApiAudioSetFmRdoDetectorMajorTh (UINT32 u4Index);

//#include "drv_comm.h"

/* FM radio scanning set threshold
*/
#define FM_SCAN_UP 0x500000
#define FM_SCAN_DOWN 0x100000
#define FM_SCAN_RES 128
#define FM_SCAN_STEP ((FM_SCAN_UP - FM_SCAN_DOWN) / FM_SCAN_RES)

static int8 fm_scan_eng_val = 0;
static bool fm_scan_eng_valid = 0; // 1 --> enter engineer mode once
...
static void MT5192_FM_SetTH (int8 i1Val)
{
	...
	vApiAudioSetFmRdoDetectorMajorTh (u4Th);
}

static bool chip_valid=0;
static bool  _is_fm_on = 0;
...
static bool MT5192_FM_GetRSSI(kal_uint32* power)
{
	...
	ret=fgApiAudioIsFmRdoDetectDone()?bApiAudioGetFmRdoDetectStatus(power):0;
	*power >>= 8;
	return ret;
}
...
static void MT5192_FM_SetFreq(int32 Freq)
{
	int count=0;
	//DANIEL_20091210
	//1. Tuner API is needed
	/* MT5162E FPGA emulation
	87.5--> 787.5 MHz
	108.0 --> 808.0 MHz
	*/
	vApiMT5192_FmRdoSetFreq(Freq,1);
	//2. Trigger FM Radio Detecter
	if (u1ApiAudioStartFmRdoDetect(0))
	{
		//3. Wait FM Radio Detect Done
		while (!fgApiAudioIsFmRdoDetectDone() && (count<200))
		{
