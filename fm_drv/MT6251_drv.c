...
extern kal_uint8 isEarphonePlugged;
#endif
...
void dcl_pmu_vfm_enable(kal_bool enable)
{
...
void dcl_pmu_set_vfm(PMU_VOLTAGE_ENUM vol)
{
 ...
 * MT6251 FM SUBSYS power on/off sequnce
 * power on:  
 *************************************************************/
void MT6251_FMSYS_PowerOn(void)
{
...
kal_bool MT6251_ReadByte(kal_uint8 addr, kal_uint16 *data)
{
    kal_uint32 active;
...
kal_bool MT6251_WriteByte(kal_uint8 addr, kal_uint16 data)
{
    kal_uint32 active;
...
{
	kal_uint16 dataRead;
	MT6251_ReadByte(FM_MAIN_CTRL, &dataRead);
	MT6251_WriteByte(FM_MAIN_CTRL, (dataRead&0xFFFE)|TUNE);
}
...
void MT6251_SEEK_ON()
{
	kal_uint16 dataRead;
	MT6251_ReadByte(FM_MAIN_CTRL, &dataRead);
	MT6251_WriteByte(FM_MAIN_CTRL, (dataRead&0xFFFD)|SEEK);
}
...
void MT6251_SCAN_ON()
{
	kal_uint16 dataRead;
	MT6251_ReadByte(FM_MAIN_CTRL, &dataRead);
	MT6251_WriteByte(FM_MAIN_CTRL, (dataRead&0xFFFB)|SCAN);
}
...
*         
***********************************************************************/
void MT6251_Mute_OnOff(kal_uint8 mute)
{
	kal_uint16 dataRead;

	MT6251_ReadByte(0x9C,&dataRead);                           
	if (mute == 1)                                             
		MT6251_WriteByte(0x9C, (dataRead&0xFFFC)|0x0008);  
	else                                                       
		MT6251_WriteByte(0x9C, (dataRead&0xFFF7));
}
...
*         
***********************************************************************/
void MT6251_SoftMute_Mode(kal_uint8 TYPE)
{
  	MT6251_WriteByte(FM_PG_SEL, 0x0001);
	if(TYPE)
	{
  		MT6251_WriteByte(0xC8, 0x0101);
  		MT6251_WriteByte(0xA0, 0x0562);
  		MT6251_WriteByte(0xD8, 0x0000);
	}
	else
	{
  		MT6251_WriteByte(0xC8, 0x0232);
  		MT6251_WriteByte(0xA0, 0x4562);
  		MT6251_WriteByte(0xD8, 0x008B);
	}
	MT6251_WriteByte(FM_PG_SEL, 0x0000);
}
...
#if (HiSideTableSize)
void MT6251_HL_Side_Adj(kal_int16 freq)
{
...
{ 1042 };
  ...
  if(isHiSide)
  {
  	...
  	MT6251_ReadByte(0x0F, &dataRead);
  	MT6251_WriteByte(0x0F, (dataRead&0xFBFF)|0x0400);
  	MT6251_WriteByte(FM_PG_SEL, 0);
  	//Set high-side injection (DFE)
  	MT6251_ReadByte(0xCB, &dataRead);
  	MT6251_WriteByte(0xCB, (dataRead&0xFFFE)|0x0001);
  }
  else
  {
  	//Set low-side injection (AFC)
   	MT6251_ReadByte(0x0F, &dataRead);
  	MT6251_WriteByte(0x0F, dataRead&0xFBFF);
  	MT6251_WriteByte(FM_PG_SEL, 0);
  	//Set low-side injection (DFE)
  	MT6251_ReadByte(0xCB, &dataRead);
  	MT6251_WriteByte(0xCB, dataRead&0xFFFE);
  }
}
#endif
/***********************************************************************
...
*
***********************************************************************/
void MT6251_ADPLL_PowerUp(void)
{
    kal_uint16 reg_value, f16mode_enable;

    MT6251_WriteByte(0x25, 0x0403);
    //Remove the Reset_N
    MT6251_WriteByte(0x20, 0x2720);
    // change DLF loop gain  
    // Set FMCR_DLF_GAIN_A = "9"
    // Set FMCR_DLF_GAIN_B = "9"
    MT6251_WriteByte(0x22, 0x7780);
    //Configure initial I_CODE for calibration
    MT6251_WriteByte(0x25, 0x0803);
   
    // C1. Enable ADPLL DCO	Set
    // FMCR_DCO_EN = ¡§1¡¨
    MT6251_ReadByte(0x1E, &reg_value);
    f16mode_enable = reg_value&0x0200;
    MT6251_WriteByte(0x1E, f16mode_enable|0x0861);    
    
    // C2. Turn on coarse calibration
    // Set FMCR_COARSE_EN = ¡§1¡¨
    MT6251_WriteByte(0x1E, f16mode_enable|0x0863);    
    // wait 5ms 
    Delayms(5);     
    ...
    do
    {
        MT6251_ReadByte(0x23, &reg_value);
    ...
    MT6251_WriteByte(0x1E, f16mode_enable|0x0865);
    // wait 5ms 
    Delayms(5);
    ...
    MT6251_WriteByte(0x1E, f16mode_enable|0x0861);
    // Set FMCR_PLL_EN = "1"
    MT6251_WriteByte(0x1E, f16mode_enable|0x0871);
    
    // C9. Disable fm adc ck top clock gating
    // Set rgfrf_top_ck = "1"
    MT6251_WriteByte(0x2A, 0x1022);      
}
...
*
***********************************************************************/
void MT6251_ADPLL_PowerDown(void)
{
    kal_uint16 reg_value;
    
    // 2. ADPLL Power Off Sequence
    // Set rgfrf_top_ck = "0"
    MT6251_ReadByte(0x2A, &reg_value);
    reg_value &= ~0x1000; 
    MT6251_WriteByte(0x2A, reg_value); 
    // Set FMCR_OPEN_LOOP_EN = "0"
    // Set FMCR_PLL_EN = "0"
    // Set FMCR_DCO_EN = "0"
    MT6251_ReadByte(0x1E, &reg_value);
    reg_value &= ~0x0091;        
    MT6251_WriteByte(0x1E, reg_value);       
    // Set rgfrf_adpll_reset_n = "0"
    MT6251_ReadByte(0x20, &reg_value);
    reg_value &= ~0x2000;        
    MT6251_WriteByte(0x20, reg_value);               
...
*
***********************************************************************/
void MT6251_Freq_Avoid(kal_int16 freq)
{
  ...
  //isADPLL_16M = 1;
  MT6251_ReadByte(0x1E, &dataRead);
  if(((dataRead&0x0200)&&(isADPLL_16M))||(!(dataRead&0x0200)&&(!isADPLL_16M)))
  return;
  
  // Disable ADPLL
  MT6251_ADPLL_PowerDown();
  
  //Set FMCR_DCO_CK_SEL = ? (default = 0, 15.36)
  MT6251_ReadByte(0x1E, &dataRead);
  if(isADPLL_16M)
  {		
  	MT6251_WriteByte(0x1E, dataRead|0x0200);
  }
  else
  {
  	dataRead &= ~0x0200;
  	MT6251_WriteByte(0x1E, dataRead);
  }
  // Ensable ADPLL
  MT6251_ADPLL_PowerUp();
  Delayms(100);
  //Set rgfrf_enable_top_ck ="1"
  MT6251_ReadByte(0x2A, &dataRead);
  MT6251_WriteByte(0x2A, dataRead|0x1000);
  if(isADPLL_16M)
  {		
  	//Set rgf_f16mode_en = X	
  	MT6251_WriteByte(0x61, 3);
  	//Set rgf_cnt_resync_b = 0    
 	MT6251_WriteByte(0x61, 1);      
  ...
  {
  	//Set rgf_f16mode_en = X  		
  	MT6251_WriteByte(0x61, 2);
  //Set rgf_cnt_resync_b = 0
 	MT6251_WriteByte(0x61, 0);          
  //Set rgf_cnt_resync_b = 1
  	MT6251_WriteByte(0x61, 2);
  } 
 
  // Power up FM modem clk
  //MT6251_WriteByte(0x30, 0x0006);        
  //Set rgf_auto_mclk_sel_en = 1 (only if muclk desense option is enabled)
  /*MT6251_WriteByte(FM_PG_SEL, 1);
  MT6251_ReadByte(0x4A, &dataRead);
  MT6251_WriteByte(0x4A, dataRead|0x0800);
  MT6251_WriteByte(FM_PG_SEL, 0);*/
}
...
*         
***********************************************************************/
void MT6251_RampDown()
{
	kal_uint16 dataRead;	
	//Enable RGF mute
	MT6251_Mute_OnOff(1);
  	//Set softmute to fast mode
	MT6251_SoftMute_Mode(1);
	Delayms(125);
    	MT6251_ReadByte(FM_MAIN_CTRL, &dataRead);		
  	MT6251_WriteByte(FM_MAIN_CTRL, (dataRead&0xFFF0)); //clear rgf_tune/seek/scan/rxcal
	MT6251_ReadByte(FM_MAIN_CHANDETSTAT, &dataRead);		
...
*         
***********************************************************************/
static kal_bool MT6251_SetFreq(kal_int32 CurFreq)
{
	...
	CHAN = (CurFreq - 640)<<1;
	MT6251_ReadByte(0x0F, &dataRead);
	MT6251_WriteByte(0x0F, (dataRead&0xFC00)|CHAN);
       MT6251_TUNE_ON();	
	do
	{
		MT6251_ReadByte(FM_MAIN_INTR, &dataRead);
   	...
   	MT6251_WriteByte(FM_MAIN_INTR, (dataRead&~MASK_STC)|MASK_STC);//clear status flag
	//Disable mute
	MT6251_Mute_OnOff(0);
   	Delayms(125);
  	//Set softmute to normal mode
  	MT6251_SoftMute_Mode(0);
...
*         		 space:1:100k, 0:200k
***********************************************************************/
void MT6251_TUNE_HiLo(kal_int32 Freq, kal_int16 band, kal_int8 space)
{
	kal_int8 rssi;
	kal_uint16 dataRead;
	...
	MT6251_Mute_OnOff(1);      
    	MT6251_RampDown();		
      	...
      	MT6251_ReadByte(3, &dataRead);
      	MT6251_WriteByte(3, ((dataRead&0xC7FF)|(space<<13))|band);//set space(100k/200k)and band(875~1080)
        
        //Read Low-Side LO Injection
  	//R11 --> clear  D15,  clear D0/D2,  D3 is the same as default
  	MT6251_ReadByte(11, &dataRead);
  	MT6251_WriteByte(11, (dataRead&0x7FFA));
  	if (MT6251_SetFreq(Freq) == 0) 
      	{
         	ASSERT(0);
      	}    
      
       	MT6251_ReadByte(ADDR_PAMD, &dataRead);
  	rssi = (dataRead & MASK_RSSI);
  	
  	//Read Hi-Side LO Injection
  	// R11-->set D15, set D0/D2,  D3 is the same as default
  	MT6251_ReadByte(11, &dataRead);
  	MT6251_WriteByte(11, (dataRead&0x8005));
  	if (MT6251_SetFreq(Freq) == 0) 
      	{
         	ASSERT(0);
      	}  
  	...
  	{ 	
		...
		MT6251_ReadByte(11, &dataRead);
  		MT6251_WriteByte(11, (dataRead&0x7FFF)|0x0005);
  	}else{ 
		//HI
		//R11-->  set D15, clear D0/D2, D3 is the same as default
		MT6251_ReadByte(11, &dataRead);
  		MT6251_WriteByte(11, (dataRead|0x8000)&0xFFFA);
  	}
...
* retrun dBuVemf,
***********************************************************************/
kal_int16 MT6251_GetCurRSSI(void)
{
    kal_uint16 TmpReg;
    kal_int16 RSSI, dBValue;
	MT6251_ReadByte(ADDR_RSSI, &TmpReg);
...
*  parameter-->
***********************************************************************/
kal_uint16 MT6251_GetCurPamd(void)
{
   kal_uint16 TmpReg, PAMD, dBValue;
   MT6251_ReadByte(ADDR_PAMD, &TmpReg);
   /*PA=PAMD
...
*  parameter-->
***********************************************************************/
void MT6251_GetSearchFreq(kal_uint16 *pfreq)
{
...
*
*********************************************************************/
kal_bool MT6251_SetAntennaType(kal_uint8 isShortATA)
{
	if(isShortATA)
	{
		MT6251_WriteByte(0x04, 0x0145);
		MT6251_WriteByte(0x05, 0x00FF);
		MT6251_WriteByte(0x26, 0x0024);
		MT6251_WriteByte(0x2E, 0x0000);	
	}
	else
	{
		MT6251_WriteByte(0x04, 0x0142);
		MT6251_WriteByte(0x05, 0x00E7);
		MT6251_WriteByte(0x26, 0x0004);
		MT6251_WriteByte(0x2E, 0x0008);	
	}
	return KAL_TRUE;
}
...
*********************************************************************/
kal_bool MT6251_GetAntennaType(void)
{
	kal_uint16 dataRead;
	
	MT6251_ReadByte(4, &dataRead);
	if((dataRead&0x07) == 5)
...
void FMDrv_radio_item_info(kal_uint16 group_idx, kal_uint16 item_idx, kal_uint32 item_value)
{
	...
	switch (group_idx)
	{
		case mono:
			MT6251_WriteByte(FM_PG_SEL, 1);
			MT6251_ReadByte(0xe0, &dataRead); 
			if(item_value == 1)
				MT6251_WriteByte(0xe0, (dataRead&0xFFFD)|0x02);
			else
				MT6251_WriteByte(0xe0, (dataRead&0xFFFD));
			MT6251_WriteByte(FM_PG_SEL, 0);
			break;
		case stereo:
			MT6251_WriteByte(FM_PG_SEL, 1);
			...
			else
			{
				...
				{
					case Sblend_ON:
						MT6251_ReadByte(0xe1, &dataRead); 
						MT6251_WriteByte(0xe1, (dataRead&0x7FFF) |(item_idx<<15));
					break;
					case Sblend_OFF:
						MT6251_ReadByte(0xe1, &dataRead); 
						MT6251_WriteByte(0xe1, (dataRead&0x7FFF));
					break;
				}
			}
			MT6251_WriteByte(FM_PG_SEL, 0);
		...
		case RSSI_threshold:
			MT6251_WriteByte(FM_PG_SEL, 1);
			MT6251_ReadByte(0xd3, &dataRead); 
			MT6251_WriteByte(0xd3, (dataRead&0xFC00) |item_value);
			MT6251_WriteByte(FM_PG_SEL, 0);
		break;
		case HCC_Enable:
			MT6251_WriteByte(FM_PG_SEL, 1);
			MT6251_ReadByte(0xcf, &dataRead); 
			if(item_idx)
				MT6251_WriteByte(0xcf, dataRead |0x10);
			else
				MT6251_WriteByte(0xcf, dataRead&(~0x10));
			MT6251_WriteByte(FM_PG_SEL, 0);
		break;
		case PAMD_threshold:
			MT6251_WriteByte(FM_PG_SEL, 1);
			MT6251_ReadByte(0xd4, &dataRead); 
			MT6251_WriteByte(0xd4, (dataRead&0xFC00) |item_value);
			MT6251_WriteByte(FM_PG_SEL, 0);
		break;
		case Softmute_Enable:
			...
			{
				MT6251_ReadByte(0xcf, &dataRead); 
				MT6251_WriteByte(0xcf, dataRead |0x20);
			}
			else
			{
				MT6251_ReadByte(0xcf, &dataRead); 
		...
		break;
		case De_emphasis:
			MT6251_WriteByte(FM_PG_SEL, 2);
			if(item_idx == 2) //75us
			{
				MT6251_ReadByte(0xa0, &dataRead); 
				MT6251_WriteByte(0xa0, (dataRead&0xFFFC) |0x02);
			}
			else if(item_idx == 1) //50us
			{
				MT6251_ReadByte(0xa0, &dataRead); 
				MT6251_WriteByte(0xa0, (dataRead&0xFFFC) |0x01);		
			}
			else if(item_idx == 0) //0us
			{
				MT6251_ReadByte(0xa0, &dataRead); 
				MT6251_WriteByte(0xa0, (dataRead&0xFFFC) |0x00);		
			}
			MT6251_WriteByte(FM_PG_SEL, 0);
		break;
		case HL_Side:
		...
		case Dynamic_Limiter:
			MT6251_WriteByte(FM_PG_SEL, 1);
				MT6251_ReadByte(0xfa, &dataRead); 
			if(item_idx)
				MT6251_WriteByte(0xfa, (dataRead&0xFFF7) |0x00);
			else
				MT6251_WriteByte(0xfa, (dataRead&0xFFF7) |0x08);		
			MT6251_WriteByte(FM_PG_SEL, 0);
		break;
		case Softmute_Rate:
			MT6251_WriteByte(FM_PG_SEL, 1);
			MT6251_ReadByte(0xc8, &dataRead); 
			MT6251_WriteByte(0xc8, (dataRead&0x80FF) |(item_value<<8));
			MT6251_WriteByte(FM_PG_SEL, 0);
		break;
		case AFC_Enable:
			MT6251_ReadByte(0x33, &dataRead);
			if(item_idx)
				MT6251_WriteByte(0x33, dataRead|0x0400);
			else
				MT6251_WriteByte(0x33, dataRead&0xFBFF);
		break;
		case Softmute_Level:
			MT6251_WriteByte(FM_PG_SEL, 1);
			MT6251_ReadByte(0xd0, &dataRead);
			if(item_value > 0x24)
				item_value = 0x24;
			MT6251_WriteByte(0xd0, (dataRead&0xFFC0) |item_value);
			MT6251_WriteByte(FM_PG_SEL, 0);
		break;
		case Analog_Volume:
			MT6251_ReadByte(0xb0, &dataRead);
			MT6251_WriteByte(0xb0, (dataRead&0xF03F)|(item_value<<6));//[11:6]
		break;
		default:
		break;
	}
...
kal_uint32 FMDrv_GetCapArray(void)
{
    ...
    MT6251_ReadByte(FM_CAP_ARRAY, &Reg_Data);
...
void FMDrv_ClrIntr(void)
{
	kal_uint16 dataRead;
	MT6251_ReadByte(FM_MAIN_INTR, &dataRead);
	MT6251_WriteByte(FM_MAIN_INTR, dataRead);//clear status flag
	IRQUnmask(IRQ_FM_CODE);
}
...
void FMDrv_PowerOnReset(void)
{
  ...
  kal_trace(TRACE_INFO, FM_INFO_FM_COMMON_POWER_ON);
 	...
 	dcl_pmu_set_vfm(PMU_VOLT_02_800000_V);
 	dcl_pmu_vfm_enable(KAL_TRUE);	

	MT6251_FMSYS_PowerOn();
	...
	for (i=0; i<POWER_ON_COMMAND_COUNT; i++)
	{
		...
		{  //polling status=1
			while(1)
			{		
				MT6251_ReadByte(PowerOnSetting[i].and, &tmp_reg);
		...
		{  //read status
			...
			switch(PowerOnSetting[i].and)
			{
				case  0x62:
					...
					break;
				case 0x48:
					MT6251_ReadByte(PowerOnSetting[i].and, &tmp_reg); //0x48
					MT6251_ReadByte(PowerOnSetting[i].or, &tmp_reg1); //0x42
					MT6251_WriteByte(PowerOnSetting[i].or, ((tmp_reg1&0xC0C0)|(tmp_reg&0x3F3F)));
					break;
				default:
					break;
			}
		...
		else
		{
			...
			{
				MT6251_ReadByte(PowerOnSetting[i].addr, &tmp_reg);
				tmp_reg &= PowerOnSetting[i].and;
				tmp_reg |= PowerOnSetting[i].or;
			}
			else
				tmp_reg = PowerOnSetting[i].or;
			MT6251_WriteByte(PowerOnSetting[i].addr, tmp_reg);
		}
	}
  if (INT_ecoVersion() == ECO_E1)
  {
   	MT6251_WriteByte(2, 0x09bc);
  }
	/* Internal Inerrupt initialize */
	FMDrv_IntrInit();
 	/*disable short antenna calibration*/
	MT6251_WriteByte(0x2F,0x4000);
...
void FMDrv_PowerOffProc(void)
{
	...
	for (i=0; i<POWER_OFF_COMMAND_COUNT; i++)
	{
		...
		else
		{
		    ...
		    {
			    MT6251_ReadByte(PowerOffProc[i].addr, &tmp_reg);
			    tmp_reg &= PowerOffProc[i].and;
			    tmp_reg |= PowerOffProc[i].or;
		    }
		    else
			    tmp_reg = PowerOffProc[i].or;
		    MT6251_WriteByte(PowerOffProc[i].addr, tmp_reg);
		}
	}
	dcl_pmu_vfm_enable(KAL_FALSE);
	_is_fm_on = KAL_FALSE;
	_current_frequency = -1;
}
...
void FMDrv_SetFreq( kal_int16 curf )  /* input value: 875 - 1080 ( 87.5 MHz - 108.0 MHz)*/
{
	...
	MT6251_Mute_OnOff(1);
	MT6251_RampDown();
	//MT6251_HL_Side_Adj(curf);
	MT6251_Freq_Avoid(curf);
	if (MT6251_SetFreq(curf) == 0)
	{
		ASSERT(0);
	}
	FreqKHz = curf;
...
{
	kal_uint16 pamd, readtimes, total=0;
...
void FMDrv_HWSearch(kal_int16 freq, kal_bool is_step_up, kal_int16 space, kal_bool is_preset)
{
	...
	{ //200K
	  _step = 2;
    	...
    	MT6251_RampDown();		
	//- Modified code start  
      	if (MT6251_SetFreq(targetFreq) == 0) 
      	{
      	   ASSERT(0);
      	}
    	MT6251_RampDown();		
	Delayms(50);
       // Setting before seek
    MT6251_ReadByte(ADDR_SEARCH_THRESHOLD, &dataRead);  
#ifdef INTERNAL_ANTENNAL_SUPPORT
    ...
    MT6251_WriteByte(ADDR_SEARCH_THRESHOLD, dataRead);
    MT6251_ReadByte(0xE1, &dataRead);  
    MT6251_WriteByte(0xE1, ((dataRead&0xFF00)|FMR_CQI_TH));
       
    	MT6251_ReadByte(FM_MAIN_CFG2, &dataRead);  
      	MT6251_WriteByte(FM_MAIN_CFG2, (dataRead&0xFC00)|((FMR_SCAN_BAND_DN-640)<<1));//set space(100k/200k)and band(875~1080)and up/down
    	MT6251_ReadByte(FM_MAIN_CFG1, &dataRead);  
      	MT6251_WriteByte(FM_MAIN_CFG1, (dataRead&0x8800)|(UpDown_flag<<10)|(1<<(12+_step))|((FMR_SCAN_BAND_UP-640)<<1));//set space(100k/200k)and band(875~1080)and up/down
    	MT6251_ReadByte(FM_MAIN_CFG1, &dataRead);

	if (!is_preset)
  	{	// this is not a auto_scan function !
		MT6251_WriteByte(FM_MAIN_CFG1, dataRead&0xF7FF|0x0800); //enable wrap , if it is not auto scan function
    }else{
		MT6251_WriteByte(FM_MAIN_CFG1, dataRead&0xF7FF); //disable wrap , if it is auto scan function
	}
	if (!is_preset)
  	{	// this is not a auto_scan function !
    		MT6251_SEEK_ON();
...
kal_uint8 FMDrv_HWSpolling(kal_uint16 *curf, kal_uint8 *is_valid)
{
	...
	MT6251_ReadByte(FM_MAIN_INTR, &dataRead);	
	MT6251_WriteByte(FM_MAIN_INTR, (dataRead&0xFFFE)|0x0001);//clear status flag	
   	MT6251_ReadByte(FM_MAIN_CHANDETSTAT, &dataRead);   	
...
void FMDrv_GetScanTbl(kal_uint16 *BitMapData, kal_uint8 offset)
{
	...
	MT6251_ReadByte(0x80+offset, &dataRead);	
...
void FMDrv_SCAN_Sort_Tune_Internal(kal_uint16 freq,kal_uint16 *p_RSSI)
{
	...
	MT6251_ReadByte(FM_MAIN_CTRL, &dataRead);		
  	MT6251_WriteByte(FM_MAIN_CTRL, (dataRead&0xFFF0)); 
	MT6251_ReadByte(FM_MAIN_CHANDETSTAT, &dataRead);		
  	while( (dataRead & MASK_STC) == 1)
  	{
		MT6251_ReadByte(FM_MAIN_CHANDETSTAT, &dataRead);
...
void FMDrv_Force_Mono(void)
{
	kal_uint16 RegTemp;
	MT6251_WriteByte(FM_PG_SEL, 1);
	MT6251_ReadByte(0xE0,&RegTemp);
	MT6251_WriteByte(0xE0, RegTemp|0x0001);
	MT6251_WriteByte(FM_PG_SEL, 0);
}
