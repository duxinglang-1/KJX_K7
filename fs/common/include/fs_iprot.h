...
typedef struct {
    ...
    FS_OpenHintLocation     * Location;   // Note: Location & Path must put at the end of FS_OpenHintPath, see MTHintInitPath()
 ...
 *
 * FUNCTION
 *  FS_FormatInitFatSector
 * DESCRIPTION
 *  Assign Cluster Value C[0] and C[1], to the first FAT sector.
 *  This function is internal use only.
 * PARAMETERS
 *  MediaDescriptor: [IN] <P>Media Descriptor from the boot sector, usually 0xF8 (Fix Disk).
 *  FATType: [IN] <P> FAT partition type FAT12, FAT16 or FAT32.
 *  Sector:  [OUT] <P> Pointer to the FAT sector buffer.
 * RETURNS
 *  Always Success, FS_NO_ERROR.
 *****************************************************************************/
 ...
 *  FS_FormatGetFatType
 * DESCRIPTION
 *  Calculate FAT partition parameters from the given FAT Boot Sector.
 *  This function is internal use only.
 * PARAMETERS
 *  BootSector: [IN] <P>Pointer to the FAT boot sector buffer
 *  RootDirSectors: [OUT] <P> Number of root directory sectors
 *  FATSectors: [OUT] <P> Number of FAT sectors
 *  Clusters: [OUT] <P> Number of Clusters
 * RETURNS
