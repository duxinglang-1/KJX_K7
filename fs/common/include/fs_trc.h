...
BEGIN_TRACE_MAP(MOD_FS)
   ...
   TRC_MSG(FS_API_FS_CLOSE_S,   "[%Mmodule_type] FS_Close <- Caller: 0x%X, FileHandle: 0x%X")
 ...
 */
   ...
   /* FS Cache */
   TRC_MSG(FS_CACHE_LIST_INSERT, "[%Mmodule_type] <Cache> Insert  LBA=%u, Len=%u, ptr=0x%X")
   TRC_MSG(FS_CACHE_LIST_REMOVE, "[%Mmodule_type] <Cache> Remove  LBA=%u, Len=%u, ptr=0x%X")
   TRC_MSG(FS_CACHE_HIT,         "[%Mmodule_type] <Cache> HIT     LBA=%u, Len=%u, ptr=0x%X")
   TRC_MSG(FS_CACHE_DISCARD,     "[%Mmodule_type] <Cache> Discard LBA=%u, Len=%u, ptr=0x%X")
   TRC_MSG(FS_CACHE_WRITE,       "[%Mmodule_type] <Cache> Write   LBA=%u, Len=%u, ptr=0x%X")
   TRC_MSG(FS_CACHE_READ,        "[%Mmodule_type] <Cache> Read    LBA=%u, Len=%u, ptr=0x%X")
   TRC_MSG(FS_CACHE_LRU,         "[%Mmodule_type] <Cache> LRU     LBA=%u, Len=%u, ptr=0x%X")
   TRC_MSG(FS_CACHE_LIST,        "[%Mmodule_type] <Cache-List> #%u, LBA=%u, Len=%u, ptr=0x%X")
   TRC_MSG(FS_CACHE_FLUSH_ALL,   "[%Mmodule_type] Flush all buffers")
   TRC_MSG(FS_CACHE_CONFIG,      "[%Mmodule_type] Change cache configuration, cache size = %d KB")
   ...
   TRC_MSG(FS_INFO_GR2_READSECTORS,   "[%Mmodule_type]       %MFS_DEVICE_TYPE_ENUM, R, %u, %u, %u, %MFS_BUFFER_TYPE_ENUM")
   TRC_MSG(FS_INFO_GR2_WRITESECTORS,  "[%Mmodule_type]       %MFS_DEVICE_TYPE_ENUM, W, %u, %u, %u, %MFS_BUFFER_TYPE_ENUM")
