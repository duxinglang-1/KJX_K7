...
static FSAL_Status FSAL_Read_UINT_X(STFSAL *pstFSAL, kal_uint32 *puValue, kal_int32 bytes)
{
    kal_uint8 data[4];
    kal_uint32 value = 0;
    kal_int32 i;
    ...
    if((ret=FSAL_Read_Bytes(pstFSAL, data, bytes))!=FSAL_OK) return ret;

    if (FSAL_BIG_ENDIAN == pstFSAL->uByteOrder) {
        for (i=0;i<bytes;i++) value = (value << 8) | data[i];
    }
    else {
        for (i=(bytes-1);i>=0;i--) value = (value << 8) | data[i];
    }
...
{
    return FSAL_Read_UINT_X(pstFSAL, puValue, 4);
}

FSAL_Status FSAL_Read_UINT24(STFSAL *pstFSAL, kal_uint32 *puValue)
{
    return FSAL_Read_UINT_X(pstFSAL, puValue, 3);
}
...
FSAL_Status FSAL_Read_UINT16(STFSAL *pstFSAL, kal_uint16 *pwValue)
{
    ...
    ret = FSAL_Read_UINT_X(pstFSAL, &value, 2);
    if (ret==FSAL_OK) *pwValue = value;
