...
typedef struct {
    BLOUD_Coef_Q5p27 q5p27[2];
    BLOUD_Coef_Q2p14 q2p14[9];
}BLOUD_FilterCoef_FixFreq;
...
typedef struct {
    ...
    BLOUD_FilterCoef_FixFreq *pFilter_Coef;
}BLOUD_InitParam;
...
|
|   BPF_COEF [6][6][3]      Band-pass filter coefficients
|             |  |  |                          -1        -2
|             |  |  |               b0 + b1 x z  + b2 x z
|             |  |  |       H(z) = -------------------------
|             |  |  |                          -1        -2
...
typedef struct {
    ...
    unsigned int BPF_COEF[6][6][3];
    unsigned int LPF_COEF[6][3];
}BLOUD_FilterCoef_NvramFreq;

typedef struct {
    BLOUD_FilterCoef_NvramFreq spk_flt_param;
    BLOUD_FilterCoef_NvramFreq hdp_flt_param;
    BLOUD_CustomParam          custom_param;
}BLOUD_NVRAM_Param_V4;
// [Tail] For Driver
