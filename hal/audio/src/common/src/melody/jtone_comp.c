...
{
   ASSERT( handle != 0 );
   return MelodyComp_GetPitchTranspose(handle);
}

Media_Status JTone_SetPitchShift(Media_Handle *handle, kal_int8 pitch_shift)
{
   ASSERT( handle != 0 );
   return MelodyComp_SetPitchTranspose( handle, pitch_shift );
}   
...
{
   ASSERT( handle != 0 );
   return MelodyComp_SetProgram( handle, 0, program );
}
...
/*
********************************************************************************
*                        Tone Sequence Control Fucntions
********************************************************************************
*/
...
Media_Handle  *JTone_Open( void (*handler)( Media_Handle *handle, Media_Event event ), Media_ToneSeq_Param **param )
{
   ...
   player->format          = MELODYFORMAT_JTS;
   handle->state           = MIDI_STATE_READY;
   handle->param           = (void *)&player->param.jts;
   handle->data            = (uint32)player;
   
   *param                  = &player->param.jts;
   (*param)->toneSeq       = NULL;
   (*param)->toneSeqLen    = 0;
   kal_brief_trace(TRACE_GROUP_MELODY_LV1, MELODY_API_OPEN, 2, handle);
...
Media_Status JTone_Close( Media_Handle *handle)
{
   ...
   kal_brief_trace(TRACE_GROUP_MELODY_LV1, MELODY_API_CLS, 2, handle);
...
Media_Status  JTone_Play( Media_Handle *handle )
{
   ...
   ASSERT( handle != 0 );
   format = MelodyComp_Format(handle);
   kal_brief_trace(TRACE_GROUP_MELODY_LV1, MELODY_API_PLAY, 2, handle, format, handle->repeat_count);
   if(format != FORMAT_JTS)
      return MEDIA_FAIL;
...
Media_Status  JTone_Stop( Media_Handle *handle )
{
   ASSERT( handle != 0 );
   handle->start_time = 0;
   JTone_SetStartTime( handle, handle->start_time);
   kal_brief_trace(TRACE_GROUP_MELODY_LV1, MELODY_API_STOP, 2, handle, handle->start_time);
...
Media_Status  JTone_Pause( Media_Handle *handle ) 
{
   ...
   JTone_SetStartTime( handle, handle->start_time);
   kal_brief_trace(TRACE_GROUP_MELODY_LV1, MELODY_API_PAUSE, 2, handle, handle->start_time);
   return MelodyComp_Stop(handle); 
}
...
{
   ASSERT( handle != 0 ); 
   kal_brief_trace(TRACE_GROUP_MELODY_LV1, MELODY_API_RESUME, 2, handle);
