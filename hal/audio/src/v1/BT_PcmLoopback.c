...
#include "fat_fs.h"  /* file system */
   ...
   static kal_uint16 btlbPcmDebugBuf[80000]; // for debug 
   static kal_uint16 *pcmDebugPtr; // for debug 
   static kal_uint32 pcmDebugCnt;
#endif    
...
extern void AcousticLoopbackBypassMode(kal_uint8 u1Mode);
void BT_PcmLoopbackTestStop(void)
{
#if defined(__TC01__)
/* under construction !*/
/* under construction !*/
...
/* under construction !*/
   ...
   kal_trace( TRACE_INFO, L1AUDIO_BT_PCMLOOPBACK_AFTER_SET, *DP_VOL_OUT_PCM);
...
#ifdef BTLB_DEBUG
{
         ...
         if ( acmLen >= BLOCK_SIZE){
            ...
            kal_trace( TRACE_INFO, L1AUDIO_BT_PCMLOOPBACK_MAGDB, magDB);
...
{
      int file_handle;
      kal_trace( TRACE_STATE, L1AUDIO_BT_PCMLOOPBACK_DUMP_PREP);
      ...
      { 
         ...
         kal_trace( TRACE_STATE, L1AUDIO_BT_PCMLOOPBACK_DUMP_DONE);
      }
      else{
      	kal_trace( TRACE_STATE, L1AUDIO_BT_PCMLOOPBACK_DUMP_FAIL);
      }
