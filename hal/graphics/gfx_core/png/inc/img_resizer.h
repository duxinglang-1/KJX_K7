...
#ifdef PNG_DEDICATED_CACHEABLE_BUFFER_SUPPORT
#undef PNG_NEW_BUFFER_INTERFACE
#endif
...
typedef struct
{
   ...
   // blending buffer for sw alpha blending
   kal_uint8 *bld_buf_ptr;
   kal_uint32 bld_buf_w;
   ...
   kal_bool enable_pargb8888;        ///< 0: output as argb8888, 1: output as pargb8888
                                     ///< this flag is valid only when output format is PARGB8888

#if defined(__SPATIAL_DITHERING__)
   kal_bool sd_enable;               ///< SW dithering enable flag.
#endif
   
   PNG_DECODE_COLOR_FORMAT_ENUM output_format;
} img_resizer_struct;
...
/// unified buffer
void png_resizer_get_work_buffer_size(kal_int32 *size);
...
// add for alpha blending support
void img_resizer_put_row_with_file_alpha(kal_uint8 *src, kal_uint32 width, kal_uint32 row_num, kal_uint32 step);
void img_resizer_put_pixel_with_file_alpha(kal_uint32 x, kal_uint32 y, kal_uint8 *src);
void composite(kal_uint8 ialpha, kal_uint8 fg_color, kal_uint8 bg_color, kal_uint8 *fb);
