...
#include "drv_comm.h"   // define DRV_Reg... macro, define __PNG_DECODER_6276_SERIES__

#if defined(__PNG_DECODER_6276_SERIES__)
...
#if PNG_DEBUG
kal_uint32 png_hw_trigger_time = 0;
kal_uint32 png_hw_decode_time = 0;
#endif
...
static void _png_drv_stop_timer(void);
 void _addMemGuardHeader(kal_uint32 *ptr);
 void _addMemGuardFooter(kal_uint32 *ptr);
...
/// 2008.05.21
/// for internal size: additional 2 * 4 bytes for each memory segment as memory guard header & footer
/// for external size: additional 16 byte to handle LZ77 8-byte alignment address   
/// #define INT_WORK_BUFFER_SIZE ((COLOR_TBL_MAX_SIZE + 2 + HLIT_TBL_MAX_SIZE + 2 + HDIST_TBL_MAX_SIZE + 2 + LINE_BUF_MAX_SIZE + 2) * 4)
/// #define EXT_WORK_BUFFER_SIZE ((((HCLEN_TBL_MAX_SIZE + 2) + (TRNS_TBL_MAX_SIZE + 2) + LZ77_BUF_MAX_SIZE + 6+4+8) * 4) + 16)
...
void png_drv_hisr(void)
{
   ...
   else if (IRQStatus & PNG_IRQ_STATUS_ADLER_ERR)
   {
      if (is_src_file_end)
      {  
         (*_png_drv_irq_callback)(PNG_STATUS_OK);
...
#if 0 /// defined(DRV_FEATURE__MM_INTMEM_IMPL_MT6276)
/* under construction !*/
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
{      
   /// On MT6276, the internal memory will not be reserved by GDI,
...
/* under construction !*/
   #endif          
   {
       internalBuf = (kal_uint32 *)(((kal_uint32)((kal_uint8 *)externalBuf + EXT_WORK_BUFFER_SIZE) + 7) & ~7);      
   }        
   ...
   internalBuf++;
   // color table need internal memory 256x4 bytes, address must 4-byte aligned
   hwcfg->colorTableStartAddr = internalBuf;     
   ...
   // Huffman HCLEN table need external memory 128k bytes, address must 4-byte aligned   
   hwcfg->HCLENStartAddr = externalBuf;       
   ...
   _addMemGuardHeader(internalBuf);
   internalBuf++;
   // Huffman HLIT table need internal memory 572 bytes, address must 4-byte aligned
   hwcfg->HLITStartAddr = internalBuf;           
   ...
   _addMemGuardHeader(internalBuf);
   internalBuf++;
   // Huffman HDIST table need internal memory 60 bytes, address must 4-byte aligned
   hwcfg->HDISTStartAddr = internalBuf;         
   ...
   _addMemGuardHeader(internalBuf);
   internalBuf++;    
   // Line buffer need internal memory LINE_BUF_MAX_SIZE x 4 bytes, address must 4-byte aligned
   hwcfg->lineBuff0StartAddr = internalBuf;    
   ...
   externalBuf = (kal_uint32 *)(((kal_uint32)((kal_uint8 *)externalBuf + 3)) & ~3); ///< make sure 4 byte align     
   if (((kal_uint32)externalBuf & 0x7) == 0) /// if 8-byte align before add header, adjust to 4-byte align
   {
      externalBuf++;
   }
   
   // LZ77 buffer need external memory 32k bytes, address must be 8 bytes aligned(6276/6573)
   _addMemGuardHeader(externalBuf); 
   externalBuf++;         
   hwcfg->lz77StartAddr = externalBuf;   /// after shift 4 byte, address become 8-byte align
   if (((kal_uint32)hwcfg->lz77StartAddr & 0x7) != 0)
   {
      ASSERT(0);
   }          
   ...
   _addMemGuardHeader(externalBuf);
   externalBuf++;
   // transparant table max size is 256 bytes, address must 4-byte aligned
   hwcfg->trnsTableAddr = externalBuf;
   externalBuf += TRNS_TBL_MAX_SIZE;
   _addMemGuardFooter(externalBuf);
   externalBuf++;
   
   /// set PNG working buffer address
...
png_drv_set_canvas(PNG_HW_INFO *hwcfg)
{
   ...
   switch (hwcfg->canvasColorFormat) 
   {
       case PNG_FORMAT_RGB565:
          ...
          DRV_WriteReg32(PNG_OUT_FORMAT, 1);
          break;

       case PNG_FORMAT_RGB888:
          sz_byte = 3 * width * height;
          DRV_WriteReg32(PNG_OUT_FORMAT, 2);
          break;

       case PNG_FORMAT_ARGB8888:
          sz_byte = 4 * width * height;
          DRV_WriteReg32(PNG_OUT_FORMAT, 3);
          break;
   ...
   DRV_WriteReg32(PNG_OUT_START, (kal_uint32)(hwcfg->canvasStartAddr));    /// must be 4-byte aligned
   DRV_WriteReg32(PNG_OUT_END, adr_value);                                 /// must be 4-byte aligned
...
{
   /// b-buffer must be 4-byte aligned(if blending cache enable)
   /// if(( 0 != (((kal_uint32)(blend_buf_adr)) & 0x03)))
   /// {
   ///    ASSERT(0);
   ///    return PNG_STATUS_ERROR_INVALID_BUFFER;
   /// }
...
png_drv_set_blending_mode(PNG_HW_INFO *hwcfg, MY_PNG_INFO *png_ptr)
{
   ...
   switch (mode)
   {
      ...
      case PNG_BLEND_FILE_ALPHA:
         if ((PNG_COLOR_TYPE_GA == png_ptr->color_type) ||
             (PNG_COLOR_TYPE_RGBA == png_ptr->color_type) ||
             ((png_ptr->num_trans > 0) && (PNG_COLOR_TYPE_PALETTE == png_ptr->color_type)))
         {
      ...
      case PNG_BLEND_USER_ASSIGNED:
         ...
         break;
      case PNG_BLEND_BG_COLOR:
         if(png_ptr->valid & PNG_INFO_bKGD)
         {
            DRV_WriteReg32(PNG_ALPHA_BLEND, 4);
         }
         else
         {
            DRV_WriteReg32(PNG_ALPHA_BLEND, 0);    // no blending
...
png_drv_decode_start(PNG_HW_INFO *hwcfg, MY_PNG_INFO *png)
{
   ...
   // pngDrvSetSourceData() has been invoked
   // pngDrvHookIRQCallback() has been invoked
   ...
   if ((PNG_COLOR_TYPE_GA == png->color_type) ||
       (PNG_COLOR_TYPE_RGBA == png->color_type) ||
       ((png->num_trans > 0) && (PNG_COLOR_TYPE_PALETTE == png->color_type)))
   {
 ...
 // ---------------------set transparency control -----------------------------
   ...
   _png_decoder_cfg.bk_color_disable = 1; ///< no out
      ...
      switch(png->color_type) 
      {
         ...
         case PNG_COLOR_TYPE_RGB:
            ...
            {            
                ...
                if (png->valid & PNG_INFO_bKGD) ///< bKGD exist
                {
                   _png_decoder_cfg.bk_color_disable = 0;    ///<output background color
                }                
            }
            break;
   ...
   /// 2008.05.19
   #if 0 /// <Robin, not sure why have this condition check, disable it>
/* under construction !*/
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
png_drv_decode_resume(PNG_HW_INFO *hwcfg)
{
   ...
   DRV_WriteReg32(PNG_B_CACHE_EN, 0);  /// disable blending cache
...
{   
   *internal_mem_sz = (kal_uint32)0;
   ...
   /// add 16 bytes to handle 8-byte aligned lz77 buffer, 
...
#endif   /// #if defined(__PNG_DECODER_6276_SERIES__)
