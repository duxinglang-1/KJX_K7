...
static struct
{
    ...
    #if defined(IDP_IMAGE_CAPTURE_TILE_MODE_SUPPORT)
    /* Tile parameters */
    ...
    kal_uint32 tar_w, kal_uint32 tar_h)
{
    ...
    // Left source index = 0
    config_ic.tile_bi_sa_src_start_pos[0] = 0;

    if (src_w != tar_w)
    {
        // Right source index = {[(O0-A0) * Ratio] >> 20} + [(src*A0)/tar] + 1
        config_ic.tile_bi_sa_src_end_pos[0] = (kal_uint32)((((curr_tile_last_idx-curr_last_round_idx) * in_ratio) >> 20) +
    ...
    if (config_ic.tile_bi_sa_src_end_pos[0] >= src_w)
    {
        config_ic.tile_bi_sa_src_end_pos[0] = src_w - 1;
    }

    config_ic.tile_adj_src_start_pos[0] = config_ic.tile_bi_sa_src_start_pos[0] & 0xFFFFFFFE;
    config_ic.tile_adj_src_end_pos[0] = config_ic.tile_bi_sa_src_end_pos[0] | 0x00000001;
    config_ic.tile_src_size_x[0] = config_ic.tile_adj_src_end_pos[0] - config_ic.tile_adj_src_start_pos[0] + 1;

    config_ic.tile_start_pos_x[0] = 0;
    config_ic.tile_init_resid_x[0] = 0;
    config_ic.tile_trunc_err_comp_x[0] = tar_w;

    config_ic.tile_start_pos_y[0] = 0;
    config_ic.tile_trunc_err_comp_y[0] = tar_h;
    config_ic.tile_init_resid_y[0] = 0;
    ...
    if (curr_tile_first_idx == (prev_last_round_idx + round_step)) // Of1 == A0 + [tar/GCD(src,tar)]
    {
        ...
        config_ic.tile_bi_sa_src_start_pos[1] = (src_w*curr_tile_first_idx) / tar_w;
        if (src_w != tar_w)
        {
            // Right source index = {[(O1-A1) * Ratio] >> 20} + [(src¡ÑA1)/tar] + 1
            config_ic.tile_bi_sa_src_end_pos[1] = (kal_uint32)((((curr_tile_last_idx-curr_last_round_idx) * in_ratio) >> 20) +
                                                               ((src_w*curr_last_round_idx)/tar_w) +
                                                               1);
        }
        else
        ...
        config_ic.tile_start_pos_x[1] = 0;
        config_ic.tile_init_resid_x[1] = 0;
    }
    else
    {
        // Left source index = {[(O0+1-A0) * Ratio] >> 20} + [(src * A0 )/tar]
        config_ic.tile_bi_sa_src_start_pos[1] = (kal_uint32)((((prev_tile_last_idx+1-prev_last_round_idx) * in_ratio) >> 20) +
        ...
        config_ic.tile_start_pos_x[1] = ((prev_tile_last_idx+1-prev_last_round_idx) * in_ratio) & 0x000FFFFF;
        // Initial residual = [(O0+1-A0) * Residual] % tar
        config_ic.tile_init_resid_x[1] = ((prev_tile_last_idx+1-prev_last_round_idx) * in_resid) % tar_w;
    }

    if (config_ic.tile_bi_sa_src_end_pos[1] >= src_w)
    {
        config_ic.tile_bi_sa_src_end_pos[1] = src_w - 1;
    }

    config_ic.tile_adj_src_start_pos[1] = config_ic.tile_bi_sa_src_start_pos[1] & 0xFFFFFFFE;
    config_ic.tile_adj_src_end_pos[1] = config_ic.tile_bi_sa_src_end_pos[1] | 0x00000001;
    config_ic.tile_src_size_x[1] = config_ic.tile_adj_src_end_pos[1] - config_ic.tile_adj_src_start_pos[1] + 1;

    config_ic.tile_trunc_err_comp_x[1] = tar_w;

    config_ic.tile_start_pos_y[1] = 0;
    config_ic.tile_trunc_err_comp_y[1] = tar_h;
    config_ic.tile_init_resid_y[1] = 0;
}

static void
_idp_scen_ic_jpeg_sa_tile(
    kal_uint32 src_w, kal_uint32 src_h,
    kal_uint32 tar_w, kal_uint32 tar_h)
{
    ...
    config_ic.tile_bi_sa_src_end_pos[0] = (kal_uint32)(((((((curr_tile_last_idx+1)<<1)-1)<<20)+(in_ratio*3)-1)/(in_ratio<<1)) - 1);
    // [(1<<20)+Ratio] / 2
    config_ic.tile_sa_init_weight[0] = (kal_uint32)(((1<<20) + in_ratio) >> 1);

    config_ic.tile_adj_src_start_pos[0] = config_ic.tile_bi_sa_src_start_pos[0] & 0xFFFFFFFE;
    config_ic.tile_adj_src_end_pos[0] = config_ic.tile_bi_sa_src_end_pos[0] | 0x00000001;
    config_ic.tile_src_size_x[0] = config_ic.tile_adj_src_end_pos[0] - config_ic.tile_adj_src_start_pos[0] + 1;

    config_ic.tile_start_pos_x[0] = config_ic.tile_sa_init_weight[0];
    config_ic.tile_start_pos_y[0] = (kal_uint32)(((1<<20) + in_ratio_y) >> 1);

    config_ic.tile_trunc_err_comp_x[0] = 0;
    config_ic.tile_trunc_err_comp_y[0] = 0;
    ...
    prev_tile_remain_weight = config_ic.tile_sa_init_weight[0] +
                              (in_ratio * (config_ic.tile_bi_sa_src_end_pos[0]-config_ic.tile_bi_sa_src_start_pos[0])) -
                              (tar_tile_size << 20);

    if (prev_tile_remain_weight == 0)
    {
        config_ic.tile_bi_sa_src_start_pos[1] = config_ic.tile_bi_sa_src_end_pos[0] + 1;
        config_ic.tile_bi_sa_src_end_pos[1] = src_w - 1;

        config_ic.tile_sa_init_weight[1] = in_ratio;
    }
    else
    {
        config_ic.tile_bi_sa_src_start_pos[1] = config_ic.tile_bi_sa_src_end_pos[0];
        config_ic.tile_bi_sa_src_end_pos[1] = src_w - 1;

        config_ic.tile_sa_init_weight[1] = (kal_uint32) prev_tile_remain_weight;
    }
    ...
    config_ic.tile_start_pos_x[1] = config_ic.tile_sa_init_weight[1];
    config_ic.tile_start_pos_y[1] = (kal_uint32)(((1<<20) + in_ratio_y) >> 1);

    config_ic.tile_trunc_err_comp_x[1] = 0;
    config_ic.tile_trunc_err_comp_y[1] = 0;
...
idp_scen_ic_jpeg__bi_tile_2nd_pass_config_crz(void)
{
    ...
    crz_struct.src_width = config_ic.tile_src_size_x[0];
    ...
    crz_struct.crop_left1 = config_ic.tile_bi_sa_src_start_pos[0] - config_ic.tile_adj_src_start_pos[0];
    crz_struct.crop_right1 = config_ic.tile_bi_sa_src_end_pos[0] - config_ic.tile_adj_src_start_pos[0];
    ...
    crz_struct.tile_start_pos_x = config_ic.tile_start_pos_x[0];
    crz_struct.tile_start_pos_y = config_ic.tile_start_pos_y[0];
    crz_struct.tile_trunc_err_comp_x = config_ic.tile_trunc_err_comp_x[0];
    crz_struct.tile_trunc_err_comp_y = config_ic.tile_trunc_err_comp_y[0];
    crz_struct.tile_resid_x = config_ic.tile_init_resid_x[0];
    crz_struct.tile_resid_y = config_ic.tile_init_resid_y[0];
...
idp_scen_ic_jpeg__bi_tile_3rd_pass_config_crz(void)
{
    ...
    crz_struct.src_width = config_ic.tile_src_size_x[1];
    crz_struct.src_height = idp_capture_to_jpeg_pass_1_tar_height;
    crz_struct.tar_width = config_ic.image_capture_width >> 1;
    crz_struct.tar_height = config_ic.image_capture_height;

    crz_struct.crop_en1 = KAL_TRUE;
    crz_struct.crop_left1 = config_ic.tile_bi_sa_src_start_pos[1] - config_ic.tile_adj_src_start_pos[1];
    crz_struct.crop_right1 = config_ic.tile_bi_sa_src_end_pos[1] - config_ic.tile_adj_src_start_pos[1];
    crz_struct.crop_top1 = 0;
    crz_struct.crop_bottom1 = idp_capture_to_jpeg_pass_1_tar_height - 1;
    ...
    crz_struct.sa_en_x = KAL_FALSE;
    crz_struct.sa_en_y = KAL_FALSE;
    crz_struct.tile_start_pos_x = config_ic.tile_start_pos_x[1];
    crz_struct.tile_start_pos_y = config_ic.tile_start_pos_y[1];
    crz_struct.tile_trunc_err_comp_x = config_ic.tile_trunc_err_comp_x[1];
    crz_struct.tile_trunc_err_comp_y = config_ic.tile_trunc_err_comp_y[1];
    crz_struct.tile_resid_x = config_ic.tile_init_resid_x[1];
    crz_struct.tile_resid_y = config_ic.tile_init_resid_y[1];
...
idp_scen_ic_jpeg__sa_tile_2nd_pass_config_crz(void)
{
    ...
    idp_scen_ic_sm_y_offset = config_ic.tile_adj_src_start_pos[0];  // + offset_x
...
idp_scen_ic_jpeg__sa_tile_3rd_pass_config_crz(void)
{
    ...
    idp_scen_ic_sm_y_offset = config_ic.tile_adj_src_start_pos[1];  // + offset_x
    idp_scen_ic_sm_uv_offset = (idp_scen_ic_sm_y_offset >> 1);      // + offset_x/2
    ...
    kal_bool const config_to_hardware)
{
    ...
    if (src_size >= tar_size)
    {
        ...
        else
        {
            ...
            _idp_scen_ic_jpeg_sa_tile(src_w, src_h, config_ic.image_capture_width, config_ic.image_capture_height);
            #else
    ...
    else
    {
        ...
        #if defined(IDP_IMAGE_CAPTURE_TILE_MODE_SUPPORT)
        {
            ...
            _idp_scen_ic_jpeg_bi_tile(src_w, src_h, config_ic.image_capture_width, config_ic.image_capture_height);
        }
        #else
        {
            ASSERT(0); // VGA sensor should not capture 1M(1280) and 2M(1600)
