...
kal_bool kjx_aux_eint_state = KJX_AUX_EINT_STATE;

void KJX_AUX_EINT_HISR(void)
{
	kal_uint32 iStatus = 0;
	kal_uint32 iA,iB;
	...
	iA = (iStatus & 0x2) >> 1;
	iB = iStatus & 0x1;
	//drv_trace4(TRACE_GROUP_3, ACCDET_EINT_STATUS_TRC,kjx_aux_eint_state,iA,iB,AUX_EINT_STATE);
	
	if (kjx_aux_eint_state == KJX_AUX_EINT_STATE)
	{
		kjx_aux_eint_state = (kal_bool)!kjx_aux_eint_state;
		EINT_Set_Polarity(AUX_EINT_NO,kjx_aux_eint_state); 
		EINT_SW_Debounce_Modify(AUX_EINT_NO,10);
		//AUX_Sendilm(MOD_EINT_HISR,MOD_UEM, MSG_ID_AUX_ID, AUX_ID_EARPHONE);
		DRV_ACCDET_WriteReg32(ACCDET_CTRL, 0x1); // enable ACCDET unit
	}
