...
DCL_STATUS dcl_pmu6255_control_handler(DCL_HANDLE handle, DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
	...
	switch(cmd)
	{
		...
		*/
		...
		{
            // Note: For MT6255 the calibration path select is in opposite
...
void pmu6255_efuse_calibration(void)
{
	...
	// 2. OV Trim
	// Read (EFUSE_BASE + 0x100)[19] := RG_VBAT_OVP_TRIM_EN(1b)
	// Read (EFUSE_BASE + 0x104)[27:24] := RG_VBAT_OVP_TRIM(4b)
	// Apply to RG_OVP_TRIM[3:0]	
	val32 = PMU_DRV_ReadReg32(EFUSE_base + 0x100);
	if (val32 & 0x00080000) // Bit19: RG_VBAT_OVP_TRIM_EN, OV Trim Enable Bit
	{
		pmu_calibration_ov_enable = DCL_TRUE;
		val32 = PMU_DRV_ReadReg32(EFUSE_base + 0x104);
		pmu_calibration_ov_val = (DCL_UINT8)((val32 & 0x0F000000) >> 24); // Bits [27:24]
