...
//	{CHR_LDO_DET,               CHR_LDO_DET_OFFSET,             CHR_LDO_DET_MASK,               CHR_LDO_DET_SHIFT              },
	...
	{VCDT_HV_DET,				VCDT_HV_DET_OFFSET,				VCDT_HV_DET_MASK,				VCDT_HV_DET_SHIFT			   },
	...
	{OVP_TRIM,					OVP_TRIM_OFFSET,				OVP_TRIM_MASK,					OVP_TRIM_SHIFT				   },
...
DCL_STATUS dcl_pmu6256_control_handler(DCL_HANDLE handle, DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
	...
	switch(cmd)
	{
		...
		case CHR_GET_VCDT_HV_DET:
		{
			PMU_CTRL_CHR_GET_VCDT_HV_DET *pChrCtrl = &(data->rPMUChrGetVcdtHvDet);
			pChrCtrl->HV_deteted = pmu_get_chr(VCDT_HV_DET);			
		}
		break;
		...
		case CHR_SET_CV_DETECTION_VOLTAGE:
		{
			...
			if (pmu_calibration_cv_enable == DCL_TRUE)
			{
		...
		{
            // MT6256 Use H/W Auto Trim, If EFUSE_DATA[63] = 1'b then use EFUSE Result else use register value
            /*
		...
		case CHR_SET_VBAT_OV_DETECTION_VOLTAGE:
		{
			...
			if (pmu_calibration_ov_enable == DCL_TRUE)
			{
				pmu_set_chr(VBAT_OV_VTH, pmu_calibration_ov_val);
			}
			else
			{
				pmu_set_chr(VBAT_OV_VTH, val);
			}
		}
		break;	
...
void pmu6256_efuse_calibration(void)
{
	...
	// Skip PMU eFuse
	PMU_DRV_SetData16((EFUSE_CON0 + SKIP_EFUSE_OUT_OFFSET), SKIP_EFUSE_OUT_MASK, SKIP_EFUSE_OUT_MASK);
	
	// 1. CV Trim
	// Read (EFUSE_BASE + 0x100)[26] := RG_VBAT_CV_VTH_EN(1b)
	// Read (EFUSE_BASE + 0x100)[31:27] := RG_VBAT_CV_VTH(5b)
	// Apply to RG_VBAT_CV_VTH[4:0]
	val32 = PMU_DRV_ReadReg32(EFUSE_base + 0x100);
	if (val32 & 0x04000000) // Bit26: RG_VBAT_CV_VTH_EN, CV Trim Enable Bit
	{
		pmu_calibration_cv_enable = DCL_TRUE;
		pmu_calibration_cv_val = (DCL_UINT8)((val32 & 0xF8000000) >> 27); // Bits [31:27]
	}

	// 2. OV Trim
	// Read (EFUSE_BASE + 0x104)[6] := RG_VBAT_OVP_TRIM_EN(1b)
	// Read (EFUSE_BASE + 0x104)[10:7] := RG_VBAT_OVP_TRIM(4b)
	// Apply to RG_OVP_TRIM[3:0]	
	val32 = PMU_DRV_ReadReg32(EFUSE_base + 0x104);
	if (val32 & 0x00000040) // Bit6: RG_VBAT_OVP_TRIM_EN, OV Trim Enable Bit
	{
		pmu_calibration_ov_enable = DCL_TRUE;
		pmu_calibration_ov_val = (DCL_UINT8)((val32 & 0x00000780) >> 7); // Bits [10:7]
		pmu_set_chr(OVP_TRIM, pmu_calibration_ov_val);
	}

	// 3. IBIAS Trim
	// Read (EFUSE_BASE + 0x100)[21] := RG_IBIAS_TRIM_EN(1b)
	// Read (EFUSE_BASE + 0x100)[25:22] := RG_IBIAS_TRIM(4b)
	// Apply to RG_IBIAS_TRIM[3:0]
	val32 = PMU_DRV_ReadReg32(EFUSE_base + 0x100);
	if (val32 & 0x00200000) // Bit21: RG_IBIAS_TRIM_EN, IBIAS Trim Enable Bit
	{
		val16 = (DCL_UINT16)((val32 & 0x03C00000) >> 22); // Bits [25:22]
		PMU_DRV_SetData16((PMU_TEST_CON0 + IBIAS_TRIM_OFFSET), IBIAS_TRIM_MASK, (val16 << IBIAS_TRIM_SHIFT));
	}
}
