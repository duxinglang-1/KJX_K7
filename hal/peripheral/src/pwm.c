 ...
 switch(register_addr)
		{
		   ...
		   case PWM_COUNT_R:
			#if !defined(DRV_PWM_2CH_ADDRESS)
	            register_value = DRV_PWM_Reg(PWM_COUNT+(channel-1)*0x0c);
            #else
	           if(channel<PWM_2CH_START_NUM)
		           register_value = DRV_PWM_Reg(PWM_COUNT);
	         else
   ...
   switch(register_addr)
		{
		   case PWM_CTRL_R:
			 ...
			 case PWM_COUNT_R:
                   #if !defined(DRV_PWM_2CH_ADDRESS)
					 DRV_PWM_WriteReg((PWM_COUNT+(channel-1)*0x0c),value);
                    #else
					if(channel<PWM_2CH_START_NUM)
						DRV_PWM_WriteReg(PWM_COUNT,value);
				    else
