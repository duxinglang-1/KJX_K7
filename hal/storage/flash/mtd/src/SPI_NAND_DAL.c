...
kal_int32 DAL_ReadPage(DAL_ACC_Type Acc_Type, kal_uint8 ZoneNo, kal_uint16 *Bank, kal_uint16 PageAddr, kal_uint16 PageLength, kal_uint8 *Data, kal_uint8 *Spare)
{
    ...
    if (NULL == Spare)
    {
        Spare = (kal_uint8 *)g_snand_dbg_spare; // give MTD a spare buffer for SW CRC checking
    }
    #endif
    ...
    else
    {
        if (SNAND_SPARE_FORMAT_GIGADEVICE_1 == Flash_Info.deviceInfo_CE[Mtd_para.ce].ID_info[7])
        {
            switch (FDMType)
            {
                ...
                case FDMDATA_PA:
                case FDMDATA_4XCTL:
                    Data = ((FDMDataU_2K_GigaDevice *)Spare_buff) -> BM_PA;
                    break;
                case FDMDATA_LBA:
                    Data = ((FDMDataU_2K_GigaDevice *)Spare_buff) -> LBA;
                    break;
                default:
                    dbg_ASSERT(0);
            }
        }
        else if (SNAND_SPARE_FORMAT_POWERCHIP_1 == Flash_Info.deviceInfo_CE[Mtd_para.ce].ID_info[7])
        {
            switch (FDMType)
            {
                case FDMDATA_BM:
                case FDMDATA_PA:
                case FDMDATA_4XCTL:
                    Data = ((FDMDataU_2K_Powerchip *)Spare_buff) -> BM_PA;
                    break;
                case FDMDATA_LBA:
                    Data = ((FDMDataU_2K_Powerchip *)Spare_buff) -> LBA;
                    break;
                default:
                    dbg_ASSERT(0);
            }
...
void DAL_SetFDMData(kal_uint16 BMPA, kal_uint16 LBA, kal_uint8 Option, kal_uint32 *Spare_buff)
{
    ...
    if (SNAND_SPARE_FORMAT_GIGADEVICE_1 == Flash_Info.deviceInfo_CE[Mtd_para.ce].ID_info[7])
    {
        ...
        {
            kal_mem_set(Spare_buff, 0xFF, sizeof(FDMDataU_2K_GigaDevice) );
        }

        if (Option & (FDMDATA_BM | FDMDATA_PA))
        {
            ((FDMDataU_2K_GigaDevice*)Spare_buff) -> BM_PA = BMPA;
        }

        if (Option & FDMDATA_LBA)
        {
            ((FDMDataU_2K_GigaDevice*)Spare_buff) -> LBA = LBA;
        }
    }
    else if (SNAND_SPARE_FORMAT_POWERCHIP_1 == Flash_Info.deviceInfo_CE[Mtd_para.ce].ID_info[7])
    {
        if (Option & FDMDATA_EMPTY)
        {
            kal_mem_set(Spare_buff, 0xFF, sizeof(FDMDataU_2K_Powerchip) );
        }

        if (Option & (FDMDATA_BM | FDMDATA_PA))
        {
            ((FDMDataU_2K_Powerchip*)Spare_buff) -> BM_PA = BMPA;
        }

        if (Option & FDMDATA_LBA)
        {
            ((FDMDataU_2K_Powerchip*)Spare_buff) -> LBA = LBA;
        }
    }
}
