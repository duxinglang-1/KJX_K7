...
#elif defined(MT6255) || defined(MT6250) || defined(MT6280) || defined(MT6260)
...
//Support MT6250 MT6260 Mask Code decompressor and LZMA when SFC Macro mode/ Flash busy
...
{
    kal_uint32 val;
#if defined(MT6250) || defined(MT6260) //Mask SFC AHB Channel 2 and 3 during SFC Mac mode/ Flash busy
    ...
    while (!(SFI_ReadReg32(RW_SFI_MISC_CTL) & SFI_REQ_IDLE));
     ...
     while (!(SFI_ReadReg32(RW_SFI_MISC_CTL) & SFI_REQ_IDLE));
#elif defined(MT6260) &&  !defined(__UBL__) //Wait until SFC AHB Channel 2 become IDLE before enter Macro mode
     while (!(SFI_ReadReg32(RW_SFI_MISC_CTL3) & SFI_CH2_TRANS_IDLE));
     while (!(SFI_ReadReg32(RW_SFI_MISC_CTL) & SFI_REQ_IDLE));
#endif
...
#if defined(MT6255) || defined(MT6250) || defined(MT6280) || defined(MT6260)
...
#if (defined(MT6250) &&  !defined(__UBL__)) || (defined(MT6260) &&  !defined(__UBL__))//UnMask SFC AHB Channel 2 and 3 when Flash is not busy
