...
int  ReadSectors(void * DriveData, kal_uint32 Sector, UINT Sectors, void * Buffer)
{
   ...
   if(dclHandle == DCL_HANDLE_INVALID)
	...
	#endif
	//if ((dclStatus == STATUS_ERROR_WRONG_STATE)) //&& (handle == &MSDC_Blk[1]))
	//{
	//	DclSD_Close(dclHandle);
	//	return FS_MSDC_READ_SECTOR_ERROR;
	//}
	...
	case FS_IOCTRL_QUERY_CARD_INFO:
		...
		*/
		...
		if (isCardExist){
		    ...
		    #if !defined(MT6260)
				info->DeviceInfo.Card.DeferredMount = KAL_FALSE;
			  #endif
		    #else
...
retry:
	...
	while(KAL_FALSE == receiveDone){
		...
		if (kal_mem_cmp(rxBuffer, CommandTag, 32) != 0) 
		{
			reWriteTimes++;
			if(2 > reWriteTimes){
				kal_print("read: not correct header, retry\r\n");
				goto retry;
			}

			kal_print("read: not correct header\r\n");
