...
char superNum[20];
int g_is_sms_send = 0;
U16 kjx_call_index = 0;/*1:sending;2:in call;3:end call;4:no number;5:incoming*/
...
void kjx_reset_nvram_info(void)
{
	...
	mmi_ucs2cpy((S8*)usc2Path, (const CHAR *)(L"Z:\\@tour\0"));
...
void kjx_monitor_make_call(void)
{
	...
	if(g_kjx_tour_struct.listenStatus&&strlen(g_kjx_tour_struct.listenNum) != 0)
	{
		memset(ucs2Num, 0x00, sizeof(ucs2Num));
		mmi_asc_to_ucs2(ucs2Num,g_kjx_tour_struct.listenNum);
	#ifdef SMS_TOUR_DEBUG
	#if defined(__KJX_LOG_OUTPUT__)
		LOGD("g_kjx_tour_struct.listenNum:%s", g_kjx_tour_struct.listenNum);
	#endif
	#endif
	}
	else
 ...
 * PARAMETERS
 *  KJX,Equipment No,Project Name(TOUR),Mode#
 *  For example:KJX,12345678901234,TOUR,1#
 * RETURNS
 *  void
 *****************************************************************************/
...
cmdcode kjx_cmd_process_handle(U8 *asciiNum, U8 *asciiContent)
{
...
#ifdef SMS_TOUR_DEBUG
#if defined(__KJX_LOG_OUTPUT__)
	LOGD("asciiNum:%s, asciiContent[%d]:%s", __func__, asciiNum, nLen, asciiContent);
#endif
#endif
	...
	if(cmdcode<SL_CMD_CODE_MAX)
	{
		...
		case SL_CMD_CODE_ADMIN:
		{
			...
			if(pos_1 && (ABS(pos_1-pos)<sizeof(g_kjx_tour_struct.deviceNumber)))
			{
				memset(g_kjx_tour_struct.deviceNumber, 0x00, sizeof(g_kjx_tour_struct.deviceNumber));
				memcpy(g_kjx_tour_struct.deviceNumber, pos, ABS(pos_1-pos));	
			}

			if(atoi(g_kjx_tour_struct.deviceNumber)!=3509006)
				return SL_CMD_CODE_MAX;

			kjx_reset_nvram_info();
			//kjx_net_nvram_reset();
			/*恢复设置成功 */
			kjx_sms_send_sms(g_kjx_tour_struct.superNum, set_cb_info[CurrCMD].cmdString, 0x08, NULL);
...
void kjx_tour_in_area_alarm_callback(void)
{
	...
	if(sms_send < 3)
	{
		if(strlen(g_kjx_nvram_struct.sosNum[sms_send])>0)
		{
			...
			kjx_sms_send_sms(g_kjx_nvram_struct.sosNum[sms_send_count],(unsigned short *)smsString, 0x08, kjx_tour_in_area_alarm_callback);
		}
...
void kjx_tour_out_area_alarm_callback(void)
{
	static U16 sms_send = 0;
	//安全提醒：您的家人已离开指定区域范围！
	...
	if(sms_send < 3)
	{
		...
		{
			sms_send_count = sms_send;
			kjx_sms_send_sms(g_kjx_nvram_struct.sosNum[sms_send_count],(unsigned short *)smsString, 0x08, kjx_tour_out_area_alarm_callback);
		}
		else if(strlen(g_kjx_nvram_struct.sosNum[sms_send+1])>0)
		{
			sms_send_count = (sms_send+1);
...
void kjx_tour_send_sms_in_area_alarm(void)
{
	U16 i = 0;
	//安全提醒：您的家人已进入指定区域范围！
	char smsString[]={0x89,0x5b,0x68,0x51,0xd0,0x63,0x92,0x91,0x1a,0xff,0xa8,0x60,0x84,0x76,0xb6,0x5b,0xba,0x4e,0xf2,0x5d,0xdb,0x8f,0x65,0x51,0x7,0x63,0x9a,0x5b,0x3a,0x53,0xdf,0x57,0x3,0x83,0xf4,0x56,0x1,0xff,0x0,0x0};
	...
	for(i=0;i<3;i++)
	{
		...
		{
			sms_send_count = i;
			kjx_sms_send_sms(g_kjx_nvram_struct.sosNum[i],(unsigned short *)smsString, 0x08, kjx_tour_in_area_alarm_callback);
			break;
		}
	}
}

void kjx_tour_send_sms_out_area_alarm(void)
{
	U16 i = 0;
	//安全提醒：您的家人已离开指定区域范围！
	char smsString[]={0x89,0x5b,0x68,0x51,0xd0,0x63,0x92,0x91,0x1a,0xff,0xa8,0x60,0x84,0x76,0xb6,0x5b,0xba,0x4e,0xf2,0x5d,0xbb,0x79,0x0,0x5f,0x7,0x63,0x9a,0x5b,0x3a,0x53,0xdf,0x57,0x3,0x83,0xf4,0x56,0x1,0xff,0x0,0x0};
	...
	for(i=0;i<3;i++)
	{
		...
		{
			sms_send_count = i;
			kjx_sms_send_sms(g_kjx_nvram_struct.sosNum[i],(unsigned short *)smsString, 0x08, kjx_tour_out_area_alarm_callback);
			break;
		}
	}
}
...
void kjx_tour_send_call_area_alarm(void)
{//电话推送报警
	
}

U16 kjx_sos_outcalling_sign = 0;
void kjx_tour_key_sos1(void)
{
#ifdef __KJX_FACTORY_TEST__
	MakeCall(L"10086");
	U_PutUARTBytes(0,"kjx_tour_key_sos1\r\n", strlen("kjx_tour_key_sos2\r\n"));
#else
...
void kjx_tour_key_sos2(void)
{
#ifdef __KJX_FACTORY_TEST__
	MakeCall(L"13480106089");
	U_PutUARTBytes(0,"kjx_tour_key_sos2\r\n", strlen("kjx_tour_key_sos2\r\n"));
#else
...
void kjx_tour_key_sos3(void)
{
	...
	if((kjx_is_in_call()))
	{
		StartTimer(KJX_TIMER_ID_KEY_SOS,1000*3,kjx_tour_key_sos3);
	#ifdef SMS_TOUR_DEBUG
	#if defined(__KJX_LOG_OUTPUT__)
		LOGD("return:is in call!");
	#endif
	...
	#ifdef SMS_TOUR_DEBUG
	#if defined(__KJX_LOG_OUTPUT__)
		LOGD("return kjx_sos_in_call==TRUE");
	#endif
	#endif
...
#ifdef SMS_TOUR_DEBUG
#if defined(__KJX_LOG_OUTPUT__)
	LOGD("kjx_sos_screen:%d, kjx_sos_in_call:%d", kjx_sos_screen, kjx_sos_in_call);
#endif
#endif
	...
	{
		StartTimer(KJX_TIMER_ID_KEY_SOS,1000*5,kjx_tour_key_sos3);
	}
#endif	
}

U8 kjx_get_sos_call_mode(void)
{
	return g_kjx_nvram_struct.sosCallMode;
}

void kjx_tour_key_sos(void)
{
	char ucs2Num[64]={0};
	char sosNum[32];
	static U8 sosIndex = 0;
	...
	if((strlen(g_kjx_nvram_struct.sosNum[0]) == 0)
		&&(strlen(g_kjx_nvram_struct.sosNum[1]) == 0)
		&&(strlen(g_kjx_nvram_struct.sosNum[2]) == 0)
	)
	{
		mmi_popup_display((WCHAR*)GetString(STR_KJX_NO_SOS_NUMBER), MMI_EVENT_FAILURE, NULL);
		return;
	}
	...
	if((kjx_is_in_call()))
	{
		StartTimer(KJX_TIMER_ID_KEY_SOS,1000*3,kjx_tour_key_sos);
	#ifdef SMS_TOUR_DEBUG
	#if defined(__KJX_LOG_OUTPUT__)
		LOGD("return:is in call!");
	#endif
	...
	StartTimer(KJX_TIMER_ID_KEY_SOS,1000*5,kjx_tour_key_sos);
}

void mmi_idle_make_call(void)
{
	S8 mynuber[] = {"13410836965"};
	U8 ucs_nuber[41];
...
extern void kjx_key_handler_extra_a(void);
void kjx_tour_set_idle_key_handler(void)
{
#ifdef SMS_TOUR_DEBUG
#if defined(__KJX_LOG_OUTPUT__)
	LOGD("begin");
	...
	//SetKeyHandler(kjx_tour_key_sos1, KEY_VOL_UP, KEY_LONG_PRESS);
	//SetKeyHandler(kjx_tour_key_sos2, KEY_VOL_DOWN, KEY_LONG_PRESS);
	//SetKeyHandler(kjx_tour_key_sos, KEY_CAMERA, KEY_LONG_PRESS);
	//SetKeyHandler(mmi_idle_make_call, KEY_CAMERA, KEY_EVENT_UP);
	//SetKeyHandler(kjx_key_handler_extra_a, KEY_CAMERA, KEY_EVENT_DOWN);
