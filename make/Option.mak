...
######################################################################################
KJX_FUN = TRUE
MOTION_SENSOR_SUPPORT = MC3451_I2C	#NONE	#MC3451_I2C	#xb add 2018-08-08 G-sensor传感器

ifeq ($(strip $(KJX_FUN)), TRUE)

	VERSION = RELEASE		#DEBUG  RELEASE
	COMPLIST += kjx
	CUS_REL_SRC_COMP += kjx
	#COMPOBJS += kjx\lib\kjxlib.a
	#COMPOBJS += custom\drv\Ble_mod\ble_mod.lib
	
	COM_DEFS += __KJX_FUN__
	COM_DEFS += __KJX_RFID_PLAY_CONTINUE_AGAIN__		#xb add 2016-08-25 rfid播放被来电或者语音推送中断之后要重播一次
	COM_DEFS += __KJX_BAT_STATUS_LED_SHOW__			#xb add 2016-08-25 专门的红绿双色灯显示电池状态
	COM_DEFS += __KJX_OLED_64_48_UI__			#xb add 2016-08-31 64X48的OLED屏幕
	COM_DEFS += __MMI_BOOTUP_ANI_WAIT_AUDIO__		#xb add 2016-09-19 开机动画等待开机铃声同步
	COM_DEFS += __KJX_TOUR_NAME_STORE_CARD__		#xb add 2016-10-17 景点名称先T卡默认存储一个文件，后面可以从平台更新
	COM_DEFS += __SLC_SWITCH_SPK2IN1__   			#听筒喇叭二合一
	COM_DEFS += __KJX_RFID__				#RFID总宏
	COM_DEFS += __KJX_AT_APP__				#xb add 2017-03-11 AT指令
	COM_DEFS += __KJX_SMS_APP__
	COM_DEFS += __KJX_LOG_OUTPUT__				#xb add 2017-03-11 LOG信息
	#COM_DEFS += __KJX_AUDIO_FILE_DECODE__			#xb add 2017-03-11 音频配置文件被加密
	COM_DEFS += __KJX_FAST_KEY_HANDLER__			#xb add 2017-06-15 快捷键响应函数
	COM_DEFS += __KJX_MULTI_LANGUAGE__			#xb add 2017-06-23 多国语
	COM_DEFS += __KJX_RESET_USED_COUNT__			#xb add 2017-05-04 复位统计
	COM_DEFS += __KJX_TOURIST_MAP_SUPPORT__			#xb add 2017-07-21 增加导游图显示
	COM_DEFS += __KJX_FENCE_SUPPORT__			#xb add 2017-12-14 增加后台围栏报警通知设备
	COM_DEFS += __KJX_RFID_SEQUENTIAL_TRIGGER__		#xb add 2019-02-20 增加RFID按照设定顺序依次触发
	COM_DEFS += __KJX_DJ_SUPPORT__					#xb add 2020-12-07 对讲功能的总宏开关
	
	COM_DEFS += __BOSCH_BMP280_PRESSURE__			#xb add 2017-06-28 博世气压传感器
	#COM_DEFS += __BLE_MODULE_SUPPORT__			#xb add 2017-07-13 iBeacon蓝牙模块
	COM_DEFS += __KJX_AGAINST_THEFT__			#xb add 2017-10-25 防丢报警
	#COM_DEFS += __KJX_AGAINST_THEFT_BY_RFID_NUM__		#xb add 2018-04-25 通过特殊编码的普通发射源触发防丢报警
	#COM_DEFS += __KJX_AGAINST_THEFT_TO_PILE__		#xb add 2017-10-25 防丢报警防盗桩
	COM_DEFS += __KJX_POSITION_TRIGGER_PLAY__		#xb add 2017-01-03 位置版
	COM_DEFS += __KJX_POSITION_TRIGGER_IN_LOCAL__		#xb add 2017-01-03 位置版经纬度设备本地判断
	#COM_DEFS += __KJX_RFID_LOCATION__			#xb add 2017-01-03 RFID辅助定位版	
	#COM_DEFS += __KJX_INCOMING_CALL_CHECK__		#xb add 2017-12-14 增加来电过滤控制宏
	#COM_DEFS += __KJX_SERVER_CMD_PLAY__			#xb add 2017-02-24 后台指令直接播放音频
	#COM_DEFS += __KJX_WEATHER_SUPPORT__			#xb add 2017-07-21 增加后台获取天气情况
	#COM_DEFS += __KJX_SOC_APP__				#xb add 2017-05-02 SOC联网总宏
	#COM_DEFS += __KJX_SMS_COMMAND__			#xb add 2016-10-20 短信指令处理
	#COM_DEFS += __KJX_ALARM__ 				#闹钟
	#COM_DEFS += __KJX_SMS_ACTIVATE__			#短信设定工作模式
	#COM_DEFS += __KJX_FOREIGN__				#xb add 支持语言自适应，根据cellid mcc 自动设置系统语言
	#COM_DEFS += __KJX_FUN_SNDREC__
	#COM_DEFS += __DRV_BMT_HIGH_BATTERY_SUPPORT__	#xb 
	
  ifneq ($(strip $(CUSTOMER_NAME)), A3)
	COM_DEFS += __KJX_MMI_SHOW_VER_IMEI__ # 按键显示imei和版本号
  endif

	COM_DEFS += __KJX_BOOT_LOGO__   #HELLO
	#COM_DEFS += __KJX_FACTORY_TEST__     #工程测试宏，量产软件必须关闭

	COMMINCDIRS += kjx\inc
	COM_DEFS += __KJX_NEW_NVRAM__

  ifeq ($(strip $(KJX_GPS_SUPPORT)), TRUE)
	COM_DEFS += __KJX_FUN_GPS__
	COM_DEFS += __KJX_FUN_AGPS__			#xb add 2018-03-01 AGPS功能
	COM_DEFS += __KJX_FUN_GPS_ZKW__  #中科微芯片
	COM_DEFS += __KJX_FUN_GPS_HD__   #华大芯片
	#COM_DEFS += __KJX_FUN_GPS_TD__   #泰斗芯片
	#COM_DEFS += __KJX_TRACKER__   #catcher 抓LOGO
  endif

  ifneq ($(strip $(CUSTOMER_NAME)),)
	COM_DEFS += CUSTOMER_NAME_$(strip $(CUSTOMER_NAME))
	COM_DEFS += CUSTOMER_NAME_CARD
  endif

	COM_DEFS += __KJX_NBR_CELL__  #多基站定位
  ifeq ($(strip $(VERSION)), DEBUG)
	COM_DEFS += __KJX_FUN_UART_TEST__  #串口1用于打印测试
	COM_DEFS += __KJX_DEBUG__
  endif

  ifeq ($(strip $(DROP_DETECT)),TRUE)
	COM_DEFS += __LIGHT_DROP_DETECT__   #光感
  endif

  ifeq ($(strip $(LED_LIGHT)),TRUE)
	COM_DEFS += __MEGE_LED_LIGHT__  #LED light 控制led灯效果
  endif

  ifeq ($(strip $(MOTION_SENSOR_SUPPORT)),MC3451_I2C)
	COM_DEFS += __KJX_FUN_SENSOR__
	COM_DEFS += __KJX_FUN_STEPS__
	COM_DEFS += __MOTION_SENSOR_MC3451_SUPPORT__
  endif
  
  ifeq ($(strip $(MOTION_SENSOR_SUPPORT)),MC3413_I2C)
	COM_DEFS += __KJX_FUN_SENSOR__
	COM_DEFS += __MOTION_SENSOR_MC3413_SUPPORT__
	COMPLIST += mc3413
	CUS_REL_SRC_COMP += mc3413
  endif

  ifeq ($(strip $(MOTION_SENSOR_SUPPORT)),MMA8452Q_I2C)
	COM_DEFS += __KJX_FUN_SENSOR__
	COM_DEFS += __MOTION_SENSOR_MMA8452Q_SUPPORT__
	COMPLIST += sensor
	CUS_REL_SRC_COMP += sensor
  endif

endif#KJX_FUN

ifeq ($(strip $(NPR_TTS_SUPPORT)),TRUE)
	COM_DEFS += __NPR_TTS_UI__
	COM_DEFS += __NPR_TTS__
	COM_DEFS += __NPR_TTS_RES_IN_ROM__
	COMPOBJS += plutommi\mmi\npr\npr_tts\lib\tts_big.a
endif

ifeq ($(strip $(HEART_RATE_SUPPORT)),TRUE)
	COM_DEFS += __HEART_RATE_SUPPORT__
	COMPOBJS += custom\drv\rate_sensor\PAH8001EI_2G\paw8001motion30_1_arm7_ds5.a
endif
