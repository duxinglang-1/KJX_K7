...
#with -o obj_list_gen
	...
	@if exist make\~FS_LinkError.tmp \
		(if exist $(strip $(COMPLIBDIR))\custom.lib (del /q /f $(strip $(COMPLIBDIR))\custom.lib)) && \
		(if exist $(strip $(COMPLIBDIR))\system.lib (del /q /f $(strip $(COMPLIBDIR))\init.lib)) && \
		(if exist $(strip $(COMPLIBDIR))\system.lib (del /q /f $(strip $(COMPLIBDIR))\mtd.lib)) && \
		(if exist $(strip $(OBJSDIR))\custom\*.obj (del $(strip $(OBJSDIR))\custom\*.obj)) && \
...
cus_ini_mtd_lib: cksysdrv
	...
	@(tools\make.exe -fmake\gsm2.mak -r -R XGC_AND_NOT_BOOTLOADER=FALSE custom.lib init.lib mtd.lib) && \
	(if ERRORLEVEL 1 exit 1)
	@if /I "$(strip $(MBIS_EN))"  EQU "TRUE" (@perl -e "print 'T_E,$@,T,'. time . \"\n\"";>>$(MBIS_BUILD_TIME_TMP))
