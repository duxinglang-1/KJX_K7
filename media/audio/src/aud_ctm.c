...
kal_bool aud_ctm_send_text_req_queue_full(aud_ctm_send_text_req_queue_struct *req_queue)
{
    ...
    if (tail < head)
    {
        return (kal_bool)(head - tail == 1);
    }

    return (kal_bool)(tail - head == CTM_MAX_SEND_REQ_NUM - 1);
}
    ...
    *more_data = KAL_FALSE;
    ...
    for (num_of_char = 0; num_of_char < max_num_of_char; num_of_char++)
    {
        ...
        if (text_bytes != 0)
        {
            ...
            MED_ASSERT(recv_bytes == (kal_uint32) text_bytes);
        }
        ...
        CTM_VALUE_TRACE(ucs2);

        dest[num_of_char << 1] = (kal_uint8) (ucs2 & 0xFF);
 ...
 *  
 *****************************************************************************/
kal_bool aud_ctm_is_tty_on(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    return (kal_bool)(aud_ctm_ctx.state != AUD_CTM_STATE_IDLE);

}

/*****************************************************************************
 * FUNCTION
 *  aud_ctm_headset_mode_update_mic
 * DESCRIPTION
 *  
 * PARAMETERS
 *  with_mic      [IN]        
 ...
 *  
 *****************************************************************************/
void aud_ctm_headset_mode_update_mic(kal_bool with_mic)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_bool tty_on = aud_ctm_is_tty_on();
    kal_uint8 input_device  = 0;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/

    if (tty_on && aud_ctm_ctx.mode != AUD_CTM_VCO_MODE && aud_ctm_ctx.mode != AUD_CTM_DIRECT_MODE)
    {
        return;
    }
    else
 ...
 *  
 *****************************************************************************/
void aud_ctm_set_speech_vol_and_path(kal_uint8 audio_mode)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        /* tty is on */
        switch(aud_ctm_ctx.mode)
        {
            case AUD_CTM_HCO_MODE:
            {
            #ifdef __MED_BT_HFP_MOD__
                if (aud_bt_hfp_is_audio_path_on())
                {
                    scene = SPH_ENH_AND_FIR_SCENE_CTM_HCO_BT;
                }
                else
            #endif /* __MED_BT_HFP_MOD__ */   
                {
                    switch(audio_mode)
                    {
                        ...
                        {
                            scene = SPH_ENH_AND_FIR_SCENE_CTM_HCO_EARPHONE;
                            break;
                        }
                        ...
                        #ifdef __MED_RTP_MOD__
                            if(voip_on)
                                scene = SPH_ENH_AND_FIR_SCENE_CTM_HCO_VOIP_LOUDSPK;
                            else
                        #endif
                                scene = SPH_ENH_AND_FIR_SCENE_CTM_HCO_LOUDSPK;
                            break;
                        }
                        ...
                        #ifdef __MED_RTP_MOD__
                            if(voip_on)
                                scene = SPH_ENH_AND_FIR_SCENE_CTM_HCO_VOIP_NORMAL;
                            else
                        #endif
                                scene = SPH_ENH_AND_FIR_SCENE_CTM_HCO_NORMAL;
                            break;
                        }
                    }
                }
            ...
            case AUD_CTM_VCO_MODE:
            {
            #ifdef __MED_BT_HFP_MOD__
                if (aud_bt_hfp_is_audio_path_on())
                {
                    scene = SPH_ENH_AND_FIR_SCENE_CTM_VCO_BT;
                }
                else
            #endif /* __MED_BT_HFP_MOD__ */   
                {
                    ...
                    {
                        case AUD_MODE_HEADSET:
                            scene = SPH_ENH_AND_FIR_SCENE_CTM_VCO_EARPHONE;
                            break;
                        case AUD_MODE_LOUDSPK:
                            scene = SPH_ENH_AND_FIR_SCENE_CTM_VCO_LOUDSPK;
                            break;
                        default:
                            scene = SPH_ENH_AND_FIR_SCENE_CTM_VCO_NORMAL;
                            break;
                    }
                }
                break;
            ...
            default:
            {
                scene = SPH_ENH_AND_FIR_SCENE_CTM_BAUDOT;
                break;
            }
    ...
    if (tty_on && aud_ctm_ctx.mode != AUD_CTM_HCO_MODE && aud_ctm_ctx.mode != AUD_CTM_DIRECT_MODE)
    {
...
#endif /* __MED_BT_HFP_MOD__ */
        {
            switch (audio_mode)
            {
                ...
                case AUD_MODE_HEADSET:
                    ...
                    break;
                case AUD_MODE_LOUDSPK:
                    input_device = AUDIO_DEVICE_MIC3;
                    break;
                default:
                    ASSERT(0);
                    break;                
        ...
        CTM_STATE_TRACE(aud_ctm_ctx.state) CTM_VALUE_TRACE(ind_p->event) switch (ind_p->event)
    {
        ...
        case CTM_CHAR_SENT:
            if (aud_ctm_ctx.state == AUD_CTM_STATE_SENDING)
            {
                ...
                MED_ASSERT(send_num_p != NULL && *send_num_p != 0);
...
void aud_ctm_close_up(void)
{
    ...
    CTM_STATE_TRACE(aud_ctm_ctx.state) if (aud_ctm_ctx.state != AUD_CTM_STATE_IDLE)
    {
        ...
        {
            med_free_aud_mem((void **)&aud_ctm_ctx.buf_p);
        }

        if (aud_ctm_ctx.ucs2_utf8_buf_p != NULL)
        {
            med_free_aud_mem((void **)&aud_ctm_ctx.ucs2_utf8_buf_p);
        }
 ...
 *  void
 *****************************************************************************/
void aud_ctm_resume(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_uint32 buf_len;
    kal_uint8 ctm_inferface;
    ...
    ctm_inferface = aud_ctm_ctx.mode;
    ...
    L1Ctm_Open((L1Ctm_Interface)ctm_inferface, aud_ctm_event_callback, aud_ctm_ctx.buf_p, buf_len, NULL);
...
void aud_ctm_open_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTM_FUNC_ENTRY(AUD_CTM_OPEN_REQ_HDLR)
    ...
    if (aud_context_p->speech_on)
    {
        ...
        /* alloc from ext mem */
        aud_ctm_ctx.buf_p = (kal_uint8*) med_alloc_aud_mem(buf_len);
        MED_ASSERT(aud_ctm_ctx.buf_p != NULL);

        aud_ctm_ctx.ucs2_utf8_buf_p = (kal_uint8*) med_alloc_aud_mem(CTM_TEXT_CONVERT_BUF_SIZE);
        MED_ASSERT(aud_ctm_ctx.ucs2_utf8_buf_p != NULL);
...
void aud_ctm_connect_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    CTM_STATE_TRACE(aud_ctm_ctx.state) 

    if (aud_ctm_ctx.state != AUD_CTM_STATE_READY)
        aud_send_ctm_connect_cnf(ilm_ptr->src_mod_id, MED_RES_FAIL);
    else
    {
        if (aud_ctm_ctx.mode == AUD_CTM_DIRECT_MODE)
        {
            aud_ctm_ctx.src_mod_id = ilm_ptr->src_mod_id;
...
void aud_ctm_send_text_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CTM_FUNC_ENTRY(AUD_CTM_SEND_TEXT_REQ_HDLR) 
    CTM_STATE_TRACE(aud_ctm_ctx.state) 
    ...
    switch (aud_ctm_ctx.state)
    {
        case AUD_CTM_STATE_CONNECTED:
            MED_ASSERT(aud_ctm_send_text_req_queue_count(&aud_ctm_ctx.send_req_queue) == 0);
            if (aud_ctm_ctx.mode == AUD_CTM_DIRECT_MODE)
            {
        ...
        case AUD_CTM_STATE_SENDING:
            ...
            if (aud_ctm_ctx.mode == AUD_CTM_DIRECT_MODE)
            {
                ...
                else
                {
                    ...
                    MED_ASSERT(sent_bytes == text_bytes);
                }
            }
            else
            {
