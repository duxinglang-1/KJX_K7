...
extern void* aud_media_get_desc_func_pointer(kal_int16 format); /* from aud_media.c */
...
kal_bool daf_parse_frame_header(kal_uint8 *data, daf_header_struct *header)
{
    ...
    /* padding bit */
    header->padding = (kal_bool)aud_get_bits(data, &bitcount, 1);
            ...
            kal_uint32 move_offset)
{
    ...
    for (round = 0; result != DAF_FIND_FRAME_TRUE; round++)
    {
        ...
        if (result == DAF_FIND_FRAME_FREE_BITRATE)
        {
            ...
            kal_mem_set((void*)&free_bitrate_header, 0, sizeof(daf_header_struct));
        ...
        else if (result == DAF_CHECK_FRAME_TRUE)
        {
            ...
            *offset = (kal_uint32) cur_pos;
            ...
            for (sync_count = 0; sync_count < 2; sync_count++)
            {
                ...
                MED_ASSERT(input_stream->ptr < input_stream->tail);
...
void daf_parse_data_info(aud_info_struct *info_p, audio_input_stream_struct *input_stream, id3_tag_struct *id3_tag)
{
    ...
    if ((data_size = audio_input_stream_get_data_size(input_stream)) >= 128)
    {
         ...
         * and parse & save data to frames
         */
        id3_tag_parse_v1_tag(input_stream, id3_tag);
    }
...
kal_bool get_aud_info(kal_wchar *filename, aud_info_struct *aud_info_p)
{
    ...
    kal_uint32 channel_num;
    audio_input_stream_struct *input_stream;
    media_get_info_func_ptr get_aud_info_func = NULL;
    void *param = NULL;
    ...
    /* Extra handling for some datatype */
    switch(format)
    {
    ...
    /* Get driver description function */
    get_aud_info_func = (media_get_info_func_ptr)aud_media_get_desc_func_pointer(format);
    ...
    if (get_aud_info_func != NULL)
    {
        ...
        if(aud_info != NULL)
        {
            ...
            {
                /*Clear buffer*/
                kal_mem_set((void*)aud_info, 0, sizeof(audInfoStruct));
            
                if (get_aud_info_func(&fsal_handle, aud_info, param) == MEDIA_SUCCESS)
                {
                //#ifdef WMA_DECODE
                    if (format == MED_TYPE_WMA || format == MED_TYPE_MP4 || format == MED_TYPE_3GP)
                    {
                        kal_wstrncpy(
                            (kal_wchar*) aud_info_p->title,
                        ...
                        if (format == MED_TYPE_MP4 || format == MED_TYPE_3GP)
                        {
                            kal_wstrncpy(
                                ...
                                (const kal_wchar*)aud_info->copyright,
                                MAX_ID3_COPYRIGHT_LEN);

                            kal_wstrncpy(
                                (kal_wchar*) aud_info_p->year,
                                (const kal_wchar*)aud_info->date,
                                MAX_ID3_YEAR_LEN);
                                
                            kal_wstrncpy(
                                (kal_wchar*) aud_info_p->genre,
                                (const kal_wchar*)aud_info->genre,
                                MAX_ID3_GENRE_LEN);                                
                        }
                    }
                //#endif /* WMA_DECODE */ 
                    ...
                    {
                        kal_wsprintf((kal_wchar*) aud_info_p->track_num, "%d", aud_info->trackNum);
                    }
                }
