...
static kal_mutexid g_input_stream_mutex;
static audio_input_stream_struct g_audio_input_stream;
 ...
 * DESCRIPTION
 *  This function is to initialize the input stream module.
 * PARAMETERS
 *
 * RETURNS
 *
 *****************************************************************************/
void audio_input_stream_init(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/    
    g_input_stream_mutex = kal_create_mutex("audio_input_stream_mutex");
}
...
audio_input_stream_struct *audio_file_input_stream_alloc(FS_HANDLE file)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    kal_take_mutex(g_input_stream_mutex);
    audio_input_stream_alloc(&g_audio_input_stream, file, 0, 0);
    return &g_audio_input_stream;
}
...
audio_input_stream_struct *audio_byte_input_stream_alloc(kal_uint8 *mem_src, kal_uint32 mem_src_size)
{
    ...
    /*----------------------------------------------------------------*/
    kal_take_mutex(g_input_stream_mutex);
    audio_input_stream_alloc(&g_audio_input_stream, NULL, mem_src, mem_src_size);
    return &g_audio_input_stream;
}
...
void audio_input_stream_free(audio_input_stream_struct *input_stream)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    med_free_aud_mem((void**)&input_stream->buf);
    kal_give_mutex(g_input_stream_mutex);
}


/*****************************************************************************
 * FUNCTION
 *  audio_input_stream_alloc
 * DESCRIPTION
 *  
 * PARAMETERS
 *  input_stream        [?]         
        ...
        kal_uint32 mem_src_size)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    input_stream->buf = (kal_uint8*)med_alloc_aud_mem(AUDIO_INPUT_STREAM_BUFFER_SIZE+1); 
