 ...
 *****************************************************************************/
#ifdef __NPR_TTS__
void iv_pcm_stop_req_hdlr(void);
#endif
static kal_int32 _aud_media_player_stop(med_aud_player_t* player_p)
{
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
#ifdef __NPR_TTS__
    if (IFLYTEK_TTSPlayer_IsPlaying())
	   	iv_pcm_stop_req_hdlr();
#endif
...
#ifdef __NPR_TTS__ /* PCM Play code for iflytek */
static MHdl 		*ivtts_mhdl = 0;
static kal_uint8 	ivtts_result = MED_RES_OK;
static kal_uint16 	ivtts_samplerate = 8000;
kal_uint16 iv_record_buffer[8*1024];		/*Â¼Òôbuf*/


extern unsigned char iv_tts_is_playing(void);
extern void iv_pcm_stop(void);
extern void iv_tts_start_step1(void);
extern void iv_tts_start_step2(void);
extern void iv_tts_play(void);
extern void iv_tts_wait(void);
extern void IV_TTS_Sleep( unsigned long nTime );

#define TTS_RING_BUFFER_LEN     (8*1024)  /* 16k */
kal_uint16 iv_ring_buffer[TTS_RING_BUFFER_LEN];		/*²¥Òôbuf*/

void iv_pcm_do(unsigned char iEven)
{
	switch (iEven)
	{
	case 0:
	...
	case 3:
		...
		break;
	case 4:
		aud_context_p->src_mod = MOD_MED;
		break;
	case 5:
		aud_context_p->src_mod = MOD_MMI;
		break;
	case 6:
...
//		AUD_ENTER_STATE(AUD_MEDIA_PLAY_FINISH);
	...
	case 14:
		...
		break;
	case 15:
		aud_context_p->last_mhdl_handle->SetBuffer(
			aud_context_p->last_mhdl_handle, 
			(kal_uint8*) iv_ring_buffer, 
			TTS_RING_BUFFER_LEN* 2);
		break;
	case 16:
		ivtts_mhdl = aud_context_p->last_mhdl_handle;
		break;
	default:
		break;
	}
...
unsigned char iv_pcm_check(unsigned char iEven)
{
	switch (iEven)
	{
	case 0:
		return (aud_context_p->last_mhdl_handle == ivtts_mhdl);
	case 1:
		return (ivtts_mhdl != 0);
	case 2:
		return (aud_context_p->speech_on != 0);
	case 3:
		return (aud_context_p->source_type != AUD_STRING);
	case 4:
		return (aud_context_p->play_mode != AUD_MEDIA_PLAY_AS_RING);
	case 5:
		return (aud_context_p->tone_playing != 0);
	case 6:
		return (aud_context_p->src_mod != MOD_MED);
	case 7:
...
kal_int8 NPR_pcm_push_data( kal_int16 *pData, kal_uint32 *pnSample)
{
	kal_uint8	*lpBuf = 0;
	kal_uint32 nBufLen = 0;
	if ( iv_tts_is_playing() )
	{
		aud_context_p->last_mhdl_handle->GetWriteBuffer(
			aud_context_p->last_mhdl_handle, 
			&lpBuf, &nBufLen);
		...
		if ( iv_tts_is_playing() )
		{
			aud_context_p->last_mhdl_handle->WriteDataDone(
				aud_context_p->last_mhdl_handle,
				nBufLen);
...
{       
	...
	if ( ivtts_mhdl )
	{
		if ( aud_context_p->last_mhdl_handle == ivtts_mhdl )
		{
			ivtts_mhdl = 0;
			aud_context_p->last_mhdl_handle->Stop(aud_context_p->last_mhdl_handle);
			aud_context_p->last_mhdl_handle->Close(aud_context_p->last_mhdl_handle);
...
kal_int32 npr_pcm_getdatacount (void)
{
	if ( iv_tts_is_playing() )
		{
		return aud_context_p->last_mhdl_handle->GetDataCount( aud_context_p->last_mhdl_handle );
		}
	else
		return 0;
}

void iv_pcm_play_callback( MHdl *mhdl, Media_Event event )
{
	switch( event )
	{
...
extern MHdl *PCM_Strm_Open_1(void(*handler)( MHdl *handle, Media_Event event ), void *param);
kal_uint8 iv_pcm_play_req_hdlr(void)
{
	MHdl* mhdl_handle_p = NULL;
	kal_int32 ivReturn = 0;
	Media_PCM_Stream_Param vpFormat;
	//NPR_Pub_trace_to_file(1, "iv_pcm_play_req_hdlr begin(%s:%d)",  __FILE__, __LINE__);

	if ( ivtts_mhdl || aud_context_p->state != AUD_MEDIA_IDLE )
	{
		ivtts_result = MED_RES_BUSY;
		return MED_RES_BUSY;
	}
	...
	else
	{
		...
		if (!(aud_context_p->state == AUD_MEDIA_RECORD ||
		aud_context_p->state == AUD_MEDIA_RECORD_PAUSED ||
		aud_context_p->state == AUD_VM_RECORD || aud_context_p->state == AUD_MEDIA_WAIT_RECORD))
		{
			aud_stop_unfinished_process();
			if ( aud_context_p->state != AUD_MEDIA_IDLE )
			{
			...
			if ( 8000 == ivtts_samplerate )
				aud_context_p->current_format = MED_TYPE_PCM_8K;
			else if ( 16000 == ivtts_samplerate )
				aud_context_p->current_format = MED_TYPE_PCM_16K;
			else
			{
				ivtts_result = MED_RES_FAIL;
				return MED_RES_FAIL;
			}
			iv_tts_start_step2();

			vpFormat.isStereo = 0;
			vpFormat.forceVoice     = KAL_TRUE;
			vpFormat.bitPerSample = 16;
			vpFormat.sampleFreq = ivtts_samplerate;

			if ((aud_context_p->last_mhdl_handle = PCM_Strm_Open_1(iv_pcm_play_callback, &vpFormat)) == NULL)
			{
			...
			iv_tts_play();
                                  ...
                                  if(aud_context_p->last_mhdl_handle->rbInfo.read != 0 ||
                                		aud_context_p->last_mhdl_handle->rbInfo.write == 0)
                                	{
                                		kal_int16 i = 0;
                                		aud_context_p->last_mhdl_handle->rbInfo.write = 4;
                                		for(; i < 4; i ++)
                                		{
                                			aud_context_p->last_mhdl_handle->rbInfo.rb_base[i] = 0;
                        			}
                                	}
			if ((ivtts_result = aud_context_p->last_mhdl_handle->Play(aud_context_p->last_mhdl_handle)) != MEDIA_SUCCESS)
			{
				aud_context_p->last_mhdl_handle->Close(aud_context_p->last_mhdl_handle);
				ivtts_result = aud_get_res(ivtts_result);
				return ivtts_result;
			}
			ivtts_result = MED_RES_OK;
			iv_tts_wait();
		}
		else
		{
...
kal_uint8 npr_tts_pcm_play_start (kal_uint16 SampleFreq)
{
	ilm_struct *send_ilm;
	module_type curr_mod = 0,dest_mod = 0;
	ivtts_samplerate = SampleFreq;
	ivtts_result = MED_RES_OK;
	/* send msg to play pcm */
	curr_mod = stack_get_active_module_id();
	dest_mod = MOD_MED;
	send_ilm = (ilm_struct *)allocate_ilm(curr_mod);
	send_ilm->src_mod_id = curr_mod;

	send_ilm->dest_mod_id = MOD_MED;
	send_ilm->msg_id = MSG_ID_L4AUD_AUDIO_PLAY_BY_IVTTS_REQ;
	msg_send_ext_queue(send_ilm);
	IV_TTS_Sleep(10);
	return ivtts_result;
}

kal_uint8 npr_tts_pcm_play_stop (void)
{
	ilm_struct *send_ilm;
	module_type curr_mod=0,dest_mod = 0;
	ivtts_result = MED_RES_OK;
	...
	send_ilm->src_mod_id = curr_mod;
                #endif
	send_ilm->dest_mod_id = MOD_MED;
	send_ilm->msg_id = MSG_ID_L4AUD_AUDIO_STOP_BY_IVTTS_REQ;
	msg_send_ext_queue(send_ilm);
	IV_TTS_Sleep(10);
