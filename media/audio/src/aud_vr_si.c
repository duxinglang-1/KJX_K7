...
kal_int32 aud_vrsi_error(void)
{
    ...
    switch (errMsg)
    {
        ...
        case VRSI_ERR_EVALUATE_VER: /* Reach trial limit of demo version */
            return MED_RES_VRSI_ERR_EVALUATE_LIMIT;
        default:
 ...
 *  
 *****************************************************************************/
kal_int32 aud_vrsi_init(kal_uint16 app_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    vrsi_ctx.app_id = app_id;
    bufLen = VRSI_GetMemRequest(MAX_VRSI_FIXED_TAGS, MAX_VRSI_DYNAMIC_TAGS, MAX_VRSI_SD_TAGS);
    if ((vrsi_ctx.buffer = (kal_uint8*) med_alloc_asm_mem(app_id, bufLen)) == NULL)
    {
        VRSI_Close();
        return MED_RES_VRSI_ERR_MEM_INSUFFICIENT;
    }
    ...
    {
        VRSI_Close();
        med_free_asm_mem(app_id, (void **)&vrsi_ctx.buffer);
        return MED_RES_VRSI_ERR_DATA_ERROR;
    }
...
void aud_vrsi_close(void)
{
    ...
    med_free_asm_mem(vrsi_ctx.app_id, (void **)&vrsi_ctx.buffer);
 ...
 *  aud_vrsi_is_active
 * DESCRIPTION
 *  This function is used to check if VRSI is being active.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
kal_bool aud_vrsi_is_active(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
