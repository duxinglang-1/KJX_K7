...
void aud_vrsi_init_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    media_vrsi_init_req_struct *msg_p = (media_vrsi_init_req_struct*) ilm_ptr->local_para_ptr;
    kal_int32 result;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    result = aud_vrsi_init(msg_p->app_id);

    aud_set_result_and_event(result,VRSI_EVT_INIT);
}
...
void aud_vrsi_add_tags_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    {
        aud_set_result_and_event(result,VRSI_EVT_ADD_TAGS);
    }
}
...
void aud_vrsi_start_trn_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    if (msg_p->grammar != NULL)
    {
        VR_SET_EVENT(VRSI_EVT_START_CMD_TRN);
    }
    else
    {
        VR_SET_EVENT(VRSI_EVT_START_DIGIT_ADAPT);
    }
}
...
void aud_vrsi_start_rcg_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    if (msg_p->grammar != NULL)
    {
        VR_SET_EVENT(VRSI_EVT_START_CMD_RCG);
    }
    else
    {
        VR_SET_EVENT(VRSI_EVT_START_DIGIT_RCG);
    }
}
...
void aud_vrsi_process_ind_hdlr(ilm_struct *ilm_ptr)
{
    ...
    switch (msg_p->proc_type)
    {
        ...
        case VRSI_ADD_TAGS_OK:
            ...
            aud_set_result_and_event(MED_RES_OK,VRSI_EVT_ADD_TAGS);
            break;
        case VRSI_TTS_OK:
            vrsi_ctx.state = MED_AUD_VRSI_STATE_READY;
            vrsi_ctx.session_on = KAL_FALSE;
...
void aud_vrsi_play_tag_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    aud_set_result_and_event(result,VRSI_EVT_PLAY_TAG);
...
void aud_vrsi_play_tts_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    aud_set_result_and_event(result,VRSI_EVT_PLAY_TTS);
...
void aud_vrsi_get_tagnum_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    aud_set_result_and_event(result,VRSI_EVT_GET_TAG_NUM);
...
void aud_vrsi_get_taginfo_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    aud_set_result_and_event(result,VRSI_EVT_GET_TAG_INFO);
...
void aud_vrsi_del_tags_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    aud_set_result_and_event(result,VRSI_EVT_DEL_TAGS);
...
void aud_vrsi_reset_tags_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    aud_set_result_and_event(result,VRSI_EVT_RESET_TAGS);
...
void aud_vrsi_sync_db_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    aud_set_result_and_event(result,VRSI_EVT_SYNC_DB);
...
void aud_vrsi_abort_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    AUD_VALUE_TRACE(vrsi_ctx.session_on, vrsi_ctx.session_id, msg_p->session_id);
    
    if (!vrsi_ctx.session_on || vrsi_ctx.session_id != msg_p->session_id)
    {
        VR_SET_EVENT(VRSI_EVT_ABORT);
        return;
    }
