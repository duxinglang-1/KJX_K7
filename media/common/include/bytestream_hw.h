...
#if !defined(WIN32) && defined(__RVCT__)
   typedef long long hw_bytestream_jmpbuf[11];
#else
   typedef int hw_bytestream_jmpbuf[11];
#endif

typedef struct _bytestream_dcb
{
	hw_bytestream_jmpbuf jmp_buf;
...
#define GIF_GET_U16() 	hw_bytestream_func[hw_bytestream_dcb[0].t].hw_bytestream_get_word(0, LITTLE_ENDIAN)
#define GIF_GET_U32() 	hw_bytestream_func[hw_bytestream_dcb[0].t].hw_bytestream_get_dword(0, LITTLE_ENDIAN)
