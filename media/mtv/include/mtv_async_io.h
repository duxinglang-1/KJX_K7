...
typedef void (*async_io_notification_func)(void* param);
typedef struct async_io_notification_t async_io_notification_t;

/**
  * Notification that should be called when a block is processed.
  */
struct async_io_notification_t
{
    async_io_notification_func func;
    void* param;

    async_io_notification_t* next;
};
...
struct async_io_block_t
{
    ...
    void* owner;
    async_io_notification_t* notifications;
...
typedef struct
{
    ...
    kal_uint32 errors;              /**< error count during I/O operations */
...
/**
  ...
  *
  * @param cleanup [in] cleanup callback
  * @param cleanup_param [in] opaque parameters for cleanup callback function
  */
...
/**
  * Check whether the writing speed matches the input rate.
  *
  * @return KAL_TRUE if the writing is not fast enough for the input rate. KAL_FALSE otherwise.
  */
kal_bool async_io_is_writing_slow(async_io_t* io);

/**
  * Register a notification function after current queued I/O blocks are done.
  */
...
/**
  * Initialize AIO.
  */
extern void mtv_init_aio(void);

/**
  * Free AIO resources.
  */
extern void mtv_free_aio_resource(void);
