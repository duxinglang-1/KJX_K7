...
void vid_set_param_req_hdlr(ilm_struct *ilm_ptr)
{
    ...
    switch (req_p->param_id)
    {
        ...
        case VID_PARAM_SCRAMBLE_BUFFER:
        {
            media_vid_param_scramble_buffer_struct *param = (media_vid_param_scramble_buffer_struct *)req_p->value;

            error = mp4_mp_scramble_buffer(param->buf_ptr, param->buf_size);

            result = med_error_to_result_enum(error);
