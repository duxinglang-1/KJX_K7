...
/* under construction !*/
        ...
        {
            cnf->status = FT_AudioCheckFunctionSupported(query_op_code);
            break;
        }
        case FT_L4AUD_REQ_EX_ID:
        {
            cnf->status = FT_AudioExCheckFunctionSupported(query_op_code);
            break;
        }
        ...
        case FT_FM_REQ_ID:
        {
            if(FT_FM_OP_END > query_op_code)
            {
#if !defined(__SMART_PHONE_MODEM__)
                cnf->status = FMR_CheckOption();
        ...
        case FT_TDMB_REQ_ID:
            {
                if(FT_TDMB_OP_END > query_op_code)
                {
                    cnf->status = FT_CNF_OK;
                }
                break;
...
#endif
        /* CMMB could be switchable, query function should not wrapped with compile option */
        case FT_CMMB_REQ_ID:
        {
            if(Custom_META_CmmbSupported() == KAL_TRUE)
            {
                if(FT_CMMB_OP_END > query_op_code)
                {
                    cnf->status = FT_CNF_OK;
                }
            }
        ...
        /* MATV could be switchable, query function should not wrapped with compile option */
        case FT_MATV_CMD_REQ_ID:
        {
            if(FT_MATV_OP_END > query_op_code)
            {
       ...
       case FT_CCT_REQ_ID:
           cnf->status = ft_cct_check_op_is_support(query_op_code);
           break;
#if defined(__WIFI_SUPPORT__)
        case FT_WIFI_REQ_ID:
            if( FT_WIFI_OP_END > query_op_code && WiFi_Feature_Exist() == KAL_TRUE)
            {
                cnf->status = FT_CNF_OK;
            }
            break;
        case FT_WIFI_WNDRV_SET_REQ_ID:
            if( WNDRV_TEST_SET_END > query_op_code && WiFi_Feature_Exist() == KAL_TRUE )
            {
                cnf->status = FT_CNF_OK;
            }
            break;
        case FT_WIFI_WNDRV_QUERY_REQ_ID:
            if( WNDRV_TEST_QUERY_END > query_op_code && WiFi_Feature_Exist() == KAL_TRUE )
            {
                cnf->status = FT_CNF_OK;
            }
            break;
        ...
        {
            cnf->status = FT_NfcCheckFunctionSupported(query_op_code);
            break;
        }
        case FT_GPS_CMD_REQ_ID:
        {
            cnf->status = FT_GpsCheckFunctionSupported(query_op_code);
            break;
        }
        default:
            cnf->status = FT_CNF_FAIL;
...
void ft_util_query_bt_module_id(FT_UTILITY_COMMAND_CNF  *cnf) {
    extern kal_uint8 BT_CHIP_ID;
    cnf->result.bt_module_id = BT_CHIP_ID;
    cnf->status = FT_CNF_OK;
}
