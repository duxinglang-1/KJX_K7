...
static mmi_ret cui_pwd_guard_proc_for_guard_pwd(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
        case EVT_ID_CUI_PWD_BASIC_DONE:
            cntx->input_pwd_cui = cui_pwd_check_create(
                cntx->this_grp_id,
                cntx->input_prompt_str_id,
                cntx->confirm_prompt_str_id,
                cntx->input_min_pwd_len,
                cntx->input_max_pwd_len);
            cui_pwd_check_set_action_str(cntx->input_pwd_cui, cntx->done_str_id, STR_GLOBAL_BACK);
            cui_pwd_check_set_diff_input_prompt(cntx->input_pwd_cui, cntx->diff_input_str_id);
            cui_pwd_check_set_proc(cntx->input_pwd_cui, cui_pwd_guard_input_proc, cntx);
            cui_pwd_check_run(cntx->input_pwd_cui);
            break;
...
static mmi_ret cui_pwd_guard_input_proc(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
        case EVT_ID_CUI_PWD_CHECK_DONE:
        {
            cui_pwd_check_done_evt_struct *input_done_evt;
            cui_pwd_guard_done_evt_struct my_done_evt;

            MMI_TRACE(CUI_PWD_TRC_GROUP, TRC_CUI_PWD_GUARD_DONE, cntx->this_grp_id);

            input_done_evt = (cui_pwd_check_done_evt_struct*)evt;
            MMI_FRM_INIT_EVENT(&my_done_evt, EVT_ID_CUI_PWD_GUARD_DONE);
            my_done_evt.guard = cui_pwd_basic_get_input(cntx->guard_pwd_cui);
            my_done_evt.input = input_done_evt->input;
            return cui_pwd_guard_send_to_parent(cntx, &my_done_evt);
        }

        case EVT_ID_CUI_PWD_CHECK_CANCEL:
            cui_pwd_check_close(cntx->input_pwd_cui);
            cntx->input_pwd_cui = 0;
            break;

        case EVT_ID_CUI_PWD_CHECK_PASSIVE_CLOSING:
            /* Allow to close */
            cntx->input_pwd_cui = 0;
            break;

        case EVT_ID_CUI_PWD_CHECK_KEY:
        {
            cui_pwd_check_key_evt_struct *src_key_evt;
            cui_pwd_guard_key_evt_struct key_evt;

            src_key_evt = (cui_pwd_check_key_evt_struct*)evt;
            MMI_FRM_INIT_EVENT(&key_evt, EVT_ID_CUI_PWD_GUARD_KEY);
            key_evt.key_evt_id = src_key_evt->key_evt_id;
            key_evt.key_code = src_key_evt->key_code;
            key_evt.key_type = src_key_evt->key_type;
        ...
        case EVT_ID_CUI_PWD_CHECK_AUX:
        {
            cui_pwd_guard_aux_evt_struct aux_evt;
            cui_pwd_basic_aux_evt_struct *src_aux_evt;

            src_aux_evt = (cui_pwd_basic_aux_evt_struct*)evt;
            if (src_aux_evt->type == CUI_PWD_AUXILIARY_CHECK_DEFAULT)
            {
                MMI_FRM_INIT_EVENT(&aux_evt, EVT_ID_CUI_PWD_GUARD_AUX);
                aux_evt.type = CUI_PWD_AUXILIARY_GUARD_DEFAULT;
                return cui_pwd_guard_send_to_parent(cntx, &aux_evt);
        ...
        S32 input_max_pwd_len)
{
    ...
    cntx->guard_prompt_str_id = guard_prompt_str_id;
...
void cui_pwd_guard_run(mmi_id instance_id)
{
    ...
    cntx->guard_pwd_cui = cui_pwd_basic_create(
        cntx->this_grp_id,
        cntx->guard_prompt_str_id,
...
const WCHAR* cui_pwd_guard_get_input_of_active(mmi_id instance_id)
{
    ...
    {
        return cui_pwd_check_get_input_of_active(cntx->input_pwd_cui);
    }
    else if (cntx->guard_pwd_cui != 0)
    {
        return cui_pwd_basic_get_input(cntx->guard_pwd_cui);
    }
...
void cui_pwd_guard_clear_input_of_active(mmi_id instance_id)
{
    ...
    if (cntx->input_pwd_cui != 0)
    {
        cui_pwd_check_clear_input_of_active(cntx->input_pwd_cui);
    }
    else if (cntx->guard_pwd_cui != 0)
    {
        cui_pwd_basic_clear_input(cntx->guard_pwd_cui);
    }
}
...
MMI_BOOL cui_pwd_guard_is_on_guard_input_box(mmi_id instance_id)
{
    ...
    return (cntx->guard_pwd_cui != 0 && cntx->input_pwd_cui == 0);
}
