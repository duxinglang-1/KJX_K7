...
void mmi_rp_timer_output_phase1_app_def_h_file(U32 app_index, FILE *p_file);
#ifndef __RESGEN_INTERNAL_POP_FILE_REDUCE__
void mmi_rp_timer_output_phase2_app_c_file(U32 app_index, FILE *p_file);
#else // __RESGEN_INTERNAL_POP_FILE_REDUCE__
void mmi_rp_timer_output_phase2_app_c_file(U32 app_index);

#endif //__RESGEN_INTERNAL_POP_FILE_REDUCE__
#define MMI_RP_TIMER_CURR_APP_TIMER       g_mmi_rp_timer_cntx.app_timer_data[g_mmi_rp_mgr_cntx.cur_app_index]

mmi_rp_timer_cntx_struct g_mmi_rp_timer_cntx;
 ...
 *  mmi_rp_timer_init
 * DESCRIPTION
 *  Initializes the timer context structure. Called for every .res file parsed.
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_rp_timer_init(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    memset(&g_mmi_rp_timer_cntx, 0, sizeof(g_mmi_rp_timer_cntx));
    g_mmi_rp_timer_cntx.app_timer_data = (mmi_rp_timer_data_struct*)malloc(sizeof(mmi_rp_timer_data_struct) * g_mmi_rp_mgr_cntx.total_apps);

    if(!g_mmi_rp_timer_cntx.app_timer_data)
    {
        MMI_RP_FATAL_ERROR(RP_ERR_MEMORY_NOT_AVAILABLE);
    }
    memset(g_mmi_rp_timer_cntx.app_timer_data, 0, sizeof(mmi_rp_timer_data_struct) * g_mmi_rp_mgr_cntx.total_apps);

    g_mmi_rp_timer_cntx.hash = mmi_rp_hash_create(0xFFFF);    /* 0xFFFF is the resource ID range */
}
 ...
 *  void
 *****************************************************************************/
void mmi_rp_timer_set_app_range(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if(MMI_RP_TIMER_CURR_APP_TIMER.timers == NULL)
    {
        range = g_mmi_rp_mgr_cntx.cur_app->max - g_mmi_rp_mgr_cntx.cur_app->min + 1;
        MMI_RP_TIMER_CURR_APP_TIMER.timers = (mmi_rp_timer_id_struct*)malloc(sizeof(mmi_rp_timer_id_struct) * range);

        if((!MMI_RP_TIMER_CURR_APP_TIMER.timers) && (range > 0))
        {
            MMI_RP_FATAL_ERROR(RP_ERR_MEMORY_NOT_AVAILABLE);
        }

        memset(MMI_RP_TIMER_CURR_APP_TIMER.timers, 0, sizeof(mmi_rp_timer_id_struct) * range);
    }
}
 ...
 *  void
 *****************************************************************************/
int mmi_rp_timer_start_handler(void *data, const kal_char *el, const kal_char **attr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    kal_int32   index = 0;
    U8 *curr_timer_id = NULL;
    S32 parent_tag_id = -1;
    
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_RP_DEBUG_INFO("Timer - start: %s\n", el);
    ...
    if(MMI_RP_TIMER_CURR_APP_TIMER.total_count >= (U32)(g_mmi_rp_mgr_cntx.cur_app->max - g_mmi_rp_mgr_cntx.cur_app->min))
    {
    ...
    if(curr_timer_id != NULL)
    {
        if(!mmi_rp_hash_find(g_mmi_rp_timer_cntx.hash, curr_timer_id))
        {
            MMI_RP_TIMER_CURR_APP_TIMER.timers[MMI_RP_TIMER_CURR_APP_TIMER.total_count].timer_id_str = mmi_rp_hash_add(g_mmi_rp_timer_cntx.hash, curr_timer_id);
            MMI_RP_TIMER_CURR_APP_TIMER.total_count++;
        }
        else
        {
 ...
 *  void
 *****************************************************************************/
int mmi_rp_timer_end_handler(void *data, const kal_char *el)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
int mmi_rp_timer_data_handler(void *resv, const kal_char *el, const kal_char *data, kal_int32 len)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_RP_DEBUG_INFO("Timer -data: %s\n", data);
    return MMI_RP_PARSE_OK;
}
...
void mmi_rp_timer_filter(void)
{
    ...
    mmi_rp_timer_data_struct *curr_app_timer1 = NULL;
    ...
    {
        curr_app_timer1 = &(g_mmi_rp_timer_cntx.app_timer_data[i]);
        for(j =0; j < curr_app_timer1->total_count; j++)
        {
            ...
            curr_app_timer1->timers[j].filter = found;
        }
    }
}
 ...
 *  void
 *****************************************************************************/
void mmi_rp_timer_output_phase1(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    for(i = 0; i < g_mmi_rp_mgr_cntx.total_apps; i++)
    {
        ...
        {
            mmi_rp_timer_output_phase1_app_def_h_file(i, p_file);
            fclose(p_file);
        }
        else
        {
...
/*****************************************************************************
 * FUNCTION
 *  mmi_rp_timer_output_phase1_app_def_h_file
 * DESCRIPTION
 *  Output Resources
 * PARAMETERS
...
void mmi_rp_timer_output_phase1_app_def_h_file(U32 app_index, FILE *p_file)
{
    ...
    mmi_rp_timer_data_struct *curr_app_timer = &(g_mmi_rp_timer_cntx.app_timer_data[app_index]);
    ...
    if(p_file)
    {
        ...
        strcat(str_enum_name, "_timer_enum");
        ...
        strcat(str_enum_max, "_TIMER_MAX");
        ...
        /* Check if no str IDs. */
        if(curr_app_timer->total_count <= 0)
        {
        ...
        {
            fprintf(p_file, "\n\n");
            fprintf(p_file, "/******************** Timer resource IDs - begin ********************/\n");
            fprintf(p_file, "typedef enum\n{\n");

            for(i = 0; i < curr_app_timer->total_count; i++)
            {
                if(curr_app_timer->timers[i].filter == MMI_FALSE)
                {
                    continue;
                }
                ...
                if(first)
                {
                    fprintf(p_file, "    %s = %d + 1, /* BASE_ID + 1 */\n", curr_app_timer->timers[i].timer_id_str, g_mmi_resource_base_table[app_index].min);
                    first = MMI_FALSE;

                    app_enum_log_counter = g_mmi_resource_base_table[app_index].min + 1;
                    MMI_RP_OUTPUT_ENUM_LOG(curr_app_timer->timers[i].timer_id_str, app_enum_log_counter);
                    app_enum_log_counter++;
                }
                else
                {
                    fprintf(p_file, "    %s,\n", curr_app_timer->timers[i].timer_id_str);
                    MMI_RP_OUTPUT_ENUM_LOG(curr_app_timer->timers[i].timer_id_str, app_enum_log_counter);
                    app_enum_log_counter++;
                }
            }
            ...
            fprintf(p_file, "    %s\n", str_enum_max);
            fprintf(p_file, "}%s;\n", str_enum_name);
            fprintf(p_file, "/******************** Timer resource IDs - finish ********************/\n");
        }
    }
    else
    {
 ...
 *  void
 *****************************************************************************/
void mmi_rp_timer_output_phase2(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    for(i = 0; i < g_mmi_rp_mgr_cntx.total_apps; i++)
    {
        ...
        if(p_file = fopen(str_output_filename, "a"))            
        {
            mmi_rp_timer_output_phase2_app_c_file(i, p_file);
            fclose(p_file);
        }
        else
        {
...
/*****************************************************************************
 * FUNCTION
 *  mmi_rp_timer_output_phase2_app_c_file
 * DESCRIPTION
 *  Output Resources
 * PARAMETERS
 *  app_index      [IN]    App_index in resource base table
...
void mmi_rp_timer_output_phase2(void)
{
    ...
    for(i = 0; i < g_mmi_rp_mgr_cntx.total_apps; i++)
    {
        ...
        mmi_rp_timer_output_phase2_app_c_file(i);
    }
}
