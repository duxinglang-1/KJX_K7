...
#define free_buffer(P,X)   free(X)
#define get_buffer(P,X)    malloc(X)

#else /* defined(__RESOURCE_GEN_) */
...
static kal_int32 vir_read_file(XML_PARSER_STRUCT *parser, void * buf, kal_uint32 size, kal_uint32* len)
{
   /*----------------------------------------------------------------*/
    ...
    if(reserdatalen >= (kal_int32)size)
    {   
        ...
        *len = size;
        ...
        {   
            ...
            {   
#if defined(__RESOURCE_GEN_)
                fseek(fileptr->fp, fileptr->realfileseekpos, SEEK_SET);
                readlen = fread(fileptr->bufptr, sizeof(kal_char), fileptr->bufsize, fileptr->fp);
                if (ferror(fileptr->fp))
                {
                    ret = -1;
                }
#else
...
#if defined(__RESOURCE_GEN_)
            fseek(fileptr->fp, fileptr->realfileseekpos + reserdatalen, SEEK_SET);
            readlen = fread(fileptr->bufptr + FILEBUF_BACKSIZE, sizeof(kal_char), 
                            fileptr->bufsize - FILEBUF_BACKSIZE, fileptr->fp);
            if (ferror(fileptr->fp))
            {
                ret = -1;
            }
...
static void xml_free_buffer(XML_PARSER_STRUCT *parser, void *p)
{
    ...
    {
    #if defined(MMI_ON_WIN32) || defined(__RESOURCE_GEN_)
        OslMfree(p);
    #else 
        free_ctrl_buffer(p);
    #endif 
...
kal_int32 xml_parse(XML_PARSER_STRUCT *parser, kal_wchar *file_name)
{
    ...
    /*----------------------------------------------------------------*/
#if defined(__RESOURCE_GEN_)
    parser->source = fopen((const S8*)file_name, "r");
    if (parser->source == NULL)
    {
        parser->source = (FS_HANDLE)FS_FILE_NOT_FOUND;
    }
#else    
    ...
    if (parser->source == (FS_HANDLE)FS_FILE_NOT_FOUND)
    {
...
kal_int32 xml_parse_file_from_offset(XML_PARSER_STRUCT *parser, kal_int32 file_handle)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    parser->source = (FS_HANDLE)file_handle;   /* FS_Open ((WCHAR*)tmp_filename, FS_READ_ONLY); */
...
#if defined(__RESOURCE_GEN_)
        fclose(handle);
        parser->data_hdlr(parser, parser->elem[parser->curr_depth-1], (kal_char*)"xml_temp", 0, parser->error);
#else    
        FS_Close(handle);
 ...
 static kal_int32 xml_process_cdata(XML_PARSER_STRUCT *parser, kal_int32 length)
{
    ...
    while (i > 0)   /* i is used as the rest length of content data */
    {
        ...
        if (handle == (FS_HANDLE)-1)
        {
            break;
        }
#if defined(__RESOURCE_GEN_)
        len_write = fwrite(parser->ap_temp, sizeof(kal_char), len_read, handle);
        if (ferror(parser->source))
        {
            ret = -1;
        }        
#else
                ...
                kal_int32 len_data)
{
    ...
    {
#if defined(__RESOURCE_GEN_)
        len = fwrite(data, sizeof(kal_char), len_data, *handle);
#else    
...
#if defined(__RESOURCE_GEN_)
            *handle = fopen("xml_temp", "w+");
            len = fwrite(*dest, sizeof(kal_char), len_dest, *handle);
            len = fwrite(data, sizeof(kal_char), len_data, *handle);
#else
                            ...
                            int*length)
{
    ...
    if (parser->source != 0)
    {
       ...
       {
#if defined(__RESOURCE_GEN_)
        fseek(parser->source, from, SEEK_SET);
        if (ferror(parser->source))
        {
            ret = -1;
        }
...
#if defined(__RESOURCE_GEN_)
        *length = fread(buffer, sizeof(kal_char), size, parser->source);
        if (ferror(parser->source))
        {
            ret = -1;
        }
...
static kal_int32 xml_seek_xmltext(XML_PARSER_STRUCT *parser, kal_int32 offset, kal_int32 ref)
{
    ...
    if (parser->source != 0)
    {
        ...
        {
#if defined(__RESOURCE_GEN_)
            fseek(parser->source, offset, ref);
            if (ferror(parser->source))
            {
                return -1;
            }
