...
*******************************************************************************/
    ...
    {
        if ((jCount == begin + 1) && first_nsm == 1)
        {
            /* ignore the first CS or ES if NSM is at the beginning */  
            continue;
        }
        ...
        if (dest_types[jCount] == CS || dest_types[jCount] == ES)
        {
            ...
            if (g_cursor_index >= 0 && g_cursor_index == jCount + 1)
                nextToken = dest_types[jCount + 2];

            if ((dest_types[jCount] == ES || dest_types[jCount] == CS) && lastToken == EN && nextToken == EN)
            {
                dest_types[jCount] = EN;
            }
            else if(dest_types[jCount] == ES && nextToken == EN)
            {
                dest_types[jCount] = EN;
            }
