...
void csb_frm_create_csb_folder(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    sprintf(csb_folder_path_ascii, "%c%s", SRV_FMGR_CARD_DRV, CSB_DEFAULT_FOLDER);
    ...
    sprintf(csb_folder_path_ascii, "%c%s", SRV_FMGR_PUBLIC_DRV, CSB_DEFAULT_FOLDER);
    mmi_asc_to_ucs2((S8*) csb_folder_path_unicode, (S8*) csb_folder_path_ascii);
...
void csb_frm_start_view_auto(void)
{
    ...
    if(g_CSB_struct.screen_counter < TOTAL_CATEGORY_SCREENS - 1)
    {
        ...
        mmi_frm_scrn_enter(
            GRP_ID_CSB_ROOT, SCR_ID_CSB_DUMMY_2_SCR_ID, NULL, csb_frm_start_view_auto, MMI_FRM_FG_ONLY_SCRN);
        ...
        mmi_frm_scrn_multiple_close(GRP_ID_CSB_ROOT, mmi_frm_scrn_get_active_id(),0,0, SCR_ID_CSB_VIEW_SCR_ID,0);
        /* Start the timer */
...
void GoBackToMainMenu(void)
{
    ...
    {
        mmi_frm_scrn_multiple_close(GRP_ID_CSB_ROOT, mmi_frm_scrn_get_active_id(),0,0, SCR_ID_CSB_SEARCH_SCR_ID,0);
    }
...
void csb_frm_start_view_manual(void)
{
    ...
    if(g_CSB_struct.screen_counter < TOTAL_CATEGORY_SCREENS)
    {
        ...
        if (!mmi_frm_check_is_back_history())
        {
            mmi_frm_scrn_enter(
                GRP_ID_CSB_ROOT, SCR_ID_CSB_DUMMY_2_SCR_ID, NULL, csb_frm_start_view_manual, MMI_FRM_FG_ONLY_SCRN);
            mmi_frm_scrn_close(GRP_ID_CSB_ROOT, SCR_ID_CSB_DUMMY_SCR_ID);
        }
...
void csb_frm_validate_screen_name(void)
{
    ...
    {
        scrID = (U32) atoi((S8*)local_buffer);
...
void csb_frm_show_error_message(void)
{
    ...
    mmi_popup_display_simple((WCHAR*)((UI_string_type)GetString(STR_ID_CSB_SCR_NOT_EXIST_TEXT)), MMI_EVENT_ERROR, GRP_ID_CSB_ROOT, NULL);
    /* reset the buffer */
    memset(csb_buffer, CSB_NULL, MAX_CSB_SCREEN_NAME_LENGTH);
    
    mmi_frm_scrn_multiple_close(GRP_ID_CSB_ROOT, mmi_frm_scrn_get_active_id(),0,1,0,0);
}
...
void csb_frm_save_view_mode(void)
{
    ...
    mmi_popup_display_simple((WCHAR*)((UI_string_type)GetString(STR_GLOBAL_DONE)), MMI_EVENT_SUCCESS, GRP_ID_CSB_ROOT, NULL);
    ...
    for(index = 0; index < TOTAL_CATEGORY_SCREENS; index++)
    {
        search_flag = g_CSB_struct.pscreen_info[index].cont_search_flags;
            ...
            if(CSB_PANEL & search_flag)
                    csb_search_hint_popups[CSB_CONT_PANEL]++;
    }
}
    ...
    for(index = 0; index < TOTAL_CATEGORY_SCREENS; index++)
    {
        search_flag = g_CSB_struct.pscreen_info[index].app_search_flags;
            ...
            if(CSB_HID & search_flag)
                    csb_app_hint_popups[CSB_APP_HID]++;
    }
}
...
void csb_frm_testtool_callback(void)
{
    ...
    if (error_code == FALSE)
    {
        ...
        mmi_popup_display_simple((WCHAR*)((UI_string_type)GetString(STR_ID_CSB_JPEG_FAIL_TEXT)), MMI_EVENT_ERROR, GRP_ID_CSB_ROOT, NULL);
    ...
    if (mmi_frm_scrn_is_present(GRP_ID_CSB_ROOT, SCR_ID_CSB_DUMMY_SCR_ID, 0))
    {
        mmi_frm_scrn_close(GRP_ID_CSB_ROOT, SCR_ID_CSB_DUMMY_SCR_ID);
    }
    ...
    else
    {
        ...
        mmi_popup_display_simple((WCHAR*)((UI_string_type)GetString(STR_ID_CSB_TESTTOOL_PASS)), MMI_EVENT_SUCCESS, GRP_ID_CSB_ROOT, NULL);
...
pBOOL csb_frm_save_screen_shot(U32 nIndex)
{
    ...
    U8  local_file_path[100 * ENCODING_LENGTH];
...
void csb_frm_select_folder_callback(void *fullpath, int is_short)
{
    ...
    sprintf((S8*)temp_ascii, "%c%s", SRV_FMGR_PUBLIC_DRV, ":\\");
    mmi_asc_to_ucs2((PS8) csb_drive_phone_unicode, (PS8) temp_ascii);

    
    sprintf((S8*)temp_ascii, "%c%s", SRV_FMGR_CARD_DRV, ":\\");
    ...
    {
        /*mmi_popup_display_simple((WCHAR*)((PU8) GetString(STR_ID_CSB_FILE_PATH_TOO_LONG)), MMI_EVENT_FAILURE, GRP_ID_ROOT, NULL);*/
        mmi_popup_display_simple((WCHAR*)((UI_string_type)GetString(STR_ID_CSB_FILE_PATH_TOO_LONG)), MMI_EVENT_ERROR, GRP_ID_CSB_ROOT, NULL);
    }
    ...
    /* Store the path returned by CallBack in the Global variable */
        mmi_ucs2cpy((PS8) g_CSB_struct.csb_settings.csb_screenshots_folder_path, (PS8)fullpath);
            ...
            /*mmi_popup_display_simple((WCHAR*)((PU8) GetString(STR_GLOBAL_DONE)), MMI_EVENT_SUCCESS, GRP_ID_ROOT, NULL);*/
            mmi_popup_display_simple((WCHAR*)((UI_string_type)GetString(STR_GLOBAL_DONE)), MMI_EVENT_SUCCESS, GRP_ID_CSB_ROOT, NULL);
        }
...
pBOOL csb_frm_check_if_card_present(void)
{
    ...
    if(status >= 0)
    {
        ...
        mmi_popup_display_simple((WCHAR*)((UI_string_type)GetString(STR_GLOBAL_OK)), MMI_EVENT_ERROR, GRP_ID_CSB_ROOT, NULL);
    }
    else
    {
        error_code = FALSE;
