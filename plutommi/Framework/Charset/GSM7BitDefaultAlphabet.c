...
void ConvertGSM7BitDefaultEncodingToAscii(U8 *message, U8 *msg, U16 length, U16 *outLen)
{
    ...
    for (i = 0; i < length; i++)
    {
        ...
        {
            /* if(msg[i]!=13)       //CR ? */
            c = mmi_7bit_gsm_to_ucs2_char_default(msg[i]);
            if (c > 255)
            {
                MMI_TRACE(MMI_FW_TRC_G4_EDITOR, MMI_GUI_GSM_7BIT_TRUNCATE_UCS2, c);
                c = 0x20;
            }
...
U8 GSM7BitMapDefaultToASCIIByte(U8 input)
{
    ...
    c = mmi_7bit_gsm_to_ucs2_char_default(input);
    if (c > 255)
    {
        MMI_TRACE(MMI_FW_TRC_G4_EDITOR, MMI_GUI_GSM_7BIT_TRUNCATE_UCS2, c);
        c = 0x20;
 ...
 *****************************************************************************/
U8 GSM7BitMapASCIIToDefaultByte(U16 input)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    return (U8)mmi_7bit_ucs2_to_gsm_char_default(input);
}
...
void ConvertAsciiEncodingToGSM7BitDefault(U8 *message, U8 *msg, U16 length, U16 *outLen)
{
    ...
    {
        c = mmi_7bit_ucs2_to_gsm_char_default(msg[i]);
...
#endif
                    mmi_7bit_ucs2_to_gsm_string(data, convertedData, (U16)(len / ENCODING_LENGTH), &out_len, 0x20, NULL, 0);

#ifdef __SMS_R8_NATION_LANGUAGE__
                    mmi_7bit_set_national_lang_table(&old_l);
#endif
                    convertedData[out_len] = 0;
                    if (outLen != NULL)
                    {
                        *outLen = out_len + 1;
                    }
...
/* under construction !*/
                ...
                InputboxConvertGSMToDeviceEncoding(data, convertedData, (U8)len, (len * ENCODING_LENGTH) + ENCODING_LENGTH, MMI_DEFAULT_DCS, 0, 1);
            ...
            case (MMI_UCS2_81):
                ...
                i = InputboxConvertGSMToDeviceEncoding(
                    data,
                    convertedData,
                    (U8) len,
            ...
            case (MMI_DEFAULT_DCS):
                {
                    ...
                    mmi_7bit_set_national_lang_table(&new_l);
#endif
                    out_len = (len + 1) * ENCODING_LENGTH;
                    mmi_7bit_gsm_to_ucs2_string(data, convertedData, len, &out_len, 0x20, NULL, 0);

#ifdef __SMS_R8_NATION_LANGUAGE__
                    mmi_7bit_set_national_lang_table(&old_l);
#endif
                    memset((void*)&convertedData[out_len * ENCODING_LENGTH], 0, ENCODING_LENGTH);
                }
                if (outLen != NULL)
                {
...
void *SwapEndian(void *Addr, const int Nb)
{
    ...
    for (i = 0; i < 16; i++)
    {
        *((char*)Result_align + i) = Swap_data[i];
    }
    ...
    if (flag)
    {
        if (UCS2_count == 0)    /* ASCII */
        {
            while (!done)
            {
                ...
                else if (UI_TEST_ESC_CHARACTER(c))
                {
                    ...
                    UI_GET_NEXT_BYTE(p1, c);
                    in_count++;
                    ret = mmi_7bit_gsm_ext_to_ucs2_char_default((U8)c);
                    if (ret >= 0)
                    {
                        c = (UI_character_type)ret;
                    ...
                    else
                    {
                        ...
                        in_count--;
                        c = (UI_character_type)mmi_7bit_ucs2_to_gsm_char_default(cc);
                        UI_STRING_INSERT_CHARACTER(p2, c);
                        out_count += ENCODING_LENGTH;
                    }
                }
                else
                {
                    ...
                    c = (UI_character_type)mmi_7bit_gsm_to_ucs2_char_default((U8)c);
                    UI_STRING_INSERT_CHARACTER(p2, c);
                    out_count += ENCODING_LENGTH;
                }
        ...
        else if (UCS2_count == 0x09)    /* MMI_PHB_UCS2_81 */
        {
            ...
            for (i = 0; i < length; i++)
            {
                ...
                if (c < 0x80)   /* Default GSM character */
                {
                    ...
                    {
                        escape = 0;
                        c = (UI_character_type)mmi_7bit_gsm_ext_to_ucs2_char_default((U8)c);
                    }
                    else
                    {
                        c = (UI_character_type)mmi_7bit_gsm_to_ucs2_char_default((U8)c);
        ...
        else if (UCS2_count == 0x10)    /* MMI_PHB_UCS2_82 */
        {
            ...
            for (i = 0; i < length; i++)
            {
                ...
                if (c < 0x80)   /* Default GSM character */
                {
                    ret = mmi_7bit_gsm_to_ucs2_char_default((U8)c);
                    UI_STRING_INSERT_CHARACTER(p2, (UI_character_type)ret);
                }
                else    /* UCS2 character */
                {
                    c &= 0x7F;
    ...
    if (flag)
    {
        if (UCS2_count == 0)
        {
            while (!done)
            {
                ...
                else if (mmi_7bit_check_gsm_ext_character_default(c))
                {
                    UI_INSERT_BYTE(p2, _ESCAPE_CHARACTER);
                    c = (UI_character_type)mmi_7bit_ucs2_to_gsm_ext_char_default(c);
                    UI_INSERT_BYTE(p2, c);
                    out_count += 2; /* MTK 2003-12-29 */
                }
                else
                {
                    c = (UI_character_type)mmi_7bit_ucs2_to_gsm_char_default(c);
                    UI_INSERT_BYTE(p2, c);
                    out_count += 1;
                }
