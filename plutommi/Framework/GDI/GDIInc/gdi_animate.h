...
typedef struct {
    ...
    void				(*draw_after_callback)(GDI_RESULT result);  /* Invoke calback function after image is decode */
...
typedef struct
{
    ...
    void								(*anim_draw_after_callback)(GDI_RESULT result);
...
extern void gdi_anim_set_frame_delay(U32 delay);
extern GDI_RESULT gdi_anim_stop_all_by_layer(gdi_handle layer);
extern GDI_RESULT gdi_anim_set_nb_job_handle(gdi_handle anim_handle,gdi_handle job_handle);
...
extern GDI_RESULT gdi_anim_set_frame_index_of_curr_anim(U32 frame_index);
