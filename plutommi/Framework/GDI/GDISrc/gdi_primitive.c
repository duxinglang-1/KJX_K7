...
void gdi_draw_darken_rect(S32 x1, S32 y1, S32 x2, S32 y2, S32 brightness_value)
{
    ...
    do
    {
        ...
        if ((!(x1 & 1) && !(rect_width & 1)) &&                     /* 4-byte align for start and end */
            (gdi_act_color_from_rgb(255, 0, 255, 0) == 0x07E0) &&   /* must be 16-bit RGB565 */
            (brightness_value == 75))                               /* 75% brightness */
        {      
            U32 pixel, out_pixel;
            U32 *pixel_buf = (U32*)(gdi_act_layer->buf_ptr + (y1 * gdi_act_layer->width + x1) * 2) ;
            ...
            line_jump = (gdi_act_layer->width - rect_width) / 2;
            line_loop_count = rect_width / 2;

            /* do 2 16-bit pixels per loop (10 instructions per loop) */
            for (y = y2 - y1; y >= 0; y--)
            {
                for (x = line_loop_count - 1; x >= 0; x--)
                {
                    pixel = *pixel_buf;
