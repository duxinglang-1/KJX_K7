            ...
            S16 y)
{
    ...
    switch (pen_event)
    {
        ...
        case MMI_PEN_EVENT_MOVE:
            ...
            {
                gui_panel_element_struct *e = &panel->elements[panel->element_pen_down_index];
                MMI_BOOL point_inside = (MMI_BOOL)PEN_CHECK_BOUND(x, y, e->x1, e->y1, e->x2 - e->x1 + 1, e->y2 - e->y1 + 1);
 ...
 * DESCRIPTION
 *  Set the callback of element
 * 
 * PARAMETERS
 *  panel                           [IN/OUT]    panel
 *  element_index                   [IN]        element index to be set callback
 *  keycode                         [IN]        registered key event (KEY_INVALID if no key event registered)
 *  down_event_callback             [IN]        callback when key/pen down
 *  up_event_callback               [IN]        callback when key/pen up
 *  repeat_event_callback           [IN]        callback when key stays pressed or pen stays on element
 ...
 *  The element index. -1 if failed
 *****************************************************************************/
void gui_panel_set_element_callback(
      gui_panel_struct *panel,
      S32 element_index,
      U16 keycode,
      FuncPtr down_event_callback,
      ...
      gui_util_painter_struct disabled_painter)
{
    ...
    if (element_index >= 0 && element_index < panel->element_count)
    {
        panel->elements[element_index].keycode = keycode;
        panel->elements[element_index].down_event_callback = down_event_callback ? down_event_callback : UI_dummy_function;
        panel->elements[element_index].up_event_callback = up_event_callback ? up_event_callback : UI_dummy_function;
        panel->elements[element_index].repeat_event_callback = repeat_event_callback ? repeat_event_callback : UI_dummy_function;
        panel->elements[element_index].painters[GUI_PANEL_ELEM_STATE_NORMAL] = normal_painter;
        panel->elements[element_index].painters[GUI_PANEL_ELEM_STATE_DOWN] = down_painter;
        panel->elements[element_index].painters[GUI_PANEL_ELEM_STATE_FOCUSSED] = focussed_painter;
        panel->elements[element_index].painters[GUI_PANEL_ELEM_STATE_DISABLED] = disabled_painter;
    }
}
