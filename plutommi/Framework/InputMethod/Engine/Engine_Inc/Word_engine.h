...
typedef struct
{
    ...
    U16                 first_index;    /* IN */
    U8                  desired_cnt;    /* IN */
    U32                 comp_selected; /* this one is to give logically which candidate is selected */
...
//extern MMI_BOOL mmi_ime_load_database_from_memory_card(WCHAR path, UI_string_type filename);
extern MMI_BOOL mmi_ime_memory_card_config_loadldb_flag(MMI_BOOL is_on);
extern MMI_BOOL mmi_ime_memory_card_get_loadldb_flag(void);
extern void mmi_ime_set_ldb_index(U16 index);
extern U16 mmi_ime_get_ldb_index(void);
extern MMI_BOOL mmi_ime_memory_card_is_language_supported_by_vendor(mmi_imm_support_writing_lang_enum lang_id);
extern void mmi_ime_memory_card_reinitialize_word_engine(const sIMEModeDetails * mode_details_ptr);
extern MMI_BOOL mmi_ime_check_ldb_support_info(mmi_imm_support_writing_lang_enum lang_id);
...
extern mmi_ime_word_capital_state_enum mmi_ime_word_get_word_case(void);
#endif
...
extern void mmi_ime_word_get_associates_chinese_when_tapped( mmi_ime_query_param_struct_p param_ptr, mmi_ime_query_result_struct_p result_ptr );
extern void mmi_ime_word_reset_first_upper_flag();
extern void mmi_ime_change_page_of_kdb(ime_mode_attr_enum temp_state);
#endif
