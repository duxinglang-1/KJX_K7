 ...
 *****************************************************************************/
U8 mmi_tc_ascii_to_hex_convert(CHAR character)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *****************************************************************************/
U8 mmi_tc_convert_ascii_hex_to_dec(CHAR character)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_tc_tones_list_tag_handler(U8 **attr_p, void *para)
{
    ...
    /*----------------------------------------------------------------*/
    U8 attrCount = 0;
    char audio_file_name[(SRV_FMGR_PATH_MAX_LEN+1)+1];
    ...
    if (!strncmp((PS8) attr_p[attrCount], "name", strlen("name")))
    {
        ...
        else if ((strstr(audio_file_name, ".ra") != NULL) || (strstr(audio_file_name, ".RA") != NULL))
        {
            g_tc_audiothmfileformat = THEME_RA;
        }
...
void mmi_tc_filler_tag_handler(U8 **attr_p, void *arg)
{
...
#ifdef DLT_DEFAULT_VALUE_SUPPORT
    /* initialize local buffer to default value first */
    mmi_tc_init_local_buffer((void *)buff_ptr, (void *)*dlt_ptr, sizeof(UI_filled_area));
#else
    /* reset local buffer */
    memset((void *)buff_ptr, 0, sizeof(UI_filled_area));
#endif
    ...
    while ((attr_name = (PS8)attr_p[attrCount]) != 0)
    {
        ...
        {
            buff_ptr->b = mmi_tm_create_non_imageid_image((U32) atoi((PS8)data_ptr), THEME_IMG_LOAD_ALWAYS);
        }
        else if (!strcmp(attr_name, "img_load_type"))
        {
            /* always use THEME_IMG_LOAD_ALWAYS for filler image */
        }
        else
        {
...
MMI_BOOL mmi_tc_convert_theme_file_to_mmi_theme(MMI_theme **theme, U32 file_handle)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
#ifdef __MMI_THEMES_V2_SUPPORT__
    memcpy(g_tc_theme_converter_p, MMI_themes[0].theme, sizeof(MMI_theme));
#else
    memcpy(g_tc_theme_converter_p, MMI_themes[0], sizeof(MMI_theme));
#endif
