...
void mmi_autoam_view_voice_message_info(void)
{
	...
	#ifdef __GEMINI__		
    U16 sim_str[MMI_AUTOAM_MAX_SIM_NAME_LEN];
    U16 sim_no = g_autoam_message.sim_no; 
    #endif
	...
	#ifdef __GEMINI__		
	/* prepare the Sim no */
	mmi_ucs2ncat((S8*) g_autoam_detail_hdlr, (S8*) GetString(STR_GLOBAL_SIM), MMI_AUTOAM_MAX_SIM_NAME_LEN);
	mmi_ucs2cat((S8*) g_autoam_detail_hdlr, (S8*) L":");	
	gui_itoa((S32) sim_no, (UI_string_type) sim_str, 10);
	mmi_ucs2cat((S8*) g_autoam_detail_hdlr, (S8*) sim_str);
	#if defined (__MMI_DYNAMIC_SIM_DYNAMIC_UI__)
	mmi_ucs2ncat((S8*) g_autoam_detail_hdlr, (S8*) g_autoam_message.sim_name, (MMI_NETSET_SIM_NAME_MAX_LEN * 2));
	#endif
	mmi_ucs2cat((S8*) g_autoam_detail_hdlr, (S8*) L"\n"); 
	#endif
    /* prepare the number */
