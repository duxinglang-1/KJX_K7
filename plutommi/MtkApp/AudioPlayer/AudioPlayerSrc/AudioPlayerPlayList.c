...
void mmi_audply_playlist_entry_listmgr(void)
{
    ...
    SetKeyHandler(mmi_audply_playlist_entry_listmgr_option, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
...
void mmi_audply_highlight_listmgr_option_new(void)
{
    ...
    SetKeyHandler(mmi_audply_playlist_listmgr_create, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
#ifdef __MMI_TOUCH_SCREEN__
    wgui_reset_list_item_selected_callback();
#endif
} 
...
static MMI_BOOL mmi_audply_playlist_save_last_path(UI_string_type path)
{
    ...
    if (0 > WriteRecord(NVRAM_EF_AUDPLY_LAST_PLAYLIST_PATH_LID, 1, (void*)path, NVRAM_EF_AUDPLY_LIST_PLAYLIST_PATH_SIZE, &error))
    {
        return MMI_FALSE;
    }
    return MMI_TRUE;
...
static MMI_BOOL mmi_audply_playlist_get_last_path(const UI_string_type path)
{
    ...
    if (ReadRecord(NVRAM_EF_AUDPLY_LAST_PLAYLIST_PATH_LID, 1, (void*)path, NVRAM_EF_AUDPLY_LIST_PLAYLIST_PATH_SIZE, &error) < 0)
    {     
        return MMI_FALSE;
    }
    return MMI_TRUE;    
...
static void mmi_audply_playlist_save_last_index(S32 index)
{
    ...
    WriteValue(NVRAM_AUDPLY_LAST_PLAYLIST_INDEX, &last_index, DS_SHORT, &error);
}
...
static S32 mmi_audply_playlist_get_last_index(void)
{
    ...
    ReadValue(NVRAM_AUDPLY_LAST_PLAYLIST_INDEX, &index, DS_SHORT, &error);
    if (index == 0xFFFF)
    {     
        return -1;
    }
...
static void mmi_audply_playlist_entry_playlist_multiple(void)
{
    ...
    if (!mmi_audply_playlist_is_valid() && browser_now_playing)
    {
        ...
        /* register left right arrow key handler */
        SetKeyHandler(mmi_audply_playlist_entry_playlist_option_multiple, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
        SetKeyHandler(GoBackHistory, KEY_LEFT_ARROW, KEY_EVENT_DOWN);
    }
...
static void mmi_audply_playlist_entry_fwd_send_screen(void)
{
    ...
    ShowCategory15Screen(
        ...
        gui_buffer);
    ClearKeyEvents();
    SetKeyHandler(mmi_audply_playlist_fwd_screen_LSK, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
