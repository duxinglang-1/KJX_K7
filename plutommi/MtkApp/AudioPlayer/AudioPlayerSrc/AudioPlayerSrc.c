...
static void mmi_audply_entry_player_screen(void);
...
void mmi_audply_read_settings(void)
{
	...
	ReadValue(NVRAM_AUDPLY_REMOVE_IN_PROCESS, &g_audply.remove_process, DS_BYTE, &error);
...
/* check the status of current driver which loading list saved. */
	...
	if (!(FS_GetDevStatus(SRV_FMGR_CARD_DRV, FS_MOUNT_STATE_ENUM) < FS_NO_ERROR))
    {
    	if(g_audply.remove_process)
		{
			...
			WriteValue(NVRAM_AUDPLY_REMOVE_IN_PROCESS, &g_audply.remove_process, DS_BYTE, &error);
		}
...
void mmi_audply_update_main(void)
{
        ...
        srv_ucm_query_call_count(SRV_UCM_CALL_STATE_ALL, SRV_UCM_CALL_TYPE_NO_CSD, NULL))
    {
        ...
        SetKeyHandler(mmi_audply_press_inc_volume_down, KEY_POUND, KEY_EVENT_DOWN);
        SetKeyHandler(mmi_audply_press_inc_volume_up, KEY_POUND, KEY_EVENT_UP);
        SetKeyHandler(mmi_audply_press_dec_volume_down, KEY_STAR, KEY_EVENT_DOWN);
        SetKeyHandler(mmi_audply_press_dec_volume_up, KEY_STAR, KEY_EVENT_UP);
...
//    SetKeyHandler(mmi_audply_playlist_entry_playlist, KEY_LSK, KEY_EVENT_UP);
//    SetKeyHandler(GoBackHistory, KEY_RSK, KEY_EVENT_UP);
    SetKeyHandler(mmi_audply_lsk_up_hdlr, KEY_LSK, KEY_EVENT_UP);
    SetKeyHandler(mmi_audply_rsk_up_hdlr, KEY_RSK, KEY_EVENT_UP);
    ...
    SetKeyHandler(mmi_audply_press_play_button_up, KEY_CSK, KEY_EVENT_UP);
    SetKeyHandler(mmi_audply_press_prev_button_up, KEY_LEFT_ARROW, KEY_EVENT_UP);
    SetKeyHandler(mmi_audply_press_next_button_up, KEY_RIGHT_ARROW, KEY_EVENT_UP);
    SetKeyHandler(mmi_audply_press_repeat_button_up, KEY_UP_ARROW, KEY_EVENT_UP);
    SetKeyHandler(mmi_audply_press_shuffle_button_up, KEY_DOWN_ARROW, KEY_EVENT_UP);
    /* click down */
    SetKeyHandler(mmi_audply_press_play_button_down, KEY_CSK, KEY_EVENT_DOWN);
    SetKeyHandler(mmi_audply_press_prev_button_down, KEY_LEFT_ARROW, KEY_EVENT_DOWN);
    SetKeyHandler(mmi_audply_press_next_button_down, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
    SetKeyHandler(mmi_audply_press_repeat_button_down, KEY_UP_ARROW, KEY_EVENT_DOWN);
    SetKeyHandler(mmi_audply_press_shuffle_button_down, KEY_DOWN_ARROW, KEY_EVENT_DOWN);
    /* longpress */
    ...
    SetKeyHandler(mmi_audply_press_play_button_up, KEY_UP_ARROW, KEY_EVENT_UP);
    SetKeyHandler(mmi_audply_press_prev_button_up, KEY_LEFT_ARROW, KEY_EVENT_UP);
    SetKeyHandler(mmi_audply_press_next_button_up, KEY_RIGHT_ARROW, KEY_EVENT_UP);
     /* click down */
    SetKeyHandler(mmi_audply_press_play_button_down, KEY_UP_ARROW, KEY_EVENT_DOWN);
    SetKeyHandler(mmi_audply_press_prev_button_down, KEY_LEFT_ARROW, KEY_EVENT_DOWN);
    SetKeyHandler(mmi_audply_press_next_button_down, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
    /* longpress */
    SetKeyHandler(mmi_audply_press_prev_button_longpress, KEY_LEFT_ARROW, KEY_EVENT_LONG_PRESS);
...
void mmi_audply_highlight_main_list_now_playing(void)
{
    ...
    SetKeyHandler(mmi_audply_entry_player, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
#ifdef __MMI_TOUCH_SCREEN__
...
void mmi_audply_highlight_main_list_listmgr(void)
{
    ...
    SetKeyHandler(mmi_audply_playlist_entry_listmgr, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
#ifdef __MMI_TOUCH_SCREEN__
    wgui_reset_list_item_selected_callback();
#endif
}
...
void mmi_audply_highlight_main_list_op_link1(void)
{
    ...
    SetKeyHandler(mmi_audply_OP_link1, KEY_RIGHT_ARROW, KEY_EVENT_DOWN);
#ifdef __MMI_TOUCH_SCREEN__
    wgui_reset_list_item_selected_callback();
#endif
}
...
static MMI_BOOL mmi_audply_check_if_need_hide_OP_link(void)
{
...
#endif  /* __FLIGHT_MODE_SUPPORT__ */
    {
        ...
        if (srv_sim_ctrl_is_available(MMI_SIM1))
        {
            if (srv_sim_ctrl_is_available(MMI_SIM2))
            {
                ...
                else	
                {
                	...
                	{
                		/* sim 1 and sim 2 both exist but both blocked. */
                		need_hide = MMI_TRUE;
                	}
                	else
                	{
            ...
            else
            {
                ...
                else if (MTPNP_AD_Get_Startup_Mode() == MTPNP_AD_NVRAM_CARD1)
                {
                	if (!srv_sim_ctrl_is_available(MMI_SIM1))
                	{
                    	need_hide = MMI_TRUE;
        ...
        else if (!srv_sim_ctrl_is_available(MMI_SIM1))
        {
            ...
            else if (MTPNP_AD_Get_Startup_Mode() == MTPNP_AD_NVRAM_CARD2)
            {
                if (!srv_sim_ctrl_is_available(MMI_SIM2))
                {
                    need_hide = MMI_TRUE;
