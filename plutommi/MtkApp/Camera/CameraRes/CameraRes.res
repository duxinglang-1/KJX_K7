...
//minus anonymouse memory, and then add the jpeg decode size, when jpeg decode the thumbnail memory is released, so it can minus
	...
	<MEMORY heap= "0" extra_base="base(SRV_MDI_CAMERA_FULL)- GDI_ASM_MEM_SIZE  - MAX_CAM_CAPTURE_THUMBNAIL_BUF_SIZE + $(JPEG_SW_DEC_EXT_MIN_MEM_SIZE)" fg="fg(SRV_MDI_CAMERA_FULL)" inc="CameraGprot.h,jpeg_mem.h,med_global.h"/> 
    #endif
    #if defined(JPEG_HW_DEC_EXT_MIN_MEM_SIZE)
	<MEMORY heap= "0" extra_base="base(SRV_MDI_CAMERA_FULL)- GDI_ASM_MEM_SIZE  - MAX_CAM_CAPTURE_THUMBNAIL_BUF_SIZE + $(JPEG_HW_DEC_EXT_MIN_MEM_SIZE)" fg="fg(SRV_MDI_CAMERA_FULL)" inc="CameraGprot.h,jpeg_mem.h,med_global.h"/> 
    #endif
#else
    #if defined(JPEG_SW_DEC_EXT_MIN_MEM_SIZE)
...
#ifdef __MMI_SUBLCD_CAMERA__
    <MENU id="MENU_ID_CAMERA_SUBLCD_FLASH" type="OPTION" str="STR_ID_CAMERA_FLASH" highlight="mmi_sublcd_camera_hilight_flash">
