...
void mmi_camera_display_popup(MMI_ID_TYPE string_id, mmi_event_notify_enum event_id, void* user_tag)
{
		...
		{												
			...
			{											
			mmi_popup_display_simple_ext(string_id, (mmi_event_notify_enum)(event_id), APP_CAMERA, user_tag);	 
			}											
		}
#endif /* defined(__CAMERA_OSD_HORIZONTAL__) */ 
...
#define MMI_CAMERA_CREATE_WATCHDOGS()
#define MMI_CAMERA_DELETE_WATCHDOGS()
#define MMI_CAMERA_RESTART_APP_WATCHDOG()
#define MMI_CAMERA_STOP_APP_WATCHDOG()
#define MMI_CAMERA_RESTART_DELCONF_WATCHDOG()
#define MMI_CAMERA_STOP_DELCONF_WATCHDOG()
#define MMI_CAMERA_STOP_ALL_WATCHDOG() 
#define MMI_CAMERA_START_ALL_WATCHDOG()
...
#ifdef __MMI_CAMERA_4_OSD_LAYER__
static void mmi_camera_create_osd_region_layer(gdi_handle* temp_handler, camera_osd_region_struct osd_region,
	PU8 osd_layer_buf_ptr, MMI_BOOL is_idp_layer);
#endif
...
void mmi_camera_init_app(void)
{
	...
	g_camera_cntx.osd_idp_layer_region_handle = GDI_LAYER_EMPTY_HANDLE;
...
static void mmi_sublcd_camera_entry_preview_screen_ex(void)
{
    ...
    *((U16*)(g_camera_cntx.storage_filepath)) = 0;
    ...
    SetKeyHandler(mmi_frm_scrn_close_active_id, KEY_CAMERA, KEY_EVENT_LONG_PRESS);
...
static void mmi_camera_display_error_popup(void)
{
    ...
    if (g_camera_cntx.last_error != MDI_RES_CAMERA_SUCCEED)
    {
		...
		#ifndef __MMI_CAMERA_SLIM__
            MMI_CAMERA_DISPLAY_POPUP(string_id, event_id, MMI_CAMERA_POPUP_RESULT_PROCESS_NONE);
		#else
           mmi_camera_display_popup(string_id, event_id, (void*)MMI_CAMERA_POPUP_RESULT_PROCESS_NONE);
		#endif
            return;
        #else
        ...
        #else
			mmi_camera_display_popup(string_id, event_id, (void*)MMI_CAMERA_POPUP_RESULT_PROCESS_DELETE_GROUP);
        #endif
		#endif
        }
...
#ifndef __MMI_CAMERA_4_OSD_LAYER__
#if defined(__CAMERA_FEATURE_ROTATE_OSD_SELF__) && defined(__CAMERA_OSD_HORIZONTAL__)
    if (g_camera_cntx.app_mem_pool==NULL)
    {
...
static void mmi_camera_pre_init_app(void)
{
    ...
    if (MMI_FALSE == mmi_frm_kbd_is_key_supported(KEY_ENTER))
    {
        g_camera_skin_cntx.capture.is_show = MMI_FALSE;
#if defined(__CAMERA_OSD_HORIZONTAL__)
        g_camera_skin_cntx_270.capture.is_show = MMI_FALSE;
#endif 
    }
#endif
 */   
...
static void mmi_camera_use_base_layer(void)
{
    ...
    g_camera_cntx.preview_buf_p = base_layer_buffer_ptr;
}
...
static void mmi_camera_use_gdi_layer(void)
{
    ...
    gdi_layer_clear(GDI_COLOR_BLACK);
    gdi_layer_pop_and_restore_active();
    g_camera_cntx.osd_buf_p = gdi_layer_buffer_ptr;
...
static void mmi_camera_osd_init_buttons()
{
    ...
    const camera_skin_layeout_struct *p_act_skin = NULL;
...
static void mmi_camera_preview_draw_icon(void)
{
	...
	{
		gdi_layer_push_and_set_active(g_camera_cntx.osd_layer_region_handle[2]);
		gdi_layer_clear(GDI_COLOR_TRANSPARENT);
		gdi_set_alpha_blending_source_layer(g_camera_cntx.osd_layer_region_handle[2]);
	}
	else
	{
		if(g_camera_cntx.osd_idp_layer_region_handle == GDI_LAYER_EMPTY_HANDLE)
		{
			temp_buf = g_camera_cntx.osd_buf_p + 
						g_camera_cntx.active_camera_osd_ptr->osd_region0.width * g_camera_cntx.active_camera_osd_ptr->osd_region0.height * 4 +
						g_camera_cntx.active_camera_osd_ptr->osd_region1.width * g_camera_cntx.active_camera_osd_ptr->osd_region1.height * 2 +
						g_camera_cntx.active_camera_osd_ptr->osd_region2.width * g_camera_cntx.active_camera_osd_ptr->osd_region2.height * 2 +
						g_camera_cntx.active_camera_osd_ptr->osd_region3.width * g_camera_cntx.active_camera_osd_ptr->osd_region3.height * 2;
			mmi_camera_create_osd_region_layer(&g_camera_cntx.osd_idp_layer_region_handle,g_camera_cntx.active_camera_osd_ptr->osd_region2, temp_buf,MMI_TRUE);
		}
	    gdi_layer_push_and_set_active(g_camera_cntx.osd_idp_layer_region_handle);
		gdi_layer_clear(GDI_COLOR_TRANSPARENT);
		gdi_set_alpha_blending_source_layer(g_camera_cntx.osd_idp_layer_region_handle);
	}
#else
    gdi_layer_push_and_set_active(g_camera_cntx.osd_layer_region_handle[2]);
	gdi_layer_clear(GDI_COLOR_TRANSPARENT);
	...
	if(g_camera_cntx.osd_data.osd_rotate == CAMERA_DISPLAY_ROTATE_270)
	{
        ...
        gdi_layer_clear(GDI_COLOR_TRANSPARENT);
        gdi_set_alpha_blending_source_layer(g_camera_cntx.osd_layer_region_handle[2]);
        mdi_util_rotate_osd_for_hw_rotator(g_camera_cntx.osd_idp_layer_region_handle, g_camera_cntx.osd_layer_region_handle[2]);
        gdi_layer_pop_and_restore_active();
		if(g_camera_cntx.osd_idp_layer_region_handle != GDI_LAYER_EMPTY_HANDLE)
		{
			gdi_layer_free(g_camera_cntx.osd_idp_layer_region_handle);
			g_camera_cntx.osd_idp_layer_region_handle = GDI_LAYER_EMPTY_HANDLE;
		}
    }   
#endif
...
gui_lock_double_buffer();
	...
	if(g_camera_cntx.osd_data.osd_rotate == CAMERA_DISPLAY_ROTATE_0)
	{
		gdi_layer_push_and_set_active(g_camera_cntx.osd_layer_region_handle[0]);
		gdi_layer_toggle_double();
		gdi_layer_clear(GDI_COLOR_TRANSPARENT);
		gdi_set_alpha_blending_source_layer(g_camera_cntx.osd_layer_region_handle[0]);
	}
	else
	{
		if(g_camera_cntx.osd_idp_layer_region_handle == GDI_LAYER_EMPTY_HANDLE)
		{
			temp_buf = g_camera_cntx.osd_buf_p + 
						...
						g_camera_cntx.active_camera_osd_ptr->osd_region2.width * g_camera_cntx.active_camera_osd_ptr->osd_region2.height * 2 +
						g_camera_cntx.active_camera_osd_ptr->osd_region3.width * g_camera_cntx.active_camera_osd_ptr->osd_region3.height * 2;
			mmi_camera_create_osd_region_layer(&g_camera_cntx.osd_idp_layer_region_handle,g_camera_cntx.active_camera_osd_ptr->osd_region0, temp_buf,MMI_TRUE);
		}
	    gdi_layer_push_and_set_active(g_camera_cntx.osd_idp_layer_region_handle);
		gdi_layer_clear(GDI_COLOR_TRANSPARENT);
		gdi_set_alpha_blending_source_layer(g_camera_cntx.osd_idp_layer_region_handle);
...
/* draw ev */
	...
	if(g_camera_cntx.osd_data.osd_rotate == CAMERA_DISPLAY_ROTATE_270)
	{
		...
		gdi_layer_clear(GDI_COLOR_TRANSPARENT);
		gdi_set_alpha_blending_source_layer(g_camera_cntx.osd_layer_region_handle[0]);
		mdi_util_rotate_osd_for_hw_rotator(g_camera_cntx.osd_idp_layer_region_handle, g_camera_cntx.osd_layer_region_handle[0]);
        gdi_layer_pop_and_restore_active();
		if(g_camera_cntx.osd_idp_layer_region_handle != GDI_LAYER_EMPTY_HANDLE)
		{
			gdi_layer_free(g_camera_cntx.osd_idp_layer_region_handle);
...
static void mmi_camera_preview_draw_softkey(void)
{
    ...
    if(CAMERA_STATE_IN(CAMERA_STATE_CONTSHOT))  //???????? new modification by yiping, need consider
        return;
#ifdef __MMI_CAMERA_4_OSD_LAYER__
     layer_offset_x = g_camera_cntx.active_camera_osd_ptr->osd_region1.offset_x;
     layer_offset_y = g_camera_cntx.active_camera_osd_ptr->osd_region1.offset_y;
#if defined(__CAMERA_OSD_HORIZONTAL__) && defined(__CAMERA_FEATURE_ROTATE_OSD_SELF__)
	if(g_camera_cntx.osd_data.osd_rotate == CAMERA_DISPLAY_ROTATE_0)
	{
		gdi_layer_push_and_set_active(g_camera_cntx.osd_layer_region_handle[1]);
		gdi_layer_clear(GDI_COLOR_TRANSPARENT);
		gdi_set_alpha_blending_source_layer(g_camera_cntx.osd_layer_region_handle[1]);
	...
	else
	{
		if(g_camera_cntx.osd_idp_layer_region_handle == GDI_LAYER_EMPTY_HANDLE)
		{
			temp_buf = g_camera_cntx.osd_buf_p + 
						...
						g_camera_cntx.active_camera_osd_ptr->osd_region2.width * g_camera_cntx.active_camera_osd_ptr->osd_region2.height * 2 +
						g_camera_cntx.active_camera_osd_ptr->osd_region3.width * g_camera_cntx.active_camera_osd_ptr->osd_region3.height * 2;
			mmi_camera_create_osd_region_layer(&g_camera_cntx.osd_idp_layer_region_handle,g_camera_cntx.active_camera_osd_ptr->osd_region1, temp_buf,MMI_TRUE);
		}
	    gdi_layer_push_and_set_active(g_camera_cntx.osd_idp_layer_region_handle);
		gdi_layer_clear(GDI_COLOR_TRANSPARENT);
		gdi_set_alpha_blending_source_layer(g_camera_cntx.osd_idp_layer_region_handle);
   ...
   if(g_camera_cntx.osd_data.osd_rotate == CAMERA_DISPLAY_ROTATE_270)
   {
        ...
        gdi_layer_clear(GDI_COLOR_TRANSPARENT);
        gdi_set_alpha_blending_source_layer(g_camera_cntx.osd_layer_region_handle[1]);
        mdi_util_rotate_osd_for_hw_rotator(g_camera_cntx.osd_idp_layer_region_handle, g_camera_cntx.osd_layer_region_handle[1]);
        gdi_layer_pop_and_restore_active();
		if(g_camera_cntx.osd_idp_layer_region_handle != GDI_LAYER_EMPTY_HANDLE)
		{
			gdi_layer_free(g_camera_cntx.osd_idp_layer_region_handle);
...
static void mmi_camera_preview_draw_hint(void)
{
	...
	if(g_camera_cntx.osd_data.osd_rotate == CAMERA_DISPLAY_ROTATE_0)
	{
		gdi_layer_push_and_set_active(g_camera_cntx.osd_layer_region_handle[3]);
		gdi_layer_clear(GDI_COLOR_TRANSPARENT);
		gdi_set_alpha_blending_source_layer(g_camera_cntx.osd_layer_region_handle[3]);
	...
	else
	{
		if(g_camera_cntx.osd_idp_layer_region_handle == GDI_LAYER_EMPTY_HANDLE)
		{
			temp_buf = g_camera_cntx.osd_buf_p + 
						...
						g_camera_cntx.active_camera_osd_ptr->osd_region2.width * g_camera_cntx.active_camera_osd_ptr->osd_region2.height * 2 +
						g_camera_cntx.active_camera_osd_ptr->osd_region3.width * g_camera_cntx.active_camera_osd_ptr->osd_region3.height * 2;
			mmi_camera_create_osd_region_layer(&g_camera_cntx.osd_idp_layer_region_handle,g_camera_cntx.active_camera_osd_ptr->osd_region3, temp_buf,MMI_TRUE);
		}
	    gdi_layer_push_and_set_active(g_camera_cntx.osd_idp_layer_region_handle);
		gdi_layer_clear(GDI_COLOR_TRANSPARENT);
		gdi_set_alpha_blending_source_layer(g_camera_cntx.osd_idp_layer_region_handle);
	...
	{
		if(g_camera_cntx.osd_data.osd_rotate == CAMERA_DISPLAY_ROTATE_270)
			gdi_layer_set_position(LCD_WIDTH - g_camera_cntx.active_camera_osd_ptr->hint_box.height,
									g_camera_cntx.active_camera_osd_ptr->hint_box.offset_x);
		else
			gdi_layer_set_position(g_camera_cntx.active_camera_osd_ptr->hint_box.offset_x,0);
	}
	else
	{
		if(g_camera_cntx.osd_data.osd_rotate == CAMERA_DISPLAY_ROTATE_270)
			gdi_layer_set_position(g_camera_cntx.active_camera_osd_ptr->hint_box.offset_y,g_camera_cntx.active_camera_osd_ptr->hint_box.offset_x);
		else
			gdi_layer_set_position(g_camera_cntx.active_camera_osd_ptr->hint_box.offset_x,g_camera_cntx.active_camera_osd_ptr->hint_box.offset_y);
	}
	gdi_layer_pop_and_restore_active();
#endif    
    /* draw hint */
    if (g_camera_cntx.osd_data.show_hint)
    {
		...
		gui_set_text_clip(
            0,
            0,
            g_camera_cntx.active_camera_osd_ptr->hint_box.width - 1,
            g_camera_cntx.active_camera_osd_ptr->hint_box.height - 1);
	#endif
        ...
        else
        {
		    ...
		    mmi_camera_draw_style_text(
                hint_str1,
                str1_offset_x - offset_x,
                str1_offset_y - offset_y,
                0,
                &g_camera_cntx.active_camera_osd_ptr->hint_box.style_text);
            
            mmi_camera_draw_style_text(
                hint_str2,
               	str2_offset_x - offset_x,
                str2_offset_y - offset_y,
                0,
                &g_camera_cntx.active_camera_osd_ptr->hint_box.style_text);
		#endif
        }
	...
	if(g_camera_cntx.osd_data.osd_rotate == CAMERA_DISPLAY_ROTATE_270)
    {
		...
		gdi_layer_clear(GDI_COLOR_TRANSPARENT);
        gdi_set_alpha_blending_source_layer(g_camera_cntx.osd_layer_region_handle[3]);
        mdi_util_rotate_osd_for_hw_rotator(g_camera_cntx.osd_idp_layer_region_handle, g_camera_cntx.osd_layer_region_handle[3]);
        gdi_layer_pop_and_restore_active();
		if(g_camera_cntx.osd_idp_layer_region_handle != GDI_LAYER_EMPTY_HANDLE)
		{
			gdi_layer_free(g_camera_cntx.osd_idp_layer_region_handle);
	...
	PU8 osd_layer_buf_ptr, MMI_BOOL is_idp_layer)
{
    ...
    width = osd_region.width;
    height = osd_region.height;
    offset_x = osd_region.offset_x;
    offset_y = osd_region.offset_y;
	...
	else if(g_camera_cntx.osd_data.osd_rotate == CAMERA_DISPLAY_ROTATE_0)
	{
	if((offset_x == g_camera_cntx.active_camera_osd_ptr->osd_region0.offset_x)
		&& (offset_y == g_camera_cntx.active_camera_osd_ptr->osd_region0.offset_y))
		{
		gdi_layer_create_cf_double_using_outside_memory(
	    GDI_COLOR_FORMAT_16,
	    ...
	    (PU8)osd_layer_buf_ptr,
	    (S32)buf_size,
	    (PU8)(osd_layer_buf_ptr+buf_size),
	    (S32)buf_size);
		}
	else
		{
    ...
    gdi_layer_push_and_set_active(*temp_handler);
    gdi_layer_clear_background(GDI_COLOR_TRANSPARENT);
    gdi_layer_set_source_key(TRUE, GDI_COLOR_TRANSPARENT);
    gdi_layer_pop_and_restore_active();
...
#endif /* defined(__CAMERA_OSD_HORIZONTAL__) */ 
#ifdef __MMI_CAMERA_4_OSD_LAYER__
		if(g_camera_cntx.osd_layer_region_handle[0] == NULL)
		{
			temp_buf = g_camera_cntx.osd_buf_p;
			mmi_camera_create_osd_region_layer(&g_camera_cntx.osd_layer_region_handle[0],g_camera_cntx.active_camera_osd_ptr->osd_region0, temp_buf,MMI_FALSE);
			temp_buf += g_camera_cntx.active_camera_osd_ptr->osd_region0.width * g_camera_cntx.active_camera_osd_ptr->osd_region0.height * 4;
			mmi_camera_create_osd_region_layer(&g_camera_cntx.osd_layer_region_handle[1],g_camera_cntx.active_camera_osd_ptr->osd_region1, temp_buf,MMI_FALSE);
			temp_buf += g_camera_cntx.active_camera_osd_ptr->osd_region1.width * g_camera_cntx.active_camera_osd_ptr->osd_region1.height * 2;
			mmi_camera_create_osd_region_layer(&g_camera_cntx.osd_layer_region_handle[2],g_camera_cntx.active_camera_osd_ptr->osd_region2, temp_buf,MMI_FALSE);
			temp_buf += g_camera_cntx.active_camera_osd_ptr->osd_region2.width * g_camera_cntx.active_camera_osd_ptr->osd_region2.height * 2;
			mmi_camera_create_osd_region_layer(&g_camera_cntx.osd_layer_region_handle[3],g_camera_cntx.active_camera_osd_ptr->osd_region3, temp_buf,MMI_FALSE);
		}
#endif    
#ifdef CAMERA_MODULE_WITH_LCD
...
static void mmi_camera_preview_set_blt_layer(BOOL is_previewing)
{
    ...
    if (is_previewing)
    {
		...
		if(g_camera_cntx.is_capturing)
		{
						...
						gdi_layer_set_blt_layer(g_camera_cntx.preview_layer_handle, 
														g_camera_cntx.osd_layer_region_handle[0],
														g_camera_cntx.osd_layer_region_handle[2],
														g_camera_cntx.osd_layer_region_handle[3]);
                   #else
						gdi_layer_set_blt_layer(g_camera_cntx.preview_layer_handle, 
											g_camera_cntx.osd_layer_region_handle[0],
											g_camera_cntx.osd_layer_region_handle[1],
											g_camera_cntx.osd_layer_region_handle[3]);
                   #endif
	...
	#else
         gdi_layer_set_blt_layer(g_camera_cntx.preview_layer_handle, g_camera_cntx.osd_layer_handle, 0, 0);
	#endif
    }else{
    #ifdef __MMI_CAMERA_4_OSD_LAYER__
		if(g_camera_cntx.is_capturing)
...
static void mmi_camera_countdown_cyclic(void)
{
    ...
    else if (g_camera_cntx.capture_countdown >= 0)
    {
        ...
        gui_start_timer(100, mmi_camera_countdown_stop_LED);
        gui_start_timer(400, mmi_camera_countdown_cyclic);
        g_camera_cntx.capture_countdown -= 400;
...
static void mmi_camera_update_setting(U16 *pval, U16 count, U16 title_str_id, U16 cont_str_base_id)
{
...
#ifndef __CAMERA_FEATURE_HIDE_HINT__
    {
        ...
        mmi_ucs2cat((CHAR * ) str_buf, (CHAR * ) GetString(STR_ID_CAMERA_IMAGE_SIZE));
...
/*
    mmi_ucs2cpy((CHAR * ) str_buf, (CHAR * ) L"[ ");
    mmi_ucs2cat((CHAR * ) str_buf, (CHAR * ) GetString(STR_GLOBAL_STORAGE));
    mmi_ucs2cat((CHAR * ) str_buf, (CHAR * ) L" ]");
    */
...
mmi_ret mmi_camera_group_proc(mmi_event_struct *evt)
{
    ...
    case EVT_ID_CUI_MENU_ITEM_SELECT:
        ...
        case MENU_ID_CAMERA_WB:
            mmi_camera_entry_wb_screen();
            break;
#endif
#ifdef __CAMERA_DSC_MODE__
        case MENU_ID_CAMERA_DSC_MODE:
...
void mmi_camera_switch_to_vdorec_app(void)
{
    ...
    else
    {
        ...
        vdorec_struct.max_file_size = 150;
        vdorec_struct.max_record_time = 15;
        g_camera_cntx.vdorec_gid = cui_vdorec_create(g_camera_cntx.cur_gid);
        cui_vdorec_run(g_camera_cntx.vdorec_gid, &vdorec_struct);
    }    
}
