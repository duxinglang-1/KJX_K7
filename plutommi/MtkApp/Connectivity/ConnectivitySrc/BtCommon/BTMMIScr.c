...
static void mmi_bt_popup_display(U16 string_id, U32 event, mmi_id group_id);
...
/* my device */
/*******************************/
static void mmi_bt_pre_entry_mydev_menu(void);
...
static void mmi_bt_show_progress_scr(mmi_id group_id, U16 scrid, U16 title, U16 context_id, FuncPtr fp);
 ...
 * Functions
 *****************************************************************************/
static void MMI_BT_SET_FLAG(U32 x)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
#ifdef MMI_TEST_MAP_CODE
U8  mmi_bt_mapc_proc_inject_string(U8 index, kal_uint8* string)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	static int local_srv_hd;
    
    //tst_inject_string_struct *inject_str = (tst_inject_string_struct *)in;
	//local_srv_hd = srv_bt_dialer_open(5);

    if (strcmp((char*)string, "1") == 0)   // inbox
    {	U8 ans;
		U8 child_folder[SRV_BT_MAP_MAX_FOLDER_NAME_LEN+1];
		app_ucs2_strcpy(child_folder,(const char *)L"inbox");
		app_ucs2_strcat(child_folder,(const kal_int8 *)L"\0");
		ans=srv_mapc_change_folder(child_folder,SRV_BT_MAPC_FORWARD,SRV_BT_MAPC_MESSAGE_LIST_ONLY,NULL);
		return ans;														

    }

	 else if (strcmp((char*)string, "12") == 0)   // inbox
    {	U8 ans;
		U8 child_folder[SRV_BT_MAP_MAX_FOLDER_NAME_LEN+1];
		//srv_bt_mapc_set_msg_listing_prop(srv_bt_mapc_set_msg_listing_prop_struct * msg)
		app_ucs2_strcpy(child_folder,(const char *)L"inbox");
		app_ucs2_strcat(child_folder,(const kal_int8 *)L"\0");
		ans=srv_mapc_change_folder(child_folder,SRV_BT_MAPC_END,SRV_BT_MAPC_MESSAGE_LIST_ONLY,NULL);
		return ans;														

    }
	 else if (strcmp((char*)string, "13") == 0)   // inbox
    {	U8 ans=0;
		U8 handle[MAP_MAX_HANDLE_STR_UCS2_LEN+2];
		app_ucs2_strcpy(handle,(const char *)L"100000000000001C");
		app_ucs2_strcat(handle,(const kal_int8 *)L"\0");
		srv_bt_mapc_set_message_status_req(handle,SRV_BT_MAP_MSG_STATUS_UNREAD,NULL);
		return ans;														

    }
	   
	else if (strcmp((char*)string, "2") == 0)   // outbox
    {
		U8 ans;
		U8 child_folder[SRV_BT_MAP_MAX_FOLDER_NAME_LEN+1];
		app_ucs2_strcpy(child_folder,(const char *)L"outbox");
		mmi_ucs2cat(child_folder,(const kal_int8 *)L"\0");
		ans=srv_mapc_change_folder(child_folder,SRV_BT_MAPC_FORWARD,SRV_BT_MAPC_NONE,NULL);
		return ans;															
    }
	else if (strcmp((char*)string, "3") == 0)   // back
    {
		U8 ans;
		U8 child_folder[SRV_BT_MAP_MAX_FOLDER_NAME_LEN+1];
		//app_ucs2_strcpy(child_folder,(const char *)L("outbox"));
		//app_ucs2_strcat(child_folder,L"\0");
		ans=srv_mapc_change_folder(child_folder,SRV_BT_MAPC_BACK,SRV_BT_MAPC_NONE,NULL);
		return ans;																	
    }
	else if (strcmp((char*)string, "4") == 0)   // abort
    {
		srv_bt_mapc_abort_req(NULL);
		return 1;														
    }
    else if (strcmp((char*)string, "5") == 0)   // set notify false
   	{
				srv_mapc_set_notify_registration_req(MMI_FALSE);
   		 		//mmi_pbapc_read_folder_req(0, "telecom/pb.vcf", filter, format,10, 0);
				return 1;								
   	}
    else if (strcmp((char*)string, "6") == 0)   // set notify true
   	{
				srv_mapc_set_notify_registration_req(MMI_TRUE);
   		 		//mmi_pbapc_read_folder_req(0, "telecom/pb.vcf", filter, format,10, 0);
				return 1;				
    }
	else if (strcmp((char*)string, "7") == 0) // update inbox 
	{
		srv_bt_mapc_update_inbox_req();
		return 1;
	}
	else if (strcmp((char*)string, "8") == 0)   // outbox
    {
		U8 ans;
		U8 child_folder[SRV_BT_MAP_MAX_FOLDER_NAME_LEN+1];
		app_ucs2_strcpy(child_folder,(const char *)L"outbox");
		mmi_ucs2cat(child_folder,(const kal_int8 *)L"\0");
		ans=srv_mapc_change_folder(child_folder,SRV_BT_MAPC_NEXT,SRV_BT_MAPC_NONE,NULL);
		return ans;															
    }
	else if (strcmp((char*)string, "9") == 0)   // inbox msg listing
    {	U8 ans;
		U8 child_folder[SRV_BT_MAP_MAX_FOLDER_NAME_LEN+1];
		app_ucs2_strcpy(child_folder,(const char *)L"inbox");
		app_ucs2_strcat(child_folder,(const kal_int8 *)L"\0");
...
void srv_bt_mapc_dump_trace_info(void *buf, int buf_len)
{
  ...
  if(buf_len > 0)
  {
      while (buf_len > 0)
      {
      	kal_buffer_trace(MMI_CONN_TRC_G7_BT, SRV_BT_MAPC_RECV_BUFFER_TRACE, buf_len, (const kal_uint8*)buf);
		buf=(kal_uint8*)buf+buf_len;
      	buf_len = buf_len - max_buf_len;
      }
  }
}

void testmapcb(S32 srv_hd, srv_bt_mapc_cbevent_enum event, void* para);
void(*testmapcb1)(S32 srv_hd, srv_bt_mapc_cbevent_enum event, void* para) = testmapcb;
//srv_bt_mapc_notify_cb  testmapcb;
void testmapcb(S32 srv_hd, srv_bt_mapc_cbevent_enum event, void* para)

{
	...
	switch(event)
	{
		...
		case SRV_BT_MAPC_EVT_ID_CONNECT_RSP:
			{	srv_bt_mapc_connect_rsp *msg=(srv_bt_mapc_connect_rsp *)para;
			kal_wap_trace(MOD_MMI_CONN_APP,TRACE_GROUP_7,"connect EVent id %d and response %d",event,msg->Error);
				if(msg->Error==SRV_BT_MAPC_SUCCESS)
				{
						mmi_frm_nmgr_popup(
            MMI_SCENARIO_ID_DEFAULT,
            MMI_EVENT_FAILURE,
            (WCHAR*)GetString(STR_GLOBAL_CONNECT));
            	//kal_wap_trace(MOD_MMI,TRACE_GROUP_7,"connect success EVent id %d",event);
            	if(srv_mapc_change_folder(NULL,SRV_BT_MAPC_ROOT,SRV_BT_MAPC_NONE,NULL)==MMI_TRUE)
            		{
            		kal_wap_trace(MOD_MMI_CONN_APP,TRACE_GROUP_7,"REQUEST SENT ");
            		}
				}
				else
					{
			...
			{
				srv_bt_mapc_set_folder_res *msg=(srv_bt_mapc_set_folder_res *)para;
				kal_wap_trace(MOD_MMI_CONN_APP,TRACE_GROUP_7,"set folder response came result %d",msg->Error);
				if(msg->Error==SRV_BT_MAPC_SUCCESS)
				{
					
					srv_bt_mapc_set_msg_listing_prop_struct msg;
					msg.FilterMessageType=SRV_BT_MAPC_FILTER_MSG_TYPE_SMS_GSM;
					msg.FilterPriority=SRV_BT_MAPC_NO_PRIORITY;
					msg.Maxlistcount=100;
					msg.ListStartOffset=0;
					msg.SubjectLength=50;
					msg.ParameterMask=0;
					msg.FilterReadStatus=SRV_BT_MAPC_NO_READ_FILTER;
					app_ucs2_strcpy((msg.FilterPeriodBegin),(const char *)("20120101T121212"));
					app_ucs2_strcat((msg.FilterPeriodBegin),(const char *)("\0"));
					app_ucs2_strcpy((msg.FilterPeriodEnd),(const char *)("20140101T121212"));
					app_ucs2_strcat((msg.FilterPeriodEnd),(const char *)("\0"));
					//app_ucs2_strcpy((msg.FilterRecipient),(const char *)("*"));
					//app_ucs2_strcat((msg.FilterRecipient),(const char *)("\0"));
					//app_ucs2_strcpy((msg.FilterOriginator),(const char *)("*"));
					//app_ucs2_strcat((msg.FilterOriginator),(const char *)("\0"));
					memset(msg.FilterRecipient,0,SRV_BT_MAPC_FILTER_SIZE);
					memset(msg.FilterOriginator,0,SRV_BT_MAPC_FILTER_SIZE);
					srv_bt_mapc_set_msg_listing_prop(&msg);
					mmi_frm_nmgr_popup(
            		MMI_SCENARIO_ID_DEFAULT,
            		MMI_EVENT_FAILURE,
            		(WCHAR*)GetString(STR_GLOBAL_MOVE));
				}
			}
			break;
			case SRV_BT_MAPC_EVT_ID_DISCONNECT_IND:
			{
			kal_wap_trace(MOD_MMI_CONN_APP,TRACE_GROUP_7,"Disconnect indication came");
			mmi_frm_nmgr_popup(
            MMI_SCENARIO_ID_DEFAULT,
            MMI_EVENT_FAILURE,
			...
			{srv_bt_mapc_disconnect_rsp *msg=(srv_bt_mapc_disconnect_rsp *)para;
			kal_wap_trace(MOD_MMI_CONN_APP,TRACE_GROUP_7,"disconnect EVent id %d and response %d",event,msg->Error);
				if(msg->Error==SRV_BT_MAPC_SUCCESS)
				{
				kal_wap_trace(MOD_MMI,TRACE_GROUP_7,"disconnect success EVent id %d",event);
						mmi_frm_nmgr_popup(
            			MMI_SCENARIO_ID_DEFAULT,
            			MMI_EVENT_FAILURE,
            			(WCHAR*)GetString(STR_GLOBAL_DISCONNECT));
			...
			{
				int hd,bufLen,rd_size; 
				void *buf;
				srv_bt_mapc_message_listing_cnf *msg_p=(srv_bt_mapc_message_listing_cnf *)para;
				hd=FS_Open((const WCHAR *)msg_p->file,FS_READ_ONLY);
				FS_GetFileSize(hd,&bufLen);
				buf=OslMalloc(200);
				FS_Read(hd,buf,bufLen,&rd_size);
				srv_bt_mapc_dump_trace_info(buf,rd_size);	
				FS_Close(hd);
			}
			break;
			case SRV_BT_MAPC_EVT_ID_SET_MSG_STATUS_RSP:
			{
				srv_bt_mapc_set_message_status_rsp *msg=(srv_bt_mapc_set_message_status_rsp *)para;
				kal_wap_trace(MOD_MMI,TRACE_GROUP_7,"Message status set done %d with rsp %d",event,msg->Error);
				if(msg->Error==SRV_BT_MAPC_SUCCESS)
				{
            			...
            			MMI_SCENARIO_ID_DEFAULT,
            			MMI_EVENT_FAILURE,
            			(WCHAR*)GetString(STR_GLOBAL_STATUS));
				}
				else
				{
						mmi_frm_nmgr_popup(
			...
			default:
				kal_wap_trace(MOD_MMI_CONN_APP,TRACE_GROUP_7,"please handle EVent id %d",event);
				break;
...
{
	//srv_bt_mapc_register_callback(1,testmapcb);
	srv_bt_mapc_connect_req(dev_addr);
	
}

#endif 
static void MMI_BT_RESET_FLAG(U32 x)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
#endif
#ifdef MMI_TEST_MAP_CODE
	//mmi_frm_set_protocol_event_handler(MSG_ID_TST_INJECT_STRING, (PsIntFuncPtr)mmi_bt_mapc_proc_inject_string, MMI_TRUE);
#endif
...
// #endif  // __MMI_BT_LIMITED_DISCOVERY_SUPPORT__
    ...
    ReadValue(NVRAM_BT_POWER_STATUS, &(g_mmi_bt_scr_cntx.sys_flag), DS_BYTE, &err);
...
//                 mmi_bt_set_limit_vis_complete(para);
//                 break;
// #endif
...
MMI_BOOL mmi_bt_flag_check()
{
	if(g_mmi_bt_scr_cntx.flag & MMI_BT_CONFIRM_SWITCH_POWER_OFF ||
        ...
        g_mmi_bt_scr_cntx.flag & MMI_BT_SET_AUDIO_PATH ||
...
void mmi_bt_power_on_bt(void (*power_on_cb_func_ptr) (MMI_BOOL))
{
    ...
    // else
    // {
    //     MMI_BT_SET_SYS_FLAG(MMI_BT_POWER_ON);
    //     WriteValue(NVRAM_BT_POWER_STATUS, &(g_mmi_bt_scr_cntx.sys_flag), DS_BYTE, &err);
    // }
}
...
void mmi_bt_power_off_bt(void (*power_off_cb_func_ptr) (MMI_BOOL))
{
    ...
    srv_bt_cm_switch_off();
    //MMI_BT_RESET_SYS_FLAG(MMI_BT_POWER_ON);
    //WriteValue(NVRAM_BT_POWER_STATUS, &(g_mmi_bt_scr_cntx.sys_flag), DS_BYTE, &err);
}
...
static void* mmi_bt_get_device_image(U32 cod)
{
    ...
    if (cod & SRV_BT_COMPUTER_MAJOR_DEVICE_MASK)
    {
    ...
    else if (cod & SRV_BT_PHONE_MAJOR_DEVICE_MASK)
    {
...
void mmi_bt_entry_passkey_input_screen(void)
{
    ...
    if (inputBuffer == NULL)
    {
        ...
        //memset(g_mmi_bt_scr_cntx.pin_scr_buf, 0, sizeof(U16) * MMI_BT_PIN_SCR_BUF_SIZE);
...
static void mmi_bt_security_user_confirm_notify(void *para)
{
    ...
    //memset(g_mmi_bt_scr_cntx.security_passkey, 0, MMI_BT_MAX_SECURITY_PASSKEY_LENGTH+1);
...
static void mmi_bt_entry_security_user_confirm_screen(void)
{
    ...
    //memset(g_mmi_bt_scr_cntx.ui_buf, 0, sizeof(U16) * MMI_BT_UI_BUF_SIZE);
    //memset(g_mmi_bt_scr_cntx.scr_buf, 0, sizeof(U16) * MMI_BT_SCR_BUF_SIZE);
    //memset(g_mmi_bt_scr_cntx.pin_scr_buf, 0, sizeof(U16) * MMI_BT_PIN_SCR_BUF_SIZE);
    //g_mmi_bt_scr_cntx.pin_scr_buf[0] = 0;
    ...
    switch (menu_evt->evt_id)
    {
        ...
        case EVT_ID_CUI_MENU_LIST_ENTRY:
            if (menu_evt->parent_menu_id == SCR_BT_MYDEV_OPT)
            {
				...
				root_ids[nItems++]=MMI_BT_PBAP_CONNECT,
#endif 
				...
				cui_menu_set_item_string(
								   menu_evt->sender_id,
								   MMI_BT_PBAP_CONNECT,
								   (WCHAR*)GetString(STR_BT_CONNECT_CONFIRM));	
#endif 
        ...
        case EVT_ID_CUI_MENU_ITEM_SELECT:
            switch (menu_evt->highlighted_menu_id)
            {
                ...
                {
                    //if (mmi_bt_is_dev_cod_match(dev_p->cod,MMI_BT_AUDIO_MAJOR_SERVICE_MASK))
                ...
                case MMI_BT_MYDEV_OPT_DEL:
                {
                  ...
                  // srv_bt_mapc_test_code((dev_p->bd_addr));
                    break;
                }
#ifdef MMI_TEST_MAP_CODE
				case MMI_BT_MYDEV_CONNECT:
				{
					 srv_bt_mapc_connect_req((dev_p->bd_addr));
                    break;
                }
				case MMI_BT_MYDEV_DISCONNECT:
				{
					 srv_mapc_disconnect(NULL);
                    break;
                }
				case MMI_BT_PBAP_CONNECT:
                {
					 mmi_pbapc_cm_connect_req(&dev_p->bd_addr);
                    break;
                }
...
void mmi_bt_set_vis_complete(void *para)
{
    ...
    if (event->is_from_mmi)
    {
       ...
       //             {
       //                 U16 str[MMI_BT_POPUP_SCR_BUF_SIZE];
       //                 U32 len;
       //                 memset(str,0,sizeof(U16) * MMI_BT_POPUP_SCR_BUF_SIZE);
       //                 mmi_wcscpy(str, (WCHAR *) GetString(STR_BT_DEV_SET_VIS_X_MINUTES_1));
       //                 len = mmi_wcslen(str);
       //                 kal_wsprintf(&str[len], "%d", MMI_BT_BT_TEMP_VISIBLE_TIMER_DUR/60000);
       //                 if ((MMI_BT_BT_TEMP_VISIBLE_TIMER_DUR/60000) == 1)
       //                 {
       //                     mmi_wcscat(str, (WCHAR *)GetString(STR_BT_DEV_SET_VIS_X_MINUTE_2));
       //                 }
       //                 else
       //                 {
       //                     mmi_wcscat(str, (WCHAR *)GetString(STR_BT_DEV_SET_VIS_X_MINUTES_2));
       //                 }
       //                 mmi_frm_nmgr_popup(MMI_SCENARIO_ID_DEFAULT, MMI_EVENT_SUCCESS, (WCHAR *)str);
       //             }
       //             else
       // #endif
       //             {
    ...
    if (g_mmi_bt_scr_cntx.main_menu_gid > 0)
    {
        if (g_mmi_bt_scr_cntx.main_menu_gid == mmi_frm_group_get_active_id())
        {
           ...
           //             U16 hintstrid = 0, lskstrid = 0;
           //             switch (srv_bt_cm_get_visibility())
           //             {
           //                 case SRV_BT_CM_VISIBILITY_ON:
           // #ifdef __MMI_BT_TEMP_VISIBILITY__
           //                     hintstrid = STR_BT_DEV_SET_VIS_PERMANENT_ON;
           // #else
           //                     hintstrid = STR_GLOBAL_ON;
           //                     lskstrid = STR_GLOBAL_OFF;
           // #endif
           //                     break;
           // #ifdef __MMI_BT_TEMP_VISIBILITY__
           //                 case SRV_BT_CM_VISIBILITY_TEMPORARY_ON:
           //                     hintstrid = STR_BT_DEV_SET_VIS_TEMPORARY_ON;
           //                     break;
           // #endif
           //                 case SRV_BT_CM_VISIBILITY_OFF:
           //                     hintstrid = STR_GLOBAL_OFF;
           // #ifndef __MMI_BT_TEMP_VISIBILITY__
           //                     lskstrid = STR_GLOBAL_ON;
           // #endif
           //                     break;
           //                 default:
           //                     break;
           //             }
           //             cui_menu_set_item_hint(
           //                 g_mmi_bt_scr_cntx.main_menu_gid,
           //                 MENU_CONN_BT_SET_VIS,
           //                 (WCHAR*)GetString(hintstrid));
           // #ifndef __MMI_BT_TEMP_VISIBILITY__
           //             cui_menu_change_left_softkey_string(
           //                 g_mmi_bt_scr_cntx.main_menu_gid,
           //                 (WCHAR*)GetString(lskstrid));
           // #endif
        }
    }
}
...
mmi_ret mmi_bt_entry_mydev_authorize_group_proc(mmi_event_struct* evt)
{
    ...
    switch (menu_evt->evt_id)
    {
        ...
        //     {
        //         case MENU_CONN_BT_MYDEV_AUTHORIZE_ALWAYS_ASK:
        //             g_mmi_bt_scr_cntx.authorize_hilight_item = SRV_BT_CM_AUTHORIZE_ALWAYS_ASK;
        //             break;
        //         case MENU_CONN_BT_MYDEV_AUTHORIZE_ALWAYS_CONNECT:
        //             g_mmi_bt_scr_cntx.authorize_hilight_item = SRV_BT_CM_AUTHORIZE_ALWAYS_CONNECT;
        //             break;
        //         case MENU_CONN_BT_MYDEV_AUTHORIZE_ALWAYS_REJECT:
        //             g_mmi_bt_scr_cntx.authorize_hilight_item = SRV_BT_CM_AUTHORIZE_ALWAYS_REJECT;
        //             break;
        //         default:
        //             break;
        //     }
...
void mmi_bt_entry_connecting(mmi_id group_id)
{
    ...
    //if (g_mmi_bt_scr_cntx.flag & MMI_BT_CONFIRM_CONNECT_AUDIO)
    //{
    //    MMI_BT_RESET_FLAG(MMI_BT_CONFIRM_CONNECT_AUDIO);
    //}
 ...
 *  void
 *****************************************************************************/
// void mmi_bt_entry_release_profile_connection(mmi_id group_id)
// {
//     /*----------------------------------------------------------------*/
//     /* Local Variables                                                */
//     /*----------------------------------------------------------------*/
//     U8 i;
//     srv_bt_cm_connect_struct *conn_info = NULL;
//     srv_bt_cm_dev_struct *a2dp_dev = NULL;
//     U32 profile_uuid = SRV_BT_CM_MAX_PROFILE_UUID;
...
//     /*when display release connection confirm, maybe opposite disconnect, so when this time user press Yes
//       it should connect another device HFP*/
//     if (mmi_bt_connecting_check(&(g_mmi_bt_scr_cntx.conn_dev_add),SRV_BT_CM_HFG_PROFILE_UUID) == SRV_BT_CM_RESULT_SUCCESS &&
//         mmi_bt_connecting_check(&(g_mmi_bt_scr_cntx.conn_dev_add),SRV_BT_CM_ADVANCED_AUDIO_PROFILE_UUID) == SRV_BT_CM_RESULT_SUCCESS)
//     {
//     #ifdef __MMI_HFP_SUPPORT__
//         mmi_btaud_attach(group_id, &(g_mmi_bt_scr_cntx.conn_dev_add));
//     #else
...
//     /*get currently device connected HFP, and the device is not currently highlight,
//     because if the device is same, will not display connect*/
//     if (mmi_bt_connecting_check(&(g_mmi_bt_scr_cntx.conn_dev_add),SRV_BT_CM_HFG_PROFILE_UUID) == SRV_BT_CM_RESULT_ONE_HEADSET_CONNECTED)
//     {
//         U32 dev_num, i;
//         const srv_bt_cm_dev_struct* dev_p;

//         dev_num = srv_bt_cm_get_dev_num(SRV_BT_CM_PAIRED_DEV);
//         for (i = 0; i < dev_num ; ++i)
//         {
//             dev_p = srv_bt_cm_get_dev_info_by_index(i, SRV_BT_CM_PAIRED_DEV);
//             if (srv_bt_cm_is_headset_connected(&dev_p->bd_addr))
//             {
//                 memcpy(
//                     &g_mmi_bt_scr_cntx.disconn_dev_add,
//                     &dev_p->bd_addr,
...
//         if (srv_bt_cm_release_profile(
//                 &(g_mmi_bt_scr_cntx.disconn_dev_add),
//                 SRV_BT_CM_HFG_PROFILE_UUID)
//             == SRV_BT_CM_RESULT_NO_CONNECTION)
//         {
//             if (g_mmi_bt_scr_cntx.flag & MMI_BT_RELEASE_AUDIO_DEV_BEFORE_CONNECT)
//             {
//                 MMI_BT_RESET_FLAG(MMI_BT_RELEASE_AUDIO_DEV_BEFORE_CONNECT);
//             #ifdef __MMI_HFP_SUPPORT__
//                 mmi_btaud_attach(group_id, &(g_mmi_bt_scr_cntx.disconn_dev_add));
//             #else
//                 mmi_popup_display_ext(
//                     STR_GLOBAL_CURRENTLY_NOT_AVAILABLE,
//                     MMI_EVENT_FAILURE,
...
//     /*get currently device connected A2DP*/
//     for (i = 0; i < srv_bt_cm_get_existed_conn_num(); i++)
//     {
//         conn_info = srv_bt_cm_get_existed_conn_info(i);
//         if (conn_info->profile_id == SRV_BT_CM_ADVANCED_AUDIO_PROFILE_UUID ||
//             conn_info->profile_id == SRV_BT_CM_AV_REMOTE_CONTROL_UUID)
//         {
//             profile_uuid = conn_info->profile_id;
//             a2dp_dev = (srv_bt_cm_dev_struct *)srv_bt_cm_get_dev_info_by_index(conn_info->index_of_dev_list, SRV_BT_CM_ALL_DEV);
//             break;
//         }
//     }
...
//     if (a2dp_dev != NULL)
//     {
//         memcpy(&(g_mmi_bt_scr_cntx.disconn_dev_add), &(a2dp_dev->bd_addr), sizeof(srv_bt_cm_bt_addr));

//         if (srv_bt_cm_release_profile(&(g_mmi_bt_scr_cntx.disconn_dev_add), profile_uuid) == SRV_BT_CM_RESULT_NO_CONNECTION)
//         {
//             if (g_mmi_bt_scr_cntx.flag & MMI_BT_RELEASE_AUDIO_DEV_BEFORE_CONNECT)
//             {
//                 MMI_BT_RESET_FLAG(MMI_BT_RELEASE_AUDIO_DEV_BEFORE_CONNECT);
...
//             #ifdef __MMI_HFP_SUPPORT__
//                 mmi_btaud_attach(group_id, &(a2dp_dev->bd_addr));
//             #else
//                 mmi_popup_display_ext(
//                     STR_GLOBAL_CURRENTLY_NOT_AVAILABLE,
//                     MMI_EVENT_FAILURE,
...
void mmi_bt_release_conn_complete(void *para)
{
    ...
    if (event->result)
    {
        ...
        //    MMI_SCENARIO_ID_DEFAULT,
        //    MMI_EVENT_SUCCESS,
        //    (WCHAR*)GetString(STR_BT_CON_RELEASED));
        //}
    }
    else
    {
...
//     mmi_wcscpy(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) GetString(STR_GLOBAL_ACCEPT));
//     mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) L" ");
//     mmi_bt_get_profile_str_id((U16) authorize_ptr->prof_id, &(str_p));
//     mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) str_p);
//     mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) L" ");
//     mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) GetString(STR_BT_FROM));
//     mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) L" ");
//     mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) g_mmi_bt_scr_cntx.scr_buf);
//     mmi_wcscat(g_mmi_bt_scr_cntx.ui_buf, (WCHAR *) GetString(STR_BT_QUESTION_MARK));
    mmi_wsprintf(
...
void mmi_bt_update_status_icon(void)
{
    ...
    MMI_TRACE(
        ...
        srv_bt_cm_is_profile_connected(SRV_BT_CM_HFP_CONNECTION) || srv_bt_cm_is_profile_connected(SRV_BT_CM_HSP_CONNECTION), srv_earphone_is_plug_in());

    wgui_status_icon_bar_reset_icon_display(STATUS_ICON_BT);
    wgui_status_icon_bar_reset_icon_display(STATUS_ICON_BT_CON);

    if (srv_bt_cm_get_power_status() == SRV_BT_CM_POWER_ON ||
        srv_bt_cm_get_power_status() == SRV_BT_CM_POWER_SWITCHING_OFF)
    {
...
MMI_BOOL mmi_bt_is_permission(MMI_BOOL is_popup)
{
    ...
    U16 act_scr_id = mmi_frm_scrn_get_active_id();
    ...
    else if (srv_bt_cm_get_power_status() == SRV_BT_CM_POWER_SWITCHING_OFF)
    {
        if (is_popup)
        {
            mmi_popup_display_ext(
...
MMI_BOOL mmi_bt_is_power_on(MMI_BOOL is_popup)
{
    ...
    //MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_BT_IS_POWER_ON, res);
    return res;
}
 ...
 *  suppress_level  if suppress_switch is MMI_TRUE, suppress_level is the bitmask with following bits.
 ...
 *  void
 *****************************************************************************/
void mmi_bt_suppress_popup(MMI_BOOL suppress_switch, U32 supress_level)
{
#if 0
/* under construction !*/
/* under construction !*/
...
mmi_ret mmi_bt_remove_disconnect_sco_screen_leave_proc(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
        ...
        // case EVT_ID_SCRN_DELETE_REQ:
        // case EVT_ID_SCRN_DELETE_REQ_IN_END_KEY:
        //     mmi_bt_remove_disconnect_sco_screen();
        //     break;
 ...
 *  mmi_bt_remove_disconnect_sco_screen
 * DESCRIPTION
 *  This function is to remove search progressing screen and invoke inquiry stop callback
 * PARAMETERS
 *  void
 * RETURNS
 *  void
...
//     /*----------------------------------------------------------------*/
//     srv_btsco_inquiry_stop_callback();
//     mmi_frm_scrn_set_leave_proc(g_mmi_bt_scr_cntx.prog_scr.gid, SCR_BT_SCH_DISCONNECT_SCO, NULL);
//     mmi_frm_scrn_close(g_mmi_bt_scr_cntx.prog_scr.gid, SCR_BT_SCH_DISCONNECT_SCO);
// }
