...
{
	static NMGR_HANDLE nmgr_alert_handle;
    mmi_frm_nmgr_alert_struct alert;

    if (nmgr_alert_handle != NULL)
    {
        mmi_frm_nmgr_alert_cancel(nmgr_alert_handle);
    }

    memset(&alert, 0, sizeof(mmi_frm_nmgr_alert_struct));
    alert.app_id = APP_DOWNLOADAGENT;
    alert.scen_id = MMI_SCENARIO_ID_DEFAULT;
	alert.event_type = MMI_EVENT_INFO;
	...
	alert.behavior_mask = MMI_FRM_NMGR_UI_POPUP_MASK;
	alert.ui_mask = MMI_FRM_NMGR_UI_POPUP_MASK;
	
	
    alert.popup_para.popup_string = (WCHAR *)GetString(STR_ID_DA_NEW_PUSH);   
    
    alert.app_proc_para.scrn_group_proc = &mmi_da_nmgr_proc;
    alert.app_proc_para.user_data = NULL;  
    nmgr_alert_handle = mmi_frm_nmgr_alert(&alert);
