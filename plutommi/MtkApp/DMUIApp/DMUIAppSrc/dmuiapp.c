...
#if defined(__MMI_DUAL_SIM_MASTER__) && defined(__MMI_DM_SELF_REGISTER_SUPPORT__)
                SetProtocolEventHandler(mmi_dmui_self_register_result_ind_hdlr, MSG_ID_MMI_DM_SELF_REGISTER_RESULT_IND);
#endif
...
/* under construction !*/
                ...
                #if defined(__MMI_DUAL_SIM_MASTER__) && defined(__MMI_DM_SELF_REGISTER_SUPPORT__)
                    init_config_info_req->sim_info = (S32)mmi_dm_used_sim();
                 #else
...
/*****************************************************************************
 * FUNCTION
 *  mmi_dmui_self_register_result_ind_hdlr
 * DESCRIPTION
 ...
 *  void
 *****************************************************************************/
void mmi_dmui_self_register_result_ind_hdlr(void *msg)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    dm_init_config_info_req_struct *init_config_info_req;
    mmi_dm_self_register_result_ind_struct *self_reg_result;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ptr_dmui_context->is_reg_sim_success = MMI_TRUE;
    self_reg_result = (mmi_dm_self_register_result_ind_struct*)msg;
    init_config_info_req = OslAllocDataPtr(dm_init_config_info_req_struct);
    init_config_info_req->sim_info = self_reg_result->sim;
    init_config_info_req->app_id = g_mmi_dmui_cbm_id;
    DMUI_SEND_MSG_TO_DMPS(MSG_ID_DM_INIT_CONFIG_INFO_REQ, init_config_info_req);
}
#endif
...
mmi_ret mmi_dmui_nw_info_srv_availability_changed(mmi_event_struct *evt)
{
    ...
    /*----------------------------------------------------------------*/
#if defined(__MMI_DUAL_SIM_MASTER__) && defined(__MMI_DM_SELF_REGISTER_SUPPORT__)
    if ((srv_evt->sim == MMI_SIM1 && mmi_dm_used_sim() == 1) ||
        (srv_evt->sim == MMI_SIM2 && mmi_dm_used_sim() == 0) ||
...
#if defined(__MMI_DUAL_SIM_MASTER__) && defined(__MMI_DM_SELF_REGISTER_SUPPORT__)
    if ((mmi_dm_used_sim() == 0 && action == MMI_DA_PUSH_SIM2) ||
        (mmi_dm_used_sim() == 1 && action == MMI_DA_PUSH_SIM1) ||
        mmi_dm_used_sim() < 0)
    {
        return;
    }
...
mmi_ret mmi_dm_sim_ctrl_det_hdlr(mmi_event_struct* para)
{
    ...
    /*----------------------------------------------------------------*/
#if defined(__MMI_DUAL_SIM_MASTER__) && defined(__MMI_DM_SELF_REGISTER_SUPPORT__)
    if (ptr_dmui_context->is_reg_sim_success == MMI_TRUE)
    {
        reg_sim = mmi_frm_index_to_sim(mmi_dm_used_sim());
    }
    else
    {
        return;
...
#if defined(__MMI_DUAL_SIM_MASTER__) && defined(__MMI_DM_SELF_REGISTER_SUPPORT__)
void mmi_dmui_dualmode_switch_notify(DUALMODE_SWITCH_STATUS *switch_status)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    /* sim changes to available */

    if ((mmi_dm_used_sim() == 0 && (switch_status->dest_status == MTPNP_SIM1_ONLY_CONFIG || switch_status->dest_status == MTPNP_DAUL_SIM_CONFIG)) ||
        (mmi_dm_used_sim() == 1 && (switch_status->dest_status == MTPNP_SIM2_ONLY_CONFIG || switch_status->dest_status == MTPNP_DAUL_SIM_CONFIG)))
    {
        g_mmi_dmui_is_reg_sim_available = MMI_TRUE;
    }
    else /* sim changes to not available */
    {
        g_mmi_dmui_is_reg_sim_available = MMI_FALSE;
    }
}
#endif /* __MMI_DUAL_SIM_MASTER__ */
...
MMI_BOOL mmi_dmui_check_self_reg_sim_match(void)
{
    ...
    else if ((mmi_dm_used_sim() == 0 && mode_state == SRV_MODE_SIM2_ONLY) ||
             (mmi_dm_used_sim() == 1 && mode_state == SRV_MODE_SIM1_ONLY) ||
             mmi_dm_used_sim() < 0)
    {
        return MMI_FALSE;
    }
...
void mmi_dmui_switch_nw_mode(void)
{
    ...
    #ifdef __MMI_DUAL_SIM_MASTER__
             | (srv_bootup_sim_is_inserted(MMI_SIM2) << 1)
    #if (MMI_MAX_SIM_NUM >= 3)
             | (srv_bootup_sim_is_inserted(MMI_SIM3) << 2)
    #if (MMI_MAX_SIM_NUM >= 4)
             | (srv_bootup_sim_is_inserted(MMI_SIM4) << 3)
    #endif
    #endif
    #endif
        ;
