...
void mmi_em_audio_debug_info_init(void)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ReadValue(nvram_ids[i], &AudioParam_debug_info[i * 4], DS_DOUBLE, &error);
        ...
        if(AudioParam_debug_info[i * 4] == 0xFFFF)
        {
            ...
            WriteValue(nvram_ids[i], &AudioParam_debug_info[i * 4], DS_DOUBLE, &error);
        }
    }
...
void EngineerModeGetAudioProfileReq(U8 mode)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_GET_AUDIO_PROFILE_RSP, EngineerModeGetAudioProfileRes, MMI_FALSE);
...
void EngineerModeSetAudioProfileReq(void *audioProfile)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_SET_AUDIO_PROFILE_RSP, EngineerModeSetAudioProfileRes, MMI_FALSE);
...
U8 EngineerModeSetAudioProfileRes(void *inMsg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_SET_AUDIO_PROFILE_RSP, NULL, MMI_FALSE);
...
void EngineerModeGetAudioParamReq()
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_GET_AUDIO_PARAM_RSP, EngineerModeGetAudioParamRes, MMI_FALSE);
...
U8 EngineerModeGetAudioParamRes(void *inMsg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_GET_AUDIO_PARAM_RSP, NULL, MMI_FALSE);

    if (rsp_p->result)
    {
        /* Read debug info from NVRAM directly, not through message*/
        ReadValue(NVRAM_AUDIO_DEBUG_INFO1, &cntx->AudioParam_debug_info[0], DS_DOUBLE, &error);
        ReadValue(NVRAM_AUDIO_DEBUG_INFO2, &cntx->AudioParam_debug_info[4], DS_DOUBLE, &error);
...
void EngineerModeSetAudioParamReq(void *audioParam)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_SET_AUDIO_PARAM_RSP, EngineerModeSetAudioParamRes, MMI_FALSE);
...
U8 EngineerModeSetAudioParamRes(void *inMsg)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_SET_AUDIO_PARAM_RSP, NULL, MMI_FALSE);
    
    if (rsp_p->result)
    {
        /* Write debug info from NVRAM directly, not through message*/
        WriteValue(NVRAM_AUDIO_DEBUG_INFO1, &cntx->AudioParam_debug_info[0], DS_DOUBLE, &error);
        WriteValue(NVRAM_AUDIO_DEBUG_INFO2, &cntx->AudioParam_debug_info[4], DS_DOUBLE, &error);
...
void mmi_em_aud_auto_vm_setting_init(void)
{
    ...
    ReadValue(NVRAM_AUTO_VM_SETTING_AUTO_VM, (void*)&ret_value, DS_BYTE, &error);
    if(ret_value == 0)
		is_on = MMI_FALSE;
	else
		is_on = MMI_TRUE;
...
static void mmi_em_aud_auto_vm_setting_hdlr(cui_menu_event_struct *evt)
{
    ...
    WriteValue(NVRAM_AUTO_VM_SETTING_AUTO_VM, (void*)&ret_value, DS_BYTE, &error);
    if(ret_value == 0)
		is_on = MMI_FALSE;
	else
		is_on = MMI_TRUE;    
