...
static void mmi_em_misc_dm_self_register_result_sim_entry(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    static S8 sim_card[14];
    U8 sim;
    S16 error;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    ...
    if(mmi_frm_scrn_enter(GRP_ID_EM_MISC_DM_SELF_REGISTER_RESULT_SIM, 
                            SCR_ID_EM_MISC_DM_SELF_REGISTER_RESULT_SIM, 
                            NULL, 
                            mmi_em_misc_dm_self_register_result_sim_entry, 
                            MMI_FRM_FULL_SCRN) == MMI_FALSE)    
    {
        return;
    }
    ...
    ReadValue(NVRAM_DM_SELF_REGISTER_SIM, &sim, DS_BYTE, &error);
   
    switch (sim)
    {	
        case 0: mmi_ucs2cat(sim_card, (PS8) L"SIM1");
                break;
	case 1: mmi_ucs2cat(sim_card, (PS8) L"SIM2");
	        break;
	default:
                mmi_ucs2cat(sim_card, (PS8) L"No SIM");
	        break;
    }
    
    ShowCategory74Screen(
    STR_ID_EM_MISC_DM_SELF_REGISTER_RESULT_DUAL,
	0,
    0,
    0,
    STR_GLOBAL_BACK,
...
void mmi_em_misc_dm_self_register_result_sim_main(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (mmi_frm_group_is_present(GRP_ID_EM_MISC_DM_SELF_REGISTER_RESULT_SIM))
    {
        mmi_frm_group_close(GRP_ID_EM_MISC_DM_SELF_REGISTER_RESULT_SIM);
    }
    root_id = mmi_em_get_root_group();
    mmi_frm_group_create(root_id, GRP_ID_EM_MISC_DM_SELF_REGISTER_RESULT_SIM, NULL, NULL);
    mmi_frm_group_enter(GRP_ID_EM_MISC_DM_SELF_REGISTER_RESULT_SIM, MMI_FRM_NODE_SMART_CLOSE_FLAG);
    mmi_em_misc_dm_self_register_result_sim_entry();
}
