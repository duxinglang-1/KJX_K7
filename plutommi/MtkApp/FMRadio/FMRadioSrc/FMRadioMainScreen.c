...
void mmi_fmrdo_init_for_theme(void)
{
    ...
    /*----------------------------------------------------------------*/
    /* LSK */
    fmrdo_main_draw.LSK_text_border_color = *(current_MMI_theme->softkey_text_border_color);
    fmrdo_main_draw.LSK_text_color = *(current_MMI_theme->LSK_up_text_color);

    /* RSK */
    fmrdo_main_draw.RSK_text_border_color = *(current_MMI_theme->softkey_text_border_color);
    fmrdo_main_draw.RSK_text_color = *(current_MMI_theme->LSK_up_text_color);

    /* channel name */
    fmrdo_main_draw.channel_name_text_border_color = *(current_MMI_theme->title_text_border_color);
    fmrdo_main_draw.channel_name_text_color = *(current_MMI_theme->title_text_color);
#ifdef __MMI_FM_RADIO_RDS__
...
#endif/*__MMI_FM_RADIO_RDS__*/
    /* frequency */
    fmrdo_main_draw.frequency_text_on_color = *(current_MMI_theme->media_player_content_text_color);
    fmrdo_main_draw.frequency_text_off_color = *(current_MMI_theme->cascade_menu_disabled_text_color);
...
void mmi_fmrdo_init_main_images(S32 skin_index)
{
    ...
    fmrdo_main_draw.frequency_text_off_color = *(current_MMI_theme->cascade_menu_disabled_text_color);
	/*Title*/
	fmrdo_main_draw.title_text_color = *(current_MMI_theme->title_text_color);
	fmrdo_main_draw.title_text_border_color = *(current_MMI_theme->title_text_border_color);
	/* channel name */
    fmrdo_main_draw.channel_name_text_border_color = *(current_MMI_theme->title_text_color);
    fmrdo_main_draw.channel_name_text_color = *(current_MMI_theme->title_text_border_color);

    fmrdo_main_draw.label_text_color = gui_color(
                                                skin_p->label_text_color_r,
                                                skin_p->label_text_color_g,
                                                skin_p->label_text_color_b);
#endif /* __MMI_SLIM_IMG_RES__ */
...
void mmi_fmrdo_util_draw_soft_key(MMI_BOOL is_lsk, MMI_BOOL is_down)
{
    ...
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    S32  dim_x1, dim_y1, dim_x2, dim_y2;
    S32  str_x1, str_y1;
    S32  str_w, str_h;
    S32  trunc_w;
    U16* str_p;
#ifdef __MMI_TOUCH_SCREEN__
    mmi_pen_point_struct *key_points;
#endif /* __MMI_TOUCH_SCREEN__ */
    WGUI_SOFTKEY_ENUM key_type;
    MMI_BOOL is_trunc;
    ...
    if (is_lsk)
    {
        key_type = MMI_LEFT_SOFTKEY;
        str_p    = fmrdo_main_draw.LSK_text;
    #ifdef __MMI_TOUCH_SCREEN__
        key_points = fmrdo_draw_cntx_p->LSK_points;
    #endif /* __MMI_TOUCH_SCREEN__ */
    }
    else
    {
        key_type = MMI_RIGHT_SOFTKEY;
        str_p    = fmrdo_main_draw.RSK_text;
    #ifdef __MMI_TOUCH_SCREEN__
        key_points = fmrdo_draw_cntx_p->RSK_points;
    #endif /* __MMI_TOUCH_SCREEN__ */
    }

    /* calculate coordinate */
    gui_measure_string(str_p, &str_w, &str_h);
    wgui_softkey_get_position(key_type, &dim_x1, &dim_y1);
    wgui_softkey_get_dimension(key_type, &dim_x2, &dim_y2);
    dim_y2 += dim_y1; /* include +1 for key down case */
    dim_x2 += dim_x1;
    ...
    {
        mmi_fmrdo_util_update_sk_area(key_points, 
                                      str_x1, 
                                      str_y1,
                                      (is_trunc ? trunc_w : str_w),
                                      str_h);
    }   
#endif /* __MMI_TOUCH_SCREEN__ */
    ...
    mmi_fmrdo_set_alpha_blending_layer(TRUE);
#endif 
    gdi_layer_push_and_set_clip(dim_x1, dim_y1, dim_x2, dim_y2);
    mmi_fmrdo_redraw_main_background(dim_x1, dim_y1, dim_x2, dim_y2);
    
    if (is_trunc)
    {
        gui_print_truncated_borderd_text(str_x1, str_y1, trunc_w, str_p);
    }
    else
    {
        gui_move_text_cursor(str_x1, str_y1);
        gui_print_bordered_text(str_p);
    }
    gdi_layer_pop_clip();
#ifdef __GDI_MEMORY_PROFILE_2__
    mmi_fmrdo_set_alpha_blending_layer(FALSE);
#endif
    gdi_layer_blt_previous(dim_x1, dim_y1, dim_x2, dim_y2);
...
#endif
    mmi_fmrdo_util_draw_soft_key(MMI_TRUE, MMI_TRUE);
}
...
void mmi_fmrdo_redraw_main_RSK_down(void)
{
	...
	gui_set_text_border_color(border_color);
#endif
    mmi_fmrdo_util_draw_soft_key(MMI_FALSE, MMI_TRUE);
}
...
void mmi_fmrdo_redraw_main_LSK_up(void)
{
    ...
    mmi_fmrdo_util_draw_soft_key(MMI_TRUE, MMI_FALSE);
}
...
void mmi_fmrdo_redraw_main_RSK_up(void)
{
    ...
    mmi_fmrdo_util_draw_soft_key(MMI_FALSE, MMI_FALSE);
}
...
void mmi_fmrdo_redraw_main_all(void)
{
    ...
    if (!fmrdo_draw_cntx_p->is_skin_layer_drawn)
    {
        ...
        gui_draw_filled_area(0, 
                             UI_device_height - MMI_SOFTKEY_BAR_HEIGHT,  
                             UI_device_width - 1, 
                             UI_device_height - 1, 
...
void mmi_fmrdo_redraw_main_title(void)
{
    ...
    rect.x = fmrdo_main_draw.title.x;
    rect.y = fmrdo_main_draw.title.y;
    rect.w = fmrdo_main_draw.title.w;
    rect.h = fmrdo_main_draw.title.h;

    gui_set_font(&MMI_medium_font);
    gui_set_text_color(fmrdo_main_draw.title_text_color);

    mmi_fmrdo_util_draw_simple_text(&rect, (U16*)GetString(STR_ID_FMRDO_TITLE), 
                    FMRDO_FLAG_SET(flags, MMI_FMRDO_TEXT_HORZ_ACENTER|MMI_FMRDO_TEXT_VERT_ACERTER));
...
void mmi_fmrdo_redraw_main_freq_label(void)
{
    ...
    gui_set_font(&MMI_small_font);
    gui_set_text_color(fmrdo_main_draw.label_text_color);
    ...
    rect.x = fmrdo_main_draw.label.x + (fmrdo_main_draw.label.w >> 1) + 1;
    mmi_fmrdo_util_draw_simple_text(&rect, str, 
...
void mmi_fmrdo_change_main_SK_hw_searching(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    fmrdo_main_draw.LSK_text = (UI_string_type) GetString(STR_GLOBAL_STOP);
    fmrdo_main_draw.RSK_text = NULL;

    mmi_fmrdo_redraw_main_LSK_up();
    mmi_fmrdo_clear_main_RSK((UI_string_type) GetString(STR_GLOBAL_BACK));
}
...
void mmi_fmrdo_clear_main_RSK(UI_string_type text)
{
    ...
    gdi_layer_push_and_set_clip(UI_device_width / 2 + 10, dim_y1, dim_x2, dim_y2 - 1);
    mmi_fmrdo_redraw_main_background(UI_device_width / 2 + 10, dim_y1, dim_x2, dim_y2 - 1);
    gdi_layer_pop_clip();
    if (mmi_fe_get_r2l_state())
    {
        gdi_layer_push_and_set_clip(x2, y1 - 1, x1, y2);
    }
    else
    {
        gdi_layer_push_and_set_clip(x1, y1 - 1, x2, y2);
    }
    if (is_truncated)
    {
        gui_print_truncated_borderd_text(x1, y1, truncated_width, fmrdo_main_draw.RSK_text);
    }
    else
    {
    ...
    mmi_fmrdo_set_alpha_blending_layer(FALSE);
#endif 
    gdi_layer_blt_previous(UI_device_width / 2 + 10, dim_y1, dim_x2, dim_y2 - 1);
}
