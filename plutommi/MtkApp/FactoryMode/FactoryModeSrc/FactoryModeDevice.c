...
#include "..\\..\\kjx\\tour\\kjx_main_tour.h"
#include "..\\..\\kjx\\inc\\kjx_prin.h"
#ifdef __KJX_FUN_GPS__
#include "..\\..\\kjx\\inc\\kjx_gps.h"
#endif
#endif
...
#ifdef __KJX_FUN_GPS__
#define KJX_GPS_TEST_GOOD_SIGNAL_MIN	4
#endif
    ...
    switch (common_cntx->sequence_counter)
    {
        ...
        default:
            ...
            if (common_cntx->autoTestMode == 0)  /* Display the screen if not in autotest mode */
            {
                ...
                SetKeyHandler(mmi_fm_exit_color_set_change, KEY_ENTER, KEY_EVENT_UP);				
            #else /*__COSMOS_MMI_PACKAGE__*/
                SetKeyHandler(mmi_fm_exit_color_set_change, KEY_BACK, KEY_EVENT_UP);
...
extern void mmi_fm_show_color(S32 type)
{
    ...
    SetKeyHandler(mmi_frm_scrn_close_active_id, KEY_SEND, KEY_EVENT_UP);
    #endif
...
#ifdef __COSMOS_MMI_PACKAGE__    
    SetKeyHandler(mmi_fm_stop_adc, KEY_BACK, KEY_EVENT_UP);
#endif
...
extern void mmi_fm_select_adc_menu(void)
{
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EQ_ADC_ALL_CHANNEL_IND, 
	                                   (PsIntFuncPtr)mmi_fm_update_adc_hdlr, MMI_FALSE);
 ...
 *  void
 *****************************************************************************/
extern void mmi_fm_highlight_bat_led(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/    
    mmi_fm_common_set_left_softkey(g_fm_contxt->bat_led_contxt.BatLedTestOn);
}

static void mmi_fm_bat_led_show(void)
{
	static kjx_bat_led led_status = 0;

	led_status++;
	if(led_status >= BAT_LED_MAX)
    	led_status = BAT_LED_BEGIN;

   	kjx_tour_battery_status_led(led_status);
	
	StartTimer(KJX_TIMER_ID_FACTORY_TEST, 1000, mmi_fm_bat_led_show);	
}

static void mmi_fm_set_bat_led(MMI_BOOL BatLedTestOn, kal_bool on, U16 s )
{
	g_fm_contxt->bat_led_contxt.BatLedTestOn = BatLedTestOn;
	...
	{
		StopTimer(KJX_TIMER_ID_FACTORY_TEST);
 ...
 *  void
 *****************************************************************************/
extern void mmi_fm_select_bat_led(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if(!g_fm_contxt->bat_led_contxt.BatLedTestOn)
    {
        mmi_fm_set_bat_led(MMI_TRUE, KAL_TRUE, STR_GLOBAL_OFF);
    }
    else
    {
        mmi_fm_set_bat_led(MMI_FALSE, KAL_FALSE, STR_GLOBAL_ON);
    }
...
#ifdef __KJX_FUN_GPS__
static U32 gps_time_count = 0;
extern U8 g_kjx_signal_good_count;
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_stop_gps(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_update_gps_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/ 
	U16 lsk_str_id = NULL,rsk_str_id = STR_GLOBAL_BACK;
    U32 i = 0,j = 0;
    U8 * MenuItemNum;
    CHAR buffer[32] = {0};
	CHAR strr[20] = {0};
	...
	gps_time_count++;
    MenuItemNum = &(g_fm_contxt->gps_contxt.gFM_MenuItemNum);
    (*MenuItemNum) = 0;

	memset(subMenuData, 0, sizeof(subMenuData));
	
	//经度
	mmi_ucs2cpy((CHAR *)subMenuData[0], GetString(STR_ID_FM_GPS_LATITUDE));
    sprintf(strr, "%c%s", gps_info_string.Latitude_Dir, gps_info_string.Latitude);    
    mmi_asc_to_ucs2(buffer, strr);
	mmi_ucs2cat((CHAR *)subMenuData[0], buffer);
    subMenuDataPtrs[0] = subMenuData[0];
	(*MenuItemNum)++;

	//纬度
	mmi_ucs2cpy(subMenuData[1], GetString(STR_ID_FM_GPS_LONGITUDE));
    sprintf(strr, "%c%s", gps_info_string.Longitude_Dir, gps_info_string.Longitude);    
	mmi_asc_to_ucs2(buffer, strr);
	mmi_ucs2cat((CHAR *)subMenuData[1], buffer);
    subMenuDataPtrs[1] = subMenuData[1];
	(*MenuItemNum)++;

	//卫星状态
	mmi_ucs2cpy(subMenuData[2], GetString(STR_ID_FM_GPS_SIGNAL));
    sprintf(strr, "%d", g_kjx_signal_good_count);    
	mmi_asc_to_ucs2(buffer, strr);
	mmi_ucs2cat((CHAR *)subMenuData[2], buffer);
	mmi_ucs2cat((CHAR *)subMenuData[2], GetString(STR_ID_FM_GPS_SIGNAL_GOOD_SATELLITE));
    subMenuDataPtrs[2] = subMenuData[2];
	(*MenuItemNum)++;

	//定位状态
	mmi_ucs2cpy((CHAR *)subMenuData[3],GetString(STR_ID_FM_GPS_STATUS));
	if(gps_info_string.gps_DataStatus == 'A')
	{
		mmi_ucs2cat((CHAR *)subMenuData[3], GetString(STR_ID_FM_GPS_LOCATION));  
	}
	else
	{
		mmi_ucs2cat((CHAR *)subMenuData[3], GetString(STR_ID_FM_GPS_NO_LOCATION));     
	}
	subMenuDataPtrs[3] = subMenuData[3];
	(*MenuItemNum)++;
	
	if((gps_info_string.gps_DataStatus == 'A') && (g_kjx_signal_good_count >= KJX_GPS_TEST_GOOD_SIGNAL_MIN))
	{
		lsk_str_id = STR_GLOBAL_OK;
	}
	else
	{
		if(gps_time_count > 300)
		{
			gps_time_count = 0;
			lsk_str_id = STR_GLOBAL_FAILED;
		}
		else
		{
			StartTimer(KJX_TIMER_ID_FACTORY_TEST, 1000, mmi_fm_update_gps_hdlr);
		}
	}
	...
	ShowCategory6Screen(
			...
			rsk_str_id,
			0,
			g_fm_contxt->gps_contxt.gFM_MenuItemNum,
			(PU8*)subMenuDataPtrs,
			NULL,
			0,
			NULL);

	SetRightSoftkeyFunction(mmi_fm_stop_gps, KEY_EVENT_UP);	
	if(lsk_str_id != NULL)
		SetLeftSoftkeyFunction(mmi_fm_stop_gps, KEY_EVENT_UP);
}

/*****************************************************************************
 * FUNCTION
 *  mmi_fm_prepare_gps_list
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_prepare_gps_list(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	//纬度
	mmi_ucs2cpy((CHAR *)subMenuData[1], GetString(STR_ID_FM_GPS_LONGITUDE));
    sprintf(strr, "%c%s", gps_info_string.Longitude_Dir, gps_info_string.Longitude);    
    mmi_asc_to_ucs2(buffer, strr);
	mmi_ucs2cat((CHAR *)subMenuData[1], buffer);
    subMenuDataPtrs[1] = subMenuData[1];
	...
	{
		mmi_ucs2cat((CHAR *)subMenuData[3],GetString(STR_ID_FM_GPS_LOCATION));  
		StartTimer(KJX_TIMER_ID_FACTORY_TEST, 200, mmi_fm_update_gps_hdlr);
	}
	else
	{
		mmi_ucs2cat((CHAR *)subMenuData[3],GetString(STR_ID_FM_GPS_NO_LOCATION));     
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_exit_gps_menu(void)
{
    gps_time_count = 0;
	kjx_close_gps();
	StopTimer(KJX_TIMER_ID_FACTORY_TEST);
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_enter_gps_menu(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
extern void mmi_fm_select_gps(void)
{
	gps_time_count = 0;
    kjx_open_gps();
    mmi_fm_enter_gps_menu();
...
#ifdef __KJX_RFID__
static U32 rfid_time_count = 0;
extern U32 g_kjx_rfid_id;
extern void kjx_rfid_init(void);
/*****************************************************************************
 * FUNCTION
 *  mmi_fm_stop_gps
 * DESCRIPTION
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_stop_rfid(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_update_rfid_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/ 
	...
	rfid_time_count++;
    MenuItemNum = &(g_fm_contxt->rfid_contxt.gFM_MenuItemNum);
    (*MenuItemNum) = 0;

	memset(subMenuData, 0, sizeof(subMenuData));
	
	//RFID
    sprintf(strr, "RFID:%08d", g_kjx_rfid_id);    
    mmi_asc_to_ucs2(buffer, strr);
	mmi_ucs2cpy((CHAR *)subMenuData[0], buffer);
    subMenuDataPtrs[0] = subMenuData[0];
	(*MenuItemNum)++;
	...
	else
	{
		...
		{
			StartTimer(KJX_TIMER_ID_FACTORY_TEST, 1000, mmi_fm_update_rfid_hdlr);
		}
	}
	...
	ShowCategory6Screen(
			...
			rsk_str_id,
			0,
			g_fm_contxt->rfid_contxt.gFM_MenuItemNum,
			(PU8*)subMenuDataPtrs,
			NULL,
			0,
			NULL);

	SetRightSoftkeyFunction(mmi_fm_stop_rfid, KEY_EVENT_UP);	
	if(lsk_str_id != NULL)
		SetLeftSoftkeyFunction(mmi_fm_stop_rfid, KEY_EVENT_UP);
}
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_prepare_rfid_list(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	if(g_kjx_rfid_id != 0)
	{
		StartTimer(KJX_TIMER_ID_FACTORY_TEST, 200, mmi_fm_update_rfid_hdlr);
	}
}
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_exit_rfid_menu(void)
{
	rfid_time_count = 0;
	StopTimer(KJX_TIMER_ID_FACTORY_TEST);
}
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_enter_rfid_menu(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    mmi_frm_scrn_enter
        (g_fm_gourp_id, 
        SCR_ID_FM_RFID, 
        mmi_fm_exit_rfid_menu, 
        mmi_fm_enter_rfid_menu, 
        MMI_FRM_FULL_SCRN);

    guiBuffer = mmi_frm_scrn_get_active_gui_buf();

#ifdef __MTK_TARGET__
    mmi_fm_prepare_rfid_list();
#endif 
 ...
 *  void
 *****************************************************************************/
extern void mmi_fm_select_rfid(void)
{
	rfid_time_count = 0;
    kjx_rfid_init();
    mmi_fm_enter_rfid_menu();
...
static U32 ble_time_count = 0;
extern U32 g_kjx_ble_id;
extern void kjx_bt_ibeacon_init(void);
/*****************************************************************************
 * FUNCTION
 *  mmi_fm_stop_ble
 * DESCRIPTION
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_stop_ble(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_update_ble_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/ 
	...
	ble_time_count++;
    MenuItemNum = &(g_fm_contxt->ble_contxt.gFM_MenuItemNum);
    (*MenuItemNum) = 0;

	memset(subMenuData, 0, sizeof(subMenuData));
	
	//BLE
    sprintf(strr, "BLE:%08d", g_kjx_ble_id);    
    mmi_asc_to_ucs2(buffer, strr);
	mmi_ucs2cpy((CHAR *)subMenuData[0], buffer);
    subMenuDataPtrs[0] = subMenuData[0];
	(*MenuItemNum)++;
	...
	else
	{
		...
		{
			StartTimer(KJX_TIMER_ID_FACTORY_TEST, 1000, mmi_fm_update_ble_hdlr);
		}
	}
	...
	ShowCategory6Screen(
			...
			rsk_str_id,
			0,
			g_fm_contxt->ble_contxt.gFM_MenuItemNum,
			(PU8*)subMenuDataPtrs,
			NULL,
			0,
			NULL);

	SetRightSoftkeyFunction(mmi_fm_stop_ble, KEY_EVENT_UP);	
	if(lsk_str_id != NULL)
		SetLeftSoftkeyFunction(mmi_fm_stop_ble, KEY_EVENT_UP);
}

/*****************************************************************************
 * FUNCTION
 *  mmi_fm_prepare_ble_list
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_prepare_ble_list(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	if(g_kjx_ble_id != 0)
	{
		StartTimer(KJX_TIMER_ID_FACTORY_TEST, 200, mmi_fm_update_ble_hdlr);
	}
}

/*****************************************************************************
 * FUNCTION
 *  mmi_fm_exit_ble_menu
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_exit_ble_menu(void)
{
    ble_time_count = 0;
	StopTimer(KJX_TIMER_ID_FACTORY_TEST);
}
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_enter_ble_menu(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
extern void mmi_fm_select_ble(void)
{
	ble_time_count = 0;
...
#endif/*__BLE_MODULE_SUPPORT__*/

#ifdef __BOSCH_BMP280_PRESSURE__
static U32 pressure_time_count = 0;
extern float g_kjx_temp;
extern float g_kjx_press;
extern float g_kjx_mmHg;
extern float g_kjx_altitude;
extern void BOSCH_BMP280_sensor_init(void);
extern void BOSCH_BMP280_get_data(void);
/*****************************************************************************
 * FUNCTION
 *  mmi_fm_stop_pressure
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_stop_pressure(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_update_pressure_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/ 
	...
	pressure_time_count++;
    MenuItemNum = &(g_fm_contxt->pressure_contxt.gFM_MenuItemNum);
    (*MenuItemNum) = 0;

	memset(subMenuData, 0, sizeof(subMenuData));
	
	//温度
	mmi_ucs2cpy((CHAR *)subMenuData[0], GetString(STR_KJX_IDLE_TEMPERATURE));
    sprintf(strr, "%d.%dC", ((kal_int32)g_kjx_temp)/100, abs((kal_int32)g_kjx_temp)%100);    
    mmi_asc_to_ucs2(buffer, strr);
	mmi_ucs2cat((CHAR *)subMenuData[0], buffer);
    subMenuDataPtrs[0] = subMenuData[0];
	(*MenuItemNum)++;
	...
	//气压
	mmi_ucs2cpy((CHAR *)subMenuData[1], GetString(STR_KJX_IDLE_ATMOSPHERE));
    sprintf(strr, "%dPa", (kal_uint32)g_kjx_press);    
    mmi_asc_to_ucs2(buffer, strr);
	mmi_ucs2cat((CHAR *)subMenuData[1], buffer);
    subMenuDataPtrs[1] = subMenuData[1];
	(*MenuItemNum)++;
	...
	//海拔
	mmi_ucs2cpy((CHAR *)subMenuData[2],GetString(STR_KJX_IDLE_ALTITUDE));
    sprintf(strr, "%.2fM", g_kjx_altitude);    
    mmi_asc_to_ucs2(buffer, strr);
	mmi_ucs2cat((CHAR *)subMenuData[2], buffer);
	subMenuDataPtrs[2] = subMenuData[2];
	(*MenuItemNum)++;
	
	if((((kal_int32)g_kjx_temp)/100 > 0) && (((kal_int32)g_kjx_temp)/100 < 40) 
		&& (g_kjx_altitude > 0) &&(g_kjx_altitude < 1000))
	{
		lsk_str_id = STR_GLOBAL_OK;
	}
	else
	{
		if(pressure_time_count > 5)
		{
			pressure_time_count = 0;
			lsk_str_id = STR_GLOBAL_FAILED;
		}
		else
		{
			...
			{
				StartTimer(KJX_TIMER_ID_FACTORY_TEST, 1000, mmi_fm_update_pressure_hdlr);
			}
		}
	}
	...
	ShowCategory6Screen(
			...
			rsk_str_id,
			0,
			g_fm_contxt->pressure_contxt.gFM_MenuItemNum,
			(PU8*)subMenuDataPtrs,
			NULL,
			0,
			NULL);

	SetRightSoftkeyFunction(mmi_fm_stop_pressure, KEY_EVENT_UP);	
	if(lsk_str_id != NULL)
		SetLeftSoftkeyFunction(mmi_fm_stop_pressure, KEY_EVENT_UP);
}

/*****************************************************************************
 * FUNCTION
 *  mmi_fm_prepare_pressure_list
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_prepare_pressure_list(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_exit_pressure_menu(void)
{
    pressure_time_count = 0;
	StopTimer(KJX_TIMER_ID_FACTORY_TEST);
}
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_enter_pressure_menu(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
extern void mmi_fm_select_pressure(void)
{
	pressure_time_count = 0;
...
#endif/*__BOSCH_BMP280_PRESSURE__*/

#ifdef __MOTION_SENSOR_MC3451_SUPPORT__
static U32 sensor_time_count = 0;
...
extern void mmi_mc3451_get_pedometer_data(void);
/*****************************************************************************
 * FUNCTION
 *  mmi_fm_stop_sensor
 * DESCRIPTION
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_stop_sensor(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_update_sensor_hdlr(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/ 
	...
	sensor_time_count++;
    MenuItemNum = &(g_fm_contxt->sensor_contxt.gFM_MenuItemNum);
    (*MenuItemNum) = 0;
	...
	//请摇晃机器
	mmi_ucs2cpy((CHAR *)subMenuData[0], GetString(STR_ID_FM_SENSOR_PLEASE_SHAKE));
    subMenuDataPtrs[0] = subMenuData[0];
	(*MenuItemNum)++;
	
	//震动
	if(g_kjx_sensor_shake > 0)
		mmi_ucs2cpy((CHAR *)subMenuData[1], GetString(STR_ID_FM_SENSOR_CHECK_SHAKE_SUCCESS));
	else
		mmi_ucs2cpy((CHAR *)subMenuData[1], GetString(STR_ID_FM_SENSOR_CHECK_SHAKE_FAIL));
    subMenuDataPtrs[1] = subMenuData[1];
	(*MenuItemNum)++;
	
	//计步
	if(g_kjx_sensro_step > 0)
		mmi_ucs2cpy((CHAR *)subMenuData[2], GetString(STR_ID_FM_SENSOR_CHECK_STEP_SUCCESS));
	else
		mmi_ucs2cpy((CHAR *)subMenuData[2], GetString(STR_ID_FM_SENSOR_CHECK_STEP_FAIL));
	subMenuDataPtrs[2] = subMenuData[2];
	(*MenuItemNum)++;
	
	if((g_kjx_sensor_shake > 0)&&(g_kjx_sensro_step > 0))
	{
		lsk_str_id = STR_GLOBAL_OK;
	}
	else
	{
		...
		{
			StartTimer(KJX_TIMER_ID_FACTORY_TEST, 500, mmi_fm_update_sensor_hdlr);
		}
	}
	...
	ShowCategory6Screen(
			...
			rsk_str_id,
			0,
			g_fm_contxt->sensor_contxt.gFM_MenuItemNum,
			(PU8*)subMenuDataPtrs,
			NULL,
			0,
			NULL);

	SetRightSoftkeyFunction(mmi_fm_stop_sensor, KEY_EVENT_UP);	
	if(lsk_str_id != NULL)
		SetLeftSoftkeyFunction(mmi_fm_stop_sensor, KEY_EVENT_UP);
}

/*****************************************************************************
 * FUNCTION
 *  mmi_fm_prepare_sensor_list
 * DESCRIPTION
 *  
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_prepare_sensor_list(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_exit_sensor_menu(void)
{
    sensor_time_count = 0;
	StopTimer(KJX_TIMER_ID_FACTORY_TEST);
}
 ...
 *  void
 *****************************************************************************/
static void mmi_fm_enter_sensor_menu(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
extern void mmi_fm_select_sensor(void)
{
	sensor_time_count = 0;
	g_kjx_sensor_shake = 0;
	g_kjx_sensro_step = 0;
...
U8 call_count = 0;
static void mmi_fm_call_proc(void)
{
	BOOL is_emergency_call = FALSE;
	S8 mynuber1[] = {"10086"};
	S8 mynuber2[] = {"112"};
    U8 ucs_nuber[41];
	...
	if(mmi_bootup_is_sim_valid())
    {
    	...
    	mmi_asc_to_ucs2((S8*)ucs_nuber,(S8*)mynuber1);
	}
	else
	{
		is_emergency_call = TRUE;
		mmi_asc_to_ucs2((S8*)ucs_nuber,(S8*)mynuber2);
	}
	
    MakeCall((S8*)ucs_nuber);

	if(is_emergency_call != FALSE)
	{
		//xb add 2017-06-16 防止信号不好的时候只拨一次就断了
		call_count++;
		if(call_count < 3)
		{
			StartTimer(KJX_TIMER_ID_FACTORY_TEST, 10*1000, mmi_fm_call_proc);
		}
	}
}

extern void mmi_fm_select_call(void)
{
	call_count = 0;
	mmi_fm_call_proc();
}
