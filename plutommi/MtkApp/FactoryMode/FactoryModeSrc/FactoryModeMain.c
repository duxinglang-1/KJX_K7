...
const mmi_fm_test_item_struct g_fm_test_item_tab[] =
{
    ...
    /* Version */
#ifdef __KJX_FUN__
	{MENU_ID_FM_IMEI,                 mmi_fm_set_lsk_null,                mmi_fm_version_menu_item_highlight},
#else
...
#ifndef __KJX_FUN__		//xb add 2018-08-06 屏蔽扬声器测试项
    /* Loud spk */
...
#ifndef __KJX_FUN__	//xb add 2018-08-06 屏蔽听筒测试项
    /* Receiver */
  ...
  #ifdef __KJX_BAT_STATUS_LED_SHOW__
  	/*Bat Led*/
	{MENU_ID_FM_BAT_LED,				(mmi_fm_select_func)mmi_fm_select_bat_led,				(mmi_fm_highlight_func)mmi_fm_highlight_bat_led},
  #endif
  #ifdef __KJX_FUN_GPS__
  	/*GPS*/
	{MENU_ID_FM_GPS,					(mmi_fm_select_func)mmi_fm_select_gps,					mmi_fm_default_func},
  #endif
  #ifdef __KJX_RFID__
  	/*RFID*/
	{MENU_ID_FM_RFID,					(mmi_fm_select_func)mmi_fm_select_rfid,					mmi_fm_default_func},
  #endif
  #ifdef __BLE_MODULE_SUPPORT__
  	/*BLE*/
	{MENU_ID_FM_BLE, 				  	(mmi_fm_select_func)mmi_fm_select_ble,			  		mmi_fm_default_func},
  #endif
  #ifdef __BOSCH_BMP280_PRESSURE__
  	/*PRESSURE*/
	{MENU_ID_FM_PRESSURE,				(mmi_fm_select_func)mmi_fm_select_pressure,				mmi_fm_default_func},
  #endif
  #ifdef __MOTION_SENSOR_MC3451_SUPPORT__
	/*G-sensor*/
	{MENU_ID_FM_SENSOR,					(mmi_fm_select_func)mmi_fm_select_sensor,				mmi_fm_default_func},
  #endif
	{MENU_ID_FM_CALL,					(mmi_fm_select_func)mmi_fm_select_call,					mmi_fm_default_func},  
#else
    /* RTC */
...
extern void DeinitFactoryModeEvent(void)
{
    ...
    {
        mmi_frm_set_protocol_event_handler(eventFMIDs[index], 
	                                    (PsIntFuncPtr)DummyFMCBHandler, MMI_FALSE);
    }
}
...
void InitFactoryModeEvent(void)
{
	...
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EM_KEYPAD_EVENT_ACT_IND, 
	                                    (PsIntFuncPtr)mmi_fm_at_keypad_test_hdlr, MMI_FALSE);
    #ifndef __MMI_MAINLCD_96X64__
	mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EM_LCM_TEST_IND, 
	                                    (PsIntFuncPtr)mmi_fm_at_lcd_test_msg_hdlr, MMI_FALSE);
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EM_RGB_TEST_REQ_IND,
                                        (PsIntFuncPtr)mmi_fm_at_lcm_rgb_test_handler, MMI_FALSE);
...
#ifdef __KJX_FUN__
	if(!kjx_is_in_call())	//xb add 2017-08-30 通话的时候会强制退出工厂测试模式，但是实际上还处于测试模式下
	{
...
extern void InitFactoryMode(void)
{
	...
	/* Code Body													  */
	/*----------------------------------------------------------------*/
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_FACTORY_TEST_IND, 
	                                   (PsIntFuncPtr)mmi_fm_at_entry_menu_hdlr, MMI_FALSE);
#if (defined(__MSDC_MS__) || defined(__MSDC_SD_MMC__) || defined(__MSDC_MSPRO__))
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_FMT_GET_MSDC_STATUS_RSP, 
									   (PsIntFuncPtr)FM_MemoryCardResponseHandler, KAL_FALSE);
#endif
    mmi_frm_set_protocol_event_handler(MSG_ID_MMI_EM_QUICK_TEST_REPORT_IND,
	                                   (PsIntFuncPtr)mmi_fm_quick_test_report, KAL_FALSE);
    MMI_FM_DCM_LOAD();
