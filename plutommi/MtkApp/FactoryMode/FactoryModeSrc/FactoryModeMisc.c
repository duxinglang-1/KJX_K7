...
#ifdef __KJX_FUN__
	/* IMEI */
	ver_struct.imei = (kal_char*)IMEIBuffer;
	mmi_fm_get_version_str((CHAR*)ver_struct.imei,index);
...
extern void mmi_fm_select_version_summary_menu(void)
{
	...
	str[1] = (CHAR*)IMEIBuffer;
	str[2] = NULL;
	str[3] = (CHAR*)build_date_time();
...
extern void mmi_fm_quick_test_report(void)
{
    ...
    ReadRecord(NVRAM_EF_QUICKTEST_LID, 2, &testdata, NVRAM_EF_QUICKTEST_SIZE, &ErrorCode);
    msg = OslConstructDataPtr(sizeof(mmi_em_quick_test_report_res_req_struct));

    msg->result = (ErrorCode == NVRAM_READ_SUCCESS)?KAL_TRUE:KAL_FALSE;
