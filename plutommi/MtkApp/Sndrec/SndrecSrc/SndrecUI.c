...
static void mmi_sndrec_entry_setting_screen(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_frm_group_create_ex(GRP_ID_SNDREC_MAIN,GRP_ID_SNDREC_SETTING,mmi_sndrec_entry_setting_screen_inline_cui_proc,NULL,MMI_FRM_NODE_SMART_CLOSE_FLAG);
...
MMI_BOOL mmi_sndrec_auto_record_is_on(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ReadValue(NVRAM_AUTO_VM_SETTING_AUTO_VM, &auto_record, DS_BYTE, &error);
 
    if (auto_record != 0 && MMI_TRUE == mdi_audio_is_recording())
    {
        MMI_SNDREC_MSG_TRACE(g_sndrec_cntx.record.state);
        mmi_popup_display_ext(STR_ID_SNDREC_AUTO_RECORD_ON, MMI_EVENT_WARNING, NULL);
        return MMI_TRUE;
    }
 #endif
    return MMI_FALSE;
