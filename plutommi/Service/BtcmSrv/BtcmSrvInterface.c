...
const U16 profile_id_to_connect_type_map[SRV_BT_CM_NUM_OF_PROFILE_UUID][2] = 
{
    ...
    {SRV_BT_CM_AUDIO_SINK_UUID, SRV_BT_CM_NO_CONNECTION},
...
S32 srv_bt_cm_set_limit_vis(MMI_BOOL vis)
{
    ...
    if (g_srv_bt_cm_cntx.state >= SRV_BT_CM_STATE_IDLE)
    {
        ...
        if (vis)
        {
            SRV_BT_CM_STATE_TRANS(SRV_BT_CM_STATE_SETTING_LIMIT_VISIBILITY);
        ...
        {
            SRV_BT_CM_STATE_TRANS(SRV_BT_CM_STATE_RESETTING_LIMIT_VISIBILITY);
...
void srv_bt_cm_activate_cnf_hdler(U32 profile_id)
{
    ...
    switch (g_srv_bt_cm_cntx.state)
    {
        case SRV_BT_CM_STATE_ACTIVATING:
            ...
            {
                //StopTimer(BT_ACTIVATE_PROFILE_TIMER);
                //SRV_BT_CM_STATE_TRANS(SRV_BT_CM_STATE_ACTIVATING_WRITE_SCAN_ENABLE);
...
/*****************************************************************************
 * FUNCTION
 *  srv_bt_cm_sco_disconnect_cnf_hdler
 * DESCRIPTION
 *  This function is to handle SCO disconnect confirm
 * PARAMETERS
 ...
 *  void
 *****************************************************************************/
/* void srv_bt_cm_sco_disconnect_cnf_hdler(U8 result, U32 connection_id) */
/* { */
/*     /\*----------------------------------------------------------------*\/ */
/*     /\* Local Variables                                                *\/ */
/*     /\*----------------------------------------------------------------*\/ */
...
/*     /\* Code Body                                                      *\/ */
/*     /\*----------------------------------------------------------------*\/ */
/*     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_SRV_BT_CM_AUDIO_DISCON_CNF, */
/*                result, g_srv_bt_cm_cntx.state, (~result)); */
/*     for (i = 0; i < g_srv_bt_cm_cntx.existed_conn_num; i++) */
/*     { */
/*         if (g_srv_bt_cm_cntx.existed_conn[i].conn_id == connection_id) */
/*         { */
/*             break; */
/*         } */
/*     } */
/*     /\* cannot find this connection *\/ */
/*     if (i == g_srv_bt_cm_cntx.existed_conn_num) */
/*     { */
/*         if (!result) */
/*         { */
/*             SRV_BT_CM_RESET_RAMFLAG(SRV_BT_CM_RAMMASK_IS_SWAP2BT); */
/*             if (g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_sco_ind) */
/*             { */
/*                 (*g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_sco_ind)(MMI_FALSE); */
/*             } */
/*         } */
/*         else */
/*         { */
/*             if (g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_sco_ind) */
/*             { */
/*                 (*g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_sco_ind)(MMI_TRUE); */
/*             } */
/*         } */
/*         return; */
/*     } */
...
/*     { */
/*         SRV_BT_CM_RESET_RAMFLAG(SRV_BT_CM_RAMMASK_IS_SWAP2BT); */
/*         g_srv_bt_cm_cntx.existed_conn[i].audio_on = MMI_FALSE; */
/*     } */
/*     switch (g_srv_bt_cm_cntx.state) */
/*     { */
/*         case SRV_BT_CM_STATE_RELEASE_ALL_CONNECTION: */
/*             /\* release connection again *\/ */
/*             if (srv_bt_cm_release_all_conn(NULL) != SRV_BT_CM_RESULT_SUCCESS) */
/*             { */
/*                 /\* all connections are released *\/ */
/*                 if (g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_release_all_conn) */
/*                 { */
/*                     (*g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_release_all_conn)(MMI_TRUE); */
/*                 } */
/*                 SRV_BT_CM_STATE_TRANS(SRV_BT_CM_STATE_IDLE); */
/*             } */
/*             break; */
/*         case SRV_BT_CM_STATE_RELEASE_DEV_CONNECTION: */
/*             /\* release connection again *\/ */
/*             if (srv_bt_cm_release_dev_conn(&(g_srv_bt_cm_cntx.paired_list[g_srv_bt_cm_cntx.action_index].bd_addr), NULL) */
/*                 != SRV_BT_CM_RESULT_SUCCESS) */
/*             { */
/*                 /\* all connections are released *\/ */
/*                 if (g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_release_all_conn) */
...
/*                 if (g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_sco_ind) */
/*                 { */
/*                     (*g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_sco_ind)(g_srv_bt_cm_cntx.existed_conn[i].audio_on); */
/*                 } */
/*             } */
/*             else */
/*             { */
...
// #ifdef __DM_LAWMO_SUPPORT__
//     /* register LAWMO notify callback */
//     if (mmi_dmui_lawmo_register_notify(
//             MMI_DMUI_APP_ID_BLUETOOTH,
//             MMI_DMUI_MO_TYPE_LAWMO_LOCK, 
//             (mmi_dmui_lawmo_cb)srv_bt_cm_lawmo_cb_hdlr) < 0)
//     {
//         MMI_ASSERT(0);
//     }
// #endif /*__DM_LAWMO_SUPPORT__*/
...
/*****************************************************************************
 * FUNCTION
 *  srv_bt_cm_hfp_sco_connect
 * DESCRIPTION
 *  This function is to process SCO connection request and invoke mmi_bt_sco_open_stream_req
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
/* void srv_bt_cm_hfp_sco_connect(void) */
/* { */
/*     /\*----------------------------------------------------------------*\/ */
/*     /\* Local Variables                                                *\/ */
/*     /\*----------------------------------------------------------------*\/ */
...
/*****************************************************************************
 * FUNCTION
 *  srv_bt_cm_hfp_sco_disconnect
 * DESCRIPTION
 *  This function is to process HFP SCO disconnect request
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
/* void srv_bt_cm_hfp_sco_disconnect(void) */
/* { */
/*     /\*----------------------------------------------------------------*\/ */
/*     /\* Local Variables                                                *\/ */
/*     /\*----------------------------------------------------------------*\/ */
...
/*               MMI_FALSE, */
/*               g_srv_bt_cm_cntx.state); */
/*     if (srv_bt_cm_get_sco_status(srv_bt_cm_get_active_hfp_connect_id())) */
/*     { */
/* #if defined(__MMI_BT_PROFILE__) */
...
/*****************************************************************************
 * FUNCTION
 *  srv_bt_cm_connect_last_headset
 * DESCRIPTION
 *  This function is to process connect last headset request
 * PARAMETERS
 *  void
 * RETURNS
 *  MMI_BOOL        return TRUE
 *****************************************************************************/
/* MMI_BOOL srv_bt_cm_connect_last_headset(void) */
/* { */
/*     /\*----------------------------------------------------------------*\/ */
/*     /\* Local Variables                                                *\/ */
/*     /\*----------------------------------------------------------------*\/ */
...
/*     /\* Code Body                                                      *\/ */
/*     /\*----------------------------------------------------------------*\/ */
/*     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_SRV_BT_CM_CONNECT_LAST_HEADSET_REQ); */
/*     paired_index = g_srv_bt_cm_cntx.idx_of_last_hs_dev; */
/*     if (paired_index == 0xFFFFFFFF) */
/*     { */
/*         if (g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_display_popup) */
/*         { */
/*             (*g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_display_popup)((PU8) GetString(STR_BT_NO_DEFAULT_HEADSET), */
/*                                                                       MMI_EVENT_FAILURE); */
/*         } */
/*          return result; */
/*     } */
...
/*     } */
/*     result = srv_bt_cm_connect_paired_headset_with_index(paired_index, SRV_BT_CM_HS_SHOW_CONNECT_SCR); */
/*     return result; */
/* } */
 ...
 *****************************************************************************/
// MMI_BOOL srv_bt_cm_switch_voice_path_incall(MMI_BOOL is_on)
// {
//     /*----------------------------------------------------------------*/
//     /* Local Variables                                                */
//     /*----------------------------------------------------------------*/
...
//         if (g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_display_popup)
//         {
//             (*g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_display_popup)(
...
//         {
// #if defined(__MMI_BT_PROFILE__)
//             if (mmi_bt_sco_is_sco_connect_allowed())
//             {
//                 if (mmi_bt_sco_open_stream_req())
//                 {
//                     if (g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_show_progress_scr)
//                     {
//                         (*g_srv_bt_cm_cntx.scr_cb_tbl.mmi_bt_scr_cb_show_progress_scr)(
...
//             else if (!(g_srv_bt_cm_cntx.deactivate_flag & SRV_BT_CM_SIMAP_DEACTIVATED) &&
//                      g_srv_bt_cm_profile_tbl[i].uuid == SRV_BT_CM_SIM_ACCESS_PROFILE_UUID)
//             {
//                 #ifdef __MMI_SIMAP_SUPPORT__
...
/*****************************************************************************
 * FUNCTION
 *  srv_bt_cm_restart_bt_module_req
 * DESCRIPTION
 *  This function is to process restart BT module request and send MSG_ID_BT_RESTART_REQ
 * PARAMETERS
 *  void
 * RETURNS
 *  MMI_BOOL    return TRUE
 *****************************************************************************/
/* MMI_BOOL srv_bt_cm_restart_bt_module_req(void) */
/* { */
/*     /\*----------------------------------------------------------------*\/ */
/*     /\* Local Variables                                                *\/ */
/*     /\*----------------------------------------------------------------*\/ */
...
/*     srv_bt_cm_send_msg(MOD_BT, BT_APP_SAP, MSG_ID_BT_RESTART_REQ, NULL, NULL); */
/*     return MMI_TRUE; */
/* } */
...
/*****************************************************************************
 * FUNCTION
 *  srv_bt_cm_disallow_acl_link_establishment_req_hdler
 * DESCRIPTION
 *  This function is to handle disallow ACL link establishment request and send  MSG_ID_BT_BM_LINK_DISALLOW_REQ
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
/* static void srv_bt_cm_disallow_acl_link_establishment_req_hdler(void) */
/* { */
/*     /\*----------------------------------------------------------------*\/ */
/*     /\* Local Variables                                                *\/ */
/*     /\*----------------------------------------------------------------*\/ */
...
srv_bt_cm_dev_struct* srv_bt_cm_get_paired_dev_info(U16 index)
{
    ...
    // ASSERT((g_srv_bt_cm_cntx.dev_list[idx].dev_info.bd_addr.lap != 0) ||
    //        (g_srv_bt_cm_cntx.dev_list[idx].dev_info.bd_addr.uap != 0) ||
    //        (g_srv_bt_cm_cntx.dev_list[idx].dev_info.bd_addr.nap != 0));
...
/*****************************************************************************
 * FUNCTION
 *  srv_bt_cm_set_default_headset_index
 * DESCRIPTION
 *  This function is to set default headset index
 * PARAMETERS
 *  paired_index            paired device index
 * RETURNS
 *  MMI_BOOL        TRUE : if the paired index is less than paired device number
                    FALSE : the paired index is larger than paired device number
 *****************************************************************************/
/* MMI_BOOL srv_bt_cm_set_default_headset_index(U32 paired_index) */
/* { */
/*     /\*----------------------------------------------------------------*\/ */
/*     /\* Local Variables                                                *\/ */
/*     /\*----------------------------------------------------------------*\/ */
...
/*     /\* Code Body                                                      *\/ */
/*     /\*----------------------------------------------------------------*\/ */
/*     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_SRV_BT_CM_SET_DEFAULT_HD_INDEX, paired_index); */
/*     if (paired_index < g_srv_bt_cm_cntx.paired_dev_num) */
/*     { */
/* #ifdef __MMI_HFP_SUPPORT__ */
/*         srv_bt_cm_dev_struct* dev = srv_bt_cm_get_paired_dev_info(paired_index); */
/*         srv_bthf_save_config(BTHF_CFG_DEF_DEVICE, &dev->bd_addr); */
/*         result = MMI_TRUE; */
/* #endif  /\* __MMI_HFP_SUPPORT__ *\/ */
/*     } */
 ...
 *  srv_bt_cm_set_state
 * DESCRIPTION
 *  This function is to set state
 * PARAMETERS
 *  set_state       [IN]            state value
 * RETURNS
 *  void
 *****************************************************************************/
/* void srv_bt_cm_set_state(U32 set_state) */
/* { */
/*     /\*----------------------------------------------------------------*\/ */
/*     /\* Local Variables                                                *\/ */
/*     /\*----------------------------------------------------------------*\/ */
...
/*     /\* Code Body                                                      *\/ */
/*     /\*----------------------------------------------------------------*\/ */
/*     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_SRV_BT_CM_SET_STATE, set_state); */
/*     SRV_BT_CM_STATE_TRANS(set_state); */
/* } */
...
// static void srv_bt_cm_switch_on_timeout_hdlr(void)
// {
//     MMI_TRACE(
//         MMI_CONN_TRC_G7_BT,
//         SRV_BT_CM_SWITCH_ON_TIMEOUT_HDLR);
//     SRV_BT_CM_SET_FLAG(SRV_BT_CM_MASK_POWER_ON_TIME_OUT);
//     SRV_BT_CM_STATE_TRANS(SRV_BT_CM_STATE_DEACTIVATING);
//     srv_bt_cm_antenna_off();
// }
 ...
 *  srv_bt_cm_activate
 * DESCRIPTION
 *  This function is to process activate request and invoke srv_bt_cm_read_local_bd_addr_req_hdler
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
/* void srv_bt_cm_activate(void) */
/* { */
/*     /\*----------------------------------------------------------------*\/ */
/*     /\* Local Variables                                                *\/ */
/*     /\*----------------------------------------------------------------*\/ */
...
/*     /\* Code Body                                                      *\/ */
/*     /\*----------------------------------------------------------------*\/ */
/*     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_SRV_BT_CM_ACTIVATE_REQ); */
/*     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_SRV_BT_CM_DEACTIVATE_FLAG, g_srv_bt_cm_cntx.deactivate_flag); */
 ...
 *  srv_bt_cm_set_simap
 * DESCRIPTION
 *  This function is to process setting SIMAP request
 * PARAMETERS
 *  activate        [IN]            activate or deactivated
 * RETURNS
 *  void
 *****************************************************************************/
// S32 srv_bt_cm_set_simap(MMI_BOOL activate)
// {
//     /*----------------------------------------------------------------*/
//     /* Local Variables                                                */
//     /*----------------------------------------------------------------*/
...
//     /* Code Body                                                      */
//     /*----------------------------------------------------------------*/
//     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_SRV_BT_CM_SET_SIMAP_REQ, activate);
//     if (g_srv_bt_cm_cntx.state >= SRV_BT_CM_STATE_IDLE)
//     {
//         if (activate)
//         {
//             SRV_BT_CM_SET_RAMFLAG(SRV_BT_CM_RAMMASK_IS_SIMAP_ACTIVATE);
//             SRV_BT_CM_STATE_TRANS(SRV_BT_CM_STATE_ACTIVATING_SIMAP);
//             /* disallow acl link establishment */
//             /* ->simap activate */
//             srv_bt_cm_send_msg(MOD_BT, BT_APP_SAP, MSG_ID_BT_BM_LINK_DISALLOW_REQ, NULL, NULL);
//             /* deal with the response at srv_bt_cm_disallow_acl_link_establishment_cnf_hdler() */
...
//                 g_srv_bt_cm_cntx.deactivate_flag);
//             SRV_BT_CM_RESET_RAMFLAG(SRV_BT_CM_RAMMASK_IS_SIMAP_ACTIVATE);
//             SRV_BT_CM_STATE_TRANS(SRV_BT_CM_STATE_DEACTIVATING_SIMAP);
//             /* simap deactivate */
//             /* -> allow acl link establishment */
// #ifdef __MMI_SIMAP_SUPPORT__
//             {
...
/*****************************************************************************
 * FUNCTION
 *  srv_bt_cm_set_audio_path_to_headset
 * DESCRIPTION
 *  This function is to set audio path to headset 
 * PARAMETERS
 *  pth_2_hg        [IN]            
 * RETURNS
 *  void
 *****************************************************************************/
/* void srv_bt_cm_set_audio_path_to_headset(MMI_BOOL pth_2_hg) */
/* { */
/*     /\*----------------------------------------------------------------*\/ */
/*     /\* Local Variables                                                *\/ */
/*     /\*----------------------------------------------------------------*\/ */
...
/*     /\* Code Body                                                      *\/ */
/*     /\*----------------------------------------------------------------*\/ */
/*     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_SRV_BT_CM_SET_AUD_PTH_2_HF, pth_2_hg); */
/* #ifdef __MMI_HFP_SUPPORT__ */
/*     if (pth_2_hg) */
/*     { */
...
static void srv_bt_cm_nvram_read_info(void)
{
    ...
    ReadRecord(NVRAM_EF_BT_SYS_INFO_LID, 1, &sys_read_struct, NVRAM_EF_BT_SYS_INFO_SIZE, &err);
    if (err == NVRAM_READ_SUCCESS)
    {
...
void srv_bt_cm_nvram_write_info(U16 LID, void* para)
{
    ...
    switch (LID)
    {
        case NVRAM_EF_BT_INFO_LID:
        {
            ...
            MMI_BT_ASSERT(err == NVRAM_WRITE_SUCCESS);
            break;
        }
        ...
        case NVRAM_EF_BT_SYS_INFO_LID:
        {
            ...
            MMI_TRACE(
                ...
                g_srv_bt_cm_cntx.paired_dev_num);
            WriteRecord(NVRAM_EF_BT_SYS_INFO_LID, 1, &sys_write_struct, NVRAM_EF_BT_SYS_INFO_SIZE, &err);
            MMI_BT_ASSERT(err == NVRAM_WRITE_SUCCESS);
            break;
        }
...
/*****************************************************************************
 * FUNCTION
 *  srv_bt_cm_get_ram_flag
 * DESCRIPTION
 *  This function is to get ram flag value
 * PARAMETERS
 *  get_flag        [IN]            the ram flag
 * RETURNS
 *  U32         ram flag value
 *****************************************************************************/
// U32 srv_bt_cm_get_ram_flag(U32 get_flag)
// {
//     /*----------------------------------------------------------------*/
//     /* Local Variables                                                */
//     /*----------------------------------------------------------------*/
...
//     /* Code Body                                                      */
//     /*----------------------------------------------------------------*/
//     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_SRV_BT_CM_GET_RAMFLAG, get_flag, SRV_BT_CM_GET_RAMFLAG(get_flag));
//     return SRV_BT_CM_GET_RAMFLAG(get_flag);
// }


/*****************************************************************************
 * FUNCTION
 *  srv_bt_cm_set_ram_flag
 * DESCRIPTION
 *  This function is to set ram flag
 * PARAMETERS
 *  set_flag        [IN]            flag value
 * RETURNS
 *  void
 *****************************************************************************/
/* void srv_bt_cm_set_ram_flag(U32 set_flag) */
/* { */
/*     /\*----------------------------------------------------------------*\/ */
/*     /\* Local Variables                                                *\/ */
/*     /\*----------------------------------------------------------------*\/ */
...
/*     /\* Code Body                                                      *\/ */
/*     /\*----------------------------------------------------------------*\/ */
/*     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_SRV_BT_CM_SET_RAMFLAG, set_flag); */
/*     SRV_BT_CM_SET_RAMFLAG(set_flag); */
/* } */


/*****************************************************************************
 * FUNCTION
 *  srv_bt_cm_reset_ram_flag
 * DESCRIPTION
 *  This function is to reset ram flag
 * PARAMETERS
 *  set_flag        [IN]            ram flag
 * RETURNS
 *  void
 *****************************************************************************/
/* void srv_bt_cm_reset_ram_flag(U32 set_flag) */
/* { */
/*     /\*----------------------------------------------------------------*\/ */
/*     /\* Local Variables                                                *\/ */
/*     /\*----------------------------------------------------------------*\/ */
...
/*     /\* Code Body                                                      *\/ */
/*     /\*----------------------------------------------------------------*\/ */
/*     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_SRV_BT_CM_RESET_RAMFLAG, set_flag); */
/*     SRV_BT_CM_RESET_RAMFLAG(set_flag); */
/* } */
 ...
 *  void
 *****************************************************************************/
// void srv_bt_cm_set_flag(U32 set_flag)
// {
//     /*----------------------------------------------------------------*/
//     /* Local Variables                                                */
//     /*----------------------------------------------------------------*/
...
//     /* Code Body                                                      */
//     /*----------------------------------------------------------------*/
//     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_SRV_BT_CM_SET_FLAG, set_flag);
//     SRV_BT_CM_SET_FLAG(set_flag);
// }


/*****************************************************************************
 * FUNCTION
 *  srv_bt_cm_reset_flag
 * DESCRIPTION
 *  This function is to reset flag
 * PARAMETERS
 *  set_flag        [IN]            flag
 * RETURNS
 *  void
 *****************************************************************************/
// void srv_bt_cm_reset_flag(U32 set_flag)
// {
//     /*----------------------------------------------------------------*/
//     /* Local Variables                                                */
//     /*----------------------------------------------------------------*/
...
//     /* Code Body                                                      */
//     /*----------------------------------------------------------------*/
//     MMI_TRACE(MMI_CONN_TRC_G7_BT, MMI_BT_G7_SRV_BT_CM_RESET_FLAG, set_flag);
//     SRV_BT_CM_RESET_FLAG(set_flag);
// }
