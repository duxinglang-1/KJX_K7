...
/* You tube PGDL Support */
				...
				if (!(S32)(applib_parse_url((CHAR*) msg->url, &url_info)))
				{
    ...
    MMI_TRACE(MMI_DA_TRACE_GROUP, MMI_DA_TRC_RECV_FILE_DL_IND, msg->session_id);
				...
				if (!(S32)(applib_parse_url((CHAR*) msg->url, &url_info)))
				{
					...
					if (is_substr)
					{
						...
						is_yt = MMI_TRUE;
						srv_da_send_wap_file_dl_res(msg->session_id, WAP_DL_ERROR_CANCELLED, NULL);
						dest_url = (WCHAR*)OslMalloc(2*(WAP_DL_MAX_URL_LEN +1));
						//memset(dest_url, 0, sizeof(WAP_DL_MAX_URL_LEN));
						mmi_asc_to_wcs(dest_url, msg->url);
						da_info.url = dest_url;
						da_info.show_confirm = MMI_FALSE;
						da_info.filepath = NULL;
						da_info.file_length = msg->file_length;
						srv_da_start_http_download(&da_info);
						OslMfree(dest_url);
						return;
