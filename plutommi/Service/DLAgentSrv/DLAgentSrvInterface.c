...
extern const srv_da_module_funcptr_table srv_da_yt_dl_module;
#endif
#endif
#if defined(BROWSER_SUPPORT)
...
srv_da_scr_select_storage_cause_enum srv_da_decide_location_auto (srv_da_storage_parameter_struct *para)
{
...
srv_da_scr_select_storage_cause_enum srv_da_decide_filepath_auto (U32 arg, srv_da_storage_parameter_struct *para, srv_da_action_enum job_type, srv_da_scr_storage_callback_type callback)
{
	...
	storage_error = srv_da_decide_location_auto (para);
...
void srv_da_resume_all_jobs (void)
{
    ...
    while (job != NULL)
    {
        if(((job->state == SRV_DA_JOB_STATE_PAUSED) ||
            (job->state == SRV_DA_JOB_STATE_FAILED)) || (job->state != SRV_DA_JOB_STATE_RESUMING)) 
        {
            srv_da_job_resume (job);
        }
        job = job->next_job;
