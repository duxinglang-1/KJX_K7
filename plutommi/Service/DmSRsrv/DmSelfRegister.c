...
#ifdef __MMI_DUAL_SIM__
static int dm_self_sim1_repeat_process_time = 10;
static int dm_self_sim2_repeat_process_time = 10;
#endif
...
#ifdef __MMI_DUAL_SIM__
static int dm_self_sim2_imsi_repeat_time = 3;
static S8 SIM2_IMSI[DM_SR_IMSI_SIZE];
//#define DM_SELF_SIM1 0
//#define DM_SELF_SIM2 1
static U8 dm_self_first_sim = 0;
static U8 mmi_dm_sr_check_last_reg_sim_state(void);
static void mmi_dm_sr_sim2_process_init(void);
static U8 dm_self_reg_sim = 0;
static U8 dm_self_first_network2_ready = 0;
static MMI_BOOL dm_self_start_flag2 = MMI_FALSE;
static void mmi_dm_sr_get_sim2_imsi(void);
static void mmi_dm_sr_repeat_get_sim2_imsi(void);
static void mmi_dm_sr_sim2_imsi_process(void);
...
{    
    ...
    if (sim == 1)
    {
	    MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_NETWORK_READY_NOTIFY_SIM2);
        if ((dm_self_first_network2_ready != 0) && (dm_self_reg_info.is_self_reg_succuss == TRUE))
        {
            MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_NETWORK_READY_NO_FIRST_SIM2);
	        return;
        }
        dm_self_first_network2_ready = 1;

	    if (srv_sms_is_sms_ready() == MMI_TRUE)
	    {
            MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_NETWORK_READY_START_SIM2);
            mmi_dm_sr_sim2_process_init();
        }
    }
#endif /* __MMI_DUAL_SIM__ */
}
...
#ifdef __MMI_DUAL_SIM__
    if (dm_self_start_flag2 == MMI_TRUE)
    {
        mmi_dm_sr_read_nvram();
        if ((dm_self_reg_info.is_self_reg_succuss != TRUE) && (dm_self_sim1_repeat_process_time > 0))
        {
            StartTimer(DM_SR_REPEAT_TIMER_ID, DM_SR_REPEAT_SIM_PROCESS_TIME, mmi_dm_sr_sim_process_init);
            dm_self_sim1_repeat_process_time --;
        }
    }
    else
    {
...
#ifdef __MMI_DUAL_SIM__
            dm_self_reg_sim = MMI_SIM1;
            stat = mmi_dm_sr_check_last_reg_sim_state();
            if (stat == FALSE) 	
            {
#endif /* __MMI_DUAL_SIM__ */        
	            mmi_dm_sr_get_imsi();
...
{    
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_SIM2_PROCESS_INIT);
    if (dm_self_start_flag == MMI_TRUE)
    {   
        mmi_dm_sr_read_nvram();
        
        if ((dm_self_reg_info.is_self_reg_succuss != TRUE) && (dm_self_sim2_repeat_process_time > 0))
        {
            
            StartTimer(DM_SR_REPEAT_TIMER_ID, DM_SR_REPEAT_SIM_PROCESS_TIME, mmi_dm_sr_sim2_process_init);
            dm_self_sim2_repeat_process_time --;
        }
    }
    else
    {
        if (dm_self_start_flag2 == MMI_FALSE)
        {   
            dm_self_start_flag2 = MMI_TRUE;
            MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_SIM2_PROCESS_INIT_START);

            memset(&dm_self_reg_info, 0, sizeof(mmi_dm_sr_info_struct));
            dm_self_reg_info.repeat_send_sms_times = DM_SR_REPEAT_TIMES;
            dm_self_reg_sim = MMI_SIM2;
            stat = mmi_dm_sr_check_last_reg_sim_state();
	        if (stat == FALSE) 	
            {
	            mmi_dm_sr_get_sim2_imsi();
            }
	        else
	        {
                mmi_dm_sr_read_nvram();
...
/*****************************************************************************
 * FUNCTION
 * mmi_dm_sr_check_last_reg_sim_state
 * DESCRIPTION
 *  check last register's state 
 * PARAMETERS
 *  void
 * RETURNS
 *  void
...
/* under construction !*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_CHECK_LAST_REG);
...
#ifdef __OP01__    
    if(ReadRecord(NVRAM_EF_DMSR_SIM_IMSI_LID, 1, imsi, DM_SR_IMSI_SIZE, &error_code) >= 0)
    {
        status = srv_sim_ctrl_get_imsi(MMI_SIM1, SIM1_IMSI, DM_SR_IMSI_SIZE);
        //kal_prompt_trace(MOD_MMI,"mmi_dm_sr_check_last_reg_sim_state_1 IMSI_1 = %s", SIM1_IMSI);
        if(status)
        {
            if(memcmp(imsi, SIM1_IMSI, DM_SR_IMSI_SIZE) != 0)
            {                   
                status = srv_sim_ctrl_get_imsi(MMI_SIM2, SIM2_IMSI, DM_SR_IMSI_SIZE);
                //kal_prompt_trace(MOD_MMI,"mmi_dm_sr_check_last_reg_sim_state_2 IMSI_2 = %s", SIM2_IMSI);
                if(status)
                {
                    if(memcmp(imsi, SIM2_IMSI, DM_SR_IMSI_SIZE) != 0)
                    {
                        //kal_prompt_trace(MOD_MMI,"mmi_dm_sr_check_last_reg_sim_state_3 it need to register");
                        stat = FALSE;
                        goto out;
                    }
                }
        ...
        {
            status = srv_sim_ctrl_get_imsi(MMI_SIM2, SIM2_IMSI, DM_SR_IMSI_SIZE);
            //kal_prompt_trace(MOD_MMI,"mmi_dm_sr_check_last_reg_sim_state_4 IMSI_2 = %s", SIM1_IMSI);
            if(status)
            {
                if(memcmp(imsi, SIM2_IMSI, DM_SR_IMSI_SIZE) != 0)
                {   
                    //kal_prompt_trace(MOD_MMI,"mmi_dm_sr_check_last_reg_sim_state_5 it need to register");
                    stat = FALSE;
                    goto out;
                }
            }
...
#else
    if (srv_sim_ctrl_is_available(dm_self_reg_info.sim))
    {
//        MMI_PRINT(MOD_MMI_CONN_APP, MMI_CONN_TRC_G3_DM, "[DM SR][Function] mmi_dm_sr_check_last_reg_sim_state SIM %d is available", dm_self_reg_info.sim);
    }
    else
    {
        stat = FALSE;
    ...
    if ((srv_sim_ctrl_get_change_check(dm_self_reg_info.sim) == SRV_SIM_CTRL_CHANGE_CHECK_THE_SAME) &&
         srv_nw_usab_is_usable(dm_self_reg_info.sim))//add by yuxiong
        
    {
//        MMI_PRINT(MOD_MMI_CONN_APP, MMI_CONN_TRC_G3_DM, "[DM SR][Function] mmi_dm_sr_check_last_reg_sim_state SIM %d is not changed", dm_self_reg_info.sim);
    }
    else
    {
        stat = FALSE;
...
out:
    MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_CHECK_LAST_REG_RESULT, stat);
//    memset(&dm_self_reg_info, 0, sizeof(mmi_dm_sr_info_struct));
    return stat;
}
...
static void mmi_dm_sr_sim2_imsi_process(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
//    MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_SIM2_IMSI_PROCESS);
    
    memcpy(dm_self_reg_info.IMSI_number, SIM2_IMSI, DM_SR_IMSI_SIZE);
	// MMI_PRINT(MOD_MMI_CONN_APP, MMI_CONN_TRC_G3_DM, "[DM SR][Function] mmi_dm_sr_sim2_imsi_process dm_self_reg_info.IMSI_number = %s", dm_self_reg_info.IMSI_number);
    mmi_dm_sr_read_nvram();
    ...
    if (mmi_dm_sr_check_is_operator_sim_card(MMI_SIM2) == MMI_TRUE)
    {
        MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_GET_IMSI_RSP_OP);
	   
	    if (dm_self_first_sim != 0)
	    {
	        dm_self_start_flag2 = MMI_FALSE;
            return;
	    }
        dm_self_first_sim = 1;

        dm_self_reg_info.sim = dm_self_reg_sim;
        mmi_dm_sr_write_nvram();

        MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_SIM_DECIDED, dm_self_reg_sim);
...
static void mmi_dm_sr_get_sim2_imsi(void)
{
    ...
    { 
	    if (dm_self_sim2_imsi_repeat_time > 0)
	    {
            StartTimer(DM_SR_SIM2_IMSI_REPEAT_TIMER_ID, DM_SR_REPEAT_GET_SIM2_IMSI_TIME, mmi_dm_sr_repeat_get_sim2_imsi);
	    }
	    else
	    {
	        dm_self_start_flag2 = MMI_FALSE;
...
/*****************************************************************************
 * FUNCTION
 *  mmi_dm_sr_repeat_get_sim2_imsi
 * DESCRIPTION
...
static void mmi_dm_sr_repeat_get_sim2_imsi(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
//    MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_REPEAT_GET_SIM2_IMSI, dm_self_sim2_imsi_repeat_time);
    StopTimer(DM_SR_IMSI_REPEAT_TIMER_ID);
    dm_self_sim2_imsi_repeat_time--;
    mmi_dm_sr_get_sim2_imsi();
}
#endif /* __MMI_DUAL_SIM__ */
...
#ifdef __MMI_DUAL_SIM__
    if (dm_self_reg_sim == MMI_SIM1)
    {			
        mmi_dm_sr_send_message(MOD_MMI, MOD_L4C, 0, MSG_ID_MMI_NW_GET_IMEI_REQ, NULL, NULL);
    }
    else
    {				
        mmi_dm_sr_send_message(MOD_MMI, MOD_L4C_2, 0, MSG_ID_MMI_NW_GET_IMEI_REQ, NULL, NULL);
    }
#else			
    mmi_dm_sr_send_message(MOD_MMI, MOD_L4C, 0, MSG_ID_MMI_NW_GET_IMEI_REQ, NULL, NULL);
#endif
...
#ifdef __MMI_DUAL_SIM__
 	WriteValue(NVRAM_DM_SELF_REGISTER_SIM, &dm_self_reg_info.sim, DS_BYTE, &error);
...
mmi_ret mmi_dm_sr_handle_sms_ready_ind(mmi_event_struct* event_data)
{
   ...
   /*----------------------------------------------------------------*/
    ...
    {
        MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_SIM2_USABLE);
        
	    if(event_info->is_sms_ready == MMI_TRUE && dm_self_first_network2_ready != 0)
        {
            mmi_dm_sr_sim2_process_init();
        }
    }
...
//    srv_sms_set_content(sms_handler, content_ucs2_buffer, ((content_asc_str_len + 1) * 2));
    ...
    if (dm_self_reg_sim == MMI_SIM1) 
    {
        MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_SEND_REG_SMS_SIM1);	
        srv_sms_set_sim_id(sms_handler, SRV_SMS_SIM_1);
    }
    else
    {
        MMI_TRACE(MMI_CONN_TRC_G3_DM, MMI_DM_TRC_SR_SEND_REG_SMS_SIM2);	
