...
void srv_nitz_get_current_nw_name(mmi_sim_enum sim, CHAR* plmn_buf, WCHAR* nw_name_buf, U8 nw_name_buf_len, MMI_BOOL* add_ci)
{
	...
	ReadRecord(NVRAM_EF_NITZ_NW_NAME_LID, 1 + sim_idx, &srv_nitz_current_op_name, sizeof(nitz_nw_name_struct), &error);
    if (plmn_buf != NULL)
    {
...
void srv_nitz_set_current_nw_name(mmi_sim_enum sim, const nitz_nw_name_struct *data)
{
    ...
    WriteRecord(NVRAM_EF_NITZ_NW_NAME_LID, (mmi_frm_sim_to_index(sim) + 1), (void*)data, NVRAM_NITZ_NW_NAME_SIZE, &error);    
}
...
void srv_set_apply_nitz_type(srv_nitz_type_enum apply_type)
{
	...
	if (apply_type < 3)
	{
		WriteValue(NVRAM_SETTING_AUTOUPDATE_DT_TIME_STATUS, &apply_type, DS_BYTE, &error);
	}
...
srv_nitz_type_enum srv_get_apply_nitz_type(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
	ReadValue(NVRAM_SETTING_AUTOUPDATE_DT_TIME_STATUS, &data, DS_BYTE, &error);

	if (data > 2)
    {
        data = 0;	// TODO
        WriteValue(NVRAM_SETTING_AUTOUPDATE_DT_TIME_STATUS, &data, DS_BYTE, &error);
	}
