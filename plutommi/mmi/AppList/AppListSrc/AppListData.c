...
MMI_BOOL mmi_applist_mark_exist(
            U32 app_id, mmi_app_package_name_struct *array, U32 array_count)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
 ...
 *  void
 *****************************************************************************/
void mmi_applist_compact_list(U32 *list, S32 list_len)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
        ...
        mmi_app_package_name_struct *array, U32 array_count)
{
    ...
    {        
        exist = mmi_applist_mark_exist(
                    applist_p->data.bar_id[i], array, array_count);
        if (!exist)
        {
    ...
    {        
        exist = mmi_applist_mark_exist(
                    applist_p->data.panel_id[i], array, array_count);
        if (!exist)
        {
    ...
    /* Compact bar item */
    mmi_applist_compact_list(applist_p->data.bar_id, MMI_APPLIST_BAR_APP_NUM);
    ...
    {       
        ...
        mmi_applist_compact_list(
            &applist_p->data.panel_id[array_idx], in_page_len);
    ...
    for (i = 0; i < array_count; ++i)
    {
        ...
        RANFOW_TRACE("[APPLIST]add new app %d, index=%d\n", app_id, append_idx);
 ...
 *  void
 *****************************************************************************/
static void mmi_applist_get_installed_app_id(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_applist_load_data(void)
{
    ...
    memset(applist_p->preinstall_id, 0, sizeof(applist_p->preinstall_id));
    ...
    #ifdef __MMI_APPLIST_APP_UNINSTALL_SUPPORT__
    memset(applist_p->installed_id, 0, sizeof(applist_p->installed_id));
    mmi_applist_get_installed_app_id();
    #endif
...
MMI_ID mmi_applist_get_self_icon_id(mmi_app_package_name_struct app_name)
{
 ...
 {
  if(strncmp(app_name, g_mmi_applist_icon_table[i].app_name, MMI_APP_NAME_MAX_LEN) == 0)
  {
 ...
 *  void
 *****************************************************************************/
void mmi_applist_set_appmgr_cell_info(U32 app_id, mmi_applist_cell_struct *cell)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (info.image.type == MMI_IMAGE_SRC_TYPE_RES_ID)
    {
        ...
        RANFOW_TRACE("[APPLIST]app=%d, image_type=%d, image_res_id=%d\n", app_id, info.image.type, cell->image.res_id);
    }
    else if(info.image.type == MMI_IMAGE_SRC_TYPE_ROM_MEMORY)
    {
        ...
        RANFOW_TRACE("[APPLIST]app=%d, image_type=%d, image_ptr=%x\n", app_id, info.image.type, cell->image_ptr);
    }
    else if (info.image.type == MMI_IMAGE_SRC_TYPE_PATH)
    {
        ...
        RANFOW_TRACE("[APPLIST]app=%d, image_type=%d, image_path=%s\n", app_id, info.image.type, cell->image.path);
    }
    else
    {
        ASSERT(0);
...
void mmi_applist_set_indicator_info(S32 page_idx)
{
    ...
    {   
        ...
        if (org_page_num != page_num &&
            org_state != MMI_APPLIST_INDICATOR_STATE_DISABLE)
        {
...
void mmi_applist_init_hilight_frame(S32 page_idx)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (applist_p->state == MMI_APPLIST_STATE_FIRST_ENTER || applist_p->hilight_frame.cell_p == NULL || applist_p->hilight_frame.cell_p->app_id == 0)
    {
        applist_p->hilight_frame.index = MMI_APPLIST_INVALID_INDEX;
        applist_p->hilight_frame.cell_p = NULL;
        mmi_applist_init_hilight_index_in_page();
    }
    else if (applist_p->hilight_frame.cell_p != NULL && applist_p->hilight_frame.cell_p->app_id != 0)
    {
        if (applist_p->hilight_frame.type == MMI_APPLIST_CELL_TYPE_PANEL)
 ...
 *  void
 *****************************************************************************/
void mmi_applist_arraw_key_press(mmi_applist_dir_enum dir)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    RANFOW_TRACE("[APPLIST]mmi_applist_arraw_key_press, applist_p->state = %d\n", applist_p->state);
