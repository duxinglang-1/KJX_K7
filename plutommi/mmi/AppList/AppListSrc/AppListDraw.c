...
/*
SWLA labels:
UA -- Update Animation Data
UA1 -- Update Animation Data part_1 assign target value
UA2 -- Update Animation Data part_2 draw panel
UA3 -- Update Animation Data part_3 blt
...
DP        -- Draw Panel
DP0    -- Draw Panel part_0 before draw title
DP1    -- Draw Panel part_1 draw title
DP2    -- Draw Panel part_2 draw indicator
DP3    -- Draw Panel part_3 draw slide area icon
DP4    -- Draw Panel part_4 draw shortcut bar
DP5    -- Draw Panel part_5 draw selected cell
*/
 ...
 *  void
 *****************************************************************************/
static MMI_BOOL mmi_applist_get_app_icon_info(U32 app_id, S32 cache_idx)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (info.image.type == MMI_IMAGE_SRC_TYPE_PATH)
    {
        ...
        RANFOW_TRACE("[APPLIST]cache app icon %d\n", app_id);
...
void mmi_applist_update_app_icon_data_cache()
{
    ...
    for (j = 0; j < MMI_APPLIST_BAR_APP_NUM; ++j)
    {
        ...
        is_cached = mmi_applist_get_app_icon_info(applist_p->data.bar_id[j], cache_idx);
        if (is_cached)
        {
            ++cache_idx;
        }
    ...
    for (j = 0; j < MMI_APPLIST_PANEL_APP_NUM; ++j)
    {
        ...
        is_cached = mmi_applist_get_app_icon_info(applist_p->data.panel_id[j], cache_idx);
        if (is_cached)
        {
            ++cache_idx;
        }
...
void mmi_applist_remove_unused_anima_node(U32 cur_time)
{
    ...
    while(count > 0)
    {
        if (node->type == MMI_APPLIST_ANIMA_TYPE_POSITION_X || node->type == MMI_APPLIST_ANIMA_TYPE_POSITION)
        {
            ...
            {
                RANFOW_TRACE("[APPLIST]mmi_applist_remove_unused_anima_node\n");
...
{ 
    ...
    if (tail == NULL)
    {
        /* Draw panel */
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("UA2", SA_start);
#endif
        mmi_applist_draw_panel();   
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("UA2", SA_stop);
#endif 
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("UA3", SA_start);
#endif
        mmi_applist_blt_previous();
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("UA3", SA_stop);
#endif
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("UA", SA_stop);
#endif
        return;
    }
    
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("UA1", SA_start);
#endif
...
void mmi_applist_add_to_used_queue(mmi_applist_anima_node_struct *node)
{
    ...
    RANFOW_TRACE("[APPLIST]add anima[t:%d,s:%d,d:%d]\n", node->type, node->start_time, node->duration);
    tail = applist_p->used_anima_tail;
    if (tail == NULL)
    {
...
void mmi_applist_draw_icon_bg_in_cache(void)
{
    ...
    if (applist_p->icon_bg_handle == GDI_LAYER_EMPTY_HANDLE)
    {
        ...
        gdi_layer_create_cf_using_outside_memory(
            ...
            applist_p->cache_pool[applist_p->empty_cache_index].buffer,
...
/* 
    ...
    if (cell->image_type == MMI_IMAGE_SRC_TYPE_ROM_MEMORY && cell->image_ptr != NULL)
    {
        ...
        RANFOW_TRACE("[APPLIST]cache cell %d, type=MMI_IMAGE_SRC_TYPE_ROM_MEMORY, shift_x=%d, shift_y=%d, icon_w=%d, icon_h=%d\n", 
            cell->app_id, shift_x, shift_y, icon_w, icon_h);
    }
    /* Image ID */
    else if (cell->image_type == MMI_IMAGE_SRC_TYPE_RES_ID && cell->image.res_id != 0)
    {        
        ...
        RANFOW_TRACE("[APPLIST]cache cell %d, type=MMI_IMAGE_SRC_TYPE_RES_ID, shift_x=%d, shift_y=%d, icon_w=%d, icon_h=%d\n", 
            cell->app_id, shift_x, shift_y, icon_w, icon_h);       
    }
    /* Image path */
    else if (cell->image_type == MMI_IMAGE_SRC_TYPE_PATH && mmi_ucs2strlen((CHAR *)cell->image.path) != 0)
    {
        ...
        if (is_cached && image_mem != NULL)
        {
            RANFOW_TRACE("[APPLIST]draw app icon from cache\n");
            gdi_image_draw_resized(
        ...
        #endif
        {
            gdi_image_draw_resized_file(
                shift_x, shift_y, icon_w, icon_h, cell->image.path);
        }
        
        RANFOW_TRACE("[APPLIST]cache cell %d, type=MMI_IMAGE_SRC_TYPE_PATH, shift_x=%d, shift_y=%d, icon_w=%d, icon_h=%d\n", 
            cell->app_id, shift_x, shift_y, icon_w, icon_h);
    }
    else
    {
    ...
    {
        gdi_layer_set_background(0);
        gdi_layer_flatten(cell_handle, applist_p->uninstall_icon_handle, NULL, NULL);
    }
    #endif
...
void mmi_applist_cache_bar_cell(void)
{
    ...
    for (i = 0; i < MMI_APPLIST_BAR_APP_NUM; i ++)
    {
        ...
        #ifdef __MMI_APPLIST_APP_UNINSTALL_SUPPORT__
        if (mmi_applist_is_installed_app(applist_p->bar_cell[i]->app_id))
        {
            if (applist_p->mode == MMI_APPLIST_MODE_EDIT && !applist_p->bar_cell[i]->is_uninstall_icon_cached)
            {
                //remove normal mode cache to cache cell with uninstall icon
                applist_p->cache_used[applist_p->bar_cell[i]->cache_idx] = MMI_FALSE;
                applist_p->bar_cell[i]->cache_idx = MMI_APPLIST_INVALID_INDEX;
                applist_p->bar_cell[i]->is_uninstall_icon_cached = MMI_TRUE;
            }
            else if (applist_p->mode != MMI_APPLIST_MODE_EDIT && applist_p->bar_cell[i]->is_uninstall_icon_cached)
            {
                //remove edit mode cache to cache cell without uninstall icon
                applist_p->cache_used[applist_p->bar_cell[i]->cache_idx] = MMI_FALSE;
                applist_p->bar_cell[i]->cache_idx = MMI_APPLIST_INVALID_INDEX;
                applist_p->bar_cell[i]->is_uninstall_icon_cached = MMI_FALSE;
            }
        }
        #endif
...
static void mmi_applist_find_empty_cache_idx(void)
{
    ...
    /* Search backward */
    for (i = applist_p->empty_cache_index; i < MMI_APPLIST_APP_CACHE_NUM; i ++)
    {
    ...
    /* found */
    if (i < MMI_APPLIST_APP_CACHE_NUM)
    {
        return;
    }
    ...
    *cell_end = 0;
    ...
    for (i = 0; i < MMI_APPLIST_MAX_PAGE_NUM; i ++)
    {
        ...
        for (j = start_idx; j <= to_idx; j++)
        {
            ...
            #ifdef __MMI_APPLIST_APP_UNINSTALL_SUPPORT__
            if (mmi_applist_is_installed_app(applist_p->panel_cell[j]->app_id))
            {
                if (applist_p->mode == MMI_APPLIST_MODE_EDIT && !applist_p->panel_cell[j]->is_uninstall_icon_cached)
                {
                    //remove normal mode cache to cache cell with uninstall icon
                    applist_p->cache_used[applist_p->panel_cell[j]->cache_idx] = MMI_FALSE;
                    applist_p->panel_cell[j]->cache_idx = MMI_APPLIST_INVALID_INDEX;
                    applist_p->panel_cell[j]->is_uninstall_icon_cached = MMI_TRUE;
                }
                else if (applist_p->mode != MMI_APPLIST_MODE_EDIT && applist_p->panel_cell[j]->is_uninstall_icon_cached)
                {
                    //remove edit mode cache to cache cell without uninstall icon
                    applist_p->cache_used[applist_p->panel_cell[j]->cache_idx] = MMI_FALSE;
                    applist_p->panel_cell[j]->cache_idx = MMI_APPLIST_INVALID_INDEX;
                    applist_p->panel_cell[j]->is_uninstall_icon_cached = MMI_FALSE;
                }
            }
            #endif
...
S32 mmi_applist_draw_shortcut_bar(void)
{
    ...
    if (applist_p->state != MMI_APPLIST_STATE_FIRST_ENTER && applist_p->state != MMI_APPLIST_STATE_RESET && 
        applist_p->state != MMI_APPLIST_STATE_ENTER && applist_p->mode != MMI_APPLIST_MODE_EDIT)
    {
        gdi_rect_struct dest_clip = {0, 0, LCD_WIDTH - 1, LCD_HEIGHT - 1};
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("LGA", SA_start);
#endif
        gdi_layer_get_active(&act_handle);
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("LGA", SA_stop);
#endif
        act_layer_struct = (gdi_layer_struct*)act_handle;
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("BWT", SA_start);
#endif
        RANFOW_TRACE("[APPLIST]copy shortcut bar from previous layer, state[%d], mode[%d]\n", applist_p->state, applist_p->mode);
    ...
    for (i = 0; i < MMI_APPLIST_BAR_APP_NUM; i ++)
    {
        if (applist_p->bar_cell[i] != NULL)
        {
            ...
            if (applist_p->mode == MMI_APPLIST_MODE_EDIT || applist_p->state == MMI_APPLIST_STATE_FIRST_ENTER || 
                applist_p->state == MMI_APPLIST_STATE_RESET || applist_p->state == MMI_APPLIST_STATE_ENTER)
            {
                ...
                (applist_p->hilight_frame.pre_index != applist_p->hilight_frame.index || applist_p->hilight_frame.pre_type != applist_p->hilight_frame.type))
            {
                ...
                {
                    memset(act_layer_struct->buf_ptr + 3 * LCD_WIDTH * (MMI_APPLIST_SHORTCUT_BAR_Y+j) + i * MMI_APPLIST_CELL_WIDTH * 3, 0,  MMI_APPLIST_CELL_WIDTH * 3);
                }
                need_redraw = MMI_TRUE;
            }
        ...
        mmi_applist_draw_info_struct *draw_info)
{
    ...
    if (applist_p->mode != MMI_APPLIST_MODE_EDIT
        ...
        && (applist_p->slide_type == MMI_APPLIST_SLIDE_BY_PEN || !applist_p->is_loop)
        #endif
        )
    {
...
void mmi_applist_draw_app_uninstall_icon_in_cache()
{
    ...
    if (applist_p->uninstall_icon_handle == GDI_LAYER_EMPTY_HANDLE)
    {
        ...
        /* draw image */       
        gdi_image_draw_id(MMI_APPLIST_APP_UNINSTALL_ICON_X, MMI_APPLIST_APP_UNINSTALL_ICON_Y, IMG_ID_APPLIST_APP_UNINSTALL_ICON);
...
void mmi_applist_draw_indicator(void)
{
    ...
    {        
        if (applist_p->indicator[i].state == MMI_APPLIST_INDICATOR_STATE_INACTIVE)
        {
            ...
            {
                memcpy(active_layer_struct->buf_ptr + LCD_WIDTH * (applist_p->indicator[i].pos.y + j) * 3 + applist_p->indicator[i].pos.x * 3, 
                    applist_p->indicator_ptr + MMI_APPLIST_INDICATOR_SIZE + MMI_APPLIST_INDICATOR_SIDE * j * 3, MMI_APPLIST_INDICATOR_SIDE * 3);
            }
        }
        else if (applist_p->indicator[i].state == MMI_APPLIST_INDICATOR_STATE_ACTIVE)
        {
            ...
            {
                memcpy(active_layer_struct->buf_ptr + LCD_WIDTH * (applist_p->indicator[i].pos.y + j) * 3 + applist_p->indicator[i].pos.x * 3, 
                    applist_p->indicator_ptr + MMI_APPLIST_INDICATOR_SIDE * j * 3, MMI_APPLIST_INDICATOR_SIDE * 3);
            }
        }
    }
}
...
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("DP0", SA_start);
#endif
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("PSA", SA_start);
#endif
    gdi_layer_push_and_set_active(applist_p->panel_layer_handle);
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("PSA", SA_stop);
#endif
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("TD", SA_start);
#endif
#ifdef __MMI_LAUNCHER_APP_LIST_VIDEO_WALLPAPER__
    gdi_layer_toggle_double();
#endif
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("TD", SA_stop);
#endif
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("LC", SA_start);
#endif
...
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("LC", SA_stop);
#endif
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("AB", SA_start);
#endif
    gdi_push_and_set_alpha_blending_source_layer(applist_p->panel_layer_handle);
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("AB", SA_stop);
#endif
#ifdef __MAUI_SOFTWARE_LA__
        SLA_CustomLogging("DP0", SA_stop);
#endif

    /* draw title */
#if defined(__MMI_APPLIST_KEY_SUPPORT__) && defined(__MMI_APPLIST_ONLY_ICON_SUPPORT__)
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("DP1", SA_start);
#endif
    mmi_applist_draw_title();
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("DP1", SA_stop);
#endif
#endif

    /* Draw indicator */
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("DP2", SA_start);
#endif
    mmi_applist_draw_indicator();  
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("DP2", SA_stop);
#endif  
    
    /* Draw slide area */
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("DP3", SA_start);
#endif
    slide_idx = mmi_applist_draw_slide_area_icon();  
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("DP3", SA_stop);
#endif

    /* Draw shortcut bar */
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("DP4", SA_start);
#endif
...
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("DP4", SA_stop);
#endif

    /* Draw selected cell */
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("DP5", SA_start);
#endif
    mmi_applist_draw_selected_cell(shortcut_idx, slide_idx);
#ifdef __MAUI_SOFTWARE_LA__
    SLA_CustomLogging("DP5", SA_stop);
#endif
