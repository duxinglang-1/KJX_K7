...
#ifdef __MMI_DUAL_SIM__
    , {MMI_SIM2, MENU_ID_CLOG_CCOST_SIM2_LAST}
    , {MMI_SIM2, MENU_ID_CLOG_CCOST_SIM2_TOTAL}
    , {MMI_SIM2, MENU_ID_CLOG_CCOST_SIM2_RESET}
    , {MMI_SIM2, MENU_ID_CLOG_CCOST_SIM2_MAX}
    , {MMI_SIM2, MENU_ID_CLOG_CCOST_SIM2_PPU}
        ...
        case EVT_ID_ALERT_QUIT:
        {
            ...
            switch (alert->result)
            {
                ...
                case MMI_ALERT_NORMAL_EXIT:
                {
		          ...
		          cntx->hilite_menu_id == MENU_ID_CLOG_CCOST_PPU
	                  #ifdef __MMI_DUAL_SIM__
			  || cntx->hilite_menu_id == MENU_ID_CLOG_CCOST_SIM2_PPU
			#endif /*__MMI_DUAL_SIM__*/
			)
                    {
                    srv_clog_ccost_get_data(
...
static void mmi_clog_ccost_set_lsk_str(MMI_MENU_ID menu_id)
{
    ...
    #ifdef __MMI_DUAL_SIM__
        case MENU_ID_CLOG_CCOST_SIM2_RESET:
            cui_menu_change_left_softkey_string_by_id(cntx->cui, STR_GLOBAL_OK);
            break;

        case MENU_ID_CLOG_CCOST_SIM2_MAX:
        case MENU_ID_CLOG_CCOST_SIM2_PPU:
            cui_menu_change_left_softkey_string_by_id(cntx->cui, STR_ID_CLOG_CCOST_SET);
            break;
    #endif /* __MMI_DUAL_SIM__ */ 
...
#ifdef __MMI_DUAL_SIM__
    else if (menu_id == MENU_ID_CLOG_CCOST_SIM2_RESET ||
             menu_id == MENU_ID_CLOG_CCOST_SIM2_MAX || menu_id == MENU_ID_CLOG_CCOST_SIM2_PPU)
    {
        cntx->sim_id = MMI_SIM2;
        ...
        cntx->hilite_sim = 1;   /* means sim2 */
        if (srv_sim_ctrl_pwd_is_enabled(MMI_SIM2, SRV_SIM_CTRL_PWD_TYPE_CHV2))
        {
            sim = MMI_SIM2;
        }
        else
...
static void mmi_clog_ccost_save_hdlr(void)
{
...
#else /* (MMI_MAX_SIM_NUM >= 3) */
#ifdef __MMI_DUAL_SIM__
    else if (cntx->hilite_menu_id == MENU_ID_CLOG_CCOST_SIM2_MAX)
    {
        CHAR acsii_max_cost_str[MMI_CLOG_CCOST_MAX_LEN];
        WCHAR *max_p = cntx->max_cost[1];
    ...
    else if (cntx->hilite_menu_id == MENU_ID_CLOG_CCOST_SIM2_PPU)
    {
        WCHAR *price_p = cntx->price[1];
        WCHAR *unit_p = cntx->unit[1];
...
static S32 mmi_clog_ccost_save_cb(void *user_data, mmi_sim_enum sim_id, U32 fields, srv_clog_ret_enum ret)
{
    ...
    else if (cntx->hilite_menu_id == MENU_ID_CLOG_CCOST_SIM2_MAX)
    {
        ...
        mmi_ucs2_to_asc(acsii_max_cost_str, (CHAR*) max_p);
        cntx->data[1].max = atoi(acsii_max_cost_str);
    }
    else if (cntx->hilite_menu_id == MENU_ID_CLOG_CCOST_SIM2_PPU)
    {
        WCHAR *price_p = cntx->price[1];
        ...
        mmi_wcscpy(cntx->data[1].ppu, price_p);
        mmi_wcscpy(cntx->data[1].currency, unit_p);
    }
#endif /* __MMI_DUAL_SIM__ */ 
#endif /* (MMI_MAX_SIM_NUM >= 3) */
