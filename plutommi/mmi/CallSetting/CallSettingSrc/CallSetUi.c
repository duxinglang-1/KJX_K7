...
static MMI_BOOL mmi_callset_delay_dial_check_time(U8 ded_time);
static void mmi_callset_hilite_delay_dial(MMI_ID menu_gid);
static void mmi_callset_create_delay_dial_mode(MMI_ID parent_gid);
static void mmi_callset_entry_delay_dial_mode(MMI_ID menu_gid);
static void mmi_callset_sel_delay_dial_mode(void *info);
#endif /* __OP01_FWPBW__ */
...
mmi_ret mmi_callset_proc_common(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
        ...
        case EVT_ID_CUI_MENU_LIST_ENTRY:
            ...
            #endif /* __CTM_SUPPORT__ */
            #ifdef __OP01_FWPBW__
                case MENU_ID_CALLSET_DELAY_DIAL_MODE:
            #endif /* __OP01_FWPBW__ */
                {
                    cui_menu_set_currlist_flags(menu_evt->sender_id, CUI_MENU_NORMAL_RADIO_BUTTON_LIST);
                    break;
        ...
        case EVT_ID_CUI_MENU_ITEM_SELECT:
        {
            switch (menu_evt->highlighted_menu_id)
            {
        ...
        #ifdef __MMI_VOIP__
                case MENU_ID_VOIP_MAIN:
                {
                    mmi_voip_pre_entry_main_menu();
                    break;
                }

                case MENU_ID_VOIP_STATUS:
                {
                    mmi_voip_entry_status();
                    break;
                }

                case MENU_ID_VOIP_SUMMARY:
                {
                    mmi_voip_entry_summary();
                    break;
                }

                case MENU_ID_VOIP_PROF_SETTING:
                {
                    mmi_voip_entry_prof_setting();
                    break;
                }

                case MENU_ID_VOIP_CALL_SETTING:
                {
            ...
            #ifdef __OP01_FWPBW__
                case MENU_ID_CALLSET_DELAY_DIAL:
                {
                    mmi_callset_create_delay_dial_mode(GRP_ID_CALLSET_COMMON);
                    break;
                }
            #endif /* __OP01_FWPBW__ */
                ...
                default:
                {
                    switch(menu_evt->parent_menu_id)
                    {
                        ...
                        case MENU_ID_CALLSET_DELAY_DIAL_MODE:
                        {
                            mmi_callset_sel_delay_dial_mode(menu_evt);
                            break;
                        }
                    #endif /* __OP01_FWPBW__ */
                    }
...
void mmi_callset_entry_call_center(MMI_ID menu_gid)
{
    ...
    if (!srv_mode_switch_is_network_service_available())
    {
        cui_menu_set_item_hidden(menu_gid, MENU_ID_VOIP_MAIN, MMI_TRUE);
    }
    else
    {
        cui_menu_set_item_hidden(menu_gid, MENU_ID_VOIP_MAIN, MMI_FALSE);
    }
#endif /* __MMI_VOIP__ */
}
...
void mmi_callset_hilite_common(MMI_ID menu_gid, mmi_callset_op_enum op)
{
    ...
    #ifdef __OP01_FWPBW__
        case MMI_CALLSET_OP_DELAY_DIAL:
            mmi_callset_hilite_delay_dial(menu_gid);
            break;
    #endif /* __OP01_FWPBW__ */
    ...
    else
    {
        ...
        switch (mmi_callset_sim_p->cb_state)
        {
            ...
            case 3:
                ...
                SetKeyHandler(mmi_callset_sel_cb_passwd_done, KEY_POUND, KEY_EVENT_UP);
                break;
            ...
            case 2:
                ...
                SetKeyHandler(mmi_callset_sel_cb_passwd_next, KEY_POUND, KEY_EVENT_UP);
                break;
        }
    }
}
...
static const U16 ded_time_list[DED_TIME_NUM] = {1, 2, 3, 4, 5, 6, 7, 8};

/*****************************************************************************
 * FUNCTION
 *  mmi_callset_delay_dial_check_time
 * DESCRIPTION
 *  Check the delay dial time is avaluable time
 * PARAMETERS
 *  ded_time    [IN]    delay dial time
 * RETURNS
 *  void
 *****************************************************************************/
MMI_BOOL mmi_callset_delay_dial_check_time(U8 ded_time)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        for (i = 0; i < DED_TIME_NUM; i++)
        {
            if (ded_time_list[i] == ded_time)
            {
                result = MMI_TRUE;
                break;
            }
...
/*****************************************************************************
 * FUNCTION
 *  mmi_callset_hilite_delay_dial
 * DESCRIPTION
 *  highlight handler for delay dial menu item
 * PARAMETERS
 *  menu_gid    [IN]    menu group id
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_callset_hilite_delay_dial(MMI_ID menu_gid)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    ded_time = srv_callset_delay_dial_get_time();
    
    /* Need to check the return value */
    if (MMI_FALSE == mmi_callset_delay_dial_check_time(ded_time))
    {
        ded_time = DED_DEFAULT_TIME;
        srv_callset_delay_dial_set_time(ded_time);
    }

    if (!ded_time)
    {
        cui_menu_set_item_hint(menu_gid, MENU_ID_CALLSET_DELAY_DIAL, get_string(STR_GLOBAL_OFF));
    }
    else
    {
        U32 i;
        for (i = 0; i < DED_TIME_NUM; i++)
        {
            if (ded_time_list[i] == ded_time)
            {
                cui_menu_set_item_hint(menu_gid, MENU_ID_CALLSET_DELAY_DIAL, get_string(STR_ID_CALLSET_DELAY_DIAL_1_SEC + i));
                break;
            }
        }
    }
...
/*****************************************************************************
 * FUNCTION
 *  mmi_callset_create_delay_dial_mode
 * DESCRIPTION
 *  start delay dial mode menu
 * PARAMETERS
 *  parent_gid  [IN]    parent group id
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_callset_create_delay_dial_mode(MMI_ID parent_gid)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    mmi_callset_common_create_cui(parent_gid, MENU_ID_CALLSET_DELAY_DIAL_MODE, MENU_ID_CALLSET_CALL_SETTING);
}
 ...
 *  void
 *****************************************************************************/
void mmi_callset_entry_delay_dial_mode(MMI_ID menu_gid)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    else
    {
        ...
        for (i = 0; i < DED_TIME_NUM; i++)
        {
            if (ded_time_list[i] == ded_time)
            {
                cui_menu_set_radio_list_item(menu_gid, MENU_ID_CALLSET_DELAY_DIAL_1_SEC + i);
                break;
            }
        }
    }
...
/*****************************************************************************
 * FUNCTION
 *  mmi_callset_sel_delay_dial_mode
 * DESCRIPTION
 *  
 * PARAMETERS
 *  info    [IN]    input information
 ...
 *  void
 *****************************************************************************/
void mmi_callset_sel_delay_dial_mode(void *info)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (menu_evt->highlighted_menu_id == MENU_ID_CALLSET_OFF)
    {
        ded_time = 0;
    }
    else
    {
        ded_time = ded_time_list[menu_evt->highlighted_menu_id - MENU_ID_CALLSET_DELAY_DIAL_1_SEC];
    }

    result = srv_callset_delay_dial_set_time(ded_time);
