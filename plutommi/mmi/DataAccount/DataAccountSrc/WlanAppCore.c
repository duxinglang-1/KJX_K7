 ...
 *****************************************************************************/
#define __MMI_WLAN_CORE_FUNC_DECLARE__ 
...
#define __MMI_WLAN_CORE_FUNC_DEFINE__ 
/*****************************************************************************
 * FUNCTION
...
#define __MMI_WLAN_CORE_SEND_REQUEST__
/*****************************************************************************
 * FUNCTION
...
#define __MMI_WLAN_CORE_EVENT_HANDLER__
 /*****************************************************************************
 * FUNCTION
...
static void mmi_wlan_init_event_hdlr(mmi_event_struct* param)
{
    ...
    /******************************************************************************
     * Test Case 7.3.1 Auto Connect
     * connect to one saved ap which has highest priority and also existed in scan result
     * 1. the auto conn. switch is on
     * 2. send connect request to DA SRV
     *****************************************************************************/
    if( g_wlan_display_context.is_auto_conn &&   /* switch is on */
       (SRV_DTCNT_WLAN_STATUS_CONNECTED != srv_dtcnt_wlan_status()) &&  /* not in connected status */
       (mmi_wlan_get_ui_flow () == MMI_WLAN_UI_NONE)                    /* in normal proc */   
       )
    {
        mmi_wlan_set_ui_flow (MMI_WLAN_UI_AUTO_CONN);
        mmi_wlan_auto_conn_trigger_by_init ();
    }
#endif    
...
static void mmi_wlan_connect_ind_event_hdlr(mmi_event_struct* param)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_OP01_WIFI__    
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, MMI_WLAN_CONNECT_IND_EVENT, g_wlan_display_context.conn_num, g_wlan_display_context.ui_flow);
#endif
    g_wlan_display_context.send_conn_req_ing = MMI_FALSE;
...
#ifdef __MMI_OP01_WIFI__
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, MMI_WLAN_CONN_IND_PROFILE_INFO, ind->profile_id, ind->need_login);
    if(ind->need_login)
    {
        if((connected_ap == NULL) ||
           (MMI_FALSE == mmi_wlan_is_default_trust_ap (connected_ap)))
        {
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, MMI_WLAN_CONN_IND_SEND_PORTAL_LOGIN_STOP_REQ);
            srv_dtcnt_wlan_send_portal_login_stop_req(MMI_TRUE);
        }
    }
#endif
    ...
    {
#ifdef __MMI_OP01_WIFI__
        mmi_wlan_cmcc_refresh_main_screen();
#else
...
#ifdef __MMI_OP01_WIFI__    
    else if(mmi_frm_scrn_get_active_id() == SCR_ID_DTCNT_WLAN_EMPTY && is_sleep == MMI_FALSE)
    {
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, MMI_WLAN_DISC_CNF_SHOW_DISCONNECT_POPUP);
        mmi_popup_display_simple(
            (WCHAR*)((UI_string_type) GetString(STR_ID_DTCNT_WLAN_DISCONNECTED)), 
...
#ifdef __MMI_OP01_WIFI__
             || active_screen_id == SCR_ID_DTCNT_WLAN_TRUST_LIST
#endif
             && is_sleep == MMI_FALSE)
    {
...
#ifdef __MMI_OP01_WIFI__
	else if (active_screen_id == SCR_ID_DTCNT_WLAN_EMPTY)
	{
...
#ifdef __MMI_OP01_WIFI__        
        if(MMI_WLAN_UI_AUTO_CONN == mmi_wlan_get_ui_flow ())
        {
            mmi_wlan_set_ui_flow (MMI_WLAN_UI_NONE);
        }
#endif
...
static void mmi_wlan_ipaddr_update_event_hdlr(mmi_event_struct* param)
{
    ...
    if(MMI_WLAN_UI_WAIT_FOR_IP_UPDATE == mmi_wlan_get_ui_flow ())
    {
        mmi_wlan_set_ui_flow (MMI_WLAN_UI_NONE);
    }
    mmi_wlan_cmcc_entry_portal_proc ();
#endif
}
...
void mmi_wlan_ip_addr_change_cnf_handler(mmi_event_struct* evt)
{
    ...
    if(profile)
    {
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, MMI_WLAN_IPADDR_CHANGE_CNF_USE_DHCP, profile->use_dhcp);
        if(!profile->use_dhcp)
        {
        	// Reset flag if AP use custom IP addr.
            if(MMI_WLAN_UI_WAIT_FOR_IP_UPDATE == mmi_wlan_get_ui_flow ())
...
static void mmi_wlan_after_connect_success(srv_dtcnt_wlan_conn_res_struct *cnf, supc_abm_bss_info_struct *active_ap)
{
...
#ifdef __MMI_OP01_WIFI__    
    // Set flag when the connected AP belongs to CMCC
    // So that user cannot disc, deinit, connect to another AP during getting IP addr.
    ...
    {
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, MMI_WLAN_CONN_CNF_SET_IP_UPDATE_FLAG);
        mmi_wlan_set_ui_flow (MMI_WLAN_UI_WAIT_FOR_IP_UPDATE);
    }
#endif
...
#ifdef __MMI_OP01_WIFI__
        case MMI_WLAN_UI_AUTO_CONN:
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, MMI_WLAN_CMCC_AUTO_CONN_RESULT_FAILED, 
                                        g_auto_conn_ap_idx, g_auto_conn_ap_total);
            // Continue the proc of auto conn if last try failed
...
#ifdef __MMI_OP01_WIFI__
        if((cnf->result == SRV_DTCNT_RESULT_FAILED) &&
           (NULL != g_wlan_display_context.active_wlan_profile_p) &&
           (g_wlan_display_context.active_wlan_profile_p->auth_mode == SRV_DTCNT_WLAN_AUTH_MODE_WAPICERT))
        {
           mmi_wlan_delete_profile (g_wlan_display_context.active_wlan_profile_p);
           g_wlan_display_context.active_wlan_profile_p = NULL;

           mmi_popup_display_simple(
                (WCHAR*)((UI_string_type) GetString(STR_ID_DTCNT_WLAN_ERROR_CONNECT_WAPI_NETWORK_FAIL)), 
                MMI_EVENT_FAILURE, 
                wlan_parent_group_id,
                NULL);
        }
...
static void mmi_wlan_after_scan_network_success(srv_dtcnt_wlan_scan_result_struct *cnf)
{
        ...
        )
    {
        ...
        {
#ifdef __MMI_OP01_WIFI__
            // Auto connect which is triggered by auto search
            if(g_wlan_display_context.is_auto_conn &&   /* switch is on */
               (SRV_DTCNT_WLAN_STATUS_CONNECTED != srv_dtcnt_wlan_status()) &&  /* not in connected status */
               (mmi_wlan_get_ui_flow () == MMI_WLAN_UI_NONE)                    /* in normal proc */   
               )
            {
                mmi_wlan_set_ui_flow (MMI_WLAN_UI_AUTO_CONN);
                mmi_wlan_auto_conn_trigger_by_scan ();
            }
            mmi_wlan_cmcc_entry_wizard_list();
            /* close saved ap list screen */
            mmi_frm_scrn_close(wlan_parent_group_id, SCR_ID_DTCNT_WLAN_WIZARD_SAVED_LIST);
#else
...
void mmi_wlan_list_auto_conn_start(srv_dtcnt_wlan_scan_result_struct *scan_res)
{
...
#ifdef __MMI_OP01_WIFI__
        if( (g_profile_saved_temp.auth_mode == SRV_DTCNT_WLAN_AUTH_MODE_OPEN) &&
            custom_wlan_is_operator_network((const char*)g_profile_saved_temp.ssid))
        {
            is_op_network_saved = MMI_FALSE;
            
            for(pos = 0; pos < g_wlan_display_context.prof_num; pos++)
            {
                if (mmi_wlan_is_same_ap (g_wlan_profile[pos], &g_profile_saved_temp))
                {
                    is_op_network_saved = MMI_TRUE;
                }
            }/* end of for */
            
            if(!is_op_network_saved)
            {
                is_op_network_found = MMI_TRUE;
                break;
            }
        }
#endif /* __MMI_OP01_WIFI__ */
...
#ifdef __MMI_OP01_WIFI__
    if(is_op_network_found)
    {
        // send connect request   
        memcpy(&g_wlan_hilt_profile, scan_res->ap_list[index], sizeof(supc_abm_bss_info_struct));
        mmi_wlan_send_connect_request (&g_profile_saved_temp);
    ...
    // 2. connect to saved AP if exsited
	    ...
	    cui_menu_close(g_wlan_display_context.option_menu_cui_id);
#ifdef __MMI_OP01_WIFI__		
	    cui_menu_close(g_wlan_display_context.inline_trust_list_cui_id);
#endif
...
S32 mmi_wlan_callback_handler(mmi_event_struct* param)
{
    ...
    case EVT_ID_GPIO_LCD_SLEEP_OUT:
        /* Search available ap if auto search is on */
        if(g_wlan_display_context.is_auto_search)  /* manually activate auto search  */
        {
            mmi_wlan_enable_entry_main();
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, MMI_WLAN_OPT_ON, WIFI_AUTO_SEARCH_TIMER);
            MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, MMI_WLAN_EDIT_SAVE, g_wlan_display_context.is_auto_search);
        }
        break;
        
    case EVT_ID_GPIO_AFTER_LCD_SLEEP_IN:
        /* Stop auto search timer when LCD light is off */
...
/* under construction !*/
#endif                        
    case EVT_ID_SRV_DTCNT_WLAN_LOGOUT_IND:
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, MMI_WLAN_CMCC_AUTO_PORTAL_LOGOUT_IND);
        mmi_wlan_cmcc_logout_ind_handler (param);
        break;
        
    case EVT_ID_SRV_DTCNT_WLAN_IPADDR_CHANGE_CNF:
        MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, MMI_WLAN_IPADDR_CHANGE_CNF_EVT_ID);
        mmi_wlan_ip_addr_change_cnf_handler (param);
        break;
#endif
...
mmi_ret mmi_wlan_cui_event_proc(mmi_event_struct *evt)
{
    ...
    /**************************  File Mgr CUI ******************************/
        case EVT_ID_CUI_FILE_SELECTOR_RESULT:
            mmi_wlan_cui_file_mgr_select_evt_proc(evt);
            break;
#endif /* __MMI_OP01_WIFI__ */            
    }
