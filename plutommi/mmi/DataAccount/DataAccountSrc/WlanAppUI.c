...
extern const char default_ap_str[5];
extern const char default_ap_edu_str[9];
...
static const cui_inline_item_select_struct cellular_switch_selector = {2, 0, switch_optional};
...
#endif
#ifdef __MMI_OP01_WIFI__
    {CUI_INLINE_ITEM_ID_BASE + WLAN_SETTING_CELLULAR_SWITCH_STR, CUI_INLINE_ITEM_TYPE_CAPTION, 0, (void*)&cellular_switch_caption},
    {CUI_INLINE_ITEM_ID_BASE + WLAN_SETTING_CELLULAR_SWICCH_SELECTOR, CUI_INLINE_ITEM_TYPE_SELECT, 0, (void*)&cellular_switch_selector},
    {CUI_INLINE_ITEM_ID_BASE + WLAN_SETTING_AUTO_PORTAL_SWITCH_STR, CUI_INLINE_ITEM_TYPE_CAPTION, 0, (void*)&auto_portal_switch_caption},
    {CUI_INLINE_ITEM_ID_BASE + WLAN_SETTING_AUTO_PORTAL_SWITCH_SELECTOR, CUI_INLINE_ITEM_TYPE_SELECT, 0, (void*)&auto_portal_switch_selector},
#else
...
#define __MMI_WLAN_MAIN_ENTRY_FUNC__
/*****************************************************************************
 * FUNCTION
 *  mmi_wlan_entry_wlan_wizard_new
 * DESCRIPTION
...
#define __MMI_WLAN_ENTRY_AP_LIST__
/*****************************************************************************
 * FUNCTION
...
#ifdef __MMI_OP01_WIFI__
    if(0 == g_wlan_display_context.display_ap_num)
    {
...
/* To read WAPI test mode configuration */
...
#ifdef __MMI_OP01_WIFI__
    if(MMI_WLAN_POWER_SAVING_MODE_CTIA == g_wlan_display_context.power_saving_mode)
    {
        mmi_wlan_cmcc_set_power_saving_mode (WNDRV_POWER_MODE_CONTINUOUS_ACTIVE);
    }

    /* initializaion for black list */
    for(i = 0; i < NVRAM_MAX_CMCC_WIFI_BLACK_LIST_TOTAL; i++)
    {
        if(g_wlan_prof_list[i].ap_type == SRV_DTCNT_WLAN_AP_BLACK_LIST)
        {
            g_wlan_display_context.black_list_num++;
        }
    }
    mmi_wlan_set_ui_flow (MMI_WLAN_UI_NONE);
    g_wlan_display_context.trust_list_num = 0;
    MMI_TRACE(MMI_CONN_TRC_G6_DTCNT, MMI_WLAN_CMCC_INIT_CNTX_STATICTIS,
    g_wlan_display_context.trust_list_num, g_wlan_display_context.black_list_num);
    ...
    srv_dtcnt_get_wlan_prefer_setting(wlan_prefer_settings);
    g_wlan_display_context.auto_portal_switch_idx = wlan_prefer_settings->auto_portal_switch;
...
void mmi_wlan_network_delete_yes(void)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_OP01_WIFI__
    cur_hilt_profile_p = g_wlan_display_context.trust_list_hilt_profile_p;
#else
...
#ifdef __MMI_OP01_WIFI__
    g_wlan_display_context.trust_list_hilt_index = 0;
    memset(g_wlan_display_context.trust_list_hilt_profile_p, 0, sizeof(mmi_wlan_profile_struct));
    g_wlan_display_context.trust_list_hilt_profile_p = NULL;
#else
...
#ifdef __MMI_OP01_WIFI__
    g_wlan_display_context.trust_list_hilt_profile_p = &g_wlan_display_context.temp_profile;
    srv_dtcnt_wlan_init_profile(&g_srv_wlan_prof);
    mmi_wlan_srv2app_profile_convert(&g_srv_wlan_prof, g_wlan_display_context.trust_list_hilt_profile_p, 1);
    memset(g_wlan_display_context.trust_list_hilt_profile_p->ssid, 0, SRV_DTCNT_PROF_MAX_SSID_LEN + 1);
    memset(g_wlan_display_context.trust_list_hilt_profile_p->name, 0, MAX_WLAN_PROF_NAME_LEN);
    mmi_wlan_store_curr_ap_into_cntx(g_wlan_display_context.trust_list_hilt_profile_p);
#else
...
void mmi_wlan_entry_network_delete(void)
{
...
#ifdef __MMI_OP01_WIFI__
    if(g_wlan_display_context.trust_list_num == 0 ||
        g_wlan_display_context.trust_list_hilt_profile_p == NULL)
    {
        return;
    }
    ...
    if (wlan_status == SRV_DTCNT_WLAN_STATUS_CONNECTED &&
        g_wlan_display_context.trust_list_hilt_profile_p == g_wlan_display_context.connected_wlan_profile)
    {
        mmi_popup_display_simple(
            (WCHAR*)((UI_string_type) GetString(STR_ID_DTCNT_WLAN_CONNECTING_NO_DEL)), 
            MMI_EVENT_FAILURE, 
...
#ifdef __MMI_OP01_WIFI__
    if(SRV_DTCNT_RESULT_IGNORE != retn)
    {
...
#define __MMI_WLAN_ACTION_CONNECT__
/*****************************************************************************
 * FUNCTION
...
#define __MMI_WLAN_NETWORK_TYPE__
/*****************************************************************************
 * FUNCTION
...
#define __MMI_WLAN_SECURITY_MODE__
/*****************************************************************************
 * FUNCTION
...
#define __MMI_WLAN_ENCRYPTION_MODE__

#define __MMI_WLAN_WEP_KEY_SETTINGS__
/*****************************************************************************
 * FUNCTION
...
#define __MMI_WLAN_WPA_WPA2_PASSPHRASE__
/*****************************************************************************
 * FUNCTION
 *  mmi_wlan_entry_passphrase_input
 * DESCRIPTION
...
void mmi_wlan_network_edit_save_yes(void)
{
    ...
    {
#ifdef __MMI_OP01_WIFI__
        active_profile = g_wlan_display_context.trust_list_hilt_profile_p;
#else
        active_profile = g_wlan_display_context.active_wlan_profile_p;
#endif
    }
...
#ifdef __MMI_OP01_WIFI__
    // It's forbid to add default trust ap by end user
    if((g_wlan_display_context.auth_mode == (U8)SRV_DTCNT_WLAN_AUTH_MODE_OPEN) &&
       ((ssid_length == 4 && 
        strncmp((const char*)ssid, (const char*)default_ap_str, 4) == 0)
        ||
        (ssid_length == 8 && 
        strncmp((const char*)ssid, (const char*)default_ap_edu_str, 8) == 0)))
    {
        mmi_popup_display_simple((WCHAR*)((UI_string_type) GetString(STR_ID_DTCNT_WLAN_ERROR_ADD_DEFAULT_AP)), MMI_EVENT_FAILURE, wlan_parent_group_id, NULL);
...
#define __MMI_WLAN_PROFILE_TCPIP_SETTINGS__
/*****************************************************************************
 * FUNCTION
...
#define __MMI_WLAN_PROFILE_ADVANCED_SETTINGS__
/*****************************************************************************
 * FUNCTION
...
#define __MMI_WLAN_GENERAL_SETTINGS__
void mmi_wlan_entry_wlan_prefer_settings(void)
{
    ...
    cui_inline_set_value (
	g_wlan_display_context.wlan_prefer_setting_id, 
        CUI_INLINE_ITEM_ID_BASE + WLAN_SETTING_CELLULAR_SWICCH_SELECTOR, 
        (void *)wlan_prefer_settings->cellular_switch);
    cui_inline_set_value (
		g_wlan_display_context.wlan_prefer_setting_id, 
        CUI_INLINE_ITEM_ID_BASE + WLAN_SETTING_AUTO_PORTAL_SWITCH_SELECTOR, 
        (void *)wlan_prefer_settings->auto_portal_switch);
    g_wlan_display_context.auto_portal_switch_idx = wlan_prefer_settings->auto_portal_switch;
#else
    cui_inline_set_value (
        g_wlan_display_context.wlan_prefer_setting_id, 
...
#ifdef __MMI_OP01_WIFI__
    wlan_prefer_settings->cellular_switch = cellular_switch;
    wlan_prefer_settings->auto_portal_switch = auto_portal_switch;
    g_wlan_display_context.auto_portal_switch_idx = auto_portal_switch;
#else
...
void mmi_wlan_change_priority(void)
{
    ...
    U8 new_priority = g_wlan_display_context.new_priority;
#ifdef __MMI_OP01_WIFI__
    U8 old_priority = g_wlan_display_context.trust_list_hilt_profile_p->priority;
#else
...
void mmi_wlan_wizard_change_priority_handler(S32 index)
{
    ...
    priority = index;
#ifdef __MMI_OP01_WIFI__
    network_type = g_wlan_display_context.trust_list_hilt_profile_p->network_type;
#else
...
void mmi_wlan_entry_change_priority(void)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_OP01_WIFI__
    if(g_wlan_display_context.trust_list_hilt_profile_p == NULL)
    {
        return;
    }
#endif
...
#ifdef __MMI_OP01_WIFI__
    g_wlan_display_context.new_priority =  g_wlan_display_context.trust_list_hilt_profile_p->priority;
#else
...
#define __MMI_WLAN_WAPI_TEST_MODE__
/*****************************************************************************
 * FUNCTION
...
#define __MMI_WLAN_SET_UI_FLOW__
...
#ifdef __MMI_OP01_WIFI__
    mmi_wlan_adjust_default_ap_priority();      // adjust the priority of default ap
#endif
...
#ifdef __MMI_OP01_WIFI__
        case MENU_ID_WLAN_WIZARD_BLACK_LIST_OPT:
            // mmi_wlan_build_black_list_options(menu_evt);
            break;

        case MENU_ID_WLAN_FIND_RESULT_LIST_OPT:
            mmi_wlan_build_find_result_list_opt(menu_evt);
            break;
#endif
...
#ifdef __MMI_OP01_WIFI__
    case MENU_ID_WLAN_WIZARD_LIST_OPT_ADD_TO_BK:
        mmi_wlan_add_into_black_list_hdlr();
        cui_menu_close (menu_evt->sender_id);
        break;
    case MENU_ID_WLAN_WIZARD_LIST_OPT_TRUST_LIST:
        mmi_wlan_entry_trust_list();
        break;
    case MENU_ID_WLAN_WIZARD_LIST_OPT_BLACK_LIST:
        mmi_wlan_entry_black_list();
        break;
    case MENU_ID_WLAN_PROXY_EXCLUSIVE_LIST:
        mmi_wlan_entry_proxy_exclusive_list();
        break;
    case MENU_ID_WLAN_PORTAL_CERT_SETTING:
        mmi_wlan_portal_cert_client_setting_entry();
        break;
    case MENU_ID_WLAN_WIZARD_LIST_OPT_FIND_AP:
        mmi_wlan_entry_find_specific_ap_scr();
        break;
    case MENU_ID_WLAN_BLACK_LIST_OPT_DELETE:
        mmi_wlan_remove_cur_ap_from_bk_list();
        cui_menu_close (menu_evt->sender_id);
        break;
    case MENU_ID_WLAN_BLACK_LIST_OPT_DELETE_ALL:
        mmi_wlan_remove_all_ap_from_bk_list();
        cui_menu_close (menu_evt->sender_id);
        break;
    case MENU_ID_WLAN_WIZARD_LIST_OPT_DETAILS:
        mmi_wlan_entry_ap_details();
        break;
    case MENU_ID_WLAN_WIZARD_LIST_OPT_LOAD_CERT:
        mmi_wlan_cmcc_opt_menu_load_cert ();
        break;
    case MENU_ID_WLAN_WIZARD_LIST_OPT_UNLOAD_CERT:
        mmi_wlan_cmcc_opt_menu_unload_cert ();
        break;
#endif
    default:
        break;
...
#ifdef __MMI_OP01_WIFI__
    /* Auto searching */
    else if (inline_submit_event->sender_id == g_wlan_display_context.wlan_auto_search_setting_cui_id)
    {
        mmi_wlan_get_auto_search_settings();
        mmi_wlan_close_inline_cui(&g_wlan_display_context.wlan_auto_search_setting_cui_id);
    }
    /* Find specific ap */
    else if (inline_submit_event->sender_id == g_wlan_display_context.wlan_find_ap_setting_cui_id)
    {
        cui_inline_get_value(
            g_wlan_display_context.wlan_find_ap_setting_cui_id,
            WLAN_FIND_AP_EDITOR_ID,
            g_wlan_display_context.find_ap_ssid);
        if(mmi_ucs2strlen((const CHAR *)g_wlan_display_context.find_ap_ssid)==0)
        {
            mmi_popup_display_simple(
                (WCHAR*)((UI_string_type) GetString(STR_ID_DTCNT_WLAN_SSID_CAN_NOT_EMPTY)), 
                MMI_EVENT_FAILURE, 
                wlan_parent_group_id, 
                NULL);

            memset(g_wlan_display_context.find_ap_ssid, 0, (SRV_DTCNT_PROF_MAX_SSID_LEN + 1) * ENCODING_LENGTH);
            cui_inline_set_value(
                g_wlan_display_context.wlan_find_ap_setting_cui_id,
                CUI_INLINE_ITEM_ID_BASE + WLAN_FIND_AP_EDITOR_ID,
                g_wlan_display_context.find_ap_ssid);
            cui_inline_set_screen_attributes(
                g_wlan_display_context.wlan_find_ap_setting_cui_id, 
                CUI_INLINE_RESET_ATTRIBUTE, 
        ...
        else
        {
            g_wlan_display_context.entry_wizard_new_list = MMI_FALSE;
            mmi_wlan_entry_find_network();
            mmi_wlan_close_inline_cui(&g_wlan_display_context.wlan_find_ap_setting_cui_id);
        }
...
#ifdef __MMI_OP01_WIFI__
    else if (inline_abort_event->sender_id == g_wlan_display_context.wlan_find_ap_setting_cui_id)
    {
        mmi_wlan_close_inline_cui(&g_wlan_display_context.wlan_find_ap_setting_cui_id);
    }
    else if (inline_abort_event->sender_id == g_wlan_display_context.wlan_auto_search_setting_cui_id)
    {
        mmi_wlan_close_inline_cui(&g_wlan_display_context.wlan_auto_search_setting_cui_id);
    }
#endif
    ...
    case CUI_INLINE_NOTIFY_SELECT_FOCUS_CHANGED:
        ...
        if (notify_evt->sender_id == g_wlan_display_context.inline_tcp_ip_cui_id)
        {
            ...
            case CUI_INLINE_ITEM_ID_BASE + WLAN_CUINLINE_TCPIP_USEDHCP:
                operation = (notify_evt->param ? CUI_INLINE_SET_ATTRIBUTE : CUI_INLINE_RESET_ATTRIBUTE);
                cui_inline_set_item_attributes(
                    notify_evt->sender_id,
                    CUI_INLINE_ITEM_ID_BASE + WLAN_CUINLINE_TCPIP_IPADDR,
                    operation,
...
#ifdef __MMI_OP01_WIFI__
        if (notify_evt->sender_id == g_wlan_display_context.wlan_auto_search_setting_cui_id)
        {
            switch (notify_evt->item_id)
            {
            case WLAN_AUTO_SEARCH_SELECT_ID:
                operation = (notify_evt->param ? CUI_INLINE_RESET_ATTRIBUTE : CUI_INLINE_SET_ATTRIBUTE);
                cui_inline_set_item_attributes(
                    notify_evt->sender_id,
