...
static void calc_exception_hdlr(S32 result)
{
    ...
    if (result == CALC_RESULT_DIV0)
        text = STR_ID_OP_DIVIDE_ERROR;
    else if (result == CALC_RESULT_EXCEED)
        text = STR_ID_OP_RESULT_EXCEED;
    mmi_popup_display_simple_ext(text, MMI_EVENT_FAILURE, GRP_ID_CALC, 0);
#else
    srv_prof_play_tone(SRV_PROF_TONE_ERROR, NULL);
#endif
...
static void calc_hilight_btn(calc_button_struct *btn)
{
    ...
    if (!btn || btn == g_calc_cntx->CurrHilite) return;
