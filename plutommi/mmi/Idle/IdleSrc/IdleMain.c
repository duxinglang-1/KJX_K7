...
/*****************************************************************************
 * FUNCTION
 *  mmi_idle_handle_dialer_exit
 * DESCRIPTION
 *  This function handles OP01 FWP idle dialer screen return.
 * PARAMETERS
 *  event           : [IN]      Event
 * RETURNS
 *  mmi_ret
 *****************************************************************************/
static mmi_ret mmi_idle_handle_dialer_exit(mmi_event_struct *event)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    MMI_ASSERT(event);
    cui_dialer_close(((mmi_group_event_struct*)event)->sender_id);
    return MMI_RET_OK;
}
#endif
...
mmi_ret mmi_idle_main_evt_hdlr(mmi_event_struct *event)
{
    ...
    switch (event->evt_id)
    {
        ...
        case EVT_ID_CUI_DIALER_CLOSE_REQ:
            return mmi_idle_handle_dialer_exit(event);                        
    #endif
