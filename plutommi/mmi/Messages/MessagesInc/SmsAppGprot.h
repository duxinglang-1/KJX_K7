...
typedef enum
{
    ...
    MMI_SMS_ENTRY_WRITE_REPLY_HIGHLIGHT_SIM1 = 0x0008,
    MMI_SMS_ENTRY_WRITE_REPLY_HIGHLIGHT_SIM2 = 0x0010,
    MMI_SMS_ENTRY_WRITE_REPLY_HIGHLIGHT_SIM3 = 0x0020,
    MMI_SMS_ENTRY_WRITE_REPLY_HIGHLIGHT_SIM4 = 0x0040,    
 ...
 * DESCRIPTION
 *  Send Text Message in the foreground; For Dual SIM Version;
 *  In this API, it will do error handling, and popup corresponding notification screen.
 * PARAMETERS
 *  ucs2_addr_list  :   [IN]    Address List (UCS2 Format)
 *  addr_count      :   [IN]    The input address total number
 *  ucs2_content    :   [IN]    The default UCS2 content will be input to the Editor 
 *  prefer_sim_id   :   [IN]    Prefer SIM Card ID, if the input parameter is SMS_SIM_1/SMS_SIM_2, it don't entry SIM Card Seleciton screen, if it is SMS_SIM_TOTAL, it will entry SIM Card Selection screen. 
 * RETURNS
 *  void
 *****************************************************************************/
