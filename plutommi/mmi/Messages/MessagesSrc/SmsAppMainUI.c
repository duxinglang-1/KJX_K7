...
mmi_sms_sim_enum resend_sim_id = SMS_SIM_UNSPECIFIC;
#endif
...
void mmi_sms_entry_viewer(void)
{
            ...
            MMI_FRM_FULL_SCRN))
    {
		...
		#ifdef __IV_TTS__
		NPR_TTS_ReadSmsContent(ems_data->textBuffer);
		#endif
...
static void mmi_sms_viewer_opt_edit(void)
{
...
#else /* __MMI_UNIFIED_COMPOSER__ */
    {
        ...
        if (curr_msg_status & SMS_STATUS_DRAFTS)
        {
...
void mmi_sms_class0_opt_save_to_inbox(srv_sms_sim_enum sim_id)
{
    ...
    if(sms_save_to_inbox_gid != GRP_ID_INVALID)
    {
        ...
        {
            cui_sms_set_save_sim_id(sms_save_to_inbox_gid, sim_id);
        }
    #endif
...
static void mmi_sms_box_list_opt_check_adv(MMI_ID menu_gid, U16 msg_id)
{
...
#endif /* __UNIFIED_MESSAGE_ARCHIVE_SUPPORT__ */
    {
        ...
        {
									cui_menu_set_item_hidden(cui_menu_gid, MENU_ID_SMS_LIST_ADV_OPT_COPY_TO_SIM, MMI_FALSE);
									cui_menu_set_item_hidden(cui_menu_gid, MENU_ID_SMS_LIST_ADV_OPT_MOVE_TO_SIM, MMI_FALSE);
								}
							}
						}
						#endif
        ...
        if (srv_sms_get_msg_storage_type(msg_id) == SRV_SMS_STORAGE_ME)
        {
            ...
            if (!srv_sim_ctrl_is_available(SRV_SMS_SMS_SIM_TO_MMI(sim_id)))
            {
                #ifdef __MMI_SMS_TCARD_STORAGE_SUPPORT__
                  if (!srv_fmgr_drv_is_accessible(srv_fmgr_drv_get_letter_by_type(SRV_FMGR_DRV_CARD_TYPE)))
                  {
...
static void mmi_sms_inbox_list_opt_entry_hdlr(cui_menu_event_struct *menu_evt, U16 msg_id)
{
    ...
    if (curr_box_type == SRV_SMS_BOX_ARCHIVE)
    {
        cui_menu_set_item_hidden(cui_menu_gid, MENU_ID_UM_FILTER_BY_OPT, MMI_TRUE);
    }
    else
    {
        cui_menu_set_item_hidden(cui_menu_gid, MENU_ID_UM_FILTER_BY_OPT, MMI_FALSE);
    }
...
#if (defined( __MMI_DUAL_SIM_MASTER__) && !defined(__MMI_MSG_EDITOR_WITH_RECIPIENT__))
    mmi_sms_set_save_send_select_sim_leaf_item(cui_menu_gid, MENU_ID_SMS_VIEWER_OPT_RESEND, srv_nw_usab_is_usable);
#endif
...
static void mmi_sms_menu_cui_select_hdlr(mmi_event_struct* evt)
{
    ...
    switch (menu_evt->parent_menu_id)
    {
        ...
        case MENU_ID_SMS_VIEWER_OPT_RESEND:
            ...
            else if (MENU_ID_SMS_VIEWER_OPT_RESEND_BY_SIM2 == highlighted_menu_id)
            {
                resend_sim_id = SMS_SIM_2;
            }
        #if (MMI_MAX_SIM_NUM >= 3)
            else if (MENU_ID_SMS_VIEWER_OPT_RESEND_BY_SIM3 == highlighted_menu_id)
            {
                resend_sim_id = SMS_SIM_3;
            }
        #if (MMI_MAX_SIM_NUM >= 4)
            else if (MENU_ID_SMS_VIEWER_OPT_RESEND_BY_SIM4 == highlighted_menu_id)
            {
        ...
        case MENU_ID_SMS_CLSASS0_OPT_SAVE_TO_INBOX:
            {
                ...
                if (MENU_ID_SMS_CLSASS0_OPT_SAVE_TO_INBOX_BY_SIM2 == highlighted_menu_id)
                {
                    sim_id = SRV_SMS_SIM_2;
                }
            #if (MMI_MAX_SIM_NUM >= 3)
                else if (MENU_ID_SMS_CLSASS0_OPT_SAVE_TO_INBOX_BY_SIM3 == highlighted_menu_id)
                {
                    sim_id = SRV_SMS_SIM_3;
                }
            #endif
            #if (MMI_MAX_SIM_NUM >= 4)
                else if (MENU_ID_SMS_CLSASS0_OPT_SAVE_TO_INBOX_BY_SIM4 == highlighted_menu_id)
                {
                    sim_id = SRV_SMS_SIM_4;
                }
            #endif

                mmi_sms_class0_opt_save_to_inbox(sim_id);
            }
            break;
#endif
    #ifndef __UNIFIED_COMPOSER_SUPPORT__
        case MENU_ID_SMS_ED_OPT_SAVE_TO_DRAFTS:
            {
                ...
                if (MENU_ID_SMS_ED_OPT_SAVE_TO_DRAFTS_BY_SIM2 == highlighted_menu_id)
                {
                    sim_id = SRV_SMS_SIM_2;
                }
            #if (MMI_MAX_SIM_NUM >= 3)
                else if (MENU_ID_SMS_ED_OPT_SAVE_TO_DRAFTS_BY_SIM3 == highlighted_menu_id)
                {
                    sim_id = SRV_SMS_SIM_3;
                }
            #endif
            #if (MMI_MAX_SIM_NUM >= 4)
                else if (MENU_ID_SMS_ED_OPT_SAVE_TO_DRAFTS_BY_SIM4 == highlighted_menu_id)
                {
                    sim_id = SRV_SMS_SIM_4;
                }
            #endif
		...
		cui_menu_set_item_disabled(menu_gid, MENU_ID_SMS_LIST_ADV_OPT_COPY_ALL_FROM_TCARD_TO_SIM, MMI_FALSE);
						...
						#if(!defined(__UNIFIED_MESSAGE_SIMBOX_SUPPORT__) || defined(__OP01__) )
						{
							...
							sim_id = srv_sms_get_msg_sim_id(msg_id);
							{
								...
								if (!srv_sim_ctrl_is_available(mmi_sim))
								{
									cui_menu_set_item_disabled(menu_gid, MENU_ID_SMS_LIST_ADV_OPT_COPY_ALL_FROM_PHONE_TO_SIM, MMI_TRUE);
									cui_menu_set_item_disabled(menu_gid, MENU_ID_SMS_LIST_ADV_OPT_COPY_ALL_FROM_SIM_TO_PHONE, MMI_TRUE);
								...
								{
									cui_menu_set_item_disabled(menu_gid, MENU_ID_SMS_LIST_ADV_OPT_COPY_ALL_FROM_PHONE_TO_SIM, MMI_FALSE);
									cui_menu_set_item_disabled(menu_gid, MENU_ID_SMS_LIST_ADV_OPT_COPY_ALL_FROM_SIM_TO_PHONE, MMI_FALSE);
									cui_menu_set_item_disabled(menu_gid, MENU_ID_SMS_LIST_ADV_OPT_COPY_ALL_FROM_SIM_TO_TCARD, MMI_FALSE);
									cui_menu_set_item_disabled(menu_gid, MENU_ID_SMS_LIST_ADV_OPT_COPY_ALL_FROM_TCARD_TO_SIM, MMI_FALSE);	
								}
							}
					 ...
					 if (srv_fmgr_drv_is_accessible(srv_fmgr_drv_get_letter_by_type(SRV_FMGR_DRV_CARD_TYPE)))
					{
						...
						#if(!defined(__UNIFIED_MESSAGE_SIMBOX_SUPPORT__) || defined(__OP01__) )
						{
							...
							sim_id = srv_sms_get_msg_sim_id(msg_id);
							{
								...
								if (!srv_sim_ctrl_is_available(mmi_sim))
								{
									cui_menu_set_item_disabled(menu_gid, MENU_ID_SMS_LIST_ADV_OPT_MOVE_ALL_FROM_PHONE_TO_SIM, MMI_TRUE);
									cui_menu_set_item_disabled(menu_gid, MENU_ID_SMS_LIST_ADV_OPT_MOVE_ALL_FROM_SIM_TO_PHONE, MMI_TRUE);
								...
								{
									cui_menu_set_item_disabled(menu_gid, MENU_ID_SMS_LIST_ADV_OPT_MOVE_ALL_FROM_PHONE_TO_SIM, MMI_FALSE);
									cui_menu_set_item_disabled(menu_gid, MENU_ID_SMS_LIST_ADV_OPT_MOVE_ALL_FROM_SIM_TO_PHONE, MMI_FALSE);
									cui_menu_set_item_disabled(menu_gid, MENU_ID_SMS_LIST_ADV_OPT_MOVE_ALL_FROM_SIM_TO_TCARD, MMI_FALSE);
									cui_menu_set_item_disabled(menu_gid, MENU_ID_SMS_LIST_ADV_OPT_MOVE_ALL_FROM_TCARD_TO_SIM, MMI_FALSE);	
								}
							}
