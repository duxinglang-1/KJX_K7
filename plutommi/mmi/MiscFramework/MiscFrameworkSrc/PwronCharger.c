...
// baiwenlin 20131109  充电时，关闭雷达		
#if defined(CUSTOMER_NAME_B606)
	GPIO_ModeSetup(6,0);
	GPIO_InitIO(1, 6);
	GPIO_WriteIO(0, 6);	    
#endif /* CUSTOMER_NAME_B606 */
...
mmi_ret mmi_charbat_main_evt_hdlr(mmi_event_struct* para)
{
    ...
    switch (p->status)
    {
        ...
        {
		#ifdef __KJX_FUN__	//xb add 2016-10-18
			g_charbat_context.chargerLevel = BATTERY_LEVEL_6;
		#else
...
void BatteryStatusIndication(U8 battery_voltage)
{
    ...
    switch (battery_voltage)
    {
	...
	#if defined(__KJX_FUN__)	//xb add 2016-10-13 电池4格电量显示
	...
	#endif
        {
			...
			battery_grid_percentage = (100/(BATTERY_LEVEL_6 -BATTERY_LEVEL_0)) * (battery_voltage - BATTERY_LEVEL_0);
		#else
