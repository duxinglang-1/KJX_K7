...
mmi_sml_menu_struct g_sml_context_2;
#endif
...
mmi_ret mmi_sml_query_state_popup_callback(mmi_event_struct *param)
{
    ...
    switch (param->evt_id)
    {
        case EVT_ID_ALERT_QUIT:
            if (g_sml_common.menu_cui_id != 0)
            {
                cui_menu_close(g_sml_common.menu_cui_id);
                g_sml_common.menu_cui_id = 0;
            }
            break;
    }
...
void mmi_sml_init(void)
{
    ...
    for (i = SIM1; i <= SIM2; i++)
    {
        mmi_sml_switch_content_2(i);
        g_sml_context_p->operation_type = 0xFF;
        g_sml_context_p->selected_category = 0;
        g_sml_context_p->action_not_from_menu = 0;
        g_sml_context_p->autolock_nibble = 0xFF;
        mmi_sml_reset_input_buffer();
    }
    ...
    if (memcmp(ansii_ssc_string, SSC_SML_LOCK_CMD, strlen(SSC_SML_LOCK_CMD)) == 0)
    {
        ...
        else
        {
            ...
            if (strlen((S8*) ptr2) == 1 && g_sml_context_p->selected_category < SML_CAT_SIZE)
            {
                ...
                {
                #ifdef __MMI_DUAL_SIM_MASTER__
                    mmi_sml_display_popup_plus_delete_sim_selection_scrn((UI_string_type) GetString(STR_ID_SML_CAT_DISABLED), MMI_EVENT_FAILURE);
                #endif
 ...
 *****************************************************************************/
#ifdef __MMI_DUAL_SIM_MASTER__
void mmi_sml_power_on_query_rsp(void *info, int mod_src)
#else
...
#endif
{
    ...
    if (mod_src == MOD_L4C)
    {
        mmi_sml_switch_content_2(SIM1);
    }
    else if (mod_src == MOD_L4C_2)
    {
        mmi_sml_switch_content_2(SIM2);
    }
    else
    {
        MMI_ASSERT(0);
...
void mmi_sml_display_auto_locked_notification(void)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_DUAL_SIM_MASTER__
    if (g_sml_context.autolock_nibble == 0xFF || g_sml_context_2.autolock_nibble == 0xFF)
    {
        return; /* not ready for both SIM cards */
    }

    if (g_sml_context.autolock_nibble != 0 && (g_sml_context.autolock_nibble & 0x10) == 0x10 && g_sml_context_2.autolock_nibble == 0)
    {
        StartTimer(SML_AUTOLOCK_IDLE_NOTIFY_TIMER, 3000, mmi_sml_entry_only_sim1_auto_locked); /* SIM1 auto locked, SIM2 not work */
    }
    else if (g_sml_context.autolock_nibble == 0 && g_sml_context_2.autolock_nibble != 0 && (g_sml_context_2.autolock_nibble & 0x10) == 0x10)
    {
        StartTimer(SML_AUTOLOCK_IDLE_NOTIFY_TIMER, 3000, mmi_sml_entry_only_sim2_auto_locked); /* SIM2 auto locked, SIM1 not work */
    }
    else if (g_sml_context.autolock_nibble != 0 && (g_sml_context.autolock_nibble & 0x10) == 0x10 && g_sml_context_2.autolock_nibble != 0 && (g_sml_context_2.autolock_nibble & 0x10) != 0x10)
    {
        StartTimer(SML_AUTOLOCK_IDLE_NOTIFY_TIMER, 3000, mmi_sml_entry_only_sim1_auto_locked); /* SIM1 auto locked, SIM2 failed */
    }
    else if (g_sml_context.autolock_nibble != 0 && (g_sml_context.autolock_nibble & 0x10) != 0x10 && g_sml_context_2.autolock_nibble != 0 && (g_sml_context_2.autolock_nibble & 0x10) == 0x10)
    {
        StartTimer(SML_AUTOLOCK_IDLE_NOTIFY_TIMER, 3000, mmi_sml_entry_only_sim2_auto_locked); /* SIM2 auto locked, SIM1 failed */
    }
    else if (g_sml_context.autolock_nibble != 0 && (g_sml_context.autolock_nibble & 0x10) == 0x10 && g_sml_context_2.autolock_nibble != 0 && (g_sml_context_2.autolock_nibble & 0x10) == 0x10)
    {
        StartTimer(SML_AUTOLOCK_IDLE_NOTIFY_TIMER, 3000, mmi_sml_entry_sim1_sim2_auto_locked); /* SIM1 auto locked, SIM2 auto locked */
    }
    else
    {
        /* other cases */
...
module_type mmi_sml_get_message_dest_module(void)
{
    ...
    mmi_sml_menu_struct *sim1_p = &g_sml_context;
    mmi_sml_menu_struct *sim2_p = &g_sml_context_2;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if ((U32)(g_sml_context_p) == (U32)(sim1_p))
    {
        return MOD_L4C;
    }
    else if ((U32)(g_sml_context_p) == (U32)(sim2_p))
    {
        return MOD_L4C_2;
    }
    else
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_set_sim1_selection_callback
 * DESCRIPTION
 *  Callback function to SIM1 selected
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_sml_set_sim1_selection_callback(FuncPtr dialfunc)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_sml_common.sim1_callback = dialfunc;
}


/*****************************************************************************
 * FUNCTION
 *  mmi_sml_set_sim2_selection_callback
 * DESCRIPTION
 *  Callback function to SIM2 selected
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_sml_set_sim2_selection_callback(FuncPtr dialfunc)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_sml_common.sim2_callback = dialfunc;
}


/*****************************************************************************
 * FUNCTION
 *  mmi_sml_group_sims_proc
 * DESCRIPTION
 *  SIM-ME-Lock SIM selection screen group
 * PARAMETERS
 *  param
 * RETURNS
 *  mmi_ret
 *****************************************************************************/
mmi_ret mmi_sml_group_sims_proc(mmi_event_struct *param)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    cui_menu_event_struct *menu_select_event;

    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    menu_select_event = (cui_menu_event_struct*) param;
    switch (menu_select_event->evt_id)
    {
        case EVT_ID_CUI_MENU_ITEM_SELECT:
            if(menu_select_event->highlighted_menu_id == MENU_ID_SML_SIM1_SELECTED)
            {
                (*g_sml_common.sim1_callback) ();
            }
            else if(menu_select_event->highlighted_menu_id == MENU_ID_SML_SIM2_SELECTED)
            {
                (*g_sml_common.sim2_callback) ();
            }
            else
            {
        ...
        case EVT_ID_CUI_MENU_LIST_ENTRY:
            cui_menu_set_currlist_title(g_sml_common.menu_cui_id, (WCHAR*) get_string(STR_ID_SML_SELECT_SIM), NULL);
            cui_menu_set_item_image(g_sml_common.menu_cui_id, MENU_ID_SML_SIM1_SELECTED, gIndexIconsImageList[0]);
            cui_menu_set_item_image(g_sml_common.menu_cui_id, MENU_ID_SML_SIM2_SELECTED, gIndexIconsImageList[1]);
            break;
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_sim_selection_menu
 * DESCRIPTION
 *  Entry function to SIM selection menu
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_sml_entry_sim_selection_menu(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    group_id = mmi_frm_group_create(GRP_ID_ROOT, GRP_ID_SML_SIMS, mmi_sml_group_sims_proc, NULL);
    mmi_frm_group_enter(GRP_ID_SML_SIMS, MMI_FRM_NODE_SMART_CLOSE_FLAG);

    g_sml_common.menu_cui_id = cui_menu_create(group_id,
                                               CUI_MENU_SRC_TYPE_RESOURCE,
                                               CUI_MENU_TYPE_FROM_RESOURCE,
                                               MENU_ID_SML_SIM_SELECTION,
                                               MMI_FALSE,
                                               NULL);

    cui_menu_run(g_sml_common.menu_cui_id);
}


/*****************************************************************************
 * FUNCTION
 *  mmi_sml_switch_content_2
 * DESCRIPTION
 *  Content switch function for SIM1 or SIM2 global variable
 * PARAMETERS
 *  simInterface    [IN]
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_sml_switch_content_2(U8 simInterface)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        g_sml_context_p = &g_sml_context;
    }
    else if (simInterface == SIM2)
    {
        g_sml_context_p = &g_sml_context_2;
    }
    else
    {
        MMI_ASSERT(0);
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_lock_category_sim1_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
static void mmi_sml_entry_lock_category_sim1_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_lock_category_sim2_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_entry_lock_category_sim2_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_lock_category_sim_interface
 * DESCRIPTION
 *  SIM selection condition
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_sml_entry_lock_category_sim_interface(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (srv_sim_ctrl_is_available(MMI_SIM1))
    {
        ...
        {
            // case 4, 8 ,12
            mmi_sml_set_sim1_selection_callback(mmi_sml_entry_lock_category_sim1_selected);
            mmi_sml_set_sim2_selection_callback(mmi_sml_entry_lock_category_sim2_selected);
            mmi_sml_entry_sim_selection_menu();
        }
        else
        {
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_unlock_category_sim1_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_entry_unlock_category_sim1_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_unlock_category_sim2_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_entry_unlock_category_sim2_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_unlock_category_sim_interface
 * DESCRIPTION
 *  SIM selection condition
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void mmi_sml_entry_unlock_category_sim_interface(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (srv_sim_ctrl_is_available(MMI_SIM1))
    {
        ...
        {
            // case 4, 8 ,12
            mmi_sml_set_sim1_selection_callback(mmi_sml_entry_unlock_category_sim1_selected);
            mmi_sml_set_sim2_selection_callback(mmi_sml_entry_unlock_category_sim2_selected);
            mmi_sml_entry_sim_selection_menu();
        }
        else
        {
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_add_category_sim1_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_entry_add_category_sim1_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_add_category_sim2_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_entry_add_category_sim2_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_add_category_sim_interface
 * DESCRIPTION
 *  SIM selection condition
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void mmi_sml_entry_add_category_sim_interface(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (srv_sim_ctrl_is_available(MMI_SIM1))
    {
        ...
        {
            // case 4, 8 ,12
            mmi_sml_set_sim1_selection_callback(mmi_sml_entry_add_category_sim1_selected);
            mmi_sml_set_sim2_selection_callback(mmi_sml_entry_add_category_sim2_selected);
            mmi_sml_entry_sim_selection_menu();
        }
        else
        {
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_remove_category_sim1_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_entry_remove_category_sim1_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_remove_category_sim2_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_entry_remove_category_sim2_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_remove_category_sim_interface
 * DESCRIPTION
 *  SIM selection condition
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void mmi_sml_entry_remove_category_sim_interface(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (srv_sim_ctrl_is_available(MMI_SIM1))
    {
        ...
        {
            // case 4, 8 ,12
            mmi_sml_set_sim1_selection_callback(mmi_sml_entry_remove_category_sim1_selected);
            mmi_sml_set_sim2_selection_callback(mmi_sml_entry_remove_category_sim2_selected);
            mmi_sml_entry_sim_selection_menu();
        }
        else
        {
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_permanent_unlock_category_sim1_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_entry_permanent_unlock_category_sim1_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_permanent_unlock_category_sim2_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_entry_permanent_unlock_category_sim2_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_permanent_unlock_sim_interface
 * DESCRIPTION
 *  SIM selection condition
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void mmi_sml_entry_permanent_unlock_sim_interface(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (srv_sim_ctrl_is_available(MMI_SIM1))
    {
        ...
        {
            // case 4, 8 ,12
            mmi_sml_set_sim1_selection_callback(mmi_sml_entry_permanent_unlock_category_sim1_selected);
            mmi_sml_set_sim2_selection_callback(mmi_sml_entry_permanent_unlock_category_sim2_selected);
            mmi_sml_entry_sim_selection_menu();
        }
        else
        {
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_send_lock_req_sim1_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_send_lock_req_sim1_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_send_lock_req_sim2_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_send_lock_req_sim2_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_lock_req_sim_interface
 * DESCRIPTION
 *  SIM selection condition
 * PARAMETERS
 *  void
 ...
 *  U8
 *****************************************************************************/
U8 mmi_sml_entry_lock_req_sim_interface(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    U8 ansii_ssc_string[MAX_DIAL_PAD];
    U8 *ptr1, *ptr2 = NULL;
    U8 i, count = 0, len, category_value;
    ...
    if (memcmp(ansii_ssc_string, SSC_SML_LOCK_CMD, strlen(SSC_SML_LOCK_CMD)) == 0)
    {
        ...
        else if (strlen((S8*) ptr1) > MAX_SML_UNLOCK_KEY_LENGTH || strlen((S8*) ptr1) < MIN_SML_UNLOCK_KEY_LENGTH)
        {
            mmi_popup_display(
                get_string(STR_ID_SML_INVALID_LENGTH),
                MMI_EVENT_FAILURE,
        ...
        {
            category_value = ((U8) atoi((S8*) ptr2) - 1);
            if (strlen((S8*) ptr2) > 1 || category_value >= SML_CAT_SIZE)
            {
                return 0;
            }
            ...
            if (srv_sim_ctrl_is_available(MMI_SIM1))
            {
                ...
                {
                    // case 4, 8 ,12
                    mmi_sml_set_sim1_selection_callback(mmi_sml_send_lock_req_sim1_selected);
                    mmi_sml_set_sim2_selection_callback(mmi_sml_send_lock_req_sim2_selected);
                    mmi_sml_entry_sim_selection_menu();
                }
                else
                {
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_send_unlock_req_sim1_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_send_unlock_req_sim1_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_send_unlock_req_sim2_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_send_unlock_req_sim2_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_unlock_req_sim_interface
 * DESCRIPTION
 *  SIM selection condition
 * PARAMETERS
 *  void
 ...
 *  U8
 *****************************************************************************/
U8 mmi_sml_entry_unlock_req_sim_interface(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (memcmp(ansii_ssc_string, SSC_SML_UNLOCK_CMD, strlen(SSC_SML_UNLOCK_CMD)) == 0)
    {
        ...
        else
        {
            ...
            if (srv_sim_ctrl_is_available(MMI_SIM1))
            {
                ...
                {
                    // case 4, 8 ,12
                    mmi_sml_set_sim1_selection_callback(mmi_sml_send_unlock_req_sim1_selected);
                    mmi_sml_set_sim2_selection_callback(mmi_sml_send_unlock_req_sim2_selected);
                    mmi_sml_entry_sim_selection_menu();
                }
                else
                {
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_send_query_req_sim1_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_send_query_req_sim1_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_send_query_req_sim2_selected
 * DESCRIPTION
 *  SIM selection confirmation
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
static void mmi_sml_send_query_req_sim2_selected(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_query_req_sim_interface
 * DESCRIPTION
 *  SIM selection condition
 * PARAMETERS
 *  void
 ...
 *  U8
 *****************************************************************************/
U8 mmi_sml_entry_query_req_sim_interface(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (memcmp(ansii_ssc_string, SSC_SML_QUERY_CMD, strlen(SSC_SML_QUERY_CMD)) == 0)
    {
        ...
        {
            category_value = ((U8) atoi((S8*) ptr1) - 1);
            if (strlen((S8*) ptr1) > 1 || category_value >= SML_CAT_SIZE)
            {
                return 0;
            }
            ...
            if (srv_sim_ctrl_is_available(MMI_SIM1))
            {
                ...
                {
                    // case 4, 8 ,12
                    mmi_sml_set_sim1_selection_callback(mmi_sml_send_query_req_sim1_selected);
                    mmi_sml_set_sim2_selection_callback(mmi_sml_send_query_req_sim2_selected);
                    mmi_sml_entry_sim_selection_menu();
                }
                else
                {
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_only_sim1_auto_locked
 * DESCRIPTION
 *  Popup screen of auto lock notification
 * PARAMETERS
 *  void
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_sml_entry_only_sim1_auto_locked(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_sml_display_popup((UI_string_type) GetString(STR_ID_SML_SIM1_AUTO_LOCKED), MMI_EVENT_SUCCESS);
}


/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_only_sim2_auto_locked
 * DESCRIPTION
 *  Popup screen of auto lock notification
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void mmi_sml_entry_only_sim2_auto_locked(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_sml_display_popup((UI_string_type) GetString(STR_ID_SML_SIM2_AUTO_LOCKED), MMI_EVENT_SUCCESS);
}


/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_sim1_sim2_auto_locked
 * DESCRIPTION
 *  Popup screen of auto lock notification
 * PARAMETERS
 *  void
 ...
 *  void
 *****************************************************************************/
void mmi_sml_entry_sim1_sim2_auto_locked(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_sml_display_popup((UI_string_type) GetString(STR_ID_SML_SIM1_SIM2_AUTO_LOCKED), MMI_EVENT_SUCCESS);
}
...
/*****************************************************************************
 * FUNCTION
 *  mmi_sml_entry_sim2_auto_lock
 * DESCRIPTION
 *  CPHS handler for SIM2 auto lock
 * PARAMETERS
 *  info            [IN]
 * RETURNS
 *  void
 *****************************************************************************/
void mmi_sml_entry_sim2_auto_lock(void *info)
{
#ifdef __MMI_SML_MENU__
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
...
void mmi_sml_report_sim_error(mmi_sim_enum sim)
{
    ...
    {
        g_sml_context_2.autolock_nibble = 0;
    }
#endif /* __MMI_DUAL_SIM_MASTER__ */
    else
    {
