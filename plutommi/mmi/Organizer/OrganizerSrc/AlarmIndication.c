    ...
    if (!g_alm_cntx.is_tone_playing)
    {
         ...
         {        
...
#endif
           {
		    ...
		    MMI_ALM_TRACE1(MMI_ALM_PLAY_TONE, 15);
                     ret = srv_prof_get_current_profile_value(SRV_PROF_SETTINGS_RING_VOL_LEVEL, &vol_level);
                     if (ret == SRV_PROF_RET_SUCCESS)
                     {
                     	MMI_ALM_TRACE1(MMI_ALM_PLAY_TONE, 16);
                         mmi_alm_aud_fmrdo_play_alarm(alarm_index, vol_level);
                     }                    
                 }
