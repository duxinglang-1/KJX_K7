...
static mmi_ret mmi_tdl_hs31_proc(mmi_event_struct *evt);
#endif
...
#if defined(__MMI_INDICAL__ ) || defined(__MMI_HIJRI_CALENDAR__)
    SetKeyHandler(mmi_clndr_switch_clndr_type, KEY_STAR, KEY_EVENT_UP);
#endif /* defined(__MMI_INDICAL__ ) || defined(__MMI_HIJRI_CALENDAR__) */
}
...
void mmi_clndr_switch_clndr_type(void)
{
    ...
    WriteValue(LUNAR_CALENDAR, &(g_clndr_cntx->lunar_state), DS_BYTE, &error);
    WriteValue(NVRAM_CLNDR_INFO_VAL, &(g_clndr_cntx->infodispval), DS_BYTE, &error);
                                                    ...
                                                    S32 *items)
{
...
#else
    if (view_mode > MMI_VIEW_MODE_WEEK)
#endif
    {
        return;
    }
    ...
    /* If not todolist, don't show new meeting button */
#ifdef __MMI_CALENDAR_EVENT__
    *items = 3;
#else
    *items = 2;
...
static void mmi_clndr_display_set_config_2(void)
{
...
#if defined (__MMI_INDICAL__) || defined (__MMI_HIJRI_CALENDAR__) 
    /* Set LunarState */
    ReadValue(LUNAR_CALENDAR, (void*)&g_clndr_cntx->lunar_state, DS_BYTE, &error);
    ...
    ReadValue(NVRAM_CLNDR_INFO_VAL, &(g_clndr_cntx->infodispval), DS_BYTE, &error);
#endif /* defined (__MMI_INDICAL__) || defined (__MMI_HIJRI_CALENDAR__) */

#ifdef	__MMI_CALENDAR_FDOW__
    ReadValue(NVRAM_CLNDR_FIRST_DAY_OF_WEEK, &g_clndr_cntx->first_day_of_week, DS_BYTE, &error);
#endif
}
...
static void mmi_clndr_first_day_save_done(void)
{
    ...
    WriteValue(NVRAM_CLNDR_FIRST_DAY_OF_WEEK, &(g_clndr_cntx->first_day_of_week), DS_BYTE, &error);
}
...
static mmi_ret mmi_tdl_hs31_proc(mmi_event_struct *evt)
{
    ...
    switch(evt->evt_id)
    {
        case EVT_ID_GROUP_FIRST_ENTRY:
            break;
        case EVT_ID_GROUP_DEINIT:
            mmi_tdl_set_type(MMI_TDL_VCAL_TYPE_TOTAL);
            break;
        case EVT_ID_TDL_SAVE_FAIL:
        case EVT_ID_TDL_SAVE_CANCEL:
        case EVT_ID_TDL_SAVE_SUCCESS:
            ...
            cui_cal_save_close(tdl_save_evt->sender_id);
            mmi_frm_group_close(GRP_ID_TDL_HOME_SCREEN);
            break;
        default:
            break;
    }
 ...
 * DESCRIPTION
 *  allocate and set calendar information
 * PARAMETERS
 *  DateStr     [?]     
 *  day         [?]     
 *  month       [?]     
 ...
 *  void
 *****************************************************************************/
void mmi_clndr_op11_hs32_entry_month(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (mmi_syncml_is_calendar_sync_now())
    {
        mmi_popup_display_simple_ext(STR_ID_SYNC_CODE_FORBIDDEN, MMI_EVENT_FAILURE, GRP_ID_ROOT, NULL);
        return;
    }
#endif /* __MMI_SYNCML_SYNC_CALENDAR_SUPPORT__ */

    mmi_frm_group_create_ex(GRP_ID_ROOT, GRP_ID_TDL_HOME_SCREEN, mmi_tdl_hs31_proc, NULL,MMI_FRM_NODE_SMART_CLOSE_FLAG);  
    mmi_clndr_launch(GRP_ID_TDL_HOME_SCREEN, MMI_CAL_LAUNCH_TYPE_MONTHLY, NULL);
}
 ...
 *  void
 *****************************************************************************/
void mmi_clndr_op11_hs32_entry_thweek(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    mmi_frm_group_create_ex(GRP_ID_ROOT, GRP_ID_TDL_HOME_SCREEN, mmi_tdl_hs31_proc, NULL,MMI_FRM_NODE_SMART_CLOSE_FLAG);   
    mmi_clndr_launch(GRP_ID_TDL_HOME_SCREEN, MMI_CAL_LAUNCH_TYPE_WEEKLY, NULL);
}
 ...
 *  void
 *****************************************************************************/
void mmi_clndr_op11_hs32_entry_today(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    mmi_frm_group_create_ex(GRP_ID_ROOT, GRP_ID_TDL_HOME_SCREEN, mmi_tdl_hs31_proc, NULL, MMI_FRM_NODE_SMART_CLOSE_FLAG);   
    mmi_clndr_launch(GRP_ID_TDL_HOME_SCREEN, MMI_CAL_LAUNCH_TYPE_DAILY, NULL);
}
 ...
 *  void
 *****************************************************************************/
void mmi_clndr_op11_hs32_entry_add(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* 
     * Read display type from NVRAM, 
     * get the current time as highlighted time in calendar. 
     */
    mmi_clndr_display_set_config_1(MMI_CAL_LAUNCH_TYPE_AUTO, NULL); 
    mmi_tdl_set_type(SRV_TDL_VCAL_EVENT);
    //mmi_clndr_prepare_for_add();
    cui_cal_save_gid = cui_cal_save_create(mmi_frm_group_get_active_id());
    cui_cal_save_set_start_time(cui_cal_save_gid, &curr_time);
    cui_cal_save_set_end_time(cui_cal_save_gid, &curr_time);
    cui_cal_save_set_operation_type(cui_cal_save_gid, MMI_TDL_OP_TYPE_ADD);
    cui_cal_save_set_title(cui_cal_save_gid, STR_GLOBAL_ADD);
    cui_cal_save_run(cui_cal_save_gid);
}
#endif /* __MMI_CALENDAR_EVENT__ */
 ...
 *  void
 *****************************************************************************/
void mmi_clndr_create_calendar_on_idle(void)
{
	/*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/    
...
/*****************************************************************************
 * FUNCTION
 *  mmi_clndr_monthly_movement_on_idle
 * DESCRIPTION
 *  set calendar prev month information for idle screen
 * PARAMETERS
 *  DateStr     [?]     
 *  day         [?]     
 *  month       [?]     
 ...
 *  void
 *****************************************************************************/
static void mmi_clndr_monthly_movement_on_idle(U32 type)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    applib_time_struct inc_time, result_time;
    
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    memset(&inc_time, 0, sizeof(applib_time_struct));
    memset(&result_time, 0, sizeof(applib_time_struct));
    ...
    switch (type)
    {
        case CLNDR_PRIVOUS_MONTH:
            ...
            applib_dt_decrease_time(
                (applib_time_struct*)&(g_clndr_cntx->date_on_idle),
    ...
    if (result_time.nYear > CLNDR_END_YEAR || result_time.nYear < CLNDR_START_YEAR)
    {
        return;
    }
    
    memcpy(&g_clndr_cntx->date_on_idle, &result_time, sizeof(applib_time_struct));
}
 ...
 * DESCRIPTION
 *  Goto calendar prev month information for idle screen
 * PARAMETERS
 *  DateStr     [?]     
 *  day         [?]     
 *  month       [?]     
 ...
 *  void
 *****************************************************************************/
void mmi_clndr_monthly_prev_on_idle(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_clndr_monthly_movement_on_idle(CLNDR_PRIVOUS_MONTH);    
}
 ...
 * DESCRIPTION
 *  Goto calendar next month information for idle screen
 * PARAMETERS
 *  DateStr     [?]     
 *  day         [?]     
 *  month       [?]     
 ...
 *  void
 *****************************************************************************/
void mmi_clndr_monthly_next_on_idle(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_clndr_monthly_movement_on_idle(CLNDR_NEXT_MONTH);   
}
 ...
 *  mmi_clndr_set_calendar_on_idle
 * DESCRIPTION
 *  set calendar information for idle screen
 * PARAMETERS
 *  DateStr     [?]     
 *  day         [?]     
 *  month       [?]     
 ...
 *  void
 *****************************************************************************/
void mmi_clndr_set_calendar_on_idle(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    memcpy(&g_clndr_cntx->date_on_idle, &rtc_time, sizeof(applib_time_struct));
}
 ...
 *  void
 *****************************************************************************/
void mmi_clndr_exit_set_calendar_on_idle(void)
{
}
...
/*****************************************************************************
 * FUNCTION
 *  mmi_clndr_is_monthly_prev_move_enable
 * DESCRIPTION
 *  set calendar information for idle screen
 * PARAMETERS
 *  DateStr     [?]     
 ...
 *  void
 *****************************************************************************/
MMI_BOOL mmi_clndr_is_monthly_prev_move_enable(void)
{
    if (g_clndr_cntx->date_on_idle.nMonth == 1 && 
        g_clndr_cntx->date_on_idle.nYear == CLNDR_START_YEAR)
    {
        return MMI_FALSE;
    }
    return MMI_TRUE;   
...
/*****************************************************************************
 * FUNCTION
 *  mmi_clndr_is_monthly_next_move_enable
 * DESCRIPTION
 *  set calendar information for idle screen
 * PARAMETERS
 *  DateStr     [?]     
 ...
 *  void
 *****************************************************************************/
MMI_BOOL mmi_clndr_is_monthly_next_move_enable(void)
{
    if (g_clndr_cntx->date_on_idle.nMonth == 12 && 
        g_clndr_cntx->date_on_idle.nYear == CLNDR_END_YEAR)
    {
        return MMI_FALSE;
    }
    return MMI_TRUE;  
