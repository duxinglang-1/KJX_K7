...
#if defined( __MMI_OP11_HOMESCREEN_0301__) || defined(__MMI_OP11_HOMESCREEN_0302__)
static void mmi_tdl_op11_hs32_search_weekly_pending_data(
                                srv_tdl_event_task_struct * const list, 
                                U16 length, 
                                S32 *pending_count);
static void mmi_tdl_op11_hs32_search_daily_pending_data(
                                srv_tdl_event_task_struct * const list, 
                                U16 length, 
                                S32 *pending_count);                                
static void mmi_tdl_op11_hs32_fill_shct_data(
                                U16 type,
                                srv_tdl_event_task_struct * const list, 
                                U16 length, 
                                void *shct_data);                    
...
/*****************************************************************************
 * FUNCTION
 *  mmi_tdl_op11_hs32_query_sidebar_info
 * DESCRIPTION
 *  This function is used to answer sidebar information
 * PARAMETERS
 *  sidebar_id  :[OUT]
 *  indctr_id   :[IN]
 * RETURNS
 ...
 *****************************************************************************/
void mmi_tdl_op11_hs32_query_sidebar_info(S32 sidebar_id, S32 indctr_id)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if(SRV_TDL_RESULT_OK == error_code)
    {
        mmi_tdl_op11_hs32_search_daily_pending_data(sorted_list, total, &pending_count);
        data.result_flag = MMI_OP11_HS32_RESULT_OK;
        data.indicator_value = pending_count;    /* today's event count */        
    }
    else
    {
 ...
 * DESCRIPTION
 *  This function is used to get sidebar information
 * PARAMETERS
 *  sidebar_id  :[OUT]
 *  indctr_id   :[IN]
 * RETURNS
 *  MMI_BOOL    MMI_FALSE means there is no next alarm
 *****************************************************************************/
void mmi_tdl_op11_hs32_update_sidebar_info(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    mmi_op11_hs32_answer_sidebar(MMI_OP11_HS32_AP_CAL, 0, &data);
}
 ...
 * DESCRIPTION
 *  This function is used to get today event information
 * PARAMETERS
 *  sidebar_id  [IN]:   short cut sidebar info from home screen.
 *  shct_id     [IN]:   short cut id from home screen
 *  shct_data   [IN]:   short cut data
 * RETURNS
 *  MMI_BOOL    MMI_FALSE means there is no next alarm
 *****************************************************************************/
void mmi_tdl_op11_hs32_get_today_shct_data(S32 sidebar_id,S32 shct_id, void *shct_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    mmi_tdl_op11_hs32_fill_shct_data(search_info.type, sorted_list, total, shct_data);
 ...
 *  MMI_BOOL    MMI_FALSE means there is no next alarm
 *****************************************************************************/
void mmi_tdl_op11_hs32_update_today_shct_data(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    shct_data_p = mmi_op11_hs32_get_shct_data_pointer(MMI_OP11_HS32_AP_CAL, MMI_OP11_HS32_SHCT_CAL_CL);
    if (shct_data_p != NULL)
    {
        ...
        mmi_tdl_op11_hs32_fill_shct_data(search_info.type, sorted_list, total, shct_data_p);
        shct_data_p->result_flag = MMI_OP11_HS32_RESULT_UPDATE;        
        mmi_op11_hs32_answer_shct_data(MMI_OP11_HS32_AP_CAL, MMI_OP11_HS32_SHCT_CAL_CL, shct_data_p);
    }
}
 ...
 *  MMI_BOOL    MMI_FALSE means there is no next alarm
 *****************************************************************************/
void mmi_tdl_op11_hs32_get_thweek_shct_data(S32 sidebar_id, S32 shct_id, void *shct_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    GetDateTime(&search_info.u.weekly_search.date_time);
 ...
 * DESCRIPTION
 *  This function is used to update sidebar information
 * PARAMETERS
 *  sidebar_id  :[OUT]
 *  indctr_id   :[IN]
 * RETURNS
 *  MMI_BOOL    MMI_FALSE means there is no next alarm
 *****************************************************************************/
void mmi_tdl_op11_hs32_update_thweek_shct_data(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    shct_data_p = mmi_op11_hs32_get_shct_data_pointer(MMI_OP11_HS32_AP_CAL, MMI_OP11_HS32_SHCT_CAL_TW);
    if (shct_data_p != NULL)
    {
        GetDateTime(&search_info.u.weekly_search.date_time);
        search_info.u.weekly_search.start_day_of_week = SRV_TDL_START_DAY_SUN;
        ...
        mmi_tdl_op11_hs32_fill_shct_data(search_info.type, sorted_list, total, shct_data_p);
        shct_data_p->result_flag = MMI_OP11_HS32_RESULT_UPDATE;        
        mmi_op11_hs32_answer_shct_data(MMI_OP11_HS32_AP_CAL, MMI_OP11_HS32_SHCT_CAL_TW, shct_data_p);
    }
}
...
/*****************************************************************************
 * FUNCTION
 *  mmi_tdl_op11_hs32_search_pending_data
 * DESCRIPTION
 *  This function is used to get pending event information
 * PARAMETERS
 *  list      : [IN] The list to be sorted and get the pending data.
 *  length    : [IN] The data count of list.
 *  count     : [OUT] The pending count.
 * RETURNS
 *****************************************************************************/
static void mmi_tdl_op11_hs32_search_weekly_pending_data(
                    srv_tdl_event_task_struct * const list, 
                    ...
                    S32 *pending_count)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    srv_tdl_sort(list, length, SRV_TDL_SORT_TYPE_ENTRY_TIME);
                    ...
                    S32 *pending_count)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    srv_tdl_sort(list, length, SRV_TDL_SORT_TYPE_TODAY_TIME);
    
    GetDateTime(&curr_time);
    
    for (start_index = 0; start_index < length; start_index++)
    {
        if (list[start_index].type == SRV_TDL_VCAL_EVENT)
        {
            pevent = mmi_tdl_get_short(SRV_TDL_VCAL_EVENT, list[start_index].index);
            memcpy(&time_to_change, &pevent->start_time, sizeof(MYTIME));
            srv_tdl_get_time_in_today(&time_to_change, pevent->repeat.rule);
        }
        else
        {
            ptask = mmi_tdl_get_short(SRV_TDL_VCAL_TASK, list[start_index].index);
            memcpy(&time_to_change, &ptask->due_time, sizeof(MYTIME));            
            srv_tdl_get_time_in_today(&time_to_change, ptask->repeat.rule);
        }
              
        result = applib_dt_compare_time(
                        (applib_time_struct*)&time_to_change,
    ...
    *pending_count = length - start_index;
}


/*****************************************************************************
 * FUNCTION
 *  mmi_tdl_op11_hs32_fill_shct_data
 * DESCRIPTION
 *  This function is used to get pending event information
 * PARAMETERS
 *  list      : [IN] The list fill the pending data.
 *  length    : [IN] The data count of list.
 *  count     : [OUT] The pending count.
 * RETURNS
 *****************************************************************************/
                    ...
                    void *shct_data)
{
    ...
    if (type == SRV_TDL_SEARCH_TYPE_WEEKLY)
    {
        mmi_tdl_op11_hs32_search_weekly_pending_data(list, length, &pending_count);
    }
    else if (type == SRV_TDL_SEARCH_TYPE_DAILY)
    {
        mmi_tdl_op11_hs32_search_daily_pending_data(list, length, &pending_count);
    }
            
    /* today's event count, if no event assign 0 */
    shct_data_p->item_count = pending_count;
    
    n = MIN(shct_data_p->item_count, MMI_OP11_HS32_SHCT_ITEM_COUNT_MAX);
    
    /* the start index in sorted_list to get data */
    start_index = length - pending_count;
    ...
    for(i = 0; i < n; i++)
    {
        srv_tdl_get(
                ...
                sizeof(srv_tdl_event_struct), 
                SRV_TDL_VCAL_EVENT, 
                list[start_index + i].index);
                
        /* event/task/...  unique id, now just for view latest event, but future extensibility */
        shct_data_p->item_uid[i] = list[start_index + i].index;
        
        /* event/task/...  flag, event: 0, task: 1, others: ... */
        shct_data_p->item_flag[i] = (U8)list[start_index + i].type;
        
        /* event title */
        mmi_ucs2ncpy(
                (CHAR*)shct_data_p->item_name[i], 
                (CHAR*)pevent->subject, 
                MAX_TODO_NOTE_LEN);
                
        /* event description */
        mmi_ucs2ncpy(
                (CHAR*)shct_data_p->item_content[i], 
                (CHAR*)pevent->details, 
                MMI_OP11_HS32_SHCT_CONTENT_STRLEN_MAX);
    
        /* event start time */
        shct_data_p->item_time[i].nYear = pevent->start_time.nYear; 
        shct_data_p->item_time[i].nMonth = pevent->start_time.nMonth;
        shct_data_p->item_time[i].nDay = pevent->start_time.nDay;
        shct_data_p->item_time[i].nHour = pevent->start_time.nHour;
        shct_data_p->item_time[i].nMin = pevent->start_time.nMin;
        shct_data_p->item_time[i].nSec = pevent->start_time.nSec;
        shct_data_p->item_time[i].DayIndex = pevent->start_time.DayIndex;
    }
