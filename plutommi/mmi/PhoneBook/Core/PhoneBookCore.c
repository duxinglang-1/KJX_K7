...
U32 mmi_phb_cntx_mgr_create_handle(U16 sg_type)
{
     ...
     */
    g_phb_cntx_mgr.sg_stack[sg_handle] = (mmi_phb_sg_data_struct*)srv_phb_mem_malloc(sizeof(mmi_phb_sg_data_struct), SRV_PHB_MEMORY_TYPE_ADM);
...
void mmi_phb_cntx_mgr_release_handle(U32 handle)
{
    ...
    if (instance != NULL)
    {
        ...
        {
            srv_phb_mem_free(instance->delay_event);
            instance->delay_event = NULL;
        }
        
        srv_phb_mem_free(g_phb_cntx_mgr.sg_stack[handle]);
    }
...
void mmi_phb_cntx_mgr_post_evt(mmi_id sg_id, MMI_BOOL evt_wait, mmi_event_struct *evt)
{
    ...
    if (evt_wait)
    {
        ...
        instance->delay_event = srv_phb_mem_malloc(evt->size, SRV_PHB_MEMORY_TYPE_ADM);
...
//    mmi_phb_delete_enter_pin2_screen();
    ...
    mmi_vrsd_train_delete_option_scr(SCR_ID_VRSD_DIAL_MAIN_MENU);
#endif
...
void mmi_phb_cntx_mgr_at(U8 operation)
{
    ...
    else
    {
        if ((0)
        #if defined(__MMI_OP11_HOMESCREEN_0301__) || defined(__MMI_OP11_HOMESCREEN_0302__)
            || (mmi_idle_is_active() && mmi_op11_hs32_is_enable())
            || (GetActiveScreenId() == SCR_ID_OP11_HS_SET_VIP)
            || (GetActiveScreenId() == SCR_ID_OP11_HS_EDIT_VIP)
            || (GetActiveScreenId() == SCR_ID_OP11_HS_VIP_OPTION)
            || (GetActiveScreenId() == SCR_ID_OP11_HS_US_VIP)
            || (mmi_frm_group_is_present(GRP_ID_PHB_VIP_CONTACT))
...
#if defined(__MMI_VOIP__)
    mmi_phb_wcsncpy(mmi_contact->voip, srv_contact->voip, VOIP_URI_LEN - 1);
#endif
...
srv_phb_contact_info_struct *mmi_phb_get_contact_build_buffer(mmi_phb_contact_info_struct* contact)
{
    ...
    contact_info = (srv_phb_contact_info_struct *) srv_phb_mem_malloc(sizeof(srv_phb_contact_info_struct), SRV_PHB_MEMORY_TYPE_ADM);
...
mmi_phb_contact_info_struct* mmi_phb_contact_get_contact(U16 store_index)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    contact_info = (mmi_phb_contact_info_struct *) srv_phb_mem_malloc(sizeof(mmi_phb_contact_info_struct), SRV_PHB_MEMORY_TYPE_ADM);
