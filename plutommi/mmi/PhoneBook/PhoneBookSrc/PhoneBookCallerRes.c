...
void mmi_phb_entry_choose_caller_ringtone(void)
{
    ...
    {
        cui_tone_selector_set_filter(profile_gid, CUI_TONE_SELECTOR_FILTER_SILENT | CUI_TONE_SELECTOR_FILTER_TONE | CUI_TONE_SELECTOR_FILTER_RING | CUI_TONE_SELECTOR_FILTER_EXT_FILE);
        ...
        cui_tone_selector_set_highlight_tone(profile_gid, 
                g_mmi_phb_resource.ring_tone_list[g_mmi_phb_res_p->selected_ring_index]);
...
void mmi_phb_entry_choose_caller_pic(void)
{
    ...
    /* show choose pic screen, contain default picture */
    ...
    if (!mmi_fmgr_util_file_limit_check(FMGR_LIMIT_CALLER_PICTURE, path, &error_str_ptr))
    {
        if (auto_resize)
        {
            ...
            if (buf_ptr != NULL)
            {
                GDI_RESULT gdi_ret;
                WCHAR* tmp_path = 
                    srv_phb_mem_malloc((SRV_FMGR_PATH_MAX_LEN + 1)* sizeof(WCHAR), SRV_PHB_MEMORY_TYPE_CTR);
...
void mmi_phb_build_video_list(void)
{
    ...
    {
        g_mmi_phb_resource.video_name_list[++count] = (U8*)GetString((U16)(STR_ID_PHB_SWFLASH_1+i));
        g_mmi_phb_resource.video_id_list[count] = MFH_ID_PHB_MTCALL_1 + i;
...
#ifdef __MMI_SWFLASH__
    /* Select From SWFlash */
    g_mmi_phb_resource.video_name_list[++count] = (U8*)GetString(STR_ID_PHB_CALLER_VIDEO_FROM_SWFLASH);
    g_mmi_phb_resource.video_id_list[count] = 0xA000;
    g_mmi_phb_resource.total_video_id++;
#endif /* __MMI_SWFLASH__ */
...
U8 mmi_phb_get_video_index(U16 video_id)
{
    ...
    if (video_id & 0x8000)
    {
        ...
        if (video_id & 0x2000)
        {
            return (U8)(g_mmi_phb_resource.total_video_id - g_phb_swflash_video_minus);
        }
    #endif /* __MMI_SWFLASH__ */
...
#ifdef __MMI_SWFLASH__
    else if (index == (g_mmi_phb_resource.total_video_id - g_phb_swflash_video_minus))
    {
        if (g_mmi_phb_res_p->video_location == MMI_PHB_SWFLASH_SELECT_PATH
            || g_mmi_phb_res_p->video_location == MMI_PHB_IMAGE_FOR_ENTRY
            || g_mmi_phb_res_p->video_location == MMI_PHB_IMAGE_CALLER_GROUP)
        {
            ChangeLeftSoftkey(STR_GLOBAL_OPTIONS, IMG_GLOBAL_OPTIONS);
        ...
        else
        {
            ...
            ChangeCenterSoftkey(0, IMG_NONE);
            SetCenterSoftkeyFunction(mmi_phb_swflash_select_from_file_mamager, KEY_EVENT_UP);
        }
    }
...
void mmi_phb_entry_choose_caller_video(void)
{
    if (g_mmi_phb_res_p->sg_id != GRP_ID_PHB_GROUP)
    {
        ...
        #ifdef __MMI_SWFLASH__
            subMenuDataPtrs[g_mmi_phb_resource.total_video_id - g_phb_swflash_video_minus] = (U8*)GetString(STR_ID_PHB_CALLER_VIDEO_FROM_SWFLASH);
        #endif
    ...
    #ifdef __MMI_SWFLASH__
        else if (g_mmi_phb_res_p->selected_video_index == (g_mmi_phb_resource.total_video_id - g_phb_swflash_video_minus))
        {
            mmi_phb_swflash_select_from_file_mamager();
        }
    #endif
        else
        {
...
void mmi_phb_entry_video_view_ext(MMI_ID group_id)
{
    ...
    if ((g_mmi_phb_res_p->selected_video_index == (g_mmi_phb_resource.total_video_id - 1))
    #ifdef __MMI_SWFLASH__
        || (g_mmi_phb_res_p->selected_video_index == (g_mmi_phb_resource.total_video_id - g_phb_swflash_video_minus))
    #endif /* __MMI_SWFLASH__ */
        )
    {
...
void mmi_phb_video_delete_file_reference(MMI_ID group_id)
{
    ...
    #ifdef __MMI_SWFLASH__
        g_mmi_phb_resource.video_name_list[g_mmi_phb_resource.total_video_id - g_phb_swflash_video_minus] =
            (U8*) GetString(STR_ID_PHB_CALLER_VIDEO_FROM_SWFLASH);
    #endif /* __MMI_SWFLASH__ */
...
void mmi_phb_video_view_from_file(mmi_scrn_essential_struct *scrn_data)
{
    ...
    #ifdef __MMI_SWFLASH__
        || (g_mmi_phb_res_p->selected_video_index == g_mmi_phb_resource.total_video_id - g_phb_swflash_video_minus
            && g_mmi_phb_res_p->video_location == MMI_PHB_SWFLASH_SELECT_PATH)
    #endif /* __MMI_SWFLASH__ */
       ) 
    {
        {
            ...
            if (file_handle >= FS_NO_ERROR)
            {
                ...
                #ifdef __MMI_SWFLASH__
                    if (g_mmi_phb_res_p->selected_video_index == g_mmi_phb_resource.total_video_id - g_phb_swflash_video_minus)
                    {
                        ShowCategory229Screen(
                            (U8*)GetString(STR_GLOBAL_PLAY),/* title_str */
                            IMG_SCR_PBOOK_CAPTION,	        /* title_icon */
...
/*****************************************************************************
* FUNCTION
*  mmi_phb_swflash_select_from_file_mamager_done
* DESCRIPTION
*
* PARAMETERS
*  void
...
void mmi_phb_swflash_select_from_file_mamager_done(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    res = (mmi_phb_caller_res_struct *) mmi_frm_scrn_get_user_data(sg_id, SCR_ID_PHB_SWFLASH_PREVIEW);
    
	mmi_wcscpy(res->video_path, res->video_temp_path);
    
	res->video_location = MMI_PHB_SWFLASH_SELECT_PATH;
    res->selected_video_index = g_mmi_phb_resource.total_video_id - g_phb_swflash_video_minus;
    g_mmi_phb_resource.video_name_list[res->selected_video_index] =
        (U8*) GetString(STR_ID_PHB_CALLER_VIDEO_SWFLASH_SELECTED);
    ...
    mmi_frm_scrn_close(sg_id, SCR_ID_PHB_TAB_EDIT_CALLER_VIDEO);
#endif
    mmi_frm_scrn_close(sg_id, SCR_ID_PHB_SWFLASH_PREVIEW);
}
...
* DESCRIPTION
*   play swflash callback function
* PARAMETERS
*  result   	   [IN]		play result     
* RETURNS
*  void
...
void mmi_phb_swflash_play_callback(S32 result)
{
    ...
    else
    {
        ...
        if (!mmi_fmgr_util_file_limit_check(FMGR_LIMIT_CALLER_VIDEO, (CHAR*)g_mmi_phb_res_p->video_temp_path, &error_str_ptr))
        {
        ...
        mmi_frm_scrn_close(g_mmi_phb_res_p->sg_id, SCR_ID_PHB_SWFLASH_PREVIEW);
        SetLeftSoftkeyFunction(UI_dummy_function,KEY_EVENT_UP);
    }
...
* DESCRIPTION
*  preview select swflash file
* PARAMETERS
*  void
* RETURNS
*  void
*****************************************************************************/
static void mmi_phb_swflash_preview(mmi_scrn_essential_struct* scrn_data)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
    /**
     * Can not preview video during call
     */
    if (srv_ucm_query_call_count(SRV_UCM_CALL_STATE_ALL, SRV_UCM_CALL_TYPE_NO_CSD, NULL) > 0)
    {
    #if defined(__MMI_BACKGROUND_CALL__)
        if (mmi_frm_is_in_backward_scenario())
        {
            if (mmi_frm_scrn_enter(
                    ...
                    NULL,
                    (FunctionPtr)mmi_phb_swflash_preview,
                    MMI_FRM_FULL_SCRN))
            {
                gui_buffer = mmi_frm_scrn_get_active_gui_buf();
...
/*****************************************************************************
* FUNCTION
*  mmi_phb_swflash_select_path_from_file_mamager
* DESCRIPTION
*  select swflash from filemgr callback function
* PARAMETERS
*  path                             [IN]
*  is_short                        [IN]
* RETURNS
*  void
*****************************************************************************/
void mmi_phb_swflash_select_path_from_file_mamager(mmi_phb_caller_res_struct *res)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
void mmi_phb_contact_res_config(void)
{
    ...
    if (g_mmi_phb_res_p->video_location != MMI_PHB_IMAGE_NO_SELECT)       /* Change Inline select prompt message. */
    {
        ...
        else if (g_mmi_phb_res_p->video_location == MMI_PHB_IMAGE_SELECT_PATH)
        ...
        mmi_phb_caller_res_struct *caller_res)
{
    ...
    #ifdef __MMI_SWFLASH__
        && (caller_res->selected_video_index != g_mmi_phb_resource.total_video_id - g_phb_swflash_video_minus)
    #endif
        )
    {
...
#ifdef __MMI_SWFLASH__
    else if ((caller_res->selected_video_index == g_mmi_phb_resource.total_video_id - g_phb_swflash_video_minus)
        && (caller_res->video_location != MMI_PHB_SWFLASH_SELECT_PATH))
    {
        caller_res->selected_video_index = 0;
        caller_res->video_location = MMI_PHB_IMAGE_NO_SELECT;
    }
...
void mmi_phb_fmgr_cui_res_handle(cui_file_selector_result_event_struct *file_select)
{
    ...
    if (file_select->result > 0)
    {
        ...
        #ifdef __MMI_SWFLASH__
            case MMI_PHB_RES_SWFLASH:
            {
                mmi_phb_swflash_select_path_from_file_mamager(res);
                break;
            }
        #endif
        #endif
...
void mmi_phb_fmgr_pre_edit_callergrp(void)
{
    ...
    #ifdef __MMI_SWFLASH__
        else if (res->recv_res_from == MMI_PHB_RECV_RES_SWFLASH)
        {
            item_index = g_mmi_phb_resource.total_video_id - g_phb_swflash_video_minus;
            video_audio = 1; /* Turn on video's audio by default */
        }
    #endif /* __MMI_SWFLASH__ */
...
void mmi_phb_fmgr_pre_edit_entry(U16 store_index)
{
    ...
    info = (srv_phb_contact_info_struct*) srv_phb_mem_malloc(sizeof(srv_phb_contact_info_struct), SRV_PHB_MEMORY_TYPE_CTR);

    info->field_mask = 0;
    info->field_data.res_type = 0;

    path = (U16*) srv_phb_mem_malloc(sizeof(U16) * (SRV_FMGR_PATH_MAX_LEN + 1), SRV_PHB_MEMORY_TYPE_CTR);
    ...
    else if (res->recv_res_from == MMI_PHB_RECV_RES_SWFLASH)
    {
        ...
        res_id = g_mmi_phb_resource.video_id_list[g_mmi_phb_resource.total_video_id - g_phb_swflash_video_minus];
        mmi_wcscpy(path, res->res_path);
...
static void mmi_phb_fmgr_video_preview(mmi_scrn_essential_struct* scrn_data)
{
            ...
            MMI_FRM_FULL_SCRN))
    {
        ...
        res = (mmi_phb_filemgr_caller_res_struct*) scrn_data->user_data;
    #ifdef __MMI_SWFLASH__    
        if(res->recv_res_from == MMI_PHB_RECV_RES_SWFLASH)
        {
            EnableCenterSoftkey(0, IMG_GLOBAL_COMMON_CSK);
...
void mmi_phb_fmgr_from_file_mgr(CHAR *path)
{
    ...
    user_data = (mmi_phb_filemgr_caller_res_struct *) srv_phb_mem_malloc(sizeof(mmi_phb_filemgr_caller_res_struct), SRV_PHB_MEMORY_TYPE_ADM);
