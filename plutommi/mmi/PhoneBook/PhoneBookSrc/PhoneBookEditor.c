...
#endif
#if defined(__MMI_VOIP__)
mmi_imm_input_mode_enum required_set[] = 
{
...
const UI_character_type voip_char_allow[] = L"_\\-.!~*\\\\'()&=+$,;?/%@:A-Za-z0-9";
...
U8 mmi_phb_get_aas_type_label(U8 index,U8 storage)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    type_array = (U8 *) srv_phb_mem_malloc(sizeof(U8)*MMI_PHB_FIELD_LABEL_COUNT ,SRV_PHB_MEMORY_TYPE_ADM);
    srv_phb_get_field_type_list(type_array, MMI_PHB_CONTACT_FIELD_ID_NUMBER, (phb_storage_enum)storage);
    type_id = type_array[index];
    srv_phb_mem_free(type_array);
    return type_id;
}
#endif
...
void mmi_phb_save_replace_list_cntx_create(void)
{
    ...
    if (g_mmi_phb_save_contact_p->update_list == NULL)
    {
        ...
        g_mmi_phb_save_contact_p->update_list = (mmi_phb_list_cntx_struct *) srv_phb_mem_malloc(sizeof(mmi_phb_list_cntx_struct), SRV_PHB_MEMORY_TYPE_ADM);
        memset(g_mmi_phb_save_contact_p->update_list, 0, sizeof(mmi_phb_list_cntx_struct));
        
        g_mmi_phb_save_contact_p->update_list->contact_array = srv_phb_mem_malloc(sizeof(U16) * MMI_PHB_ENTRIES ,SRV_PHB_MEMORY_TYPE_ADM);
    }
}
...
mmi_ret mmi_phb_number_editor_proc(mmi_event_struct *evt)
{
    ...
    switch (evt->evt_id)
    {
        ...
        case EVT_ID_CUI_FSEDITOR_CSK_PRESSED:
            if (g_mmi_phb_save_contact_p->highlight_type == MMI_PHB_FIELD_ADD_NUMBERS)
            {
...
#if defined(__MMI_VOIP__)
static void mmi_phb_voip_set_allowed_char(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    mmi_imm_set_characters(MMI_TRUE, voip_char_allow);
}


/*****************************************************************************
* FUNCTION
*  mmi_phb_voip_set_full_scr_lsk
* DESCRIPTION
...
*  void
*****************************************************************************/
static void mmi_phb_voip_set_full_scr_lsk(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/* under construction !*/
    ...
    switch (field_index)
    {
        ...
        case MMI_PHB_FIELD_EMAIL2:
            ...
            #ifdef __OP01__
                image_list[2 * item_index] = IMG_EMAIL_ADDRESS2;
            #else
                image_list[2 * item_index] = IMG_EMAIL_ADDRESS;
            #endif
        #ifdef __MMI_UI_INLINE_EDIT_DEFAULT_TEXT_EFFECT__
...
/* under construction !*/
        ...
        case MMI_PHB_FIELD_VOIP:
            SetInlineItemCaption(&wgui_inline_items[2 * item_index], (U8*) GetString(STR_ID_PHB_VOIP_ID));
            SetInlineItemActivation(&wgui_inline_items[2 * item_index + 1], INLINE_ITEM_ACTIVATE_WITHOUT_KEY_EVENT, 0);
            SetInlineItemTextEdit_ext(
                &wgui_inline_items[2 * item_index + 1],
                (PU8) g_mmi_phb_contact_p->voip,
                VOIP_URI_LEN,
                IMM_INPUT_TYPE_USER_SPECIFIC,
                INPUT_TYPE_EXT_NO_SHOW_NEW_LINE_SYMBOL,
                (mmi_imm_input_mode_enum*)required_set);
            SetInlineTextEditCustomFunction(&wgui_inline_items[2 * item_index + 1], mmi_phb_op_edit_voip_custom_function);
            image_list[2 * item_index] = IMG_ID_PHB_VOIP;
        #ifdef __MMI_UI_INLINE_EDIT_DEFAULT_TEXT_EFFECT__
            SetInlineItemTextEditDefaultItemNameIfEmpty(&wgui_inline_items[2 * item_index + 1], STR_ID_PHB_VOIP_ID);
        #endif 
            break;
    #endif /* defined(__MMI_VOIP__) */
...
/* under construction !*/
        ...
        case MMI_PHB_FIELD_VOIP:
            ...
            SetInlineItemFullScreenEdit_ext2(
                ...
                (mmi_imm_input_mode_enum*)required_set);
            image_list[2 * item_index] = IMG_ID_PHB_VOIP;
            SetInlineFullScreenEditCustomFunction(&wgui_inline_items[2 * item_index + 1], mmi_phb_voip_set_full_scr_lsk);
        #ifdef __MMI_UI_INLINE_EDIT_DEFAULT_TEXT_EFFECT__
            SetInlineItemTextEditDefaultItemNameIfEmpty(&wgui_inline_items[2 * item_index + 1], STR_ID_PHB_VOIP_ID);
        #endif 
            break;
...
void mmi_phb_op_edit_inline_tab_highlight_hdlr(S32 index)
{
    ...
    switch (g_mmi_phb_save_contact_p->highlight_type)
    {
        ...
        case MMI_PHB_FIELD_RING:
        {
            ...
            SetLeftSoftkeyFunction(mmi_phb_entry_choose_caller_ringtone, KEY_EVENT_UP);
...
/* under construction !*/
    ...
    #if defined(__MMI_VOIP__)
        case MMI_PHB_FIELD_VOIP:
            item_text.buffer_size = VOIP_URI_LEN;
            item_text.input_type = IMM_INPUT_TYPE_USER_SPECIFIC;
            item_text.input_extended_type = INPUT_TYPE_EXT_NO_SHOW_NEW_LINE_SYMBOL;
            item_text.required_input_mode_set = &required_set[0];
            cui_inline_insert_item(inline_id, pre_item_id, &item);
#ifndef __MMI_MAINLCD_96X64__
            cui_inline_set_value(inline_id, item_id, g_mmi_phb_contact_p->voip);
	#else
            if((g_mmi_phb_contact_p->voip) && (mmi_ucs2strlen((const CHAR *)(g_mmi_phb_contact_p->voip)) == 0))
            {
				cui_inline_set_value(inline_id, item_id, (void *) (get_string(STR_GLOBAL_EMPTY)));
            }
			else
			...
			#endif
            cui_inline_set_characters_filter(inline_id, MMI_TRUE, voip_char_allow);
            break;
    #endif /* defined(__MMI_VOIP__) */
...
#endif /* defined(__MMI_IMPS__) */
#if defined(__MMI_VOIP__)
        mmi_phb_get_value(gid, MMI_PHB_INLINE_FIELD_VOIP, g_mmi_phb_contact_p->voip);
#endif
    }
}
...
void mmi_phb_view_entry_hdlr(mmi_phb_view_contact_cntx_struct *cntx, mmi_phb_op_action_enum op_action)
{
    ...
    #if defined(__MMI_VOIP__)
        case MMI_PHB_FIELD_VOIP:
            op_cntx.data = cntx->contact->voip;
            op_cntx.data_type = SRV_PHB_ENTRY_FIELD_VOIP;
            break;
    #endif /* __MMI_VOIP__ */
...
void mmi_phb_sns_view_unlink(U16 store_index, MMI_ID group_id)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    unlink_info = (mmi_phb_sns_unlink_struct *)srv_phb_mem_malloc(sizeof(mmi_phb_sns_unlink_struct), SRV_PHB_MEMORY_TYPE_CTR);
			...
			switch (type)
			{
                ...
                case MMI_PHB_SNS_EMAIL:
                    if (sns_info->email_len > 0 && sns_info->email)
                    {
                        ...
                        g_mmi_phb_view_contact_p->account_string = (WCHAR *)srv_phb_mem_malloc(32 * sizeof(WCHAR), SRV_PHB_MEMORY_TYPE_CTR);
...
/* under construction !*/
        ...
        case MMI_PHB_FIELD_VOIP:
            SetInlineItemCaption(&wgui_inline_items[2 * item_index], (U8*) GetString(STR_ID_PHB_VOIP_ID));
            SetInlineItemActivation(&wgui_inline_items[2 * item_index + 1], INLINE_ITEM_ACTIVATE_WITHOUT_KEY_EVENT, 0);
            SetInlineItemDisplayOnly(&wgui_inline_items[2 * item_index + 1], (PU8) g_mmi_phb_contact_p->voip);
            image_list[2 * item_index] = IMG_ID_PHB_VOIP;
            break;
    #endif /* defined(__MMI_VOIP__) */ 
...
void mmi_phb_entry_view_contact_exit_inline_tab_2(void)
{
    ...
    {
        srv_phb_mem_free(g_mmi_phb_view_contact_p->account_string);
...
/* under construction !*/
    ...
    #if defined(__MMI_VOIP__)
        case MMI_PHB_FIELD_VOIP:
            *entry_list = (CHAR*) g_mmi_phb_contact_p->voip;
            break;
    #endif /* defined(__MMI_VOIP__) */
...
/* under construction !*/
    ...
    #if defined(__MMI_VOIP__)
        case MMI_PHB_FIELD_VOIP:
            if (field_mask & SRV_PHB_ENTRY_FIELD_VOIP)
            {
                is_display = MMI_TRUE;
            }
            break;
...
/* under construction !*/
    ...
    #if defined(__MMI_VOIP__)
        case MMI_PHB_FIELD_VOIP:
            field_mask = SRV_PHB_ENTRY_FIELD_VOIP;
            break;
    #endif
