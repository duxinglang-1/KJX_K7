...
S8 *GetAudio(U16 AudioId)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    MMI_RES_ADO_CHK_RECORD(AudioId, NULL);
...
#define MMI_RES_IMG_CHK_PATH (const WCHAR *)L"imagecheck.txt"
#define MMI_RES_ADO_CHK_PATH (const WCHAR *)L"audiocheck.txt"

#define MMI_RES_STR_CHK_ON 0x01
#define MMI_RES_IMG_CHK_ON 0x02
#define MMI_RES_ADO_CHK_ON 0x04
...
/*****************************************************************************
 * FUNCTION
 *  mmi_res_img_chk_set_status
 * DESCRIPTION
 *  Set status of image checking tool
 * PARAMETERS
 *  is_on        : [IN]        Is On checking tool
 * RETURNS
 *  S32    0 is success
 *****************************************************************************/
S32 mmi_res_img_chk_set_status(MMI_BOOL is_on)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (is_on)
    {
        if (SRV_FMGR_PUBLIC_DRV > 0)
        {
            mmi_res_str_chk_get_path(path, MMI_RES_IMG_CHK_PATH);
            FS_Delete(path);
            file = FS_Open(path, FS_CREATE);
            if (file != 0)
            {
                ...
                g_mmi_res_on |= MMI_RES_IMG_CHK_ON;
                return 0;
            }
        }
    }
...
/*****************************************************************************
 * FUNCTION
 *  mmi_res_ado_chk_set_status
 * DESCRIPTION
 *  Set status of audio checking tool
 * PARAMETERS
 *  is_on        : [IN]        Is On checking tool
 * RETURNS
 *  S32    0 is success
 *****************************************************************************/
S32 mmi_res_ado_chk_set_status(MMI_BOOL is_on)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    if (is_on)
    {
        if (SRV_FMGR_PUBLIC_DRV > 0)
        {
            mmi_res_str_chk_get_path(path, MMI_RES_ADO_CHK_PATH);
            FS_Delete(path);
            file = FS_Open(path, FS_CREATE);
            if (file != 0)
            {
                ...
                g_mmi_res_on |= MMI_RES_ADO_CHK_ON;
                return 0;
            }
        }
    }
...
/*****************************************************************************
 * FUNCTION
 *  mmi_res_img_chk_record
 * DESCRIPTION
 *  Add a image to checking tool file
 * PARAMETERS
 *  str_id        : [IN]        String ID
 *  string        : [IN]        String buffer 
 * RETURNS
 *  S32     0 is success
 *****************************************************************************/
S32 mmi_res_img_chk_record(MMI_IMG_ID img_id, WCHAR *string)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/    
    UI_UNUSED_PARAMETER(string);
    if (!(g_mmi_res_on & MMI_RES_IMG_CHK_ON) || SRV_FMGR_PUBLIC_DRV < 0)
    {
        return 0;
    }

    mmi_wcs_itow(img_id, tempbuf, 10);
    mmi_wcscpy(id_buf_wc, tempbuf);
    mmi_wcscat(id_buf_wc, L"\r\n");
...
/*****************************************************************************
 * FUNCTION
 *  mmi_res_ado_chk_record
 * DESCRIPTION
 *  Add a audio to checking tool file
 * PARAMETERS
 *  str_id        : [IN]        String ID
 *  string        : [IN]        String buffer 
 * RETURNS
 *  S32     0 is success
 *****************************************************************************/
S32 mmi_res_ado_chk_record(MMI_ID ado_id, WCHAR *string)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/    
    UI_UNUSED_PARAMETER(string);
    if (!(g_mmi_res_on & MMI_RES_ADO_CHK_ON) || SRV_FMGR_PUBLIC_DRV < 0)
    {
        return 0;
    }

    mmi_wcs_itow(ado_id, tempbuf, 10);
    mmi_wcscpy(id_buf_wc, tempbuf);
    mmi_wcscat(id_buf_wc, L"\r\n");
