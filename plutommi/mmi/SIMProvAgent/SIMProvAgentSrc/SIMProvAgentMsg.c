                ...
                mmi_spa_sim_status_info_ind_struct *info)
{
	...
	case SRV_MODE_SWITCH_SIM_ALL:
            info->setting_mode = MMI_SPA_SIM_ID_DUAL_SIM;
            break;
#endif
        default:
...
#ifdef __MMI_DUAL_SIM__
    info->sim2_can_read = srv_sim_ctrl_is_available(MMI_SIM2);
#endif
    /* sim1_type */
    if (!sim1_insert)
    {
    ...
    if (!sim2_insert)
    {
        info->sim2_type = MMI_SPA_SIM_TYPE_UNKNOWN;
    }
    else if (srv_nw_info_sim_is_rejected(MMI_SIM2))
    {
        info->sim2_type = MMI_SPA_SIM_TYPE_INVALID_SIM;
    }
    else if (srv_sim_ctrl_chv1_is_blocked(MMI_SIM2))
    {
        info->sim2_type = MMI_SPA_SIM_TYPE_BLOCKED_SIM;
    }
    else
    {
        info->sim2_type = MMI_SPA_SIM_TYPE_VALID_SIM;
    }
#endif
}
