...
static void mmi_secset_entry_sim_switch_chv1_menu(void);
#endif
...
static mmi_ret mmi_secset_sim_menu_on_selected(const cui_menu_event_struct *menu_evt)
{
    ...
    switch (menu_evt->highlighted_menu_id)
    {
        case MENU_ID_SECSET_SIM1_CHV1_LOCK:
        #ifdef __MMI_SECSET_APP_WITH_BWUI__
        	mmi_secset_entry_sim_switch_chv1_menu();
        #else
...
static void mmi_secset_sim_set_chv1_enabled(mmi_sim_enum sim)
{
    ...
    cui_pwd_basic_set_small_screen_enabled(cntx->set_chv_lock_cui, MMI_TRUE);
#endif
...
static mmi_ret mmi_secset_sim_switch_chv1_grp_proc(mmi_event_struct *evt)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    CurrentMenu=(srv_sim_ctrl_pwd_is_enabled(MMI_SIM1, SRV_SIM_CTRL_PWD_TYPE_CHV1))?MENU_ID_SECURITY_SETTING_ON : MENU_ID_SECURITY_SETTING_OFF;
    ...
    if (cui_menu_is_menu_event(evt->evt_id))
    {
        ...
        {
            case EVT_ID_CUI_MENU_LIST_ENTRY:
                if (menu_evt->parent_menu_id == MENU_ID_SECSET_SIM1_CHV1_LOCK)
                {
                    cui_menu_set_currlist_title(
                        menu_evt->sender_id,
