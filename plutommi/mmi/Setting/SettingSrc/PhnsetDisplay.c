...
static void mmi_phnset_show_calendar_idle_screen_select_handler(mmi_event_struct *param);
static mmi_ret mmi_phnset_show_calendar_idle_screen_proc(mmi_event_struct *param);
static void mmi_phnset_entry_show_calendar_idle_screen(MMI_ID parent_gid);
...
static void EntryPhnsetHomeScreenSetup(MMI_ID parent_gid);
static void PhnsetHomeScreenLSKHandler(U8 highlighted_item);
static void mmi_phnset_init_homescreen_status_list(void);
static mmi_ret mmi_phnset_homescreen_status_proc(mmi_event_struct *param);
...
static void mmi_phnset_entry_show_operator_name(MMI_ID parent_gid);
static mmi_ret mmi_phnset_show_operator_name_proc(mmi_event_struct *param);
static void mmi_phnset_show_operator_name_select_handler(mmi_event_struct *param);
#endif
...
/*****************************************************************************
 * FUNCTION
 *  mmi_phnset_entry_show_calendar_idle_screen
 * DESCRIPTION
 *  Entry show calendar idle screen setting menu 
 * PARAMETERS
 *  
 * RETURNS
 *  static void
...
static void mmi_phnset_entry_show_calendar_idle_screen(MMI_ID parent_gid)
{
    ...
    mmi_phnset_appcreate_menucui_create(
        ...
        mmi_phnset_show_calendar_idle_screen_proc);
}
 ...
 *  mmi_phnset_show_calendar_idle_screen_proc
 * DESCRIPTION
 *  Proc function for show calendar idle screen setting menu 
 * PARAMETERS
 *  
 * RETURNS
 *  static mmi_ret
...
static mmi_ret mmi_phnset_show_calendar_idle_screen_proc(mmi_event_struct *param)
{
    ...
    case EVT_ID_CUI_MENU_LIST_ENTRY:
        mmi_phnset_init_on_off_radio_button_menu(
            ...
            g_phnset_cntx_p->curCalendarIdleScrStatus);
        return MMI_RET_OK;

    case EVT_ID_CUI_MENU_ITEM_SELECT:
    case EVT_ID_CUI_MENU_ITEM_TAP:
        mmi_phnset_show_calendar_idle_screen_select_handler(param);
        return MMI_RET_OK;
...
/*****************************************************************************
 * FUNCTION
 *  mmi_phnset_show_calendar_idle_screen_select_handler
 * DESCRIPTION
 *  Select handler for show calendar idle screen setting menu 
 * PARAMETERS
 *  
 * RETURNS
 *  static void
...
static void mmi_phnset_show_calendar_idle_screen_select_handler(mmi_event_struct *param)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_phnset_cntx_p->curCalendarIdleScrStatus = (U8)menucui_param->highlighted_menu_id;
    WriteValue(NVRAM_SET_CALENDAR_IDLESCREEN, &(g_phnset_cntx_p->curCalendarIdleScrStatus), DS_BYTE, &error);
    mmi_popup_display_simple_ext(STR_GLOBAL_DONE, MMI_EVENT_SUCCESS, GRP_ID_ROOT, NULL);    
    mmi_frm_group_close(GRP_ID_PHNSET_CALENDAR_IDLESCREEN);    
}
 ...
 *  void
 *****************************************************************************/
void HintPhnsetCalendarIdleScreen(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
/*****************************************************************************
 * FUNCTION
 *  PhnsetGetHomeScreentStatus
 * DESCRIPTION
 *  return current home screen status
 * PARAMETERS
 *  void
 * RETURNS
 *  current home screen status
 *****************************************************************************/
MMI_BOOL PhnsetGetHomeScreenStatus(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (g_phnset_cntx_p->curHomeScreenStatus)
    {
        return MMI_TRUE;
    }
    else
...
/*****************************************************************************
 * FUNCTION
 *  PhnsetHomeScreenLSKHandler
 * DESCRIPTION
 *  This function is Left soft key handler
 *  for Phone setup-> Display -> Home Screen 
 * PARAMETERS
 *  void           
 * RETURNS
 *  void
...
static void PhnsetHomeScreenLSKHandler(U8 highlighted_item)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (g_phnset_cntx_p->curHomeScreenStatus != highlighted_item)
    {
        g_phnset_cntx_p->curHomeScreenStatus = highlighted_item;
        
        WriteValue(NVRAM_SET_HOMESCREEN, &(g_phnset_cntx_p->curHomeScreenStatus), DS_BYTE, &error);
...
/* under construction !*/
    ...
    mmi_frm_group_close(GRP_ID_PHNSET_HOMESCREEN);
}


/*****************************************************************************
 * FUNCTION
 *  EntryPhnsetHomeScreenSetup
 * DESCRIPTION
 *  This function is Left soft key handler
 *  for Phone setup->IdleScreenDisplay-> Home Screen Status
 * PARAMETERS
 *  void           
 * RETURNS
 *  void
...
static void EntryPhnsetHomeScreenSetup(MMI_ID parent_gid)
{
    ...
    mmi_phnset_appcreate_menucui_create(
        ...
        MENU_ID_PHNSET_HOMESCREEN,
        mmi_phnset_homescreen_status_proc);
}
 ...
 *  void
 *****************************************************************************/
void HintPhnsetHomeScreenStatus(void)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
        g_phnset_cntx_p->curHomeScreenStatus = 0; 
        hint_str = (CHAR*) GetString(STR_GLOBAL_OFF);
    }
    cui_menu_set_item_hint(
        mmi_frm_group_get_active_id(),
 ...
 *  mmi_phnset_homescreen_status_proc
 * DESCRIPTION
 *  Menu CUI proc function formmi_phnset_homescreen_status 
 * PARAMETERS
 *  
 * RETURNS
 *  mmi_ret
 *****************************************************************************/
mmi_ret mmi_phnset_homescreen_status_proc(mmi_event_struct *param)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    {
    case EVT_ID_CUI_MENU_LIST_ENTRY:
        mmi_phnset_init_homescreen_status_list();
        break;

    case EVT_ID_CUI_MENU_ITEM_SELECT:
    case EVT_ID_CUI_MENU_ITEM_TAP:
        PhnsetHomeScreenLSKHandler((U8)((cui_menu_event_struct*)param)->highlighted_menu_id);
        break;
 ...
 *  mmi_phnset_init_homescreen_status_list
 * DESCRIPTION
 *  Initialize homescreen status setting menu list 
 * PARAMETERS
 *  
 * RETURNS
 *  static void
...
static void mmi_phnset_init_homescreen_status_list(void)
{
    ...
    radio_button_menu.group_id = GRP_ID_PHNSET_HOMESCREEN;
    radio_button_menu.menu_title_str_id = STR_ID_PHNSET_HOMESCREEN;
    radio_button_menu.menu_icon_id = MAIN_MENU_TITLE_SETTINGS_ICON;
    radio_button_menu.menu_item_num = 2;
    radio_button_menu.menu_item_str_list = subMenuDataPtrs;
    radio_button_menu.menu_item_str_id_list = NULL;
    radio_button_menu.highlighted_item_idx = g_phnset_cntx_p->curHomeScreenStatus;
...
/*****************************************************************************
 * FUNCTION
 *  mmi_phnset_entry_show_operator_name
 * DESCRIPTION
 *  Entry show operator name setting menu 
 * PARAMETERS
 *  
 * RETURNS
 *  static void
...
static void mmi_phnset_entry_show_operator_name(MMI_ID parent_gid)
{
    ...
    mmi_phnset_appcreate_menucui_create(
        ...
        MENU_SETTING_SHOW_OPN,
        mmi_phnset_show_operator_name_proc);
}
 ...
 *  mmi_phnset_show_operator_name_proc
 * DESCRIPTION
 *  Proc function for show operator name setting menu 
 * PARAMETERS
 *  
 * RETURNS
 *  static mmi_ret
...
static mmi_ret mmi_phnset_show_operator_name_proc(mmi_event_struct *param)
{
    ...
    case EVT_ID_CUI_MENU_LIST_ENTRY:
        mmi_phnset_init_on_off_radio_button_menu(
            ...
            STR_OPN,
            MAIN_MENU_TITLE_SETTINGS_ICON,
            g_phnset_cntx_p->curShowOPNStatus);
        return MMI_RET_OK;

    case EVT_ID_CUI_MENU_ITEM_SELECT:
    case EVT_ID_CUI_MENU_ITEM_TAP:
        mmi_phnset_show_operator_name_select_handler(param);
        return MMI_RET_OK;
...
/*****************************************************************************
 * FUNCTION
 *  mmi_phnset_show_operator_name_select_handler
 * DESCRIPTION
 *  Select handler for show operator name setting menu 
 * PARAMETERS
 *  
 * RETURNS
 *  static void
...
static void mmi_phnset_show_operator_name_select_handler(mmi_event_struct *param)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    g_phnset_cntx_p->curShowOPNStatus = (U8)menucui_param->highlighted_menu_id;
    WriteValue(NVRAM_SETTING_OPN_DISPLAY, &(g_phnset_cntx_p->curShowOPNStatus), DS_BYTE, &error);
    mmi_popup_display_simple_ext(STR_GLOBAL_DONE, MMI_EVENT_SUCCESS, GRP_ID_ROOT, NULL);    
    mmi_frm_group_close(GRP_ID_PHNSET_SHOW_OPN);    
}
#ifdef __MMI_AP_DCM_SETTING__
#pragma arm section rodata , code
#endif 
...
MMI_BOOL PhnsetSetDualClockState(U8 status)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_DUAL_CLOCK__
    g_phnset_cntx_p->curDualClockStatus = status;
    WriteValue(NVRAM_SET_DUALCLOCK_IDLESCREEN, &(g_phnset_cntx_p->curDualClockStatus), DS_BYTE, &error);
...
static mmi_ret mmi_phnset_disp_setup_sub_menu_select_handler(cui_menu_event_struct *param, MMI_ID parent_gid)
{
...
#ifdef __MMI_CALENDAR_ON_IDLE_SCREEN__        
    case MENU_ID_PHNSET_CALENDAR_IDLESCREEN:
        mmi_phnset_entry_show_calendar_idle_screen(parent_gid);
        return MMI_RET_OK;        
#endif /* __MMI_CALENDAR_ON_IDLE_SCREEN__ */        
        
#ifdef __MMI_DUAL_CLOCK__
#ifndef __MMI_VUI_HOMESCREEN_DUALCLOCK__
    case MENU_ID_PHNSET_DUAL_CLOCK_IDLESCREEN:
        mmi_phnset_entry_dual_clock_idle_screen(parent_gid);
        return MMI_RET_OK;        
#endif /* __MMI_VUI_HOMESCREEN_DUALCLOCK__ */
#endif /* __MMI_DUAL_CLOCK__ */

#if defined(__MMI_OP11_HOMESCREEN_0301__) || defined(__MMI_OP11_HOMESCREEN_0302__) || defined(__MMI_OP11_HOMESCREEN__)
    case MENU_ID_PHNSET_HOMESCREEN:
        EntryPhnsetHomeScreenSetup(parent_gid);
        return MMI_RET_OK;        
    ...
    case MENU_SETTING_SHOW_OPN:
        mmi_phnset_entry_show_operator_name(parent_gid);
        return MMI_RET_OK;
#endif 
