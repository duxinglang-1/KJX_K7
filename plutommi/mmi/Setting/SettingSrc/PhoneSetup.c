...
void InitPhoneSetupCntx(void)
{
    ...
    g_phnset_cntx_p->curShowOwnNumStatus2 = mmi_phnset_show_owner_number_is_on(MMI_SIM2);    
#endif
#endif
    /* Lisen 0921-1 */
    ...
    g_phnset_cntx_p->ShowCalendarIdleSrcDirtyFlag = MMI_FALSE;
    ReadValue(NVRAM_SET_CALENDAR_IDLESCREEN, &(g_phnset_cntx_p->curCalendarIdleScrStatus), DS_BYTE, &Err);
    if (g_phnset_cntx_p->curCalendarIdleScrStatus == 0xFF || Err != NVRAM_READ_SUCCESS)
    {
    ...
    ReadValue(NVRAM_SET_HOMESCREEN, &(g_phnset_cntx_p->curHomeScreenStatus), DS_BYTE, &Err);
    if (g_phnset_cntx_p->curHomeScreenStatus == 0xFF)
    {
        g_phnset_cntx_p->curHomeScreenStatus = 1;
    }
...
#ifdef __GGT_TASK__
void mmi_set_language(U8 langtype)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
U8  mmi_get_language(void)
{
  ...
  // else  if (strcmp((PS8) gLanguageArray[gCurrLangIndex].aLangSSC, SSC_TCHINESE) == 0)
   //{
  //    return MMI_PHNSET_LANG_TR_CH;
 //  }
...
void mmi_oldman_set_language(U8 langtype)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
static void mmi_phnset_set_uart(U16 select_id)
{
	...
	srv_setting_set_primary_card(mmi_frm_index_to_sim(select_id));
...
#ifdef __MMI_DUAL_SIM_MASTER__
#ifndef __MMI_PHNSET_AT_DISABLE__
    case MENU_ID_PHNSET_UART_SETUP:
#ifdef __MTK_TARGET__
        EntryPhnsetUARTSetup(parent_gid);
#else 
        PopupNoSupport();
#endif         
        return MMI_RET_OK;
...
#ifdef __MMI_DUAL_SIM_MASTER__           
    || srv_cphs_is_service_available(SRV_CPHS_GROUP_VALUE_ADDED_SERVICES, SRV_CPHS_SERVICE_LANGUAGE, MMI_SIM2)
#endif
        )
	{
...
#ifdef __MMI_DUAL_SIM_MASTER__
#ifndef __MMI_PHNSET_AT_DISABLE__
    mmi_phnset_set_UART_setup_hint(main_menu_cui_id);
#endif
#endif
