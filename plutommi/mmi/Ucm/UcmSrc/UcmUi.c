...
#if defined(__KJX_FUN__)
static U8 slc_listen_status = 0;
static U8 incoming_num[SRV_UCM_MAX_NUM_URI_LEN + 1] = {0};

extern kal_bool unknown_call;
#ifdef __KJX_AGAINST_THEFT__
extern kal_bool g_rfid_against_theft;
#endif

#if defined(__KJX_OLED_64_48_UI__)	//xb add 2016-09-01
U32 kjx_call_time = 0;
U8 kjx_call_display[SRV_UCM_MAX_NUM_URI_LEN + 1] = {0};
static char incoming_show[] = {0x65, 0x67, 0x35, 0x75, 0x3A, 0x00, 0x00, 0x00};//来电:
static char outcoming_show[] = {0xE8, 0x62, 0xF7, 0x53, 0x3A, 0x00, 0x00, 0x00};//拨号:
static char incall_show[] = {0x1A, 0x90, 0xDD, 0x8B, 0x2D, 0x4E, 0x3A, 0x00, 0x00, 0x00};//通话中:

extern BOOL kjx_tts_play_is_stop;
extern UI_string_type kjx_scrolling_show[512];

extern void mmi_disp_idlescreen(void);
extern void mmi_exit_idlescreen(void);
#endif

//extern MMI_BOOL kjx_incoming_check_sos_number(U16* display_name);
...
extern void kjx_tour_charge_led(U16 led_type);
#endif
...
{
     extern void city_easy_led_off(U16 led_id);
     extern void city_easy_led_slow(U16 led_id);
     extern BOOL is_on_network;
...
#if defined(__GGT_TASK__)&&defined(__CITY_EASY_MODEL_W__)
{
   extern void city_easy_led_off(U16 led_id);
   extern void city_easy_led_slow(U16 led_id);
   extern BOOL is_on_network;
...
void mmi_ucm_outgoing_call_endkey(void)
{
	...
	{
		if(IsMyTimerExist(KJX_TIMER_ID_FACTORY_TEST))
...
void mmi_ucm_entry_dtmf_editor(mmi_scrn_essential_struct* scr_info)
{
    ...
    if (srv_ucm_query_group_count(SRV_UCM_INCOMING_STATE, SRV_UCM_CALL_TYPE_ALL, MMI_FALSE, NULL) > 0)
    {
        mmi_frm_set_key_handler(mmi_ucm_incoming_call_endkey, KEY_END, KEY_EVENT_DOWN);
    }
    else if (srv_ucm_query_group_count(SRV_UCM_OUTGOING_STATE, SRV_UCM_CALL_TYPE_ALL, MMI_FALSE, NULL) > 0)
    {
        mmi_frm_set_key_handler(mmi_ucm_outgoing_call_endkey, KEY_END, KEY_EVENT_DOWN);
    }
    else
    {
        mmi_frm_set_key_handler(mmi_ucm_in_call_endkey, KEY_END, KEY_EVENT_DOWN);
    }
 ...
 *****************************************************************************/
#if defined(__GGT_TASK__)&&defined(__CITY_EASY_MODEL_W__)
extern void city_easy_led_on(U16 led_id);
extern BOOL city_easy_incoming_whether_sos_number(U16* display_name);
#endif
#if defined(__GGT_TASK__)
extern kal_bool is_unknown_call;
extern U8 wrt_sys_get_listen_status(void);
extern U8 wrt_sys_get_firewall_status(void);
extern U8 cityeasy_is_outcall_with_monitor(void);
extern void cityeasy_set_outcall_monitor_state(U8 state);

U8 city_listen_status = 0;
#endif /* __GGT_TASK__ */
#if defined(CUSTOMER_NAME_B603)
extern MMI_BOOL city_easy_incoming_check_number(U16* display_name);
#endif /* CUSTOMER_NAME_B603 */


#ifdef __KJX_FUN__
void kjx_ucm_incoming_call_extra_key_1(void)
{
	mmi_ucm_incoming_call_endkey();

	kjx_sms_send_sms(incoming_num,(unsigned short *)GetString(STR_KJX_INCOMING_CALL_END_SMS_1), SRV_SMS_DCS_UCS2, NULL);
}

void kjx_ucm_incoming_call_extra_key_2(void)
{
	mmi_ucm_incoming_call_endkey();

	kjx_sms_send_sms(incoming_num,(unsigned short *)GetString(STR_KJX_INCOMING_CALL_END_SMS_2), SRV_SMS_DCS_UCS2, NULL);
}
#endif
...
void mmi_ucm_entry_incoming_call(mmi_scrn_essential_struct* scr_info)
{
...
#ifdef __KJX_FUN__	//xb add 2018-05-10 K7没有听筒，来电铃声要么从耳机出，要么从外放出
	if(kjx_get_earphone_plugged()==1)
	{
		srv_speech_disable_hand_free();
	}
	else
...
#ifdef __KJX_FUN__	//xb add 2017-07-25	左软键接听,右键拒接
	lsk_str = STR_ID_UCM_ANSWER;
	rsk_str = STR_ID_UCM_REJECT;
	ResetCenterSoftkey();
#endif
    ...
    {        
        ...
        else
        {
            ...
            if (g_ucm_p->call_misc.is_ringtone_suppress == MMI_FALSE &&
                srv_ucm_query_call_count(SRV_UCM_INCOMING_STATE, SRV_UCM_CALL_TYPE_NO_CSD, NULL) == srv_ucm_query_call_count(SRV_UCM_CALL_STATE_ALL, SRV_UCM_CALL_TYPE_NO_CSD, NULL))
            {
                mmi_frm_set_key_handler(mmi_ucm_suppress_incoming_tone, KEY_VOL_DOWN, KEY_EVENT_DOWN);
                mmi_frm_set_key_handler(mmi_ucm_set_key_after_suppress_ring, KEY_VOL_DOWN, KEY_EVENT_UP);
            }
        ...
        #ifdef __MMI_CLAMSHELL__
        #ifdef __MMI_SIDEKEY_SUPPRESS_RING__
            mmi_frm_set_key_handler(mmi_ucm_incoming_call_endkey, KEY_VOL_DOWN, KEY_EVENT_LONG_PRESS);
        #else /* __MMI_SIDEKEY_SUPPRESS_RING__ */    
            mmi_frm_set_key_handler(mmi_ucm_incoming_call_sendkey, KEY_VOL_UP, KEY_EVENT_DOWN);
            mmi_frm_set_key_handler(mmi_ucm_incoming_call_endkey, KEY_VOL_DOWN, KEY_EVENT_DOWN);
        #endif /* __MMI_SIDEKEY_SUPPRESS_RING__ */    
        #endif /* __MMI_CLAMSHELL__ */
        }
    }
...
#ifdef __KJX_FUN__	//xb add 2017-07-25 左软键接听
	SetLeftSoftkeyFunction(mmi_ucm_incoming_call_sendkey, KEY_EVENT_UP);
	SetRightSoftkeyFunction(mmi_ucm_incoming_call_endkey, KEY_EVENT_UP);
    ...
    if (!mmi_is_redrawing_bk_screens())
    {
		...
		{
			//extern U8 wrt_sys_get_listen_status(void);
			//extern U8 wrt_sys_get_firewall_status();
			if (wrt_sys_get_listen_status()&&KAL_FALSE == is_unknown_call ||wrt_sys_get_firewall_status() == 0)
			{
				city_listen_status = 1;
				ExecuteCurrKeyHandler(KEY_SEND, KEY_EVENT_DOWN);
				OslMfree(call_info);
				return;
			}
		}
		...
		SetKeyHandler(mmi_ucm_incoming_call_sendkey,KEY_ENTER, KEY_EVENT_DOWN);
		OslMfree(call_info);
		return;
	#endif
		...
		mmi_asc_to_ucs2((CHAR *) display_name, (CHAR *) call_info->remote_info.num_uri);

		if(kjx_incoming_check_number(display_name)&&!kjx_is_on_class_time()
			...
			)
		{
			...
			//SetKeyHandler(mmi_ucm_incoming_call_sendkey, KEY_VOL_DOWN, KEY_EVENT_DOWN);
			//SetKeyHandler(mmi_ucm_incoming_call_sendkey, KEY_CAMERA, KEY_EVENT_DOWN);

			memset(incoming_num, 0, sizeof(incoming_num));
			strcpy(incoming_num, call_info->remote_info.num_uri);
			SetKeyHandler(kjx_ucm_incoming_call_extra_key_1, KEY_UP_ARROW, KEY_EVENT_UP);
			SetKeyHandler(kjx_ucm_incoming_call_extra_key_2, KEY_DOWN_ARROW, KEY_EVENT_UP);
			...
			if(IsMyTimerExist(KJX_TIMER_ID_PLAY_TTS))
				StopTimer(KJX_TIMER_ID_PLAY_TTS);
			if(IsMyTimerExist(KJX_TIMER_ID_PLAY_TTS_AFTER))
			{
				StopTimer(KJX_TIMER_ID_PLAY_TTS_AFTER);
				srv_vibrator_off();
			}
		#endif
		#endif
        ...
        {
		#ifdef __IV_TTS__
			if(NPR_TTS_PlayIncomingcall((char*)display_name))
				mmi_ucm_play_incoming_tone();
		#else
			mmi_ucm_play_incoming_tone();
		#endif//__IV_TTS__
...
#ifdef __KJX_FUN__	//xb add 2017-07-24 左软键直接显示免提
	lsk_str = NULL;//(mdi_audio_get_audio_mode() == AUD_MODE_LOUDSPK) ? (STR_ID_UCM_HAND_HELD) : (STR_ID_UCM_HAND_FREE);
	ResetCenterSoftkey();//xb add 2017-08-07 中间软键不再显示
#endif
...
#if defined(__GGT_TASK__)&&defined(__CITY_EASY_MODEL_W__)
#if !defined(CUSTOMER_NAME_B606)
	if(cityeasy_is_outcall_with_monitor() == 1)
	{
		srv_speech_disable_hand_free();
	}
	else
...
void mmi_ucm_entry_disconnecting_screen(void)
{
    ...
    if (MMI_TRUE == mmi_ucm_is_ucm_sg(parent_id))
    {
        ...
        wgui_cat9001_show();
	#endif /* __OP01_FWPBW__ */
        mmi_frm_set_key_handler(mmi_ucm_end_all_processing_action, KEY_END, KEY_EVENT_DOWN);
    }
}
...
void mmi_ucm_set_mute(void)
{
    ...
    if (srv_speech_is_phone_mute() == TRUE)
    {
        srv_speech_unmute_phone();
    #ifdef __MMI_UCM_RSK_MUTE__
	  #ifdef __KJX_FUN__	//xb add 2017-07-24 通话中界面左软键静音设置，右软键挂机
		ChangeLeftSoftkey(STR_GLOBAL_MUTE, 0);
	  #else
    ...
    #ifdef __MMI_UCM_RSK_MUTE__
	  #ifdef __KJX_FUN__	//xb add 2017-07-24 通话中界面左软键静音设置，右软键挂机
		ChangeLeftSoftkey(STR_ID_UCM_UNMUTE, 0);
	  #else
 ...
 *  void
 *****************************************************************************/
extern void kjx_factory_call_test_end(void);
void mmi_ucm_entry_in_call(mmi_scrn_essential_struct* scr_info)
{
	...
	rsk_str = STR_ID_UCM_END_CALL;
	EnableCenterSoftkey(0, NULL);
#endif
...
#endif /* __MMI_UCM_SINGLE_CALL_SCREEN_ENHANCE__ */
    {
		...
		for (i = 0; i < num_group; i++)
		{
			...
			else
			{
				if (group_info.call_state == SRV_UCM_ACTIVE_STATE)
				{
					...
					index.call_index = 0;
					srv_ucm_query_call_data(index, call_info);
					//active_cm[0].active_caller = (PU8) subMenuDataPtrs[0]; 
					
					active_cm[0].active_caller = OslMalloc(SRV_UCM_MAX_DISP_NAME_LEN);
					memcpy(active_cm[0].active_caller,(PU8) call_info->remote_info.disp_name, SRV_UCM_MAX_DISP_NAME_LEN);
					//active_cm[0].active_caller = (PU8) call_info->remote_info.disp_name; 	
				}
				else if (group_info.call_state == SRV_UCM_HOLD_STATE)
				{
					index.group_index = group_index_array[i];
					index.call_index = 0;
					srv_ucm_query_call_data(index, call_info);
					//active_cm[1].held_caller = (PU8) subMenuDataPtrs[0];
					active_cm[1].held_caller = OslMalloc(SRV_UCM_MAX_DISP_NAME_LEN);
					memcpy(active_cm[1].held_caller,(PU8) call_info->remote_info.disp_name, SRV_UCM_MAX_DISP_NAME_LEN);
					//active_cm[1].held_caller = (PU8) call_info->remote_info.disp_name;
				}
				else
				{
					MMI_TRACE(MMI_COMMON_TRC_G5_CALL, TRC_MMI_UCM_ENTRY_INCALL_CALL_STATE);
        ...
        if (num_group > 1)
        {
            ...
            SetKeyHandler(mmi_ucm_tab_prev, KEY_LEFT_ARROW, KEY_LONG_PRESS); 
            SetKeyHandler(mmi_ucm_tab_prev, KEY_RIGHT_ARROW, KEY_LONG_PRESS); 
            SetKeyHandler(mmi_ucm_tab_prev, KEY_LEFT_ARROW, KEY_REPEAT); 
            SetKeyHandler(mmi_ucm_tab_prev, KEY_RIGHT_ARROW, KEY_REPEAT); 
...
#if defined(__KJX_FUN__)
#if !defined(__KJX_BAT_STATUS_LED_SHOW__)
	kjx_tour_charge_led(2);
#endif
    if((kjx_get_foctory_test_mode() != 0) && (kjx_test_get_call_flag()!=1))
    {
    	if(IsMyTimerExist(KJX_TIMER_ID_FACTORY_TEST))
			StopTimer(KJX_TIMER_ID_FACTORY_TEST);

		if(strcmp(call_info->remote_info.num_uri, "10086") == 0)
        	StartTimer(KJX_TIMER_ID_FACTORY_TEST, 1000*5, kjx_factory_call_test_end);
    }
#endif
...
#if defined(__GGT_TASK__)&&defined(__CITY_EASY_MODEL_W__)
#if !defined(CUSTOMER_NAME_B606)
	if(city_listen_status == 0)
	{
		srv_speech_enable_hand_free();
	}
	else
	{
		city_listen_status = 0;
		srv_speech_disable_hand_free();
	}
#endif
#endif
...
#else
	{
		...
		kjx_sos_in_call = TRUE;
	}

	//xb add 2018-05-10 K7没有听筒，来电铃声要么从耳机出，要么从外放出
	if(kjx_get_earphone_plugged()==1)
	{
		srv_speech_disable_hand_free();
	}
...
void mmi_ucm_enter_confirm(void)
{
    ...
    SetKeyHandler(mmi_ucm_confirm_hdlr_sendkey, KEY_SEND2, KEY_EVENT_DOWN);
    ...
    SetKeyHandler(mmi_ucm_confirm_hdlr_sendkey, KEY_SEND, KEY_EVENT_DOWN); 
    mmi_frm_set_key_handler(mmi_ucm_confirm_hdlr_endkey, KEY_END, KEY_EVENT_DOWN);
#ifdef __PXS_APP_ENABLE__
    if (MMI_TRUE == g_ucm_p->pxs_need_handle)
    {        
    }
...
void mmi_ucm_entry_processing_screen_internal(mmi_scrn_essential_struct* scr_info)
{
    ...
    mmi_frm_set_key_handler(g_ucm_p->proc_scr.end_funcPtr, KEY_END, KEY_EVENT_DOWN);
...
void mmi_ucm_entry_auto_redial(mmi_scrn_essential_struct* scr_info)
{
    ...
    mmi_frm_set_key_handler(mmi_ucm_auto_redial_abort, KEY_END, KEY_EVENT_DOWN);
...
void mmi_ucm_highlight_call_type_voice_sim4(void)
{
    ...
    SetKeyHandler(mmi_ucm_dial_option_make_call, KEY_SEND, KEY_EVENT_UP);
}
#endif /* (MMI_MAX_SIM_NUM >= 4) */
...
void mmi_ucm_highlight_call_type_voice(void)
{
    ...
    else if (g_ucm_p->mo_info.dial_type_count > 1)
    {
        ...
        SetKeyHandler(mmi_ucm_dial_option_select_sim, KEY_SEND, KEY_EVENT_UP);
    }
...
void mmi_ucm_set_key_after_suppress_ring(void)
{
    ...
    /*----------------------------------------------------------------*/
#ifdef __MMI_SIDEKEY_SUPPRESS_RING__
    mmi_frm_set_key_handler(MMI_dummy_function, KEY_VOL_UP, KEY_EVENT_UP);
    mmi_frm_set_key_handler(MMI_dummy_function, KEY_VOL_UP, KEY_EVENT_LONG_PRESS);
    mmi_frm_set_key_handler(MMI_dummy_function, KEY_VOL_UP, KEY_EVENT_REPEAT);
    mmi_frm_set_key_handler(MMI_dummy_function, KEY_VOL_DOWN, KEY_EVENT_UP);
    mmi_frm_set_key_handler(MMI_dummy_function, KEY_VOL_DOWN, KEY_EVENT_LONG_PRESS);
    mmi_frm_set_key_handler(MMI_dummy_function, KEY_VOL_DOWN, KEY_EVENT_REPEAT);
    
#ifdef __MMI_CLAMSHELL__
    mmi_frm_set_key_handler(mmi_ucm_incoming_call_sendkey, KEY_VOL_UP, KEY_EVENT_DOWN);
    mmi_frm_set_key_handler(mmi_ucm_incoming_call_endkey, KEY_VOL_DOWN, KEY_EVENT_DOWN);
#else /* __MMI_CLAMSHELL__ */
    mmi_frm_set_key_handler(MMI_dummy_function, KEY_VOL_UP, KEY_EVENT_DOWN);
    mmi_frm_set_key_handler(MMI_dummy_function, KEY_VOL_DOWN, KEY_EVENT_DOWN);
#endif /* __MMI_CLAMSHELL__ */
#ifdef  __MMI_FTE_SUPPORT__
    /* update scrn */                 
...
void mmi_ucm_set_sidekey(void)
{
	...
	#endif /* __CTM_SUPPORT__ */
	    {
	        SetKeyHandler(mmi_sndrec_entry_in_call, KEY_VOL_UP, KEY_EVENT_LONG_PRESS);
	    }
	}
#endif /* __MMI_SOUND_RECORDER__ */
}
...
void mmi_ucm_vt_entry_dialer_screen(mmi_scrn_essential_struct* scr_info)
{
    ...
    SetKeyHandler(mmi_ucm_vt_dialer_make_vt, KEY_SEND, KEY_EVENT_DOWN);    
}
