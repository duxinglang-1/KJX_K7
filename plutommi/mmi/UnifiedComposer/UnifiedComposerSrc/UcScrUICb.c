...
void mmi_uc_write_msg_toolbar_callback(S32 index)
{
    ...
    case UC_EDITOR_FTE_TOOLBAR_OPERATION_SEND:
        ...
        else
        {
                        ...
                        if(srv_um_get_prefer_sim() != SRV_UM_SIM_UNCLASSIFIED && srv_sim_ctrl_is_available(SRV_UC_SIM_ID_GSM_SIM1) && srv_sim_ctrl_is_available(SRV_UC_SIM_ID_GSM_SIM2))
                        {
                            if(g_uc_p->srv_main->state == MMI_UC_STATE_REPLY || g_uc_p->srv_main->state == MMI_UC_STATE_REPLY_ALL || g_uc_p->srv_msg_type->caller_specific_msg_type == MMI_UC_MSG_TYPE_SMS_ONLY)
                                g_uc_p->srv_send_info->send_sim_id = g_uc_p->send_info.highlight_sim_id;
                            else
                            {
                                g_uc_p->srv_send_info->send_sim_id = srv_um_get_prefer_sim();
                                g_uc_p->srv_send_info->send_sim_id = g_uc_p->srv_send_info->send_sim_id>>1;

                            }
                            mmi_uc_process_send();
                        }
                        else if((g_uc_p->srv_main->state == MMI_UC_STATE_REPLY || g_uc_p->srv_main->state == MMI_UC_STATE_REPLY_ALL || g_uc_p->srv_msg_type->caller_specific_msg_type == MMI_UC_MSG_TYPE_SMS_ONLY) 
                            && srv_sim_ctrl_is_available(SRV_UC_SIM_ID_GSM_SIM1) 
                            && srv_sim_ctrl_is_available(SRV_UC_SIM_ID_GSM_SIM2))
                        {
                                g_uc_p->srv_send_info->send_sim_id = g_uc_p->send_info.highlight_sim_id;
                                mmi_uc_process_send();
                        }
...
#endif
#if (defined(__OP01__) && defined(__MMI_DUAL_SIM_MASTER__))
        if((srv_um_get_prefer_sim() != SRV_UM_SIM_UNCLASSIFIED || g_uc_p->srv_main->state == MMI_UC_STATE_REPLY || g_uc_p->srv_main->state == MMI_UC_STATE_REPLY_ALL || g_uc_p->srv_msg_type->caller_specific_msg_type == MMI_UC_MSG_TYPE_SMS_ONLY) 
            && srv_sim_ctrl_is_available((mmi_sim_enum)SRV_UC_SIM_ID_GSM_SIM1) 
            && srv_sim_ctrl_is_available((mmi_sim_enum)SRV_UC_SIM_ID_GSM_SIM2))
           cui_menu_set_non_leaf_item(sender_id, MENU_ID_UC_OPT_SEND, MMI_FALSE);
#endif
	}/*kuldeep_nosim*/
	if(g_uc_p->srv_sim_info->valid_sim_id_info == SRV_UC_SIM_ID_UNCLASSIFIED)
