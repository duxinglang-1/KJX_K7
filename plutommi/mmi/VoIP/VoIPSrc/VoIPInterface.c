...
MMI_BOOL mmi_voip_is_srv_available(void)
{
    ...
    MMI_BOOL sim1_usable;
    MMI_BOOL sim2_usable;
    srv_mode_state_enum current_mode;
#endif
    ...
    /*----------------------------------------------------------------*/
        ...
        sim1_usable = srv_nw_usab_is_usable(MMI_SIM1);
        sim2_usable = srv_nw_usab_is_usable(MMI_SIM2);

        switch (current_mode)
    	{
        case SRV_MODE_SIM1_ONLY:
            is_sim_ready = sim1_usable;
            break;

        case SRV_MODE_SIM2_ONLY:
            is_sim_ready = sim2_usable;
            break;

    	default:
            if (!sim1_usable && !sim2_usable)
        	{
        	    is_sim_ready = MMI_FALSE;
        	}
            else
            {
