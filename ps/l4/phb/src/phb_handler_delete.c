       ...
       */
        ...
        if (phb_ptr->state == PHB_STATE_READY)
        {
            if (control_block->proc_stage == DELETE_NONE)
            {
                ...
                if (!phb_is_type_valid((phb_type_enum)delete_req->type) ||
                    ...
                    delete_req->record_index != (kal_uint16) PHB_INVALID_VALUE)
                {
                    ...
                    #endif
                        phb_se_is_in_range((phb_type_enum)delete_req->type, delete_req->index, &delete_req->total) == KAL_TRUE)
                    {
                   ...
                   */
                        ...
                        if (phb_se_set_control_block(
                                control_block,
                                OP_DELETE,
                                (phb_storage_enum)delete_req->storage,
...
static void phb_delete_err_handler(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    ...
    */
    phb_delete_confirm((phb_errno_enum)control_block->result, control_block->actual_count, control_block->src_id, control_block);
