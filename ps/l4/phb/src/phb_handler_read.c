...
void phb_read_handler(ilm_struct *ilm_ptr, control_block_type *control_block)
{
    ...
    if (ilm_ptr != NULL)
    {
        ...
        if (control_block == NULL)
        {
            phb_read_confirm_no_cb(
                ...
                0,
                (phb_type_enum)l4cphb_read_req->type);
            return;
        }
             ...
             */
                ...
                else
                {
                    ...
                    if (phb_se_set_control_block(
                            control_block,
                            OP_READ,
                            (phb_storage_enum)l4cphb_read_req->storage,
    ...
    else
    {
            ...
            control_block->proc_stage == READ_CONTINUE_SPECIFIC)
        {
            ...
            phb_read_confirm(
                (phb_errno_enum)control_block->result,
                control_block->actual_count,
                control_block->src_id,
                control_block->cnf_msg_id,
                control_block);
...
kal_bool phb_is_read_param_valid(l4cphb_read_req_struct *l4cphb_read_req)
{
    ...
    /*----------------------------------------------------------------*/
    /* Is type valid ? */
    if (!phb_is_type_valid((phb_type_enum)l4cphb_read_req->type))
    {
        return KAL_FALSE;
    }
    ...
    if (l4cphb_read_req->record_index == (kal_uint16) PHB_INVALID_VALUE)
    {
        ...
        return phb_se_is_in_range((phb_type_enum)l4cphb_read_req->type, l4cphb_read_req->index, &l4cphb_read_req->total);
    }
...
void phb_read_usim_fake(control_block_type *control_block, void *cached_data)
{
    ...
    phb_usim_field = (l4c_phb_usim_field*) get_32bits_aligned_val(get_pdu_ptr(peer_buf_ptr, &pdu_length));
