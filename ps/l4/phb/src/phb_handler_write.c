    ...
    if (ilm_ptr != NULL)
    {
        ...
        if (cblk == NULL)
        {
            phb_write_confirm_no_cb(
                ...
                0,
                0,
                (phb_type_enum)l4cphb_write_req->type);
            return;
        }
        ...
        if (phb_ptr->state == PHB_STATE_READY)
        {
            if (cblk->proc_stage == PHB_WRITE_STAGE_NONE)
            {
                ...
                if (!phb_is_type_valid((phb_type_enum)l4cphb_write_req->type) ||
             ...
             */
                ...
                phb_control_block_set_IO(cblk, (phb_type_enum)l4cphb_write_req->type, phb_entry->index, 1);
...
static void phb_write_err_handler(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    */
    cblk->actual_count = 0;
    phb_write_confirm((phb_errno_enum)cblk->result, cblk->actual_count, cblk->src_id, cblk);
                ...
                data_desc_enum desc)
{
    ...
    if (result == PHB_ERRNO_FAIL && desc != DATA_DESC_TOTAL)
    {
        ...
        switch (type)
        {
            case PHB_SNE:
                if (req->file_bitmap[PHB_SNE] & (1 << occurrence))
                {
                    ...
                    {
                        if (phb_desc_is_type2_file((data_desc_enum) (DATA_DESC_SNE + occurrence)))
                        {
                            check_array[count++] = (data_desc_enum) (DATA_DESC_SNE + occurrence);
                        }
                    }
                    else
                    {
                        check_array[0] = (data_desc_enum) (DATA_DESC_SNE + occurrence);
                        return -1;
                    }
                }
            ...
            case PHB_ANR:
                if (req->file_bitmap[PHB_ANR] & (1 << occurrence))
                {
                    ...
                    {
                        if (phb_desc_is_type2_file((data_desc_enum) (DATA_DESC_ANR + occurrence)))
                        {
                            check_array[count++] = (data_desc_enum) (DATA_DESC_ANR + occurrence);
                        }
                    }
                    else
                    {
                        check_array[0] = (data_desc_enum) (DATA_DESC_ANR + occurrence);
                        return -1;
                    }
                }
            ...
            default: /* case PHB_EMAIL: */
                if (req->file_bitmap[PHB_EMAIL] & (1 << occurrence))
                {
                    ...
                    {
                        if (phb_desc_is_type2_file((data_desc_enum) (DATA_DESC_EMAIL + occurrence)))
                        {
                            check_array[count++] = (data_desc_enum) (DATA_DESC_EMAIL + occurrence);
                        }
                    }
                    else
                    {
...
void phb_check_write_usim_continue(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    {
        desc = (data_desc_enum)cblk->check_array[check_index];
    ...
    for (check_index = 0; check_index < PHB_MAX_TYPE2_ENTRIES; check_index++)
    {
        ...
        {
            pbr_pos = phb_desc_get_pbr_pos((data_desc_enum)cblk->check_array[check_index]);
            if (!(phb_ptr->phb_pbr_table[pbr_pos][pbr_rec - 1].free_count))
            {
                phb_check_write_usim_confirm(
                    ...
                    cblk->src_id,
                    cblk->record_index,
                    (data_desc_enum)cblk->check_array[check_index]);
                phb_free_control_block(cblk);
                return;
            }
        }
...
void phb_check_write_usim_handler(ilm_struct *ilm_ptr)
{
    ...
    else
    {
        ...
        if (l4cphb_check_req->record_index == (kal_uint16)PHB_INVALID_VALUE) /* add op */
        {
            ...
            data_desc_enum fail_desc = DATA_DESC_TOTAL;
            kal_uint8 pbr_pos;
