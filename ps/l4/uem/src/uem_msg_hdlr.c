...
//            FLAG_SET_OFF(g_uem_cntx_p->acc_latest_state, UEM_ACCESSORY_TYPE_CLAM);
		...
		case AUX_ID_RFID_OPEN:
			uemdrv_gpio_detect_ind(EXT_DEV_RFID_OPEN);
			break;
		case AUX_ID_RFID_CLOSE:
			uemdrv_gpio_detect_ind(EXT_DEV_RFID_CLOSE);
			break;
	#endif
	#if defined(__BLE_MODULE_SUPPORT__)
		case AUX_ID_BLE_MOD_SPI_OPEN:
			uemdrv_gpio_detect_ind(EXT_DEV_BLE_MOD_SPI_OPEN);
			break;
		case AUX_ID_BLE_MOD_SPI_CLOSE:
			uemdrv_gpio_detect_ind(EXT_DEV_BLE_MOD_SPI_CLOSE);
			break;
	#endif
