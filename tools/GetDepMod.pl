...
my ($CODEBASE_PATH,$PROJECT_NAME,$MODEM_TYPE,@CHANGE_SET) = @ARGV;
my @DEP_MODS = ();
...
open (MF,"<make\\${PROJECT_NAME}_${MODEM_TYPE}.mak") or die "can NOT open make\\${PROJECT_NAME}_${MODEM_TYPE}.mak!!!\n";
while(<MF>)
{
	if (/^\s*(\w+)\s*=\s*(\w+)/)
	{
...
my @tmp = &NOTExistDEPMods("build\\$PROJECT_NAME\\$MODEM_TYPE\\${PLATFORM}r", 
                           &GetSRCMods("build\\$PROJECT_NAME\\log\\infomake.log"));
...
@DEP_MODS = &GetDEPMods("build\\$PROJECT_NAME\\$MODEM_TYPE\\${PLATFORM}r", \@CHANGE_SET);
if (@DEP_MODS)
{
