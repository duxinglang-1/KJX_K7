...
extern kal_uint32 nvram_gen_backup_size;
extern kal_uint32 nvram_gen_backup_total;
...
void nvram_gen_sys_cache_res_func(FILE *fh_file)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    fprintf(fh_file, NVRAM_RESTORE_FUNCTION_NAME, "system", "double", "d", "double");
...
void nvram_gen_ltable_list(void)
{
    ...
    tbl_list= fopen("..\\..\\..\\custom\\common\\hal\\nvram\\nvram_ltable_list.h","w+");
    if (tbl_list == NULL)
    {
    ...
    while (fgets(s, 512, map_file) > 0)
    {
        ...
        if (process_indirect_init == INDIRECT_PROCESS_YET)
        {
            ...
            if (strstr(s, "*(_indirect_init)"))
            {
                ...
                if (strstr(s, "_indirect_init_end = ."))
                {
...
void nvram_gen_lid_size(void)
{
    ...
    logOfTools = fopen("..\\..\\..\\~nvram_lid_information.log","w+");
    if (logOfTools == NULL)
    {
    ...
    else
    {
        ...
        #ifdef __NVRAM_PSEUDO_MERGE__
        logOfSize_Package = fopen("..\\..\\..\\nvram_lid_size_merge.log","w+");
    ...
    for (i = 0;i < nvram_gen_total_lid; i++)
    {
        ...
        for (j = i + 1 ;j < nvram_gen_total_lid; j++)
        {
            ...
            if (stricmp(ldi1->fileprefix, ldi2->fileprefix) == 0 &&
                stricmp(ldi1->fileverno, ldi2->fileverno) == 0)
            {
...
void nvram_gen_statistic(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    outputHeader = fopen("..\\..\\..\\custom\\common\\hal\\nvram\\nvram_lid_statistics_bak.h","w+");
    if (outputHeader == NULL)
    {
        fprintf(stderr, "error: %s - custom\\common\\hal\\nvram\\nvram_lid_statistics_bak.h\n", strerror(errno));
    }
    else
    {
        ...
        fprintf_blank_line(outputHeader);
        fprintf(outputHeader, "#define NVRAM_BACKUP_PARTITION_SIZE     %d\n", nvram_gen_backup_size);
    #endif
        fprintf(outputHeader,"\n#endif /* NVRAM_LID_STATISTIC_H */\n\n");
        fclose(outputHeader);


        memset(sys_str, 0, 2048);
        sprintf(sys_str, "fc %s %s > nul", "..\\..\\..\\custom\\common\\hal\\nvram\\nvram_lid_statistics_bak.h",
        ...
        if(error)
        {
            ...
            /* Overwrite */
            memset(sys_str, 0, 2048);
            sprintf(sys_str, "copy /Y %s %s > nul", "..\\..\\..\\custom\\common\\hal\\nvram\\nvram_lid_statistics_bak.h",
                "..\\..\\..\\custom\\common\\hal\\nvram\\nvram_lid_statistics.h");
            system(sys_str);
        }
        else
        ...
        /* Delete bak file */
        memset(sys_str, 0, 2048);
        sprintf(sys_str, "del %s > nul", "..\\..\\..\\custom\\common\\hal\\nvram\\nvram_lid_statistics_bak.h");
        system(sys_str);
    }
}
...
void nvram_gen_secro(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    outputHeader = fopen("..\\..\\..\\custom\\common\\hal\\nvram\\custom_nvram_secro.c","w+");
    outputHeader2 = fopen("..\\..\\..\\custom\\common\\hal\\nvram\\custom_nvram_secro_tbl.c","w+");
...
void nvram_gen_restore_function(char *project_name, char *broad_ver)
{
    ...
    char output_path[NVRAM_AUTO_GEN_MAX_PATH_LEN];
    int  loop_app_idx;
    int  loop_space;
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    sprintf(output_pathname, "custom_nvram_restore.c");
    sprintf(output_path, "..\\..\\..\\custom\\common\\hal\\nvram\\");
    nvram_gen_create_file(&fh_restore_factory_c,output_path,output_pathname);

    sprintf(output_pathname,"nvram_restore_dom.h");
    sprintf(output_path, "..\\..\\..\\tools\\NVRAMStatistic\\include\\");
    nvram_gen_create_file(&fh_restore_dom_h,output_path,output_pathname);
...
void nvram_gen_database_key(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    sprintf(output_pathname, "custom_nvram_database.h");
    sprintf(output_path, "..\\..\\..\\custom\\common\\hal\\nvram\\");
    nvram_gen_create_file(&database_h,output_path,output_pathname);
...
* RETURNS
*****************************************************************************/
void nvram_gen_custpack_table(char *project_name, char *broad_ver)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    sprintf(output_path, "..\\..\\..\\custom\\common\\VENUS_MMI\\nvram_custpack_table.c");
#elif defined(PLUTO_MMI)
    sprintf(output_path, "..\\..\\..\\custom\\common\\PLUTO_MMI\\nvram_custpack_table.c");
#elif defined(EXTERNAL_MMI) || defined(EMPTY_MMI)
    sprintf(output_path, "..\\..\\..\\custom\\common\\PLUTO_MMI\\nvram_custpack_table.c");
#else
    sprintf(output_path, "..\\..\\..\\custom\\common\\NEPTUNE_MMI\\nvram_custpack_table.c");
#endif

    sprintf(output_path2, "..\\..\\..\\custom\\common\\hal\\nvram\\nvram_custpack_table_lid.c");
...
//trim unexpected type casting(kal_uint32*) & address loock up (&)
kal_uint8* get_str_default_value(kal_uint8 * str_default_value)
{
    kal_int32 i = 0;
	for(i = strlen(str_default_value)-1; i >= 0; i--)
	{
       if(str_default_value[i]=='&' || str_default_value[i]==')')
	      return (str_default_value+i+1);
	}
...
void nvram_gen_cat_xml(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    sprintf(output_pathname, "custom_nvram_lid_cat.xml");
    sprintf(output_path, "..\\..\\..\\custom\\common\\hal\\nvram\\");
    nvram_gen_create_file(&cat_xml_file,output_path,output_pathname);
    ...
    for (i = 0; i < nvram_gen_total_lid; i++)
    {
        ...
        if (ldi == NULL || (ldi->LID == 0 && ldi->size == 0 && ldi->total_records == 0) || ldi->LID < NVRAM_EF_START)
        {
            continue;
        }
        ...
        {
            fprintf(cat_xml_file, " 	   <SYMBOL_NAME>NVRAM_SECUPACK_DEFAULT</SYMBOL_NAME>\n");
        }
        else
        {
            fprintf(cat_xml_file, " 	   <SYMBOL_NAME>%s</SYMBOL_NAME>\n", get_str_default_value(ldi->str_default_value));
        }
        ...
        else
        {
            ...
            for ( j = 0 ; j < default_value_size ; j++)
            {
                ...
                {
                    fprintf(cat_xml_file, "            [0x%02X]</DEFAULT_VALUE>\n", ldi->default_value[j]);
                }
                else if (j == 0)
                {
...
void nvram_gen_trace(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    sprintf(output_pathname, "nvram_gen_trc.h");
    sprintf(output_path, "..\\..\\..\\custom\\common\\hal\\nvram\\");
    nvram_gen_create_file(&trace_file,output_path,output_pathname);
...
int main(int argc, char *argv[])
{
    ...
    if (argc > 1)
    {
        ...
        nvram_gen_custpack_table(argv[1], argv[2]);
    #endif
