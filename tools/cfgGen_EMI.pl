...
sub gen_external_memory_setting
{
 ...
 #   }
    ...
    if (($bb ne 'MT6276') and ($bb ne 'MT6256') and ($bb ne 'MT6255') and ($bb ne 'MT6922') and ($bb ne 'MT6250') and ($bb ne 'MT6260'))
    {
...
__TEMPLATE
    ...
    &error_handler("\\tools\\cfgGen_EMI.pl: pre-compile $custom_EMI_c_file erro!", __FILE__, __LINE__) if ($status != 0);
...
#    print "memory_type=$memory_type\n";
    ...
    for (0..($mcp_count-1))
    {
        ...
        {
           if(($href_MAKEFILE_OPTIONS->{'platform'} eq 'MT6250') || ($href_MAKEFILE_OPTIONS->{'platform'} eq 'MT6260'))
           {
...
__TEMPLATE
     ...
     elsif(($href_MAKEFILE_OPTIONS->{'platform'} eq 'MT6250') || ($href_MAKEFILE_OPTIONS->{'platform'} eq 'MT6260'))
    {
