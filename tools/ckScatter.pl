...
# Constants
#****************************************************************************
my $CKSCATTER_VERNO     = " V0.52";
                         #  v0.52 , Support checking unexpected dummy scatter
                         #  v0.51 , Phase-out system_drive_on_nand feature option
                         #  v0.50 , Support EWS via tools::pack_dep_gen
                         #  v0.49 , Error message refinement
...
print "phase=$phase, stop_build=$stop_build\n";
    ...
    open (FLASHC_HANDLE, "<$CUSTOM_FLASH_H") or &error_handler($stop_build, "[3.2]$CUSTOM_FLASH_H: file error!", __FILE__, __LINE__);
    while (<FLASHC_HANDLE>)
    {
    ...
    open (EMIRELEASEH_HANDLE, "<$CUSTOM_EMI_RELEASE_H") or &error_handler("[3.2/2.0]$CUSTOM_EMI_RELEASE_H: file error!", __FILE__, __LINE__);
    ...
    {
        &error_handler($stop_build, "[3.2]make\\Gsm2.mak: Unsupported ckscatter command!", __FILE__, __LINE__);
    }
...
sub ckscatter_phase1
{
    ...
    open (SCAT_HANDLE, "<$SCATTERFILE") or &error_handler($stop_build, "[3.2/2.0]$SCATTERFILE: file error!", __FILE__, __LINE__);
    while (<SCAT_HANDLE>) {
       if (/\(.+\)/) {next;}
       if (/;/) {s/;.*//;}
    ...
    if (($MAKEFILE_OPTIONS{'platform'} ne 'MT6276') and ($MAKEFILE_OPTIONS{'platform'} ne 'MT6575') and ($MAKEFILE_OPTIONS{'platform'} ne 'MT6573'))  # MT6276 is skipped because these regions no longer obey the alignment rule for shrinking purpose
    {
        ...
        if ($rule > 0)
        {
            &error_handler($stop_build, "[2.0]$SCATTERFILE: Scatter File DSP check rule fail! Please contact with Scatter File owner!", __FILE__, __LINE__);
        }
    }
        ...
        if (exists $MAKEFILE_OPTIONS{'platform'} and (&config_query_arm9($MAKEFILE_OPTIONS{'platform'}) == 1))
        {
            foreach my $e ( keys %ExecView )
            {
                if (($e =~ /EXTSRAM_DSP_[TR]X/) || ($e eq 'CODE_PATCH_CODE'))
                {
                    ...
                    {
                        &error_handler($stop_build, "[2.0]$SCATTERFILE: $e lack of start address! Please contact with Scatter File owner!", __FILE__, __LINE__);
                    }
                    if (hex($ExecView{$e}->{'addr'}) % (4*1024) != 0)
                    {
                        &error_handler($stop_build, "[2.0]$SCATTERFILE: $e start address needs to be 4KB-aligned! Please contact with Scatter File owner!", __FILE__, __LINE__);
                    }
                }
            }
        }
    ...
    if (1 != &sysUtil::is_SmartPhone($MAKEFILE_OPTIONS{'platform'})) # smartphone has no need to be block-aligned
    {
        &get_block_table();
            ...
            if (/BOODLOADER|__HIDDEN_SECURE_RO|__HIDDEN_FLASHTOOL_CFG|__HIDDEN_CUST_PARA/)
            {
                foreach my $i (@offset)
                {
                    if (hex($LoadView{$_}->{'begin'}) >= $i)
                    {
                        ...
                        if ($tmp != 0)
                        {
                            &error_handler($stop_build, "[2.0]$SCATTERFILE: $_ start address needs to be block-aligned! Please contact with Scatter File owner!", __FILE__, __LINE__);
                        }
                        else
                        {
                            last;
...
sub ckscatter_phase2
{
    ...
    # 2.10 >>>  When DUMMY_SCATTER_ENABLE is turned on (only available on NOR booting), ckscatter should bypass
    #      >>>  When unexpected dummy scatter is used, ckscatter fail
    if (&FileInfo::is("dummy_scatter_enable", "TRUE") and $DUMMY_SCATTER_ALLOW == 1)
    {
        ...
        if (&FileInfo::is("nand_flash_booting", "NONE") and (!exists $MAKEFILE_OPTIONS{'emmc_booting'} or $MAKEFILE_OPTIONS{'emmc_booting'} eq 'NONE'))
        {
            print "bypass check when DUMMY_SCATTER_ENABLE = TRUE!\n";
            return;
        }
    }
    else  # DUMMY_SCATTER_ENABLE != TRUE or !InsideMTK
    {
        # To check if the input scatter is dummy scatter
        if(&is_DummyScatter() == 1) 
        {
            &error_handler($stop_build, "[1.5]Unexpected dummy scatter is used, please do command m sysgen to generate the correct scatter file!", __FILE__, __LINE__);
        }    	
    }

    # 2.11 >>>  6516 series do not have FAT information and thus is bypassed in ckscatter_phase2
    ...
    # 2.12 >>>  BT/MT projects do not contain EXTSRAM and thus is bypassed in ckscatter_phase2
    ...
    # 2.13 >>>  When ZIMAGE_SUPPORT or DCM Compression is turned on, ckscatter will misjudge because it cannot recognize compressed case. Besides, another tool will be used to check ZIMAGE_SUPPORT/DCM Compression memory expiration
    ...
    open (LIS_HANDLE, "<$LIS_TEMP") or &error_handler($stop_build, "[3.2/2.0]$LIS_TEMP: file error!", __FILE__, __LINE__);
    my $LoadRegion_seq = 0;
    my $ExecRegion_seq = 0;
    while (<LIS_HANDLE>) {
...
#           print $_ ,  "\n";
    ...
    &error_handler($stop_build, "[1.5]$LIS_TEMP: LIS file empty! Please resolve link errors first!", __FILE__, __LINE__) if (($LoadRegion_seq == 0) && ($ExecRegion_seq == 0));

    # 2.3 >>> parse key definition in ~flash_cfg_tmp.c
    open (FLASH_TMP_HANDLE, "<$FLASH_CFG_TMP") or &error_handler($stop_build, "[3.2]$FLASH_CFG_TMP: file error!", __FILE__, __LINE__);
    ...
    if ((exists $MAKEFILE_OPTIONS{'nand_flash_booting'} and $MAKEFILE_OPTIONS{'nand_flash_booting'} ne 'NONE') or (exists $MAKEFILE_OPTIONS{'emmc_booting'} and $MAKEFILE_OPTIONS{'emmc_booting'} ne 'NONE'))
    {
        ...
        {
            &error_handler($stop_build, "[1.1]$SCATTERFILE: RAM consumption has exceeded its upper bound! Please turn off some features or change to a larger MCP!", __FILE__, __LINE__);
        }
    ...
    else
    {
        ...
        {
            &error_handler($stop_build, "[1.1]$SCATTERFILE: ROM consumption has exceeded its upper bound! Please turn off some features or change to a larger MCP!", __FILE__, __LINE__);
        }
        if (($MAKEFILE_OPTIONS{'platform'} ne 'MT6251'))
        {
            printf "EXEC VIEW , Physical Device Density 0x%x , Scatter File Configuration 0x%x , Scatter File DUMMY_END 0x%x , Scatter File Usage 0x%x\n",
...
sub comp_ram_size_dummy_end
{
    ...
    {
        die "[2.0]Unknown Scatter File Structure";
    }
...
sub parse_scatter_file_structure
{
    ...
    foreach my $file_line_no (sort {$a <=> $b} keys %Scat_Content)
    {
        ...
        if    ($text_line =~ m/{/)
        {
            if ($view > 1)
            {
                &error_handler($stop_build, "[2.0]$SCATTERFILE: Unrecognizable view at $file_line_no! Please contact with Scatter File owner!", __FILE__, __LINE__);
            }
            $view++;
            if (not defined $last_line_no)
            {
                &error_handler($stop_build, "[2.0]$SCATTERFILE: No region name detected before $file_line_no!Please contact with Scatter File owner!", __FILE__, __LINE__);
            }
...
#****************************************************************************
# subroutine:  is_DummyScatter
# return:      0: is not dummy scatter
#              1: is dummy scatter
#****************************************************************************
sub is_DummyScatter
{
