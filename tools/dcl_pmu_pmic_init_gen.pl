...
$now_date=join("_",($year+1900,$mon+1,$day));	#年 $year 是從1900 年啟算；而月份的話是從0起算
...
for($i=20;$i>=0;$i--)
{
		...
		if($ECO_version[$i] == 1)
		{
			...
			foreach $data_handle (@raw_data)
			{
				...
				if($data_handle =~/^ECO_TRUE_E(\d+)/ )	#check 是否為ECO開頭, check ECO version
				{
					if($1 == $i)
					{
						$parsing = 1;	
				...
				elsif($data_handle =~/^ECO_E(\d+)/ )	#check 是否為ECO開頭, check ECO version
				{
					if($1 == $i)
					{
						$parsing = 1;	
				...
				if($parsing == 1)
				{
					...
					elsif($data_handle =~/^ECO_/ )	#check 是否為ECO開頭, check ECO version	
					{
