...
if ($BOOL_EMI_CLEAN eq 'TRUE')
{
    ...
    if(($MAKEFILE_OPTIONS{'platform'} eq 'MT6260') && ($MAKEFILE_OPTIONS{'nand_flash_booting'} ne 'NONE'))
    {
...
if(($MAKEFILE_OPTIONS{'platform'} eq 'MT6250') || ($MAKEFILE_OPTIONS{'platform'} eq 'MT6260'))
{
...
if ($BBtbl_SERIALFLASH{$MAKEFILE_OPTIONS{'platform'}} == 1)
{
  ...
  elsif(($MAKEFILE_OPTIONS{'platform'} eq 'MT6250') || ($MAKEFILE_OPTIONS{'platform'} eq 'MT6260'))
  {
  	    if($MAKEFILE_OPTIONS{'serial_flash_support'} eq 'TRUE')
  	    {
...
my $combo_emi_count;
if (($MAKEFILE_OPTIONS{'platform'} eq 'MT6252') || ($MAKEFILE_OPTIONS{'platform'} eq 'MT6250') || ($MAKEFILE_OPTIONS{'platform'} eq 'MT6260'))
{
...
print "RAM size :" . $COMM_MDL_INFO{1}->{'Size (Mb)'} . "Mb\n";
print "ROM size : " . $rom_size . "Mb\n";
#****************************************************************************
...
sub Find_MDL
{
 ...
 elsif ($MAKEFILE_OPTIONS_LOCAL->{'platform'} eq 'MT6250')  # MT6256 uses separate MDL
  {
      $MEMORY_DEVICE_LST = "MemoryDeviceList_MT6250_Since11BW1212.xls";
      $MEMORY_DEVICE_LST_INT = "MemoryDeviceList_MT6250_Internal.xls";
      $IDX_COUNT = 3;
  }
  elsif ($MAKEFILE_OPTIONS_LOCAL->{'platform'} eq 'MT6260')  # MT6256 uses separate MDL
  {
...
while (<CUSTOM_MEMORY_DEVICE_HDR>)
{
   ...
   if (/^#define\s+(\w+)\s+\(([\w|\-]*)\)/ || /^#define\s+(\w+)\s+([\w|\-]*)/)
   {
      ...
      if ($option =~ /COMBO_MEM(\d+)_CS(\d+)_PART_NUMBER/)
      {
         ...
         elsif(($MAKEFILE_OPTIONS_LOCAL->{'platform'} eq 'MT6250') || ($MAKEFILE_OPTIONS_LOCAL->{'platform'} eq 'MT6260'))
         {
                  $TMP_PART_NUMBER{$cs} = $content;
         }
         else
...
elsif (($CUSTOM_MEM_DEV_OPTIONS_LOCAL->{MEMORY_DEVICE_TYPE} eq 'SERIAL_FLASH') && ($MAKEFILE_OPTIONS_LOCAL->{'sip_serial_flash_size'} eq 'NONE'))
{
    ...
    else
    {
    	  if($MAKEFILE_OPTIONS_LOCAL->{'dual_serial_flash_support'} eq 'TRUE')
    	  {
    	         	  for (1..$CUSTOM_MEM_DEV_OPTIONS_LOCAL->{COMBO_MEM_ENTRY_COUNT})
               {
               	  if(($MAKEFILE_OPTIONS_LOCAL->{'platform'} eq 'MT6250') || ($MAKEFILE_OPTIONS_LOCAL->{'platform'} eq 'MT6260'))
               	  {
