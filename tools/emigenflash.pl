...
sub get_common_MDL_info
{
    ...
    for (1..$combo_mem_count)
    {
        ...
        if (($mem_dev_type eq 'LPSDRAM') or ($mem_dev_type eq 'LPDDR') or ($mem_dev_type eq 'LPDDR2'))
        {
            ...
            if($MAKEFILE_OPTIONS_LOCAL->{'platform'} eq 'MT6276')
            {
             ...
             elsif(($mem_dev_type eq 'SPI_NAND'))
             {
                 ...
                 {
                     $cur_flash_size = $info_list_href->[$_]->{0}->{'NAND Size (Mb)'} * 1024 * 1024;
                 }
             }
        }
        ...
        if (($platform ne 'MT6252') and ($platform ne 'MT6255') and ($platform ne 'MT6250') and ($platform ne 'MT6260'))  # MT6252 uses SIP
        {
        ...
        if (($platform ne 'MT6252') and ($platform ne 'MT6250') and ($platform ne 'MT6260'))  # MT6252 uses SIP
        {
  ...
  else
  {
    ...
    ### for MT6252 SIP case
    if (($platform eq 'MT6252') || ($platform eq 'MT6250') || ($platform eq 'MT6260'))
    {
