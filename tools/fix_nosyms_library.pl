...
#* Description:
#* ------------
#*   This script will add a armlibc_rt.obj into a library, armlibc_rt.obj only
...
END
	...
	open FH, ">", "armlibc_rt.c" || die "Can't open armlibc_rt.c for writing";
	print FH $source;
	close FH;
	
	my $status = system($ARMCC, "-c", "-o", "armlibc_rt.obj", "armlibc_rt.c");
	if ($status != 0) {
		die "Can't compile armlibc_rt.c";
	}
}
...
sub usage {
	...
	print STDERR "    --force		replace armlibc_rt.obj in library unconditionally\n";
}
...
for my $lib (@to_be_fix_libs) {
	...
	if (not $force_fix) {
		my $has_it = grep { lc $_ eq "armlibc_rt.obj" } @lib_objs;
		if ($has_it) {
			print "$lib: failed, already has armlibc_rt.obj\n";
			next;
		}
	}
	my $status = system("armar", "-sr", "$lib", "armlibc_rt.obj");
	if ($status != 0) {
		print "$lib: failed, can't add armlibc_rt.obj into $lib\n";
		next;
	}
	print "$lib: succeeded\n";
}

unlink "armlibc_rt.c", "armlibc_rt.obj";
