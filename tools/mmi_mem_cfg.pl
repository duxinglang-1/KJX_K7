...
my $g_vrtMemUnitSize = "VRT_MEM_UNIT_SIZE";                             # defined at vfx_sys_config.h
my $g_vrtMemFactorDefault = "VFX_SYS_APP_DEFAULT_VRT_CACHE_FACTOR";     # defined at vfx_sys_config.h

# flags
my $g_flagMulti = "multi";
...
sub genAppDataH()
{
    ...
    print hFile "#define MMI_APP_INFO_TOTAL_NUMBER (".scalar(keys(%{$memInfo_href})).")\n\n";
    print hFile "#endif /* $hMacro */\n\n";
    close(hFile);
}
...
sub readCfgFile()
{
    ...
    foreach my $line (<hFile>)
    {
        ...
        while ($line =~ /\[([\w]+)\]([^\[]+)/g)
        {
            ...
            if ($attr eq $g_tagId)
            {
                ...
                {
                    if (!defined ${$memInfo_href}{$id}{$g_flagMulti})
                    {
                        ${$memInfo_href}{$id}{$g_flagMulti} = 0;
                    }
                    ${$memInfo_href}{$id}{$g_flagMulti}++;

                    $id = $id."_S${$memInfo_href}{$id}{$g_flagMulti}S_";
                }
            }
    ...
    foreach my $id (sort keys(%{$memInfo_href}))
    {
        my $count;
        my @datas;
        next if (!defined ${$memInfo_href}{$id}{$g_flagMulti});

        $count = ${$memInfo_href}{$id}{$g_flagMulti};
        delete(${$memInfo_href}{$id}{$g_flagMulti});

        ${$memInfo_href}{$id."_S0S_"} = ${$memInfo_href}{$id};
        ${$memInfo_href}{$id} = ();
        ${$memInfo_href}{$id}{$g_flagMulti} = $count + 1;

        ${$memInfo_href}{$id}{$g_tagStr} = ${$memInfo_href}{$id."_S0S_"}{$g_tagStr} if (defined(${$memInfo_href}{$id."_S0S_"}{$g_tagStr}));
        ${$memInfo_href}{$id}{$g_tagVrtMemFactor} = ${$memInfo_href}{$id."_S0S_"}{$g_tagVrtMemFactor} if (defined(${$memInfo_href}{$id."_S0S_"}{$g_tagVrtMemFactor}));
    }
}
...
sub processDependency()
{
    ...
    foreach my $id (sort keys(%{$memInfo_href}))
    {
        ...
        $heapStr    = "0";
        $heapStr    = ${$memInfo_href}{$id}{$g_tagHeap} if (defined ${$memInfo_href}{$id}{$g_tagHeap});
        ...
        if (defined ${$memInfo_href}{$id}{$g_flagMulti})
        {
            ...
            $heapStr = $g_prefix{$g_tagHeap}.$base_id if (defined ${$memInfo_href}{$base_id}{$g_tagHeap});
            $extraStr = "";
            $appFgStr = "";
            
            @datas = ();
            for($i=0; $i<${$memInfo_href}{$id}{$g_flagMulti}; $i++)
            {
                push(@datas, $g_prefix{$g_tagBase}.$id."_S".$i."S_");
            }
            $baseStr = "max$i(".join(",", @datas).")";

            @datas = ();
            for($i=0; $i<${$memInfo_href}{$id}{$g_flagMulti}; $i++)
            {
                push(@datas, $g_prefix{$g_tagTotal}.$id."_S".$i."S_");
            }
            $totalStr = "max$i(".join(",", @datas).")";

            $fgStr = $g_prefix{$g_tagTotal}.$id." - ".$g_prefix{$g_tagBase}.$id;
        }
        ...
        @datas = ($baseStr, $fgStr, $totalStr, $extraStr, $appFgStr, $cuiBaseStr, $cuiFgStr, $heapStr);
        ...
        ${$memInfo_href}{$id}{$g_tagHeap}       = $datas[7];
    }
}
