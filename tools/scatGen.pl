...
# Scatter files with no Check-in history will be deleted
...
&ParseFlashInfo($CUSTOM_FLASH_H, \%FLASH_H_Value, \@FLASH_H_Value_BLK_LIST, 
...
&SetBlockTable(\@FLASH_H_Value_BLK_LIST, \%NORFLASH_INFO_BLOCKS_TABLE);
#****************************************************************************
...
sub GenBLScatter
{
    ...
    require "tools/scatGenLib.pl";
    PrintDependModule();
    my ($bl_scat_content, $ext_bl_scat_content) = &scatGenBL_main(sub { my ($key) = @_;
                                            if (exists $MAKEFILE_OPTIONS{$key})
                                            {   return $MAKEFILE_OPTIONS{$key};
                                            }
                                            else
    ...
    ### Generate BL scatter
    open (BL_SCATTER_TXT, ">$BL_SCATTERFILE") or &sysUtil::sysgen_die("[2.0]$BL_SCATTERFILE: file error!", __FILE__, __LINE__);

    print BL_SCATTER_TXT &copyright_file_header_for_scatter();

    print BL_SCATTER_TXT &description_file_header_for_scatter( $BL_SCATTERFILE,
                             "defines the memory map for boot-loader\n",
                             "system auto generator ". &scatGen_verno(). " + sysGenUtility" . &sysUtil::sysgenUtility_verno());
    print BL_SCATTER_TXT $bl_scat_content;
    close BL_SCATTER_TXT;

    print "$BL_SCATTERFILE is generated\n";

    ### Generate EXT-BL scatter
    open (EXT_BL_SCATTER_TXT, ">$EXT_BL_SCATTERFILE") or &sysUtil::sysgen_die("[2.0]$EXT_BL_SCATTERFILE: file error!", __FILE__, __LINE__);

    print EXT_BL_SCATTER_TXT &copyright_file_header_for_scatter();
    print EXT_BL_SCATTER_TXT &description_file_header_for_scatter( $EXT_BL_SCATTERFILE,
                             "defines the memory map for boot-loader\n",
                             "system auto generator ". &scatGen_verno(). " + sysGenUtility" . &sysUtil::sysgenUtility_verno());
    print EXT_BL_SCATTER_TXT $ext_bl_scat_content;
    close EXT_BL_SCATTER_TXT;
...
sub custom_secure_config_h_file_body
{
    ...
    $sds_block_size = &config_query_flash_block_size($flash_end_addr - 1) if(&FileInfo::is_NOR(\%MAKEFILE_OPTIONS));
    ...
    if ((defined $MAKEFILE_OPTIONS{'sds_support'} and $MAKEFILE_OPTIONS{'sds_support'} eq 'TRUE') and (&FileInfo::is_NOR(\%MAKEFILE_OPTIONS)))
    {
        ...
        {   
            my $cur_blk_size = &config_query_flash_block_size($sds_tmp_addr - 1);
...
sub custom_img_config_h_file_body
{
    ...
    if ($cs_swap_config == 1)
    {
        $cs_swap_address = &sysUtil::query_cs1_addr($BOARD_PARAMETERS{'BB'}, 0);
    }
    else
    {
    	   $cs_swap_address = 0;
...
__TEMPLATE
    ...
    else
    {
        ...
        if (&sysUtil::is_sv5($BOARD_PARAMETERS{'BB'}) == 1)
        {
            ...
            elsif ((($BOARD_PARAMETERS{'BB'} eq 'MT6250') or ($BOARD_PARAMETERS{'BB'} eq 'MT6260'))and ($package_seg eq '32_32_SEG' or $package_seg eq '16_32_SEG'))
            {
                $HEADER_BLOCK_MIN_SIZE = 2 * 1024;
                $BL_MIN_SIZE = 10 * 1024;
                $ROOT_CERT_MIN_SIZE = 0;
            ...
            $nReservedBLSize = $HEADER_BLOCK_MIN_SIZE + $BL_MIN_SIZE + $ROOT_CERT_MIN_SIZE + $EXT_BL_MIN_SIZE + $EXT_BL_BAK_MIN_SIZE;
            $ue_flash_base_address = &_config_query_next_block_aligned_addr(0x0, $nReservedBLSize);
            if(&FileInfo::is("ext_bl_update_support", "TRUE", \%MAKEFILE_OPTIONS))
            {
                $ue_flash_base_address = &_config_query_next_block_aligned_addr($HEADER_BLOCK_MIN_SIZE + $BL_MIN_SIZE + $ROOT_CERT_MIN_SIZE, $EXT_BL_MIN_SIZE);
                $ue_flash_base_address = &_config_query_next_block_aligned_addr($ue_flash_base_address, $EXT_BL_BAK_MIN_SIZE);
            }
        }
        else
        {
            ...
            $ue_flash_base_address=&_config_query_next_block_aligned_addr(0x0, $ReservedBLSize);
            if(&FileInfo::is("ext_bl_update_support", "TRUE", \%MAKEFILE_OPTIONS))
            {
                $ue_flash_base_address = &_config_query_next_block_aligned_addr($BL_MIN_SIZE, $EXT_BL_MIN_SIZE);
                $ue_flash_base_address = &_config_query_next_block_aligned_addr($ue_flash_base_address, $EXT_BL_BAK_MIN_SIZE);
            }
        }
        $str_ue_flash_base_address = sprintf("0x%08X", $ue_flash_base_address);
        ...
        {
        	   $tmp_ue_flash_space_size -= &config_query_flash_block_size($image_base_addr);
       	    $image_base_addr += &config_query_flash_block_size($image_base_addr);
        }
        ...
        if (defined $fota and $fota eq 'FOTA_DM')
        {
            ...
            while ($blocks != 0)
            {
                $last_block_size         = &config_query_flash_block_size($query_address);
        ...
        {
            my $last_block_size      = &config_query_flash_block_size($query_address);
            $query_address           -= $last_block_size;
        }
        ...
        if(exists $MAKEFILE_OPTIONS{'split_binary_support'} and $MAKEFILE_OPTIONS{'split_binary_support'} eq 'FACTORY_COMBINE')
        {
            ...
            my $nFactoryBlock = &config_query_flash_block_size($nFactoryBinSize);
            if($nFactoryBinSize % &config_query_flash_block_size($nFactoryBinSize) == 0)
            {
...
sub comp_ram_size
{
    ...
    elsif ((defined $FEATURE_CONFIG_Value{'PROJECT_EXPECTED_RAM_LIMIT_NFB'}) && ((exists $MAKEFILE_OPTIONS{'nand_flash_booting'} and $MAKEFILE_OPTIONS{'nand_flash_booting'} ne 'NONE') or (exists $MAKEFILE_OPTIONS{'emmc_booting'} and $MAKEFILE_OPTIONS{'emmc_booting'} or 'NONE')))
    {
...
#****************************************************************************
sub config_query_flash_block_size
{
    my ($offset) = @_;
    my $flash_size = ($g_USE_DUMMY_SCATTER eq 'TRUE') ? 0x8000000 : hex($FLASH_H_Value{'NOR_FLASH_DENSITY'});
    ...
    my $href   = \%NORFLASH_INFO_BLOCKS_TABLE;
    &sysUtil::sysgen_die("[3.2]Block Size Information Unavailable, Please check EMI DataBase and custom_flash.c!", __FILE__, __LINE__) unless defined $href;
    ...
    &sysUtil::sysgen_die("[3.2]The configuration of block size in custom_flash.h is incorrect!", __FILE__, __LINE__);
}
...
#****************************************************************************
sub _config_query_next_block_aligned_addr
{
    my ($addr, $offset) = @_;
    my $flash_size   = ($g_USE_DUMMY_SCATTER eq 'TRUE') ? 0x8000000 : hex($FLASH_H_Value{'NOR_FLASH_DENSITY'});
    &sysUtil::sysgen_die("[2.0]Query Block-aligned addr at ($addr + $offset) larger than available size!", __FILE__, __LINE__) if (($addr+$offset) > $flash_size);

    my $tmp_addr = $addr;
    while ($tmp_addr < ($addr+$offset))
    {
        my $nblock_size = &config_query_flash_block_size($tmp_addr);
        $tmp_addr += $nblock_size;
        $tmp_addr = ($tmp_addr % $nblock_size ==0) ? $tmp_addr : (int($tmp_addr /$nblock_size)+1)*$nblock_size;
    }
    &sysUtil::sysgen_die("[2.0]Query Block-aligned addr at $tmp_addr larger than available size!", __FILE__, __LINE__) if ($tmp_addr > $flash_size);
    return $tmp_addr;
}
...
sub config_query_package_storage_size
{
    ...
    if ($MAKEFILE_OPTIONS{'fota_enable'} eq 'FOTA_DM')
    {
        ...
        else
        {
            ...
            {
                my $tmp_block_sz;
                $tmp_block_sz = &config_query_flash_block_size($tmp_fs_base);
...
sub config_query_backup_storage_size
{
    ...
    if ($MAKEFILE_OPTIONS{'fota_enable'} eq 'FOTA_DM')
    {
        ...
        else
        {
            ...
            {
                my $cur_block_size = &config_query_flash_block_size($query_address);
...
sub copyright_file_header_for_scatter
{
    ...
    if (&sysUtil::NeedPreCompile($bb, $dsp_solution, $alice, $mba, &isNFB()))
    {
...
#****************************************************************************
# Subroutine: SetBlockTable
# Purpose: To gather Block Table
# Input: \@FLASH_H_Value_BLK_LIST(input), 
#        \%NORFLASH_INFO_BLOCKS_TABLE(output)
#****************************************************************************
sub SetBlockTable
{
    my ($FLASH_H_Value_BLK_LIST_aref, $BlockTableHash_ref) = @_;
    ...
    if (scalar(@FLASH_H_Value_BLK_LIST) > 0)
    {
        ...
        foreach(@FLASH_H_Value_BLK_LIST)
        {
            if (/\{(0x\w+),\s*(0x\w+)\}/)
            {
               $BlockTableHash_ref->{hex($1)} = hex($2);
            }
        }
    }
}
...
#****************************************************************************
# Subroutine: ParseFlashInfo
# Purpose: parse key definition in CUSTOM_FLASH_H
# Input: 1. strFilePath, 2. Hash_ref(Result Container)
#        3. Block_List Array_ref
#        4. Region_List Array_ref
#        5. Bank_List Array_ref
#****************************************************************************
sub ParseFlashInfo
{
    my ($strFilePath, $Hash_ref, $BLK_LIST_aref, $REGION_LIST_aref, $BANK_LIST_aref) = @_;
    open (FLASHC_HANDLE, "<$strFilePath") or &sysUtil::sysgen_die("[3.2]$strFilePath: file error!", __FILE__, __LINE__);
    PrintDependency($strFilePath);
    while (<FLASHC_HANDLE>) {
       ...
       elsif (defined $FLASH_H_Value{'RegionInfoFlag'})
       {
          ...
          delete $FLASH_H_Value{'RegionInfoFlag'} if (/^\s+EndoriRegionInfo\s+/);
       }
       elsif (/^static\s+NORBankInfo\s+oriBankInfo\S+/)
       {
          $Hash_ref->{'BankInfoFlag'} ++;
       }
       elsif (defined $FLASH_H_Value{'BankInfoFlag'})
       {
