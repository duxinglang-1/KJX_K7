...
sub layout_ext_bl
{
    ...
    else
    {
        ...
        else
        {
            ...
            $ext_bootloader_base = sprintf("0x%08x", hex($maui_ext_bootloader_base)) if($bb eq "MT6255" or $bb eq "MT6922" or $bb eq "MT6250" or $bb eq "MT6260");
        }
    }
...
sub layout_read_only_ev
{
    ...
    $template .= &format_execution_view('STACK_AREA',
                                        &config_query_hash_bb(\%BBtbl_STACK_AREA_base, $bb, __LINE__),
                                        '0x1000',
                                        \@stack_area_libs);
    $template .= &format_execution_view('EXT_UN_INIT',
                                        $ext_un_init_base,
                                        undef,
                                        \@ext_un_init_libs);
    $template .= &format_execution_view('SHARE_BUF',
                                        $share_buf_base,
                                        $share_buf_size,
                                        ["*                          (MINI_EXT_DA_SHARE)"]);
    if (($nfb ne 'NONE') and (&sysUtil::is_noSYSRAM($bb) != 1))  # Only generate on NFB and which has SysRAM
    {
...
sub layout_ext_bootloader_ev
{
    ...
    $template .= &format_execution_view('REGION_INIT_SUB',
                                        '+0x0',
                                        undef,
                                        ["* (REGION_INIT_SUB)"]);
