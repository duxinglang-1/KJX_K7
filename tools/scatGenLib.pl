...
sub scatGen_main
{
    ...
    if (&isNAND($func))
    {
        ...
        if($is_secure and $g_nfb_or_emb eq 'ENFB')
        {
            &sysUtil::sysgen_die("[1.1]Unsupported Feature Combination : SECURE + ENFB!", __FILE__, __LINE__);
        }
    ...
    else
    {
        ...
        if (defined $zimage and $zimage eq 'TRUE')
        {
            if ((defined $fota and $fota eq 'TRUE'))
            {
...
sub scatGen_verno
{
    return "v6.80" . " + scatFrame". scatFrame::scatFrame_verno() ;
...
sub layout13
{
    ...
    my $sip_flash_size = &$feature_query_function_ptr('sip_serial_flash_size');
    my $isSmallseg = (($bb eq 'MT6250' || $bb eq 'MT6260') and (($sip_flash_size eq '32M_BITS') or ($sip_flash_size eq '24M_BITS'))) ? 1 : 0;    
    ...
    elsif (($bb eq 'MT6250' or $bb eq 'MT6260') and $isSmallpackage)
    {
        $HEADER_BLOCK_MIN_SIZE = 2 * 1024;
        $BL_MIN_SIZE = 10 * 1024;
        $ROOT_CERT_MIN_SIZE = 0;
    ...
    my $cbr_region;
    my $cbr_block_size;
    $cbr_block_size = $isSmallseg ? (4*1024) : (64*1024); #the CBR block size is smaller of MT6250/MT6260 small segment
    ...
    my $cbr_size = $cbr_blk * $cbr_block_size;
...
__LAYOUT
    ...
    or (($bb eq 'MT6250' or $bb eq 'MT6260') and $isSmallpackage))
    {
...
sub layout12
{
    ...
    my $comp_zimage = $feature_config_href->{'CONFIG_ZIMAGE_MAX_COMPRESSED_SIZE'};
    my $mba = &$feature_query_function_ptr('multiple_binary_files');
    # --- --- ---
    my $fbase   = sprintf("0x%08x", &lv_rom_fbase($bb));
    ...
    &WarnMinus(($available_rom - hex($comp_zimage)), __FILE__, __LINE__, "$bb_folder\\custom_FeatureConfig.h: CONFIG_ZIMAGE_MAX_COMPRESSED_SIZE cannot be configured larger than available ROM size! Please configure CONFIG_ZIMAGE_MAX_COMPRESSED_SIZE within ".&CommonUtil::Dec2Hex($available_rom)."!");
    $available_rom -= hex($comp_zimage);
    # if secure support
    my $secure = &$feature_query_function_ptr('secure_support');
    ### Get bottom regions
    my $bottom_region_size = 0;
    ...
    DCM compressed region($nDCMCompressedSize)/ALICE region($comp_alice)/BOOT_ZIMAGE regions($comp_boot_zimage)");
    $available_rom -= $total_viva_regions;
 
    ### VIVA layout
    my $viva_base = "+0x400";
    $viva_base = sprintf("0x%08x", &config_query_flash_base_address($bb) + $available_rom) if(defined $mba and $mba eq 'TRUE');
    ...
    ### ALICE layout
    my $lv_alice;
    $lv_alice = &layoutALICE("+0x0") if(defined $alice and $alice eq 'TRUE');
    ### MBA layout
    my $lv_mba;
    my $lv_jump_table;
    my $mba_decomp;
...
__LAYOUT
    ...
    my $zimage_ev_attribute = ((defined $boot_zimage and $boot_zimage eq 'TRUE') or (defined $mba and $mba eq 'TRUE')) ? "OVERLAY" : "";
...
__LAYOUT
    ...
    {    
        ...
        # ALICE
        $lv_alice = &layoutALICE("+0x20000000");
    }
...
sub config_query_flash_block_sz
{
    ...
    &sysUtil::sysgen_die("[2.0]Query Block Size at $offset larger than available size!", __FILE__, __LINE__) if ($offset > $flash_size);
...
$sv5_lang_gfh
    ...
    LANG_PACK_ROM_CONTENT $lang_pack_content_base $lang_pack_content_size
    {
         *  (LANGPACK_RODATA, LANGPACK_ROCODE)
         ...
         vf_*.obj (+RO-CODE, +RO-DATA)
         CustStrSearchComp.obj (+RO-CODE, +RO-DATA)
         CustStrResComp.obj (+RO-CODE, +RO-DATA)
         CustStrMapComp.obj (+RO-CODE, +RO-DATA)
    }
__TEMPLATE
    return $template;
}
...
sub EXTSRAM_type_7
{
    ...
    $cached_dummy_end_start_address = sprintf("0x%08x", (($cached_dummy_end_start_address_val - $cached_dummy_end_skip_for_zimage) | 0xF0000000));
...
__TEMPLATE
   # --- --- --- 
    ...
    if (defined $BBtbl_extra_smallcfg{$bb} and $BBtbl_extra_smallcfg{$bb})
    {
       ...
       elsif ($bb eq 'MT6250' || $bb eq 'MT6260')
       {
...
sub DCMP_code_layout
{
    ...
    foreach my $strPostfix (@$ExistCombination)
    {
        ...
        $layout .= &DCMP_code_template($strPostfix, $nBaseAddr, $nMaxSize, $strInputSection);
        ...
        {                                #($strPostfix, $nBaseAddr, $nMaxSize, $InputSection)
            ...
            $layout .= &DCMP_code_template($item->[0], $item->[1], $item->[2], $strInputSection);
        }
    }
