...
if ($flag == $ERR_OK)
{
    ...
    if($g_stack_sharing_option == 1)
    {
        ...
        if($g_stack_sharing_stack_size < 50*1024)    		
        {
            $STACK_POOL_COEFFECIENT = 10;
        }
        elsif($g_stack_sharing_stack_size >= 50*1024 && $g_stack_sharing_stack_size < 60*1024)    		
        {
            $STACK_POOL_COEFFECIENT = 15;
        }
        elsif($g_stack_sharing_stack_size >= 60*1024 && $g_stack_sharing_stack_size < 70*1024)    		
        {
            $STACK_POOL_COEFFECIENT = 20;
        }
        elsif($g_stack_sharing_stack_size >= 70*1024 && $g_stack_sharing_stack_size < 80*1024)    		
        {
            $STACK_POOL_COEFFECIENT = 25;
        }
        elsif($g_stack_sharing_stack_size >= 80*1024 && $g_stack_sharing_stack_size < 160*1024)
        {
            $STACK_POOL_COEFFECIENT = 30;
        }
        elsif($g_stack_sharing_stack_size >= 160*1024 && $g_stack_sharing_stack_size < 180*1024)
        {
            $STACK_POOL_COEFFECIENT = 35;
        }
        else
        {
            $STACK_POOL_COEFFECIENT = 40;
        }
    }
 ...
 */
    #define GLOBAL_MEM_SIZE  ($accurate_sys_mem + $estimate_sys_mem - $g_stack_sharing_stack_size - $g_custom_stack_size)
    #define TOTAL_TASK_STACK_SIZE  $g_stack_sharing_stack_size + $g_custom_stack_size
    #define GLOBAL_DEBUG_MEM_SIZE  $debug_mem_size
    #define STACK_POOL_COEFFECIENT (100 - $STACK_POOL_COEFFECIENT)
    #define STACK_POOL_SIZE        ($g_stack_sharing_stack_size * (100 - $STACK_POOL_COEFFECIENT)/100 + $g_custom_stack_size)
#endif  
__TEMPLATE
...
## the follwing parameter are from syscomp_config.o
    ...
    ($flag, $int_ram_defined,  $debug_buf2_defined,  
        ...
        $task_stack_common_plus, $internal_taskend_indx) = 
...
#	}
        ...
        if($index < $internal_taskend_indx)
        {
