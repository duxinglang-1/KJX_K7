...
sub DCMP_Get_MM_IME_subRegionInfo
{
    ...
    {
       if(&FileInfo::isnot("INPUT_METHOD", "NONE"))
       {
...
sub DCMP_query_MM_IME_Grouplength
{
    ...
    my $nMaxSize = 0;
    # --- --- --- 
    my $isIME = &FileInfo::isnot("INPUT_METHOD", "NONE");
    my $nVKLAYOUTGroupSize =0;
    $nVKLAYOUTGroupSize =     GetGroupSize("VKLAYOUT", $isIME) if($isIME);
    my $nMDPGroupSize =       GetGroupSize("MDP", $isIME);
    my $nVECOMGroupSize =     GetGroupSize("VECOM");
    my $nCAMCALGroupSize =    GetGroupSize("CAMCAL");
    my $nMDICAMGroupSize =    GetGroupSize("MDICAM");
    my $nMWRECGroupSize =     GetGroupSize("MWREC");
    ...
    my @OverLaygroup;
    push(@OverLaygroup, $nMDICAMGroupSize, $nMWRECGroupSize, $nVIDMSGGroupSize, $nMWWEBCAMGroupSize, 
                                                             $nMDIMTVGroupSize, $nMDISTREAMGroupSize);
...
sub GetGroupSize
{
    my ($strName, $bQueryIME) = @_;
    my $nGroupSize = DCMP_GetRegionSize($strName);
    my $subRegion_aref = ($bQueryIME)?  sysUtil::DCMP_query_IME_subRegion($strName) 
