...
sub getFreeRAMSize
{
   ...
   # Check if MBA_COMPRESSION is enabled
   {
      ...
      if ($mbaEnabled)
      {
         ...
         debugLog("Free RAM size = External Total RAM Size - (End of cacheable ZI + DSP TX RAM + DSP RX RAM) - Cache Dummy End Size - LANG PACK ROM Decompressed Size - CUSTPACK ROM Decompressed Size");
         debugLog("   = $result");
      }
      else
      {
