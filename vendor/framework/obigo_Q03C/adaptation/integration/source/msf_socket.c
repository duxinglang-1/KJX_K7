...
#define SOCKET_TYPE_FROM_HDI(type)     (type == HDI_SOCKET_TYPE_UDP) ? SOC_SOCK_DGRAM : ((type == HDI_SOCKET_TYPE_TCP) ? SOC_SOCK_STREAM : SOC_SOCK_SMS)
...
int HDIa_socket_retcode(kal_int8 ret)
{
    ...
    switch (ret)
    {
        ...
        case SOC_LIMIT_RESOURCE:
      	    last_error = HDI_SOCKET_ERROR_RESOURCE_LIMIT;
...
void HDI_registerSmsPush(void)
{
    ...
    {
        acct_id = cbm_set_app_id( 0, app_id);
    }
    else
    {
        ASSERT(0);
    ...
    socketid = MSF_SOCKET_CREATE(MSF_MODID_STK, HDI_SOCKET_TYPE_SMS_DATA, acct_id);
    if (socketid < 0)
    {
        ASSERT(0);
    }
    ...
    addr.addrLen = 0;
    addr.port = 2948;
    kal_mem_set(addr.addr, 0, HDI_SOCKET_ADDR_MAX_LEN);
    if (MSF_SOCKET_BIND(socketid, &addr) < 0)
    {
        ASSERT(0);
    }
...
void HDIa_holdBearer(MSF_UINT8 modId, MSF_INT32 networkAccountID)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    app_id = cbm_get_app_id(networkAccountID);
...
void HDIa_socketNotify(local_para_struct *local_para_ptr)
{
    ...
    if ((ind_ptr->socket_id < 0) || (ind_ptr->socket_id >= HDI_MAX_SOCKET_NUM) || (HDI_SOCKET_CNTX(mod_id[ind_ptr->socket_id]) == 0xff))
    {
        /* LOG ERROR */
        return;
    }
    ...
    if (event == HDI_SOCKET_EVENT_CONNECTED)
    {
        if (ind_ptr->result == KAL_FALSE)
        {
			...
			ret_val = HDI_SOCKET_ERROR_CONNECTION_FAILED;
        }

        HDIc_socketConnectResponse(HDI_SOCKET_CNTX(mod_id[ind_ptr->socket_id]), ind_ptr->socket_id, ret_val /* succ */ );
...
void HDIa_socketGetHostAddr(local_para_struct *local_para_ptr)
{
    ...
    if (ind_ptr->result == KAL_FALSE)
    {
		...
		ret_val = HDI_SOCKET_ERROR_HOST_NOT_FOUND;
    }
 ...
 *  void
 *****************************************************************************/
int HDIa_socketMsgHdlr(ilm_struct *ilm_ptr)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
int HDIa_socketCreate(MSF_UINT8 modId, int socketType, MSF_INT32 networkAccountID)
{
    ...
    kal_int8 socketid;
    socket_type_enum so_type = SOCKET_TYPE_FROM_HDI(socketType);
    kal_uint8 app_id;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    ...
    else if (modId == MSF_MODID_STK && socketType == MSF_SOCKET_TYPE_UDP && networkAccountID == 0 )
    {
        if(CBM_OK == cbm_register_app_id(&app_id))
	{
	    networkAccountID = cbm_set_app_id(0, app_id);
	}
	else
	{
		ASSERT(0);
    ...
    socketid = soc_create(SOC_PF_INET, so_type, 0, MOD_WAP, (kal_uint32)networkAccountID);
...
int HDIa_socketBind(int socketId, msf_sockaddr_t *addr)
{
    ...
    kal_uint8 bearer = CBM_BEARER_NONE;
    sockaddr_struct socaddr;
...
long HDIa_socketRecvFrom(int socketId, void *buf, long bufLen, msf_sockaddr_t *fromAddr)
{
    ...
    if (error > 0)
    {
        if (fromAddr)
        {
            switch (HDI_SOCKET_CNTX(soc_type[socketId]))
            {
                case HDI_SOCKET_TYPE_UDP:
                case HDI_SOCKET_TYPE_TCP:
                    fromAddr->type = MSF_SOCKADDR_TYPE_IPV4;
                    break;
                case HDI_SOCKET_TYPE_SMS_DATA:
                case HDI_SOCKET_TYPE_SMS_RAW:
                    fromAddr->type = MSF_SOCKADDR_TYPE_SMS;
                    break;
                default:
                    fromAddr->type = MSF_SOCKADDR_TYPE_UNKNOWN;
                    break;
            }        
            if(sockaddr.sock_type == SOC_SOCK_SMS)
            {
                ASSERT(sizeof(sms_addr_struct) == 12);
                memcpy(fromAddr->addr + 2 * sizeof(sms_addr_struct), sockaddr.addr + 2 * sizeof(sms_addr_struct), sizeof(kal_uint32));
//#if MSF_CFG_PUSH_ADDRESS_OPTION == 1 /* Obtain sender address */
		        ...
		        fromAddr->addr[fromAddr->addrLen++] = 0xff;          
//#else /* Obtain SMSC address */
                smsaddr = (sms_addr_struct *)(sockaddr.addr);
                memcpy(fromAddr->addr + fromAddr->addrLen , smsaddr->addr_bcd, smsaddr->addr_length);
//#endif           
				fromAddr->addrLen += smsaddr->addr_length;
				/* Add 0xff for deliminator */
				fromAddr->addr[fromAddr->addrLen++] = 0xff;
            }            
            else
            {
