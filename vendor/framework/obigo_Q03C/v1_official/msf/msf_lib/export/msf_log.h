...
#define MSF_LOG_MSG_MEMORY(x)                  MSF_LOG_MSG_ARGS(x) 
#endif
...
#define MSF_LOG_MSG_ARGS_(type, modId, msgId, format, s, ...)       gmms_HDIa_logTrace (type, modId, msgId, format, __VA_ARGS__)
#define MSF_LOG_MSG_NOARG_(type, modId, msgId, format, s)           gmms_HDIa_logTrace (type, modId, msgId, format)
#define MSF_LOG_MSG_NOARG(x)                                        MSF_LOG_MSG_NOARG_ x
#define MSF_LOG_MSG_ARGS(x)                                         MSF_LOG_MSG_ARGS_ x

#else

#define MSF_LOG_MSG_NOARG(x)                                        gmms_HDIa_logTrace x
#define MSF_LOG_MSG_ARGS(x)                                         gmms_HDIa_logTrace x

#endif

#define MSF_LOG_MSG_OFF(x)                                          
#define MSF_LOG_MSG(x)                              gmms_HDIa_logMsg x
#define MSF_LOG_DATA(level, modId, data, dataLen)   gmms_HDIa_logData (level, modId, data, dataLen)
#else
...
void gmms_MSF_LogSigBegin(LogSigHelper* pObj, const char* signalName, MSF_UINT8 when, MSF_UINT8 src, MSF_UINT8 dest); 
void gmms_MSF_LogSigParam(LogSigHelper* pObj, const char* paramName, int type, void* paramValue, int paramSize);
void gmms_MSF_LogSigColl(LogSigHelper* pObj, const char* collName);
void gmms_MSF_LogSigCollEnd(LogSigHelper* pObj);
void gmms_MSF_LogSigEnd(LogSigHelper* pObj);
extern MSF_BOOL gmms_MSF_LogSigDoLog;
...
{\
  ...
  lsHelper.doLog = gmms_MSF_LogSigDoLog;\
  if(lsHelper.doLog) { gmms_MSF_LogSigBegin(&lsHelper, signalName, when, src, dest); } 

#define MSF_LOG_SIG_UINT(paramName, paramValue)\
  if(lsHelper.doLog) { gmms_MSF_LogSigParam(&lsHelper, paramName, LS_UINT, &paramValue, sizeof(paramValue)); }

#define MSF_LOG_SIG_UINT8(paramName, paramValue)\
  if(lsHelper.doLog) { gmms_MSF_LogSigParam(&lsHelper, paramName, LS_UINT8, &paramValue, sizeof(paramValue)); }
   
#define MSF_LOG_SIG_UINT16(paramName, paramValue)\
  if(lsHelper.doLog) { gmms_MSF_LogSigParam(&lsHelper, paramName, LS_UINT16, &paramValue, sizeof(paramValue)); }
   
#define MSF_LOG_SIG_UINT32(paramName, paramValue)\
  if(lsHelper.doLog) { gmms_MSF_LogSigParam(&lsHelper, paramName, LS_UINT32, &paramValue, sizeof(paramValue)); }
   
#define MSF_LOG_SIG_INT(paramName, paramValue)\
  if(lsHelper.doLog) { gmms_MSF_LogSigParam(&lsHelper, paramName, LS_INT, &paramValue, sizeof(paramValue)); }
   
#define MSF_LOG_SIG_INT8(paramName, paramValue)\
  if(lsHelper.doLog) { gmms_MSF_LogSigParam(&lsHelper, paramName, LS_INT8, &paramValue, sizeof(paramValue)); }
   
#define MSF_LOG_SIG_INT16(paramName, paramValue)\
  if(lsHelper.doLog) { gmms_MSF_LogSigParam(&lsHelper, paramName, LS_INT16, &paramValue, sizeof(paramValue)); }
   
#define MSF_LOG_SIG_INT32(paramName, paramValue)\
  if(lsHelper.doLog) { gmms_MSF_LogSigParam(&lsHelper, paramName, LS_INT32, &paramValue, sizeof(paramValue)); }
   
#define MSF_LOG_SIG_FLOAT(paramName, paramValue)\
  if(lsHelper.doLog) { gmms_MSF_LogSigParam(&lsHelper, paramName, LS_FLOAT32, &paramValue, sizeof(paramValue)); }
   
#define MSF_LOG_SIG_STR(paramName, paramValue)\
  if(lsHelper.doLog) { gmms_MSF_LogSigParam(&lsHelper, paramName, LS_STR, &paramValue, paramValue ? (msf_cmmn_utf8_strlen(paramValue) + 1) : 0); }

#define MSF_LOG_SIG_BYTE(paramName, paramValue, paramLength)\
  if(lsHelper.doLog) { gmms_MSF_LogSigParam(&lsHelper, paramName, LS_BYTES, &paramValue, paramValue ? paramLength : 0); }

#define MSF_LOG_SIG_VOID(paramName)\
  if(lsHelper.doLog) { gmms_MSF_LogSigParam(&lsHelper, paramName, LS_VOID, NULL, 0); }

#define MSF_LOG_SIG_COLL(collName)\
  if(lsHelper.doLog) { gmms_MSF_LogSigColl(&lsHelper, collName); }

#define MSF_LOG_SIG_COLL_END()\
  if(lsHelper.doLog) { gmms_MSF_LogSigCollEnd(&lsHelper); }


#define MSF_LOG_SIG_END()\
  if(lsHelper.doLog) { gmms_MSF_LogSigEnd(&lsHelper); }\
}
...
#define MSF_LogSigBegin gmms_MSF_LogSigBegin

#define MSF_LogSigParam gmms_MSF_LogSigParam

#define MSF_LogSigColl gmms_MSF_LogSigColl

#define MSF_LogSigCollEnd gmms_MSF_LogSigCollEnd

#define MSF_LogSigEnd gmms_MSF_LogSigEnd

#define MSF_LogSigDoLog gmms_MSF_LogSigDoLog
