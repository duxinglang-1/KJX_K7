...
mmi_ret VappWallpaperDaydream::mmi_ig_lw_daydream_proc(mmi_frm_asm_evt_struct * evt)
{
	...
	case EVT_ID_ASM_FREE_REQ_NO_WAIT: /* volunteer free*/
		{
			//MMI_ASSERT(0);
			VappWallpaperDaydream *pThis = (VappWallpaperDaydream*)VfxObject::handleToObject(evt->user_data);
			if(pThis)
			{
...
void VappWallpaperDaydream::onCreateView()
{
	...
	mmi_frm_group_create(GRP_ID_ROOT, APP_VENUS_WALLPAPER_INTERGRAFX, (mmi_proc_func)VappWallpaperDaydream::mmi_ig_lw_daydream_proc, getObjHandle());
    ...
    VFX_OBJ_CREATE(m_draw, VappDaydreamFilter, this);
...
VfxBool VappWallpaperDaydream::onPenInput(VfxPenEvent &event)
{
	...
	switch (event.type)
    {
        ...
        {
			WP2_E_PostEvent(IGFK_TOUCH1_DOWN, 0L, event.downPos.x, event.downPos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_MOVE :
        {
			WP2_E_PostEvent(IGFK_TOUCH1_MOVE, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_UP :
        {
			WP2_E_PostEvent(IGFK_TOUCH1_UP, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
    }
    return VFX_TRUE;
...
void VappWallpaperDaydream::restoreWallpaperSetting(void)
{
#if defined(__COSMOS_MMI__)
	m_imgBg->setFilter(NULL);
...
/*****************************************************************************
 * Class VappDaydreamFilter
 *****************************************************************************/
VappDaydreamFilter::VappDaydreamFilter() : m_isSuspend(VFX_FALSE)
{
}

void VappDaydreamFilter::onInit()
{
...
void VappDaydreamFilter::onDeinit()
{
	VfxFrameFilter::onDeinit();
}

VfxFrameFilter::BufferModeEnum VappDaydreamFilter::onGetBufferMode() const
{
...
U32 VappDaydreamFilter::GetElapsedTime()
{
	m_time.setCurrentTime();
	WP2_E_SetLocalTime(m_time.getHour(), m_time.getMinute(), m_time.getSecond());
	...
	dt = WP2_E_GetElapsedTime(cur_ms);
...
VfxBool VappDaydreamFilter::onProcess(VfxU8 *outBuf, VfxImageBuffer &inBuf, VfxFloat timing)
{
    if (g_lw_memPool != NULL)
    {
        U32 dt = GetElapsedTime();
        WP2_E_OnUpdate(outBuf, dt);
    }
    return VFX_TRUE;
}
