 ...
 * Description:
 * ------------
 *  Home screen fish wallpaper class
 *
 * Author:
 * -------
 * -------
...
#include "mmi_rp_app_venus_wallpaper_fish_def.h"
...
/*****************************************************************************
 * Home screen UI componet: fish wallpaper
 *****************************************************************************/
VFX_IMPLEMENT_CLASS("Fish Wallpaper", VappWallpaperFish, VappWallpaper);

VappWallpaperFish::VappWallpaperFish()
{
	m_imgBg = NULL;
	m_draw = NULL;
	m_timeline = NULL;
...
void VappWallpaperFish::onInit()
{
    VappWallpaper::onInit();
    this->setOpaqueMode(VFX_FRAME_OPAQUE_MODE_FULLY_OPAQUE);
}

void VappWallpaperFish::onDeinit()
{
    VappWallpaper::onDeinit();
}

mmi_ret VappWallpaperFish::mmi_ig_lw_fish_proc(mmi_frm_asm_evt_struct * evt)
{
	switch (evt->evt_id)
	{
	...
	case EVT_ID_ASM_FREE_REQ_NO_WAIT: /* volunteer free*/
		{
			//MMI_ASSERT(0);
			VappWallpaperFish *pThis = (VappWallpaperFish*)VfxObject::handleToObject(evt->user_data);
			if(pThis)
			{
...
void VappWallpaperFish::onCreateView()
{
    if (m_isRestored)
    {
		return;
	...
	mmi_frm_group_create(GRP_ID_ROOT, APP_VENUS_WALLPAPER_INTERGRAFX, (mmi_proc_func)VappWallpaperFish::mmi_ig_lw_fish_proc, getObjHandle());
    ...
    VFX_OBJ_CREATE(m_draw, VappFishFilter, this);
...
void VappWallpaperFish::onReleaseView()
{
    if (m_isSerialized)
    {
		return;
...
void VappWallpaperFish::onSerializeView()
{
    if(!m_isSerialized)
    {
        onReleaseView();
...
void VappWallpaperFish::onRestoreView()
{
    if(!m_isRestored)
    {
        onCreateView();
...
MMI_IMG_ID VappWallpaperFish::getThumbnail()
{
    return IMG_ID_VAPP_WALLPAPER_FISH_THUMBNAIL;
}

void VappWallpaperFish::getName(VfxWString &wallpaperName)
{
    wallpaperName = VFX_WSTR_RES(STR_ID_WALLPAPER_VUI_LIVE_IG_FISH);
    return;
}


void VappWallpaperFish::onSuspend()
{
	if (m_timeline != NULL)
		m_timeline->stop();
...
void VappWallpaperFish::onResume()
{
	if (m_timeline != NULL)
		m_timeline->start();
...
VfxBool VappWallpaperFish::onPenInput(VfxPenEvent &event)
{
	...
	switch (event.type)
    {
        ...
        {
			WP3_D_PostEvent(IGFK_TOUCH1_DOWN, 0L, event.downPos.x, event.downPos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_MOVE :
        {
			WP3_D_PostEvent(IGFK_TOUCH1_MOVE, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_UP :
        {
			WP3_D_PostEvent(IGFK_TOUCH1_UP, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
    }
    return VFX_TRUE;
...
void VappWallpaperFish::onDesktopBoundsChanged(
...
void VappWallpaperFish::restoreWallpaperSetting(void)
{
#if defined(__COSMOS_MMI__)
	m_imgBg->setFilter(NULL);
...
/*****************************************************************************
 * Class VappFishFilter
 *****************************************************************************/
VappFishFilter::VappFishFilter() : m_isSuspend(VFX_FALSE)
{
}

void VappFishFilter::onInit()
{
...
void VappFishFilter::onDeinit()
{
	VfxFrameFilter::onDeinit();
}

VfxFrameFilter::BufferModeEnum VappFishFilter::onGetBufferMode() const
{
...
U32 VappFishFilter::GetElapsedTime()
{
	m_time.setCurrentTime();
	WP3_D_SetLocalTime(m_time.getHour(), m_time.getMinute(), m_time.getSecond());
	...
	dt = WP3_D_GetElapsedTime(cur_ms);
...
VfxBool VappFishFilter::onProcess(VfxU8 *outBuf, VfxImageBuffer &inBuf, VfxFloat timing)
{
    if (g_lw_memPool != NULL)
    {
        U32 dt = GetElapsedTime();
        WP3_D_OnUpdate(outBuf, dt);
    }
    return VFX_TRUE;
}
