 ...
 * Description:
 * ------------
 *  Home screen girl wallpaper class
 *
 * Author:
 * -------
 * -------
...
#include "mmi_rp_app_venus_wallpaper_girl_def.h"
...
/*****************************************************************************
 * Home screen UI componet: girl wallpaper
 *****************************************************************************/
VFX_IMPLEMENT_CLASS("Girl Wallpaper", VappWallpaperGirl, VappWallpaper);

VappWallpaperGirl::VappWallpaperGirl()
{
	m_imgBg = NULL;
	m_draw = NULL;
	m_timeline = NULL;
...
void VappWallpaperGirl::onInit()
{
    VappWallpaper::onInit();
    this->setOpaqueMode(VFX_FRAME_OPAQUE_MODE_FULLY_OPAQUE);
}

void VappWallpaperGirl::onDeinit()
{
    VappWallpaper::onDeinit();
}

mmi_ret VappWallpaperGirl::mmi_ig_lw_girl_proc(mmi_frm_asm_evt_struct * evt)
{
	switch (evt->evt_id)
	{
	...
	case EVT_ID_ASM_FREE_REQ_NO_WAIT: /* volunteer free*/
		{
			//MMI_ASSERT(0);
			VappWallpaperGirl *pThis = (VappWallpaperGirl*)VfxObject::handleToObject(evt->user_data);
			if(pThis)
			{
...
void VappWallpaperGirl::onCreateView()
{
    if (m_isRestored)
    {
		return;
	...
	mmi_frm_group_create(GRP_ID_ROOT, APP_VENUS_WALLPAPER_INTERGRAFX, (mmi_proc_func)VappWallpaperGirl::mmi_ig_lw_girl_proc, getObjHandle());
    ...
    VFX_OBJ_CREATE(m_draw, VappGirlFilter, this);
...
void VappWallpaperGirl::onReleaseView()
{
    if (m_isSerialized)
    {
		return;
...
void VappWallpaperGirl::onSerializeView()
{
    if(!m_isSerialized)
    {
        onReleaseView();
...
void VappWallpaperGirl::onRestoreView()
{
    if(!m_isRestored)
    {
        onCreateView();
...
MMI_IMG_ID VappWallpaperGirl::getThumbnail()
{
    return IMG_ID_VAPP_WALLPAPER_GIRL_THUMBNAIL;
}

void VappWallpaperGirl::getName(VfxWString &wallpaperName)
{
    wallpaperName = VFX_WSTR_RES(STR_ID_WALLPAPER_VUI_LIVE_IG_GIRL);
}

void VappWallpaperGirl::onSuspend()
{
	if (m_timeline != NULL)
		m_timeline->stop();
...
void VappWallpaperGirl::onResume()
{
	if (m_timeline != NULL)
		m_timeline->start();
...
VfxBool VappWallpaperGirl::onPenInput(VfxPenEvent &event)
{
	...
	switch (event.type)
    {
        ...
        {
			WP2_B_PostEvent(IGFK_TOUCH1_DOWN, 0L, event.downPos.x, event.downPos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_MOVE :
        {
			WP2_B_PostEvent(IGFK_TOUCH1_MOVE, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_UP :
        {
			WP2_B_PostEvent(IGFK_TOUCH1_UP, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
    }
    return VFX_TRUE;
...
void VappWallpaperGirl::onDesktopBoundsChanged(
...
void VappWallpaperGirl::restoreWallpaperSetting(void)
{
#if defined(__COSMOS_MMI__)
	m_imgBg->setFilter(NULL);
...
/*****************************************************************************
 * Class VappGirlFilter
 *****************************************************************************/
VappGirlFilter::VappGirlFilter() : m_isSuspend(VFX_FALSE)
{
}

void VappGirlFilter::onInit()
{
...
void VappGirlFilter::onDeinit()
{
	VfxFrameFilter::onDeinit();
}

VfxFrameFilter::BufferModeEnum VappGirlFilter::onGetBufferMode() const
{
...
U32 VappGirlFilter::GetElapsedTime()
{
	m_time.setCurrentTime();
	WP2_B_SetLocalTime(m_time.getHour(), m_time.getMinute(), m_time.getSecond());
	...
	dt = WP2_B_GetElapsedTime(cur_ms);
...
VfxBool VappGirlFilter::onProcess(VfxU8 *outBuf, VfxImageBuffer &inBuf, VfxFloat timing)
{
    if (g_lw_memPool != NULL)
    {
        U32 dt = GetElapsedTime();
        WP2_B_OnUpdate(outBuf, dt);
    }
    return VFX_TRUE;
}
#endif /*__MMI_VUI_HOMESCREEN_IG_LIVE_WALLPAPER_GIRL__*/
