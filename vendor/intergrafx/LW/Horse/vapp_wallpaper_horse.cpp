 ...
 * Description:
 * ------------
 *  Home screen horse wallpaper class
 *
 * Author:
 * -------
 * -------
...
#include "mmi_rp_app_venus_wallpaper_horse_def.h"
...
/*****************************************************************************
 * Home screen UI componet: horse wallpaper
 *****************************************************************************/
VFX_IMPLEMENT_CLASS("Horse Wallpaper", VappWallpaperHorse, VappWallpaper);

VappWallpaperHorse::VappWallpaperHorse()
{
	m_imgBg = NULL;
	m_draw = NULL;
	m_timeline = NULL;
...
void VappWallpaperHorse::onInit()
{
    VappWallpaper::onInit();
    this->setOpaqueMode(VFX_FRAME_OPAQUE_MODE_FULLY_OPAQUE);
}

void VappWallpaperHorse::onDeinit()
{
    VappWallpaper::onDeinit();
}

mmi_ret VappWallpaperHorse::mmi_ig_lw_horse_proc(mmi_frm_asm_evt_struct * evt)
{
	switch (evt->evt_id)
	{
	...
	case EVT_ID_ASM_FREE_REQ_NO_WAIT: /* volunteer free*/
		{
			//MMI_ASSERT(0);
			VappWallpaperHorse *pThis = (VappWallpaperHorse*)VfxObject::handleToObject(evt->user_data);
			if(pThis)
			{
...
void VappWallpaperHorse::onCreateView()
{
    if (m_isRestored)
    {
		return;
	...
	mmi_frm_group_create(GRP_ID_ROOT, APP_VENUS_WALLPAPER_INTERGRAFX, (mmi_proc_func)VappWallpaperHorse::mmi_ig_lw_horse_proc, getObjHandle());
    ...
    VFX_OBJ_CREATE(m_draw, VappHorseFilter, this);
...
void VappWallpaperHorse::onReleaseView()
{
    if (m_isSerialized)
    {
		return;
...
void VappWallpaperHorse::onSerializeView()
{
    if(!m_isSerialized)
    {
        onReleaseView();
...
void VappWallpaperHorse::onRestoreView()
{
    if(!m_isRestored)
    {
        onCreateView();
...
MMI_IMG_ID VappWallpaperHorse::getThumbnail()
{
    return IMG_ID_VAPP_WALLPAPER_HORSE_THUMBNAIL;
}

void VappWallpaperHorse::getName(VfxWString &wallpaperName)
{
    wallpaperName = VFX_WSTR_RES(STR_ID_WALLPAPER_VUI_LIVE_IG_HORSE);
    return;
}


void VappWallpaperHorse::onSuspend()
{
	if (m_timeline != NULL)
		m_timeline->stop();
...
void VappWallpaperHorse::onResume()
{
	if (m_timeline != NULL)
		m_timeline->start();
...
VfxBool VappWallpaperHorse::onPenInput(VfxPenEvent &event)
{
	...
	switch (event.type)
    {
        ...
        {
			WP2_D_PostEvent(IGFK_TOUCH1_DOWN, 0L, event.downPos.x, event.downPos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_MOVE :
        {
			WP2_D_PostEvent(IGFK_TOUCH1_MOVE, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_UP :
        {
			WP2_D_PostEvent(IGFK_TOUCH1_UP, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
    }
    return VFX_TRUE;
...
void VappWallpaperHorse::onDesktopBoundsChanged(
...
void VappWallpaperHorse::restoreWallpaperSetting(void)
{
#if defined(__COSMOS_MMI__)
	m_imgBg->setFilter(NULL);
...
/*****************************************************************************
 * Class VappHorseFilter
 *****************************************************************************/
VappHorseFilter::VappHorseFilter() : m_isSuspend(VFX_FALSE)
{
}

void VappHorseFilter::onInit()
{
...
void VappHorseFilter::onDeinit()
{
	VfxFrameFilter::onDeinit();
}

VfxFrameFilter::BufferModeEnum VappHorseFilter::onGetBufferMode() const
{
...
U32 VappHorseFilter::GetElapsedTime()
{
	m_time.setCurrentTime();
	WP2_D_SetLocalTime(m_time.getHour(), m_time.getMinute(), m_time.getSecond());
	...
	dt = WP2_D_GetElapsedTime(cur_ms);
...
VfxBool VappHorseFilter::onProcess(VfxU8 *outBuf, VfxImageBuffer &inBuf, VfxFloat timing)
{
    if (g_lw_memPool != NULL)
    {
        U32 dt = GetElapsedTime();
        WP2_D_OnUpdate(outBuf, dt);
    }
    return VFX_TRUE;
}
#endif /*__MMI_VUI_HOMESCREEN_IG_LIVE_WALLPAPER_HORSE__*/
