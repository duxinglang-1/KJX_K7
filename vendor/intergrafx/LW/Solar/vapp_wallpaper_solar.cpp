 ...
 * Description:
 * ------------
 *  Home screen solar wallpaper class
 *
 * Author:
 * -------
 * -------
...
#include "mmi_rp_app_venus_wallpaper_solar_def.h"
...
/*****************************************************************************
 * Home screen UI componet: solar wallpaper
 *****************************************************************************/
VFX_IMPLEMENT_CLASS("Solar Wallpaper", VappWallpaperSolar, VappWallpaper);

VappWallpaperSolar::VappWallpaperSolar()
{
	m_imgBg = NULL;
	m_draw = NULL;
	m_timeline = NULL;
...
void VappWallpaperSolar::onInit()
{
    VappWallpaper::onInit();
    this->setOpaqueMode(VFX_FRAME_OPAQUE_MODE_FULLY_OPAQUE);
}

void VappWallpaperSolar::onDeinit()
{
    VappWallpaper::onDeinit();
}

mmi_ret VappWallpaperSolar::mmi_ig_lw_solar_proc(mmi_frm_asm_evt_struct * evt)
{
	switch (evt->evt_id)
	{
	...
	case EVT_ID_ASM_FREE_REQ_NO_WAIT: /* volunteer free*/
		{
			//MMI_ASSERT(0);
			VappWallpaperSolar *pThis = (VappWallpaperSolar*)VfxObject::handleToObject(evt->user_data);
			if(pThis)
			{
...
void VappWallpaperSolar::onCreateView()
{
    if (m_isRestored)
    {
		return;
	...
	mmi_frm_group_create(GRP_ID_ROOT, APP_VENUS_WALLPAPER_INTERGRAFX, (mmi_proc_func)VappWallpaperSolar::mmi_ig_lw_solar_proc, getObjHandle());
    ...
    VFX_OBJ_CREATE(m_draw, VappSolarFilter, this);
...
void VappWallpaperSolar::onReleaseView()
{
    if (m_isSerialized)
    {
		return;
...
void VappWallpaperSolar::onSerializeView()
{
    if(!m_isSerialized)
    {
        onReleaseView();
...
void VappWallpaperSolar::onRestoreView()
{
    if(!m_isRestored)
    {
        onCreateView();
...
MMI_IMG_ID VappWallpaperSolar::getThumbnail()
{
    return IMG_ID_VAPP_WALLPAPER_SOLAR_THUMBNAIL;
}

void VappWallpaperSolar::getName(VfxWString &wallpaperName)
{
    wallpaperName = VFX_WSTR_RES(STR_ID_WALLPAPER_VUI_LIVE_IG_SOLAR);
    return;
}

void VappWallpaperSolar::onSuspend()
{
	if (m_timeline != NULL)
		m_timeline->stop();
...
void VappWallpaperSolar::onResume()
{
	if (m_timeline != NULL)
		m_timeline->start();
...
VfxBool VappWallpaperSolar::onPenInput(VfxPenEvent &event)
{
	...
	switch (event.type)
    {
        ...
        {
			WP3_C_PostEvent(IGFK_TOUCH1_DOWN, 0L, event.downPos.x, event.downPos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_MOVE :
        {
			WP3_C_PostEvent(IGFK_TOUCH1_MOVE, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_UP :
        {
			WP3_C_PostEvent(IGFK_TOUCH1_UP, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
    }
    return VFX_TRUE;
...
void VappWallpaperSolar::onDesktopBoundsChanged(
...
void VappWallpaperSolar::restoreWallpaperSetting(void)
{
#if defined(__COSMOS_MMI__)
	m_imgBg->setFilter(NULL);
...
/*****************************************************************************
 * Class VappSolarFilter
 *****************************************************************************/
VappSolarFilter::VappSolarFilter() : m_isSuspend(VFX_FALSE)
{
}

void VappSolarFilter::onInit()
{
...
void VappSolarFilter::onDeinit()
{
	VfxFrameFilter::onDeinit();
}

VfxFrameFilter::BufferModeEnum VappSolarFilter::onGetBufferMode() const
{
...
U32 VappSolarFilter::GetElapsedTime()
{
	m_time.setCurrentTime();
	WP3_C_SetLocalTime(m_time.getHour(), m_time.getMinute(), m_time.getSecond());
	...
	dt = WP3_C_GetElapsedTime(cur_ms);
...
VfxBool VappSolarFilter::onProcess(VfxU8 *outBuf, VfxImageBuffer &inBuf, VfxFloat timing)
{
    if (g_lw_memPool != NULL)
    {
        U32 dt = GetElapsedTime();
        WP3_C_OnUpdate(outBuf, dt);
    }
    return VFX_TRUE;
}
#endif /*__MMI_VUI_HOMESCREEN_IG_LIVE_WALLPAPER_SOLAR__*/
