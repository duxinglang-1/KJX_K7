...
void VappWallpaperFish::onCreateView()
{
	...
	mmi_frm_group_create(GRP_ID_ROOT, APP_VENUS_WALLPAPER_INTERGRAFX, (mmi_proc_func)VappWallpaperFish::mmi_ig_lw_fish_proc, NULL);
    ...
    VFX_OBJ_CREATE(m_draw, VappFishDraw, this);
...
VfxBool VappWallpaperFish::onPenInput(VfxPenEvent &event)
{
	switch (event.type)
    {
        ...
        {
			M3DLWFish_PostEvent(IGV5EID_BEGINTOUCH, 0L, event.downPos.x, event.downPos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_MOVE :
        {
			M3DLWFish_PostEvent(IGV5EID_TOUCH, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_UP :
        {
			M3DLWFish_PostEvent(IGV5EID_ENDTOUCH, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
    }
    return VFX_TRUE;
        ...
        const VfxRect &oldBounds)
{
	...
	M3DLWFish_PostEvent(IGV5EID_LWBOUNDSCHANGED, 0L, source->getBounds().origin.x, 0);
 ...
 * Class VappFishDraw
 *****************************************************************************/
VappFishDraw::VappFishDraw():m_suspend(VFX_FALSE)
{
}

void VappFishDraw::onInit()
{
    VfxOwnerDraw::onInit();
}

void VappFishDraw::onDeinit()
{
    VfxOwnerDraw::onDeinit();
}

void VappFishDraw::onInitResource()
{
    M3DConfig PC;
	...
	M3DLWFish_OnStartup(&PC);
}

void VappFishDraw::onDeinitResource()
{	
    M3DLWFish_OnShutdown();
	m3d_adaption_reset_appid();
}

void VappFishDraw::onDraw(VfxImageBuffer &dstBuffer, VfxRect &dstRect, VfxRect &srcRect)
{
    M3DLWFish_OnUpdate(m_suspend);
}
