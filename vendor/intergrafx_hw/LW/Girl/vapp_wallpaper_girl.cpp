...
void VappWallpaperGirl::onCreateView()
{
	...
	mmi_frm_group_create(GRP_ID_ROOT, APP_VENUS_WALLPAPER_INTERGRAFX, (mmi_proc_func)VappWallpaperGirl::mmi_ig_lw_girl_proc, NULL);
    ...
    VFX_OBJ_CREATE(m_draw, VappGirlDraw, this);
...
VfxBool VappWallpaperGirl::onPenInput(VfxPenEvent &event)
{
	...
	switch (event.type)
    {
        ...
        {
			M3DLWGirl_PostEvent(IGV5EID_BEGINTOUCH, 0L, event.downPos.x, event.downPos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_MOVE :
        {			
			M3DLWGirl_PostEvent(IGV5EID_TOUCH, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_UP :
        {
			M3DLWGirl_PostEvent(IGV5EID_ENDTOUCH, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
    }
    return VFX_TRUE;
        ...
        const VfxRect &oldBounds)
{
	...
	M3DLWGirl_PostEvent(IGV5EID_LWBOUNDSCHANGED, 0L, source->getBounds().origin.x, 0);
}

/*****************************************************************************
 * Class VappGirlDraw
 *****************************************************************************/
VappGirlDraw::VappGirlDraw():m_suspend(VFX_FALSE)
{
}

void VappGirlDraw::onInit()
{
    VfxOwnerDraw::onInit();
}

void VappGirlDraw::onDeinit()
{
    VfxOwnerDraw::onDeinit();
}

void VappGirlDraw::onInitResource()
{
	M3DConfig PC;
	...
	M3DLWGirl_OnStartup(&PC);
}

void VappGirlDraw::onDeinitResource()
{
	M3DLWGirl_OnShutdown();
	m3d_adaption_reset_appid();
}

void VappGirlDraw::onDraw(VfxImageBuffer &dstBuffer, VfxRect &dstRect, VfxRect &srcRect)
{
	M3DLWGirl_OnUpdate(m_suspend);
}
