...
void VappWallpaperHorse::onCreateView()
{
	...
	mmi_frm_group_create(GRP_ID_ROOT, APP_VENUS_WALLPAPER_INTERGRAFX, (mmi_proc_func)VappWallpaperHorse::mmi_ig_lw_horse_proc, NULL);
    ...
    VFX_OBJ_CREATE(m_draw, VappHorseDraw, this);
...
VfxBool VappWallpaperHorse::onPenInput(VfxPenEvent &event)
{
	...
	switch (event.type)
    {
        ...
        {
			M3DLWHorse_PostEvent(IGV5EID_BEGINTOUCH, 0L, event.downPos.x, event.downPos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_MOVE :
        {
			M3DLWHorse_PostEvent(IGV5EID_TOUCH, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_UP :
        {
			M3DLWHorse_PostEvent(IGV5EID_ENDTOUCH, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
    }
    return VFX_TRUE;
        ...
        const VfxRect &oldBounds)
{
	...
	M3DLWHorse_PostEvent(IGV5EID_LWBOUNDSCHANGED, 0L, source->getBounds().origin.x, 0);
}

/*****************************************************************************
 * Class VappHorseDraw
 *****************************************************************************/
VappHorseDraw::VappHorseDraw():m_suspend(VFX_FALSE)
{
}

void VappHorseDraw::onInit()
{
    VfxOwnerDraw::onInit();
}

void VappHorseDraw::onDeinit()
{
    VfxOwnerDraw::onDeinit();
}

void VappHorseDraw::onInitResource()
{
	M3DConfig PC;
	...
	M3DLWHorse_OnStartup(&PC);
}

void VappHorseDraw::onDeinitResource()
{
	M3DLWHorse_OnShutdown();
	m3d_adaption_reset_appid();
}

void VappHorseDraw::onDraw(VfxImageBuffer &dstBuffer, VfxRect &dstRect, VfxRect &srcRect)
{
	M3DLWHorse_OnUpdate(m_suspend);
}
