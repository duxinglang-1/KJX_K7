...
void VappWallpaperLighthouse::onCreateView()
{
	...
	mmi_frm_group_create(GRP_ID_ROOT, APP_VENUS_WALLPAPER_INTERGRAFX, (mmi_proc_func)VappWallpaperLighthouse::mmi_ig_lw_lighthouse_proc, NULL);
    ...
    VFX_OBJ_CREATE(m_draw, VappLighthouseDraw, this);
...
VfxBool VappWallpaperLighthouse::onPenInput(VfxPenEvent &event)
{
	...
	switch (event.type)
    {
        ...
        {
			M3DLWLighthouse_PostEvent(IGV5EID_BEGINTOUCH, 0L, event.downPos.x, event.downPos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_MOVE :
        {
			M3DLWLighthouse_PostEvent(IGV5EID_TOUCH, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_UP :
        {
			M3DLWLighthouse_PostEvent(IGV5EID_ENDTOUCH, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
    }
    return VFX_TRUE;
        ...
        const VfxRect &oldBounds)
{
	...
	M3DLWLighthouse_PostEvent(IGV5EID_LWBOUNDSCHANGED, 0L, source->getBounds().origin.x, 0);
}

/*****************************************************************************
 * Class VappLighthouseDraw
 *****************************************************************************/
VappLighthouseDraw::VappLighthouseDraw():m_suspend(VFX_FALSE)
{
}

void VappLighthouseDraw::onInit()
{
    VfxOwnerDraw::onInit();
}

void VappLighthouseDraw::onDeinit()
{
    VfxOwnerDraw::onDeinit();
}

void VappLighthouseDraw::onInitResource()
{
	M3DConfig PC;
	...
	M3DLWLighthouse_OnStartup(&PC);
}

void VappLighthouseDraw::onDeinitResource()
{
	M3DLWLighthouse_OnShutdown();
	m3d_adaption_reset_appid();
}

void VappLighthouseDraw::onDraw(VfxImageBuffer &dstBuffer, VfxRect &dstRect, VfxRect &srcRect)
{
	M3DLWLighthouse_OnUpdate(m_suspend);
}
