...
void VappWallpaperMimosa::onCreateView()
{
	...
	mmi_frm_group_create(GRP_ID_ROOT, APP_VENUS_WALLPAPER_INTERGRAFX, (mmi_proc_func)VappWallpaperMimosa::mmi_ig_lw_mimosa_proc, NULL);
    ...
    VFX_OBJ_CREATE(m_draw, VappMimosaDraw, this);
...
VfxBool VappWallpaperMimosa::onPenInput(VfxPenEvent &event)
{
	switch (event.type)
    {
        ...
        {	
			M3DLWMimosa_PostEvent(IGV5EID_BEGINTOUCH, 0L, event.downPos.x, event.downPos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_MOVE :
        {
			M3DLWMimosa_PostEvent(IGV5EID_TOUCH, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_UP :
        {
			M3DLWMimosa_PostEvent(IGV5EID_ENDTOUCH, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
    }
    return VFX_TRUE;
        ...
        const VfxRect &oldBounds)
{
	...
	M3DLWMimosa_PostEvent(IGV5EID_LWBOUNDSCHANGED, 0L, source->getBounds().origin.x, 0);
}

/*****************************************************************************
 * Class VappMimosaDraw
 *****************************************************************************/
VappMimosaDraw::VappMimosaDraw():m_suspend(VFX_FALSE)
{
}

void VappMimosaDraw::onInit()
{
    VfxOwnerDraw::onInit();
}

void VappMimosaDraw::onDeinit()
{
    VfxOwnerDraw::onDeinit();
}

void VappMimosaDraw::onInitResource()
{
	M3DConfig PC;
	...
	M3DLWMimosa_OnStartup(&PC);
}

void VappMimosaDraw::onDeinitResource()
{
	M3DLWMimosa_OnShutdown();
	m3d_adaption_reset_appid();
}

void VappMimosaDraw::onDraw(VfxImageBuffer &dstBuffer, VfxRect &dstRect, VfxRect &srcRect)
{
	M3DLWMimosa_OnUpdate(m_suspend);
}
