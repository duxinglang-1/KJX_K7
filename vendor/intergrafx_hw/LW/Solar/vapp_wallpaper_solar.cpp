...
void VappWallpaperSolar::onCreateView()
{
	...
	mmi_frm_group_create(GRP_ID_ROOT, APP_VENUS_WALLPAPER_INTERGRAFX, (mmi_proc_func)VappWallpaperSolar::mmi_ig_lw_solar_proc, NULL);
    ...
    VFX_OBJ_CREATE(m_draw, VappSolarDraw, this);
...
VfxBool VappWallpaperSolar::onPenInput(VfxPenEvent &event)
{
	...
	switch (event.type)
    {
        ...
        {
			M3DLWSolar_PostEvent(IGV5EID_BEGINTOUCH, 0L, event.downPos.x, event.downPos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_MOVE :
        {
			M3DLWSolar_PostEvent(IGV5EID_TOUCH, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
		case VFX_PEN_EVENT_TYPE_UP :
        {
			M3DLWSolar_PostEvent(IGV5EID_ENDTOUCH, 0L, event.pos.x, event.pos.y);
            return VFX_TRUE;
        }
    }
    return VFX_TRUE;
        ...
        const VfxRect &oldBounds)
{
	...
	M3DLWSolar_PostEvent(IGV5EID_LWBOUNDSCHANGED, 0L, source->getBounds().origin.x, 0);
}

/*****************************************************************************
 * Class VappSolarDraw
 *****************************************************************************/
VappSolarDraw::VappSolarDraw():m_suspend(VFX_FALSE)
{
}

void VappSolarDraw::onInit()
{
    VfxOwnerDraw::onInit();
}

void VappSolarDraw::onDeinit()
{
    VfxOwnerDraw::onDeinit();
}

void VappSolarDraw::onInitResource()
{
	M3DConfig PC;
	...
	M3DLWSolar_OnStartup(&PC);
}

void VappSolarDraw::onDeinitResource()
{
	M3DLWSolar_OnShutdown();
	m3d_adaption_reset_appid();
}

void VappSolarDraw::onDraw(VfxImageBuffer &dstBuffer, VfxRect &dstRect, VfxRect &srcRect)
{
	M3DLWSolar_OnUpdate(m_suspend);
}
