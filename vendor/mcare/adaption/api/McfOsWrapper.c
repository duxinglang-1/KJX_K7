...
MINT McfGetMtpnpAdChannel(MINT simId)
{
#if MCR_MTK_VER < MTK_VER_1108
	E_MTPNP_AD_SIMCARD_USABLE_SIDE usable;
	E_MTPNP_AD_CHANNELS  channel;

	usable = MTPNP_AD_Get_UsableSide();
	if ((simId ==MSIM_CARD1 || simId ==MSIM_DUALCARD)&& (usable & MTPNP_AD_SIMCARD1_USABLE))
	{
		channel = MTPNP_AD_CHANNEL_MASTER;
	}
	else if((simId == MSIM_CARD2 || simId ==MSIM_DUALCARD )&& (usable & MTPNP_AD_SIMCARD2_USABLE))
	{
		channel = MTPNP_AD_CHANNEL_SLAVE;
	}
	else 
	{
		channel = MTPNP_AD_CHANNEL_NONE;
	}
	return channel;
#else
    return 0;
...
#ifdef __MMI_DUAL_SIM_MASTER__
MVOID __set_slave_protocol_event_handler( _fPsFuncPtr funcPtr,MUSHORT eventID, MUINT isMultiHandler)
{
#if MCR_MTK_VER >=MTK_VER_1032
  #ifdef __MMI_FRAMEWORK_BACKWARD_COMPATIBLE__
	mmi_frm_set_slave_protocol_event_handler(eventID,funcPtr ,isMultiHandler);
  #else
    mmi_frm_set_protocol_event_handler(eventID, funcPtr, isMultiHandler);
  #endif
#elif (MCR_MTK_VER == MTK_VER_1012) || (MCR_MTK_VER == MTK_VER_936) ||(MCR_MTK_VER == MTK_VER_952)
    mmi_frm_set_slave_protocol_event_handler(eventID,(PsIntFuncPtr)funcPtr,isMultiHandler);
/*
#elif MCR_MTK_VER > MTK_VER_816
    mmi_frm_set_slave_protocol_event_handler(eventID,funcPtr);
 */
#else
...
MVOID __clear_slave_protocol_event_handler(MUSHORT eventID, _fPsFuncPtr funcPtr)
{
#if MCR_MTK_VER >= MTK_VER_1032
    #ifdef __MMI_FRAMEWORK_BACKWARD_COMPATIBLE__
    mmi_frm_clear_slave_protocol_event_handler(eventID,funcPtr);
    #else
    mmi_frm_clear_protocol_event_handler(eventID,funcPtr);
    #endif
#elif (MCR_MTK_VER == MTK_VER_1012) || (MCR_MTK_VER == MTK_VER_936) ||(MCR_MTK_VER == MTK_VER_952)
    mmi_frm_clear_slave_protocol_event_handler(eventID,(PsIntFuncPtr)funcPtr);
#elif MCR_MTK_VER > MTK_VER_816
	mmi_frm_clear_slave_protocol_event_handler(eventID,funcPtr);
#else
	mmi_frm_clear_slave_protocol_event_handler(eventID);
#endif
...
//end add
	...
	if(((ret&&!sim1Removed) ||sim1Removed ) && !mmi_bootup_is_sim2_removed())
	{
		ret= mmi_bootup_is_sim2_network_searching_complete();
	}
#endif
...
MINT McfSim_CheckValidSim(MINT bChkNwAvail)
{
    ...
    #ifdef __MMI_DUAL_SIM_MASTER__
    sim2Inserted =srv_sim_ctrl_is_inserted(MMI_SIM2);
    sim2NwAvail = srv_nw_info_get_service_availability(MMI_SIM2);
    #endif
    ...
    if(sim2Inserted)
    {
        if(!bChkNwAvail || sim2NwAvail ==SRV_NW_INFO_SA_LIMITED_SERVICE || sim2NwAvail==SRV_NW_INFO_SA_FULL_SERVICE)
        {
          ret |= MSIM_CARD2;
        }
    }
...
{//
...
#endif
#ifdef __MMI_DUAL_SIM_MASTER__
	E_MTPNP_AD_NVRAM_STATUS mode;
	E_MTPNP_AD_SERVICE_TYPE type1,type2;
	MTPNP_BOOL card1Absent,card2Absent;

	mode = MTPNP_AD_Get_Startup_Mode();	
	type1 = MTPNP_AD_Get_Card1ServiceType();
	type2 = MTPNP_AD_Get_Card2ServiceType();
	card1Absent = MTPNP_PFAL_Is_Card1Absent();
	card2Absent = MTPNP_PFAL_Is_Card2Absent();

   // if(!card1Absent)
    //{
        //if(!bChkNwAvail || (mode == MTPNP_AD_NVRAM_DUALCARD || mode == MTPNP_AD_NVRAM_CARD1) && type1 != MTPNP_AD_NO_SERVICE)
        //maomaolin modify select no use sim card1, connet network bug
      if(!card1Absent && (mode == MTPNP_AD_NVRAM_DUALCARD || mode == MTPNP_AD_NVRAM_CARD1))
      {
        if(!bChkNwAvail || type1 != MTPNP_AD_NO_SERVICE)
        {
   ...
   // if(!card2Absent)
   // {
       // if(!bChkNwAvail || (mode == MTPNP_AD_NVRAM_DUALCARD || mode == MTPNP_AD_NVRAM_CARD2) && type2 != MTPNP_AD_NO_SERVICE)
       if( !card2Absent &&( mode==MTPNP_AD_NVRAM_DUALCARD || mode == MTPNP_AD_NVRAM_CARD2 ) )
        {
        	 if(!bChkNwAvail || type2 != MTPNP_AD_NO_SERVICE)
        	 {
		 	 nValidSim |= MSIM_CARD2;
        }
    }
	...
	if(1)//todo..
	{
		nValidSim |= MSIM_CARD4;
	}
#endif

//fixed WIFI support. add by charmenli 2012-3-13
#if (MCR_WIFI_SUPPORT == __CFG_YES__)&&(MCR_MTK_VER <= MTK_VER_952)
...
#ifdef __MMI_DUAL_SIM_MASTER__
		E_MTPNP_AD_CHANNELS channel;
		channel = McfGetMtpnpAdChannel(nSimId);
		if(channel != MTPNP_AD_CHANNEL_NONE && channel != MTPNP_AD_Get_Channel(MTPNP_AD_SMS_CHANNEL))
		{
...
void __read_imei(MINT nSimId,MCHAR *pImei,MINT nLen)
{
   ...
   sim_interface_enum eSim = SIM1;
	...
	strncpy (pImei, (const char*)mmi_bootup_get_imei_value(eSim), nLen);
   #else
...
#ifdef __MMI_DUAL_SIM_MASTER__
static MCHAR g_abyImsi2[__MCR_MAX_IMEI_IMSI_LENGTH__] = {0};

static void McfGetImsiRsp2(MVOID* pBuf)
{
	mmi_smu_get_imsi_rsp_struct* pImsi = (mmi_smu_get_imsi_rsp_struct *) pBuf;
	McfTrace_TraceFormatA("[McfGetImsiRsp2] result %d %s\r\n", pImsi->result, pImsi->imsi);

	if (pImsi && pImsi->result == ST_SUCCESS)
	{
		memset(g_abyImsi2, 0x00, sizeof(g_abyImsi2));
		memcpy(g_abyImsi2, pImsi->imsi+1, sizeof(pImsi->imsi));//MTK,ignore first byte of '9'

		__clear_slave_protocol_event_handler(PRT_GET_IMSI_RSP,MNULL);    
	}
}
#endif
...
MVOID McfMisc_GetImsiRequst(MVOID)
{
    ...
    if (srv_sim_ctrl_is_inserted(MMI_SIM2))
    {
        if(g_abyImsi2[0] ==0)
        {
            srv_sim_ctrl_get_imsi(MMI_SIM2, (CHAR *) imsi, __MCR_MAX_IMEI_IMSI_LENGTH__);
            strncpy((CHAR*)g_abyImsi2,&imsi[1],__MCR_MAX_IMEI_IMSI_LENGTH__-1);//remove prefix '9'
        }
    }
#endif
#if MCR_SIM3_SUPPORT == __CFG_YES__
...
MVOID McfMisc_GetImsiRequst(MVOID)
{
...
#ifdef __MMI_DUAL_SIM_MASTER__
	if (!MTPNP_PFAL_Is_Card1Absent() && (g_abyImsi1[0] == 0 ||g_abyImsi1[0]=='0'))
	{
	...
	if ( !MTPNP_PFAL_Is_Card2Absent()&& (g_abyImsi2[0] == 0 ||g_abyImsi2[0]=='0'))
	{

		message.oslDestId = MOD_L4C_2;
		OslMsgSendExtQueue(&message);
		__set_slave_protocol_event_handler(McfGetImsiRsp2,PRT_GET_IMSI_RSP,MFALSE);
		bShouldCheck = MTRUE;        
	}
#else
...
#ifdef __MMI_DUAL_SIM_MASTER__
	else if (nSimId == MSIM_CARD2)
	{
		memcpy(pImsi, g_abyImsi2, nLen);
	}
#endif
}
...
MVOID McfMisc_GetSimImsi(MINT simId, MCHAR * pImsi, MINT nSize)
{
	if(pImsi)
	{
		...
		{
#ifdef __MMI_DUAL_SIM_MASTER__   		
			strncpy(pImsi,g_abyImsi2,nSize-1);
#endif
		}
		else if(simId ==MSIM_CARD3)
		{
...
MINT McfTel_MakeCall(MINT nSimId,const MCHAR *pNumber)
{
	...
	E_MTPNP_AD_CHANNELS sChannel;
	sChannel =McfGetMtpnpAdChannel(nSimId);
	if(sChannel != MTPNP_AD_CHANNEL_NONE && sChannel != MTPNP_AD_Get_Channel(MTPNP_AD_CALL_CHANNEL))
	{
		MTPNP_AD_Free_Channel(MTPNP_AD_CALL_CHANNEL);
		MTPNP_AD_Set_Channel(MTPNP_AD_CALL_CHANNEL, sChannel);
	}
#endif
...
{
#ifdef __MMI_DUAL_SIM_MASTER__
    if (stroe_index >= (MAX_PB_PHONE_ENTRIES + MAX_PB_SIM_ENTRIES))
    {
        return MMI_SIM2;
    }
    else
...
static U16 __SrvEntryIdToStoreIndex(MINT nStorage, MINT nId)
{
...
#ifdef __MMI_DUAL_SIM_MASTER__
    else if (nStorage == MPHB_ST_SIM2)
    {
        if (nId <= MMI_PHB_SIM2_ENTRIES)
        {
            return nId + MMI_PHB_PHONE_ENTRIES + MMI_PHB_SIM_ENTRIES - 1;
        }
    }
#endif 
#else
 if (nStorage == MPHB_ST_PHONE)
    ...
    {
        if (nId <= MAX_PB_SIM2_ENTRIES)
        {
            return nId + MAX_PB_PHONE_ENTRIES + MAX_PB_SIM_ENTRIES - 1;
        }
    }
#endif 
#endif
...
static MINT __SrvStoreIndexToEntryId(U16 store_index)
{
    ...
    if (store_index >= (MAX_PB_PHONE_ENTRIES + MAX_PB_SIM_ENTRIES))
    {
        ...
        return store_index - MAX_PB_PHONE_ENTRIES - MAX_PB_SIM_ENTRIES + 1;
    }
    else
#endif
...
#ifdef __MMI_DUAL_SIM_MASTER__
    if (pEntry->storage == MPHB_ST_SIM2)
    {
...
MINT McfPhb_DelEntry(MVOID *pUser, MINT nId,MINT nStorage,pfnPhbNotifyCb fnDelCb)
{
  ...
  //  if(g_isPhbBusy)                   //murphy
    ...
    if (storage == MMI_SIM2)
    {
        record_index = (store_index + 1 - MAX_PB_PHONE_ENTRIES - MAX_PB_SIM_ENTRIES);
        MTPNP_AD_DELETE_ADN_Record(record_index);
        return;                     
    }
    else
...
MINT McfPhb_QueryStatus(MINT nStorage, MUSHORT *pTotalCount,MUSHORT *pUsedCount)
{
	...
	switch(nStorage)
	{
		...
		case MPHB_ST_SIM2:
		#ifdef __MMI_DUAL_SIM_MASTER__
			total = MTPNP_AD_ADN_SIM2GetCapacity();
			used = MTPNP_AD_ADN_SIM2GetUsedNumber();
	    #endif
			break;
		case MPHB_ST_ALL:
...
#ifdef __MMI_DUAL_SIM_MASTER__
    if((MMI_BOOL)srv_sim_ctrl_is_inserted(MMI_SIM1) == MMI_FALSE &&
        (MMI_BOOL)srv_sim_ctrl_is_inserted(MMI_SIM2) == MMI_TRUE)
    {
        nDstMod = MOD_L4C_2;
    }
    else
...
MINT McfMisc_RequestCellInfo(MINT nSimId, MVOID *pUser, pfnCellInfoFunc fnRequestCb)
{
	...
	if(nSimId == MSIM_CARD2)
	{
      ...
      __clear_slave_protocol_event_handler(MSG_ID_MMI_EM_UPDATE_RSP,NULL);

      __set_slave_protocol_event_handler(__EmStatusEventHandler,MSG_ID_MMI_EM_STATUS_IND,MFALSE);
