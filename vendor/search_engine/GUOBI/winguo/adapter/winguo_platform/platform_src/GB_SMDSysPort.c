...
const winguo_item_struct GBMD_FlaotAPP_FunList[]={
	//AB卡放到一块，加快查找速度
	{1,MENU9185_NETWORK_SETUP},//A卡网络设置
	{2,MENU_MTPNP_NETWORK_SETUP},//B卡网络设置
	{2,MENU_MTPNP_CALL_HISTORY},//B卡通话记录
//	{1,MENU9101_SECURITY},//A卡安全设置
	{2,MENU_MTPNP_SECURITY_SETTING},//B卡安全设置
	...
	{0,MENU_ID_UM_SENT},//已发信息
//	{0,MENU_ID_UM_ARCHIVE},//文件夹
	{0,MAIN_MENU_FILE_MNGR_MENUID}, //文件管理
//	{0,MENU_ID_UM_DELETE_FOLDER},//删除信息
//	{0,MENU_ID_EMAIL_MAIN},//电子邮件
	{0,MENU_ID_UM_SETTING},//信息设置
//	{0,MENU_ID_UM_CB_MAIN},//小区广播
...
GB_INT GB_JudgeAppMenuvalid(GB_U16 usIndex)
{
	...
	if(!(
#ifdef __MMI_DUAL_SIM_MASTER__
		srv_nw_usab_is_usable(MMI_SIM1) || srv_nw_usab_is_usable(MMI_SIM2)
#if (MMI_MAX_SIM_NUM >= 3)
		|| srv_nw_usab_is_usable(MMI_SIM3)
#endif
#if (MMI_MAX_SIM_NUM >= 4)
	...
	//当有AB卡时，要区分它们的有效性
#ifdef __MMI_DUAL_SIM_MASTER__
	if((CardFlog == 1	&& !(srv_nw_usab_is_usable(MMI_SIM1)))
		|| (CardFlog == 2 && !(srv_nw_usab_is_usable(MMI_SIM2))))
	{
		return 0;
	}
#endif
...
#define IMG_GB_SMARTWINGUO_PHB_STORAGE_SIM1 IMG_WG_SMARTDAIL_CONTACT_SIM1//IMG_ID_CARD1_PHB_STORAGE_SIM
#define IMG_GB_SMARTWINGUO_PHB_STORAGE_SIM2 IMG_WG_SMARTDAIL_CONTACT_SIM2//IMG_ID_CARD2_PHB_STORAGE_SIM
#define IMG_GB_SMARTWINGUO_DIALED_CALLS_SIM1 IMG_WG_SMARTDAIL_CALL_OUT_SIM1//IMG_MTPNP_CARD1_MENU_DIALED_CALLS
#define IMG_GB_SMARTWINGUO_MISSED_CALLS_SIM1 IMG_WG_SMARTDAIL_MISSCALL_SIM1//IMG_MTPNP_CARD1_MENU_MISSED_CALLS
#define IMG_GB_SMARTWINGUO_RECEIVED_CALLS_SIM1 IMG_WG_SMARTDAIL_CALL_IN_SIM1//IMG_MTPNP_CARD1_MENU_RECEIVED_CALLS
#define IMG_GB_SMARTWINGUO_DIALED_CALLS_SIM2 IMG_WG_SMARTDAIL_CALL_OUT_SIM2//IMG_MTPNP_CARD2_MENU_DIALED_CALLS
#define IMG_GB_SMARTWINGUO_MISSED_CALLS_SIM2 IMG_WG_SMARTDAIL_MISSCALL_SIM2//IMG_MTPNP_CARD2_MENU_MISSED_CALLS
#define IMG_GB_SMARTWINGUO_RECEIVED_CALLS_SIM2 IMG_WG_SMARTDAIL_CALL_IN_SIM2//IMG_MTPNP_CARD2_MENU_RECEIVED_CALLS
#if (MMI_MAX_SIM_NUM >= 3)
#define IMG_GB_SMARTWINGUO_PHB_STORAGE_SIM3  IMG_WG_SMARTDAIL_CONTACT_SIM3
#define IMG_GB_SMARTWINGUO_DIALED_CALLS_SIM3 IMG_WG_SMARTDAIL_CALL_OUT_SIM3
#define IMG_GB_SMARTWINGUO_MISSED_CALLS_SIM3 IMG_WG_SMARTDAIL_MISSCALL_SIM3
#define IMG_GB_SMARTWINGUO_RECEIVED_CALLS_SIM3 IMG_WG_SMARTDAIL_CALL_IN_SIM3
#endif
#if (MMI_MAX_SIM_NUM >= 4)
#define IMG_GB_SMARTWINGUO_PHB_STORAGE_SIM4  IMG_WG_SMARTDAIL_CONTACT_SIM4
#define IMG_GB_SMARTWINGUO_DIALED_CALLS_SIM4 IMG_WG_SMARTDAIL_CALL_OUT_SIM4
#define IMG_GB_SMARTWINGUO_MISSED_CALLS_SIM4 IMG_WG_SMARTDAIL_MISSCALL_SIM4
#define IMG_GB_SMARTWINGUO_RECEIVED_CALLS_SIM4 IMG_WG_SMARTDAIL_CALL_IN_SIM4
#endif
#else // __MMI_DUAL_SIM_MASTER__
...
GB_INT GBMD_GetIconForSmd(GB_U16 CachType, GB_U16 index_type, GB_U16 index, GB_PU8 *img_buff_p)
{
	...
	else if(CachType == GBMD_CachType_Recalled2)
	{
...
#if defined(GB_MTK_10APLAT)
		{
			...
			//*img_buff_p = (GB_PU8)get_image(mmi_clog_get_log_icon(&idt));
#if (MMI_MAX_SIM_NUM >= 4)
			if(MMI_CLOG_BIT2IDX(idt.sim_id) == SRV_CLOG_SIM_IDX_SIM4)
			{
				if(idt.log_type == SRV_CLOG_CLT_DIALED)
				{
					*img_buff_p  = (GB_PU8)get_image(IMG_GB_SMARTWINGUO_DIALED_CALLS_SIM4);
				}
				else if (idt.log_type == SRV_CLOG_CLT_MISSED)
				{
					*img_buff_p  = (GB_PU8)get_image(IMG_GB_SMARTWINGUO_MISSED_CALLS_SIM4);
				}
				else if (idt.log_type == SRV_CLOG_CLT_RECVED)
				{
					*img_buff_p  = (GB_PU8)get_image(IMG_GB_SMARTWINGUO_RECEIVED_CALLS_SIM4);
				}
			}
			else 
#endif
#if (MMI_MAX_SIM_NUM >= 3)
			if (MMI_CLOG_BIT2IDX(idt.sim_id) == SRV_CLOG_SIM_IDX_SIM3)
			{
				if(idt.log_type == SRV_CLOG_CLT_DIALED)
				{
					*img_buff_p  = (GB_PU8)get_image(IMG_GB_SMARTWINGUO_DIALED_CALLS_SIM3);
				}
				else if (idt.log_type == SRV_CLOG_CLT_MISSED)
				{
					*img_buff_p  = (GB_PU8)get_image(IMG_GB_SMARTWINGUO_MISSED_CALLS_SIM3);
				}
				else if (idt.log_type == SRV_CLOG_CLT_RECVED)
				{
					*img_buff_p  = (GB_PU8)get_image(IMG_GB_SMARTWINGUO_RECEIVED_CALLS_SIM3);
				}
			}
			else
#endif
			{
				if(idt.log_type == SRV_CLOG_CLT_DIALED)
				{
					*img_buff_p  = (GB_PU8)get_image(IMG_GB_SMARTWINGUO_DIALED_CALLS_SIM2);
				}
				else if (idt.log_type == SRV_CLOG_CLT_MISSED)
				{
					*img_buff_p  = (GB_PU8)get_image(IMG_GB_SMARTWINGUO_MISSED_CALLS_SIM2);
				}
				else if (idt.log_type == SRV_CLOG_CLT_RECVED)
				{
					*img_buff_p  = (GB_PU8)get_image(IMG_GB_SMARTWINGUO_RECEIVED_CALLS_SIM2);
				}
			}
		}
#else // #if defined(GB_MTK_10APLAT)
...
#ifdef __MMI_DUAL_SIM_MASTER__
#if (MMI_MAX_SIM_NUM >= 4)
				if(index >= MAX_PB_PHONE_ENTRIES + 3*MAX_PB_SIM_ENTRIES)
				{
					*img_buff_p  = (GB_PU8)get_image(IMG_GB_SMARTWINGUO_PHB_STORAGE_SIM4);
				}
				else
#endif

#if (MMI_MAX_SIM_NUM >= 3)
				if(index >= MAX_PB_PHONE_ENTRIES + 2*MAX_PB_SIM_ENTRIES)
				{
					*img_buff_p  = (GB_PU8)get_image(IMG_GB_SMARTWINGUO_PHB_STORAGE_SIM3);
				}
				else
#endif
				if(index >= MAX_PB_PHONE_ENTRIES + MAX_PB_SIM_ENTRIES)
				{
					*img_buff_p  = (GB_PU8)get_image(IMG_GB_SMARTWINGUO_PHB_STORAGE_SIM2);
				}
				else
				{
...
#if defined(__MMI_DUAL_SIM_MASTER__)
		GB_U16 numlen;
		GB_STRTYPE ptempstr = pstrBuf;
		
		if(Logtype == GBMD_Numtype_DialedNum)
		{
			numlen = gb_wg_wcslen((GB_WCHAR*)slave_chis_p->dialedCalls[id].number);	
			GB_MEMCPY(ptempstr, slave_chis_p->dialedCalls[id].number, sizeof(GB_U16)*(numlen+1));
			ptempstr += numlen;
			*ptempstr = 0;
		}
		else if(Logtype == GBMD_Numtype_MissedNum)
		{
			numlen = gb_wg_wcslen((GB_WCHAR*)slave_chis_p->missedCalls[id].number);	
			GB_MEMCPY(ptempstr, slave_chis_p->missedCalls[id].number, sizeof(GB_U16)*(numlen+1));
			ptempstr += numlen;
			*ptempstr = 0;
		}
		else if(Logtype == GBMD_Numtype_ReceivedNum)
		{
			numlen = gb_wg_wcslen((GB_WCHAR*)slave_chis_p->recvdCalls[id].number);	
			GB_MEMCPY(ptempstr, slave_chis_p->recvdCalls[id].number, sizeof(GB_U16)*(numlen+1));
			ptempstr += numlen;
			*ptempstr = 0;
		}
#else 
