...
GB_INT GB_WG_GetIMSI(GB_CHAR *pIMSI, GB_UINT nBuffLen)
{
	...
	if(!ret)
	{
		ret = srv_sim_ctrl_get_imsi(MMI_SIM2, pIMSI, nBuffLen);
	}
#endif
	return ret;
}
...
GB_BOOL GB_WG_judge_phone_mode(GB_VOID)
{
	...
	int Sim1Valid, Sim2Valid;
#if defined(GB_MTK_10APLAT)
	Sim1Valid = srv_nw_info_get_service_availability(MMI_SIM1);
	Sim2Valid = srv_nw_info_get_service_availability(MMI_SIM2);
#else
	Sim1Valid = MTPNP_AD_Get_Card1ServiceType();
	Sim2Valid = MTPNP_AD_Get_Card2ServiceType();
#endif
#endif	
...
#ifdef __MMI_DUAL_SIM_MASTER__
#if defined(GB_MTK_10APLAT)
		if(Sim1Valid==SRV_NW_INFO_SA_FULL_SERVICE||Sim2Valid==SRV_NW_INFO_SA_FULL_SERVICE)
#else
		if(Sim1Valid==MTPNP_AD_FULL_SERVICE||Sim2Valid==MTPNP_AD_FULL_SERVICE)
#endif
		{
			return TRUE;
		}
