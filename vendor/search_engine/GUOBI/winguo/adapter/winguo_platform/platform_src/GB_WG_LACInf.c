...
void GB_WG_Start_Send_Req_Cell_Id (void)
{
	...
	#ifdef GB_MTK_10APLAT
	if ((!srv_sim_ctrl_is_available(MMI_SIM1)) || (srv_sim_ctrl_get_one_available_sim() == MMI_SIM2))   
	#else
	 if ((MTPNP_AD_Get_Card1Status() == MTPNP_AD_SIMCARD_ABSENT) || (MTPNP_AD_Get_Status() == MTPNP_AD_CARD2))   
	#endif
	{
...
void GB_WG_Check_Cell_Msg(void)
{
	...
	{
		ClearSlaveProtocolEventHandler(MSG_ID_MMI_EM_STATUS_IND);
		SetSlaveProtocolEventHandler(GB_WG_Back_Req_Cell_Id, MSG_ID_MMI_EM_UPDATE_RSP);
		SetSlaveProtocolEventHandler(GB_WG_Check_Cell_Id, MSG_ID_MMI_EM_STATUS_IND);
	}
	else
	#endif
	#endif
