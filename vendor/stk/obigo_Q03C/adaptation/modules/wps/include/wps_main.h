 ...
 * Constants
 ************************************************************/
#ifdef MSF_NUM_INST_VM
#define WPS_MAX_CHANNELS         4 + MSF_NUM_INST_VM
#else
#define WPS_MAX_CHANNELS         4
#endif

#define WPS_MAX_REQUESTS         4
#define WPS_MAX_X509_CA_QUERY    1

#define WPS_FLAG_FILE_NO_PROG_IND        0x01
#define WPS_FLAG_FILE_RESUME             0x02
#define WPS_STK_FALG_WAIT_CONFIRM        0x04
#define WPS_FLAG_FILE_FS_COMMIT          0x08
#define WPS_FLAG_RECREATE_DL_FILE        0x20
#define WPS_FLAG_PGDL_DL_FILE            0x40
#define WPS_FLAG_ENABLE_PGDL_DL          0x80

#ifdef WAP_DOWNLOAD_SPEEDUP
#define WPS_FLAG_USE_LARGE_PIPE          0x10
#endif
/************************************************************
 * Macro
 ************************************************************/
...
} wps_sec_get_session_info_struct;

typedef struct {
  kal_uint8      format;
  kal_uint8     *cert;
  kal_uint16     certLen;
} wps_sec_get_current_cert_struct;
...
typedef struct
{
    ...
    kal_uint8 flag;
	kal_uint8 numEntries;
	kal_uint8 currNumEntry;
	kal_uint8 post_type;
	...
	stk_content_t *multipart_content;
	int *multipart_pipe_handle;
...
typedef struct
{
    ...
    wps_sec_get_session_info_struct session_info;
    wps_sec_get_current_cert_struct cert_info;
} wps_chl_info_struct;
...
extern void wps_send_reset_ind(void);
...
extern wps_req_info_struct *wps_create_request(kal_uint8 channel_id, kal_uint16 request_id);
...
extern void wps_release_bearer(wps_chl_info_struct *ch);
...
extern void wps_sec_handle_delete_ca_cert_response(sec_delete_cert_response_t *p);

extern void wps_sec_handle_show_confirm_dialog(wps_show_sec_confirm_dialog_struct *p);

#ifdef STK_CFG_USE_MTK_TLS
extern void wps_sec_get_current_cert(wps_chl_info_struct *chl_info);
extern void wps_sec_get_session_info( wps_chl_info_struct *chl_info);
