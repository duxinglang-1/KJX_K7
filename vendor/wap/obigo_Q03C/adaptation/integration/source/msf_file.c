...
#if 0
#if (!defined(APP_STORAGE_IN_SYS_DRV)) && defined(OBIGO_Q03C_MMS_V01)
/* under construction !*/
#else
/* under construction !*/
#endif
 ...
 *  void
 *****************************************************************************/
void HDIa_fileFlush(int fileHandle)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
kal_uint8 *HDI_FileNameTranslate(const char *filename)
{
    ...
    sprintf((char*)wap_fs_root_folder, "%c:%s", SRV_FMGR_SYSTEM_DRV, WAP_FS_ROOT_FOLDER);
...
#ifdef MMS_IN_LARGE_STORAGE
        len = FILE_UTF8_STRLEN(src) + 8;    /* e.g. "D:\MMS\" */
#else 
    ...
    else if (use_external == 7)
    {
        len = FILE_UTF8_STRLEN(src) + 9;    /* e.g. "z:\@push\" */
    }
    else
    {
    ...
    else if (use_external == 7)
    {
        ...
        sprintf(push_folder, "%c:\\@push\\", drv_letter);

        FILE_UTF8_ADP2FS(dest, push_folder, size);
        FILE_UTF8_ADP2FS(dest + FILE_WCH2CH_LEN(9), src, size - FILE_WCH2CH_LEN(9));
    }
    else
    {
 ...
 * DESCRIPTION
 *  This function is to wrap HDI_FileNameTranslate for other modules to use.
 * PARAMETERS
 ...
 *  a pointer of translated file name, integration shall free this pointer.
 *****************************************************************************/
kal_uint8 *HDIa_fileGetExternalFileName(MSF_UINT8 modId, const char *src)
{
	kal_uint8 *dest = NULL;
	kal_uint8 *src_tmp = NULL;

    src_tmp = HDI_FileNameTranslate(src);
	dest = widget_ucs2_strdup(modId, src_tmp);
    if (src_tmp)
    {
		FILE_NAME_FREE(src_tmp);
    }
    return dest;
...
int HDIa_fileOpen(MSF_UINT8 modId, const char *fileName, int mode, long size)
{
 ...
 if(fd < 0)
    {
        //kal_wap_trace(MOD_WAP, TRACE_FUNC, "MSF_FILE_OPEN: FS_Open returned error:%d",fd);
        if(fd == FS_READ_ONLY_ERROR || fd == FS_WRITE_PROTECTION || fd == FS_ACCESS_DENIED)
        {
            int attr = 0;
            int set_attr_error = 0;

            attr = FS_GetAttributes((FILE_CHAR_T*)fname);
            if(attr < 0)
            {
                //kal_wap_trace(MOD_WAP, TRACE_FUNC, "MSF_FILE_OPEN: FS_GetAttributes returned error:%d",attr);
                goto done;
            }
            ...
            set_attr_error = FS_SetAttributes((FILE_CHAR_T*)fname, (BYTE)attr);

            if(set_attr_error < 0)
            {
                //kal_wap_trace(MOD_WAP, TRACE_FUNC, "MSF_FILE_OPEN: FS_SetAttributes returned error:%d",set_attr_error);
                goto done;
            }
            ...
            if(fd < 0)
            {
                //kal_wap_trace(MOD_WAP, TRACE_FUNC, "MSF_FILE_OPEN: Retry FS_Open returned error:%d",fd);
            }
        }
    }
done:
