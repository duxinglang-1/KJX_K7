...
MSF_UINT32 HDIa_timeGetCurrent(void)
{
    ...
    DclRTC_Control(rtc_handler, RTC_CMD_GET_TIME, (DCL_CTRL_DATA_T *)&rtc);
...
void *HDIa_memAlloc(MSF_UINT8 modId, MSF_UINT32 size)
{
    ...
    extern void brs_remove_channel (void);

#ifdef MSF_LOG_MEMORY
    
    MSF_LOG_MSG_ARGS ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM,
        MSF_MISC_MEM_ALLOC_SIZE, "ModId = %d, asked for size = %d", modId, size));
#endif

#ifndef WAP_USE_ASM
    if (modId == MSF_MODID_BRS && wap_mem_balance(WAP_MEM_POOL) < (15 * 1024))
    {
        extern mtk_jmpbuf brs_jmp_buf;

        MSF_LOG_MSG_NOARG ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM, MSF_MISC_MEM_ALLOC, "WAP Mem Balance less than 15k"));
...
#ifdef WAP_USE_ASM
    if ((modId == MSF_MODID_BRS ) )/*|| ((wap_app_mem)&&(modId == MSF_MODID_WIDGET))) || modId == MSF_MODID_SMA || modId == MSF_MODID_SLS)*/
    {
        extern mtk_jmpbuf brs_jmp_buf;
        extern mtk_jmpbuf wap_jmpbuf;

        poolId = 	WAP_APP_MEM_POOL;		
        memory = wapadp_mem_alloc(poolId, modId, size);
        if (memory == NULL)
        {
            MSF_LOG_MSG_NOARG ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM, MSF_MISC_NO_MEM_ASM, "ASM : No Memory Allotted"));
            /* Remove STK channel for BRS, currently, the channel id is 1 */
            if (modId == MSF_MODID_BRS)
            {
                    /* Remove STK channel for BRS, Invoke brs_remove_channel() */
                    brs_remove_channel();             
                    longjmp(brs_jmp_buf, 1);
            }
            else
    ...
    else
    {
		...
		if (modId == MSF_MODID_MEA || modId == MSF_MODID_SMA || modId == MSF_MODID_SLS)
		{
                  poolId = 	WAP_MMS_APP_MEM_POOL;		
                }
        ...
        {
            extern mtk_jmpbuf wap_jmpbuf;
            MSF_LOG_MSG_NOARG ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM, MSF_MISC_NO_MEM, "No Memory Allotted"));

        #ifdef WPS_TASK_SUPPORT
            if (stack_int_get_active_module_id() == MOD_WPS)
            {
                return NULL;
            }
...
void HDIa_memFree(MSF_UINT8 modId, void *p)
{
...
#ifdef WAP_USE_ASM
    if ((modId == MSF_MODID_BRS))/* || ((wap_app_mem)&&(modId == MSF_MODID_WIDGET))) || modId == MSF_MODID_MEA || modId == MSF_MODID_SMA || modId == MSF_MODID_SLS)*/
    {
    #ifndef __MTK_TARGET__
	...
	if (modId == MSF_MODID_MEA || modId == MSF_MODID_SMA || modId == MSF_MODID_SLS)
    {
		wapadp_mem_free(WAP_MMS_APP_MEM_POOL, modId, p);
    }
	else
#endif /*MMS_V01_USE_ASM*/
	{
...
void HDIa_error(MSF_UINT8 modId, int errorNo)
{
    ...
    else if ((modId == MSF_MODID_STK || modId == MSF_MODID_WPS || modId == MSF_MODID_PMG ||
			modId == MSF_MODID_PHS) && 
        errorNo == HDI_ERR_MODULE_OUT_OF_MEMORY)
    {
