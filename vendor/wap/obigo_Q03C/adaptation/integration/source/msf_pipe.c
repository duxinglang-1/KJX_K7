...
int name_pipe_occupied_bytes(name_pipe_struct *p)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    return (p->is_full) ? p->pipe_size : ((p->tail + p->pipe_size - p->head) % p->pipe_size);
}
...
int name_pipe_remaining_bytes(name_pipe_struct *p)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    return (p->pipe_size - name_pipe_occupied_bytes(p));
}
...
int HDIa_pipeCreate(MSF_UINT8 msfUid, const char *name)
{
    ...
    MSF_LOG_MSG_ARGS ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM,
        MSF_HDI_PIPE_CREATE, "In HDIa_pipeCreate, Module = %d, is creating pipe with name = %s", msfUid, name));
    ...
    pipe->pipe_size = MAX_NAME_PIPE_SIZE;

    pipe->name = (kal_char*) MSF_MEM_ALLOC(MSF_MODID_MSM, MAX_HDI_FILE_NAME_SIZE);
    pipe->queue = (kal_uint8*) MSF_MEM_ALLOC(MSF_MODID_MSM, MAX_NAME_PIPE_SIZE);
...
int HDIa_pipeOpen(MSF_UINT8 msfUid, const char *name)
{
    ...
    MSF_LOG_MSG_ARGS ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM,
        MSF_HDI_PIPE_OPEN, "In HDIa_pipeOpen, Module = %d, is opening pipe with name = %s", msfUid, name));
 ...
 *  
 *****************************************************************************/
int HDIa_pipeOpenExtended(MSF_UINT8 msfUid, const char *name)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
	...
	if(big_pipes_available())
	{
		...
		if(buf != NULL)
		{
			occupy = name_pipe_occupied_bytes(HDI_PIPE_POOL(indx));
			name_pipe_read(HDI_PIPE_POOL(indx), buf, occupy);
			MSF_MEM_FREE(MSF_MODID_MSM, pipe->queue);
			pipe->queue = buf;
			pipe->pipe_size = MAX_NAME_PIPE_BIG_SIZE;
			pipe->head = 0;
			pipe->tail = occupy;
		}	
	}
...
int HDIa_pipeDelete(const char *name)
{
    ...
    MSF_LOG_MSG_ARGS ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM,
        MSF_HDI_PIPE_DELETE, "In HDIa_pipeDelete, Pipe with name = %s to be deleted", name));
    MSF_MEM_FREE(MSF_MODID_MSM, HDI_PIPE_POOL(indx)->name);
    MSF_MEM_FREE(MSF_MODID_MSM, HDI_PIPE_POOL(indx)->queue);
...
int HDIa_pipeCloseError(int handle)
{
    ...
    {
        MSF_LOG_MSG_ARGS ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM,
        MSF_HDI_PIPE_INVALID_HANDLE, "In HDIa_pipeCloseError, Pipe with invalid handle = %d", handle));
        PIPEDBG_(("pipeClose: invalid handle=%d\n", handle));
        return HDI_PIPE_ERROR_BAD_HANDLE;
    }
...
static int big_pipes_available()
{
	...
	for (i = 1; i < MAX_NAME_PIPE_NUM; i++)
    {
        if (HDI_PIPE_POOL(i) != NULL)
        {
            if (HDI_PIPE_POOL(i)->pipe_size == MAX_NAME_PIPE_BIG_SIZE )
			{				
				count++;
