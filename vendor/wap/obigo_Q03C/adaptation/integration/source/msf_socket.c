...
#ifdef MSF_MODID_BRA
    if (networkAccountID == (MSF_INT32) bam_get_net_acc_id())
    {
        BRAa_setBearerState(CBM_DEACTIVATING);
    }
#endif
...
void HDI_registerSmsPush(void)
{
    ...
    socketid = HDIa_socketCreate(MSF_MODID_STK, HDI_SOCKET_TYPE_SMS_DATA, acct_id);
    if (socketid < 0)
    {
        ASSERT(0);
    }
    ...
    if (HDIa_socketBind(socketid, &addr) < 0)
    {
        ASSERT(0);
    }
...
int HDIa_socketCreate(MSF_UINT8 modId, int socketType, MSF_INT32 networkAccountID)
{
    ...
    socketid = soc_create(SOC_PF_INET, so_type, 0, MOD_WAP, networkAccountID);
