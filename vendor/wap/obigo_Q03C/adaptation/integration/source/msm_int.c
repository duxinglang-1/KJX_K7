...
#define MSF_NUMBER_OF_WPS_MODULES 4
#endif 
*/
...
#ifdef WPS_TASK_SUPPORT
const kal_uint8 msf_wps_mods[MSF_NUMBER_OF_WPS_MODULES] = {MSF_MODID_STK, MSF_MODID_PHS, MSF_MODID_WPS, MSF_MODID_PMG};
...
void MSMa_createTask(MSF_UINT8 msfUid)
{
    ...
    MSF_SET_ACTIVE_MOD(msfUid) switch (msfUid)
    {
		...
		case MSF_MODID_PMG:
            msf_int_func_tbl[msfUid].wantsToRun = PMGc_wantsToRun;
            msf_int_func_tbl[msfUid].run = PMGc_run;
            msf_int_func_tbl[msfUid].kill = PMGc_kill;
            PMGc_start();
            break;
#ifdef MSF_MODID_PRS
        case MSF_MODID_PRS:
        ...
        case MSF_MODID_DAS:
            ...
            break;
         case MSF_MODID_BAM:
            msf_int_func_tbl[msfUid].wantsToRun = BAMc_wantsToRun;
            msf_int_func_tbl[msfUid].run = BAMc_run;
            msf_int_func_tbl[msfUid].kill = BAMc_kill;
            BAMc_start();
            break;
        ...
        case MSF_MODID_SLIMMMA:
        {
            extern void SLIMMMAc_start(void);
            extern int SLIMMMAc_wantsToRun(void);
            extern void SLIMMMAc_run(void);
            extern void SLIMMMAc_kill(void);
            
            msf_int_func_tbl[msfUid].wantsToRun = SLIMMMAc_wantsToRun;
            msf_int_func_tbl[msfUid].run = SLIMMMAc_run;
            msf_int_func_tbl[msfUid].kill = SLIMMMAc_kill;
            SLIMMMAc_start();
            break;
        }
    #endif
    ...
    MSF_LOG_MSG_ARGS ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM,
        MSF_MSM_INT_CREATE_TASK, "In MSMa_createTask for Module = %d", msfUid));
...
void MSMa_moduleStatus(MSF_UINT8 msfUid, int status)
{
    ...
    MSF_LOG_MSG_ARGS ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM,
        MSF_MSM_INT_MODULE_STATUS, "In MSMa_moduleStatus for Module = %d with status to be set as = %d", msfUid, status)); 
    ...
    else
    {
        ...
        #endif
        #ifdef __MMI_MMS_VENDOR_APP_SUPPORT__
            MSMc_startModule(MSF_MODID_SLIMMMA, NULL);
        #endif

        #endif
	       
            }
            if (msfUid == MSF_MODID_MMS || msfUid == MSF_MODID_MEA)
            {
        ...
        {
             /* This section is for handling some scenarios when MODULE STATUS is TERMINATED */
        }
    }
}
    ...
    while (run)
    {
        ...
        for (i = 0; i < MSF_NUMBER_OF_MODULES; i++)
        {
            ...
            if (wps_want_to_run == KAL_TRUE)
            {
                ...
                if (!(msg_get_ext_queue_info(task_info_g[INDX_WPS].task_ext_qid, &num_of_msg) && num_of_msg > 0))
                {
