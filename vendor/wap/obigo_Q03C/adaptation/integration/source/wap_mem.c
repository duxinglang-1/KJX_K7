...
#include "msf_cmmn.h"
        ...
        extern void stk_dcmp_init(void *mem_pool);
...
#ifdef WAP_USE_ADM
static KAL_ADM_ID wap_mem_pool[WAP_MEM_POOL_NUM] = {NULL};
#else 
static wapadp_mem_internal_t wapadp_mem_internal[WAP_MEM_POOL_NUM] = {NULL};
#endif 
...
static kal_mutexid hdi_mem_mutex;
...
static void *wapadp_mem_internal_malloc(MSF_UINT8 poolId, MSF_UINT32 size)
{
...
#if defined (WAP_USE_ASM) || defined(MMS_V01_USE_ASM)
    MSF_UINT32 sz;
...
#ifdef MMS_V01_USE_ASM
    else if (poolId == WAP_MMS_APP_MEM_POOL)
    {
        sz = MMS_ASM_MEM_SIZE;
    }
#endif
    ...
    MSF_LOG_MSG_NOARG ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM, MSF_WAP_NO_LARGE_ENOUGH_CHUNK, "WAP :No Chunk found for asked size"));
    return 0;
  ...
  found:
    ...
    MSF_LOG_MSG_ARGS ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM,
        MSF_WAP_REMAINING_MEMORY, "wapadp_mem_internal_malloc called: remaining size : sz - nb =%d", remsize));
#endif
...
#ifdef MSF_LOG_MEMORY
    MSF_LOG_MSG_ARGS ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM,
        MSF_WAP_ADP_ALLOC_MEMORY, "wapadp_alloc_mem[poolId]: =%d", wapadp_alloc_mem[poolId]));
#endif
#ifdef WAP_MEM_DEBUG_SUPPORT
...
#ifdef MSF_LOG_MEMORY
    MSF_LOG_MSG_ARGS ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM,
        MSF_WAP_ADP_ALLOC_REMAINING_MEMORY, "wapadp_alloc_mem[poolId]:in wapadp_mem_free : =%d", wapadp_alloc_mem[poolId]));
#endif
#ifdef WAP_MEM_DEBUG_SUPPORT
...
/*    if (poolId == WAP_APP_MEM_POOL)
    {
        ASSERT(wap_app_mem != NULL);
        kal_app_mem_free(wap_app_mem);
        wap_app_mem = NULL;
    }*/
//#else /* WAP_USE_ADM */ 
 ...
 *  void
 *****************************************************************************/
void wapadp_mem_init_pool(MSF_UINT8 poolId)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    else if (poolId == WAP_APP_MEM_POOL)
    {
       // ASSERT(wap_app_mem == NULL) wap_app_mem = kal_app_mem_alloc(WAP_APP_MEM_SIZE, KAL_APPMEM_APP, MOD_WAP);
        ...
        ASSERT(wap_mem_pool[poolId])
    #else /* WAP_USE_ADM */ 
        wapadp_mem_internal_init(poolId, wap_app_mem, WAP_ASM_MEM_SIZE);
    #endif /* WAP_USE_ADM */ 
    }
#endif /* WAP_USE_ASM */ 
#ifdef MMS_V01_USE_ASM
	else if(poolId == WAP_MMS_APP_MEM_POOL)
	{
		//ASSERT(mms_app_mem);
	#ifdef WAP_USE_ADM
        wap_mem_pool[poolId] = kal_adm_create(mms_app_mem, MMS_ASM_MEM_SIZE, (kal_uint32*) maxsizes, debug);
        ASSERT(wap_mem_pool[poolId])
    #else /* WAP_USE_ADM */ 
        wapadp_mem_internal_init(poolId, mms_app_mem, MMS_ASM_MEM_SIZE);
    #endif /* WAP_USE_ADM */ 
	}
...
int wap_mem_balance(MSF_UINT8 poolId)
{
    ...
    {
        MSF_LOG_MSG_ARGS ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM,
        MSF_WAP_MEM_BALANCE_ASM, "Memory Balance: =%d", (int)(WAP_ASM_MEM_SIZE - wapadp_alloc_mem[poolId])));
        return (int)(WAP_ASM_MEM_SIZE - wapadp_alloc_mem[poolId]);
    }
#endif /* WAP_USE_ASM */ 
    MSF_LOG_MSG_ARGS ((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_MSM,
        MSF_WAP_MEM_BALANCE, "Memory Balance :No ASM: =%d", (int)(WAP_MEM_SIZE - wapadp_alloc_mem[poolId])));
...
/*****************************************************************************
 * FUNCTION
 *  wapadp_set_wap_app_mem
 * DESCRIPTION
 *  set the value of wap_app_mem variable
 * PARAMETERS
 *  value       [IN]    value to be set
 *  free_mem    [IN]    If Set FALSE - Memory will NOT be freed
 *                      If Set TRUE  - Memory will be freed
 * COMBINATIONS:
 * Value != NULL AND free_mem == FALSE -> INTIALIZE MEMORY
 * Value == NULL AND free_mem == TRUE  -> FREE MEMORY
 * Value == NULL AND free_mem == FALSE -> RESET MEMORY
 * Value != NULL AND free_mem == TRUE  -> INVALID COMBINATION (ASSERT)
 *
 * RETURNS
 *  void
 *****************************************************************************/
void wapadp_set_wap_app_mem(void *value, kal_bool free_mem)
{
    if ((NULL != value) && (TRUE == free_mem))
    {
        ASSERT(0);
    }
    ...
    else if (TRUE == free_mem)
    {
        wapadp_mem_free_all(WAP_APP_MEM_POOL);
        return;
    }
    
    if (NULL != wap_app_mem)
    {
        wapadp_mem_init_pool(WAP_APP_MEM_POOL);
#ifdef WAP_DECODE_USE_ASM_POOL
        stk_dcmp_init((void*)((char*)wap_app_mem+WAP_ASM_MEM_SIZE));
#endif /*WAP_DECODE_USE_ASM_POOL*/
    }
}
...
/*****************************************************************************
 * FUNCTION
 *  wapadp_get_wap_app_mem
 * DESCRIPTION
 *  get the value of wap_app_mem variable
 * PARAMETERS
 *  void
 * RETURNS
 *  void*
 *****************************************************************************/
void* wapadp_get_wap_app_mem(void)
{
    return wap_app_mem;
}
#endif
...
/*****************************************************************************
 * FUNCTION
 *  wapadp_set_mms_app_mem
 * DESCRIPTION
 *  set the value of mms_app_mem variable
 * PARAMETERS
 *  value       [IN]    value to be set
 *  Returns
 *  void
 *****************************************************************************/
void wapadp_set_mms_app_mem(void *value)
{ 
    ASSERT(value);
    ...
    if (NULL != mms_app_mem)
    {
        wapadp_mem_init_pool(WAP_MMS_APP_MEM_POOL);
    }
}

/*****************************************************************************
 * FUNCTION
 *  wap_mms_asm_pool_deinit
 * DESCRIPTION
 *  reset the value of mms_app_mem variable and free the corresponding pool
 * PARAMETERS
 *  value       
 *  Returns
 *  void
...
void wap_mms_asm_pool_deinit()
{
	if (NULL != mms_app_mem)
    {
        wapadp_mem_free_all(WAP_MMS_APP_MEM_POOL);
	}
}
#endif /*MMS_V01_USE_ASM*/
...
void wapadp_mem_free_all(MSF_UINT8 poolId)
{
    extern void deinit_memory_pool(MSF_UINT8 pool_id);

    deinit_memory_pool(poolId);
}
...
void wapadp_set_wap_app_mem(void *value, kal_bool free_mem)
{
    void *wap_app_mem = NULL;
    extern void init_memory_pool(MSF_UINT8 poolId, void *memory_addr, MSF_UINT32 pool_size);
    ...
    if (NULL != wap_app_mem)
    {
        init_memory_pool(WAP_APP_MEM_POOL, wap_app_mem, WAP_ASM_MEM_SIZE);
#ifdef WAP_DECODE_USE_ASM_POOL
        stk_dcmp_init((void*)((char*)wap_app_mem+WAP_ASM_MEM_SIZE));
#endif /*WAP_DECODE_USE_ASM_POOL*/
    }
