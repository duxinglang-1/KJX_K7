...
void DASc_run(void)
{
    ...
    if (setjmp(das_jmpbuf) == 0)
    {
        ...
        das_insideRun = FALSE;
        das_wantsToRun = (ret == 0) ? FALSE : TRUE;
    }
    else
    {
        /*
...
void das_receive(MSF_UINT8 srcModule, MSF_UINT16 signal, void *p)
{
    ...
    switch (signal)
    {
        case MSF_SIG_MODULE_EXECUTE_COMMAND:
			...
			{
				msf_module_execute_cmd_t *q ;

            MSF_LOG_MSG_NOARG((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_DAS, DAS_ENV_47EBAD081A2B64226BDE30054D0926E6,
                               "DAS: Received MSF_SIG_MODULE_EXECUTE_COMMAND\n"));
			q = (msf_module_execute_cmd_t *)p;
            das_execute_commands(q);
			                if(q)
                        {
                                MSF_MEM_FREE(MSF_MODID_DAS, q->commandOptions);
                                msf_content_data_free(MSF_MODID_DAS, q->contentData);
                                MSF_MEM_FREE(MSF_MODID_DAS, q);                         
                        }
			}
        ...
        case MSF_SIG_PIPE_NOTIFY:
            MSF_LOG_MSG_NOARG((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_DAS, DAS_ENV_126C1138F85E2D63C63D403A1625FA07,
                               "DAS: Received MSF_SIG_PIPE_NOTIFY\n"));
        ...
        case MSF_SIG_MODULE_TERMINATE:
            ...
            break;
        default:
            MSF_LOG_MSG_ARGS((MSF_LOG_DETAIL_MEDIUM, MSF_MODID_DAS, DAS_ENV_ECA18A1F0BEDED2E0A123B14F4837B1A,
                              "DAS: Received unknown signal (%04x)\n", signal));
            MSF_MEM_FREE(MSF_MODID_DAS, p);
            break;
    }
}
