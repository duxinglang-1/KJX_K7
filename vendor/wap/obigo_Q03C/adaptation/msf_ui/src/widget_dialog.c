...
/*****************************************************************************
 * FUNCTION
 *  HDIa_widgetDialogCreateWithType
 * DESCRIPTION
 *  
 * PARAMETERS
 *  modId               [IN]        
 ...
 *  
 *****************************************************************************/
MsfWindowHandle HDIa_widgetDialogCreateWithType(
                    MSF_UINT8 modId,
        ...
        int maxSize)
{
    ...
    /* To free MSM module Memory */
    MSF_UINT8 modId = ((widget_header_struct *)dialog)->module_id;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (_DIALOG(dialog)->input_sum == 0)
    {
        ...
        //inp = _GINP(HDIa_widgetTextInputCreate(0, label, inputText, inputMode, maxSize, 1, &size, 0, 0));
        /* Earlier we were using ModID as "0" which is for MSM but in widget_free this memory
        * is never freed. So now we will use the ModID of that module which is creating this gadget.
        * So it is the responsibily of the module (which is creating this gadget) to free this memory */
        inp = _GINP(HDIa_widgetTextInputCreate(modId, label, inputText, inputMode, maxSize, 1, &size, 0, 0));
