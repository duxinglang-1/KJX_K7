...
stFontAttribute widget_convert_to_system_font(MsfFont *font)
{
    ...
    {
        f.size = FONT_PIXEL_SIZE(font->size);
    }
#else
    if (font->size == 0)
    {
 ...
 *  
 *****************************************************************************/
int HDIa_widgetExtGetInputboxWidthBySize(MsfBrushHandle handle, int char_cnt, kal_bool is_passwd)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    else
    {
        ...
        gui_measure_character((UI_character_type) 'W', &w, &h);
    }

    /* FIXME.  no predefined theme component for MMI_multiline_inputbox.text_x = 3 */
	return char_cnt * (w + current_multi_line_input_box_theme->text_gap) + 9 * 2;
}
        ...
        MsfSize *size)
{
    ...
    if (nbrOfChars > WIDGET_STRING_SUBSIZE_CACHE_CRITERIA)
    {
        int i = 0;
  ...
  size->height = (S16) Get_CharHeightOfAllLang(pfont.size);
	widget_get_string_width_height(str, 0, STR_LEN(str), &w, &h);
	//size->height = h;	
...
/* under construction !*/
    ...
    if (subwidth)
    {
        ...
        int max_width = 0;
        S32 _w, _h;
        S32 _d = 0;//widget_get_string_width_deviation(pfont, size->height);

        //Get_CharWidthHeight(CHR(' '), &spaceWidth, &_h);  
        Get_StringWidthHeight_n(space, 1, &spaceWidth, &_h);      
        widget_get_max_word_width(
            str, 
            index, 
            end_index, 
        ...
        max_width += _d;
        /* In case of table BRS asks maximum and minimum length of a string. If string 
        * contains only one word in that case maximum length returned by widget will be
        * 1 less than the minimum length because of following check. Which is not reasonable 
        * and can affect the  rendering of table at BRS end. So there is no need of "+1" */
        size->width = max_width /*+ 1*/;
    ...
    else
    {
        ...
        if (nbrOfChars > WIDGET_STRING_SUBSIZE_CACHE_CRITERIA)
        {
            ...
            widget_get_max_word_width(
                ...
                &width,
                &_h);
            widget_get_string_width_height(str, index, end_index, &width, &_h);// MAUI_01586596
        ...
        int *nbrOfInitialWhiteSpaces)
{
    ...
    /* Initial white spaces are excluded (when necessary) */
    {
        ...
        if (size->width != 0)
        {
            if (r > size->width / 2)   /* MAUI_03119812 (replace >= with >) */
            {
                r = (size->width / 2) - 1;
            }
        ...
        h = (S16) Get_CharHeightOfAllLang(pfont.size);
...
//        if (g_new_font_api)
        {
            ...
            width = (size->width) ;//- widget_get_string_width_deviation(pfont, h);
            ...
            if (width > 0)
            {
                ...
                m = Get_StringWidthHeight_variant((U8*)input_str, 0, len, &w, &h, width, KAL_FALSE, KAL_FALSE) - 1;
...
void widget_release_get_visible_cache_by_string(MsfStringHandle str)
{
    ...
    if (str && str == WFONT_CTX->cache_string)
    {
        ...
        {
#ifdef WAP_USE_ASM
            widget_internal_mem_free(_H(WFONT_CTX->cache_string)->module_id, WFONT_CTX->cache);
#else
