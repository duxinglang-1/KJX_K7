...
MsfIconHandle HDIa_widgetIconCreate(MSF_UINT8 modId, MsfIconType iconType, MsfStyleHandle defaultStyle)
{
    ...
    switch (iconType)
    {
        ...
        case MsfIcon4:
            ...
            break;
        case MsfAltText:
            w->icon_data = (int)get_image(WAP_IMG_ICON_BROKEN_IMAGE);
            break;
#ifdef  __MMI_WAP_MOUSE_POINTER_SUPPORT__            
        case MsfMousePointer: 
        	  w->icon_data = (int)get_image(WAP_IMG_ICON_MOUSE_POINTER);
            break;   
        case MsfLinkSelectTool: 
        	  w->icon_data = (int)get_image(WAP_IMG_ICON_LINK_SELECT_TOOL);
            break;
