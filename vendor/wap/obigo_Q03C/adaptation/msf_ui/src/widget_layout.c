...
int HDIa_widgetGetSize(MSF_UINT32 handle, MsfSize *size)
{
    ...
    S32 longest = -1;
    widget_gadget_select_group_struct *node = NULL;
    widget_choice_element_struct *choice = NULL;
    MsfStringHandle string;
    MsfSize size_element;
    /*----------------------------------------------------------------*/
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    if (handle)
...
/* under construction !*/
            ...
            case WIDGET_GADGET_SELECT_GROUP_TYPE:
                node = _GADGET_SELECT_GROUP(handle);
                if (node->size.width == 0)
                {
                     foreach(node, CHOICE_ELEMENT, choice,
                                {
                                    string = choice->string1;
                                    str = widget_string_content(string);                                    
                                    //widget_get_string_width_height(str, 0, STR_LEN(str), &w, &h);
                                    HDIa_widgetStringGetSubSize(string, NULL, 0, STR_LEN(str), 0, &size_element);
                                        if (longest < size_element.width)
                                        {
                                            longest = size_element.width;
                                        }                                    
                                }
                            );
                    if (longest == -1)
                {
                    ...
                    {
                        _GADGET_SELECT_GROUP(handle)->size.width = longest + 70; /* 70 for checkbox width */
                    }
                }
