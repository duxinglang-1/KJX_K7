...
static MsfEvent create_event(kal_uint16 keycode, kal_uint16 keytype)
{
    ...
    switch (keytype)
    {
        ...
        case KEY_REPEAT:
            ...
            break;
        default:
            event.eventClass = MsfPress; // This is the default case to handle the uninitialized case
            break;
    }
    ...
    if (keytype == KEY_EVENT_UP || keytype == KEY_EVENT_DOWN || keytype == KEY_REPEAT)
    {
        ...
        if (active_screen && (active_screen->accesskey & active_screen->accesskey_override & (1 << key)))
        {
            ...
            HDIc_widgetAccessKey(
                ...
                widget_lookup_accesskey((MSF_UINT32) active_screen, (kal_uint16) key), &event); /* MAUI_01865762 */
            return KAL_TRUE;
        }
        ...
        if (active_window && (active_window->accesskey & active_window->accesskey_override & (1 << key)))
        {
            ...
            HDIc_widgetAccessKey(
                ...
                widget_lookup_accesskey((MSF_UINT32) active_window, (kal_uint16) key), &event); /* MAUI_01865762 */
            return KAL_TRUE;
        }
#endif /* MSF_CFG_BRW_ACCESSKEY_SUPPORT */
        ...
        if (active_window)
        {
            ...
            foreach_childs(active_window, node,
                           ...
                           HDIc_widgetAccessKey(node->module_id,
                                                (MSF_UINT32) node,
                                                widget_lookup_accesskey((MSF_UINT32) node, (kal_uint16) key), &event);
                           return KAL_TRUE;}
                           }
            );
#endif /* MSF_CFG_BRW_ACCESSKEY_SUPPORT */
        ...
        if (active_window && (active_window->accesskey & (1 << key)))
        {
            ...
            HDIc_widgetAccessKey(
                active_window->module_id,
                (MSF_UINT32) active_window,
                widget_lookup_accesskey((MSF_UINT32) active_window, (kal_uint16) key), &event);
            return KAL_TRUE;
        }
#endif /* MSF_CFG_BRW_ACCESSKEY_SUPPORT */
        ...
        if (active_screen && (active_screen->accesskey & (1 << key)))
        {
            ...
            HDIc_widgetAccessKey(
                active_screen->module_id,
                (MSF_UINT32) active_screen,
                widget_lookup_accesskey((MSF_UINT32) active_screen, (kal_uint16) key), &event);
            return KAL_TRUE;
        }
#endif /* MSF_CFG_BRW_ACCESSKEY_SUPPORT */
         ...
         * CSK Should not work on Dialogs also. As if CSK works than we need to show CSK Icon also 
         * which is not feasible in current design.*/
        if (WGUI_CTX->current_window &&
            (!IS_MENU_TYPE(WGUI_CTX->current_window) ||
             (IS_MENU_TYPE(WGUI_CTX->current_window) && WGUI_CTX->is_painted_after_first_focus_changed)) && (!IS_DIALOG_TYPE(WGUI_CTX->current_window)))
        {
