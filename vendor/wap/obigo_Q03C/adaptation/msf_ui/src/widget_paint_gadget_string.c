...
void widget_paint_gadget_string(widget_gadget_string_struct *gstr, MsfSize *size, MsfPosition *pos)
{
    ...
    int x1, y1, x2, y2, pos_x, pos_y, text_y, i;    
    int bound_x1, bound_x2, bound_y1, bound_y2;
    stFontAttribute pfont;
    S32 str_width, str_height, tmp, ascent_val; 
    U16 MaxDeployedLangs;  
	...
	text_y = pos_y + WIDGET_FONT_TOP_SPACING;
   ...
   // gui_print_text((UI_string_type) widget_string_content(gstr->text));
	...
	MaxDeployedLangs = gMaxDeployedLangs;
	tmp = 0;
	ascent_val = 0;	
//	for (i = 0; i < MaxDeployedLangs; i++)  //MAUI_03254250
	{
//		SetFont(pfont, (kal_uint8)i);   //MAUI_03254250
	...
	ShowString_n_baseline(UI_text_x, UI_text_y, pfont, 0, (U8 *) widget_string_content(gstr->text), gui_strlen((UI_string_type) widget_string_content(gstr->text)), ascent_val);	


    /* Text attributes */
    if (style->text_property.decoration & MSF_TEXT_DECORATION_UNDERLINE)
    {
        gui_draw_horizontal_line(x1, x2, text_y + ascent_val + WIDGET_FONT_TOP_SPACING, fg_color);
    }
...
//            gui_push_clip();
	      ...
	      // gui_push_text_clip();
            ...
            {
                /* Unexpected. 'str_height' should be less than (str_height + WIDGET_FONT_BOTTOM_SPACING) */
                text_y = pos_y;
            }
            else
            {
           ...
           // gui_set_clip(x1, y1, x2, y2);
			...
			tmp = 0;
			ascent_val = 0;	
			for (i = 0; i < MaxDeployedLangs; i++)
			{
				SetFont(pfont, (kal_uint8)i);
				tmp = Get_CharAscent();
				if (tmp > ascent_val)
				{
					ascent_val = tmp;
...
void widget_setup_blinking(widget_gadget_string_struct *gstr, MsfColor bgcolor)
{
    ...
    {
#ifdef WAP_USE_ASM
        gstr->blinking = widget_internal_mem_alloc(MSF_MODID_WIDGET, sizeof(widget_blinking_struct));
#else
...
#ifdef WAP_USE_ASM
        widget_internal_mem_free(MSF_MODID_WIDGET, gstr->blinking);
#else
