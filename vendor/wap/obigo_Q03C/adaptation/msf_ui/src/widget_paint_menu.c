...
extern fixed_icontext_list_menuitem_type *widget_icontext_menu_items;
/*dinesh ends*/
#endif /* WIDGET_USE_MMI_ICONTEXT_MENU */ 
#ifndef WIDGET_USE_MMI_ICONTEXT_MENU  //MAUI_03162130
...
extern fixed_twostate_menuitem_type *widget_twostate_menu_items;
/*Dinesh Ends*/
#ifndef WIDGET_USE_MMI_ICONTEXT_MENU  //MAUI_03162130
                ...
                kal_bool is_gadget)
{
    ...
    menu_gui->act_layer_handle = paintbox_layer_handle;
#endif
    /* This flag is set for show the menu items on the top of the rendering window to avoid any gap */
    menu_gui->flags |= UI_LIST_MENU_ALIGN_TO_TOP;
    ...
    menu_item->parent_list = menu_gui;
...
#ifndef __MMI_TITLE_BAR_NO_ICON_SHCT__
    wgui_title_disable_menu_shortcut_display(is_gadget);
#endif
...
const UI_filled_area g_widget_option_group_disable_filler = {UI_FILLED_AREA_TYPE_COLOR | UI_FILLED_AREA_SINGLE_BORDER,
    NULL,
    NULL,
    {128, 128, 128, 100},
    {0, 0, 0, 0},
                ...
                kal_bool is_gadget)
{
	...
	#ifdef OBIGO_Q03C_BROWSER
	extern MSF_BOOL brs_le_gadget_is_disabled(MSF_UINT32 handle);
    ...
    if (is_multi_selection)
    {
	    ...
	    create_fixed_twostate_menuitems(get_image(img_on),get_image(img_off)); 
	...
	menu_gui->flags |= UI_LIST_MENU_DISABLE_GRID_LINE;
     ...
     */
    if (((menu->highlight_index == -1) || (menu->highlight_index == 0)) && (menu->choice_type == MsfExclusiveChoice))    
    {
        ...
        (MAX_FIXED_TWOSTATE_MENU_ITEMS - WIDGET_MENU_FOCUSED_ELEMENT_OFFSET))
    {
        ...
        else
        {
			...
			*/
			...
			/*MAUI_03127020 starts*/

            gui_fixed_list_menu_goto_item(
                    menu_gui, 
                    menu->highlight_index  );
...
#ifdef OBIGO_Q03C_BROWSER
		if (brs_le_gadget_is_disabled((MSF_UINT32)menu))
		{
			 menu_gui->normal_filler = (UI_filled_area*)&g_widget_option_group_disable_filler;
		}
		else
#endif/*#ifdef OBIGO_Q03C_BROWSER*/
		{
	...
	/*if disabled*/
		backup_c1 = menu_item->normal_text_color;
        backup_c2 = menu_item->focussed_text_color;
        backup_c3 = menu_item->selected_text_color;
		#ifdef OBIGO_Q03C_BROWSER
		if (brs_le_gadget_is_disabled((MSF_UINT32)menu))
		{
			menu_item->normal_text_color.r = (*((widget_style_struct*)menu->style)).fg_color.r;
			menu_item->normal_text_color.g = (*((widget_style_struct*)menu->style)).fg_color.g;
			menu_item->normal_text_color.b = (*((widget_style_struct*)menu->style)).fg_color.b;
			menu_item->normal_text_color.alpha = 100;

			menu_item->focussed_text_color.r =(*((widget_style_struct*)menu->style)).fg_color.r;
			menu_item->focussed_text_color.g =(*((widget_style_struct*)menu->style)).fg_color.g;
			menu_item->focussed_text_color.b =(*((widget_style_struct*)menu->style)).fg_color.b;
			menu_item->focussed_text_color.alpha =100;

			menu_item->selected_text_color.r = (*((widget_style_struct*)menu->style)).fg_color.r;
			menu_item->selected_text_color.g = (*((widget_style_struct*)menu->style)).fg_color.g;
			menu_item->selected_text_color.b = (*((widget_style_struct*)menu->style)).fg_color.b;
			menu_item->selected_text_color.alpha = 100;
		}
		else
    #endif/*#ifdef OBIGO_Q03C_BROWSER*/
		{
	...
	/*endif*/
		...
		widget_restore_menu_background_if_required();  //MAUI_03183603
		disable_bit_flag(menu_gui->flags, UI_LIST_MENU_DISABLE_SCROLLBAR);
...
void widget_paint_menu(widget_menu_struct *menu, MsfSize *size, MsfPosition *pos)
{
     ...
     */
  /*  if (!menu->menu_items_sum)   MAUI_02076367
    {
        return;
    } */
    ...
    if (menu == WENTER_MENU_CTX->target_menu && WENTER_MENU_CTX->state == WIDGET_ENTER_MENU_PREPARE_TO_ENTER && !mmi_brw_is_in_transaction())
#else
