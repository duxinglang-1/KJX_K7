...
/*#ifdef OBIGO_Q03C_MMS_V01
    ...
    bar_window->handle = HDIa_widgetExtCreateProgressView(handle->modid, MSF_STR_ID_SAS_BAR_TITLE, MEA_IMG_TITLE_NORMAL, KAL_TRUE);
...
msf_sas_update_progressbar(msf_sas_handle_t* handle, msf_sas_op_t *op)
{
    ...
    {
		buf = (kal_uint8*) HDIa_widgetExtStringGetPredefinedUCS2buffer(MSF_STR_ID_CONNECTING);//pls update
    }
...
msf_sas_read_pipe_data(msf_sas_handle_t* handle, msf_sas_op_t *op)
{
    ...
    else if(bytes_read == MSF_PIPE_ERROR_CLOSED_ERROR || MSF_PIPE_STATUS(op->pipe_handle, &isOpen, &available) != 0)
    {
        MSF_PIPE_CLOSE(op->pipe_handle);
        MSF_PIPE_DELETE(op->pathname);
        msf_sas_create_error_dialog (handle, op, MSF_WIDGET_STRING_GET_PREDEFINED(MSF_STR_ID_CONNECTION_FAILED));
        op->state = MSF_SAS_STATE_MESSAGE_WAIT;
        return MSF_PACKAGE_SIGNAL_HANDLED;
    }
...
msf_sas_send_dl_ind(MSF_UINT8 mod_src, MSF_UINT8 data_type, const char *url, const char *mime_type, const kal_uint8 *file_name)
{
    ...
    p->mime_subtype = 0;
    p->extra_info.size = 0;     
    p->extra_info.data[0] = '\0';
    p->extra_info.data[1] = '\0';
    p->ui_setting.title_string_id = 0 ; 
    p->ui_setting.title_icon_id = 0;

    memset(p->url, 0, sizeof(p->url));

	if (mod_src == MSF_MODID_SMA || mod_src == MSF_MODID_MEA)
    {
...
handle_buffer_content(msf_sas_handle_t* handle, msf_sas_op_t *op)
{
    ...
    if(!op->data)
    {
        msf_sas_create_error_dialog (handle, op, MSF_WIDGET_STRING_GET_PREDEFINED(MSF_STR_ID_SAVE_AS_FAIL));
        op->state = MSF_SAS_STATE_MESSAGE_WAIT;
        return;
    }
    ...
    file_handle = FS_Open((WCHAR *)op->file_name, FS_READ_WRITE | FS_CREATE);
    ...
    bytes_written = 0;
    data = (const unsigned char *)op->data;

    while (size_remain > 0)
    {
        if (FS_Write(file_handle, (void*)(data + bytes_written), size_remain, (UINT*) & size) != FS_NO_ERROR)
        {
            msf_sas_create_error_dialog (handle, op, MSF_WIDGET_STRING_GET_PREDEFINED(MSF_STR_ID_SAVE_AS_FAIL));
            op->state = MSF_SAS_STATE_MESSAGE_WAIT;
...
handle_file_content(msf_sas_handle_t* handle, msf_sas_op_t *op)
{
    ...
    if (ret != FS_NO_ERROR)
    {
        ...
        else
        {
            HDIa_widgetExtShowPopupWithType(
                handle->modid,
                HDIa_widgetStringGetPredefined(MSF_STR_ID_SAVE_AS_FAIL),
                MsfFailure);
        }
        op->status = MSF_SAS_RETURN_ERROR;
        op->state = MSF_SAS_STATE_FINISHED;
                    ...
                    msf_pck_result_t  *result)
{
    ...
    if (op->status != MSF_SAS_RETURN_NOT_AVAILABLE)
    {
        ...
        if (op->status != MSF_SAS_RETURN_OK)
        {
            ...
            {
                HDIa_widgetExtShowPopupWithType(sas_handle->modid, 
                    MSF_WIDGET_STRING_GET_PREDEFINED(MSF_STR_ID_SAS_OPERATION_CANCELED), 
                    MsfInfo);
            }
        }
        else
        {
            ...
            {
                msf_sas_send_dl_ind(sas_handle->modid, (MSF_UINT8)op->data_type, contentUrl, op->mime_type, (const kal_uint8 *)op->file_name);
                        ...
                        MSF_INT32 bit_flag)
{
    ...
    if(data_len < 0) /* File system error occurs. */
    {
      HDIa_widgetExtShowPopupWithType(
        sas_handle->modid,
        HDIa_widgetStringGetPredefined(MSF_STR_ID_SAVE_AS_FAIL),
    ...
    
    {
        ...
        {
#endif /* SLIM_WAP_MMS */
            r = FS_GetDrive(FS_DRIVE_V_REMOVABLE, 1, FS_NO_ALT_DRIVE);
            // If there is no more space on public drive, see if we have memory card or not.
            sprintf(temp_ext, "%s%c:\\", HDI_EXTERNAL_PATH, r);
#ifdef SLIM_WAP_MMS			
			
			ret = FS_GetDevStatus(r, FS_MOUNT_STATE_ENUM);
			if(ret < 0)
			{
				HDIa_widgetExtShowPopupWithType(
					sas_handle->modid,
					HDIa_widgetStringGetPredefined(MSF_STR_ID_DLG_NO_MEM_CARD),
					MsfFailure);
				op->status = MSF_SAS_RETURN_ERROR;
				op->state = MSF_SAS_STATE_FINISHED;
				goto end;
        ...
        /* add extention for file without extention due to the limitation of FS on target */
        if (temp[strlen(temp) - 1] == '.')
        {
            char *temp2 = msf_cmmn_strcat(sas_handle->modid, temp, "tmp");
