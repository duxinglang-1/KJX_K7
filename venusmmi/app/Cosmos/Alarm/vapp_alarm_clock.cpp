...
void VappAlarmTimeFrame::onInit(void)
{
	...
	//m_time->setAutoResized(VFX_TRUE);//default;
	...
	//m_ampm->setAutoResized(VFX_TRUE);//default
...
void VappAlarmTimeFrame::setTime(const VfxU8 &hour, const VfxU8 &min)
{
	...
	if(length > 6)//with AMPM
	{
	  ...
	  m_ampm->setString(VFX_WSTR_MEM(ampmBuf));
	}
	
	m_time->setString(VFX_WSTR_MEM(buffer));
