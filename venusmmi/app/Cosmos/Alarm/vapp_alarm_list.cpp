...
void VappAlarmRecentFrame::onInit()
{
	...
	//m_txtLabel->setColor(VFX_COLOR_RES(CLR_COSMOS_TEXT_SUB));//(VfxColor(255, 21, 21, 21));
...
void VappAlarmRecentFrame::update(void)
{
	...
	if (node != NULL)
	{
		...
		if (m_time == NULL)
		{
			...
			//m_time->setTimeColor(VFX_COLOR_RES(CLR_COSMOS_TEXT_MAIN), VFX_COLOR_RES(CLR_COSMOS_TEXT_SUB));
		}
		else
		{
	...
	else
	{
		...
		m_txtLabel->setString(VFX_WSTR_RES(STR_ID_VAPP_ALARM_NO_ALARM_SET));
		m_txtLabel->setColor(VFX_COLOR_RES(CLR_COSMOS_TEXT_SUB));//(VfxColor(255, 21, 21, 21));
...
void VappAlarmlistPage::reset(void)
{
	...
	m_toolbar->addItem(ID_TOOLBAR_BTN1, VFX_WSTR_RES(STR_GLOBAL_ADD), VCP_IMG_TOOL_BAR_COMMON_ITEM_ADD);
	m_toolbar->addItem(ID_TOOLBAR_BTN2, VFX_WSTR_RES(STR_GLOBAL_DELETE), VCP_IMG_TOOL_BAR_COMMON_ITEM_DELETE);
	m_toolbar->addItem(ID_TOOLBAR_BTN3, VFX_WSTR_RES(STR_GLOBAL_SETTINGS), VCP_IMG_TOOL_BAR_COMMON_ITEM_SETTING);
...
void VappAlarmlistPage::onToolbarClick(VfxObject* obj, VfxId id)
{
    ...
    switch(id)
    {
		...
		case ID_TOOLBAR_BTN4: /* Select All */
		{
			...
			for (VfxU32 i = 0; i < m_list->getCount(); i++) //KEMY
			{
			...
			m_bAll = !m_bAll;
						...
						{
							m_toolbarDel->changeItem(ID_TOOLBAR_BTN1, VFX_WSTR_RES(STR_GLOBAL_MARK_ALL), VCP_IMG_TOOL_BAR_COMMON_ITEM_SELECT_ALL);
						}
						else
						{
							m_toolbarDel->changeItem(ID_TOOLBAR_BTN1, VFX_WSTR_RES(STR_GLOBAL_UNMARK_ALL), VCP_IMG_TOOL_BAR_COMMON_ITEM_UNSELECT_ALL);
						}*/
			break;
		}
...
void VappAlarmlistPage::createDelToolbar(void)
{
	...
	m_toolbar->addItem(ID_TOOLBAR_BTN4, VFX_WSTR_RES(STR_ID_VAPP_ALARM_MARK_ALL), VCP_IMG_TOOL_BAR_COMMON_ITEM_SELECT_ALL);
    m_toolbar->addItem(ID_TOOLBAR_BTN5, VFX_WSTR_RES(STR_GLOBAL_DELETE), VCP_IMG_TOOL_BAR_COMMON_ITEM_DELETE);
	m_toolbar->addItem(ID_TOOLBAR_BTN6, VFX_WSTR_RES(STR_GLOBAL_CANCEL), VCP_IMG_TOOL_BAR_COMMON_ITEM_CANCEL);
...
/*void VappAlarmlistPage::onDelToolbarClick(VfxObject* obj, VfxId id)
{
	MMI_TRACE(ALM_TRC_GRP, VAPP_ALM_LIST_ON_DEL_TOOLBAR, id);
...
void VappAlarmlistPage::onSelChange(VfxObject *obj, VfxU32 count)
{
	...
	if (m_list->getMenuMode() == VCP_LIST_MENU_MODE_MULTI_SELECTION)
	{
		...
		{
			m_bAll = VFX_TRUE;
			m_toolbar->changeItem(ID_TOOLBAR_BTN4, VFX_WSTR_RES(STR_ID_VAPP_ALARM_UNMARK_ALL), VCP_IMG_TOOL_BAR_COMMON_ITEM_UNSELECT_ALL);
		}
		else
	{
			m_bAll = VFX_FALSE;			
			m_toolbar->changeItem(ID_TOOLBAR_BTN4, VFX_WSTR_RES(STR_ID_VAPP_ALARM_MARK_ALL), VCP_IMG_TOOL_BAR_COMMON_ITEM_SELECT_ALL);
		}
	}
}
