...
{
    MMI_TRACE(TRACE_GROUP_2, VAPP_CAMCO_OSD_CP_SLIDER_UPDATE, value, m_minValue, m_maxValue, m_isVerticalMode, m_isInverseMode);
...
void VappCamcoCpSlider::setRot(VfxBool isEnvHorz, VfxBool isCPHorz, VfxBool isCPMirror)
{
    m_isHorzUI = isCPHorz;
    ...
    if(isEnvHorz)
    {
        if(isCPHorz)
        {
            ...
            m_maxImg->setTransform(t);
            m_minImg->setTransform(t);                                           
        }
        else
        {
            VfxTransform t;
