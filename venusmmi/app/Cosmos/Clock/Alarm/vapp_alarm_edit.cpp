...
void VappAlarmRepeatPage::onInit(void)
{
	...
	titleBar->setTitle(STR_ID_VAPP_ALARM_REPEAT);
	setTopBar(titleBar);

    //tool bar
    VcpToolBar *toolBar; 
    VFX_OBJ_CREATE(toolBar,VcpToolBar,this);
    toolBar->addItem(ID_TOOLBAR_SELECT_ALL, STR_GLOBAL_MARK_ALL, VCP_IMG_TOOL_BAR_COMMON_ITEM_SELECT_ALL);
    toolBar->addItem(ID_TOOLBAR_SAVE, STR_GLOBAL_SAVE, VCP_IMG_TOOL_BAR_COMMON_ITEM_SAVE);
	toolBar->addItem(ID_TOOLBAR_CANCEL, STR_GLOBAL_CANCEL, VCP_IMG_TOOL_BAR_COMMON_ITEM_CANCEL);
	...
	{
		m_bAll = VFX_TRUE;
		toolBar->changeItem(ID_TOOLBAR_SELECT_ALL, STR_GLOBAL_UNMARK_ALL, VCP_IMG_TOOL_BAR_COMMON_ITEM_UNSELECT_ALL);
	}
	else
	{
		m_bAll = VFX_FALSE;
...
void VappAlarmRepeatPage::onListItemSelect(VcpListMenu *obj, VfxU32 index, VcpListMenuItemStateEnum newState)
{
	...
	{
		m_bAll = VFX_FALSE;
		toolBar->changeItem(ID_TOOLBAR_SELECT_ALL, STR_GLOBAL_MARK_ALL, VCP_IMG_TOOL_BAR_COMMON_ITEM_SELECT_ALL);	
	}
...
void VappAlarmEditPage::onInit(void)
{
	...
	VcpTitleBar *titleBar;
	VFX_OBJ_CREATE(titleBar, VcpTitleBar, this);
	titleBar->setTitle(STR_ID_VAPP_ALARM_SET_ALARM);
	setTopBar(titleBar);
	...
	launchRepeat->setMainText(STR_ID_VAPP_ALARM_REPEAT);
	...
	switchRingTone->setMainText(STR_GLOBAL_RINGTONE);
	...
	switchVib->setMainText(STR_ID_VAPP_ALARM_VIBRATE);
	...
	txtLabel->setLabelText(STR_ID_VAPP_ALARM_LABEL);	
   ...
   if(node->Hour != 0xFF) //edit a node
	...
	txtLabel->getTextBox()->setText((VfxWChar *)m_pNode->label,
   ...
   launchRepeat->setHintText(STR_ID_VAPP_ALARM_REPEAT_ONCE);
...
void VappAlarmEditPage::save(void)
{
	...
	#else
	VfxU8 snoozeVal = ALM_DEFAULT_SNOOZE;
	#endif
