...
mmi_ret VappAlarmIndApp::onReminderEvt(mmi_event_struct *evt)
{
	...
	{
	    srv_prof_play_tone_with_id(SRV_PROF_TONE_ALARM,TONE_WARNING2,SRV_PROF_RING_TYPE_ONCE, NULL);	
	}
...
void VappAlarmIndicationPopup::onInit(void)
{
	...
	if (vfx_sys_wcslen((const VfxWChar*)node->label) != 0)
	{
        ...
        label->setFont(font);
		label->setString((const VfxWChar*)node->label);
	...
	m_btnStop->setText(STR_ID_VAPP_ALARM_DISMISS);
	...
	if (node->Snooze <= VappAlarmApp::MAX_SNOOZE_TIME)
	{
	    ...
	    m_btnSnooze->setText(ALARM_GET_SNOOZE_FOR_RES(snoozeIndex));
	...
	{
	    VFX_OBJ_CREATE(m_btnSnooze, VcpButton, this);
	    m_btnSnooze->setText(STR_ID_VAPP_ALARM_SNOOZE);
		...
		m_btnSnooze->setAnchor(0.0F, 1.0F);
		m_btnSnooze->setPos(BTN_LEFT_MARGIN, getSize().height - BTN_BOTTOM_MARGIN);
		m_btnSnooze->setSize(getSize().width/2-(1*BTN_LEFT_MARGIN), m_btnSnooze->getSize().height);//slim min

		m_btnStop->setSize(getSize().width/2 - (1 * BTN_LEFT_MARGIN), 
					   m_btnStop->getSize().height);
	}
	else
	{
	...
	VfxU16 timeHeight = getSize().height - 
						...
						m_btnStop->getSize().height - 
						BTN_BOTTOM_MARGIN;	
	time->setAnchor(0.5f, 0.5f);
	time->setPos(getSize().width/2, timeHeight);
	...
	if (label != NULL)
	{
		...
		label->setSize(time->getSize().width, LABEL_FONT_SIZE);
	    label->setMaxLines(1);
        label->setTruncateMode(VfxTextFrame::TRUNCATE_MODE_END);
	}
	#endif
...
srv_prof_ret VappAlarmIndicationPopup::onPlayToneCallback(srv_prof_ret result)
{
	...
	if (result == SRV_PROF_RET_FAIL)
	{
		...
		#else
		play_req.volume = MDI_AUD_VOL_MUTE(ALM_DEFAULT_VOL);
		#endif
...
mmi_ret VappAlarmIndicationPopup::onPlayAlarmTone(mmi_event_struct *evt)
{
		...
		case VAPP_ALARM_POP_SNOOZE:
		{
			...
			if (!VappAlarmApp::needPwrOff())
			{
			    ...
			    mmi_frm_nmgr_balloon(
	                    ...
	                    MMI_EVENT_INFO_BALLOON,
	                    MMI_NMGR_BALLOON_TYPE_INFO,
	                    (WCHAR*)VFX_WSTR_RES(STR_ID_VAPP_ALARM_SNOOZING_FOR_5MIN).getBuf());
            #endif
...
void VappAlarmCard::onInit()
{
	...
	if (vfx_sys_wcslen((const VfxWChar*)node->label) != 0)
	{
		...
		m_label->setString((const VfxWChar*)node->label);
...
mmi_ret VappAlarmCard::onPlayAlarmTone(mmi_event_struct *evt)
{
	mmi_nmgr_custom_evt_struct *nmgr_evt = (mmi_nmgr_custom_evt_struct*)evt;
