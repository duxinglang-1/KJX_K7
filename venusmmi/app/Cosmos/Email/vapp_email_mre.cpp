...
static srv_email_addr_struct g_srv_addr[MMI_EMAIL_MAX_ADDR_NUM];
srv_email_attach_struct g_comp_att_list[MMI_EMAIL_MAX_ATTACH_NUMBER];
...
static
mmi_email_app_mre_send_error_code_enum
mmi_email_app_mre_send_smtp(void);
...
BOOL mmi_email_is_mre_busy(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    //MMI_TRACE(MMI_INET_TRC_G9_EMAIL, EMAILAPP_FUNC_MMI_EMAIL_IS_MRE_BUSY, (g_mre_cntx.fsm_state != MRE_FSM_STATE_NULL));
...
/*****************************************************************************
 * FUNCTION 
 *  mmi_email_app_mre_check_resouce
 * DESCRIPTION 
 *  check the environment of Email
 * PARAMETERS 
 *  void
 * RETURNS 
 *  static mmi_email_app_mre_send_error_code_enum 
*****************************************************************************/
static mmi_email_app_mre_send_error_code_enum mmi_email_app_mre_check_resouce(void)
{
    ...
    switch (emailState)
    {
        ...
        case VAPP_EMAIL_STATE_INITIALIZING:
...
static void mmi_email_app_mre_copy_para(mmi_email_app_mre_send_param_struct *para)
{
    ...
    memset(&g_comp_att_list[0], 0, MMI_EMAIL_MAX_ATTACH_NUMBER * sizeof(srv_email_attach_struct));
    for (i = 0; i < para->attach_file_num; i++)
    {
        mmi_wcsncpy(g_comp_att_list[i].path, para->attch_file_list[i], SRV_EMAIL_MAX_FILE_NAME_LEN);
    }
    ...
    if (para->content != NULL)
    {
        ...
        if (len > VAPP_EMAIL_MAX_CONT_LEN)
        {
            len = VAPP_EMAIL_MAX_CONT_LEN;
        }
        mmi_wcsncpy(g_content, para->content, len);
        g_content[len] = 0;
    }
}
    ...
    S32* req_id)
{
    ...
    /* Check if allowed */
    ret = mmi_email_app_mre_check_resouce();
    if (ret != MMI_EMAIL_APP_MRE_ERROR_CODE_SUCCESS)
    {
        //MMI_TRACE(MMI_INET_TRC_G9_EMAIL, EMAILAPP_FUNC_MMI_EMAIL_APP_MRE_SEND, 2);
        return ret;
    }
...
mmi_email_app_mre_fsm(void)
{
    ...
    while (1)
    {
        ...
        case MRE_FSM_STATE_SEND:
            ret = mmi_email_app_mre_send_smtp();
            break;
...
static void mmi_email_app_mre_cleanup(void)
{
    ...
    case MRE_FSM_STATE_SEND:
        ...
        if (g_mre_cntx.msg_handle != EMAIL_MSG_INVALID_HANDLE)
        {
            ...
            srv_email_msg_clear_proc_callback(g_mre_cntx.msg_handle);
        }
        break;
...
mmi_email_mre_init_signature(void)
{
    ...
    sig_info = (mmi_email_sig_struct*)OslMalloc(sizeof(mmi_email_sig_struct));
    if (sig_info == NULL)
    {
        return MMI_EMAIL_APP_MRE_ERROR_CODE_SUCCESS;
    }
    ...
    if (core_ret == MMI_EMAIL_SUCCESS)
    {
        ...
        /* Copy content */
        len = mmi_wcslen(g_content);
        if (len + mmi_wcslen(sig_info->text) <= VAPP_EMAIL_MAX_CONT_LEN)
        {
            mmi_wcscpy(g_content + len, sig_info->text);
        }
        ...
        {
            comp_att = &g_comp_att_list[total_att];
...
mmi_email_mre_compose_basic(void)
{
    ...
    // Update basic info
    basic_info = (srv_email_msg_update_basic_info_struct*)OslMalloc(sizeof(srv_email_msg_update_basic_info_struct));
    ...
    //if (g_content[0] != 0)
    {
        ...
        cont_buf.buff = g_content;
        cont_buf.buff_len = mmi_wcslen(g_content) + 1;
...
mmi_email_app_mre_set_sender(mmi_email_app_mre_send_param_struct *para)
{
    ...
    acct_info = (srv_email_acct_info_struct*)OslMalloc(sizeof(srv_email_acct_info_struct));
    ...
    if (para->from != NULL)
    {
        ...
        srv_addr = (srv_email_addr_struct*)OslMalloc(sizeof(srv_email_addr_struct));
...
mmi_email_app_mre_set_recipient(mmi_email_app_mre_send_param_struct *para)
{
    ...
    if (para->to_num > 0 && para->to != NULL)
    {
        ...
        {
            mmi_email_app_mre_copy_addr(&g_srv_addr[i], &para->to[i]);
        }
    ...
    if (para->cc_num > 0 && para->cc != NULL)
    {
        ...
        {
            mmi_email_app_mre_copy_addr(&g_srv_addr[i], &para->cc[i]);
        }
    ...
    if (para->bcc_num > 0 && para->bcc != NULL)
    {
        ...
        {
            mmi_email_app_mre_copy_addr(&g_srv_addr[i], &para->bcc[i]);
        }
...
mmi_email_app_mre_att_copy(void)
{
    ...
    comp_att = &g_comp_att_list[g_mre_cntx.att_copy_idx];
    file_path = g_comp_att_list[g_mre_cntx.att_copy_idx].path;
...
#ifdef __DRM_SUPPORT__
    vapp_email_util_get_mine_type(file_path, &mime_type, &mime_subtype);
    ...
    vapp_email_drm_is_file_lock(
        file_path,
        &is_drm_obj);
    comp_att->drm_object = is_drm_obj ? MMI_TRUE : MMI_FALSE;
    ...
    copy_file_path = (WCHAR*)OslMalloc((SRV_FMGR_PATH_MAX_LEN + 1) * sizeof(WCHAR));
    kal_wsprintf(copy_file_path, "%scopy_file\\", srv_email_get_root_path());
    if (!vapp_email_util_dir_exist(copy_file_path))
    {
...
/*****************************************************************************
 * FUNCTION 
 *  mmi_email_app_mre_send_smtp
 * DESCRIPTION 
 *  mmi email app mre send smtp
 * PARAMETERS 
 *  void
 * RETURNS 
 *  static mmi_email_app_mre_send_error_code_enum 
...
mmi_email_app_mre_send_smtp(void)
{
    ...
    /* Code Body                                                      */
    /*----------------------------------------------------------------*/
    //MMI_TRACE(MMI_INET_TRC_G9_EMAIL, EMAILAPP_FUNC_MMI_EMAIL_APP_MRE_SEND_SMTP, __LINE__);

    ASSERT(g_mre_cntx.msg_handle == EMAIL_MSG_INVALID_HANDLE);
    srv_ret = srv_email_msg_create(0, &g_mre_cntx.msg_handle);
    if (srv_ret != SRV_EMAIL_RESULT_SUCC)
    {
        return MMI_EMAIL_APP_MRE_ERROR_CODE_EMAIL_NOT_READY;
    }

    srv_ret = srv_email_msg_open(
        g_mre_cntx.msg_handle,
        g_mre_cntx.acct_id,
        g_mre_cntx.fld_id,
        g_mre_cntx.msg_id);
    ...
    vapp_email_util_get_fldr_id_by_acct(g_mre_cntx.acct_id, 1, fldrTypeArr, fldrIdArr);
    ...
    res = srv_email_msg_open(msghd, g_mre_cntx.acct_id, g_mre_cntx.fld_id, g_mre_cntx.msg_id);
    if (res != SRV_EMAIL_RESULT_SUCC)
    {
    ...
    res = srv_email_msg_move(msghd, g_mre_cntx.acct_id, fldrIdArr[0], &msgId);
    if (res != SRV_EMAIL_RESULT_SUCC)
    {
        srv_email_msg_destroy(msghd);
        return MMI_EMAIL_APP_MRE_ERROR_CODE_EMAIL_NOT_READY;
    ...
    VappEmailMgr::getInstance()->sendOutbox(g_mre_cntx.acct_id);
