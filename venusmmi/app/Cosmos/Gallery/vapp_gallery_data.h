...
public:
    ...
    {
        thumbBuf.width = GDI_LCD_WIDTH;
        thumbBuf.height = GDI_LCD_HEIGHT;
        thumbBuf.colorFormat = GALLERY_VRT_CF_VIEWER;
        thumbBuf.pitchBytes = GDI_LCD_WIDTH * BPP;
...
public:
    ...
    void getImageBuffer(VfxImageBuffer &thumbBuf) const
    {
        thumbBuf.width = VappGalleryMediaGridConstant::THUMB_WIDTH;
        thumbBuf.height = VappGalleryMediaGridConstant::THUMB_HEIGHT;
        thumbBuf.colorFormat = GALLERY_VRT_CF_THUMB;
        thumbBuf.pitchBytes = VappGalleryMediaGridConstant::THUMB_WIDTH * BPP;
        thumbBuf.ptr = getThumbBufferPtr(); //const_cast<VfxU8*>(&m_thumbBuffer[0]);
    }
    U8* getThumbBufferPtr() const
    {
...
public:
    ...
    virtual VappGalleryDecodeResult lockThumb(const VcpMenuPos &itemIdx, VfxImageBuffer& buffer, srv_mediacache_image_source_enum decodeMethod);
...
/*****************************************************************************
 * VappGalleryMediaListMediaCache
 *****************************************************************************/
enum VappGalleryMediaCacheType
{
    VGCT_THUMBNAIL,
    VGCT_PREVIEW_ZOOM,
};

// Currently we use 1 single folder only
class VappGalleryMediaListMediaCache : public IVappGalleryMediaList
{
// override
public:
    virtual void release();
    ...
    virtual VfxS32 getGroupCount()
    {
        // since we're using a single folder, there is always 1 group only.
        return 1;
    }
...
public:
    ...
    // this is a temp solution, for Gallery Widget ONLY.
    void configUseAnonymousASM(VfxBool useAnonymousASM = VFX_FALSE)
    {
        m_useAnonymousASM = useAnonymousASM;
    }

    // set the cache purpose. default is VGCT_THUMBNAIL.
    // this must be called before prepareList().
    void configCache(VappGalleryMediaCacheType cacheType, VfxS32 cacheCount);

    // only available when setCacheType(VGCT_THUMBNAIL) is called first.
    // VGCT_PREVIEW does not have a fixed size.
    void configThumbnailSize(const VfxSize size);
    ...
    void configPath(
            ...
            const VfxWString &dbFolderPath,
            const VfxWString &dbName,
...
// constants
private:
    static const S32 DEFAULT_THUMB_DIM = VappGalleryMediaGridConstant::THUMB_WIDTH;

// data members
protected:
    VfxWString m_folderPath;                // path to folder to scan
    VfxWString m_dbFolderPath;              // path to database foler
    VfxWString m_dbName;                    // name of database
    VfxWString m_searchPattern;             // optional search pattern during preparation
    VfxSize m_thumbSize;
    VappGalleryMediaCacheType m_cacheType;  // cache purpose (thumbnail grid or large preview?)
    VfxS32 m_cacheCount;                    // desired cache count
    ...
    VappGalleryDecoderBase *m_thumbDecoder;  // custom decoder for media cache thumbnails

    void *m_pMediaCacheHeap;    // working heap for MediaCache
    void *m_pCachePtr;          // pointer to lvl2 cache. Note: this is NOT part of m_pMediaCacheHeap.
    srv_handle m_hMediaCache;   // handle to media cache service
    mmi_id m_appId;             // app id used to request APP ASM
    VfxBool m_canQueryThumb;
    VfxBool m_prepared;
    VfxBool m_backgroundMode;   // do not allocate FG memory when in background mode
    VfxBool m_useAnonymousASM;  // temp solution for home screen widgets.
...
protected:
    U32 determineDefaultThumbanilCount();
    U32 determineCacheSizeByType(VappGalleryMediaCacheType type);
    ...
    VappGalleryDecodeResult getThumbImageSrcImpl(
                        ...
                        VfxBool isAsync,
                        srv_mediacache_image_source_enum decodeMethod = SRV_MEDIACACHE_IMAGE_SOURCE_CACHE_AND_DATABASE_AND_DECODE);

    void onStartDecodeAfterFileReady(VcpMenuPos index);
    MMI_BOOL onStartDecodeAfterFileReadyImpl(VcpMenuPos index, VfxWString &path);

// callback functions for services
private:
    // MediaCache calls this callback when it cannot retrieve file name
    // from FileMgrSrv
    static S32 onMediaCacheEvent(srv_mediacache_events_enum evt, void* param, void *user_data);
    // MediaCache calls this callback when it completes decoding a cache cell
    static void onMediaCacheDecodeResult(S32 errGroup, S32 errCode, void *userData);

    // MediaCache calls this function to decode cell
    static MMI_BOOL onMediaCacheStartDecode(
                        U32 *handle,
                        srv_mediacache_cell_struct *cell_p,
                        srv_mediacache_nb_decode_callback done_callback,
                        void *user_data                                     // TODO: Currently, this is NOT the user_data we passed in register engine!
                        );
    // method called by onMediaCacheStartDecode
    MMI_BOOL handleMediaCacheStartDecode(
                        U32 *handle,                                        // handle to MediaCache. Actually broken...
                        srv_mediacache_cell_struct *cell,                   // cell to be decoded
                        srv_mediacache_nb_decode_callback done_callback,    // call this function after decode finished/failed
                        srv_mediacache_cntx_struct *mediaCacheContext       // the (internal) MediaCache context
                        );

    void handleThumbnailDecodeResult(VappGalleryDecoderBase* sender, VappGalleryDecodeResult result);

    // callback to MediaCache to report OK/Error and clear callbacks
    void notifyThumbDecodeError(VappGalleryDecodeResult result);

    // Get GDI color format from cache type
    gdi_color_format getGDIColorFormat();
    // Get Byte per pixel from cache type
    S32 getBPP();

    void closeDecodingJob(const VfxU32 line);

    // MediaCache calls this function to cancel decode
    static MMI_BOOL onMediaCacheCancelDecode(U32 handle);

    static S32 convertDecodeResultToMediaCacheResult(VappGalleryDecodeResult result);

//  data for services
private:
    static VappGalleryMediaListMediaCache *g_instance; // we need this because srv_mediacache_register_engine cannot properly store user_data in callbacks
    srv_mediacache_cell_struct *m_decodingCell;
    srv_mediacache_cntx_struct *m_mediaCacheContext;
    srv_mediacache_nb_decode_callback m_mediaCacheDoneCallback;
};

class VappGalleryWallpaperCacheItem
{
public:
    static const VfxS32 NAME_CACHE_LEN = 20;
    static const VfxS32 BPP = 2;
    static const VfxU32 bufSize = (VappGalleryMediaGridConstant::THUMB_WIDTH * VappGalleryMediaGridConstant::THUMB_HEIGHT * BPP + 31) / 32 * 32;
    VfxU8 m_thumbBuffer[bufSize];
    VappGalleryDecodeResult m_result;

public:
    VappGalleryWallpaperCacheItem():
        m_result(DECODING)
    {
    }
    ...
    void getImageBuffer(VfxImageBuffer &thumbBuf) const
    {
        thumbBuf.width = VappGalleryMediaGridConstant::THUMB_WIDTH;
        thumbBuf.height = VappGalleryMediaGridConstant::THUMB_HEIGHT;
        thumbBuf.colorFormat = VRT_COLOR_TYPE_RGB565;
        thumbBuf.pitchBytes = VappGalleryMediaGridConstant::THUMB_WIDTH * BPP;
        thumbBuf.ptr = const_cast<VfxU8*>(&m_thumbBuffer[0]);
    }
};

class VappGalleryMediaListWallpaper : public IVappGalleryMediaList
{

public:
    // Passed from Wallpaper Picker CUI
...
public:
    ...
    virtual DecodeEvent* getDecodeEvent();
    virtual VfxS32 getGroupCount();
    virtual VfxS32 getMediaCountInGroup(VfxS32 group);
    virtual void getGroupName(const VfxS32 group, VfxWString &groupName);
    virtual VappGalleryDecodeResult getMediaName(const VcpMenuPos &itemIdx, VfxWString &nameStr);
    virtual VappGalleryMediaGroupType getMediaGroupType(const VcpMenuPos &itemIdx);
    // retrieve the original media source.
...
protected:
    ...
    ListReloadEvent m_signalReloadFinished;
    DecodeEvent m_signalDecode;
    typedef VappGalleryCache<VappGalleryWallpaperCacheItem> VappGalleryWallpaperCache;
    VappGalleryWallpaperCache *m_wallpaperCache;

    // TODO: maybe we should refactor this section into wallpaper content provider instead.
    VfxS32 m_groupCount;
