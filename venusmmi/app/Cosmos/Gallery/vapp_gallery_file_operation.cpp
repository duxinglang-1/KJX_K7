...
mmi_ret VappGalleryMultiFileOperationUI::onProc(mmi_event_struct *param)
{
    ...
    {
        removeCacheElementFromDBImpl(deletedPath);
        // finished all files
...
void VappGalleryMultiFileOperationUI::removeCacheElementFromDBImpl(VfxWString &path)
{
    // remove cache element
    if(m_mediaList.isValid() && m_mediaList.get()->canRemoveDBCacheElement())
    {
        m_mediaList.get()->removeDBCacheElement(path);
    }
}
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_SHOW_DELETE_UI);
...
void VappGallerySingleFileOperationUI::onDeleteConfirm(VfxObject* sender, VfxId id)
{
    if(m_confirmDelete.get() == sender)
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_ON_DELETE_CONFIRM, 1);
        VFX_OBJ_CLOSE(m_confirmDelete);
    }
    ...
    if(VCP_CONFIRM_POPUP_BUTTON_USER_2 == id ||
       VCP_CONFIRM_POPUP_BUTTON_CANCEL == id)
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_ON_DELETE_CONFIRM, 2);
        // cancel, note: popup is auto-close
        m_signalOperationDone.emit(this, VGFOR_CANCELLED);
    }
    else if(VCP_CONFIRM_POPUP_BUTTON_USER_1 == id)
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_ON_DELETE_CONFIRM, 3);
        // ok, start file operation with progress indicator
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_START_DELETE_OPERATION, 1, VAPP_GALLERY_INTERNAL_UNUSED_PARAM);
    ...
    S32 job_id = srv_fmgr_async_delete(
                                ...
                                &fileOperationFinishProc,
                                this);
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_START_DELETE_OPERATION, 2, job_id);
    if(job_id > 0)
    {
        // job succesfully created, show inicator
        m_jobId = job_id;
...
mmi_ret VappGallerySingleFileOperationUI::onFileOperationFinished(mmi_event_struct *param)
{
    ...
    case EVT_ID_SRV_FMGR_ASYNC_DONE:
        {
            removeCacheElementFromDBImpl(m_filePath);
            // successfully deleted, notify client
            srv_fmgr_async_done_event_struct *doneEvent = (srv_fmgr_async_done_event_struct*)param;
            MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_FILE_OPERATION_FINISHED, param->evt_id,  doneEvent->result);
...
void VappGallerySingleFileOperationUI::removeCacheElementFromDBImpl(VfxWString &path)
{
    // remove cache element
    if(m_mediaList.isValid() && m_mediaList.get()->canRemoveDBCacheElement())
    {
        IVappGalleryMediaList *pMediaList = m_mediaList.get();
        VfxBool res = pMediaList->removeDBCacheElement(path);
    }
}
...
void VappGallerySingleFileOperationUI::launchSNSUploader(const VfxWString &filePath)
{
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_LAUNCH_SNS_UPLOADER, m_cuiId);
...
{
#ifdef __MMI_MMS_2__
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_LAUNCH_UNIFIED_COMPOSER);
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_LAUNCH_UC_PRE_CHECK, preCheck);
    if(!preCheck)
    {
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_CUI_ID_D, m_cuiId);
        mmi_frm_group_set_caller_proc(m_cuiId, &VappGallerySingleFileOperationUI::callerProc, getObjHandle());
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_VGFOR_ERROR);
        m_signalOperationDone.emit(this, VGFOR_ERROR);
    }
    #else
    VfxBool result = vappUcAppLauncher(&data) ? VFX_TRUE : VFX_FALSE;
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_ON_CUI_EVENT);
    switch(evt->evt_id)
    {
    ...
    case EVT_ID_VCUI_SNS_RESULT:
        ...
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_ON_CUI_EVENT_VGFOR_DONE);
        m_signalOperationDone.emit(this, VGFOR_DONE);
        return MMI_RET_OK;
    default:
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_FILE_OP_ON_CUI_EVENT_UNKNOWN_EVENT);
        return MMI_RET_OK;
    }
}
