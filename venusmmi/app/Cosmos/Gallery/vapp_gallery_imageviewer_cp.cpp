...
//#include "vapp_gallery_imageviewer_cp.h"
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_DRAWER_DRAW_FRAME, clipWindow.origin.x, clipWindow.origin.y, clipWindow.size.width, clipWindow.size.height, frameIndex);
...
VappGalleryMediaListMediaCache* VappGalleryViewerMediaState::createMediaList(VfxObject *parent)
{
    // use file manager data source
    VappGalleryMediaListMediaCache *pList = NULL;
    VFX_OBJ_CREATE(pList, VappGalleryMediaListMediaCache, parent);
#ifdef  __LOW_COST_SUPPORT_COMMON__
    pList->configCache(VGCT_PREVIEW_ZOOM, m_cacheCount);
#else
    pList->configCache(VGCT_PREVIEW_ZOOM, 3);
#endif
    VfxWString dbFolder;
    VfxWString dbName;
    ...
    VappGalleryDB::getDBName(dbName, VappGalleryDB::PREVIEW);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_TRANSLATE_GESTURE, e.type, e.pos.x, e.pos.y);
    ...
    else if(VFX_PEN_EVENT_TYPE_UP == e.type)
    {
        ...
        do
        {
            if(!m_draggingMode) // only move a little, so this is a TAP.
            {
                ...
                MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_DOUBLE_TAP_DIST_D_THRESHOLD_D, doubleTapDist, DOUBLE_TAP_THRESHOLD);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_HANDLE_ENTER, blockRefresh);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_HANDLE_ENTERED, blockRefresh);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_HANDLE_EXIT);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_HANDLE_EXITED2, releaseForegroundOnly);
...
{
#ifdef __VAPP_MAV_MOTION_TRANSLATOR_SUPPORT__
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_SETUP_G_SENSOR);
...
{
#ifdef __VAPP_MAV_MOTION_TRANSLATOR_SUPPORT__
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_TEARDOWN_G_SENSOR);
...
{
#ifdef __VAPP_MAV_MOTION_TRANSLATOR_SUPPORT__
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_ENABLE_G_SENSOR_SUPPORT, enable);
    if(enable)
    {
        m_enableGSensorSupport = VFX_TRUE;
    }
...
void VappImageViewerComponent::clearPanel(VappGalleryViewerPanel *panel)
{
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_CLEAR_PANEL, panel->getMediaIndex(), panel->hasCachedBuffer());
...
void VappImageViewerComponent::refresh()
{
    ...
    VappGalleryMediaListMediaCache *mediaCacheList = static_cast<VappGalleryMediaListMediaCache*>(m_mediaList);
    if(mediaCacheList)
    {
...
void VappImageViewerComponent::onListUpdated(VappGalleryPrepareResult prepareResult)
{
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_ON_LIST_UPDATED_MEDIA_COUNT, mediaCount);
    if(0 < mediaCount)
    {
    ...
    if(!m_initState.m_displayed)
    {
        ...
        else if(!m_initState.m_isSyncContentProvider)
        {
            ...
            MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_ON_LIST_UPDATED_MEDIA_COUNT_ASYNC_PROVIDER_FOCUS, m_initState.m_focusIndex);
            m_initState.m_useIndexInList = VFX_TRUE;
            m_currentIndex = m_initState.m_focusIndex;
        }
        else
...
void VappImageViewerComponent::onShiftAnimationFinish(VfxTimer *sender)
{
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_ON_SHIFT_ANIMATION_FINISH);
...
{
#if !defined(__VAPP_IMAGE_VIEWER_CP_ENABLE_RESERVE_ADJACENT_PREVIEW_IMG__)
    // only full Gallery or async media list case enable reserve all preview panel in SLIM project
...
void VappImageViewerComponent::checkAllPanel(const VfxU32 line, VfxBool startEnhance)
{
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_CHECK_ALL_PANEL, startEnhance, m_currentIndex, line, -100);
        return;
    }
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_CHECK_ALL_PANEL, startEnhance, m_currentIndex, line, -200);
        return;
    }
    ...
    if(shouldReserveAdjacentPreview())
    {
        ...
        {
            MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_CHECK_ALL_PANEL, startEnhance, m_currentIndex, line, -300);
            return;
        }
        ...
        {
            MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_CHECK_ALL_PANEL, startEnhance, m_currentIndex, line, -400);
            return;
        }
    }
    ...
    if(!startEnhance)
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_CHECK_ALL_PANEL, startEnhance, m_currentIndex, line, -500);
        return;
    }

    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_CHECK_ALL_PANEL, startEnhance, m_currentIndex, line, 0);
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_CHECK_ALL_PANEL_FOUND_ANIMATED_IMAGE_D, m_currentIndex);
...
void VappImageViewerComponent::updatePanelFileInfoImpl(VappGalleryViewerPanel *panel, VfxBool isVideo)
{
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_UPDATE_FILE_INFO_IMPL, panel, mediaIdx, gdiImgFileType, frameCount);
...
void VappImageViewerComponent::onPartialEnhnaceDecodeStart(VfxSize decodedLayerSize, VfxRect clipRect)
{
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_ON_PARTIAL_ENHANCE_DECODE_START, mediaIdx, -1);
        return;
    }

    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_ON_PARTIAL_ENHANCE_DECODE_START, mediaIdx, 0);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_ON_PARTIAL_ENHANCE_DECODE_FINISHED, result);
    do
    {
        if(OK != result)
            break;
...
VappImageViewerComponent::EnhanceMediaCheckResult VappImageViewerComponent::enhancePanelMediaPreCheckImpl(VappGalleryViewerPanel *panel, MediaIndex mediaIdx)
{
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_ENHANCE_PANEL_MEDIA_PRE_CHECK_IMPL, mediaIdx, 1, ENHANCE_MEDIA_CHECK_BUF_NOT_READY);
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_ENHANCE_PANEL_MEDIA_PRE_CHECK_IMPL, mediaIdx, 2, ENHANCE_MEDIA_CHECK_BUF_NOT_READY);
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_ENHANCE_PANEL_MEDIA_PRE_CHECK_IMPL, mediaIdx, 3, ENHANCE_MEDIA_CHECK_IS_DECODING);
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_ENHANCE_PANEL_MEDIA_PRE_CHECK_IMPL, mediaIdx, 4, ENHANCE_MEDIA_CHECK_ERROR);
        return ENHANCE_MEDIA_CHECK_ERROR;
    }
    ...
    if(isPlayingSlideshow())
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_ENHANCE_PANEL_MEDIA_PRE_CHECK_IMPL, mediaIdx, 5, ENHANCE_MEDIA_CHECK_ERROR);
        return ENHANCE_MEDIA_CHECK_ERROR;
    }

    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_ENHANCE_PANEL_MEDIA_PRE_CHECK_IMPL, mediaIdx, 6, ENHANCE_MEDIA_CHECK_SUCCESS);
...
VfxBool VappImageViewerComponent::enhancePanelMedia(
    ...
    MediaIndex mediaIdx)
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_TRY_ENHANCE_PANEL_MEDIA);
    // 1. input check,
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_ENHANCE_PANEL_MEDIA, mediaIdx, -100,  enhanceBuf.width, enhanceBuf.height);
        return VFX_FALSE;
    }
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_ENHANCE_PANEL_MEDIA, mediaIdx, mediaIdx, 0,  enhanceBuf.width, enhanceBuf.height);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_ENHANCE_RESULT_D, result);
    do
    {
        if(OK != result)
            break;
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_STOP_ENHANCE);
    if(m_enhancer)
    {
        VFX_OBJ_CLOSE(m_enhancer);
        m_enhancer = NULL;
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_OFFSET_VISIBLE_PANELS_OFFSET_X_D_IS_ANIM_D, offsetX, isAnim);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_SNAP_PANEL_TO_INITIAL_POSITION_PANEL_D_IS_ANIM_D, panelPos, isAnim);
    VfxAutoAnimate::begin();
    VfxAutoAnimate::setDisable(!isAnim);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_GO_NEXT_DIR_D_IDX_D_PANEL_D, isGoNext, m_currentIndex, m_currentPanel);
...
#if 1
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_START_TIMER0);
...
/* under construction !*/
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_SNAP_PANEL_TO_INITIAL_POSITION);
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_CLEAR_PANEL);
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_SHIFT_PANEL_RETURN);
}

void VappImageViewerComponent::onTimer0(VfxTimer *sender)
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_ON_TIMER0);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_CHANGE_VIEW, 100, VAPP_GALLERY_INTERNAL_UNUSED_PARAM);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_CHANGE_VIEW, 200, (VfxS32)(value * 10000.f));
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_CHANGE_VIEW, 300, (VfxS32)(value * 10000.f));
...
void VappImageViewerComponent::updateImageFrame(VappGalleryViewerPanel *panel, const VfxFloat frameIndexHint, VfxBool isRelativeHint)
{
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_UPDATE_IMAGE_FRAME_ERR, frameIndex, isRelativeHint, -100);
        return;
    }
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_UPDATE_IMAGE_FRAME,
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_UPDATE_CURRENT_IMAGE_FRAME, (VfxS32)(frameIndexHint * 10000.f), isRelativeHint);
    m_isViewSliderDraging = VFX_FALSE;
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_GESTURE_D, gesture);
    if(isPlayingSlideshow())
    {
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_ON_CURRENT_PANEL_CHANGED_L_D, __LINE__);
    checkUpdate();
    VappGalleryViewerPanel *curPanel = getCurrentPanel();
    updateZoomSliderByPanel(curPanel);
...
void VappImageViewerComponent::onFileRightExpired(VappGalleryDRMConsumer *sender, VfxS32 index, drm_proc_enum reason)
{
    ...
    if(panel)
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_ON_FILE_RIGHT_EXPIRED_IDX_D, index);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_ON_DECODE_RESULT_IDX_D_RES_D, menuPos.pos, result);
    ...
    if(panel)
    {
        ...
        if(OK == result)
        {
            VappGalleryDecodeResult result = m_mediaList->lockThumb(menuPos, imgBuf, SRV_MEDIACACHE_IMAGE_SOURCE_CACHE);
            ...
            VfxBool isFinished)
{
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_SLIDESHOW_START_ANIMATED_GIF, m_currentIndex);
        // for animated files, directly pass the file path
        {
            if(curPanel->isOwnerDraw())
            {
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_HANDLE_SINGLE_TOUCH_PEN_INPUT, event.type, event.pos.x, event.pos.y);
...
VfxBool VappImageViewerComponent::handleMultiTouchPenInput(VfxPenEvent &event)
{
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_MULTI_TOUCH_EVENT_POS, 0, event.touchEvents[0].pos.x, event.touchEvents[0].pos.y, event.touchEvents[0].prevPos.x, event.touchEvents[0].prevPos.y);
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWER_MULTI_TOUCH_EVENT_POS, 1, event.touchEvents[1].pos.x, event.touchEvents[1].pos.y, event.touchEvents[1].prevPos.x, event.touchEvents[1].prevPos.y);
...
void VappGalleryViewerPanel::setPreviewImageAnimated(VfxBool isAnim)
{
    if(m_previewImg)
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_PANEL_SET_PREVIEW_IMAGE_ANIMATED_THIS_X_ANIM_D, this, isAnim);
...
VfxBool VappGalleryViewerPanel::setOwnerDrawHint(DrawHint hint)
{
    if(m_draw)
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_VIEWERCP_SET_OWNER_DRAWER_HINT, hint);
...
void VappGalleryViewerPanel::setPreviewCachedBuffer(const VfxImageBuffer &imgBuf,
                                                           ...
                                                           VfxRect clipWindow)
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_PANEL_SET_PREVIEW_CACHED_BUFFER);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_PANEL_SET_ENHANCED_BUFFER);
    VFX_ASSERT(imgBuf.ptr);
...
#endif
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_PANEL_OWNER_DRAW, 1);
...
void VappGalleryViewerPanel::teardownOwnerDraw()
{
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_PANEL_OWNER_DRAW, 0);
...
void VappGalleryViewerPanel::resetViewState(VfxBool resetSize)
{
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_PANEL_SET_HAS_ANIMATE_IMAGE_VFX_FALSE_THIS_X, this);
    //m_previewImg->stop();
...
void VappGalleryViewerPanel::setFrameIndex(VfxS32 frameIndex)
{
    ...
    if(m_previewImg)
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_PANEL_SET_FRAME_INDEX, frameIndex);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_PANEL_OFFSET_VIEW, offsetX, offsetY, hint, m_mediaIndex);
...
void VappGalleryViewerPanel::startAnimTimer(const VfxBool disabledAnimTimer, const VfxMsec duration, const VfxU32 line)
{
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_PANEL_START_ANIM_TIMER, disabledAnimTimer, duration, line, m_mediaIndex);
...
{
#ifdef __DRM_SUPPORT__
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_DRM_SET_FILE);
...
{
#ifdef __DRM_SUPPORT__
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_DRM_START_CONSUME);
...
{
#ifdef __DRM_SUPPORT__
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_DRM_STOP_CONSUME);
    if(!m_shouldConsume)
    {
        return;
    }
...
void VappGalleryDRMConsumer::consumeDRMCountImpl()
{
    ...
    {
        // already failed?
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_DRM_START_CONSUME_REGISTER_INSTANCE_FAILED);
        DRM_close_file(hDRMFile);
        hDRMFile = -1;
        m_state = STATE_IDLE;
    ...
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_DRM_START_CONSUME_RIGHT_FAILED);
        // already failed?
...
{
#ifdef __DRM_SUPPORT__
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_DRM_PAUSE_CONSUME);
    if(!m_shouldConsume)
    {
        return;
    }
...
{
#ifdef __DRM_SUPPORT__
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_DRM_RESUME_CONSUME);
    if(!m_shouldConsume)
    {
        return;
    }
...
{
#ifdef __DRM_SUPPORT__
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_DRM_HANDLE_RIGHT_EXPIRED);
    // stop consumption
    stopConsume();
    // alert client
...
{
#ifdef __DRM_SUPPORT__
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_DRM_ON_RIGHT_EXPIRED);
    for(VfxS32 i = 0; i < MAX_CONSUMER_INSTANCE; ++i)
    {
        if(g_instances[i])
        {
...
VfxBool VappGalleryEnhancedImage::getEnhancedImageSize(
    ...
    VfxSize originalDim)
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_ENHANCE_GET_MAX_ENHANCE_IMG_SIZE, fileTypeHint, originalDim.width, originalDim.height);
...
VfxS32 VappGalleryEnhancedImage::getMaxEnhanceDimensionByPath(const VfxWString &path, VfxU16 fileTypeHint)
{
    ...
    if(GDI_IMAGE_TYPE_INVALID == srcType)
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_ENHANCE_GET_MAX_ENHANCE_DIM_BY_PATH, fileTypeHint);
        GALLERY_MEASURE_TIME_START("Ggf");
