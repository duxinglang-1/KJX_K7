...
//#include "vapp_gallery_mediagrid_cp.h"
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_GRID_ON_INIT);
    VcpBaseMenu::onInit();
...
void VappMediaGridComponent::pauseUpdate(const VfxBool pause, const U32 line)
{
    ...
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_GRID_PAUSE_UPDATE, pause, line);
    if(pause)
        m_timer->stop();
    else
        m_timer->start();
...
void VappMediaGridComponent::onUpdateCell(const VfxRect &rect, VfxS32 group, VfxS32 pos, VfxFrame* cellFrame, VfxU32 flag)
{
    ...
    const VappMediaGridComponentCell::CellDataType type = VappMediaGridComponentCell::DATA_TYPE_MEMORY;
    ...
    if(pMediaList)
    {
        ...
        VappGalleryMediaGroupType type = pMediaList->getMediaGroupType(VcpMenuPos(group, pos));
        isVideo = (MEDIA_GROUP_VIDEO == type)? VFX_TRUE : VFX_FALSE;
        if(MEDIA_GROUP_VIDEO != type)
        {
...
void VappMediaGridComponent::heavyUpdate(void)
{
    ...
    while(!itr.isEnd())
    {
        ...
        if (cell && (cell->getImageState() != VappMediaGridComponentCell::STATE_IMAGE) && isCellDecode)
        {
            ...
            MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_GRID_HEAVY_UPDATE, p.group, p.pos, decodeResult);
            switch(decodeResult)
            {
            case OK:
...
{
    GALLERY_MEASURE_TIME_START("GGh");
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_GRID_HEAVY_UPDATE_WITH_INDEX, 0, index, result);
    const int group = 0;
    ...
    {
        if (result == OK)
            cell->updateImage(m_mediaList.get(), VappMediaGridComponentCell::DATA_TYPE_DB);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_GRID_ON_RELOAD_LIST, mediaPos.group, mediaPos.pos, 1);
    pauseUpdate(VFX_TRUE, __LINE__);
...
VfxBool VappMediaGridComponentCell::updateImage(IVappGalleryMediaList *pSource, CellDataType type)
{
    ...
    else
    {
        ...
        srv_mediacache_image_source_enum decodeMethod = getDecodeMethodFromCellType(type);
        VappGalleryDecodeResult result;
        VfxImageBuffer thumbBuf;
        {
            GALLERY_MEASURE_TIME(type == DATA_TYPE_MEMORY ? "UBC" : "UBD");
            result = pSource->lockThumb(p, thumbBuf, decodeMethod);
        }
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_GRID_SET_ERROR_ICON, m_group, m_index, line);
...
srv_mediacache_image_source_enum VappMediaGridComponentCell::getDecodeMethodFromCellType(VappMediaGridComponentCell::CellDataType type)
{
    srv_mediacache_image_source_enum decodeMethod = SRV_MEDIACACHE_IMAGE_SOURCE_CACHE;
    switch(type)
    {
    case VappMediaGridComponentCell::DATA_TYPE_MEMORY:
        decodeMethod = SRV_MEDIACACHE_IMAGE_SOURCE_CACHE;
        break;
    case VappMediaGridComponentCell::DATA_TYPE_DB:
        decodeMethod = SRV_MEDIACACHE_IMAGE_SOURCE_CACHE_AND_DATABASE;
        break;
    case VappMediaGridComponentCell::DATA_TYPE_FILE:
        decodeMethod = SRV_MEDIACACHE_IMAGE_SOURCE_CACHE_AND_DATABASE_AND_DECODE;
        break;
    default:
        VFX_DEV_ASSERT(VFX_FALSE);
        break;
