...
protected:   
    ...
    static mmi_ret onVideoClipperEvent(mmi_event_struct *evt);
    mmi_ret handleVideoClipperEvent(mmi_event_struct *evt);
...
public:
    ...
    void configStoreLastPlay(VfxBool writeLastPlayToDB);
...
private:
    ...
    VfxBool m_writeVideoInfoToDB;   // update current play position to DB when leaving
    VfxBool m_writeLastPlayInfo;    // store this video as "last play" video of Gallery
...
private:
    ...
    // create video mode control
    void createScreenModeControl(const VappVideoPlayerScreenMode defaultVSMode, VfxFrame *parentFrame);
    ...
    // handle 3D sound effect(BesSurround)
    VfxBool isBesSurroundSupported();
    VfxBool turnOnBesSurround(VfxBool isTurnOn);
    ...
    // check and update if we need to update last play info to DB
    void updateVideoPlayInfo(VappGalleryPlayerVideoFinishState finishState,
                            VappVideoMsec lastPlayPos);
...
class VappGalleryScreenModeHelper : public VfxObject
{
    VFX_DECLARE_CLASS(VappGalleryScreenModeHelper);
public:
    VappGalleryScreenModeHelper();

    void setup(VcpImageButton *normalBtn, 
               VcpImageButton *clippedBtn, 
               VcpImageButton *resizedBtn,
               const VappVideoPlayerScreenMode defaultVS);  
    void setPosition(const VfxS32 x, const VfxS32 y);
    VappGalleryScreenModeHelperID convertID(VappVideoPlayerScreenMode vs);
    VappVideoPlayerScreenMode convertToVSMode(VappGalleryScreenModeHelperID id);
    VappVideoPlayerScreenMode getNextScreenMode(VfxId curBtnId);
    void setIsDisabled(const VfxBool b);
    VfxSignal2<VfxObject*, VfxId>* getSignalTapped();
    void updateBtnStateByCurrentScreenMode(const VappVideoPlayerScreenMode mode);
    
private:    
    void showButtonById(VappGalleryScreenModeHelperID id);
    void onButtonTapped(VfxObject *button, VfxId id);
    void setAllBtnHidden(const VfxBool b);
    
private: 
    VappGalleryScreenModeHelperID m_btnIndices[3]; 
    
    VcpImageButton *m_normalBtn;
    VcpImageButton *m_clippedBtn;
    VcpImageButton *m_resizedBtn;    
    VfxSignal2 <VfxObject*, VfxId>  m_signalClicked;
    VfxBool m_isAllBtnHidden;
};
...
private:
    ...
    VfxBool _calculateNextSeparatorAndTapPos(VfxS32 &tapPosX, VfxS32 &tapPosY, 
                                                       VfxS32 &separatorPosX, VfxS32 &separatorPosY);
    void _createSeparator(const VfxS32 x, const VfxS32 y);
