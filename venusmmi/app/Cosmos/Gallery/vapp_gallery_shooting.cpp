...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SHOOTING_ON_INIT);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SHOOTING_ON_DEINIT);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SHOOTING_ON_EXIT);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SHOOTING_ON_EXITED2);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SHOOTING_ON_ENTER);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SHOOTING_ON_ENTERED);
...
void VappGalleryShootingPage::createMediaList()
{
    ...
    if(NULL == m_mediaList)
    {
        ...
        else
        {
            ...
            VappGalleryMediaListWallpaper *pList = NULL;
            VFX_OBJ_CREATE(pList, VappGalleryMediaListWallpaper, this);
...
VappGalleryMediaListMediaCache* VappGalleryShootingPage::createMediaCacheMediaList()
{
    // use file manager data source
    VappGalleryMediaListMediaCache *pList;
    VFX_OBJ_CREATE(pList, VappGalleryMediaListMediaCache, this);
    ...
    VappGalleryDB::getDBName(dbName, VappGalleryDB::THUMBNAIL);

    if(appGallery)
    {
        VfxSize thumbSize = getThumbSize();
        #ifdef __LOW_COST_SUPPORT_COMMON__
        VfxS32 cacheSize = LCD_WIDTH * LCD_HEIGHT * VAPP_GALLERY_IMG_VIEWER_PREVIEW_CACHE_COUNT / VAPP_GALLERY_MEDIA_GRID_FG_SIZE_FACTOR;
        #else
        VfxS32 cacheSize = LCD_WIDTH * LCD_HEIGHT * VAPP_GALLERY_IMG_VIEWER_ENHANCE_BUFFER_RATIO +
                           LCD_WIDTH * LCD_HEIGHT * VAPP_GALLERY_IMG_VIEWER_PREVIEW_CACHE_COUNT / VAPP_GALLERY_MEDIA_GRID_FG_SIZE_FACTOR;
        #endif
        VfxS32 cacheCount = cacheSize / (thumbSize.width * thumbSize.height);

        pList->configCache(VGCT_THUMBNAIL, cacheCount);
    }
    else
    {
        // use list default
        pList->configCache(VGCT_THUMBNAIL, 0);
    }

    pList->configThumbnailSize(getThumbSize());
...
void VappGalleryShootingPage::onMediaListReady(VappGalleryPrepareResult prepareResult)
{
    ...
    else
    {
        ...
        // restore view state for Non-wallpaper picker case
...
void VappGalleryShootingPage::getButtonNameImageById(VfxId id, VfxWString &name, MMI_IMG_ID &imgId)
{
    ...
    {
    case 'SLDS':
        name = VFX_WSTR_RES(STR_ID_VAPP_GALLERY_IMG_PLAY_SLIDESHOW);
        imgId = IMG_ID_VAPP_GALLERY_IMG_BAR_ICON_SLIDESHOW;
        break;
    case 'CAMR':
...
void VappGalleryShootingPage::onToolbarCommand(VfxObject* sender, VfxId id)
{
    ...
    {
    case 'SLDS':
        // This function is disabled in W11.12 MP
        // start playback from 1st media
        //if(m_mediaList && m_mediaList->getMediaCountInGroup(0) > 0)
        //{
        //    gotoImageViewer(VcpMenuPos(0, 0), VFX_TRUE);
        //}
        break;
    case 'CAMR':
        launchCameraApp();
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SHOOTING_ON_REQUIRE_RES_RELEASE);
    onPageInactiveImpl();
    onPageInactived2Impl();
}
...
void VappGalleryShootingPage::onThumbTapped(VcpMenuPos tappedPos, VfxFrame* thumbTapped)
{
    ...
    if(NULL != m_wallpaperInfo)
    {
        ...
        VappGalleryMediaListWallpaper::getGroupInfoFromWallpaperInfo(
                                                    *m_wallpaperInfo,
                                                    groupCount,
    ...
    else
    {
        ...
        else
        {
            ...
            if(MEDIA_GROUP_VIDEO == m_mediaList->getMediaGroupType(tappedPos))
            {
                // launch video player
...
void VappGalleryShootingPage::onThumbLongPressed(VcpMenuPos pressedPos)
{
    ...
    m_mediaList->getMediaPath(pressedPos, m_singleFilePopup->m_filePath);
