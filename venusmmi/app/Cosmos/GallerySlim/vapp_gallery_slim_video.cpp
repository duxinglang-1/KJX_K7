...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SLIM_VIDEO_PAGE_ON_ENTER);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SLIM_VIDEO_PAGE_ON_ENTERED);
    // in Snapshot mode, we're not in FG, cannot allocate resources!
    if(getMainScr() && getMainScr()->getIsSnapshotDrawing())
    {
        return;
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SLIM_VIDEO_PAGE_ON_EXIT);
    // in Snapshot mode, we're not in FG, cannot allocate resources!
    if(getMainScr() && getMainScr()->getIsSnapshotDrawing())
    {
        return;
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SLIM_VIDEO_PAGE_ON_EXITED2);
    // in Snapshot mode, we're not in FG, cannot allocate resources!
    if(getMainScr() && getMainScr()->getIsSnapshotDrawing())
    {
        return;
...
void VappGalleryVideoPage::handleViewScrollEnd(VcpBaseListMenu *menu)
{
    m_isScrolling = VFX_FALSE;
    startRefreshTimer(VFX_TRUE, __LINE__);
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SLIM_VIDEO_PAGE_ON_VIEW_SCROLL_END_M_THUMB_REFRESH_TIMER_START);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SLIM_VIDEO_PAGE_ON_MEDIA_LIST_READY);
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SLIM_VIDEO_PAGE_ON_RELOAD_LIST, mediaPos.group, mediaPos.pos, 1);
    startRefreshTimer(VFX_FALSE, __LINE__);
    showPageWaitIcon(VFX_TRUE);
}
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SLIM_VIDEO_PAGE_ON_RELOAD_LIST, mediaPos.group, mediaPos.pos, 2);
    startRefreshTimer(VFX_TRUE, __LINE__);
    showPageWaitIcon(VFX_FALSE);
}
...
void VappGalleryVideoPage::startRefreshTimer(const VfxBool start, const VfxU32 line)
{
    ...
    if(start)
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SLIM_VIDEO_PAGE_M_THUMB_REFRESH_TIMER_START_LINE_D, line);
        m_thumbRefreshTimer->start();
        GALLERY_MEASURE_TIME_START("Grt");
    }
    else
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SLIM_VIDEO_PAGE_M_THUMB_REFRESH_TIMER_STOP_LINE_D, line);
        m_thumbRefreshTimer->stop();
...
{
    MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SLIM_VIDEO_PAGE_ON_REQUIRE_RES_RELEASE);
    onPageInactiveImpl();
    onPageInactived2Impl();
}
    ...
    )
{
       ...
       OK == m_mediaList->getMediaName(p, text))
    {
        ...
        return VFX_TRUE;
            ...
            !m_isExit)
    {
        MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SLIM_VIDEO_PAGE_GET_ITEM_TEXT_SUB_TEXT1_M_IS_EXIT_D, m_isExit);
        U64 duration = 0;
    ...
    )
{
    ...
    if(VCP_LIST_MENU_FIELD_ICON == fieldType)
    {
        ...
        else
        {
            ...
            MMI_TRACE(TRACE_GROUP_2, TRC_VAPP_GALLERY_SLIM_VIDEO_PAGE_GET_ITEM_IMAGE, pos.group, pos.pos, decodeResult, m_isScrolling);
