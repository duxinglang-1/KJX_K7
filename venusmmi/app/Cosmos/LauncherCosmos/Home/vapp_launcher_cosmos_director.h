 ...
 * Filename:
 * ---------
 *  vapp_launcher_cosmos_director.h
 *
 * Project:
 * --------
 *  MAUI
...
#ifndef VAPP_LAUNCHER_COSMOS_DIRECTOR_H
#define VAPP_LAUNCHER_COSMOS_DIRECTOR_H
...
class VappLauncherCosmosDirector : public VappLauncher
{
    VAPP_LAUNCHER_DECLARE_CLASS(VappLauncherCosmosDirector);
...
// status for recover history because the launcher object may be free
private:
...
private:
    // Desktop.
    VappLauncherCosmosDesktop *m_desktop;
...
private:
    ...
    void onDesktopPageChanged(
        VappLauncherCosmosDesktop *desktop    // [IN] Desktop
    );
...
private:
    // Indicator.
    VappLauncherCosmosPageIndicator *m_indicator;
...
private:
    ...
    void onIndicatorClicked(
        VappLauncherCosmosPageIndicator *indicator    // [IN] Indicator
    );
...
private:
    // Shortcut bar.
    VappLauncherCosmosShortcutBar *m_shortcutBar;

    // If wallpaper is suspended because notification center is shown.
    VfxBool m_isMMLaunchedByShortcutBar;
...
private:
    ...
    // This function handles the main menu launched signal.
...
private:
    // Toolbar.
    VappLauncherCosmosToolbar *m_toolbar;
...
private:
    ...
    void onToolbarExecute(
        VappLauncherCosmosToolbar *toolbar,           // [IN] Toolbar
        VappLauncherCosmosToolbar::ButtonEnum button  // [IN] The clicked button
    );
...
private:
    ...
    VfxBool m_isPageFull[VAPP_LAUNCHER_COSMOS_MAX_PAGE_NUM];
...
private:
    ...
    void onPageSelectorAssembled(
        VappLauncherCosmosPageSelector *pageSelector  // [IN] Page selector
    );
    ...
    void onPageSelectorAborted(
        VappLauncherCosmosPageSelector *pageSelector,         // [IN] Page selector
        VappLauncherCosmosPageSelector::AbortCauseEnum cause  // [IN] Error cause
    );
    ...
    VappLauncherCosmosPageSelector *createPageSelector();
...
// Variable
private:
    // record how many shortcut widget
    VfxS32 m_numShortcut;

    // Shortcut selector.
    VappLauncherCosmosShortcutSelector *m_shortcutSelector;

    // Widget selector.
    VappLauncherCosmosWidgetSelector *m_widgetSelector;

    VfxWeakPtr <VfxLauncherCosmosPageSlideTransition> m_pageSlideTransitionWeakPtr;
...
private:
    ...
    // This function activates the widget.
    void activateWidget(
        VappWidget *widget      // [IN] Widget
    );
    ...
    // This function sets all the created widgets to suspended or not.
    void setWidgetSuspended(
        VfxBool isSuspended // [IN] Suspended or not
    );
...
class VfxLauncherCosmosPageSlideTransition : public VfxPageSlideTransition
{
    VFX_DECLARE_CLASS(VfxLauncherCosmosPageSlideTransition);
    
    virtual VfxBool customOuted();
};


#endif /* VAPP_LAUNCHER_COSMOS_DIRECTOR_H */
