...
const VfxFloat VappLauncherCosmosPageSelectorDebugPanel::s_opacity = 0.6f;


VappLauncherCosmosPageSelectorDebugPanel::VappLauncherCosmosPageSelectorDebugPanel() :
    m_isOn(VFX_FALSE),
    m_aniTime(0),
    m_aniTimeText(NULL),
...
void VappLauncherCosmosPageSelectorDebugPanel::onInit()
{
    VfxControl::onInit();
    ...
    m_aniTimeAdjustTimer->m_signalTick.connect(this, &VappLauncherCosmosPageSelectorDebugPanel::onAniTimeAdjustTimer);
...
void VappLauncherCosmosPageSelectorDebugPanel::onDeinit()
{
    VfxControl::onDeinit();
}


VfxBool VappLauncherCosmosPageSelectorDebugPanel::onContainPoint(
...
VfxBool VappLauncherCosmosPageSelectorDebugPanel::onPenInput(VfxPenEvent &event)
{
...
VfxBool VappLauncherCosmosPageSelectorDebugPanel::onAniTimePenInput(VfxPenEvent &event)
{
    switch (event.type)
    {
...
void VappLauncherCosmosPageSelectorDebugPanel::initAnimationTime(VfxS32 time)
{
    m_aniTime = time;
...
void VappLauncherCosmosPageSelectorDebugPanel::updateAniTime(VfxS32 diff)
{
...
void VappLauncherCosmosPageSelectorDebugPanel::onAniTimeAdjustTimer(VfxTimer *timer)
{
    updateAniTime(m_aniTimeDiff);
...
const VfxColor VappLauncherCosmosPageSnapshot::s_alertFilterMin(0, 128, 0, 0);    // ARGB
const VfxColor VappLauncherCosmosPageSnapshot::s_alertFilterMax(255, 255, 0, 0);  // ARGB
const VfxFloat VappLauncherCosmosPageSnapshot::s_highlightOpacity = 0.6f;


VappLauncherCosmosPageSnapshot::VappLauncherCosmosPageSnapshot() :
    m_id(0),
    m_bgImg(NULL),
...
void VappLauncherCosmosPageSnapshot::onInit()
{
    VfxControl::onInit();
    ...
    m_bgImg->setResId(IMG_ID_VAPP_LAUNCHER_COSMOS_SNAPSHOT_BG);
...
void VappLauncherCosmosPageSnapshot::onDeinit()
{
    VfxControl::onDeinit();
}


VfxBool VappLauncherCosmosPageSnapshot::onPreviewPenInput(VfxPenEvent &event)
{
    return VfxControl::onPreviewPenInput(event);
}


VfxBool VappLauncherCosmosPageSnapshot::onPenInput(VfxPenEvent &event)
{
    switch (event.type)
    {
        case VFX_PEN_EVENT_TYPE_DOWN:
...
void VappLauncherCosmosPageSnapshot::setBounds(const VfxRect &value)
{
...
void VappLauncherCosmosPageSnapshot::setId(VfxId id)
{
    m_id = id;
}


VfxId VappLauncherCosmosPageSnapshot::getId() const
{
    return m_id;
}


void VappLauncherCosmosPageSnapshot::setLayout(const VfxRect &sourceRect)
{
...
void VappLauncherCosmosPageSnapshot::getLayout(VfxRect &sourceRect) const
{
    sourceRect = m_sourceRect;
}


void VappLauncherCosmosPageSnapshot::addToContainer(VfxFrame *frame)
{
    if (frame)
    {
...
void VappLauncherCosmosPageSnapshot::removeFromContainer(VfxObjList &objList)
{
...
VfxBool VappLauncherCosmosPageSnapshot::isEmpty()
{
    return m_objList.isEmpty();
}


void VappLauncherCosmosPageSnapshot::setHighlight(VfxBool isOn)
{
...
void VappLauncherCosmosPageSnapshot::setAlert(VfxBool isOn)
{
#if defined(__COSMOS_3D_V10__)
    if (isOn)
    {
...
void VappLauncherCosmosPageSnapshot::setObjectHidden(VfxBool isHidden)
{
    for (VfxObjListEntry *entry = m_objList.getHead();
         entry != NULL;
         entry = entry->getNext())
...
void VappLauncherCosmosPageSnapshot::setBgImgOpacity(VfxFloat value)
{
...
void VappLauncherCosmosPageSnapshot::setBgImgHidden(VfxBool isHidden)
{
    VFX_OBJ_ASSERT_VALID(m_bgImg);
...
void VappLauncherCosmosPageSnapshot::setBgImgRect(const VfxRect &rect)
{
    VFX_OBJ_ASSERT_VALID(m_bgImg);
...
void VappLauncherCosmosPageSnapshot::setContentRect(
    const VfxRect &rect,
...
VfxPoint VappLauncherCosmosPageSnapshot::getRectOffset(
    const VfxRect &dst,
...
void VappLauncherCosmosPageSnapshot::getInner9SliceRect(VfxRect &rect)
{
...
VfxFPoint VappLauncherCosmosPageSnapshot::getResizeRatio(
    const VfxRect &dst,
    const VfxRect &src) const
{
...
VfxS32 VappLauncherCosmosPageSelector::s_autoAnimationTime = 400;


VappLauncherCosmosPageSelector::VappLauncherCosmosPageSelector() :
...
#endif /* defined(VAPP_LAUNCHER_COSMOS_UT_PAGE_SELECTOR_DEBUG_PANEL) */
{
    VFX_ASSERT(VAPP_LAUNCHER_COSMOS_MAX_PAGE_NUM >= SNAPSHOT_PER_ROW);

    m_totalRowNum = (VfxS32)vfx_sys_ceil((VfxFloat)VAPP_LAUNCHER_COSMOS_MAX_PAGE_NUM / SNAPSHOT_PER_ROW);
    ...
    //
    VfxS32 snapshotNum = VAPP_LAUNCHER_COSMOS_MAX_PAGE_NUM;
    VfxS32 idx = 0;
...
void VappLauncherCosmosPageSelector::onInit()
{
    VfxFrame::onInit();
    ...
    m_assembleTimer->m_signalTick.connect(this, &VappLauncherCosmosPageSelector::onTimerAssembled);

    VFX_OBJ_CREATE(m_dismissTimer, VfxTimer, this);
    m_dismissTimer->setStartDelay(s_autoAnimationTime);
    m_dismissTimer->m_signalTick.connect(this, &VappLauncherCosmosPageSelector::onTimerDismissed);

    VFX_OBJ_CREATE(m_delayAbortTimer, VfxTimer, this);
    m_delayAbortTimer->setStartDelay(1);
    m_delayAbortTimer->m_signalTick.connect(this, &VappLauncherCosmosPageSelector::onTimerDelayAbortCallback);
    ...
    for (VfxS32 i = 0; i < VAPP_LAUNCHER_COSMOS_MAX_PAGE_NUM; i++)
    {
        VFX_OBJ_CREATE(m_snapshot[i], VappLauncherCosmosPageSnapshot, this);

        m_snapshot[i]->setId((VfxId)i);
        m_snapshot[i]->m_signalTapped.connect(this, &VappLauncherCosmosPageSelector::onSnapshotTapped);
    }
...
void VappLauncherCosmosPageSelector::onDeinit()
{
    VfxFrame::onDeinit();
}


VfxBool VappLauncherCosmosPageSelector::onPenInput(VfxPenEvent &event)
{
    return VFX_TRUE;
}


VfxBool VappLauncherCosmosPageSelector::onKeyInput(VfxKeyEvent &event)
{
...
void VappLauncherCosmosPageSelector::setInnerBorder(VfxS32 horizWidth, VfxS32 vertiWidth)
{
...
void VappLauncherCosmosPageSelector::setOuterBorder(
    VfxS32 topWidth,
    VfxS32 bottomWidth,
    VfxS32 leftWidth,
    VfxS32 rightWidth)
...
void VappLauncherCosmosPageSelector::setLayout(
    ...
    VfxS32 totalRowNum)
{
    ...
    VFX_ASSERT(totalSnapshot == VAPP_LAUNCHER_COSMOS_MAX_PAGE_NUM);
}


VfxBool VappLauncherCosmosPageSelector::doPenInput(VfxPenEvent &event)
{
    switch (event.type)
    {
        case VFX_PEN_EVENT_TYPE_DOWN:
...
void VappLauncherCosmosPageSelector::setId(VfxId id)
{
    m_id = id;
}


VfxId VappLauncherCosmosPageSelector::getId() const
{
    return m_id;
}


VfxS32 VappLauncherCosmosPageSelector::getLastSelectedIndex() const
{
    return m_lastSelectedIndex;
}


void VappLauncherCosmosPageSelector::setFocusSnapshot(VfxS32 idx)
{
    m_focusSnapshotIndex = idx;
}


VfxS32 VappLauncherCosmosPageSelector::getFocusSnapshot() const
{
    return m_focusSnapshotIndex;
}


void VappLauncherCosmosPageSelector::setSnapshotLayout(
    VfxS32 idx,
...
void VappLauncherCosmosPageSelector::addToSnapshot(VfxS32 idx, VfxFrame *frame)
{
...
void VappLauncherCosmosPageSelector::removeFromSnapshot(VfxS32 idx, VfxObjList &objList)
{
    if (idx < 0 || idx >= VAPP_LAUNCHER_COSMOS_MAX_PAGE_NUM)
    {
        return;
...
void VappLauncherCosmosPageSelector::assemble()
{
    //
...
void VappLauncherCosmosPageSelector::dismiss()
{
    //
...
void VappLauncherCosmosPageSelector::revive()
{
    m_isResponseProvided = VFX_FALSE;
}


void VappLauncherCosmosPageSelector::setSnapshotAlert(VfxS32 idx, VfxBool isOn)
{
    VFX_ASSERT(idx >= 0 && idx < VAPP_LAUNCHER_COSMOS_MAX_PAGE_NUM);
...
void VappLauncherCosmosPageSelector::disableSnapshotAlert()
{
    for (VfxS32 i = 0; i < VAPP_LAUNCHER_COSMOS_MAX_PAGE_NUM; i++)
    {
...
void VappLauncherCosmosPageSelector::emitSignalSelected(VfxS32 idx)
{
    //
...
void VappLauncherCosmosPageSelector::emitSignalAbort(AbortCauseEnum cause)
{
    //
    // Guarantee that we only provide the response (i.e., either 'selected' or
    // 'aborted') once.
...
VfxBool VappLauncherCosmosPageSelector::isSnapshotHitted(VfxPenEvent &event, VfxS32 idx)
{
...
VfxS32 VappLauncherCosmosPageSelector::getMaxSnapshotNumPerRow() const
{
    VfxS32 max = 0;
...
void VappLauncherCosmosPageSelector::getCoordinate(VfxS32 idx, VfxS32 &x, VfxS32 &y) const
{
    x = idx;
    y = 0;
...
VfxSize VappLauncherCosmosPageSelector::getSnapshotSize() const
{
    //
...
VfxRect VappLauncherCosmosPageSelector::getSnapshotRoom(VfxS32 idx) const
{
    VFX_ASSERT(idx >= 0 && idx < VAPP_LAUNCHER_COSMOS_MAX_PAGE_NUM);
...
VfxRect VappLauncherCosmosPageSelector::getSnapshotOutsideRoom(VfxS32 idx) const
{
    VFX_ASSERT(idx >= 0 && idx < VAPP_LAUNCHER_COSMOS_MAX_PAGE_NUM);
...
VfxRect VappLauncherCosmosPageSelector::tuneSnapshotRoom(
    const VfxRect &room,
...
void VappLauncherCosmosPageSelector::onSnapshotTapped(VappLauncherCosmosPageSnapshot *snapshot)
{
...
void VappLauncherCosmosPageSelector::onTimerAssembled(VfxTimer *timer)
{
...
void VappLauncherCosmosPageSelector::onTimerDismissed(VfxTimer *timer)
{
    m_state = STATE_DISMISSED;
...
void VappLauncherCosmosPageSelector::onTimerDelayAbortCallback(VfxTimer *timer)
{
    emitSignalAbort(ABORT_CAUSE_DROP_OUTSIDE);
}
...
#if defined(VAPP_LAUNCHER_COSMOS_UT_PAGE_SELECTOR_DEBUG_PANEL)
void VappLauncherCosmosPageSelector::createDebugPanel()
{
    VFX_OBJ_CREATE(m_debugPanel, VappLauncherCosmosPageSelectorDebugPanel, this);
    ...
    m_debugPanel->m_signalAnimationTimeChanged.connect(this, &VappLauncherCosmosPageSelector::onAnimationTimeChanged);
}


void VappLauncherCosmosPageSelector::onAnimationTimeChanged(VfxS32 time)
{
