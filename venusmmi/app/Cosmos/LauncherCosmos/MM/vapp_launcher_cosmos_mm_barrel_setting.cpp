 ...
 * Filename:
 * ---------
 *  vapp_launcher_cosmos_MM_barrel_setting.cpp
 *
 * Project:
 * --------
 *  Launcher Cosmos MM
...
#include "vapp_launcher_cosmos_mm_barrel_setting.h"
...
extern "C"
{
    MMI_ID vapp_launcher_cosmos_mm_barrel_setting_launch(void* param, U32 param_size)
    {
        VappLauncherCosmosMmContent *launcherMM = vappLauncherCosmosMmGetInstance();

        VappLauncherCosmosMmBarrelSetting *barrelSetting;
        VFX_OBJ_CREATE(barrelSetting, VappLauncherCosmosMmBarrelSetting, launcherMM);
...
VFX_IMPLEMENT_CLASS("VappLauncherCosmosMmBarrelSetting", VappLauncherCosmosMmBarrelSetting, VfxObject);

VappLauncherCosmosMmBarrelSetting::VappLauncherCosmosMmBarrelSetting():
    m_menuPop(NULL)
{
}

VfxS32 VappLauncherCosmosMmBarrelSetting::s_barrelSettingLastSelected = VCP_PAGE_MENU_TRANSITION_TYPE_NORMAL;

void VappLauncherCosmosMmBarrelSetting::onInit()
{
    VfxObject::onInit();
    ...
    m_menuPop->setText((STR_ID_VAPP_LAUNCHER_COSMOS_MM_BARREL_SETTING_NAME));
    ...
    m_menuPop->m_signalButtonClicked.connect(this, &VappLauncherCosmosMmBarrelSetting::onBarrelSettingMenuCallack); 
...
void VappLauncherCosmosMmBarrelSetting::onDeinit()
{
#ifdef VAPP_LAUNCHER_COSMOS_MM_MOTION_DOUBLE_TAP_TO_CHANGE_SWITCH_EFFECT
    // get mainmenu's pointer
    VappLauncherCosmosMmContent *launcherMM = vappLauncherCosmosMmGetInstance();
...
void VappLauncherCosmosMmBarrelSetting::onBarrelSettingMenuCallack(VfxObject* sender, VfxId id)
{
    VappLauncherCosmosMmContent *launcherMM = vappLauncherCosmosMmGetInstance();
...
void VappLauncherCosmosMmBarrelSetting::writeToNVRAM(VfxU32 writeValue)
{
    m_selectedID= writeValue;

    // write to NVRAM
    VfxS16 sError = 0;
    WriteValue(NVRAM_VAPP_LAUNCHER_COSMOS_MM_BARREL_SETTING, (void*)&writeValue, DS_BYTE, & sError);

    //MMI_TRACE(TRACE_GROUP_3, TRC_VAPP_LAUNCHER_COSMOS_MM_BARREL_SETTING_WRITE_TO_NVRAM, ret);
}


void VappLauncherCosmosMmBarrelSetting::readFromNVRAM()
{
    // read MMI_CACHE
    VfxS16 sError = 0;
    ...
    {
        VfxU8 selected = 0;
        ReadValue(NVRAM_VAPP_LAUNCHER_COSMOS_MM_BARREL_SETTING, (void*)&selected, DS_BYTE, & sError);

	setSelectedValue(selected);
    }

    //MMI_TRACE(TRACE_GROUP_3, TRC_VAPP_LAUNCHER_COSMOS_MM_BARREL_SETTING_READ_FROM_NVRAM, ret);
}


void VappLauncherCosmosMmBarrelSetting::setSelectedValue(VfxU32 selectedID)
{
