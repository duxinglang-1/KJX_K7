...
VFX_IMPLEMENT_CLASS("VappUcContactList", VappUcContactList, VfxFrame);
void VappUcContactList::onInit()
{
	VfxFrame::onInit();	
	...
	this->m_contactlist->m_signalReady.connect(this, &VappUcContactList::onListReady);
    this->m_contactlist->m_signalRefresh.connect(this, &VappUcContactList::onListRefresh);
#if 0
/* under construction !*/
/* under construction !*/
/* under construction !*/
...
VfxBool VappUcContactList::getItemText(
...
{
#ifdef __MMI_EMAIL__
  	if (m_contactlisttype == ENUM_VAPP_UC_CONTACTLIST_TYPE_EMAIL)
	{
		WCHAR address[200];
...
VfxBool VappUcContactList::getItemTextFormat(
					VfxU32 index,                    // [IN] the index of item
					VcpListMenuFieldEnum fieldType,  // [IN] the type of the field in the cell
...
VfxU32 VappUcContactList::getCount() const
{
#ifdef __MMI_EMAIL__
	if (m_contactlisttype == ENUM_VAPP_UC_CONTACTLIST_TYPE_EMAIL)
	{
...
VfxBool VappUcContactList::getItemImage(
		VfxU32 index,                    
...
VappUcContactList::VappUcContactList(VfxS32 len):	m_hintlength(len),
                                                    	m_hinttext(),
                                                    	m_listmenu(NULL),
                                                    	m_contactlist(NULL),
                                                    	m_alignField(0),
...
void VappUcContactList::updateHintList(VfxWChar *text)
{
	VfxWString itemText;
#ifdef __MMI_EMAIL__    
	...
	{
                m_contactlisttype = ENUM_VAPP_UC_CONTACTLIST_TYPE_EMAIL;
...
#endif
	{
	      m_contactlisttype = ENUM_VAPP_UC_CONTACTLIST_TYPE_PHBOOK;
...
void VappUcContactList::setSearchString(const VfxWString& search)
{
	// query result
    m_contactlist->setQuery(search, MMI_PHB_CONTACT_FIELD_NAME | m_alignField);
}

void VappUcContactList::setAlignField(VfxU32 alignField)
{
	m_alignField = alignField;
	m_contactlist->setAlignField(alignField);
}

void VappUcContactList::getList()
{
    m_contactlist->getList();
}
void VappUcContactList::onListReady()
{
#ifdef __MMI_EMAIL__
	if (m_contactlisttype != ENUM_VAPP_UC_CONTACTLIST_TYPE_EMAIL)
#endif
...
void VappUcContactList::onListRefresh(ContactList* list, mmi_event_struct* evt)
{
    getList();
}
