...
void VappWCViewCityCellItem::onUpdateElements()
{
	...
	if(cp == 0)
	{
		m_date->setString(VFX_WSTR_RES(STR_ID_VAPP_WC_DATE_TODAY));
	}
	else if(cp == 1)
	{
		m_date->setString(VFX_WSTR_RES(STR_ID_VAPP_WC_DATE_YESTERDAY));
	}
	else
	{
		m_date->setString(VFX_WSTR_RES(STR_ID_VAPP_WC_DATE_TOMORROW));
	}
...
void VappWCListViewPage::onInit()
{
	...
	m_toolbar->addItem(ID_VAPP_WC_ADD, VFX_WSTR_RES(STR_GLOBAL_ADD), VCP_IMG_TOOL_BAR_COMMON_ITEM_ADD);
	m_toolbar->addItem(ID_VAPP_WC_LIST, VFX_WSTR_RES(STR_GLOBAL_LIST), VCP_IMG_TOOL_BAR_COMMON_ITEM_LIST);
...
void VappWCListViewPage::showFrnEditMenu()
{
    ...
    cmdPopup->addItem(F_REPLACE_CITY, VFX_WSTR_RES(STR_ID_VAPP_WC_REPLACE_CICY), VCP_POPUP_BUTTON_TYPE_NORMAL);
    if (m_dataControl.getFrnCityDst((VfxU8)(m_selectID-1)))
    {
        cmdPopup->addItem(F_ACTIVE_DAY_LIGHT, VFX_WSTR_RES(STR_ID_VAPP_WC_DST_DISABLE), VCP_POPUP_BUTTON_TYPE_NORMAL);
    }
    else
    {
        cmdPopup->addItem(F_ACTIVE_DAY_LIGHT, VFX_WSTR_RES(STR_ID_VAPP_WC_DST_ACT), VCP_POPUP_BUTTON_TYPE_NORMAL);
    }
    cmdPopup->addItem(F_SET_AS_HOMECITY, VFX_WSTR_RES(STR_ID_VAPP_WC_HOMECICY_MENU), VCP_POPUP_BUTTON_TYPE_NORMAL);
    cmdPopup->addItem(F_REMOVE, VFX_WSTR_RES(STR_GLOBAL_REMOVE), VCP_POPUP_BUTTON_TYPE_NORMAL);
    cmdPopup->addItem(F_CLOSE, VFX_WSTR_RES(STR_GLOBAL_CANCEL), VCP_POPUP_BUTTON_TYPE_CANCEL);
	...
	switch (id)
	   {
		   ...
		   case VCP_CONFIRM_POPUP_BUTTON_USER_1:
		   {
				...
				m_dataControl.updateSysTime(cityIndex, dst);
