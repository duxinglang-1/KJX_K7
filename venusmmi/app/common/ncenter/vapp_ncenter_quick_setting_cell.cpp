...
extern "C" 
{
	#include "NotificationGprot.h" // for mmi_frm_nmgr_notify_by_app
}
#else
#include "vcp_confirm_popup.h" // for VcpConfirmPopup
extern "C" 
{
	#include "AlertScreen.h" // for mmi_popup_display_ext
}
#endif
...
//func implementation:
	...
	if(VAPP_QSETTING_TOTAL - 1 > 5)
		m_elementsInOneLine = 5;
	else
		m_elementsInOneLine = VAPP_QSETTING_TOTAL - 1;
	
	createBgImg(cellSize);
	createElements(VfxSize((getSize().width - 10)/m_elementsInOneLine, ELEMENT_HIGHT));
	registerEvent();
...
typedef struct 
{
    ...
    VfxS32 popup_handle;
} vapp_quick_setting_cell_cancel_popup_event_struct;





static mmi_ret vapp_quick_setting_cell_cancel_popup(mmi_event_struct *evt)
{
    vapp_quick_setting_cell_cancel_popup_event_struct *evnt = (vapp_quick_setting_cell_cancel_popup_event_struct *)evt;
    vcp_global_popup_cancel(evnt->popup_handle);    
    return MMI_RET_OK;
}
...
//func implementation:
	...
	if(m_popupHandle)
	{
        vapp_quick_setting_cell_cancel_popup_event_struct postInvoke;
        MMI_FRM_INIT_EVENT(&postInvoke, 0);
        postInvoke.popup_handle = m_popupHandle;
        MMI_FRM_POST_EVENT(&postInvoke, vapp_quick_setting_cell_cancel_popup, NULL);        
	}
#endif
	VfxFrame::onDeinit();
}
...
//func implementation:
...
#if defined(__COSMOS_MMI_PACKAGE__)
	m_popupHandle = vcp_global_popup_show_confirm_two_button_id(
		GRP_ID_ROOT,
		VCP_POPUP_TYPE_WARNING,
		textResId,
		button1ResId,
		STR_ID_QSETTING_BUTTON_CANCEL,
		VCP_POPUP_BUTTON_TYPE_WARNING,
		VCP_POPUP_BUTTON_TYPE_CANCEL,
		&VappQuickSettingCell::onUserComfirmed,
		getObjHandle()
		);
	...
	mmi_frm_nmgr_notify_by_app(MMI_SCENARIO_ID_DEFAULT,
		MMI_EVENT_WARNING,
		&VappQuickSettingCell::warningToneCallback,
		this
		);

#else //show popup in FTE.
	#if defined(__MMI_BT_SLIM__)
		if(VAPP_QSETTING_COMFIRM_BT_OFF_WITH_CONNECTING == m_comfirmType)
		{
...
MMI_BOOL VappQuickSettingCell::warningToneCallback(mmi_scenario_id scen_id, void *user_data)
{
	return MMI_TRUE;
}

#ifndef __MMI_BT_IN_FLIGHT_MODE__
void VappQuickSettingCell::onNullComfirmed(VfxId id, void *userData)
{
	return;
}
...
*/
			...
			if(aNewStatus)// turn on wifi
			{
				...
				{ // popup flightmode ask.
					m_comfirmType = VAPP_QSETTING_COMFIRM_WIFI_ON_WITH_FLIGHT_MODE;
					showComfirmPopup();
					return;
				}
				else
				...
				{//flight mode and tethering won't turn on at the same time.
					m_comfirmType = VAPP_QSETTING_COMFIRM_WIFI_ON_WITH_TETHERING;
					showComfirmPopup();
					return;
				}
			...
			{
				element->showProcessInd();
#if defined(__MMI_OP01_WIFI__) && defined(__COSMOS_MMI_PACKAGE__)
...
*/
			...
			if(aNewStatus && SRV_BT_CM_POWER_OFF == btStatus)// turn on BT
			{
				if (!srv_mode_switch_is_network_service_available()) // ZHY: if in flight mode
				{
					...
					m_comfirmType = VAPP_QSETTING_COMFIRM_BT_ON_WITH_FLIGHT_MODE;
					showComfirmPopup();
#else
#if defined(__COSMOS_MMI_PACKAGE__)
						m_popupHandle = vcp_global_popup_show_confirm_two_button_id(
							GRP_ID_ROOT,
							VCP_POPUP_TYPE_INFO,
							STR_ID_QSETTING_NOT_AVAILABLE,
							STR_ID_QSETTING_BUTTON_OK,
							STR_ID_QSETTING_BUTTON_CANCEL,
							VCP_POPUP_BUTTON_TYPE_NORMAL,
							VCP_POPUP_BUTTON_TYPE_CANCEL,
							&VappQuickSettingCell::onNullComfirmed,
							NULL
							);
					...
					if(scr)
					{
						...
						VFX_OBJ_CREATE(popup, VcpConfirmPopup, scr);
						popup->setInfoType(VCP_POPUP_TYPE_WARNING);
						popup->setText(VFX_WSTR_RES(STR_ID_QSETTING_NOT_AVAILABLE));
			...
			else if(aNewStatus == 0 && SRV_BT_CM_POWER_ON == btStatus)// turn off BT
			{
				...
				{
					m_comfirmType = VAPP_QSETTING_COMFIRM_BT_OFF_WITH_CONNECTING;
					showComfirmPopup();
					return;
				}
				else
		...
		case VAPP_QSETTING_FLIGHT_MODE:
		{
			...
			{				
				...
				balloon->addItem(
				VCP_INFO_BALLOON_TYPE_INFO,
				VFX_WSTR_RES(STR_ID_QSETTING_BALLOON_IN_CALL));
#else //FTE
				{
					VappNCenter* nc = VFX_OBJ_GET_INSTANCE(VappNCenter);
					VappNCenterScr *scr = nc->getNCenterScr();
					if(scr)
					{
						...
						popup->setText(VFX_WSTR_RES(STR_ID_QSETTING_BALLOON_IN_CALL));
						popup->setButtonSet(VCP_CONFIRM_BUTTON_SET_OK);
						popup->setAutoDestory(VFX_TRUE);
						popup->show(VFX_TRUE);
					}
...
void VappQuickSettingCell::setWidth(VfxU32 width)
{
	VfxU32 elementWidth;
	VFX_OBJ_CLOSE(m_backgroundFrame);
	createBgImg(VfxSize(width, ELEMENT_HIGHT * m_rowNum+ 10));
	m_backgroundFrame->sendToBack();
	elementWidth = (width-10)/m_elementsInOneLine;
	for (VfxS32 i = 0; i < VAPP_QSETTING_TOTAL - 1; i++ )
    {
		if(m_elementInfo[i].elementHandle)
		{
			m_elementInfo[i].elementHandle->setPos(VfxPoint(
							5 +((i%5)* elementWidth) + (elementWidth>>1), 
							5 + ((i/5)*ELEMENT_HIGHT))
							);	
		}
	}
...
//func implementation:
	...
	m_backgroundFrame->setSize(aBackgroundSize);
	
	VFX_OBJ_CREATE(bgImg,VfxImageFrame,m_backgroundFrame);
	bgImg->setResId(IMG_ID_VAPP_QUICK_SETTING_BG_N);
    bgImg->setSize(aBackgroundSize.width - BG_IMGE_WIDTH, BG_IMGE_HEIGHT);
    bgImg->setContentPlacement(VFX_FRAME_CONTENT_PLACEMENT_TYPE_RESIZE);
	bgImg->setPos(BG_IMG_POS_X, BG_IMG_POS_Y);
    // Separator
    for (VfxS32 i = 1; i < m_elementsInOneLine; i++)
    {
        VfxImageFrame *separator;

        VFX_OBJ_CREATE(separator, VfxImageFrame, m_backgroundFrame);
        ...
        separator->setPos(
							5 + (((aBackgroundSize.width -10)/m_elementsInOneLine)* i), 
							5
							);
    }
}
