 ...
 * Filename:
 * ---------
 *  vapp_screen_lock_native_factory.cpp
 *
 * Project:
 * --------
 *  MAUI
...
VAPP_SCREEN_LOCK_SYS_PUBLISH_FACTORY(
    ...
    VappScreenLockNativeFactoryPublisher);

VAPP_SCREEN_LOCK_FACTORY_IMPLEMENT_CLASS(VappScreenLockNativeFactory);


VappScreenLockNativeFactory::ConfigEntry
    VappScreenLockNativeFactory::s_table[VappScreenLockNativeFactory::MAX_CONFIG_SIZE];

VfxS32 VappScreenLockNativeFactory::s_count = 0;


VappScreenLockNativeFactory::VappScreenLockNativeFactory()
{
    for (VfxS32 i = 0; i < MAX_CONFIG_SIZE; i++)
    {
        s_table[i].m_type = VAPP_SCREEN_LOCK_TYPE_INVALID;
        s_table[i].m_create = NULL;
    }
}


void VappScreenLockNativeFactory::add(
    VappScreenLockTypeEnum type,
...
VfxS32 VappScreenLockNativeFactory::getAllId(
    VappScreenLockId **id,
    VfxS32 size) const
{
    ...
    {
        id[i]->assignWith(VAPP_SCREEN_LOCK_SRC_NATIVE, s_table[i].m_type);
    }
...
VappScreenLock *VappScreenLockNativeFactory::createScreenLock(
