 ...
 *---------------------------------------------------------------------------*/
    void onStateChanged(VcpImageButton* img_bt, VcpImageButtonStateEnum state);

/*---------------------------------------------------------------------------*
 * Function: setDisplayFocus()
 * Date:Feb/28/2012
 * Description: set display focus.
 *    
 * Parameters
 *    @flag.
 *---------------------------------------------------------------------------*/
    void setDisplayFocus(VfxBool flag);

/*---------------------------------------------------------------------------*
 * Function: isDisplayFocus()
 * Date:Feb/28/2012
 * Description: check focus state.
 *    
 * Parameters
 * 
 *---------------------------------------------------------------------------*/
    VfxBool isDisplayFocus();
 ...
 * Date:Feb/28/2012
 * Description: set focus index.
 *    
 * Parameters
 *    @index.
 *---------------------------------------------------------------------------*/
    void setFocusIndex(VfxS32 idx);

/*---------------------------------------------------------------------------*
 * Function: moveFocus()
 * Date:Feb/28/2012
 * Description: move focus index.
 *    
 * Parameters
 *    @key_code.
 *---------------------------------------------------------------------------*/
    void moveFocus(VfxKeyCodeEnum key_code);

/*---------------------------------------------------------------------------*
 * Function: getFocusIndex()
 * Date:Feb/28/2012
 * Description: get the LSK string by state.
 *    
 * Parameters
 * 
 *---------------------------------------------------------------------------*/
    VfxS32 getLSKStringID();

/*---------------------------------------------------------------------------*
 * Function: changeHighlightItemState()
 * Date:Feb/28/2012
...
private:
	enum
    {
        ...
        FOCUS_GAP_Y1 = 8,
        FOCUS_GAP_Y2 = 4,
        FOCUS_GAP_SEPATATOR = 3,
        FOCUS_X = FOCUS_GAP_X1 + 1,
        FOCUS_Y = FOCUS_GAP_Y1,
        FOCUS_W = (LCD_WIDTH - FOCUS_GAP_X1 - FOCUS_GAP_X2 - (FOCUS_GAP_SEPATATOR * (VAPP_QSETTING_TOTAL - 2))) / (VAPP_QSETTING_TOTAL - 1),
        FOCUS_H = BG_IMGE_HEIGHT - FOCUS_GAP_Y1 - FOCUS_GAP_Y2
#elif defined(__MMI_MAINLCD_320X480__)//HVGA
        BG_IMG_POS_X = 1,
        BG_IMG_POS_Y = 4,
        BG_IMGE_WIDTH = 1,
        ...
        FOCUS_GAP_Y1 = 8,
        FOCUS_GAP_Y2 = 5,
        FOCUS_GAP_SEPATATOR = 4,
        FOCUS_X = FOCUS_GAP_X1 + 1,
        FOCUS_Y = FOCUS_GAP_Y1 - 1,
        FOCUS_W = (LCD_WIDTH - FOCUS_GAP_X1 - FOCUS_GAP_X2 - (FOCUS_GAP_SEPATATOR * (VAPP_QSETTING_TOTAL - 2))) / (VAPP_QSETTING_TOTAL - 1),
        FOCUS_H = BG_IMGE_HEIGHT - FOCUS_GAP_Y1 - FOCUS_GAP_Y2
#elif defined(__MMI_MAINLCD_240X400__)//WQVGA
        BG_IMG_POS_X = 1,
    ...
    VfxImageFrame* m_focusImg;
    VfxBool m_display_focus;
    VfxS32 m_focus_idx;
