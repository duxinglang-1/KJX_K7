 ...
 *****************************************************************************/
#define VTST_RT_SAMPLE_MENU_2_CAMERA_DISTANCE       (440)
#define VTST_RT_SAMPLE_MENU_2_CAMERA_NEARPLANE      (240)
#define VTST_RT_SAMPLE_MENU_2_CELL_WIDTH            (100)
#define VTST_RT_SAMPLE_MENU_2_CELL_HEIGHT           (200)
#define VTST_RT_SAMPLE_MENU_2_SEMIMAJOR             (400)
#define VTST_RT_SAMPLE_MENU_2_SEMIMINOR             (200)
#define VTST_RT_SAMPLE_MENU_2_FOCUSED_ANGLE         (60)
#define VTST_RT_SAMPLE_MENU_2_ANGLE                 (45)
#define VTST_RT_SAMPLE_MENU_2_SHIFT_X               (-250)
...
class VtstRtSampleMenu2 : public VcpWheelMenu
{
    typedef VtstRtSampleMenu2       Self;
    ...
    public:
        ...
        VtstRtSampleMenu2* getMenu()
        {
            if (getParentFrame())
                return (VtstRtSampleMenu2*) getParentFrame();
            return NULL;
        }
        ...
        virtual void onInit()
        {
            ...
            setRect(VfxRect(0, 0, VTST_RT_SAMPLE_MENU_2_CELL_WIDTH, VTST_RT_SAMPLE_MENU_2_CELL_HEIGHT));
            ...
            /*
            VfxImage *m_image1;
            VFX_OBJ_CREATE(m_image1, VfxImage, this);
            m_image1->setSrc(VFX_WSTR("E:\\page\\test4100x200.bmp"));
            setContent(m_image1);
            */
        }
...
public:
    ...
    virtual void onTap(VfxPoint pt)
    {
        ...
        VfxFPoint P0,P1,P2,P3;
        // set vertex of rectangle. calculate from modis
        P0.x = 16.9725f;
        P0.y = 35.1112f;
        ...
        {
            // Line 1 fucntion
            VfxFloat up = ((P1.y - P0.y)/(P1.x - P0.x)) * (pt.x - P0.x) + P0.y;
            VfxFloat down = ((P3.y - P2.y)/(P3.x - P2.x)) * (pt.x - P2.x) + P2.y;
    ...
    )
    {
        ...
        VfxFloat a = VTST_RT_SAMPLE_MENU_2_SEMIMAJOR;
    	VfxFloat b = VTST_RT_SAMPLE_MENU_2_SEMIMINOR;
        
        // define the theta of ellipse parametric equation
        // the focus item is on the 60 degree of ellipse.
    	VfxFloat theta = VTST_RT_SAMPLE_MENU_2_FOCUSED_ANGLE + VTST_RT_SAMPLE_MENU_2_ANGLE * offset;
    	VfxFloat norm; 
        ...
        cameraView.setCamera(
            ...
            (VfxFloat)VTST_RT_SAMPLE_MENU_2_CAMERA_NEARPLANE, 
            (VfxFloat)VTST_RT_SAMPLE_MENU_2_CAMERA_DISTANCE, 
            0, 
            0, 
            0);
        ...
        modelView.setTranslation((VfxFloat)((-1 * VTST_RT_SAMPLE_MENU_2_CELL_WIDTH/2)), 
                            (VfxFloat)(-1 * VTST_RT_SAMPLE_MENU_2_CELL_HEIGHT/2), 
                            0);
        ...
        norm = (VfxFloat)sqrt(a * a * vfxSin(VFX_DEG_TO_RAD(theta)) * vfxSin(VFX_DEG_TO_RAD(theta)) + b * b * vfxCos(VFX_DEG_TO_RAD(theta)) * vfxCos(VFX_DEG_TO_RAD(theta)));
        
        RT.m[0] = a * vfxSin(VFX_DEG_TO_RAD(theta))/norm;
    	RT.m[2] = b * vfxCos(VFX_DEG_TO_RAD(theta))/norm;
    	RT.m[8] =  RT.m[2];
    	RT.m[10] = -1 * RT.m[0];
    	...
    	RT.m[3] = a * vfxCos(VFX_DEG_TO_RAD(theta)) + VTST_RT_SAMPLE_MENU_2_SHIFT_X;
    	RT.m[11] = -1 * b * vfxSin(VFX_DEG_TO_RAD(theta));
    ...
    {
        return &VtstRtSampleMenu2::myFrameEffectCB;
    }
...
class VtstRtSampleMenu2Scr : public VtstRtScr
{
public:
    typedef VtstRtSampleMenu2Scr Self;
    typedef VtstRtScr   Super;

// Variable
public:
    VtstRtSampleMenu2 *m_menu;

// Constructor / Destructor
public:
    VtstRtSampleMenu2Scr() : m_menu(NULL) {}
...
protected:
    ...
    {
        VtstRtScr::onInit();
        setTitle(VFX_WSTR("Sample Menu 2 Test"));

        // scrollable body
        VFX_OBJ_CREATE(m_menu, VtstRtSampleMenu2, this);
...
VtstTestResultEnum vtst_rt_sample_menu_2(VfxU32 param)
{
    VFX_UNUSED(param);
    VTST_START_SCRN(VtstRtSampleMenu2Scr);
    return VTST_TR_OK;
}
