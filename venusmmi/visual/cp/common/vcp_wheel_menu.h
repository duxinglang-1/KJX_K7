...
#ifndef _VCP_WHEEL_MENU_H_
#define _VCP_WHEEL_MENU_H_
...
// Define invalid menu item index
#define VCP_WHEELMENU_INVALID_INDEX -1

// A macro help to design whether this index is valid
#define VCP_WHEELMENU_IS_VALID_INDEX(_a) (_a >= 0)

// The value that cache will increase its size at time
#define VCP_MENUCACHE_ADDITION 16

struct VcpWheelMenuFrameData;

enum VcpWheelMenuState
{
    VCP_WHEEL_MENU_NORMAL_STATE,
    VCP_WHEEL_MENU_ENTER_STATE,
    VCP_WHEEL_MENU_LEAVE_STATE,
    VCP_WHEEL_MENU_END_STATE
};

enum VcpWheelMenuOrder
{
    VCP_WHEEL_MENU_INCREASE,
    VCP_WHEEL_MENU_DECREASE,
    VCP_WHEEL_MENU_FOCUS_FISRT,
    VCP_WHEEL_MENU_FOCUS_LAST,
    VCP_WHEEL_MENU_ALL_TOGETHER,
    VCP_WHEEL_MENU_END
};

// This function pointer is used to callback during frame drawing.
// In each frame drawing, we will callback for each cell,
// cell can adjust its property according to fPos.
// RETURNES: VRT_TRUE if you modify any property.
typedef vrt_render_dirty_type_enum (*VcpWheelMenuFrameEffectCBFuncPtr)(
...
/***************************************************************************** 
 * VcpWheelMenuCache
 *  VcpMenuCache is dictionary for resource manager. It can map a index to a frame.
 *****************************************************************************/
class VcpWheelMenuCache : public VfxBase
{
// Declaration
public:
    ...
    public:
        Key                         key;
        VfxFrameWeakPtr             ptr;
        VfxFloatTimelineWeakPtr     timelinePtr;
    };
...
public:
    ...
    VfxFrame* getItem(Key p);
    
    // <group DOM_WheelMenu_VcpWheelMenuCache_Method>
    // Set a item to cahce.
    void setItem(Key p, VfxFrame* frame);
    ...
    // <group DOM_WheelMenu_VcpWheelMenuCache_Method>
    // Assing a tag to an item.
    void setItemTimeline(Key p, VfxFloatTimeline *timeline);

    // <group DOM_WheelMenu_VcpWheelMenuCache_Method>
    // Get a tag from an item.
    VfxFloatTimeline* getItemTimeline(Key p);
...
protected:
    ...
    // <group DOM_WheelMenu_VcpWheelMenuCache_Variable>
    VcpWheelMenu    *m_menu;            // The menu object.
...
/***************************************************************************** 
 * Class VcpWheelMenu
 *  VcpWheelMenu provides a basic framework for venus wheel menu.
 *  You should inherit it and provides your layouter.
 *****************************************************************************/
class VcpWheelMenu : public VfxControl
{
    VFX_DECLARE_CLASS(VcpWheelMenu);

    // Here I typedef some useful type, so that I can coding intuitively.
    typedef VcpWheelMenu            Self;
    typedef VfxControl              Super;
    typedef VcpWheelMenuCache       Cache;
    typedef VcpWheelMenuCache::Key  Key;
...
public:
    ...
    void setCellUserData(
        VfxS32 cell,        // [IN] The cell index
        VfxS32 subCell,     // [IN] The sub cell index
        VfxFrame* frame,    // [IN] The cell frame pointer
        void* data,         // [IN] The user data pointer
        VfxU32 dataSize     // [IN] The user data size
    );
    ...
    void pushInSubCell(
        VfxFrame *frame,                        // [IN] The frame be put in
        VfxS32 cell,                            // [IN] The cell index
        VfxS32 subCell                          // [IN] The sub cell index
    );
    ...
    // <group DOM_WheelMenu_VcpWheelMenu_Method>
    // configure enter/leave state parameters
    void configureStatePara(
        VcpWheelMenuState state,                // [IN] The state to be configured 
        VfxMsec durationPerCell,                // [IN] duration of each cell
        VfxMsec delay,                          // [IN] delay between each cell
        VcpWheelMenuOrder order                 // [IN] the order of cell
        );
    // <group DOM_WheelMenu_VcpWheelMenu_Method>
    // start to change state to target state
    void changeState(
        VcpWheelMenuState state                 // [IN] The state to be configured 
        );
#endif

	void setDefaultAnimTime(VfxMsec time)
	{
		m_defaultAnimTime = time;
	}

	VfxMsec getDefaultAnimTime()
	{
		return m_defaultAnimTime;
	}
...
protected:  
    ...
    virtual void handleTap(VfxPoint pt);
    // <group DOM_WheelMenu_VcpWheelMenu_Overridable>
...
protected:
    ...
    // <group DOM_WheelMenu_VcpWheelMenu_Variable>
    VcpWheelMenuCache   m_cache;            // The cache pool object.
    // <group DOM_WheelMenu_VcpWheelMenu_Variable>
    ...
    // <group DOM_WheelMenu_VcpWheelMenu_Variable>
    VcpWheelMenuState   m_state;            // state: normal, enter, or leave;
    VfxMsec             m_durationPerCell[2];                
    VfxMsec             m_delay[2];                          
    VcpWheelMenuOrder   m_order[2]; 
    // <group DOM_WheelMenu_VcpWheelMenu_Variable>
...
protected:
    ...
    // <group DOM_WheelMenu_VcpWheelMenu_Implementation>
    friend class VcpWheelMenuCache;
...
private:
    ...
    // <group DOM_WheelMenu_VcpWheelMenu_Implementation>
    const VcpWheelMenuCellHeader* getCellHeader(Key key);

    // Set the cell header
    // <group DOM_WheelMenu_VcpWheelMenu_Implementation>
    void setCellHeader(Key key, VcpWheelMenuCellHeader* header);

#if !defined(__MMI_VUI_COSMOS_CP_SLIM__)
    // <group DOM_WheelMenu_VcpWheelMenu_Implementation>
    void onEndofChangeTimer(VfxTimer *source);

    // <group DOM_WheelMenu_VcpWheelMenu_Implementation>
    void changeFPETriggerMethod(vrt_frame_property_effect_trigger_type_enum value);
   
    // <group DOM_WheelMenu_VcpWheelMenu_Implementation>
    VfxS32 startCellFilterTimeline(VfxS32 cell, VfxMsec duration, VfxMsec delay);
#endif
...
protected:
    ...
    friend class VcpWheelMenuCache;
    // <group DOM_WheelMenu_VcpWheelMenu_Implementation>
    void closeCell(VfxS32 cell, VfxS32 subCell, VfxFrame* frame);
    // <group DOM_WheelMenu_VcpWheelMenu_Implementation>
    void closeCell(VfxS32 cell);
};

#endif // _VCP_WHEEL_MENU_H_
