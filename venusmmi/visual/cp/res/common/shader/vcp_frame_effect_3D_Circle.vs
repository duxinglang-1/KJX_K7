...
uniform float v3d_FilterTiming;
...
{    
    ...
    if (v3d_FilterTiming < u_startTime)
    {
        x = 0.0;
    }
    else if (v3d_FilterTiming > u_startTime && v3d_FilterTiming < u_endTime)
    {
        x = (v3d_FilterTiming - u_startTime)/(u_endTime - u_startTime);
    }
    else
    {
        x = 1.0;
    ...
    vec4 tempCenter = vec4(size.x / 2.0, size.y / 2.0, size.x / 2.0, 1);
