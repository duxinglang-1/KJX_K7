...
void main()
{
  ...
  float imy = float(v3d_BaseTexSize.y);
	...
	vec2 anchor = vec2(float(v3d_Anchor.x) + imx, size.y - float(v3d_Anchor.y));	
	pos.xy -= anchor; 
	...
	AA = -A;
	R = sqrt((pos.x) * (pos.x) + (pos.y - AA) * (pos.y - AA));
	r = R * sin_theta;
	beta = asin( (pos.x) / R) / sin_theta;
	sin_beta = sin(beta);
	cos_beta = cos(beta);
	pos.x  = r * sin_beta;
	pos.y  = AA - R + r * (1.0 - cos_beta) * sin_theta;
	pos.z  = r * (1.0 - cos_beta) * cos_theta;
	
	
	u = normalize(vec3(0.0, - R + r*sin_theta, r*cos_theta));
	center = R / cos_theta * u + vec3(0.0, AA, 0.0);
	...
	pos.x  = (pos.x * cos_rho - pos.z * sin_rho);	
	pos.z  = (pos.x * sin_rho + pos.z * cos_rho);
	center.x  = (center.x * cos_rho - center.z * sin_rho);	
	center.z  = (center.x * sin_rho + center.z * cos_rho);		

	lightpos.xy = vec2(-imx / 2.0, 0); 		
