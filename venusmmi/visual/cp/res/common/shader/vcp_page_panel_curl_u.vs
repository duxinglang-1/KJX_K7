...
void main()
{
  ...
  float imy = float(v3d_BaseTexSize.y);
	...
	vec2 anchor = vec2(v3d_Anchor.x, size.y - float(v3d_Anchor.y));	
	pos.xy -= anchor; 
	...
	R = sqrt((pos.x-A) * (pos.x-A) + (pos.y) * (pos.y));
	r = R * sin_theta;
	beta = asin( (pos.y) / R) / sin_theta;
	sin_beta = sin(beta);
	cos_beta = cos(beta);
	pos.y  = r * sin_beta ;
	pos.x  = R + A - r * (1.0 - cos_beta) * sin_theta;
	pos.z  = r * (1.0 - cos_beta) * cos_theta;
	
	u = normalize(vec3(R - r*sin_theta,0.0,r*cos_theta));
	center = R / cos_theta * u + vec3(A, 0.0 , 0.0);
