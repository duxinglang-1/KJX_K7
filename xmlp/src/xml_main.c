...
kal_bool xml_init_folder(void)
{
    ...
    if (fh >= FS_NO_ERROR)
    {
        kal_wap_trace(MOD_MMI, XML_TRACE_INFO, "[XML] xml folder exsit");
        FS_Close(fh);
    }
    else
    {
        ...
        if (ret < 0)
        {
            kal_wap_trace(MOD_MMI, XML_TRACE_INFO, "[XML] xml creat folder failed");
            return KAL_FALSE;
        }
...
kal_int32 xml_parse(XML_PARSER_STRUCT *parser, kal_wchar *file_name)
{
	...
	kal_wap_trace(MOD_MMI, XML_TRACE_INFO, "we have finished prolog parse!\r\n");
	kal_wap_trace(MOD_MMI, XML_TRACE_INFO, "the result is %d", result);
    /* 2. parse element */
...
kal_int32 xml_parse_file_from_offset(XML_PARSER_STRUCT *parser, kal_int32 file_handle)
{
    ...
    fileptr = &(parser->virfileptr);
    fileptr->realfilebeginpos = parser->pos;
...
static kal_int32 xml_parse_prolog(XML_PARSER_STRUCT *parser)
{
    ...
    kal_wap_trace(MOD_MMI, XML_TRACE_INFO, "we have get buffer for parser!\r\n");
    while ((parser->stop == 0)  &&
           (parser->pause == 0) && 
	...
	kal_wap_trace(MOD_MMI, XML_TRACE_INFO, "the length of data read is %d", len);
